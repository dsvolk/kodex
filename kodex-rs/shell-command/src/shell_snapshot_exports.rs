...
pub(super) fn script(shell_type: ShellType) -> String {
    let script = match shell_type {
        ShellType::Bash => {
            r#"(
  while IFS= read -r __kodex_snapshot_export_name; do
    case "$__kodex_snapshot_export_name" in
      ""|[0-9]*|*[!A-Za-z0-9_]*|PWD|OLDPWD) continue ;;
    esac
    RECORD_START
    declare -xp "$__kodex_snapshot_export_name" 2>/dev/null || true
    RECORD_END
  done < <(compgen -e)
)
"#
  ...
  for __kodex_snapshot_export_name in ${(f)"$(typeset +x)"}; do
    case "$__kodex_snapshot_export_name" in
      ""|[0-9]*|*[!A-Za-z0-9_]*|PWD|OLDPWD) continue ;;
    esac
    case "${(tP)__kodex_snapshot_export_name}" in
      *readonly*) continue ;;
      *export*) ;;
      *) continue ;;
    esac
    RECORD_START
    typeset -xp "$__kodex_snapshot_export_name"
    RECORD_END
  done
)
"#
...
export -p | __kodex_snapshot_command awk '
/^(export|declare -x|typeset -x) [A-Za-z_][A-Za-z0-9_]*$/ {
  ...
  if eval '[ "${'"$__kodex_snapshot_export_name"'+x}" = x ]'; then
    continue
  fi
  # Only preserve an unset export if the native shell confirms it already exists.
  if (
    set +a
    set -- "$__kodex_snapshot_export_name" "$(export -p)"
    export "$1"
    [ "$2" = "$(export -p)" ]
  ); then
    RECORD_START
    printf 'export %s\n' "$__kodex_snapshot_export_name"
    RECORD_END
  fi
done
fi
"#
    ...
    script
        .replace(
            "RECORD_START",
            "printf '%s\\0' \"$__kodex_snapshot_export_name\"",
        )
