...
pub(crate) fn identity(
    ...
    let mut api_provider = provider_info.to_api_provider(auth.map(KodexAuth::auth_mode))?;
    ...
    field(format!("{:?}", auth.map(KodexAuth::api_auth_mode)).as_bytes());
    ...
    let has_stable_account = auth.is_some_and(|auth| {
        matches!(
            auth,
            KodexAuth::Chatgpt(_) | KodexAuth::ChatgptAuthTokens(_) | KodexAuth::AgentIdentity(_)
