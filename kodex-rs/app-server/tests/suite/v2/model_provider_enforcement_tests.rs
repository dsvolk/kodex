...
async fn provider_requirement_changes_reject_inputs_to_existing_threads(
    ...
    MockResponsesConfig::new(gateway.uri())
        .enable_feature(kodex_features::Feature::Goals)
        .write(home.path())?;
...
"#,
    ...
    let id = server
        ...
        .await?;
    let _: kodex_app_server_protocol::ThreadGoalSetResponse = server.read_response(id).await?;
    ...
    let expected_error = JSONRPCErrorError {
        code: -32600,
        message: "failed to load configuration: Your organization's required model provider settings changed. Restart Kodex to apply them; this request was not sent".to_string(),
        data: None,
    };
    ...
    let id = server
        ...
        .await?;
    let goal: kodex_app_server_protocol::ThreadGoalGetResponse = server.read_response(id).await?;
    ...
    let id = server
        ...
        .await?;
    let _: kodex_app_server_protocol::ThreadGoalClearResponse = server.read_response(id).await?;
    let id = server
        .send_raw_request(
            "thread/queue/start",
            Some(json!({
    ...
    let id = server
        ...
        .await?;
    let _: kodex_app_server_protocol::ThreadQueueDeleteResponse = server.read_response(id).await?;
