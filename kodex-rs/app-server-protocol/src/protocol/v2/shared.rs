...
/// This translation layer make sure that we expose kodex error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `kodexErrorInfo` variant.
...
impl From<CoreKodexErrorInfo> for KodexErrorInfo {
    fn from(value: CoreKodexErrorInfo) -> Self {
        match value {
            CoreKodexErrorInfo::ContextWindowExceeded => KodexErrorInfo::ContextWindowExceeded,
            ...
            CoreKodexErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                KodexErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            ...
            CoreKodexErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                KodexErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreKodexErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                KodexErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreKodexErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                KodexErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreKodexErrorInfo::Other => KodexErrorInfo::Other,
        }
    }
}
