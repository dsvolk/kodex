...
from .async_client import AsyncKodexClient
from .client import KodexClient, KodexConfig
...
class Kodex:
    """Synchronous client for creating threads and running Kodex turns.

    The client starts its runtime connection during construction. Use it as a
    context manager so resources are closed promptly.
    """

    def __init__(self, config: KodexConfig | None = None) -> None:
        self._client = KodexClient(config=config)
        try:
            self._client.start()
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate Kodex with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current Kodex account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current Kodex account session."""
        self._client.account_logout()

    # BEGIN GENERATED: Kodex.flat_methods
    def thread_start(
        self,
        *,
    ...
    # END GENERATED: Kodex.flat_methods

    def models(self, *, include_hidden: bool = False) -> ModelListResponse:
        """List available models reported by Kodex.
...
class AsyncKodex:
    """Async mirror of :class:`Kodex`.

    Prefer ``async with AsyncKodex()`` so initialization and shutdown are
    explicit and paired. The async client initializes lazily on context entry
    or first awaited API use.
    """

    def __init__(self, config: KodexConfig | None = None) -> None:
        self._client = AsyncKodexClient(config=config)
        self._init: InitializeResponse | None = None
        self._initialized = False
        self._init_lock = asyncio.Lock()

    async def __aenter__(self) -> "AsyncKodex":
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncKodex is not initialized yet. Prefer `async with AsyncKodex()`; "
    ...
    # BEGIN GENERATED: AsyncKodex.flat_methods
    ...
    # END GENERATED: AsyncKodex.flat_methods
...
class AsyncThread:
    ...
    async def turn(
        ...
        *,
        ...
        """
        wire_input, tool_output = _to_wire_turn_input(input)
        await self._kodex._ensure_initialized()
        approval_policy, approvals_reviewer = _approval_mode_override_settings(approval_mode)
        params = TurnStartParams(
            thread_id=self.id,
            input=wire_input,
        ...
        turn, subscription = await self._kodex._client._start_turn(
            self.id, wire_input, params=params, for_handle=True
        )
        return AsyncTurnHandle(self._kodex, self.id, turn.turn.id, _subscription=subscription)
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        """Read this thread, optionally including its turn history."""
        await self._kodex._ensure_initialized()
        return await self._kodex._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._kodex._ensure_initialized()
        return await self._kodex._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._kodex._ensure_initialized()
        return await self._kodex._client.thread_compact(self.id)
...
class TurnHandle:
    ...
    def __init__(
        self, _client: KodexClient, thread_id: str, id: str, *, _subscription=None
    ) -> None:
...
class AsyncTurnHandle:
    ...
    def __init__(self, _kodex: AsyncKodex, thread_id: str, id: str, *, _subscription=None) -> None:
        self._kodex, self.thread_id, self.id = _kodex, thread_id, id
        if _subscription is None:
            self.__post_init__()
        else:
            self._subscription = _subscription
    ...
    def __post_init__(self) -> None:
        self._subscription = self._kodex._client._subscribe_turn_notifications(self.id)

    async def steer(self, input: Input | str) -> TurnSteerResponse:
        """Send additional user input to this active turn."""
        await self._kodex._ensure_initialized()
        return await self._kodex._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        """Request interruption of this active turn."""
        await self._kodex._ensure_initialized()
        return await self._kodex._client.turn_interrupt(self.thread_id, self.id)
