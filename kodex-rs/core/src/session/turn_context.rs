...
use kodex_core_plugins::ResolvedPluginMetricsOperation;
use kodex_core_plugins::TrustedPluginRoots;
...
use kodex_protocol::permissions::RawFileSystemSandboxPolicy;
...
use kodex_sandboxing::policy_transforms::effective_permission_profile;
...
impl TurnEnvironment {
    ...
    pub(crate) fn windows_sandbox_selection_for_turn_metadata(
        &self,
    ) -> kodex_file_system::WindowsSandboxSelection {
        let config = self.config();
...
pub struct TurnContext {
    ...
    pub(crate) extension_data: Arc<kodex_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(super) fn active_plugin_ids_for_telemetry(&self) -> Option<Vec<String>> {
        ...
        for identity in identities {
            let id = match identity.remote_plugin_id {
                Some(remote_id) => {
                    if !kodex_core_plugins::remote::is_valid_remote_plugin_id(&remote_id) {
                        return None;
                    }
                    remote_id
                }
                None => {
                    if kodex_plugin::PluginId::parse(&identity.plugin_id).is_err() {
                        return None;
                    }
                    identity.plugin_id
                }
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_kodex_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_kodex_backend);
        self.config
            .features
            .apps_enabled_for_auth(uses_kodex_backend)
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let extension_data = Arc::new(kodex_extension_api::ExtensionData::new(sub_id.clone()));
    ...
    ) -> KodexResult<(Arc<TurnContext>, ThreadSettingsSnapshot)> {
    ...
    pub(super) async fn new_turn_with_sub_id_if(
    ...
    ) -> KodexResult<Option<(Arc<TurnContext>, ThreadSettingsSnapshot)>> {
        ...
        let commit = match self.update_settings_if(updates, should_start).await {
            ...
            Err(error) => {
                let message = KodexErr::InvalidRequest(error.to_string()).to_string();
                ...
                .await;
                return Err(KodexErr::InvalidRequest(message));
            }
        };
    ...
    async fn new_turn_context_from_configuration(
        ...
        let trusted_plugin_roots = TrustedPluginRoots::from_plugin_load_outcome(
            &plugin_outcome,
            per_turn_config.kodex_home.as_path(),
        );
        ...
        if turn_context.config.model_provider_id == kodex_model_provider_info::OPENAI_PROVIDER_ID {
    ...
    pub(crate) async fn maybe_emit_model_warnings_for_turn(&self, tc: &TurnContext) {
        ...
        if !tc.code_mode_available
            && matches!(
                ...
                kodex_protocol::openai_models::ToolMode::CodeMode
                    | kodex_protocol::openai_models::ToolMode::CodeModeOnly
            )
