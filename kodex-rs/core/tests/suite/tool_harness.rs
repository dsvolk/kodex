...
async fn exec_command_tool_executes_command_and_streams_output() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex().with_model("test-gpt-5-kodex");
    let TestKodex {
        kodex,
        cwd,
        session_configured,
...
async fn update_plan_tool_emits_plan_update_event() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex().with_config(|config| config.update_plan_enabled = true);
    let TestKodex {
        kodex,
        cwd,
        session_configured,
    ...
    wait_for_event(&kodex, |event| match event {
        ...
        EventMsg::ItemCompleted(completed) => {
            if let TurnItem::FileChange(item) = &completed.item {
                ...
                assert_eq!(
                    item.status,
                    Some(kodex_protocol::protocol::PatchApplyStatus::Completed)
                );
            }
            false
        }
