...
use kodex_extension_api::McpToolResultInput;
...
impl AuthFailureResponder {
    fn result(self) -> Value {
        ...
        if self.scenario == Scenario::AuthMetadataRemoved {
            response["_meta"] = json!({"_kodex_apps": {"connector_auth_failure": {
...
"#,
    ...
    let submitted = test.kodex.start_or_steer_turn(request).await?;
            ...
            match event.msg {
                ...
                message @ (EventMsg::ItemStarted(_) | EventMsg::ItemCompleted(_)) => {
                        ...
                        && item.id == CALL_ID
                    {
                        ...
                        assert_eq!(item.server, KODEX_APPS_MCP_SERVER_NAME);
                ...
                EventMsg::ElicitationRequest(request) => {
                    ...
                    let action = match &request.request {
                        ...
                        ElicitationRequest::Url { .. } => {
                            ...
                            assert_eq!(
                                request.id,
                                kodex_protocol::mcp::RequestId::String(format!(
                                    "kodex_apps_auth_{CALL_ID}"
                                ))
                            );
    ...
    let expected_auth_request = if matches!(scenario, Scenario::DefaultAuth | Scenario::ModernAuth)
    {
        Some(ElicitationRequest::Url {
            ...
            elicitation_id: format!("kodex_apps_auth_{CALL_ID}"),
        })
    } else {
        None
    };
    ...
    let app_used_events = events
        .iter()
        .filter(|event| {
            event["event_type"] == "kodex_app_used"
...
"#
    ...
    let child = test
        ...
        .await?
        .thread;
    wait_for_mcp_server(&child, KODEX_APPS_MCP_SERVER_NAME).await?;
