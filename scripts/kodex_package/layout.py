"""Canonical Kodex package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "kodex-resources"
    path_dir = package_dir / "kodex-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"kodex-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.kodex_command_runner_bin is not None:
        copy_executable(
            inputs.kodex_command_runner_bin,
            resources_dir / "kodex-command-runner.exe",
            is_windows=True,
        )

    if inputs.kodex_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.kodex_windows_sandbox_setup_bin,
            resources_dir / "kodex-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    write_json(package_dir / "kodex-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "kodex-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: kodex-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"kodex-code-mode-host{spec.exe_suffix}",
        Path("kodex-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("kodex-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("kodex-resources") / "bwrap")
        executable_files.append(Path("kodex-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("kodex-resources") / "kodex-command-runner.exe",
                Path("kodex-resources") / "kodex-windows-sandbox-setup.exe",
            ]
        )
