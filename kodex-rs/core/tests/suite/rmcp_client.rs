...
use kodex_core::EnvironmentConfig;
use kodex_core::EnvironmentMcpPolicy;
...
use kodex_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use core_test_support::test_kodex::test_env;
...
use kodex_protocol::mcp_policy::McpServerIdentity;
use kodex_protocol::mcp_policy::McpServerRequirement;
use kodex_protocol::mcp_policy::PluginMcpRequirements;
...
use kodex_protocol::openai_models::ConfigShellToolType;
use kodex_protocol::openai_models::ConfirmationPolicies;
use kodex_protocol::openai_models::InputModality;
use kodex_protocol::openai_models::ModelInfo;
use kodex_protocol::openai_models::ModelVisibility;
...
use kodex_protocol::protocol::McpStartupStatus;
...
use kodex_protocol::protocol::TurnSettingsUpdate;
use kodex_protocol::protocol::TurnSettingsUpdateOutcome;
...
fn read_only_user_turn(fixture: &TestKodex, text: impl Into<String>) -> TurnInputRequest {
...
fn auto_approved_user_turn(fixture: &TestKodex, text: impl Into<String>) -> TurnInputRequest {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        kodex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/kodex-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.kodex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.kodex, |ev| {
    ...
    wait_for_event(&fixture.kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_kodex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.kodex, "bounded").await?;
...
async fn text_only_mcp_content_uses_content_items() -> anyhow::Result<()> {
    ...
    assert_eq!(
        serde_json::to_value(kodex_core::test_support::mcp_attribution_snapshot(
            &fixture.kodex
        ))?,
        json!({
            "status": "complete",
...
async fn environment_mcp_policy_filters_runtime_config_and_model_tools(
    ...
    let kodex_home = Arc::new(tempdir()?);
    let test_env = test_env().await?;
    if from_plugin {
        let plugin_root =
            super::plugins::write_sample_plugin_manifest_and_config(kodex_home.as_ref());
    ...
    fixture
        ...
        .await?;
    let (pending_config, _) = fixture.kodex.current_mcp_config_and_runtime_context().await;
    ...
    let (runtime_config, _) = fixture.kodex.current_mcp_config_and_runtime_context().await;
    ...
    fixture
        ...
        .await?;
    let (failed_config, _) = fixture.kodex.current_mcp_config_and_runtime_context().await;
...
async fn future_environment_mcp_policy_applies_on_the_next_turn() -> anyhow::Result<()> {
    ...
    let fixture = test_kodex()
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.kodex, "environment_policy").await?;
    let selection = fixture.kodex.environment_selections().await.remove(0);
    ...
    submit_thread_settings(&fixture.kodex, settings(config.clone())).await?;
    fixture
        .kodex
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&fixture.kodex, |event| {
    ...
    submit_thread_settings(&fixture.kodex, settings(config)).await?;
    fixture
        .kodex
...
async fn stdio_server_round_trip(server_name: &'static str, namespace: &str) -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.kodex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_kodex()
        .with_pre_build_hook(move |kodex_home| {
            fs::write(
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.kodex, "history").await?;
...
-> anyhow::Result<()> {
    ...
    let startup = loop {
        let event = fixture.kodex.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let event = fixture
                ...
                .next_event()
                .await
                .context("event stream ended before Kodex Apps became ready")?;
            if let EventMsg::McpStartupUpdate(update) = event.msg
                && update.server == KODEX_APPS_MCP_SERVER_NAME
    ...
    .await
    .context("Kodex Apps should finish starting before the first turn")??;
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.kodex.shutdown_and_wait())
        .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.kodex.start_or_steer_turn(input).await?;
    let turn_id = core_test_support::wait_for_event_match(&fixture.kodex, |event| match event {
    ...
    fixture.kodex.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.kodex, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    let namespace = format!("mcp__{server_name}");
    let mut models = kodex_models_manager::bundled_models_response()?;
    let model = models
        .models
        .iter_mut()
    ...
    fixture
        ...
        .list_models(
            ...
            kodex_core::test_support::default_http_client_factory(),
        )
        .await;
    ...
    let response_metadata: Value = serde_json::from_str(
        initial_request["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
    ...
    assert_eq!(
        output_json.pointer("/x-kodex-turn-metadata/node_repl_auto_review_required"),
        Some(&json!(node_repl_auto_review_required))
    );
    assert_eq!(
        output_json.pointer("/x-kodex-turn-metadata/node_repl_disabled"),
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            kodex_linux_sandbox_exe: fixture.config.kodex_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.kodex, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: kodex_protocol::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "KODEX_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Kodex should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Kodex can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip(mode: HeadersHelperMode) -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Kodex will call the MCP echo tool
    ...
    // Phase 3: configure Kodex with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Kodex begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.kodex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_kodex()
        .with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.kodex, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    let untrusted_mcp_url = format!("{}/api/kodex/ps/mcp", untrusted_apps.chatgpt_base_url);
    ...
    wait_for_mcp_server(&fixture.kodex, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/kodex/ps/mcp")
        .filter_map(|request| {
...
/// This test writes to a fallback credentials file in KODEX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire KODEX_HOME through the code.
#[test]
#[serial(kodex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Kodex will call the OAuth-backed
    ...
    // Phase 3: seed an isolated KODEX_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _kodex_home_guard = EnvVarGuard::set("KODEX_HOME", temp_home.path().as_os_str());
    let unset_authorization_env_var = format!(
        "KODEX_TEST_UNSET_MCP_OAUTH_AUTHORIZATION_{}",
        std::process::id()
    );
    ...
    // Phase 4: configure Kodex with the OAuth-backed Streamable HTTP MCP
    ...
    let recovery_hint = if credential_config.is_local_environment() {
        format!("Run `kodex mcp login {server_name}`.")
    } else {
    ...
    let startup = wait_for_event(&fixture.kodex, |event| {
    ...
    let (contended_turn, refreshed_failure, refreshed_failed_servers) =
        tokio::time::timeout(Duration::from_secs(5), async {
            ...
            {
                match fixture.kodex.next_event().await?.msg {
    ...
    let discovered_turn = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        fixture
            ...
            .await?;
        loop {
            if let EventMsg::TurnComplete(turn) = fixture.kodex.next_event().await?.msg {
    ...
    tokio::time::timeout(
        ...
        wait_for_mcp_server(&fixture.kodex, discovered_server_name),
    )
    .await
    .context("the newly discovered OAuth server did not recover after its store was unlocked")??;

    assert!(
        kodex_rmcp_client::delete_oauth_tokens(
            discovered_credential_name.as_ref(),
            http_server.url(),
            OAuthCredentialsStoreMode::File,
            kodex_config::types::AuthKeyringBackendKind::default(),
        )
        .await?
    );
    ...
    // Phase 7: assert Kodex begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.kodex, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Kodex fixture.
...
async fn write_fallback_oauth_tokens(
    ...
    kodex_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        kodex_config::types::AuthKeyringBackendKind::default(),
