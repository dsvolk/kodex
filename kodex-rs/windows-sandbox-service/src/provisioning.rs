...
use kodex_windows_sandbox::run_elevated_provisioning_setup_with_retained_handles;
...
fn setup_is_complete(
    identity: &ClientIdentity,
    settings: &kodex_windows_sandbox::WindowsSandboxProvisioningSettings,
) -> Result<bool> {
    crate::package_lifecycle::with_owner_impersonation(identity.token.0, || {
        Ok(
            kodex_windows_sandbox::sandbox_setup_is_complete_with_settings(
                &identity.kodex_home,
                settings,
            ),
        )
...
pub(crate) fn run(
    ...
    let request = match request {
        ServiceRequest::RegisterInstallation { .. } => {
            let installation = InstallationRecord {
                kodex_home: identity.kodex_home,
    ...
    let helper = std::env::current_exe()
        ...
        .with_file_name("kodex-windows-sandbox-setup.exe");
