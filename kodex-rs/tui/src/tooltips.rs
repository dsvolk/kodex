...
use kodex_features::FeatureSpec;
...
const ANNOUNCEMENT_TIP_URL: &str =
    "https://raw.githubusercontent.com/openai/kodex/main/announcement_tip.toml";

const IS_MACOS: bool = cfg!(target_os = "macos");
const IS_WINDOWS: bool = cfg!(target_os = "windows");

const APP_TOOLTIP: &str = "Try the **Desktop app**. Run 'kodex app' or visit https://chatgpt.com/kodex?app-landing-page=true";
const MACOS_APP_TOOLTIP: &str =
    "Run `kodex app` to open the Desktop app (it installs on macOS if needed).";
...
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Desktop app**. Run 'kodex app' or visit https://chatgpt.com/kodex?app-landing-page=true";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Kodex.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Kodex is included in your plan for free – let’s build together.";
...
lazy_static! {
    ...
    static ref ALL_TOOLTIPS: Vec<&'static str> = {
        ...
        tips.extend(experimental_tooltips(
            FEATURES,
            kodex_realtime_webrtc::RealtimeWebrtcSession::is_supported,
        ));
        tips
    };
}
...
/// Pick a random tooltip to show to the user when starting Kodex.
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // Kodex currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
        ...
        pub(super) fn select(&self, plan: Option<PlanType>) -> Option<String> {
            ...
            self.0
                ...
                .find(|tip| {
                    ...
                    tip.version_matches(KODEX_CLI_VERSION)
...
mod tests {
    ...
    fn experimental_voice_tooltip_requires_runtime_support() {
        ...
        features
            ...
            .unwrap()
            .stage = kodex_features::Stage::Experimental {
            name: "Voice conversations",
            menu_description: "Talk with Kodex using /voice.",
            announcement: "NEW: Voice conversations can now be enabled from /experimental. Restart Kodex after enabling, then use /voice.",
        };
    ...
    fn desktop_app_tooltip_uses_supported_platform_launcher() {
        ...
        if linux_app_tooltip(LinuxDesktopSession::current()).is_some() {
            ...
            insta::assert_snapshot!(tooltip, @"Run `kodex app` to open the Desktop app (it installs on macOS if needed).");
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Kodex TUI.
...
[[announcements]]
content = "Welcome to Kodex! Check out the new onboarding flow."
