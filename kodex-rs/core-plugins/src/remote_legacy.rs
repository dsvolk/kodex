...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    url.query_pairs_mut().append_pair(
        "platform",
        product.unwrap_or(Product::Kodex).to_app_platform(),
    );
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_kodex_backend()) {
        request =
            request.headers(kodex_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_kodex_backend_auth(
    auth: Option<&KodexAuth>,
) -> Result<&KodexAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_kodex_backend_auth(auth)?;
