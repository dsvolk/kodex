...
async fn run_turn(test: &TestKodex, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestKodex, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_kodex_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestKodex> {
    let mut builder = test_kodex().with_model("test-gpt-5.1-kodex");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_kodex_with_test_tool(&server).await?;
