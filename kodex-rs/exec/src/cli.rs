...
#[command(
    version,
    override_usage = "kodex exec [OPTIONS] [PROMPT]\n       kodex exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Kodex outside a Git repository.
    ...
    /// Do not load `$KODEX_HOME/config.toml`; auth still uses `KODEX_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "kodex resume --last <prompt>" treats the positional as a prompt, not a session id.
