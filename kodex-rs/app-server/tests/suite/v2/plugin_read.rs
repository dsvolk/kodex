...
use kodex_app_server_protocol::AppMetadata;
...
use kodex_app_server_protocol::ScheduledTaskSchedule;
use kodex_app_server_protocol::ScheduledTaskSummary;
use kodex_app_server_protocol::ScheduledTaskWeekday;
use kodex_app_server_protocol::SkillInterface;
...
[plugins."demo-plugin@kodex-curated"]
...
"#
    ...
    write_installed_plugin(&kodex_home, "kodex-curated", "demo-plugin")?;
...
async fn plugin_read_returns_canonical_openai_curated_marketplace_name() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let repo_root = kodex_home.path().join(".tmp/plugins");
    write_plugin_marketplace(&repo_root, "openai-curated", "demo-plugin", "./demo-plugin")?;
    std::fs::create_dir_all(repo_root.join("demo-plugin/.kodex-plugin"))?;
    std::fs::write(
        repo_root.join("demo-plugin/.kodex-plugin/plugin.json"),
        r#"{
  "name": "demo-plugin",
...
"#,
    ...
    write_installed_plugin(&kodex_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.kodex-plugin"))?;
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.kodex-plugin/plugin.json"),
    ...
    write_plugin_share_local_path_mapping(kodex_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
        .and(path("/backend-api/ps/plugins/plugins_123"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&kodex_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "kodex-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(kodex_home.path().join(".tmp"))?;
    std::fs::write(
        kodex_home
            .path()
...
async fn plugin_read_agent_plugin_excludes_nested_skills() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![kodex_app_server_protocol::SkillSummary {
...
[hooks.state."demo-plugin@kodex-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&kodex_home, "kodex-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "kodex-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@kodex-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            kodex_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@kodex-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            kodex_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@kodex-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            kodex_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@kodex-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let kodex_home = TempDir::new()?;
    write_connectors_config(kodex_home.path(), &server.uri())?;
    write_chatgpt_auth(
        kodex_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(kodex_home: &TempDir) -> Result<()> {
    std::fs::write(
        kodex_home.path().join("config.toml"),
        r#"[features]
plugins = true
