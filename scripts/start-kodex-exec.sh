...
remote_path='~/code/kodex-sync'
local_exec_server_port="${KODEX_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${KODEX_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/kodex-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/kodex-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/kodex-sync"
remote_kodex_rs="$remote_repo_root/kodex-rs"

cd "${remote_kodex_rs}"
cargo build -p kodex-cli --bin kodex

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/kodex exec-server --listen ws://127.0.0.1:0 \
...
echo "Start kodex via: "
printf '  KODEX_EXEC_SERVER_URL=ws://127.0.0.1:%s kodex -C %q\n' \
