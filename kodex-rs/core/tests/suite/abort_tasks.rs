...
use kodex_core::SuspendTurnOutcome;
...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    let fixture = test_kodex()
        ...
        .await
        .unwrap();
    let kodex = Arc::clone(&fixture.kodex);
    ...
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    kodex.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
    kodex.submit(Op::CleanBackgroundTerminals).await.unwrap();
}
...
async fn root_turn_suspension_preserves_unfinished_turn_history() {
    ...
    let kodex_core::TurnInputSubmission::Started { turn_id } = submitted else {
    ...
    let items = rollout
        .lines()
        .map(|line| {
            kodex_rollout::parse_rollout_line(line)
    ...
    assert_eq!(
        resumed
            .kodex
            .recover_turn_if_idle(kodex_core::RecoverTurnRequest {
                turn_id: turn_id.clone(),
        ...
        kodex_core::StartIfIdleSubmission::Started {
            turn_id: turn_id.clone(),
        },
    );
    let completed = wait_for_event(&resumed.kodex, |event| {
