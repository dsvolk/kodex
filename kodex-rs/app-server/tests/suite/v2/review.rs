...
async fn review_start_rejects_detached_delivery_for_paginated_parent() -> Result<()> {
    ...
    create_config_toml(kodex_home.path(), &server.uri())?;
...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": kodex_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    create_config_toml(kodex_home.path(), &startup_provider.uri())?;
    let colliding_skill_dir = kodex_home.path().join("skills/review-agent-collision");
    ...
    let canonical_kodex_home = std::fs::canonicalize(kodex_home.path())?.try_into()?;
    let review_skill_path = system_cache_root_dir(&canonical_kodex_home)
        .join("review-agent")
        .join("SKILL.md");
...
fn create_config_toml(kodex_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
