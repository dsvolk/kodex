...
use http::Method;
use http::StatusCode;
use kodex_client::RetryOn;
use kodex_client::RetryPolicy;
    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "kodex_http_client::transport" {
        ...
        if event.metadata().target() != "kodex_otel.trace_safe" {
            return;
        }

        let mut visitor = RetryTelemetryVisitor::default();
        event.record(&mut visitor);
        if visitor.name.as_deref() != Some("kodex.retry") {
            return;
        }
...
async fn submit_user_input(test: &TestKodex, text: &str) -> Result<()> {
    test.kodex
...
async fn wait_for_turn_completion(test: &TestKodex) {
        ...
        loop {
            match wait_for_event(&test.kodex, |_| true).await {
                ...
                EventMsg::TurnComplete(event) => {
                    assert_eq!(
                        event.error.and_then(|error| error.kodex_error_info),
                        Some(KodexErrorInfo::ServerOverloaded)
                    );
                    break;
                }
    ...
    loop {
        match wait_for_event(&test.kodex, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.kodex_error_info,
                    Some(KodexErrorInfo::RateLimitExceeded)
                );
    ...
    loop {
        match wait_for_event(&test.kodex, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.kodex_error_info,
                    Some(KodexErrorInfo::ResponseTooManyFailedAttempts {
