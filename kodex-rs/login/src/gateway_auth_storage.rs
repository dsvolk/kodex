...
pub(super) async fn lock_credentials(kodex_home: &Path) -> io::Result<File> {
    let directory = kodex_home.join("secrets");
...
impl GatewayAuthStorage {
    pub(super) fn new(kodex_home: PathBuf, keyring: Arc<dyn KeyringStore>) -> Self {
