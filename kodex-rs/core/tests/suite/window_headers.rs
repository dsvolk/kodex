...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.kodex);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.kodex, "after resume").await?;
    shutdown_thread(&resumed.kodex).await?;

    let forked = resumed
        .thread_manager
        .fork_thread(
            /*snapshot*/ 0usize,
            kodex_core::StartThreadOptions::new(resumed.config.clone()),
            rollout_path,
        )
        .await?;
...
async fn submit_user_turn(kodex: &Arc<KodexThread>, text: &str) -> Result<()> {
    kodex
...
async fn submit_compact_turn(kodex: &Arc<KodexThread>) -> Result<()> {
    kodex.submit(Op::Compact).await?;
    let warning_event = wait_for_event(kodex, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(kodex: &Arc<KodexThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-kodex-window-id header");
