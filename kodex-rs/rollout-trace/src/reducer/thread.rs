...
impl TraceReducer {
    ...
    /// Starts a Kodex turn inside an existing thread.
    pub(super) fn start_kodex_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.kodex_turns.contains_key(&kodex_turn_id) {
            bail!("duplicate kodex turn start for {kodex_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.kodex_turns.insert(
            kodex_turn_id.clone(),
            KodexTurn {
                kodex_turn_id,
                thread_id,
    ...
    /// Marks a Kodex turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_kodex_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.kodex_turns.get(&kodex_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "kodex turn end for {kodex_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.kodex_turns.get_mut(&kodex_turn_id) else {
            bail!("kodex turn end referenced unknown turn {kodex_turn_id}");
        };
