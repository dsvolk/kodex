...
async fn unified_exec_zsh_fork_guardian_reviews_persistent_terminal_in_current_turn() -> Result<()>
{
    ...
    let first_completion = wait_for_event(&test.kodex, |event| {
...
async fn build_unified_exec_zsh_fork_test_or_skip<F>(
    ...
    pre_build_hook: F,
) -> Result<Option<(MockServer, TestKodex)>>
where
    F: FnOnce(&Path) + Send + 'static,
{
    ...
    impl kodex_extension_api::ApprovalReviewContributor for ExecveIdentityCheck {
        fn decide<'a>(
            &'a self,
...
async fn approve_expected_exec(test: &TestKodex, expected_command: &str) -> Result<()> {
    let approval = expect_exec_approval(test, expected_command).await;
    approve_exec(test, approval.effective_approval_id()).await
}

async fn approve_exec(test: &TestKodex, approval_id: String) -> Result<()> {
    test.kodex
