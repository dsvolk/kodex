...
async fn usage_notice_yields_to_interactions_and_blocking_banners() {
    ...
    for (spend_control, reached_type) in [
        ...
        (
            None,
            Some(kodex_app_server_protocol::RateLimitReachedType::RateLimitReached),
        ),
    ] {
...
async fn hints_render_clickable_links_and_release_them_when_disabled() -> Result<()> {
    ...
    let destination = "https://community.openai.com/c/kodex/37";
    select_catalog_tip(
        &mut app,
        /*width*/ 85,
        "Tip: Visit the Kodex community forum",
    );
    let mut tui = crate::tui::test_support::make_test_tui()?;
    tui.set_owned_screen(/*owned*/ true)?;
