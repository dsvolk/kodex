...
pub(crate) fn collab_spawn_error(err: KodexErr) -> FunctionCallError {
    match err.details() {
        KodexErrorDetails::UnsupportedOperation(message) if message == "thread manager dropped" => {
...
pub(crate) fn collab_agent_error(agent_id: ThreadId, err: KodexErr) -> FunctionCallError {
    match err.details() {
        KodexErrorDetails::ThreadNotFound(id) => {
...
pub(crate) fn collab_v2_agent_error(agent_id: ThreadId, err: KodexErr) -> FunctionCallError {
    match err.details() {
        KodexErrorDetails::UnsupportedOperation(message) if message != "thread manager dropped" => {
            FunctionCallError::RespondToModel(message.clone())
        }
