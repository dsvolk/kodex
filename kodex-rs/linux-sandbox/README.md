...
- a `kodex-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Kodex CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `kodex-exec` CLI can check if its arg0 is `kodex-linux-sandbox` and, if so, execute as if it were `kodex-linux-sandbox`
  - this should also be true of the `kodex` multitool CLI

On Linux, Kodex prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `kodex-resources/bwrap` binary shipped
with Kodex.
Kodex also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Kodex surfaces the same startup warning
...
it cannot create the required user namespaces, so Kodex rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `kodex-resources/bwrap` path.
- If `bwrap` is missing, Kodex also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Kodex surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Kodex rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.kodex`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `kodex sandbox`; the host OS selects the sandbox backend.
