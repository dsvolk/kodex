...
use kodex_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
...
struct ParentInstructionsProvider(kodex_extension_api::Instructions);

impl kodex_extension_api::UserInstructionsProvider for ParentInstructionsProvider {
    fn load_user_instructions(&self) -> kodex_extension_api::LoadInstructionsFuture<'_> {
        Box::pin(async move {
...
impl kodex_extension_api::ThreadInstructionsProvider for ParentInstructionsProvider {
    fn load_thread_instructions(&self) -> kodex_extension_api::LoadInstructionsFuture<'_> {
        kodex_extension_api::UserInstructionsProvider::load_user_instructions(self)
    }
}
...
async fn live_fork_keeps_instructions_when_source_is_unloaded_during_setup() {
    ...
    config.kodex_home = temp_dir.path().join("kodex-home").abs();
    config.cwd = config.kodex_home.abs();
    std::fs::create_dir_all(&config.kodex_home).expect("create kodex home");
    let global = kodex_extension_api::Instructions {
        text: "global instructions".to_string(),
        source: None,
    };
    let thread = kodex_extension_api::Instructions {
...
async fn thread_analytics_opt_out_overrides_shared_client() {
    ...
    wiremock::Mock::given(wiremock::matchers::method("POST"))
        .and(wiremock::matchers::path("/kodex/analytics-events/events"))
    ...
    for (name, client_override, expected_enabled) in [
        ...
        config.kodex_home = temp_dir.path().join(name).abs();
        config.cwd = config.kodex_home.abs();
        std::fs::create_dir_all(&config.kodex_home).expect("create kodex home");
        let mut manager = ThreadManager::with_models_provider_and_home_for_tests(
            auth.clone(),
            ...
            .zip(expected_enabled)
        {
            ...
            services.analytics_events_client.track_app_used(
                kodex_analytics::TrackEventsContext {
                    turn_metadata: None,
                    model_slug: "test-model".to_string(),
                    turn_id: format!("test-turn-{thread_id}"),
                    thread_id,
                    product_client_id: "kodex_work_cca".to_string(),
                },
                kodex_analytics::AppInvocation {
    ...
    let mut actual_thread_ids: Vec<String> = events
        .iter()
        .filter(|event| event["event_type"] == "kodex_app_used")
        .map(|event| {
...
async fn reserved_thread_id_is_used_without_changing_normal_id_generation() {
    ...
    assert!(matches!(
        ...
        kodex_protocol::error::KodexErrorDetails::InvalidRequest(message)
...
impl kodex_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: kodex_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> kodex_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<kodex_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> kodex_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<kodex_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> kodex_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: kodex_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl kodex_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: kodex_extension_api::McpServerContributionContext<'a, Config>,
        ) -> kodex_extension_api::ExtensionFuture<'a, Vec<kodex_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("dummy"));
    let manager = Arc::new(ThreadManager::new(
        &config,
        Arc::clone(&auth_manager),
        build_models_manager(&config, auth_manager),
        crate::KodexAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(kodex_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_internal_guardian_session_preserves_windows_sandbox_proxy_settings() {
    ...
    assert_eq!(
        ...
        (
            ...
            kodex_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
        )
    );
...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<Config> for ParentLifecycleContributor {}

    impl kodex_extension_api::McpServerContributor<Config> for ParentLifecycleContributor {
        fn id(&self) -> &'static str {
        ...
        fn contribute<'a>(
            &'a self,
            context: kodex_extension_api::McpServerContributionContext<'a, Config>,
        ) -> kodex_extension_api::ExtensionFuture<'a, Vec<kodex_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let mut managed_exec_policy = kodex_execpolicy::Policy::empty();
    managed_exec_policy
        .add_prefix_rule(&["rm".to_string()], kodex_execpolicy::Decision::Forbidden)
        .expect("add managed execution restriction");
    let mut requirements = config.config_layer_stack.requirements().clone();
    requirements.exec_policy = Some(kodex_config::Sourced::new(
        kodex_execpolicy::RequirementsExecPolicy::new(managed_exec_policy),
        kodex_config::RequirementSource::Unknown,
    ));
    requirements.additional_developer_instructions = Some(kodex_config::Sourced::new(
    ...
    config.config_layer_stack = kodex_config::ConfigLayerStack::new(
        config
            .config_layer_stack
    ...
    let parent_instructions = kodex_extension_api::Instructions {
        text: "parent user instructions must not be inherited".to_string(),
        source: Some(config.kodex_home.join("AGENTS.md")),
    };
    let mut manager = ThreadManager::with_models_provider_and_home_for_tests(
        KodexAuth::from_api_key("dummy"),
        config.model_provider.clone(),
    ...
    manager_state.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        manager_state.mcp_manager.kodex_apps_tools_cache(),
    ));
    ...
    let parent = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("kodex_work_desktop".to_string()),
    ...
    reviewer_environment.config =
        EnvironmentConfigState::Ready(kodex_protocol::protocol::EnvironmentConfig {
            ...
            exec_policy: Some(kodex_execpolicy::RequirementsExecPolicy::new(
                kodex_execpolicy::Policy::empty(),
            )),
            mcp_policy: None,
            network_policy: None,
    ...
    assert_eq!(reviewer_config.originator, "kodex_work_desktop");
    assert_eq!(
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: kodex_extension_api::ThreadStartInput<'a, Config>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
    ...
    impl kodex_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        ) -> kodex_extension_api::ExtensionFuture<'a, Vec<kodex_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = kodex_mcp::kodex_apps_mcp_server_config(
                ...
                vec![
                    kodex_extension_api::McpServerContribution::SelectedPluginPackage {
                        selected_root_id: plugin_id.clone(),
                        plugin_id: plugin_id.clone(),
                        plugin_display_name: plugin_id.clone(),
                        connector_ids: vec![],
                    },
                    kodex_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = kodex_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &kodex_mcp::McpConfig| {
        kodex_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let kodex_apps_server = kodex_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(kodex_mcp::KODEX_APPS_MCP_SERVER_NAME)
        .expect("Kodex Apps server should be configured");
    let kodex_apps_headers = match kodex_apps_server.transport {
        kodex_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        kodex_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("Kodex Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        kodex_apps_headers
            .expect("Kodex Apps headers should be configured")
            .get("originator"),
        Some(&"kodex_work_desktop".to_string())
    );
...
#[tokio::test]
async fn explicit_installation_id_skips_kodex_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.kodex_home = temp_dir.path().join("kodex-home").abs();
    config.cwd = config.kodex_home.abs();
    ...
    assert!(!config.kodex_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.kodex_home.join("models_cache.json").exists());
}
