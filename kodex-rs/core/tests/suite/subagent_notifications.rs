...
fn write_home_skill(kodex_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = kodex_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestKodex) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_kodex::TestKodexBuilder,
    ) -> core_test_support::test_kodex::TestKodexBuilder,
) -> Result<(
    TestKodex,
    String,
    ...
    let mut builder = configure_test(test_kodex().with_config(move |config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.kodex_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.kodex_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.kodex_home_path(), "stop_hook_log.jsonl")?;
...
async fn spawned_child_receives_forked_parent_context(
    ...
    let child_metadata: serde_json::Value = serde_json::from_str(
        child_body["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
    ...
    let reused_metadata: Value = serde_json::from_str(
        reused_child_body["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
...
async fn submit_turn_with_trigger(test: &TestKodex, prompt: &str, trigger: &str) -> Result<()> {
    test.kodex
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![UserInput::Text {
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
    ...
    let mut builder = test_kodex().with_config(move |config| {
        ...
        if matches!(selection, FullHistoryV2ModelSelection::CurrentTimeReminders) {
            ...
            config.current_time_reminder = Some(CurrentTimeReminderConfig {
                ...
                clock_source: kodex_features::CurrentTimeSource::External,
    ...
    if matches!(selection, FullHistoryV2ModelSelection::CurrentTimeReminders) {
        ...
        impl kodex_core::TimeProvider for FailFirstClockRead {
            fn current_time(&self, _thread_id: ThreadId) -> kodex_core::TimeFuture<'_> {
    ...
    if matches!(selection, FullHistoryV2ModelSelection::WorldStateIdentity) {
        ...
        let child_rollout = kodex_rollout::RolloutRecorder::get_rollout_history(
            &child_thread
                .rollout_path()
...
async fn plaintext_multi_agent_v2_completion_sends_agent_message(
    ...
    let (child_request, child_turn_metadata) = loop {
        let child_request = child_request.requests().into_iter().find_map(|request| {
            ...
            let turn_metadata: Value =
                serde_json::from_str(body["client_metadata"]["x-kodex-turn-metadata"].as_str()?)
                    .ok()?;
            turn_metadata
                .get("parent_turn_id")
                .and_then(Value::as_str)?;
    ...
    if let Some((parent_turn_id, child_turn_id, child_thread_id)) = expected_completed_activity {
        ...
        let rollout = kodex_rollout::RolloutRecorder::get_rollout_history(
...
async fn spawn_agent_role_overrides_requested_model_and_reasoning_settings() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.kodex_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_preserves_configured_defaults_through_unrelated_role() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.kodex_home.join("instructions-only-role.toml");
    ...
    let test = test_kodex()
        .with_config(|config| {
            config
                ...
                .enable(Feature::Collab)
                .expect("test config should allow feature update");
            let role_path = config.kodex_home.join("model-only-role.toml");
