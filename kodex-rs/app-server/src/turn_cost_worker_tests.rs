use super::*;
use kodex_backend_client::ApiKeyResponseCost;
...
const TURN_COST_PATH: &str = "/v1/analytics/kodex/turn-costs";
...
async fn worker_starts_with_otlp_metrics_exporter_without_log_exporter() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let kodex_home = TempDir::new().expect("temporary Kodex home");
    ...
    let auth_manager = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("sk-test"));
...
async fn custom_provider_auth_failure_retries_without_auth_changes() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/analytics/kodex/turn-costs"))
...
async fn auth_manager_at(kodex_home: &std::path::Path) -> Arc<AuthManager> {
    Arc::new(
        AuthManager::new(
...
async fn turn_cost_failures_do_not_log_response_bodies(status: u16) {
    ...
    let mut runtime = test_runtime(
        &server,
        AuthManager::from_auth_for_testing(KodexAuth::from_api_key("sk-test")),
    )
    .await;
