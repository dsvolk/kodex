...
use kodex_network_proxy::CredentialBrokerContext;
use kodex_network_proxy::NetworkProxy;
use kodex_network_proxy::brokered_credential_marker_env_keys;
use kodex_network_proxy::brokered_credential_value_env_keys;
use kodex_otel::SessionTelemetry;
use kodex_protocol::ThreadId;
use kodex_protocol::config_types::ShellEnvironmentPolicy;
use kodex_protocol::shell_environment::create_env_from_vars;
use kodex_shell_command::shell_snapshot::CapturedSnapshot;
use kodex_shell_command::shell_snapshot::PreparedSnapshot;
use kodex_shell_command::shell_snapshot::SnapshotCaptureOptions;
use kodex_shell_command::shell_snapshot::SnapshotCredentialEnvironment;
use kodex_shell_command::shell_snapshot::SnapshotStartup;
use kodex_shell_command::shell_snapshot::prepare_snapshot_credentials;
use kodex_shell_command::shell_snapshot::snapshot_capture_script;
...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            ...
            let timer = config
                .session_telemetry
                .start_timer("kodex.shell_snapshot.duration_ms", &[("version", "v1")]);
            ...
            config
                .session_telemetry
                .counter("kodex.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_create(
        ...
        let temp_path = kodex_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let kodex_home = kodex_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&kodex_home, cleanup_session_id, state_db).await
            {
...
async fn capture_snapshot(
    ...
    snapshot_allowed_env.remove("KODEX_NETWORK_PROXY_BROKERED_CREDENTIALS");
...
async fn run_script_with_timeout(
    ...
    kodex_protocol::shell_environment::scrub_non_inheritable_env_vars(handler.as_std_mut());
    #[cfg(unix)]
    unsafe {
        handler.pre_exec(|| {
            kodex_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(
    ...
    let snapshot_dir = kodex_home.join(SNAPSHOT_DIR);
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(kodex_home, session_id, state_db.as_deref()).await?;
