...
use kodex_protocol::protocol::ElicitationAction;
...
async fn auto_session_approval_is_scoped_to_tool_link_id() -> Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |request: &Request| {
            ...
            if *lists_link_selector.lock().unwrap() {
                ...
                tool["_meta"]["_kodex_apps"]["requires_explicit_link_id"] = json!(true);
    ...
    for (index, request) in requests.iter().skip(1).step_by(2).enumerate() {
        ...
        if cases[index].0.is_some() {
            expected_result["_kodex_apps"]["requires_explicit_link_id"] = json!(true);
        }
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.kodex_home.join("config.toml").abs();
...
fn attribution_models(model_slugs: [&str; 2]) -> Vec<kodex_protocol::openai_models::ModelInfo> {
...
async fn wait_for_mcp_tool_call_item(
    ...
    wait_for_event_match(&test.kodex, |event| {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request(
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.kodex, |event| {
    ...
    if !strict_auto_review {
        let EventMsg::ElicitationRequest(request) = wait_for_event(&test.kodex, |event| {
            matches!(
                event,
                EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
            )
    ...
    let mcp_turn_metadata = apps_tool_call
        .pointer("/params/_meta/x-kodex-turn-metadata")
        .expect("MCP tools/call turn metadata");
    assert_eq!(
        mcp_turn_metadata["kodex_version"],
    ...
    assert_eq!(
        ...
        Some(&response_body["client_metadata"]["x-kodex-window-id"])
    );
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-kodex-turn-metadata/user_input_requested_during_turn"),
...
"#
                ...
                config.config_layer_stack = config
                    .config_layer_stack
                    .with_user_config(&config.kodex_home.join("config.toml").abs(), user_config)
                    .expect("apps user config should be valid");
            } else {
                set_default_app_approval_mode_and_reviewer(
                    config,
    ...
    let route_event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn apps_default_writes_prompts_for_writes_but_not_reads() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(read_begin) = wait_for_event(&test.kodex, |event| {
        matches!(event, EventMsg::McpToolCallBegin(_))
    })
    .await
    else {
    ...
    let next_route = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
...
async fn mcp_tool_call_metadata_uses_captured_step_after_request_user_input() -> Result<()> {
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-kodex-turn-metadata/model"),
        Some(&json!(model_b))
    );
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-kodex-turn-metadata/reasoning_effort"),
        Some(&json!("high"))
    );
