...
/// directory. If you need Kodex to work from such a checkout simply pass the
...
mod tests {
    ...
    async fn git_metadata_commands_do_not_inherit_stdin() {
        const CHILD_ENV: &str = "KODEX_GIT_UTILS_STDIN_CHILD";
    ...
    fn canonicalize_git_remote_url_normalizes_github_variants() {
        for remote in [
            "git@github.com:OpenAI/Kodex.git",
            "ssh://git@github.com/openai/kodex.git",
            "ssh://git@github.com:22/OpenAI/Kodex.git",
            "https://github.com/openai/kodex.git",
            "https://github.com:443/openai/kodex.git",
            "https://token@github.com/openai/kodex/",
            "github.com/OpenAI/Kodex.git",
        ] {
            assert_eq!(
                canonicalize_git_remote_url(remote),
                Some("github.com/openai/kodex".to_string())
            );
        }
    }
