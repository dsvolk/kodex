...
fn state(store: &LocalThreadStore) -> ThreadStoreResult<&kodex_rollout::StateDbHandle> {
    store
        .state_db
        .as_ref()
...
fn state_root(root: StoredProjectRoot) -> kodex_state::ProjectRoot {
    kodex_state::ProjectRoot { path: root.path }
}

fn stored_project(project: kodex_state::Project) -> StoredProject {
    StoredProject {
        id: project.id,
        name: project.name,
        roots: project
