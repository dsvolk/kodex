...
Quickstart for running and hitting `kodex app-server`.

## Quickstart

Run from `<reporoot>/kodex-rs`.

```bash
# 1) Build debug kodex binary
cargo build -p kodex-cli --bin kodex

# 2) Start websocket app-server in background
cargo run -p kodex-app-server-test-client -- \
  --kodex-bin ./target/debug/kodex \
  serve --listen ws://127.0.0.1:4222 --kill

# 3) Call app-server (defaults to ws://127.0.0.1:4222)
cargo run -p kodex-app-server-test-client -- model-list
```

`send-message` and `send-message-v2` handle `request_user_input` server requests interactively.
When Kodex asks a question, choose a numbered option (or `o` for a free-form answer when offered)
and the client will send the response and continue streaming the same turn.

## Testing Kodex-managed Amazon Bedrock login
...
credential and sets `model_provider = "amazon-bedrock"`, so use an isolated `KODEX_HOME` when
testing.

```bash
export KODEX_HOME="$(mktemp -d)"
printf 'cli_auth_credentials_store = "file"\n' > "$KODEX_HOME/config.toml"
...
The test client redacts `apiKey` from its outbound request log. After login, start a fresh Kodex
process with the same `KODEX_HOME` to verify that it uses the persisted managed credential.

## Testing logout

`test-logout` initializes the app-server, sends an `account/logout` request, and waits for the
resulting `account/updated` notification. It uses the active `KODEX_HOME`, so point it at an
...
active Kodex profile must be authenticated. On a fresh local cache, the command
retries ephemeral turns while the installed remote bundle finishes syncing.

```bash
# Build a debug Kodex binary; analytics capture is unavailable in release builds.
cargo build -p kodex-cli --bin kodex
...
The command validates one `kodex_plugin_disabled`, `kodex_plugin_enabled`, and
`kodex_plugin_used` event with the expected local and remote plugin identities
...
installed, installs it, validates `kodex_plugin_installed`, uninstalls it, and
validates `kodex_plugin_uninstalled`, and verifies that the original
uninstalled state was restored.

The mutation events include the local Kodex ID in `plugin_id` and the backend ID
in `remote_plugin_id`.
...
Cleanup does not require analytics capture or a debug Kodex binary. When the
...
```bash
cargo run -p kodex-app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/kodex-app-server-test-client/app-server.log`
...
```bash
cargo run -p kodex-app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p kodex-app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin kodex-app-server-test-client -- \
...
```bash
cargo run --bin kodex-app-server-test-client -- thread-resume <THREAD_ID>
```
