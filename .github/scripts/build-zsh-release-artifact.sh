...
work_root="$(mktemp -d "${temp_root%/}/kodex-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/kodex-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${KODEX_WRAPPER_LOG:?missing KODEX_WRAPPER_LOG}"
printf '%s\n' "$@" > "$KODEX_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
KODEX_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" kodex-zsh)
