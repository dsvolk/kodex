...
async fn cold_pool_uses_http_during_open_timeout_then_recovers_after_cooldown() -> Result<()> {
    ...
    for uses_kodex_backend in [false, true] {
        ...
        let base_path = if uses_kodex_backend {
            "/backend-api/kodex"
        } else {
            "/v1"
        };
        ...
        config.provider = create_model_provider(
            ...
            Some(AuthManager::from_auth_for_testing(if uses_kodex_backend {
        ...
        let expected_path = if uses_kodex_backend {
        ...
        assert_eq!(
            first.header("x-kodex-guardian").as_deref(),
            uses_kodex_backend.then_some("classifier")
        );
        ...
        assert_eq!(
            ...
            uses_kodex_backend.then_some("resp-parent")
        );
...
async fn http_cache_preserves_no_constraint_redirect_policy_and_configured_headers() -> Result<()> {
    let base_url = "https://localhost:9/backend-api/kodex".to_owned();
    let auth =
    ...
    for (routing, expected_policy) in [
        ...
        *resolver.0.lock().unwrap() = routing.map(|account_routing_override| WorkspaceRouting {
        ...
        assert_eq!(
            ...
            (
                "https://localhost:9/backend-api/kodex/responses",
                expected_policy
            ),
        );
