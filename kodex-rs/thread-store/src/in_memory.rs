...
use kodex_rollout::persisted_rollout_items;
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        shared
            ...
            .await?;
        let metadata = kodex_state::ThreadMetadataBuilder::new(
            thread_id,
...
impl InMemoryThreadStore {
    ...
    pub fn with_state_db(&self, state_db: Option<kodex_rollout::StateDbHandle>) -> Self {
        Self {
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| kodex_state::ThreadSection {
                name: if id == kodex_state::PINNED_THREAD_SECTION_ID {
                    kodex_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<kodex_protocol::protocol::GitInfo> {
    ...
    Some(kodex_protocol::protocol::GitInfo {
