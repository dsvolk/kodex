...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/kodex-arg0-session/kodex-linux-sandbox".to_string(),
    );
    let daemon_directory = kodex_uds::shared_daemon_socket_directory()
        .unwrap()
        .display()
        .to_string();
    assert_eq!(
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/kodex-arg0-session/kodex-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--kodex-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
