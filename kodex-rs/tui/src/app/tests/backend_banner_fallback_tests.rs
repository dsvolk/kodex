...
async fn backend_banner_fallback_updates_task_settings_and_keeps_notice() -> Result<()> {
    for mode_kind in [ModeKind::Default, ModeKind::Plan] {
        ...
        let config_path = app.config.kodex_home.join("config.toml");
        ...
        assert!(queued.iter().any(|event| matches!(event, AppEvent::OpenUrlInBrowser { url } if url == "https://chatgpt.com/kodex/settings/usage")));
        let notices = queued
            .iter()
        ...
        assert!(!queued.iter().any(|event| matches!(
            event,
            AppEvent::PersistModelSelection { .. } | AppEvent::KodexOp(_)
        )));
...
async fn backend_banner_fallback_handles_settings_failure_and_queued_manual_selection() -> Result<()>
{
        ...
        .flat_map(|reserve| [Some(-32601), Some(-32603), None].map(|error| (reserve, error)))
    {
        ...
        let kodex_home = app.config.kodex_home.display().to_string();
        ...
        let fake = tokio::spawn(async move {
            ...
            while let Some(frame) = socket.next().await {
                ...
                let response = if request["method"] == "initialize" {
                    json!({"id": request["id"], "result": {"userAgent": "test", "kodexHome": kodex_home}})
                } else {
        ...
        let pending_turn = if use_reserve && error_code.is_some() {
            ...
            Some(
                std::iter::from_fn(|| events.try_recv().ok())
                    .find(|event| matches!(event, AppEvent::KodexOp(AppCommand::UserTurn { .. })))
        ...
        if use_reserve && error_code.is_some() {
            ...
            assert!(
                !std::iter::from_fn(|| events.try_recv().ok())
                    .any(|event| matches!(event, AppEvent::KodexOp(AppCommand::UserTurn { .. })))
            );
        }
        server.shutdown().await?;
        fake.await??;
...
async fn backend_banner_fallback_preserves_permissions_for_first_eligible_cyber_model() -> Result<()>
{
    ...
    assert!(
        app.apply_permission_profile_selection(PermissionProfileSelection {
            profile_id: kodex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY.into(),
    ...
    assert_eq!(
        ...
        (
            ...
            kodex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY.to_string()
        ),
    );
    server.shutdown().await?;
    Ok(())
