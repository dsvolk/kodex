...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::KodexOp(Op::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::KodexOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::KodexOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn slash_init_does_not_depend_on_loaded_instruction_sources() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.instruction_source_paths = vec![kodex_utils_path_uri::PathUri::from_abs_path(
...
async fn goal_slash_command_emits_attached_images() {
    ...
    let local_image = chat.config.kodex_home.join("goal-local.png");
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(kodex_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(kodex_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::KodexOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn slash_rename_requests_and_prefills_an_editable_title_suggestion() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::KodexOp(Op::SetThreadName { name })) if name == "Fix login timeout"
    );
}
...
async fn slash_rename_preserves_manual_edits_when_suggestion_arrives() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::KodexOp(Op::SetThreadName { name })) if name == "My own title"
    );
}
...
async fn usage_command_runs_with_backend_auth_from_widget_init() {
    ...
    assert!(!chat.has_chatgpt_account());
    assert!(chat.has_kodex_backend_auth());
}
...
async fn slash_copy_picker_copies_status_fields_and_preserves_source_after_copying() {
    ...
    assert_chatwidget_snapshot!(
        ...
        render_bottom_popup(&chat, /*width*/ 100)
            ...
            .replace(crate::version::KODEX_CLI_VERSION, "VERSION"),
    );
    chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(kodex_config::types::KeybindingsSpec::One(
        kodex_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            kodex_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: kodex_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: kodex_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/kodex-test-rollout.jsonl");
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::KodexOp(Op::OverrideTurnContext {
...
async fn compact_queues_user_messages_snapshot() {
    ...
    handle_error(
        ...
        Some(KodexErrorInfo::ActiveTurnNotSteerable {
