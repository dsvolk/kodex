...
use kodex_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = kodex_home.clone();
    let content = kodex_http_client::NetworkPolicyController::default();
    let local = kodex_http_client::NetworkPolicyController::default();
    let local_policy = local.policy();
    local.publish(
        local_policy.revision(),
        kodex_http_client::DestinationPolicy::Unrestricted,
    );
    let factory = kodex_http_client::HttpClientFactory::new(
        kodex_http_client::OutboundProxyPolicy::ReqwestDefault,
    );
    let routes = kodex_login::AuthRouteConfig::from_http_client_factory(
    ...
    let opts = ServerOptions {
        kodex_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: routes,
        client_id: kodex_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = kodex_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: kodex_login::test_support::transport_default_auth_route_config(),
        client_id: kodex_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/kodex/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_kodex_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let kodex_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = HttpClientBuilder::new().build_direct()?;
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_kodex_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Kodex"),
        "error body should clearly explain the Kodex access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_kodex_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Kodex to retry"),
    ...
    assert!(
        !body.contains("get access to Kodex"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not kodex"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        kodex_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_kodex_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        kodex_home: first_kodex_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: kodex_login::test_support::transport_default_auth_route_config(),
        client_id: kodex_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_kodex_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        kodex_home: second_kodex_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: kodex_login::test_support::transport_default_auth_route_config(),
        client_id: kodex_login::CLIENT_ID.to_string(),
        issuer,
