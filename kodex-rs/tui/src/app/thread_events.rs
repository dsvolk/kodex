...
pub(super) struct ThreadEventSnapshot {
    ...
    pub(super) active_reasoning_item: Option<kodex_app_server_protocol::ItemStartedNotification>,
...
impl ThreadEventStore {
    ...
    fn push_notification_inner(&mut self, notification: Cow<'_, ServerNotification>) {
        ...
        match notification.as_ref() {
            ...
            ServerNotification::Error(n)
                if self.active_turn_id.is_none()
                    && !n.will_retry
                    && n.error.kodex_error_info
                        == Some(AppServerKodexErrorInfo::MisalignmentPolicyViolation) =>
            {
            ...
            ServerNotification::ItemStarted(
                started @ kodex_app_server_protocol::ItemStartedNotification {
                ...
                }) =>
            {
                self.active_reasoning_item =
                    Some(kodex_app_server_protocol::ItemStartedNotification {
    ...
    pub(super) fn file_change_changes(
        ...
        turn_id: &str,
        item_id: &str,
    ) -> Option<Vec<kodex_app_server_protocol::FileUpdateChange>> {
...
mod tests {
    ...
    use kodex_app_server_protocol::HookEventName as AppServerHookEventName;
    use kodex_app_server_protocol::HookExecutionMode as AppServerHookExecutionMode;
    use kodex_app_server_protocol::HookHandlerType as AppServerHookHandlerType;
    use kodex_app_server_protocol::HookOutputEntry as AppServerHookOutputEntry;
    use kodex_app_server_protocol::HookOutputEntryKind as AppServerHookOutputEntryKind;
    use kodex_app_server_protocol::HookRunStatus as AppServerHookRunStatus;
    use kodex_app_server_protocol::HookRunSummary as AppServerHookRunSummary;
    use kodex_app_server_protocol::HookScope as AppServerHookScope;
    ...
    use kodex_app_server_protocol::McpToolCallProgressNotification;
        ...
        for _ in 0..32 {
            store.push_notification_ref(&ServerNotification::ThreadAttachmentUpdated(
                ThreadAttachmentUpdatedNotification {
                    ...
                    identity_key: "openai/kodex#1".to_string(),
    ...
    fn thread_event_store_rebase_preserves_mcp_startup_notifications() {
        ...
        let notification = ServerNotification::McpServerStatusUpdated(
            kodex_app_server_protocol::McpServerStatusUpdatedNotification {
                thread_id: Some(thread_id.to_string()),
                name: "sentry".to_string(),
                status: kodex_app_server_protocol::McpServerStartupState::Failed,
