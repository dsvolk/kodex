...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "kodex-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "kodex.turns",
        "Total number of Kodex turns.",
        /*inc*/ 1,
    ...
    metrics.gauge_with_description(
        "kodex.active",
        "Number of active Kodex operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    let counter = find_metric(&resource_metrics, "kodex.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Kodex turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "kodex-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "kodex.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "kodex.tool_latency")
            ...
            .expect("kodex.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "kodex.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Kodex operations.");
...
fn histogram_uses_explicit_bucket_boundaries() -> Result<()> {
    ...
    assert_eq!(
        histogram_data(&latest_metrics(&exporter), "kodex.payload_bytes"),
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "kodex.alpha").expect("kodex.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "kodex.beta").expect("kodex.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("kodex.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
