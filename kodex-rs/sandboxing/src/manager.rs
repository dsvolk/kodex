...
use crate::landlock::KODEX_LINUX_SANDBOX_ARG0;
...
pub use kodex_protocol::sandbox::SandboxType;
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: kodex_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing kodex-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::WindowsMxc => {
                if !kodex_mxc_sandbox::is_available() {
                ...
                let exe = sandbox_exe.ok_or_else(|| {
                    SandboxTransformError::WindowsMxcPreparation(
                        "missing Kodex executable path".to_string(),
                    )
                })?;
                let mut full_command =
                    vec![os_string_to_command_component(exe.as_os_str().to_owned())];
                full_command.extend(
                    kodex_mxc_sandbox::create_command_args(
                        kodex_mxc_sandbox::CreateMxcCommandArgsParams {
                            command: argv,
            ...
            SandboxType::MacosSeatbelt => {
                ...
                let mut args = create_seatbelt_command_args_with_profile(
                    ...
                    self.allowed_symlinked_kodex_home.as_ref(),
                )
    ...
    pub fn transform_for_direct_spawn(
        ...
        if request.transform.sandbox == SandboxType::WindowsRestrictedToken {
            let kodex_home = kodex_utils_home_dir::find_kodex_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            return self.transform_for_direct_spawn_with_kodex_home(request, kodex_home.as_path());
        }
        self.transform(request.transform)
    }

    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_kodex_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        kodex_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    kodex_home: &Path,
    proxy_settings_mode: kodex_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = kodex_windows_sandbox::resolve_exe_for_launch(source.as_path(), kodex_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        kodex_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn add_windows_sandbox_wrapper_setup_env_from_vars(
    ...
    env.retain(|key, _| !key.eq_ignore_ascii_case("KODEX_WINDOWS_REGISTERED_CORE"));
    if registered_core {
        env.insert("KODEX_WINDOWS_REGISTERED_CORE".into(), "1".into());
    }
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(KODEX_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        KODEX_LINUX_SANDBOX_ARG0.to_string()
    }
}
