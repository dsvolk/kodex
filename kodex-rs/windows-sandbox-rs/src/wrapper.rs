//! Internal `kodex.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const KODEX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const KODEX_HOME_FLAG: &str = "--kodex-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        KODEX_WINDOWS_SANDBOX_ARG1.to_string(),
        KODEX_HOME_FLAG.to_string(),
        kodex_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned = crate::unified_exec::spawn_windows_sandbox_session_with_desktop(
        crate::WindowsSandboxSessionRequest {
            ...
            kodex_home: request.kodex_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            KODEX_HOME_FLAG => kodex_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let kodex_home = kodex_home.ok_or_else(|| anyhow!("missing required {KODEX_HOME_FLAG}"))?;
    if !kodex_home.is_absolute() {
        bail!(
            "{KODEX_HOME_FLAG} must be absolute: {}",
            kodex_home.display()
        );
    }
