...
pub fn allowed_symlinked_kodex_home(
    ...
    let enabled = config_layer_stack
        ...
        .config
        .get("allow_symlinked_kodex_home")?
        .as_bool()?;
    if !enabled {
        return None;
    }
    ...
    let alias = std::env::var_os("KODEX_HOME")
        .filter(|path| !path.is_empty())
        .and_then(|path| AbsolutePathBuf::from_absolute_path(path).ok())
        .filter(|path| path.canonicalize().ok().as_ref() == Some(kodex_home));
    Some(alias.unwrap_or_else(|| kodex_home.clone()))
}
