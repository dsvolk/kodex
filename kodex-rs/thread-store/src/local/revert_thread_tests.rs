...
async fn revert_keeps_thread_id_and_hides_suffix_across_repeated_reverts() {
    ...
    assert_ne!(
        kodex_rollout::rollout_id_from_path(first_replacement_path.as_path()),
        Some(thread_id)
    );
    let replacement_meta = kodex_rollout::read_session_meta_line(first_replacement_path.as_path())
        .await
    ...
    let stored = state_db
        ...
        .expect("read metadata")
        .expect("thread metadata");
    let recovered = kodex_rollout::read_session_meta_line(&stored.rollout_path)
        .await
    ...
    for version in [
        ...
        for (before_turn_id, multi_agent_version) in [("turn-2", Some(version)), ("turn-1", None)] {
            ...
            let path = state_db
                ...
                .expect("thread metadata")
                .rollout_path;
            let meta = kodex_rollout::read_session_meta_line(&path)
                .await
                .expect("read replacement metadata");
...
async fn rollout_paths_for_thread(
    ...
    kodex_rollout::RolloutReferenceIndex::scan(home)
        .await
...
async fn create_paginated_thread(store: &LocalThreadStore, thread_id: ThreadId) {
    store
        .create_thread(CreateThreadParams {
            ...
            runtime_workspace_roots: Some(vec![store.config.kodex_home.join("workspace").abs()]),
