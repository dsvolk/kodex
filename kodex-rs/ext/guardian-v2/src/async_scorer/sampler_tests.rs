...
use kodex_extension_api::ContextualUserFragment;
use kodex_extension_api::ExtensionMetrics;
use kodex_guardian_context::PreviousReviews;
use kodex_http_client::HttpClientFactory;
use kodex_http_client::OutboundProxyPolicy;
use kodex_login::AgentIdentityAuthPolicy;
...
use kodex_prompts::GuardianClassifierInstructions;
...
fn assert_connection_metadata(
    ...
    assert_eq!(
        [
            ...
            handshake.header("x-kodex-window-id"),
        ],
        [
    ...
    for (request, (parent_turn_id, root_turn_id)) in requests.iter().zip(expected_lineage) {
        ...
        metadata["x-kodex-turn-metadata"] = serde_json::from_str(
            metadata["x-kodex-turn-metadata"]
                .as_str()
        ...
        let mut expected = json!({
            ...
            "x-kodex-window-id": format!("{thread_id}:0"),
            "ws_request_header_x_openai_internal_kodex_responses_lite": "true",
            "x-kodex-turn-metadata": {
                "session_id": "session-1",
                "thread_id": thread_id,
        ...
        if let Some(root_turn_id) = root_turn_id {
            ...
            expected["x-kodex-turn-metadata"]["root_turn_id"] = json!(root_turn_id);
        }
...
pub(super) fn sampler_config(base_url: String) -> LunaSamplerConfig {
    LunaSamplerConfig {
        workspace_routing: kodex_model_provider::WorkspaceRoutingContext::new(
        ...
        max_input_tokens: kodex_guardian_context::DEFAULT_MAX_INPUT_TOKENS,
        metrics: None,
    }
}
...
async fn sampler_records_token_usage_after_returning_an_early_classification() -> Result<()> {
    ...
    let estimated = input
        .iter()
        .map(kodex_guardian_context::estimate_input_tokens)
        .sum::<usize>();
    assert!(metrics.0.lock().unwrap().contains(&(
        kodex_guardian_context::REQUEST_TOKENS_METRIC.to_owned(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn classifier_sends_guardian_header_only_with_kodex_backend_auth() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn preconnected_sampler_reuses_authenticated_websocket_for_classifications() -> Result<()> {
    ...
    .await?;
    let manager = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("test-api-key"));
    manager
    ...
    assert_eq!(
        handshake.header("x-openai-internal-kodex-responses-lite"),
        Some("true".to_owned())
    );
...
async fn sampler_retries_expired_websockets_on_another_warm_connection() -> Result<()> {
    ...
    request.input.insert(
        /*index*/ 1,
        ContextualUserFragment::into(kodex_guardian_context::TrustedSkills {
...
async fn parent_response_id_survives_classifier_transport_retry() -> Result<()> {
    skip_if_no_network!(Ok(()));
    for uses_kodex_backend in [false, true] {
        let healthy = responses::start_websocket_server(vec![vec![vec![
        ...
        for server in [&expired, &healthy] {
            ...
            assert_eq!(
                ...
                (uses_kodex_backend.then(|| json!(parent_response_id)), None)
            );
