...
#[tokio::test]
async fn cloud_provider_auth_merges_before_parsing_and_resolves_cwd_from_kodex_home() -> Result<()>
{
    let home = tempdir()?;
    std::fs::write(
...
name = "Local gateway"
base_url = "{local_url}"
env_key = "KODEX_TEST_LOCAL_GATEWAY_KEY_MUST_NOT_BE_USED"
experimental_bearer_token = "local-token"
    ...
    if !required_selection.is_empty() {
        for cli_provider in ["openai", "gateway"] {
            let overridden = ConfigBuilder::default()
                .kodex_home(test.config.kodex_home.to_path_buf())
                .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
            ...
            let rebuilt = Config::rebuild_with_session_layers(
                ...
                &overridden.config_layer_stack,
                overridden.kodex_home.clone(),
                overridden
                    .zsh_path
                    .clone()
                    .map(kodex_utils_absolute_path::AbsolutePathBuf::try_from)
                    .transpose()?,
            )
            .await?;
