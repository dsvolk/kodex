...
use kodex_history_notes_extension::install;
...
async fn installed_extension_exposes_and_invokes_history_notes_tools() -> TestResult {
    ...
    Mock::given(method("POST"))
        .and(path("/backend-api/kodex/alpha/notes/v2/read_file"))
        ...
        .await;
    Mock::given(method("POST"))
        .and(path("/backend-api/kodex/alpha/notes/v2/thread_hint"))
        .and(header("x-openai-actor-authorization", "actor-biscuit"))
    ...
    let mut config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    config.model_provider.base_url = Some(format!("{}/backend-api/kodex", server.uri()));
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(KodexAuth::Headers(AuthHeaders::new(headers)));
    ...
    for (namespace, name, mut arguments) in [
        ...
        Mock::given(method("POST"))
            .and(path(format!(
                "/backend-api/kodex/alpha/{namespace}/v2/{name}"
            )))
            .and(header("x-openai-actor-authorization", "actor-biscuit"))
    ...
    Mock::given(method("POST"))
        .and(path("/backend-api/kodex/alpha/history/v2/read_item"))
        .and(header("x-openai-actor-authorization", "actor-biscuit"))
        .respond_with(ResponseTemplate::new(200).set_body_json(response))
        .mount(&server)
        .await;
...
#[tokio::test]
async fn history_notes_require_an_openai_provider_and_kodex_backend_auth() -> TestResult {
    let kodex_home = TempDir::new()?;
    let mut config = ConfigBuilder::default()
        .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        .kodex_home(kodex_home.path().to_path_buf())
