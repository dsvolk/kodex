...
fn listen_unix_socket_accepts_absolute_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix:///tmp/kodex.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: absolute_path("/tmp/kodex.sock")
        })
    );
}
...
fn listen_unix_socket_accepts_relative_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix://kodex.sock"),
        Ok(AppServerTransport::UnixSocket {
...
async fn control_socket_acceptor_upgrades_and_forwards_websocket_text_messages_and_pings() {
    ...
    let advertised_max = response
        .headers()
        .get("x-kodex-websocket-max-unfragmented-message-bytes")
...
fn default_control_socket_path() -> AbsolutePathBuf {
    ...
    app_server_control_socket_path(&kodex_home).expect("default control socket path")
}
