...
use kodex_analytics::CompactionStrategy;
...
use crate::client::X_KODEX_INSTALLATION_ID_HEADER;
use crate::client::X_KODEX_PARENT_THREAD_ID_HEADER;
use crate::client::X_KODEX_TURN_METADATA_HEADER;
use crate::client::X_KODEX_WINDOW_ID_HEADER;
...
pub(crate) enum KodexResponsesRequestKind {
    Turn,
    Prewarm,
...
impl KodexResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            KodexResponsesRequestKind::Turn => ("turn", None),
            KodexResponsesRequestKind::Prewarm => ("prewarm", None),
            KodexResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            KodexResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_thread_identity(self) -> bool {
        !matches!(self, KodexResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Kodex metadata sent to ResponsesAPI.
///
/// The full Kodex turn metadata blob is transported canonically as
/// `client_metadata["x-kodex-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct KodexResponsesMetadata {
    ...
    pub(crate) request_kind: Option<KodexResponsesRequestKind>,
...
impl KodexResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            ...
            (X_KODEX_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_KODEX_PARENT_THREAD_ID_HEADER.to_string(),
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_KODEX_WINDOW_ID_HEADER, &self.window_id);
        // Direct x-kodex-turn-metadata is compatibility output. Keep the unbounded tool inventory
        // in client_metadata only so HTTP and WebSocket compatibility headers remain bounded.
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&KodexTurnMetadataPayload {
                tool_namespaces_info: None,
    ...
    fn turn_metadata_payload(&self) -> KodexTurnMetadataPayload<'_> {
        ...
        let has_thread_identity =
            request_kind.is_none_or(KodexResponsesRequestKind::has_thread_identity);
        let has_request_identity =
            request_kind.is_some_and(KodexResponsesRequestKind::has_thread_identity);
        KodexTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the Kodex turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved Kodex-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
...
#[derive(Serialize)]
struct KodexTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
