...
async fn authenticated_plugin_migration_uses_chatgpt_curated_marketplace() {
    ...
    fs::create_dir_all(plugin_root.join(".kodex-plugin")).expect("create curated plugin directory");
    fs::write(
  ...
  "plugins": [{
    ...
    let mut service = service_for_paths(external_agent_home.clone(), kodex_home);
...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _kodex_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _kodex_home) = fixture_paths();
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_kodex() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let kodex_home = root.path().join(".kodex");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_kodex() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let kodex_home = root.path().join(".kodex");
    let repo_root = root.path().join("repo");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, kodex_home)
...
#[tokio::test]
async fn detect_repo_skips_plugins_only_configured_in_project_kodex() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let kodex_home = root.path().join(".kodex");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".kodex")).expect("create repo kodex dir");
    fs::create_dir_all(&kodex_home).expect("create kodex home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project kodex config");
...
async fn detect_repo_skips_plugins_even_with_installed_marketplace() {
    ...
    let kodex_home = root.path().join(".kodex");
    let repo_root = root.path().join("repo");
    let marketplace_root = kodex_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
