...
async fn strict_tool_collisions_fail_the_turn_before_sampling(
    ...
    thread
        .start_or_steer_turn(
            ...
            .on_start(kodex_core::TurnStartOptions {
...
async fn namespaced_custom_tool_call_preserves_namespace_through_dispatch_and_replay() -> Result<()>
{
    ...
    let expected_escaped_calls = json!([{
        "name": format!("{namespace}__{tool_name}"),
        "arguments": {
            "_kodex_executed_tool_call_truncated": {
    ...
    assert_eq!(
        ...
        json!({
            "executed_tool_calls": [{
                "name": kodex_code_mode::PUBLIC_TOOL_NAME,
                "arguments": input,
            }],
            "tool_calls_complete": true,
        }),
...
async fn collect_tools(availability: CommandToolAvailability) -> Result<Vec<String>> {
    ...
    let mut builder = match availability {
        CommandToolAvailability::Default => test_kodex(),
        CommandToolAvailability::ManagedUnifiedExecDisabled => test_kodex()
...
"#,
        ...
        CommandToolAvailability::ShellToolDisabled => test_kodex().with_config(|config| {
            config
                .features
        ...
        CommandToolAvailability::ModelDisabled => {
            test_kodex().with_model_info_override("gpt-5.4", |model| {
