...
async fn kodex_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    kodex_returns_json_result("gpt-5.4".to_string()).await
}

async fn kodex_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("kodex_output_schema".into()))
    ...
    let TestKodex { kodex, config, .. } = test_kodex().build(&server).await?;
    ...
    let message = wait_for_event(&kodex, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
