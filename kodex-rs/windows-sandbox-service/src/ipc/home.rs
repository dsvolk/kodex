//! Validates and pins local Kodex home and sandbox directories, keeping sandbox
...
use kodex_windows_sandbox::open_directory_no_reparse;
...
pub(super) fn prepare_kodex_home(
    requested: &Path,
    ...
    let requested_root = requested
        .ancestors()
        .last()
        .context("find the root of the requested Kodex home")?;
    if unsafe { filesystem::GetDriveTypeW(to_wide(requested_root.as_os_str()).as_ptr()) }
        != DRIVE_FIXED
    {
        return Err(ServiceUnavailable("Kodex home must be located on a fixed local drive").into());
    }
    let parent = requested
        .parent()
        .context("Kodex home must have an existing parent directory")?;
    ...
    let home = requested
        .canonicalize()
        .with_context(|| format!("canonicalize Kodex home {}", requested.display()))?;
    ...
    .enumerate()
    {
        ...
        let handle = pin_directory(directory, access, disposition)
            ...
            if index == 0 {
                format!(
                    "requesting user must be permitted to write Kodex home {}",
                    directory.display()
                )
            } else {
                format!(
