    ...
    assert!(matches!(
        default_store
            .add_thread_attachment(AddThreadAttachmentParams {
                ...
                payload: json!({"url": "https://github.com/openai/kodex/pull/1"}),
            })
            .await,
...
async fn unloaded_thread_attachments_support_listing_and_removal() {
    ...
    let second_params = AddThreadAttachmentParams {
        thread_id: first_thread_id,
        identity_key: "openai/kodex#2".to_string(),
        payload: json!({"url": "https://github.com/openai/kodex/pull/2"}),
        ..params.clone()
    };
...
async fn attachment_owner_deletion_can_retry_after_state_cleanup_fails() {
    ...
    runtime
        .upsert_thread_spawn_edge(
            parent_id,
            child_id,
            kodex_state::DirectionalThreadSpawnEdgeStatus::Closed,
        )
        .await
