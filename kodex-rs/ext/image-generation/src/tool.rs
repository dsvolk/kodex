...
use kodex_extension_api::parse_tool_input_schema;
...
use crate::backend::KodexImagesBackend;
...
impl<'call> ToolExecutor<ToolCall<'call>> for ImageGenerationTool {
    ...
    fn handle<'a>(&'a self, call: ToolCall<'call>) -> kodex_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
    ...
    async fn handle_call(
        ...
        .map_err(|error| {
            (
                ...
                usage_limit_failure(error.kodex_error()),
...
fn usage_limit_failure(error: &KodexErr) -> Option<ImageGenerationFailure> {
    let KodexErrorDetails::UsageLimitReached(usage_limit) = error.details() else {
        return None;
    };
