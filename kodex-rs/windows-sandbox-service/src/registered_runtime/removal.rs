...
pub(crate) fn restore_disabled_accounts() -> Result<()> {
    let _lock = kodex_windows_sandbox::acquire_sandbox_setup_lock(/*timeout_ms*/ 5_000)?;
    ...
    for index in 0..record.runtime()?.accounts.len() {
        ...
        let flags = kodex_windows_sandbox::local_user_flags(account.account.username())?
            .context("cleanup account disappeared before flag restoration")?;
        kodex_windows_sandbox::set_local_user_flags(
...
pub(crate) fn prepare(
    ...
    for index in 0..record.runtime()?.accounts.len() {
        ...
        let Some(flags) = kodex_windows_sandbox::local_user_flags(account.account.username())?
        else {
            ensure!(
        ...
        let token = with_owner_impersonation(owner_token, || {
            ...
            crate::ipc::pin_existing_ancestors(
                &kodex_windows_sandbox::sandbox_secrets_dir(&record.kodex_home),
                &mut pins,
            )?;
            kodex_windows_sandbox::logon_existing_sandbox_account(
                &record.kodex_home,
                account.account,
            )
        });
        if flags & UF_ACCOUNTDISABLE != 0 {
            super::validate_account_sid(&account)?;
            let current_flags =
                kodex_windows_sandbox::local_user_flags(account.account.username())?
                    .context("cleanup account disappeared before flag restoration")?;
            kodex_windows_sandbox::set_local_user_flags(
                account.account.username(),
    ...
    let group_sid = if tokens.is_empty() {
        ...
        Some(
            kodex_windows_sandbox::string_from_sid_bytes(&kodex_windows_sandbox::resolve_sid(
                kodex_windows_sandbox::SANDBOX_USERS_GROUP,
            )?)
    ...
    let plan = serde_json::json!({
        ...
        "record": record,
        "targets": targets,
        "service_name": kodex_windows_sandbox::windows_sandbox_service_name()?,
        "group_sid": group_sid,
    });
