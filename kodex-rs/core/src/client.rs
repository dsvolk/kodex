...
//!
//! `ModelClient` is intended to live for the lifetime of a Kodex session and holds the stable
...
//! per-turn state such as the `x-kodex-turn-state` token used for sticky routing.
...
use crate::KodexResponsesHeaders;
use async_channel::Sender;
use kodex_api::AgentIdentityTelemetry;
use kodex_api::ApiError;
use kodex_api::AuthProvider;
use kodex_api::Compression;
use kodex_api::MemoriesClient as ApiMemoriesClient;
use kodex_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use kodex_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use kodex_api::Provider as ApiProvider;
use kodex_api::RawMemory as ApiRawMemory;
use kodex_api::RealtimeCallClient as ApiRealtimeCallClient;
use kodex_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use kodex_api::Reasoning;
use kodex_api::ReasoningContext;
use kodex_api::RequestTelemetry;
use kodex_api::ReqwestTransport;
use kodex_api::ResponseCreateWsRequest;
use kodex_api::ResponsesApiRequest;
use kodex_api::ResponsesClient as ApiResponsesClient;
use kodex_api::ResponsesOptions as ApiResponsesOptions;
use kodex_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use kodex_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use kodex_api::ResponsesWsRequest;
use kodex_api::SharedAuthProvider;
use kodex_api::SseTelemetry;
use kodex_api::StreamOptions;
use kodex_api::TransportError;
use kodex_api::WebsocketTelemetry;
use kodex_api::auth_header_telemetry;
use kodex_api::build_session_headers;
use kodex_api::create_text_param_for_request;
use kodex_api::response_create_client_metadata;
...
use kodex_login::UnauthorizedRecovery;
use kodex_login::default_client::ClientRedirectPolicy;
use kodex_login::default_client::add_originator_header;
use kodex_login::default_client::create_client_for_route;
use kodex_otel::SessionTelemetry;
use kodex_otel::WEBSOCKET_CONTINUATION_COUNT_METRIC;
...
use kodex_protocol::protocol::Event as ProtocolEvent;
...
use kodex_rollout_trace::InferenceTraceAttempt;
use kodex_rollout_trace::InferenceTraceContext;
use kodex_tools::create_tools_json_for_responses_api;
use kodex_tools::create_tools_json_for_responses_lite;
use kodex_tools::create_tools_raw_json_for_responses_api;
...
use kodex_feedback::FeedbackRequestTags;
use kodex_feedback::emit_feedback_request_tags_with_auth_env;
use kodex_login::auth::AgentIdentityAuthPolicy;
use kodex_login::auth_env_telemetry::AuthEnvTelemetry;
use kodex_login::auth_env_telemetry::collect_auth_env_telemetry;
use kodex_model_provider::AgentIdentitySessionFallback;
use kodex_model_provider::ProviderAuthScope;
use kodex_model_provider::ProviderUnauthorizedRecovery;
use kodex_model_provider::ResponsesConnectionKey;
use kodex_model_provider::SharedModelProvider;
use kodex_model_provider::WorkspaceRoutingContext;
use kodex_model_provider::create_model_provider;
#[cfg(test)]
use kodex_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use kodex_response_debug_context::extract_response_debug_context_from_api_error;
use kodex_response_debug_context::telemetry_api_error_message;
use kodex_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_KODEX_INSTALLATION_ID_HEADER: &str = "x-kodex-installation-id";
pub const X_KODEX_ROUTING_HINT_HEADER: &str = "x-kodex-routing-hint";
pub const X_KODEX_TURN_STATE_HEADER: &str = "x-kodex-turn-state";
pub const X_KODEX_TURN_METADATA_HEADER: &str = "x-kodex-turn-metadata";
pub const X_KODEX_PARENT_THREAD_ID_HEADER: &str = "x-kodex-parent-thread-id";
pub const X_KODEX_WINDOW_ID_HEADER: &str = "x-kodex-window-id";
...
const X_KODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-kodex-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_kodex_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_KODEX_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-kodex-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a Kodex session
...
pub struct ModelClient {
    ...
    kodex_responses_headers: Option<Arc<KodexResponsesHeaders>>,
...
/// - The `x-kodex-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Kodex turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-kodex-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-kodex-turn-state` request header for
    ...
    url::Url::parse(&provider.base_url).ok().is_some_and(|url| {
        url.scheme() == "https"
            && url.host_str().is_some_and(|host| {
                host == "api.openai.com" || kodex_http_client::is_allowed_chatgpt_host(host)
            })
    })
}
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Kodex session. Per-turn values
    ...
    pub fn new(
        ...
        let kodex_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.kodex_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), kodex_api_key_env_enabled);
    ...
    pub(crate) fn with_session_context(
        ...
        self.kodex_responses_headers = kodex_responses_headers;
        self
    }

    fn prompt_cache_key(&self, responses_metadata: &KodexResponsesMetadata) -> String {
    ...
    fn responses_session_id(&self, metadata: &KodexResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "kodex.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub async fn summarize_memories(
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(KodexAuth::auth_mode),
                client_setup.api_auth.as_ref(),
    ...
    fn build_responses_compatibility_headers(
        &self,
        responses_metadata: &KodexResponsesMetadata,
    ) -> ApiHeaderMap {
    ...
    pub(crate) fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: kodex_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    fn responses_headers(&self, auth: Option<&KodexAuth>, model: &str) -> ApiHeaderMap {
        self.kodex_responses_headers
            .as_ref()
            .filter(|config| {
                config.model == model
                    && self.uses_kodex_backend(auth)
                    && self.state.provider.info().supports_kodex_backend_routes()
            })
            .map(|config| config.headers.clone())
            .unwrap_or_default()
    }

    fn uses_kodex_backend(&self, auth: Option<&KodexAuth>) -> bool {
        let provider = self.state.provider.info();
        auth.is_some_and(KodexAuth::uses_kodex_backend)
            && provider.is_openai()
    ...
    fn set_guardian_metadata(
        ...
        if !guardian_reviewer
            ...
            && matches!(
                auth,
                Some(KodexAuth::Chatgpt(_) | KodexAuth::ChatgptAuthTokens(_))
            )
            && self.uses_kodex_backend(auth)
            && self.state.provider.info().supports_kodex_backend_routes()
        {
    ...
    fn build_routing_hint_header(
        ...
        if !self.uses_kodex_backend(auth) {
            return None;
        }
    ...
    fn build_api_transport(
        ...
        let redirect_policy = if api_provider
            .headers
            .contains_key(kodex_model_provider::ACCOUNT_ROUTING_HEADER)
        {
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: kodex_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &KodexResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                kodex_login::default_client::default_headers(),
                /*turn_state*/ None,
    ...
    async fn build_websocket_headers(
        ...
        if let Some(routing_hint) = &responses_metadata.routing_hint {
            headers.insert(X_KODEX_ROUTING_HINT_HEADER, routing_hint.clone());
        }
        if let Some(header_value) = self.generate_attestation_header_for().await {
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&KodexAuth>) -> Compression {
    ...
    async fn stream_responses_api(
        ...
        loop {
                ...
                )
            {
                options
                    .extra_headers
                    .insert(X_KODEX_ROUTING_HINT_HEADER, header_value);
            }
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(X_KODEX_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
    ...
    /// Permanently disables WebSockets for this Kodex session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .get_or_insert_with(HashMap::new)
        .insert(
            X_KODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Kodex-specific conventions:
///
/// - `x-kodex-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-kodex-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-kodex-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_KODEX_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_KODEX_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: kodex_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let kodex_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = kodex_api::ResponseStream {
        rx_event,
...
/// the mapped `KodexErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_auth: SharedAuthProvider,
    auth_owner_generation: Option<u64>,
    responses_metadata: &'a KodexResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
...
async fn handle_unauthorized(
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Policy(error)) => Err(KodexErr::Fatal(error.to_string())),
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(KodexErr::RefreshTokenFailed(failed))
            }
