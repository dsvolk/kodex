...
use kodex_otel::StatsigMetricsSettings;
...
fn lock_sandbox_dir(
    ...
    // ProvisionOnly accepts another user's KODEX_HOME; keep its ACL mutation
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(kodex_home) = std::env::var("KODEX_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&kodex_home));
...
fn run_payload(payload: &Payload) -> Result<()> {
    let sbx_dir = sandbox_dir(&payload.kodex_home);
    ...
    if let Err(err) = &result {
        ...
        let write_report = match payload.mode {
            SetupMode::ProvisionOnly => serde_json::to_vec_pretty(&report)
                ...
                .and_then(|json| {
                    write_file_atomically(&setup_error_path(&payload.kodex_home), &json)
                }),
            SetupMode::Full | SetupMode::InteractiveProvision | SetupMode::ReadAclsOnly => {
                write_setup_error_report(&payload.kodex_home, &report)
            }
        };
...
fn lock_persistent_sandbox_dirs(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    lock_sandbox_dir(
        &sandbox_dir(&payload.kodex_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.kodex_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.kodex_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.kodex_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.kodex_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.kodex_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.kodex_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_kodex_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.kodex_home, &payload.command_cwd, root)?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.kodex`, and `.agents`.
        //
...
mod tests {
    ...
    pub(super) fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "KodexSandboxOffline",
            "online_username": "KodexSandboxOnline",
            "kodex_home": "C:\\kodex-home",
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let kodex_home = temp.path().join("kodex-home");
        let workspace = temp.path().join("workspace");
        fs::create_dir_all(&kodex_home).expect("create kodex home");
        fs::create_dir_all(&workspace).expect("create workspace");

        let sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &workspace)
    ...
    fn write_root_refresh_checks_each_sid() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &workspace)
            .expect("workspace sid");
        let other_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &other_root)
    ...
    fn deny_path_under_active_root_uses_only_matching_root_sid() {
        ...
        let stale_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &stale_root)
            .expect("stale sid");
        let active_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &active_root)
            .expect("active sid");
        let workspace_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &workspace)
            .expect("workspace sid");
        let caps = load_or_create_cap_sids(&kodex_home).expect("load caps");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let kodex_home = temp.path().join("kodex-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".kodex");
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &workspace)
            .expect("workspace sid");
        let nested_sid = workspace_write_cap_sid_for_root(&kodex_home, &workspace, &nested_root)
            .expect("nested sid");
