...
async fn code_mode_model_output_uses_structured_host_timing(
    ...
    let mut host = match transport {
        "grpc" => Some(
            Command::new(kodex_utils_cargo_bin::cargo_bin("kodex-code-mode-host")?)
    ...
    let mut builder = TestAppServer::builder()
        ...
        .with_json_logging("kodex_code_mode::timing=info,kodex_core::tools::parallel=info");
    ...
    let host_timing = app_server
        .wait_for_json_log_event("kodex.code_mode.host_timing")
        .await?;
...
async fn app_server_shares_flag_selected_grpc_code_mode_host_across_threads() -> Result<()> {
    let host_program = kodex_utils_cargo_bin::cargo_bin("kodex-code-mode-host")?;
    ...
    let original_config = std::fs::read_to_string(kodex_home.path().join("config.toml"))?;
