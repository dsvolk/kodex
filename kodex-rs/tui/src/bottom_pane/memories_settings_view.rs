...
impl MemoriesSettingsView {
    ...
    fn settings_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(
            Paragraph::new(Line::from(
                "Choose how Kodex uses and creates memories. Changes are saved to config.toml"
                    .dim(),
            ))
    ...
    fn reset_confirmation_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(
            Paragraph::new(Line::from(
                "This clears local memory files and rollout summaries for the current Kodex home."
                    .dim(),
            ))
            .wrap(Wrap { trim: false }),
        );
