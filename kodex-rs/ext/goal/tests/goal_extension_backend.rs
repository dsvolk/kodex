...
use kodex_extension_api::ThreadResumeInput;
use kodex_extension_api::ThreadStartInput;
use kodex_extension_api::ThreadStopInput;
...
use kodex_extension_api::TurnStopInput;
use kodex_goal_extension::GoalExtensionConfig;
use kodex_goal_extension::GoalObjectiveUpdate;
use kodex_goal_extension::GoalRuntimeHandle;
use kodex_goal_extension::GoalService;
use kodex_goal_extension::GoalSetRequest;
use kodex_goal_extension::GoalTokenBudgetUpdate;
use kodex_goal_extension::install_with_backend;
...
async fn spawned_descendant_usage_exhausts_root_goal_budget_once() -> anyhow::Result<()> {
    ...
    assert_eq!(kodex_state::ThreadGoalStatus::BudgetLimited, goal.status);
    Ok(())
}
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(kodex_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(kodex_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
        ...
        [(None, None, 3), (Some(2), None, 5), (None, Some(2), 4)]
    {
        ...
        for turn in 1..=blocking_turn {
            ...
            let expected = if turn == blocking_turn {
                kodex_state::ThreadGoalStatus::Blocked
            } else {
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(kodex_state::ThreadGoalStatus::Active, goal.status);
...
async fn update_goal_can_stop_and_accounts_final_progress() -> anyhow::Result<()> {
    for (status, token_budget, expected_status, state_status) in [
        ...
        (
            ...
            100,
            ThreadGoalStatus::Paused,
            kodex_state::ThreadGoalStatus::Paused,
        ),
        (
            ThreadGoalStatus::Paused,
            20,
...
async fn installed_tools(
    runtime: Arc<kodex_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: Arc<kodex_extension_api::ExtensionRegistry<()>>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish_with_outcome(
        ...
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = kodex_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall<'static> {
    ToolCall {
        ...
        tool_name: kodex_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        kodex_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        source: ToolCallSource::Direct,
        conversation_history: kodex_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<kodex_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    kodex_state::StateRuntime::init(
        kodex_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &kodex_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = kodex_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: kodex_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        kodex_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        kodex_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        kodex_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        kodex_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        kodex_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        kodex_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
