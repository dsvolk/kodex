...
pub(crate) struct TestKodexHelperPaths {
    pub(crate) kodex_exe: PathBuf,
    pub(crate) kodex_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_kodex_helper_paths() -> anyhow::Result<TestKodexHelperPaths> {
    let (helper_binary, kodex_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestKodexHelperPaths {
        kodex_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.kodex_exe);
...
impl ExecServerHarness {
    pub(crate) async fn start(mut command: Command) -> anyhow::Result<Self> {
        ...
        if !command
            .as_std()
            .get_envs()
            .any(|(key, value)| key == "KODEX_HOME" && value.is_some())
        {
            command.env("KODEX_HOME", kodex_home.path());
        }
    ...
    pub(crate) fn kodex_home(&self) -> &std::path::Path {
