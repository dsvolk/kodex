...
use crate::protocol_event::kodex_turn_trace_event;
...
pub const KODEX_ROLLOUT_TRACE_ROOT_ENV: &str = "KODEX_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `KODEX_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Kodex
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(KODEX_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Kodex turn lifecycle events from protocol lifecycle events.
    pub fn record_kodex_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            kodex_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Kodex did while executing that boundary.
    pub fn record_tool_call_event(&self, kodex_turn_id: impl Into<KodexTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(kodex_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_kodex_turn_id: impl Into<KodexTurnId>,
    ...
    pub fn record_kodex_turn_started(&self, kodex_turn_id: impl Into<KodexTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let kodex_turn_id = kodex_turn_id.into();
        context.append_with_context_best_effort(
            kodex_turn_id.clone(),
            RawTraceEventPayload::KodexTurnStarted {
                kodex_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Kodex turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        kodex_turn_id: KodexTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            kodex_turn_id: Some(kodex_turn_id),
        };
