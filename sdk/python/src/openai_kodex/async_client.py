...
_TURN_START_EXECUTOR = ThreadPoolExecutor(thread_name_prefix="kodex-turn-start")


class AsyncKodexClient:
    """Async wrapper around KodexClient using thread offloading."""

    def __init__(self, config: KodexConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = KodexClient(config=config)

    async def __aenter__(self) -> "AsyncKodexClient":
        """Start the Kodex process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Kodex process when leaving an async context."""
        await self.close()
