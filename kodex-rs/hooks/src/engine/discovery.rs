...
use kodex_config::TomlValue;
use kodex_config::version_for_toml;
use kodex_plugin::PluginHookSource;
use kodex_plugin::is_allowlisted_bundled_cleanup_hook;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_kodex_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_kodex_folder.clone())
...
fn append_matcher_groups(
    ...
    event_name: kodex_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        kodex_protocol::protocol::HookEventName::PreToolUse
                            | kodex_protocol::protocol::HookEventName::PostToolUse
                            | kodex_protocol::protocol::HookEventName::SessionStart
                            | kodex_protocol::protocol::HookEventName::UserPromptSubmit
                            | kodex_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/kodex/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.kodex/config.toml").abs();
        let dot_kodex_folder = test_path_buf("/tmp/worktree/.kodex").abs();
