...
pub use kodex_connectors::AppBranding;
pub use kodex_connectors::AppInfo;
pub use kodex_connectors::AppMetadata;
...
use kodex_mcp::ToolPluginContext;
use kodex_mcp::effective_mcp_servers;
use kodex_mcp::tool_plugin_context;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = kodex_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        kodex_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref().is_some_and(KodexAuth::uses_kodex_backend))
    {
        return Some(Vec::new());
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    mcp_servers.retain(|name, _| name == KODEX_APPS_MCP_SERVER_NAME);
    ...
    let kodex_apps_auth_manager =
        kodex_mcp::host_owned_kodex_apps_enabled(&mcp_config, auth.as_ref())
    ...
    let mcp_runtime = McpRuntime::new(McpRuntimeInput {
        ...
        runtime_context,
        kodex_apps_tools_cache: mcp_manager.kodex_apps_tools_cache(),
        tool_catalog_cache: mcp_manager.tool_catalog_cache(),
        kodex_apps_tools_cache_key: connector_runtime_context_key(auth.as_ref()),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: auth.clone(),
        auth_manager: kodex_apps_auth_manager,
    ...
    let refreshed_tools = if force_refetch {
        match mcp_runtime
            .latest_hard_refresh_kodex_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{KODEX_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let kodex_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(KODEX_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_runtime
            .latest_wait_for_server_ready(KODEX_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_runtime
                .latest_wait_for_server_ready(KODEX_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if kodex_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let account_id = auth.and_then(KodexAuth::get_account_id);
    let chatgpt_user_id = auth.and_then(KodexAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(KodexAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + kodex_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    kodex_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != KODEX_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(kodex_connectors::accessible::AccessibleConnectorTool {
    ...
    kodex_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &kodex_config::ConfigLayerStack,
    ...
    let app_reviewer = if server_name == KODEX_APPS_MCP_SERVER_NAME {
