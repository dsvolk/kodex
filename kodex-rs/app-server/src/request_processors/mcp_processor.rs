...
use kodex_mcp::McpServerSource;
use kodex_mcp::ReadResourceRequestParams;
...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = kodex_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        if matches!(server.auth, kodex_config::McpServerAuth::EmaAuth) {
        ...
        let discovered_scopes = if scopes.is_none() && server.scopes.is_none() {
            discover_supported_scopes(
                ...
                kodex_rmcp_client::OAuthDiscoveryTimeout::Requested,
                redirect_mode,
            )
            .await
        } else {
    ...
    async fn list_mcp_server_status_response(
        request_id: String,
        params: ListMcpServerStatusParams,
        mcp_config: kodex_mcp::McpConfig,
        auth: Option<KodexAuth>,
        runtime_context: McpRuntimeContext,
        mcp_manager: Arc<McpManager>,
        thread: Option<Arc<kodex_core::KodexThread>>,
        ...
        let snapshot = collect_mcp_server_status_snapshot_with_detail(
            ...
            request_id,
            runtime_context,
            mcp_manager.kodex_apps_tools_cache(),
    ...
    async fn read_mcp_resource(
        ...
        {
            if server != kodex_mcp::KODEX_APPS_MCP_SERVER_NAME
            ...
            {
                return Err(invalid_request(
                    "target requires kodex_apps, a connectorId, and a real linkId or null",
                ));
            }
            meta.insert(
        ...
        if let Some(thread_id) = thread_id {
            ...
            tokio::spawn(async move {
                let origin_call_id =
                    origin_call_id.filter(|_| server == kodex_mcp::KODEX_APPS_MCP_SERVER_NAME);
        ...
        let mcp_manager = self.thread_manager.mcp_manager();
        let mcp_config = mcp_manager.runtime_config(&config).await;
        let kodex_apps_tools_cache = mcp_manager.kodex_apps_tools_cache();
...
fn mcp_operation_error(error: anyhow::Error) -> JSONRPCErrorError {
    match kodex_rmcp_client::mcp_error(&error) {
