...
pub(crate) struct LocalAgentControl {
    ...
    /// `ThreadManagerState -> KodexThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl LocalAgentControl {
    ...
    pub(crate) async fn send_input(
            ...
            .await
        {
            ...
            Ok(TurnInputSubmission::NotSubmitted { reason }) => Err(KodexErr::InvalidRequest(
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> KodexResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: KodexResult<String>,
    ) -> KodexResult<String> {
        if result
            .as_ref()
            .is_err_and(|err| matches!(err.details(), KodexErrorDetails::InternalAgentDied))
        {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> KodexResult<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or_else(|| KodexErr::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> KodexResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
        let thread = state.get_thread(agent_id).await?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> KodexResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> KodexResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| KodexErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> KodexResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> KodexResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::KodexThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                kodex_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {
