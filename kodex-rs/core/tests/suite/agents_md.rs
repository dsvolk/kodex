...
use core_test_support::test_kodex::RecordingUserInstructionsProvider;
use core_test_support::test_kodex::TestKodex;
use core_test_support::test_kodex::TestKodexBuilder;
use core_test_support::test_kodex::executor_path_uri;
...
struct WarningInstructionsProvider {
    inner: KodexHomeUserInstructionsProvider,
...
async fn agents_instructions(mut builder: TestKodexBuilder) -> Result<String> {
...
pub(super) async fn submit_thread_turn(
    thread: &Arc<kodex_core::KodexThread>,
    prompt: &str,
) -> Result<()> {
    thread
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions =
        agents_instructions(test_kodex().with_workspace_setup(|cwd, fs| async move {
...
-> Result<()> {
    ...
    let error = error
        .replace(&canonical_home_path, "$KODEX_HOME")
        .replace(&home_path, "$KODEX_HOME");
    insta::assert_snapshot!(error, @"
    failed to load AGENTS.md instructions for environment `local`: failed to prepare fs sandbox: failed to prepare Seatbelt sandbox: writable root $KODEX_HOME/visualizations contains symlink component $KODEX_HOME/visualizations; symlinked writable roots are not supported.
    If this writable root is at or beneath KODEX_HOME and you trust its symlink targets, set `allow_symlinked_kodex_home = true` at the top level of `$KODEX_HOME/config.toml` (normally `~/.kodex/config.toml`) on the execution host, then restart Kodex or its executor. This opt-out trusts targets outside KODEX_HOME and targets changed between commands. It does not apply to other writable roots.
    ");
...
-> Result<()> {
    ...
    let provider = Arc::new(WarningInstructionsProvider {
        inner: KodexHomeUserInstructionsProvider::new(home.path().to_path_buf().abs()),
    ...
    let EventMsg::Error(error) = wait_for_event(&test.kodex, |event| {
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        KodexHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn thread_provider_composes_and_clears_only_its_instructions() -> Result<()> {
    ...
    let sources = vec![
        PathUri::from_abs_path(&fixture.test.config.kodex_home.join(GLOBAL_AGENTS_FILENAME)),
...
async fn isolated_guardian_keeps_applied_thread_instructions() -> Result<()> {
    ...
    let test = test_kodex()
        .with_config(|config| {
            config.permissions.approval_policy = kodex_core::config::Constrained::allow_any(
                kodex_protocol::protocol::AskForApproval::OnRequest,
            );
            config.approvals_reviewer = kodex_protocol::config_types::ApprovalsReviewer::AutoReview;
        })
        .build_with_auto_env(&server)
        .await?;
    let provider =
    ...
    let parent = test
        .thread_manager
        .start_thread(StartThreadOptions {
            environments: Some(test.kodex.environment_selections().await),
            thread_instructions_provider: Some(provider.clone()),
            ..StartThreadOptions::new(test.config.clone())
        })
        .await?;
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.kodex.instruction_sources().await, creation_sources);
...
async fn global_instruction_warnings_reappear_only_after_recovery() -> Result<()> {
    ...
    let mut builder = test_kodex().with_home(Arc::clone(&home));
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_event(
        &test.kodex,
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_kodex().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_kodex().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.kodex.instruction_sources().await,
    ...
    let mut resume_builder = test_kodex().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    assert_eq!(
        parent.kodex.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.kodex.ensure_rollout_materialized().await;
    parent.kodex.flush_rollout().await?;
    let rollout_path = parent.kodex.rollout_path().expect("rollout path");
    ...
    fork_config.kodex_self_exe = parent.config.kodex_self_exe.clone();
    fork_config
        .features
    ...
    let forked = parent
        ...
        .fork_thread(
            ...
            kodex_core::StartThreadOptions::new(fork_config),
            rollout_path,
        )
        .await?;
