...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            kodex_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            kodex_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            kodex_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            kodex_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[kodex_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
...
mod agent {
    ...
    pub(super) fn get_config(
        ...
        // Preserve the parent's explicit choice to skip Kodex-managed sandboxing.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<kodex_state::Stage1Output>,
        memory_root: kodex_utils_absolute_path::AbsolutePathBuf,
        version: MemoryVersion,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<kodex_otel::Timer>,
    ) {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[kodex_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
