# kodex-app-server-daemon

> `kodex-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`kodex-app-server-daemon` backs the machine-readable `kodex app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Kodex instances launched over SSH, including fresh developer
...
shorter `KODEX_HOME` to share the daemon; discovery does not trust a mutable alias.
...
An invocation that sets `KODEX_EXEC_SERVER_URL` skips implicit daemon attachment
...
```sh
kodex app-server daemon start
kodex app-server daemon restart
kodex app-server daemon update
kodex app-server daemon enable-remote-control
kodex app-server daemon disable-remote-control
kodex app-server daemon stop
kodex app-server daemon version
kodex app-server daemon bootstrap --remote-control
```
...
default. Edit `KODEX_HOME/app-server-daemon/settings.json` to change this:
...
not affect an explicit `kodex update` command or `daemon update`.
...
$HOME/.kodex/packages/standalone/current/kodex app-server daemon bootstrap --remote-control
```

On Windows, use a non-elevated PowerShell terminal whose host allows breakaway:

```powershell
irm https://chatgpt.com/kodex/install.ps1 | iex
$kodexHome = if ($env:KODEX_HOME) { $env:KODEX_HOME } else { Join-Path $HOME '.kodex' }
& "$kodexHome\packages\standalone\current\bin\kodex.exe" app-server daemon bootstrap --remote-control
```

`bootstrap` can use any complete CLI package. If no daemon package is installed,
it copies the invoking package into `KODEX_HOME/packages/app-server-daemon` and
...
It records the daemon settings under `KODEX_HOME/app-server-daemon/`, starts app-server as a
...
New daemons use `KODEX_HOME/packages/app-server-daemon/current/bin/kodex`
(`kodex.exe` on Windows). The package contains the executable and its helpers.
Daemon-only installer updates leave the user's CLI command and shell setup alone.

Previously launched legacy daemons retain `KODEX_HOME/packages/standalone/current`,
...
- if the updater was absent during a same-version binary replacement, a later
  ...
  executable identity; use `kodex app-server daemon restart` to refresh the server
...
Top-level `kodex remote-control start` enables and persists remote control for
the managed daemon, overriding a saved disabled value. It starts or bootstraps
the daemon as needed. Plain `kodex remote-control` runs a separate foreground
server and does not change daemon settings; `kodex remote-control stop` stops
...
All mutating lifecycle commands are serialized per `KODEX_HOME`, so a concurrent
...
The daemon stores its local state under `KODEX_HOME/app-server-daemon/`:
