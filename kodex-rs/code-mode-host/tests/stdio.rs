...
use kodex_code_mode::CodeModeSessionCellExecutionLimits;
use kodex_code_mode::CodeModeSessionDelegate;
use kodex_code_mode::CodeModeSessionProvider;
use kodex_code_mode::CodeModeToolKind;
use kodex_code_mode::ExecuteRequest;
use kodex_code_mode::FunctionCallOutputContentItem;
use kodex_code_mode::NotificationFuture;
use kodex_code_mode::ProcessOwnedCodeModeSessionProvider;
use kodex_code_mode::RuntimeResponse;
use kodex_code_mode::ToolDefinition;
use kodex_code_mode::ToolInvocationFuture;
use kodex_code_mode::WaitOutcome;
use kodex_code_mode::WaitRequest;
use kodex_code_mode::host::MAX_FRAME_BYTES;
...
async fn session_execution_limits_are_isolated_on_a_shared_process_host() {
    let provider: Arc<dyn CodeModeSessionProvider> =
        Arc::new(ProcessOwnedCodeModeSessionProvider::with_host_program(
            kodex_utils_cargo_bin::cargo_bin("kodex-code-mode-host").expect("host binary"),
        ));
