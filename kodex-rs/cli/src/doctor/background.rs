...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.kodex_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match kodex_app_server::app_server_control_socket_path(&config.kodex_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run kodex app-server daemon version for more details.");
    }
    check
}
...
async fn socket_status(socket_path: &Path) -> SocketStatus {
    ...
    match kodex_app_server_daemon::probe_app_server_version(socket_path).await {
...
mod tests {
    ...
    async fn test_config(kodex_home: PathBuf) -> Config {
    ...
    fn create_socket_placeholder(config: &Config) {
        let socket_path = kodex_app_server::app_server_control_socket_path(&config.kodex_home)
            .expect("socket path");
    ...
    async fn configured_updates_and_missing_updater_record_are_visible() {
        ...
        let details = check
            ...
            .map(|detail| {
                detail
                    .replace(&temp.path().display().to_string(), "KODEX_HOME")
                    .replace('\\', "/")
            })
            .collect::<Vec<_>>()
            .join("\n");
