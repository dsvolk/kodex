...
> **Privacy:** Rollout tracing is not telemetry. Kodex does **not** upload or
> report these traces; it writes local bundles only when
> `KODEX_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a Kodex session. It records raw runtime evidence into a local bundle on
...
Hot-path Kodex code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Kodex got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["kodex-core runtime"]
...
one from `KODEX_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Kodex session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `kodex debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Kodex rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
kodex debug trace-reduce <trace-bundle>
```

By default this writes `<trace-bundle>/state.json`. Rust callers can also call
`kodex_rollout_trace::replay_bundle` directly.
