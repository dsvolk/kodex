...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `kodex` to start a saved session, or `kodex resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let kodex_home = app_server.kodex_home_path(&self.config.kodex_home);
        match goal_files::objective_text_for_edit(app_server, kodex_home.as_ref(), &goal.objective)
            .await
        {
