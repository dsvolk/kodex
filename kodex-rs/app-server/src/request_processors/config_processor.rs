...
use kodex_app_server_protocol::BrowserUseOriginPolicy;
use kodex_app_server_protocol::BrowserUseRequirements;
use kodex_app_server_protocol::CliAuthCredentialsStoreMode;
use kodex_app_server_protocol::ClientResponsePayload;
use kodex_app_server_protocol::ComputerUseMacosRequirements;
use kodex_app_server_protocol::ComputerUseRequirements;
use kodex_app_server_protocol::ComputerUseWindowsExeRequirement;
use kodex_app_server_protocol::ComputerUseWindowsRequirements;
use kodex_app_server_protocol::ConfigBatchWriteParams;
use kodex_app_server_protocol::ConfigReadParams;
use kodex_app_server_protocol::ConfigReadResponse;
use kodex_app_server_protocol::ConfigRequirements;
use kodex_app_server_protocol::ConfigRequirementsReadResponse;
use kodex_app_server_protocol::ConfigValueWriteParams;
use kodex_app_server_protocol::ConfigWriteErrorCode;
use kodex_app_server_protocol::ConfigWriteResponse;
use kodex_app_server_protocol::ConfiguredHookHandler;
use kodex_app_server_protocol::ConfiguredHookMatcherGroup;
use kodex_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use kodex_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use kodex_app_server_protocol::FeedbackRequirements;
use kodex_app_server_protocol::InAppBrowserRequirements;
use kodex_app_server_protocol::JSONRPCErrorError;
use kodex_app_server_protocol::ManagedHooksRequirements;
use kodex_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use kodex_app_server_protocol::ModelsRequirements;
use kodex_app_server_protocol::NetworkDomainPermission;
use kodex_app_server_protocol::NetworkRequirements;
use kodex_app_server_protocol::NetworkUnixSocketPermission;
use kodex_app_server_protocol::NewThreadModelDefaults;
use kodex_app_server_protocol::SandboxMode;
use kodex_app_server_protocol::WindowsSandboxImplementation;
use kodex_config::ConfigRequirementsToml;
use kodex_config::HookEventsToml;
use kodex_config::HookHandlerConfig as CoreHookHandlerConfig;
use kodex_config::ManagedHooksRequirementsToml;
use kodex_config::MatcherGroup as CoreMatcherGroup;
use kodex_config::ResidencyRequirement as CoreResidencyRequirement;
use kodex_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use kodex_core::ThreadManager;
use kodex_features::Feature;
use kodex_features::canonical_feature_for_key;
use kodex_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<kodex_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = kodex_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_to_api(
    ...
    Some(ConfigRequirements {
        ...
        application: requirements.application.map(|application| {
            kodex_app_server_protocol::ApplicationRequirements {
                network: application.network.map(|network| {
                    kodex_app_server_protocol::ApplicationNetworkRequirements {
        ...
        cli_auth_credentials_store: requirements.cli_auth_credentials_store.map(
            |mode| match mode {
                kodex_config::types::AuthCredentialsStoreMode::File => {
                    CliAuthCredentialsStoreMode::File
                }
                kodex_config::types::AuthCredentialsStoreMode::Keyring => {
                    CliAuthCredentialsStoreMode::Keyring
                }
                kodex_config::types::AuthCredentialsStoreMode::Auto => {
                    CliAuthCredentialsStoreMode::Auto
                }
                kodex_config::types::AuthCredentialsStoreMode::Ephemeral => {
        ...
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(kodex_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(kodex_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            kodex_config::WindowsSandboxImplementationToml::Elevated => {
                                WindowsSandboxImplementation::Elevated
                            }
                            kodex_config::WindowsSandboxImplementationToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: kodex_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: kodex_config::BrowserUseRequirementsToml,
...
fn map_browser_use_origin_policy_to_api(
    policy: kodex_config::BrowserUseOriginPolicyToml,
...
fn map_allow_deny_requirement_to_api(
    requirement: kodex_config::AllowDenyRequirementToml,
) -> AllowDenyRequirement {
    match requirement {
        kodex_config::AllowDenyRequirementToml::Allow => AllowDenyRequirement::Allow,
        kodex_config::AllowDenyRequirementToml::Deny => AllowDenyRequirement::Deny,
    }
}

fn map_browser_use_access_approval_lifetime_to_api(
    lifetime: kodex_config::BrowserUseAccessApprovalLifetimeToml,
) -> BrowserUseAccessApprovalLifetime {
    match lifetime {
        kodex_config::BrowserUseAccessApprovalLifetimeToml::Turn => {
            BrowserUseAccessApprovalLifetime::Turn
        }
        kodex_config::BrowserUseAccessApprovalLifetimeToml::Thread => {
...
) -> kodex_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => kodex_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: kodex_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(kodex_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(kodex_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(kodex_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: kodex_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        kodex_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        kodex_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: kodex_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        kodex_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        kodex_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use kodex_app_server_protocol::FeedbackRequirements;
    use kodex_app_server_protocol::WindowsSandboxImplementation;
    use kodex_config::AllowDenyRequirementToml;
    use kodex_config::AutoReviewRequirementsToml;
    ...
    ) -> kodex_app_server_protocol::ConfigRequirements {
