...
use kodex_analytics::TurnResolvedConfigFact;
...
use kodex_extension_api::TurnInputContext;
use kodex_extension_api::TurnInputEnvironment;
use kodex_features::Feature;
use kodex_file_system::FindUpErrorPolicy;
use kodex_file_system::find_nearest_ancestor_with_markers;
...
use kodex_protocol::items::PlanItem;
...
use kodex_protocol::protocol::AgentMessageContentDeltaEvent;
use kodex_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use kodex_protocol::protocol::PlanDeltaEvent;
use kodex_protocol::protocol::ReasoningContentDeltaEvent;
use kodex_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use kodex_skills::ToolMentionKind;
use kodex_skills::app_id_from_path;
use kodex_skills::build_skill_name_counts;
use kodex_skills::collect_explicit_skill_mentions;
use kodex_skills::tool_kind_for_path;
use kodex_skills_extension::HostSkillPrompts;
use kodex_skills_extension::InjectedHostSkillPrompts;
use kodex_thread_store::PersistContext;
use kodex_tools::DiscoverableTool;
use kodex_tools::ToolName;
use kodex_tools::filter_request_plugin_install_discoverable_tools_for_client;
use kodex_utils_path_uri::PathUri;
use kodex_utils_stream_parser::AssistantTextChunk;
use kodex_utils_stream_parser::AssistantTextStreamParser;
use kodex_utils_stream_parser::ProposedPlanSegment;
use kodex_utils_stream_parser::extract_proposed_plan_text;
use kodex_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "kodex_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        ...
        .await;
        if matches!(err.details(), KodexErrorDetails::TurnAborted) {
            return Err(err);
        }
        if matches!(err.details(), KodexErrorDetails::ToolCollision(_)) {
            return Err(err);
        }
        let error = err.to_kodex_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), KodexErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(
                &sess,
                &turn_context,
                &turn_context.capture_current_model_info(),
    ...
    if crate::guardian::is_basic_session_source(&turn_context.session_source)
        && let Err(error) = crate::guardian::finalize_guardian_input(
            ...
            &first_step_context,
            &mut input,
            kodex_guardian_context::HistoryTruncation::Preserve,
        )
        .await
    {
        ...
        if !matches!(error.details(), KodexErrorDetails::ContextWindowExceeded)
        ...
        crate::guardian::finalize_guardian_input(
            ...
            &first_step_context,
            &mut input,
            kodex_guardian_context::HistoryTruncation::Allow,
        )
        .await?;
    }
    ...
    // Although from the perspective of kodex.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: KodexResult<_> = async {
            ...
            let responses_metadata = sess
                .responses_metadata(step_context.as_ref(), KodexResponsesRequestKind::Turn)
                .await;
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if !needs_follow_up {
                        ...
                        .await
                    {
                        if matches!(
                            err.details(),
                            KodexErrorDetails::Interrupted | KodexErrorDetails::TurnAborted
                        ) {
                            return Err(err);
                        }
            ...
            Err(err)
                if matches!(err.details(), KodexErrorDetails::ContextWindowExceeded)
            ...
            {
                sess.track_turn_kodex_error(turn_context.as_ref(), &kodex_error);
                let error = KodexErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
            ...
            Err(e) => {
                ...
                let error = e.to_kodex_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_kodex_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &kodex_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = kodex_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
    ...
    sess.services
        ...
        .track_turn_resolved_config(TurnResolvedConfigFact {
            ...
            guardian_v2_enabled: sess
                .services
                .thread_extension_data
                .get::<kodex_extension_api::GuardianV2Enabled>()
                .is_some(),
...
///
/// Returns `None` when the active authentication does not use the Kodex backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> KodexResult<Option<Arc<StepContext>>> {
    let uses_kodex_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(kodex_login::AuthManager::current_auth_uses_kodex_backend);
    if !uses_kodex_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = kodex_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<kodex_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &KodexResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> KodexResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                KodexErrorDetails::UsageLimitReached(e) => {
...
pub(crate) async fn built_tools(
    sess: &Session,
    turn_context: &TurnContext,
    model_info: &kodex_protocol::openai_models::ModelInfo,
    environments: &TurnEnvironmentSnapshot,
    mcp: &Arc<kodex_mcp::McpBinding>,
    step_store: &ExtensionData,
    prepared_recommendations: PreparedToolRecommendations,
) -> KodexResult<Arc<ToolRouter>> {
...
pub(super) fn agent_message_text(item: &kodex_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            kodex_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: kodex_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(kodex_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    step_context: &StepContext,
    turn_store: &kodex_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> KodexResult<SamplingRequestResult> {
    let turn_context = Arc::clone(&step_context.turn);
    feedback_tags!(
    ...
    let outcome: KodexResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            kodex.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            kodex.usage.reasoning_output_tokens = field::Empty,
            kodex.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(kodex_async_utils::CancelErr::Cancelled) => {
                break Err(KodexErr::TurnAborted);
            }
        };
        ...
        match event {
            ResponseEvent::Created { response_id } => {
                if let Some(response_id) = response_id {
                    turn_context
                        .extension_data
                        .insert(kodex_api::ResponseId(response_id));
                }
            }
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(kodex_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![kodex_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        kodex_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
