...
use kodex_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn inter_agent_communication(
    ...
    start_options: kodex_protocol::turn_input::TurnStartOptions,
) {
...
pub async fn resolve_elicitation(
    ...
    decision: kodex_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        kodex_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        kodex_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        kodex_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
...
async fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor
            .on_thread_stop(kodex_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Kodex instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(kodex_rollout_trace::RolloutStatus::Completed);
    true
}
            ...
            match sub.op {
                ...
                Op::SuspendTurnAndShutdown { reply } => {
                    ...
                    let should_exit = matches!(
                        &result,
                        Ok(kodex_protocol::turn_input::SuspendTurnOutcome::Suspended { .. })
                    );
                    let _ = reply.send(result);
                    should_exit
                }
