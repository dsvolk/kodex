...
use crate::events::KodexAppMentionedEventRequest;
use crate::events::KodexAppServerClientMetadata;
use crate::events::KodexAppUsedEventRequest;
use crate::events::KodexAppUsedMetadata;
use crate::events::KodexCollabAgentToolCallEventParams;
use crate::events::KodexCollabAgentToolCallEventRequest;
...
use crate::events::KodexControlToolCallEventParams;
use crate::events::KodexControlToolCallEventRequest;
use crate::events::KodexDynamicToolCallEventParams;
use crate::events::KodexDynamicToolCallEventRequest;
use crate::events::KodexFileChangeEventParams;
use crate::events::KodexFileChangeEventRequest;
use crate::events::KodexGoalEventRequest;
use crate::events::KodexHookRunEventRequest;
use crate::events::KodexImageGenerationEventParams;
use crate::events::KodexImageGenerationEventRequest;
use crate::events::KodexMcpToolCallEventParams;
use crate::events::KodexMcpToolCallEventRequest;
use crate::events::KodexOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::KodexOnboardingExternalAgentImportCompleteMetadata;
use crate::events::KodexOnboardingExternalAgentImportFailureEventRequest;
use crate::events::KodexOnboardingExternalAgentImportFailureMetadata;
use crate::events::KodexPluginEventRequest;
use crate::events::KodexPluginInstallFailedEventRequest;
use crate::events::KodexPluginInstallFailedMetadata;
use crate::events::KodexPluginInstallRequestedEventRequest;
use crate::events::KodexPluginMeasurementEventParams;
use crate::events::KodexPluginMeasurementEventRequest;
use crate::events::KodexPluginUsedEventRequest;
use crate::events::KodexReviewEventParams;
use crate::events::KodexReviewEventRequest;
use crate::events::KodexRuntimeMetadata;
use crate::events::KodexToolItemEventBase;
use crate::events::KodexTurnEventParams;
use crate::events::KodexTurnEventRequest;
use crate::events::KodexTurnSteerEventParams;
use crate::events::KodexTurnSteerEventRequest;
use crate::events::KodexWebSearchEventParams;
use crate::events::KodexWebSearchEventRequest;
...
use crate::events::kodex_app_metadata;
use crate::events::kodex_artifact_operation_event_request;
use crate::events::kodex_compaction_event_params;
use crate::events::kodex_goal_event_params;
use crate::events::kodex_hook_run_metadata;
use crate::events::kodex_plugin_install_requested_metadata;
use crate::events::kodex_plugin_metadata;
use crate::events::kodex_plugin_used_metadata;
...
use crate::facts::KodexGoalEvent;
...
use crate::facts::TurnKodexError;
use crate::facts::TurnKodexErrorFact;
...
use kodex_app_server_protocol::ClientResponse;
use kodex_app_server_protocol::CollabAgentStatus;
use kodex_app_server_protocol::CollabAgentTool;
use kodex_app_server_protocol::CollabAgentToolCallStatus;
use kodex_app_server_protocol::CommandAction;
use kodex_app_server_protocol::CommandExecutionApprovalDecision;
use kodex_app_server_protocol::CommandExecutionApprovalKind;
use kodex_app_server_protocol::CommandExecutionSource;
use kodex_app_server_protocol::CommandExecutionStatus;
use kodex_app_server_protocol::DynamicToolCallOutputContentItem;
use kodex_app_server_protocol::DynamicToolCallStatus;
use kodex_app_server_protocol::FileChangeApprovalDecision;
use kodex_app_server_protocol::GuardianApprovalReviewAction;
use kodex_app_server_protocol::GuardianApprovalReviewStatus;
use kodex_app_server_protocol::InitializeParams;
use kodex_app_server_protocol::KodexErrorInfo;
use kodex_app_server_protocol::McpToolCallStatus;
use kodex_app_server_protocol::NetworkPolicyRuleAction;
use kodex_app_server_protocol::PatchApplyStatus;
use kodex_app_server_protocol::PatchChangeKind;
use kodex_app_server_protocol::RequestId;
use kodex_app_server_protocol::RequestPermissionProfile;
use kodex_app_server_protocol::ServerNotification;
use kodex_app_server_protocol::ServerRequest;
use kodex_app_server_protocol::ServerResponse;
use kodex_app_server_protocol::SubAgentActivityKind;
...
use kodex_app_server_protocol::WebSearchAction;
...
use kodex_protocol::items::is_safe_plugin_relative_path;
...
use kodex_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use kodex_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
use sha1::Digest;
...
struct ConnectionState {
    app_server_client: KodexAppServerClientMetadata,
    runtime: KodexRuntimeMetadata,
}
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a KodexCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a KodexGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a kodex_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<KodexErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    kodex_error: Option<TurnKodexError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::ThreadHintStatus(input) => {
                    if let Some((connection, thread, metadata)) =
                        self.thread_context_or_warn(AnalyticsDropSite {
                            event_name: "kodex_thread_hint_status",
                            thread_id: &input.thread_id,
                            turn_id: None,
                            review_id: None,
                            item_id: None,
                        })
                    {
                        out.push(TrackEventRequest::ThreadHintStatus(Box::new(
                            crate::thread_hint::ThreadHintStatusEventRequest {
                                event_type: "kodex_thread_hint_status",
                ...
                CustomAnalyticsFact::GuardianV2(input) => {
                    let event_type = match &input.kind {
                        GuardianV2EventKind::Classification { .. } => {
                            "kodex_guardian_v2_classification"
                        }
                        GuardianV2EventKind::FastDecision { .. } => {
                            "kodex_guardian_v2_fast_decision"
                        }
                    };
                    if let Some((connection, thread, metadata)) =
                        self.thread_context_or_warn(AnalyticsDropSite {
                ...
                CustomAnalyticsFact::TurnKodexError(input) => {
                    self.ingest_turn_kodex_error(*input);
                }
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            kodex_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
    ...
    fn ingest_code_mode_tool_call(
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(KodexDynamicToolCallEventRequest {
                    event_type: "kodex_dynamic_tool_call_event",
                    event_params: KodexDynamicToolCallEventParams {
                        base,
    ...
    fn ingest_control_tool_call(
        ...
        base.cell_id = cell_id;
        let event = TrackEventRequest::ControlToolCall(KodexControlToolCallEventRequest {
            event_type: "kodex_control_tool_call_event",
            event_params: KodexControlToolCallEventParams {
                base,
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "kodex_guardian_review",
    ...
    fn ingest_turn_kodex_error(&mut self, input: TurnKodexErrorFact) {
        let TurnKodexErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.kodex_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = kodex_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(KodexAppMentionedEventRequest {
                event_type: "kodex_app_mentioned",
                event_params,
            })
        }));
    }
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = KodexAppUsedMetadata {
            app: kodex_app_metadata(&tracking, app),
            voice_session_id: self
                .turns
                .get(&tracking.turn_id)
                .and_then(|turn| turn.voice_session_id.clone()),
        ...
        out.push(TrackEventRequest::AppUsed(KodexAppUsedEventRequest {
            event_type: "kodex_app_used",
            event_params,
        }));
    }
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(KodexHookRunEventRequest {
            event_type: "kodex_hook_run",
            event_params: kodex_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(KodexPluginUsedEventRequest {
            event_type: "kodex_plugin_used",
            event_params: kodex_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            KodexPluginInstallRequestedEventRequest {
                event_type: "kodex_plugin_install_requested",
                event_params: kodex_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = KodexPluginEventRequest {
            event_type: plugin_state_event_type(state),
            event_params: kodex_plugin_metadata(plugin),
        };
    ...
    fn ingest_plugin_install_failed(
        ...
        out.push(TrackEventRequest::PluginInstallFailed(
            KodexPluginInstallFailedEventRequest {
                event_type: "kodex_plugin_install_failed",
                event_params: KodexPluginInstallFailedMetadata {
                    plugin: kodex_plugin_metadata(plugin),
                    source,
                    error_type,
                    sub_error_type,
                },
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            KodexOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "kodex_onboarding_external_agent_import_complete",
                event_params: KodexOnboardingExternalAgentImportCompleteMetadata {
    ...
    fn ingest_external_agent_config_import_failure(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportFailure(
            KodexOnboardingExternalAgentImportFailureEventRequest {
                event_type: "kodex_onboarding_external_agent_import_failure",
                event_params: KodexOnboardingExternalAgentImportFailureMetadata {
                    import_id: input.import_id,
                    source: input.source,
                    provider_id: input.provider_id,
                    item_type: input.item_type,
        ...
        match notification {
            ServerNotification::ThreadArchived(notification) => {
                out.push(TrackEventRequest::ThreadArchive(ThreadArchiveEvent {
                    event_type: "kodex_thread_archive_event",
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.kodex_error_info),
    ...
    fn ingest_plugin_measurements(
        ...
        out.extend(
            rows.into_iter()
                ...
                .map(|row| {
                    TrackEventRequest::PluginMeasurement(KodexPluginMeasurementEventRequest {
                        event_type: "kodex_plugin_measurement_event",
                        event_params: KodexPluginMeasurementEventParams {
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: kodex_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
        ...
        let is_worktree =
            kodex_git_utils::repository_identity(thread.cwd.as_path()).and_then(|_| {
                kodex_git_utils::get_git_repo_root(thread.cwd.canonicalize().ok()?.as_path())
        ...
        out.push(TrackEventRequest::ThreadInitialized(
            ThreadInitializedEvent {
                event_type: "kodex_thread_initialized",
    ...
    fn ingest_compaction(&mut self, input: KodexCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            KodexCompactionEventRequest {
                event_type: "kodex_compaction_event",
                event_params: kodex_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: KodexGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(KodexGoalEventRequest {
            event_type: "kodex_goal_event",
            event_params: kodex_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: kodex_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(KodexTurnSteerEventRequest {
            event_type: "kodex_turn_steer_event",
            event_params: KodexTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(KodexReviewEventRequest {
            event_type: "kodex_review_event",
            event_params: KodexReviewEventParams {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(KodexTurnEventRequest {
            event_type: "kodex_turn_event",
            event_params: kodex_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
...
fn tool_event_base_mut(event: &mut TrackEventRequest) -> Option<&mut KodexToolItemEventBase> {
    match event {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                KodexCommandExecutionEventRequest {
                    event_type: "kodex_command_execution_event",
                    event_params: KodexCommandExecutionEventParams {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(KodexFileChangeEventRequest {
                event_type: "kodex_file_change_event",
                event_params: KodexFileChangeEventParams {
                    base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                KodexMcpToolCallEventRequest {
                    event_type: "kodex_mcp_tool_call_event",
                    event_params: KodexMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                KodexDynamicToolCallEventRequest {
                    event_type: "kodex_dynamic_tool_call_event",
                    event_params: KodexDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                KodexCollabAgentToolCallEventRequest {
                    event_type: "kodex_collab_agent_tool_call_event",
                    event_params: KodexCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(KodexWebSearchEventRequest {
                event_type: "kodex_web_search_event",
                event_params: KodexWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                KodexImageGenerationEventRequest {
                    event_type: "kodex_image_generation_event",
                    event_params: KodexImageGenerationEventParams {
                        base,
...
fn file_change_counts(changes: &[kodex_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn accepted_line_event_input(
    ...
    Some((
        AcceptedLineFingerprintEventInput {
            event_type: "kodex.accepted_line_fingerprints",
            turn_id: turn_id.to_string(),
            thread_id,
            product_surface: Some("kodex".to_string()),
...
fn kodex_turn_event_params(
    app_server_client: KodexAppServerClientMetadata,
    runtime: KodexRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> KodexTurnEventParams {
    let (
        Some(thread_id),
    ...
    let kodex_error = turn_state.kodex_error.as_ref();
    KodexTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        kodex_turn_source: turn_metadata.kodex_turn_source(),
        app_server_client,
        runtime,
        submission_type,
        ephemeral,
        ...
        kodex_error_kind: kodex_error.map(|error| error.kind),
        kodex_error_http_status_code: kodex_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: kodex_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        kodex_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        kodex_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        kodex_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        kodex_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.kodex/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(
