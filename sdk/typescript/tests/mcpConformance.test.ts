...
import { kodexExecPath } from "./testKodex";
...
function conformanceDirectory(): string {
  ...
  for (const candidate of candidates) {
    if (existsSync(path.join(candidate, "run_kodex_compliance.py"))) {
      return candidate;
    }
  }
...
describe("MCP client conformance", () => {
  it(
    ...
    () => {
      ...
      const kodexBinary = requireFile(kodexExecPath, "built Kodex binary");
      ...
      const reportPath = path.join(reportDirectory, `kodex-mcp-conformance-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          path.join(directory, "run_kodex_compliance.py"),
          kodexBinary,
          "--mode",
          "all",
          "--transport",
  ...
  it(
    ...
    () => {
      ...
      const reportDirectory = process.env.RUNNER_TEMP ?? os.tmpdir();
      const reportPath = path.join(reportDirectory, `kodex-mcp-review-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          reviewer,
