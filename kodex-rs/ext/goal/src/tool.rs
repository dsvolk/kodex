...
use kodex_extension_api::JsonToolOutput;
use kodex_extension_api::ToolCall;
use kodex_extension_api::ToolExecutor;
use kodex_extension_api::ToolName;
use kodex_extension_api::ToolOutput;
use kodex_extension_api::ToolSpec;
use kodex_protocol::ThreadId;
use kodex_protocol::protocol::ThreadGoal;
use kodex_protocol::protocol::ThreadGoalStatus;
use kodex_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    ...
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<kodex_state::StateRuntime>,
...
impl<'call> ToolExecutor<ToolCall<'call>> for GoalToolExecutor {
    ...
    ) -> kodex_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
        Box::pin(async move {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => kodex_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked | ThreadGoalStatus::Paused => {
                    kodex_state::GoalAccountingMode::ActiveOrStopped
                }
    ...
    async fn account_active_goal_progress(
        &self,
        mode: kodex_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            kodex_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            kodex_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<kodex_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &kodex_state::StateRuntime,
    thread_id: ThreadId,
    goal: &kodex_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: kodex_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: kodex_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> kodex_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => kodex_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => kodex_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => kodex_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => kodex_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => kodex_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => kodex_state::ThreadGoalStatus::Complete,
    }
}
