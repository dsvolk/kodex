...
async fn signed_image_renewal_preserves_live_mcp_and_plugin_skills() -> Result<()> {
    ...
    for (path, contents) in [
        (".kodex-plugin/plugin.json", r#"{"name":"demo-plugin"}"#),
    ...
    let warm_skills = list_plugin_skills(&mut app_server, kodex_home.path()).await?;
    ...
    assert_eq!(
        list_plugin_skills(&mut app_server, kodex_home.path()).await?,
        warm_skills
    );
    ...
    assert!(
        list_plugin_skills(&mut app_server, kodex_home.path())
            .await?
            .is_empty()
    );
    read_mcp_skill_resource(&mut app_server, &thread.id).await?;
