...
use kodex_plugin::PluginLoadOutcome;
...
fn synced_plugin_root(kodex_home: &Path, marketplace: &str, plugin_name: &str) -> AbsolutePathBuf {
    let synced_root = curated_plugins_repo_path(kodex_home);
    match marketplace {
    ...
    let root = PluginStore::new(kodex_home.to_path_buf()).plugin_root(
        &plugin_id,
...
fn cached_remote_plugin_root(kodex_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    let root = PluginStore::new(kodex_home.to_path_buf()).plugin_root(&plugin_id, "1.2.3");
    fs::create_dir_all(root.as_path()).expect("create cached remote plugin root");
    root
}
fn installed_remote_plugin_root(kodex_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let root = cached_remote_plugin_root(kodex_home, plugin_name);
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    PluginStore::new(kodex_home.to_path_buf())
...
fn resolves_primary_runtime_scripts_from_the_installed_plugin_cache() {
    ...
    fs::create_dir_all(source_root.join(".kodex-plugin")).expect("create manifest directory");
    fs::write(
...
fn roots_for(kodex_home: &Path, plugins: Vec<LoadedPlugin>) -> TrustedPluginRoots {
...
fn recognizes_windows_executor_plugin_cache_root() {
    ...
    let script = PathUri::parse(
        "file:///C:/Users/user/.kodex/plugins/cache/openai-primary-runtime/presentations/0.1.29/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
    ...
    let wrong_version = PathUri::parse(
        "file:///C:/Users/user/.kodex/plugins/cache/openai-primary-runtime/presentations/0.1.28/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
...
fn assert_invalid_metrics_manifest(kodex_home: &Path, root: &AbsolutePathBuf, manifest: &str) {
...
fn assert_untrusted(kodex_home: &Path, config_name: &str, root: &Path) {
    assert!(
        roots_for(kodex_home, vec![loaded_plugin(config_name, root, ENABLED)])
            .roots
            .is_empty()
    );
}
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.kodex/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}
...
fn executor_cache_identity_obeys_windows_and_posix_case_conventions() {
    ...
    for prefix in ["file:///C:/Users/user/.kodex", "file://server/share/.kodex"] {
    ...
    for script in [
        format!(
            "file:///home/user/.kodex/PLUGINS/CACHE/OPENAI-CURATED-REMOTE/SITES/2.0.0-rc1/{REFERENCE_HELPER}"
        ),
        format!(
            "file:///home/user/.kodex/plugins/cache/openai-curated-remote/SITES/2.0.0-RC1/{REFERENCE_HELPER}"
        ),
        format!(
            "file:///C:/Users/user/.kodex/PLUGINS/CACHE/OPENAI-CURATED-REMOTE/SITES/2.0.0-rc2/{REFERENCE_HELPER}"
        ),
        format!(
            "file:///C:/Users/user/.kodex/PLUGINS/CACHE/OPENAI-CURATED-REMOTE/OTHER/2.0.0-rc1/{REFERENCE_HELPER}"
        ),
    ] {
    ...
    for version in ["2.0.0-RC1", "2.0.0-rc1"] {
        let script = PathUri::parse(&format!(
            "file:///home/user/.kodex/plugins/cache/openai-curated-remote/sites/{version}/{REFERENCE_HELPER}"
        ))
        .expect("POSIX script");
        let target =
...
async fn measurement_target_rejects_inline_commands_and_local_versions() -> anyhow::Result<()> {
    ...
    for args in [&["node", "-e", "42"][..], &["sh", REFERENCE_HELPER][..]] {
        ...
        let fs = kodex_exec_server::LOCAL_FS.as_ref();
        assert_eq!(
