...
pub struct LoaderOverrides {
    /// Optional configuration file supplied with the installed Kodex package.
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("kodex-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, kodex_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.kodex/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_kodex_folder } => Some(dot_kodex_folder.clone()),
    ...
    /// Returns the `.kodex/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$KODEX_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<()> {
    ...
    // user config.
    let mut previous_project_dot_kodex_folder: Option<&AbsolutePathBuf> = None;
    for layer in layers {
        if let ConfigLayerSource::Project {
            dot_kodex_folder: current_project_dot_kodex_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_kodex_folder {
                ...
                if previous == current_project_dot_kodex_folder
                    || !current_project_dot_kodex_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_kodex_folder = Some(current_project_dot_kodex_folder);
        }
    }
