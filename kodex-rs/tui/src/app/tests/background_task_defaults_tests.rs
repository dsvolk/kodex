...
async fn command_center_new_reads_server_defaults_for_actual_destination() -> Result<()> {
    ...
    for (mode, explicit_cwd, launch_override, expected_cwd, expected_model) in [
        ...
        std::fs::create_dir(destination.path().join(".kodex"))?;
        std::fs::write(
            destination.path().join(".kodex/config.toml"),
        ...
        app.config = ConfigBuilder::default()
            .kodex_home(client_home.path().to_path_buf())
        ...
        server_config.kodex_home = server_home.path().to_path_buf().abs();
    ...
    for _ in 0..2 {
        ...
        assert_eq!(
            ...
            (
                ...
                kodex_protocol::protocol::AskForApproval::UnlessTrusted,
...
async fn command_center_new_restores_blank_drafts_and_builtin_permissions() -> Result<()> {
    ...
    for profile_id in [":read-only", ":workspace"] {
        ...
        while let Ok(event) = events.try_recv() {
            if matches!(event, AppEvent::KodexOp(_)) {
