...
fn bundle_layers_resolve_paths_and_requirements_for_the_execution_host() {
    ...
    let executor_kodex_home = AbsolutePathBuf::from_absolute_path(executor_home.join(".kodex"))
        .expect("absolute executor Kodex home");
    let bundle = CloudConfigBundle {
        config_toml: CloudConfigTomlBundle {
            enterprise_managed: vec![CloudConfigFragment {
                id: "config".to_string(),
...
"#
    ...
    let (config, requirements) = AbsolutePathBufGuard::with_home_directory(&executor_home, || {
        let layers =
            CloudConfigBundleLayers::from_bundle_strict_config(bundle, &executor_kodex_home)
        ...
        Some(SandboxWorkspaceWrite {
            writable_roots: vec![
                ...
                AbsolutePathBuf::from_absolute_path(
                    executor_kodex_home.as_path().join("relative-root"),
                )
