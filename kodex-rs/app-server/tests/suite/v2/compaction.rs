...
use kodex_app_server_protocol::ResponseUsageMetadata;
...
async fn thread_compact_start_triggers_compaction_and_returns_empty_response() -> Result<()> {
    ...
    compaction_config(&server.uri(), /*auto_compact_limit*/ 1_000_000).write(kodex_home.path())?;
...
async fn thread_compact_start_rejects_invalid_thread_id() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let kodex_home = TempDir::new()?;
    compaction_config(&server.uri(), AUTO_COMPACT_LIMIT).write(kodex_home.path())?;
