...
fn evicted_voice_delegation_marker_still_suppresses_private_replay() {
    ...
    for notification in [
        ServerNotification::ReasoningSummaryPartAdded(
            kodex_app_server_protocol::ReasoningSummaryPartAddedNotification {
                thread_id: "thread".into(),
                turn_id: "voice-turn".into(),
                item_id: "reasoning".into(),
                summary_index: 0,
        ...
        ServerNotification::ReasoningSummaryTextDelta(
            kodex_app_server_protocol::ReasoningSummaryTextDeltaNotification {
                thread_id: "thread".into(),
                turn_id: "voice-turn".into(),
                item_id: "reasoning".into(),
        ...
        ServerNotification::ReasoningTextDelta(
            kodex_app_server_protocol::ReasoningTextDeltaNotification {
                thread_id: "thread".into(),
                turn_id: "voice-turn".into(),
                item_id: "reasoning".into(),
...
fn snapshot_keeps_typed_output_before_voice_handoff_in_the_same_turn() {
    let commentary = |id: &str, text: &str| ThreadItem::AgentMessage {
        id: id.into(),
        text: text.into(),
        phase: Some(kodex_protocol::models::MessagePhase::Commentary),
        questions: None,
        memory_citation: None,
        delivery: None,
    };
...
fn buffered_replay_preserves_text_before_interactive_requests() {
    ...
    store.push_notification(delta("thread", "turn", "answer"));
    store.push_request(
        kodex_app_server_protocol::ServerRequest::ToolRequestUserInput {
            request_id: kodex_app_server_protocol::RequestId::Integer(1),
            params: kodex_app_server_protocol::ToolRequestUserInputParams {
                thread_id: "thread".into(),
                turn_id: "turn".into(),
                item_id: "tool".into(),
                questions: Vec::new(),
...
async fn misalignment_buffered_replay_preserves_input_after_continuation() {
    ...
    let error = AppServerTurnError {
        ...
        kodex_error_info: Some(AppServerKodexErrorInfo::MisalignmentPolicyViolation),
    ...
    let failed_turn = Turn {
        ...
        items: vec![ThreadItem::McpToolCall {
            ...
            server: "sample".into(),
            tool: "inspect".into(),
            status: kodex_app_server_protocol::McpToolCallStatus::InProgress,
    ...
    let events = [
        ...
        ServerNotification::Error(kodex_app_server_protocol::ErrorNotification {
            thread_id: thread_id.to_string(),
        ...
        (
            ...
            ServerNotification::Error(kodex_app_server_protocol::ErrorNotification {
                ...
                error: AppServerTurnError {
                    kodex_error_info: Some(AppServerKodexErrorInfo::BadRequest),
                    ..error
                },
                will_retry: false,
            }),
