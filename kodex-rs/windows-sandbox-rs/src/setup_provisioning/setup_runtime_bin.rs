...
pub(super) fn ensure_kodex_app_runtime_paths_readable(
    ...
    if let Some(local_app_data) = local_app_data {
        let runtime_root = local_app_data.join("OpenAI").join("Kodex").join("runtimes");
...
fn runtime_paths(local_app_data: Option<PathBuf>, user_profile: Option<PathBuf>) -> Vec<PathBuf> {
    ...
    if let Some(local_app_data) = local_app_data {
        let kodex_root = local_app_data.join("OpenAI").join("Kodex");
        runtime_paths.push(kodex_root);
    }
    // The managed primary runtime is installed outside the LocalAppData runtime roots.
    if let Some(user_profile) = user_profile {
        runtime_paths.push(user_profile.join(".cache").join("kodex-runtimes"));
    }
