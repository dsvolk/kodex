...
use kodex_protocol::realtime::BemItemPresentation as CoreBemItemPresentation;
use kodex_protocol::realtime::RealtimeItem as CoreRealtimeItem;
use kodex_protocol::realtime::RealtimeItemContent as CoreRealtimeItemContent;
use kodex_protocol::realtime::RealtimeSessionOutcome as CoreRealtimeSessionOutcome;
use kodex_protocol::realtime::RealtimeTranscriptRole as CoreRealtimeTranscriptRole;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Kodex response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Routes any transcript tail remaining at session end through Kodex. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub kodex_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `kodex_responses_as_items`.
    /// Optional prefix added to automatic Kodex response items when `kodexResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub kodex_response_item_prefix: Option<String>,
    /// Selects how automatic Kodex responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub kodex_response_handoff_mode: Option<KodexResponseHandoffMode>,
    ...
    /// Set to false to start without Kodex's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
    ...
    /// Developer instructions given to the backing Kodex model when this realtime session starts.
    #[ts(optional = nullable)]
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to the backing Kodex model when this realtime session ends.
    #[ts(optional = nullable)]
