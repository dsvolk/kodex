...
use kodex_client::RetryOperation;
...
pub(crate) async fn handle_response_stream_error(
    ...
    if turn_context
        ...
        && matches!(err.details(), KodexErrorDetails::ConnectionFailed(_))
        ...
        && !turn_context.provider.info().is_amazon_bedrock()
    {
        ...
        kodex_client::record_retry!(retry_state.connection_retries, retry_delay, operation);
    ...
    if retry_state.retries < max_retries {
        ...
        kodex_client::record_retry!(retry_count, delay, operation);
