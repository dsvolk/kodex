...
const READY_PACKAGE: &str = "OpenAI.Kodex_1.0.0.0_arm64__publisher";
...
fn readiness_requires_a_receipt_for_the_current_package_version() {
    ...
    assert!(!runtime.ready_for_package("OpenAI.Kodex_2.0.0.0_arm64__publisher"));
...
fn admission_accepts_package_family_casing() {
    for stored in ["OpenAI.Kodex_publisher", "openai.kodex_PUBLISHER"] {
        let mut record = record();
        record.runtime_mut().unwrap().package_family = stored.into();
        let expected = record.clone();
        for requested in ["OpenAI.Kodex_publisher", "OPENAI.KODEX_PUBLISHER"] {
...
fn admission_rejects_other_owner_home_and_family() {
    ...
    for (home, sid, family) in [
        ...
        (
            record.kodex_home.as_path(),
            record.user_sid.as_str(),
            "OpenAI.KodexBeta_publisher",
        ),
        (
            record.kodex_home.as_path(),
            record.user_sid.as_str(),
            "OpenAI.Kodex_otherpublisher",
        ),
    ] {
        let mut requested = record.clone();
        requested.kodex_home = home.to_path_buf();
...
fn production_record_without_optional_core_or_desktop_fields_still_loads() {
    ...
    let json = r#"{"user_sid":"S-1-5-21-1-2-3-1000","kodex_home":"C:\\Users\\owner\\.kodex","session_id":1}"#;
    assert_eq!(
        serde_json::from_str::<InstallationRecord>(json).unwrap(),
        expected
    );
...
fn interrupted_cleanup_logon_blocks_admission_after_record_reload() {
    ...
    assert!(
        restored
            .admit_owner(&original, "OpenAI.Kodex_publisher")
            .is_err()
    );
