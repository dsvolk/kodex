...
BUILD_SCRIPT = REPO_ROOT / "kodex-cli" / "scripts" / "build_npm_package.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
GITHUB_REPO = "openai/kodex"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("kodex_build_npm_package", BUILD_SCRIPT)
...
KODEX_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "KODEX_PLATFORM_PACKAGES", {})
KODEX_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "KODEX_PACKAGE_COMPONENT", "kodex-package"
)
...
BINARY_COMPONENTS = {
    "kodex-responses-api-proxy": BinaryComponent(
        artifact_prefix="kodex-responses-api-proxy",
        dest_dir="kodex-responses-api-proxy",
        binary_basename="kodex-responses-api-proxy",
    ),
}
...
def install_from_downloaded_artifacts(
    ...
    if KODEX_PACKAGE_COMPONENT in components:
        install_kodex_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = KODEX_PACKAGE_COMPONENT in components or any(
...
def install_kodex_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing Kodex package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_kodex_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_kodex_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"kodex-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in KODEX_PLATFORM_PACKAGES:
        platform = package.removeprefix("kodex-")
        return f"kodex-npm-{platform}-{version}.tgz"
