...
use kodex_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestKodex) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Kodex helper binary
...
impl Expectation {
    fn verify(&self, test: &TestKodex, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestKodex) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestKodex) {
    let event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestKodex) {
...
async fn wait_for_spawned_thread(test: &TestKodex) -> Result<Arc<KodexThread>> {
...
async fn shell_startup_credentials_are_brokered(
    ...
    let builder = if shell_mode == "zsh_fork" {
        ...
        let Some(zsh) = kodex_core::shell::get_shell(kodex_core::shell::ShellType::Zsh) else {
            return Ok(());
        };
        test_kodex()
            .with_user_shell(zsh)
...
"#,
    ...
    if direct && !allow_login_shell {
        let Some(bash) = kodex_core::shell::get_shell(kodex_core::shell::ShellType::Bash) else {
            return Ok(());
        };
    ...
    let command = r#"printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$GH_HOST" "$GH_ENTERPRISE_TOKEN" "$CUSTOM_HOST" "$CUSTOM_API_KEY" "$KODEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE" "$PWD" "$BROKERED_STARTUP_CWD" "${LOGIN_SNAPSHOT_READY-unset}" "$AUTH_HEADER""#;
    let command = format!(
...
async fn brokered_posix_startup_preserves_application_env(credential_startup: bool) -> Result<()> {
    ...
    let Some(shell) = kodex_core::shell::get_shell(kodex_core::shell::ShellType::Sh) else {
        return Ok(());
    };
    let home = Arc::new(TempDir::new()?);
...
"#,
    ...
    let test = test_kodex()
        ...
        .build(&server)
        .await?;
    let script = r#"printf '%s\n%s\n%s\n%s\n' "$GH_TOKEN" "$KODEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE" "${ENV-unset}" "$CORP_REGION""#;
    let command = format!(
        "{} 2>/dev/null",
...
/// same sandbox with `kodex sandbox -P`. The nested invocation must also pass
...
printenv KODEX_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit KODEX_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `kodex sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Kodex sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("KODEX_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("KODEX_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("KODEX_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected kodex sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Kodex sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/kodex-fallback-rule-test.txt && echo hello > /tmp/kodex-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://kodex-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
