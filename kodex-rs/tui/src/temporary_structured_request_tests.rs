...
async fn preserves_custom_permissions_and_disables_required_mcp_servers() -> color_eyre::Result<()>
{
    ...
    let kodex_home = tempdir()?;
    let denied_path = kodex_home.path().join("denied");
    ...
    std::fs::write(
        ...
        format!(
            "default_permissions = \"title-restricted\"\n\n\
             ...
             command = \"kodex-auto-title-missing-mcp\"\n\
             required = true\n"
        ),
    )?;
    config.kodex_home = kodex_home.path().to_path_buf().abs();
    config.sqlite =
        kodex_state::SqliteConfig::new_for_testing(kodex_home.path().to_path_buf().abs());
