...
async fn websocket_handshakes_share_routing_cookies_and_respect_cookie_scope() {
    ...
    let parent_url = format!("wss://{host}/backend-api/kodex/responses");
    let review_url = format!("wss://{host}/backend-api/kodex/guardian");
    let requests = [
        (parent_url, None),
        ...
        (
            format!("wss://{other_host}/backend-api/kodex/responses"),
            None,
        ),
        ("wss://api.openai.com/v1/responses".to_string(), None),
        ("wss://api.openai.com/v1/responses".to_string(), None),
        (format!("ws://{host}/backend-api/kodex/responses"), None),
        (review_url.clone(), None),
        (review_url.clone(), None),
        (review_url, None),
    ];
