...
use kodex_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == kodex_features::Feature::Artifact {
            continue;
        }
        if feature.id == kodex_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::CodeModeHost {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::CodeModeHostConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::GuardianThreadContext {
            // This setting is already part of the guardianv2 feature table.
            continue;
        }
        if feature.id == kodex_features::Feature::GuardianV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
        ...
        if feature.id == kodex_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::ContextManagement {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::ContextManagementConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::SleepTool {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::SleepToolConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == kodex_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == kodex_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<kodex_features::FeatureToml<
                    kodex_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
    ...
    validation.properties.insert(
        ...
        schema_gen.subschema_for::<kodex_features::ToolRegistryConfigToml>(),
    );
