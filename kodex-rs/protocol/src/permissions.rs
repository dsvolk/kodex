...
const PROTECTED_METADATA_KODEX_PATH_NAME: &str = ".kodex";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_KODEX_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Kodex degrades to warn-and-ignore
    /// instead of failing to load. Kodex 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "kodex-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".kodex");
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_kodex*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_kodex*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_kodex*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    fn get_writable_roots_with_cwd_impl(
        ...
        .map(|root| {
            ...
            let protect_missing_dot_kodex = effective_cwd.as_ref() == Some(&root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_kodex)
                    .into_iter()
            ...
            // Example: if `<root>/.kodex -> <root>/decoy`, bwrap must still see
            // `<root>/.kodex`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.kodex` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_kodex: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .kodex read-only to the agent by
    ...
    let top_level_kodex = writable_root.join(PROTECTED_METADATA_KODEX_PATH_NAME);
    if protect_missing_dot_kodex || top_level_kodex.as_path().is_dir() {
        subpaths.push(top_level_kodex);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "KODEX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry::new(
                path("file:///C:/workspace/.kodex").into(),
                FileSystemAccessMode::Read,
            ),
        ...
        assert!(!policy.can_write_path(&path("file:///C:/workspace/.kodex/config"), &context));
        assert_eq!(
            policy.metadata_write_denial(&path("file:///C:/workspace/.kodex/config"), &context),
            Some(".kodex"),
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".kodex".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_kodex_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_kodex = expected_root.join(".kodex");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_kodex.clone().into(),
                },
                access: FileSystemAccessMode::Write,
                missing_path_behavior: None,
            },
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".kodex".to_string()),
            "explicit .kodex rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_kodex),
            "explicit .kodex rule should win over the default protected carveout"
        );
        assert!(policy.can_write_local_path_with_cwd(
            explicit_dot_kodex.join("config.toml").as_path(),
            cwd.path()
        ));
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_kodex_config = cwd.path().join(".kodex").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_local_path_with_cwd(&dot_kodex_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_kodex_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_kodex*/ true,
            )
            .into_iter()
            .map(|path| {
        ...
        assert!(
            !file_system_policy
                .can_write_local_path_with_cwd(Path::new(".kodex/config.toml"), relative_cwd,)
        );
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let kodex_dir = real_root.join(".kodex");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&kodex_dir).expect("create .kodex");
        ...
        let expected_kodex = link_root.join(".kodex");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_kodex = expected_root.join(".kodex");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-kodex");
        let dot_kodex = root.join(".kodex");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_kodex).expect("create .kodex symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_kodex = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".kodex"),
        )
        .expect("absolute .kodex symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let kodex_dir = real_tmpdir.join(".kodex");
