...
pub(super) fn check(config: Option<&Config>, cwd: &Path) -> DoctorCheck {
    let home = config
        .map(|config| config.kodex_home.as_path().to_path_buf())
        .or_else(|| find_kodex_home().ok().map(Into::into))
        .or_else(|| std::env::var_os("KODEX_HOME").map(Into::into));
...
fn check_with_paths(
    ...
    for (label, path) in [("KODEX_HOME", home.as_deref()), ("worktree", Some(cwd))] {
