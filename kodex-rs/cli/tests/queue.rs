    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let initialized_response = json!({
        "id": initialize["id"],
        "result": {
            "userAgent": "kodex_cli_rs/0.0.0-test",
...
fn queue_rejects_empty_message() -> Result<()> {
    let output = std::process::Command::new(kodex_utils_cargo_bin::cargo_bin("kodex")?)
...
async fn run_remote_queue_command(response: QueueResponse) -> Result<(Output, Value)> {
    ...
    let server_home = kodex_home.path().to_path_buf();
    ...
    let output = tokio::process::Command::new(kodex_utils_cargo_bin::cargo_bin("kodex")?)
        .env("KODEX_HOME", kodex_home.path())
        .env("KODEX_REMOTE_TOKEN", "test-token")
        .args(remote_args)
...
async fn queue_rejects_local_daemon_that_does_not_support_queueing() -> Result<()> {
    ...
    let socket_path = kodex_app_server::app_server_control_socket_path(kodex_home.path())?;
    ...
    assert!(!kodex_home.path().join("queue_1.sqlite").exists());
    Ok(())
}
