...
async fn ultra_reasoning_uses_highest_non_ultra_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.kodex, "hello", /*effort*/ None).await?;
...
async fn mode_hints_override_reasoning_effort(source: ModeHintSource) -> Result<()> {
    ...
    let test = test_kodex()
        ...
        .build(&server)
        .await?;
    submit_turn(&test.kodex, "explicit", Some(ReasoningEffort::High)).await?;
    submit_turn(&test.kodex, "proactive", Some(ReasoningEffort::Ultra)).await?;
...
async fn catalog_proactive_mode_is_ultra_only(
    ...
    submit_turn(&test.kodex, "hello", Some(effort)).await?;
...
async fn model_switch_refreshes_catalog_role_and_mode(
    ...
    let test = test_kodex()
        ...
        .build_with_auto_env(&server)
        .await?;
    submit_turn(&test.kodex, "first model", Some(effort.clone())).await?;
    ...
    .await?;
    submit_turn(&test.kodex, "second model", Some(effort)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    .await;
    let initial = test_kodex()
        .with_config(configure_custom_mode_hint)
        .build(&server)
        .await?;

    submit_turn(&initial.kodex, "before resume", /*effort*/ None).await?;
    ...
    drop(initial);
    submit_turn(&resumed.kodex, "after resume", /*effort*/ None).await?;
...
async fn live_mode_change_appends_mode_without_reappending_usage_hint() -> Result<()> {
    ...
    submit_turn(&test.kodex, "proactive", /*effort*/ None).await?;
    submit_turn(&test.kodex, "explicit", Some(ReasoningEffort::High)).await?;
...
async fn leaving_ultra_after_cold_resume_emits_explicit_mode() -> Result<()> {
    ...
    let resumed = resume_builder.restart(&server, &initial).await?;
    drop(initial);
    submit_turn(&resumed.kodex, "after resume", Some(ReasoningEffort::High)).await?;
