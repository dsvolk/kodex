...
use kodex_utils_pty::ProcessMode;
#[cfg(unix)]
...
impl LocalStdioServerLauncher {
    fn launch_server(
        ...
        #[cfg(windows)]
        let job = match kodex_utils_pty::JobObject::create_without_breakaway() {
            Ok(job) => {
        ...
        let terminator = match job {
            ...
            None => process_id.and_then(|process_id| {
                match kodex_utils_pty::JobObject::open_process_handle(process_id) {
    ...
    fn terminate(&self) {
        let result = match self {
            ...
            Self::Process(process_handle) => {
                kodex_utils_pty::JobObject::terminate_process_handle(process_handle)
            }
        };
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from Kodex. Start from `All` only so the
