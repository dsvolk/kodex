...
use kodex_apply_patch::ApplyPatchAction;
use kodex_apply_patch::ApplyPatchFileChange;
use kodex_apply_patch::ApplyPatchFileUpdateMode;
use kodex_apply_patch::Hunk;
use kodex_apply_patch::StreamingPatchParser;
...
use kodex_protocol::protocol::PatchApplyUpdatedEvent;
use kodex_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use kodex_sandboxing::policy_transforms::merge_permission_profiles;
use kodex_sandboxing::policy_transforms::normalize_additional_permissions;
use kodex_sandboxing::policy_transforms::normalize_additional_permissions_with_context;
...
fn format_update_chunks_for_progress(chunks: &[kodex_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    file_paths: &[PathUri],
    file_system_sandbox_policy: &kodex_protocol::permissions::FileSystemSandboxPolicy,
...
async fn effective_patch_permissions(
    ...
    kodex_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match kodex_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match kodex_apply_patch::verify_apply_patch_args_with_mode(
            args,
        ...
        .await
        {
            kodex_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            kodex_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            kodex_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            kodex_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    let sandbox = turn_environment.sandbox_context(/*additional_permissions*/ None);
    match kodex_apply_patch::maybe_parse_apply_patch_verified_with_mode(
        command,
        cwd,
        apply_patch_file_update_mode(turn),
        fs,
    ...
    .await
    {
        ...
        kodex_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
