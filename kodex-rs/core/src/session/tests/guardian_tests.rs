...
use core_test_support::kodex_linux_sandbox_exe_or_skip;
...
use kodex_network_proxy::NetworkDecision;
use kodex_network_proxy::NetworkPolicyRequest;
use kodex_network_proxy::NetworkProtocol;
use kodex_protocol::config_types::ApprovalsReviewer;
use kodex_protocol::error::SandboxErr;
use kodex_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    ...
    let (current_turn, _) = session
        .new_turn_with_sub_id(
            ...
            SessionSettingsUpdate {
                ...
                permission_profile: Some(kodex_protocol::models::PermissionProfile::Disabled),
                ..Default::default()
            },
...
) -> kodex_protocol::protocol::ExecApprovalRequestEvent {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                kodex_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_exec_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(kodex_protocol::models::PermissionProfile::Disabled)
    ...
    config.kodex_linux_sandbox_exe = kodex_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
...
async fn strict_auto_review_turn_grant_forces_guardian_for_exec_command_policy_skip() {
    ...
    *session.active_turn.lock().await = Some(active_turn);
    ...
    config
        .permissions
        .set_permission_profile(kodex_protocol::models::PermissionProfile::External {
...
async fn sandbox_denied_retry_uses_the_action_policy_and_reviewer() {
    ...
    impl ToolRuntime<TurnEnvironment, String> for DeniedOnceRuntime {
        ...
        async fn run(
            ...
            self.attempts += 1;
            if self.attempts == 1 {
                return Err(ToolError::Kodex(KodexErr::Sandbox(SandboxErr::Denied {
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let kodex_home = tempdir().expect("create kodex home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_kodex_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(kodex_thread_store::LocalThreadStore::new(
        kodex_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));

    let mut thread_extension_init = kodex_extension_api::ExtensionDataInit::default();
    thread_extension_init.insert(kodex_extension_api::SessionIsolation::Isolated);
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(kodex_code_mode::DisabledCodeModeSessionProvider),
        extensions: kodex_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: kodex_rollout_trace::ThreadTraceContext::disabled(),
