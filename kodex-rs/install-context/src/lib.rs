...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    ...
    "kodex-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "kodex-package.json";
const PATH_DIRNAME: &str = "kodex-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "kodex-resources";
...
/// Version metadata recorded in a bundled Kodex runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct KodexPackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<KodexPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.kodex/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `kodex-resources/`, and `kodex-path/`.
    ...
    /// A Kodex binary launched through the npm-managed `kodex.js` shim.
    Npm,
    /// A Kodex binary launched through the bun-managed `kodex.js` shim.
    Bun,
    /// A Kodex binary launched through the pnpm-managed `kodex.js` shim.
    Pnpm,
    /// A Kodex binary launched through the Vite+-managed `kodex.js` shim.
    VitePlus,
    /// A Kodex binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Kodex binaries, custom
    /// internal launchers, and tests that execute Kodex from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let kodex_home = kodex_utils_home_dir::find_kodex_home().ok();
        Self::from_exe_with_kodex_home(
            is_macos,
            current_exe,
            method_override,
            kodex_home.as_deref(),
    ...
    fn from_exe_with_kodex_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        kodex_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(KodexPackageLayout::from_exe);
        let method = if let Some(method) = method_override {
            method
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, kodex_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("KODEX_MANAGED_BY_VITE_PLUS").is_some() {
                Some(InstallMethod::VitePlus)
            } else if std::env::var_os("KODEX_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("KODEX_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("KODEX_MANAGED_BY_BUN").is_some() {
    ...
    pub fn package_manifest(&self) -> Option<KodexPackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under kodex-resources
    ...
    fn from_exe(exe_path: &Path) -> Option<Self> {
        ...
        match exe_dir.file_name() {
            ...
            Some(name) if name == OsStr::new("MacOS") => {
                ...
                // package. current_exe points inside the bundle, not at bin/kodex.
                let contents = exe_dir.parent()?;
                let bundle = contents.parent()?;
                if contents.file_name()? != OsStr::new("Contents")
                    || bundle.file_name()? != OsStr::new("KodexCLI.app")
                {
                    return None;
                }
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, kodex_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_kodex_home = canonical_absolute_path(kodex_home?)?;
    ...
    let releases_root = canonical_kodex_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "kodex-test-helper";
    ...
    fn code_mode_host_program_prefers_package_resource_over_legacy_binary() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "kodex.exe" } else { "kodex" });
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = kodex_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "kodex.exe" } else { "kodex" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_kodex_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*kodex_home*/ Some(kodex_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
...
"#,
        ...
        let package_layout = KodexPackageLayout {
    ...
    fn code_mode_host_program_accepts_symlinks_to_files() -> std::io::Result<()> {
        ...
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
        let exe_path = bin_dir.join("kodex");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_kodex_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn bundled_file_lookups_ignore_directories() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "kodex.exe" } else { "kodex" });
        fs::write(&exe_path, "")?;
        let fallback_exe_path = package_dir.path().join("fallback-kodex");
    ...
    fn code_mode_host_program_is_next_to_the_executable_even_when_missing() {
        ...
        assert_eq!(
            context.code_mode_host_program_from_exe(Some(Path::new("/opt/kodex/bin/kodex"))),
            PathBuf::from("/opt/kodex/bin").join(CODE_MODE_HOST_EXECUTABLE_NAME)
        );
    }
    ...
    fn package_manager_method_overrides_take_precedence() {
        let vite_plus_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/kodex")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_kodex_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/kodex")),
            /*method_override*/ None,
            /*kodex_home*/ None,
        );
        assert_eq!(
