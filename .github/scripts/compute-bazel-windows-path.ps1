...
publishes the result via `GITHUB_ENV` as `KODEX_BAZEL_WINDOWS_PATH` so later
...
Write-Host 'Derived KODEX_BAZEL_WINDOWS_PATH entries:'
foreach ($pathEntry in $stablePathEntries) {
  Write-Host "  $pathEntry"
}
"KODEX_BAZEL_WINDOWS_PATH=$stablePath" | Out-File -FilePath $env:GITHUB_ENV -Encoding utf8 -Append
