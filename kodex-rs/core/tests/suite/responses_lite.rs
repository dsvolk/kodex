...
use kodex_image_generation_extension::install as install_image_generation_extension;
...
const RESPONSES_LITE_HEADER: &str = "x-openai-internal-kodex-responses-lite";

fn responses_extensions(auth: &KodexAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut kodex_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_uses_input_items_for_instructions_and_tools() -> Result<()> {
    ...
    let turn_metadata: Value = serde_json::from_str(
        client_metadata["x-kodex-turn-metadata"]
            .as_str()
...
async fn responses_lite_includes_tool_namespaces_info_when_enabled() -> Result<()> {
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "direct": true,
            "deferred": false,
            "code_mode_name": "mcp__kodex_apps__calendar_create_event",
            "source": {
                "kind": "mcp",
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_kodex().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;
