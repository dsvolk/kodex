...
pub const KODEX_WINDOWS_MXC_ARG1: &str = "--__kodex-windows-mxc";
...
pub fn create_command_args(args: CreateMxcCommandArgsParams<'_>) -> Result<Vec<String>> {
    ...
    Ok(vec![KODEX_WINDOWS_MXC_ARG1.to_owned()])
}
...
/// Entry point dispatched before ordinary Kodex CLI parsing.
pub fn run_main() -> ! {
    #[cfg(windows)]
    {
        match windows::run() {
