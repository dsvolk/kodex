...
async fn runtime_with_threads(count: usize) -> Result<(Arc<StateRuntime>, PathBuf, Vec<ThreadId>)> {
    ...
    Ok((runtime, kodex_home, thread_ids))
}

#[tokio::test]
async fn copying_attachments_is_atomic_and_independent() -> Result<()> {
    let (runtime, _kodex_home, thread_ids) = runtime_with_threads(/*count*/ 2).await?;
...
async fn attachment_attachments_are_idempotent_and_scoped_to_their_thread() -> Result<()> {
    ...
    let first = runtime
        .add_thread_attachment(
            ...
            "pull_request",
            "openai/kodex#123",
            &json!({ "url": "https://github.com/openai/kodex/pull/123" }),
        )
        .await?;
    ...
    let repeated = runtime
        .add_thread_attachment(
            ...
            "pull_request",
            "openai/kodex#123",
            &json!({ "url": "https://github.com/openai/kodex/pull/456" }),
        )
        .await?;
    assert_eq!(
        repeated,
...
async fn attachment_removals_return_not_found_or_the_removed_record() -> Result<()> {
    let (runtime, _kodex_home, thread_ids) = runtime_with_threads(/*count*/ 1).await?;
    let thread_id = thread_ids[0];
    let removed_before_attach = runtime
        .remove_thread_attachment(thread_id, "pull_request", "openai/kodex#123")
        .await?;
    assert_eq!(
...
async fn attachments_survive_restart_and_archive_but_cascade_on_thread_deletion() -> Result<()> {
    let (runtime, kodex_home, thread_ids) = runtime_with_threads(/*count*/ 1).await?;
    let thread_id = thread_ids[0];
    ...
    let rollout_path = kodex_home.join("archived.jsonl");
    reopened
