...
use kodex_protocol::protocol::HookCompletedEvent;
...
use kodex_skills::SkillError;
use kodex_utils_git_discovery::GitRootDiscovery;
use kodex_utils_path::replace_path_and_deduplicate;
...
pub(crate) struct Session {
    ...
    pub(super) isolation: kodex_extension_api::SessionIsolation,
    pub(crate) tool_policy: Arc<kodex_extension_api::ToolPolicy>,
    ...
    pub(super) mcp_elicitation_reviewer_handle: OnceLock<kodex_mcp::ElicitationReviewerHandle>,
    pub(super) mcp_elicitation_lifecycle_handle: OnceLock<kodex_mcp::ElicitationLifecycle>,
...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all Kodex state for this session.
    pub(super) kodex_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn kodex_home(&self) -> &AbsolutePathBuf {
        &self.kodex_home
    }
    ...
    pub(super) fn sandbox_policy(
        ...
        kodex_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
    ...
    pub(super) fn thread_config_snapshot(
        ...
        ThreadConfigSnapshot {
            ...
            full_access: kodex_protocol::protocol::has_full_access(
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: kodex_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
async fn warm_plugins_and_skills_for_session_init(
    ...
    extensions: &kodex_extension_api::ExtensionRegistry<Config>,
) -> Vec<SkillError> {
...
impl Session {
    ...
    pub(crate) async fn responses_metadata(
        ...
        request_kind: KodexResponsesRequestKind,
    ...
    pub(crate) async fn compaction_responses_metadata(
        ...
        let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
            self.installation_id.clone(),
            window_id,
            KodexResponsesRequestKind::Compaction(compaction_metadata),
        );
        self.with_window_and_fork_metadata(
            turn_context,
            responses_metadata,
    ...
    pub(crate) async fn new(
        ...
        code_mode_session_provider: Arc<dyn kodex_code_mode::CodeModeSessionProvider>,
        extensions: Arc<kodex_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        windows_sandbox_proxy_settings_mode: kodex_sandboxing::WindowsSandboxProxySettingsMode,
            ...
            ForkPersistence::Copied | ForkPersistence::CopiedDeferred => match &initial_history {
                InitialHistory::Resumed(resumed) => {
                    ...
                    resumed.history.first().and_then(|item| match item {
                        ...
                        {
                            kodex_rollout::forked_from_ordinal_exclusive(
                                &meta.meta,
        ...
        if !session_configuration.session_source.is_non_root_agent()
            && isolation != kodex_extension_api::SessionIsolation::Isolated
        {
        ...
        thread_extension_init.insert(kodex_extension_api::ThreadOriginator(
        ...
        let tool_policy = thread_extension_init
            .get::<kodex_extension_api::ToolPolicy>()
            .unwrap_or_else(|| {
                ...
                if crate::guardian::is_basic_session_source(&session_configuration.session_source) {
                    Arc::new(kodex_guardian_reviewer::reviewer_tool_policy())
                } else {
                    Arc::default()
                }
            });
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = kodex_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
                ...
                let live_thread = match &initial_history {
                    InitialHistory::New | InitialHistory::Cleared | InitialHistory::Forked(_) => {
                        ...
                        let params = CreateThreadParams {
                            creator_user_id: auth.as_ref().and_then(KodexAuth::get_chatgpt_user_id),
                            creator_account_id: auth.as_ref().and_then(KodexAuth::get_account_id),
                            session_id,
                            thread_id,
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let config_path = config.kodex_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = telemetry_auth
                .map(KodexAuth::auth_mode)
                .map(TelemetryAuthMode::from);
            let account_id = telemetry_auth.and_then(KodexAuth::get_account_id);
            let account_email = telemetry_auth.and_then(KodexAuth::get_account_email);
            ...
            let auth_env_telemetry = collect_auth_env_telemetry(
                ...
                auth_manager.kodex_api_key_env_enabled(),
            );
            ...
            if let Some(metrics) = thread_extension_data.get::<kodex_otel::MetricsClient>() {
            ...
            let is_worktree = session_configuration.cwd().canonicalize().ok().and_then(|cwd| {
                kodex_git_utils::repository_identity(&cwd).and_then(|_| {
            ...
            let mcp_server_names =
                kodex_mcp::effective_mcp_servers(
            ...
            let session_extension_data =
                kodex_extension_api::ExtensionData::new(session_id.to_string());
            ...
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(kodex_extension_api::ThreadStartInput {
            ...
            let kodex_responses_headers = thread_extension_data.get::<crate::KodexResponsesHeaders>();
        ...
        mcp_elicitation_reviewer_handle: OnceLock::new(),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Forked(_) if forked_from_id.is_some() => {
                    kodex_hooks::SessionStartSource::Fork
                }
                // `thread/resume` with supplied history uses `Forked` internally
                // without a fork parent, so it should still report `resume`.
                InitialHistory::Resumed(_) | InitialHistory::Forked(_) => {
                    kodex_hooks::SessionStartSource::Resume
                }
                InitialHistory::New => kodex_hooks::SessionStartSource::Startup,
                InitialHistory::Cleared => kodex_hooks::SessionStartSource::Clear,
            };
