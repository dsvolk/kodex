...
impl ExperimentalFeaturesView {
    pub(crate) fn new(
        ...
        let mut view = Self {
            ...
            .to_string(),
            thread_id,
            voice_supported: kodex_realtime_webrtc::RealtimeWebrtcSession::is_supported(),
            write_rx: None,
            catalog_rx,
            unconfirmed: Vec::new(),
    ...
    fn header(&self, width: u16) -> impl Renderable {
        ...
        for text in [
            "Checked features are configured on. Some experimental features take effect only in new tasks or after restarting the Kodex server.",
    ...
    fn pre_draw_tick(&mut self, _now: Instant) -> bool {
        ...
        match result {
            ...
            Err(error) => {
                ...
                self.discovery_status = "Server experiments unavailable. Reopen /experimental to retry; restart this Kodex client if requests remain unanswered.".to_string();
            }
        }
        true
    }
