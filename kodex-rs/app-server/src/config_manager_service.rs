...
use kodex_app_server_protocol::Config as ApiConfig;
...
use kodex_app_server_protocol::OverriddenMetadata;
use kodex_app_server_protocol::WriteStatus;
use kodex_config::CONFIG_TOML_FILE;
use kodex_config::ConfigLayerEntry;
use kodex_config::ConfigLayerMetadata;
use kodex_config::ConfigLayerSource;
use kodex_config::ConfigLayerStack;
use kodex_config::ConfigRequirementsToml;
use kodex_config::ShellEnvironmentPolicyFilterRepresentation;
use kodex_config::config_toml::ConfigToml;
use kodex_config::is_structured_feature_path;
use kodex_config::merge_toml_values;
use kodex_config::shell_environment_filter_entry;
use kodex_config::validate_shell_environment_policy_filter_config;
use kodex_core::config::deserialize_config_toml_with_base;
use kodex_core::config::edit::ConfigEditsBuilder;
use kodex_core::config::validate_feature_requirements_for_config_toml;
use kodex_core::path_utils;
use kodex_core::path_utils::SymlinkWritePaths;
use kodex_core::path_utils::resolve_symlink_write_paths;
use kodex_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.kodex_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .kodex/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_kodex_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_kodex_folder.display(),
        ),
