...
//! interactive-session layers. Direct `kodex login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `kodex-login.log` artifact that
...
/// Installs a small file-backed tracing layer for direct `kodex login` flows.
///
...
/// command produce a durable `kodex-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match kodex_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("kodex-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("kodex_cli=info,kodex_core=info,kodex_login=info"));
    ...
    // Direct `kodex login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `kodex login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | kodex login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv KODEX_ACCESS_TOKEN | kodex login --with-access-token`.",
...
/// `kodex login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    if is_workload_identity_selected() {
        match AuthManager::shared_from_config(&config, /*enable_kodex_api_key_env*/ false).await {
            Ok(_) => {
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let kodex_home = tempdir().expect("create temporary Kodex home");
        login_with_api_key(
            kodex_home.path(),
            "sk-existing",
