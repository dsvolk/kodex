...
This crate implements the business logic for Kodex. It is designed to be used by the various Kodex UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //kodex-rs/core:core-all-wine-exec-test`.
...
Note that `kodex-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.kodex` read-only.
...
Expects the binary containing `kodex-core` to run the equivalent of `kodex sandbox` when `arg0` is `kodex-linux-sandbox`. See the `kodex-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `kodex-resources/bwrap`
binary shipped with Kodex and Kodex surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Kodex also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Kodex rejects sandboxed shell commands that would enter the
...
Expects the binary containing `kodex-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--kodex-run-as-apply-patch`. See the
`kodex-arg0` crate for details.
