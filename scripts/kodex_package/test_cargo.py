...
from kodex_package.cargo import build_source_binaries
from kodex_package.cargo import source_binaries_for_target
...
class SourceBinariesForTargetTest(unittest.TestCase):
    def test_macos_package_with_prebuilt_entrypoint_builds_nothing(self) -> None:
        self.assertEqual(
            source_binaries_for_target(
                ...
                PACKAGE_VARIANTS["kodex"],
                build_entrypoint=False,
                build_code_mode_host=False,
                build_bwrap=False,
                build_kodex_command_runner=False,
                build_kodex_windows_sandbox_setup=False,
            ),
            [],
        )
    ...
    def test_missing_windows_helpers_are_built(self) -> None:
        self.assertEqual(
            source_binaries_for_target(
                ...
                build_code_mode_host=False,
                build_bwrap=False,
                build_kodex_command_runner=True,
                build_kodex_windows_sandbox_setup=True,
            ),
            ["kodex-command-runner", "kodex-windows-sandbox-setup"],
        )
    ...
    def test_build_uses_prebuilt_windows_helpers_without_running_cargo(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            entrypoint = touch_file(root / "kodex.exe")
            code_mode_host = touch_file(root / "kodex-code-mode-host.exe")
            command_runner = touch_file(root / "kodex-command-runner.exe")
            sandbox_setup = touch_file(root / "kodex-windows-sandbox-setup.exe")
            ...
            outputs = build_source_binaries(
                ...
                bwrap_bin=None,
                kodex_command_runner_bin=command_runner,
                kodex_windows_sandbox_setup_bin=sandbox_setup,
            )

        self.assertEqual(outputs.entrypoint_bin, entrypoint)
        self.assertEqual(outputs.code_mode_host_bin, code_mode_host)
        self.assertEqual(outputs.kodex_command_runner_bin, command_runner)
        self.assertEqual(outputs.kodex_windows_sandbox_setup_bin, sandbox_setup)
