...
use kodex_app_server_protocol::ThreadAttachmentAddParams;
use kodex_app_server_protocol::ThreadAttachmentAddResponse;
use kodex_app_server_protocol::ThreadAttachmentListParams;
use kodex_app_server_protocol::ThreadAttachmentListResponse;
use kodex_app_server_protocol::ThreadAttachmentRemoveParams;
use kodex_app_server_protocol::ThreadAttachmentRemoveResponse;
...
use kodex_rollout::append_thread_name;
...
async fn assert_thread_fork_at_named_boundary_keeps_only_terminal_prefix(
    ...
    let attachment: ThreadAttachmentAddResponse = mcp
        .request(|request_id| ClientRequest::ThreadAttachmentAdd {
            ...
            params: ThreadAttachmentAddParams {
                ...
                identity_key: "openai/kodex#123".to_string(),
                payload: json!({"url": "https://github.com/openai/kodex/pull/123"}),
            },
        })
        .await?;
    let sqlite = kodex_state::SqliteConfig::new_for_testing(kodex_home.path().abs());
    ...
    assert_eq!(
        copied.data[0],
        kodex_app_server_protocol::ThreadAttachment {
...
async fn thread_fork_defers_inherited_active_goal_until_next_turn() -> Result<()> {
    ...
    let source_goal = state_db
        ...
        .replace_thread_goal(
            ...
            kodex_state::ThreadGoalStatus::Active,
    ...
    assert_eq!(
        forked_goal.status,
        kodex_state::ThreadGoalStatus::BudgetLimited
    );
    assert_eq!(
        state_db
            .thread_goals()
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(kodex_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(kodex_home.path(), filename_ts, &conversation_id);
...
async fn thread_fork_creates_reference_backed_paginated_thread() -> Result<()> {
    ...
    let turn_metadata: Value = serde_json::from_str(
        request_body["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    let source_path = rollout_path(kodex_home.path(), "2025-01-05T12-00-00", &source_thread_id);
    ...
    assert!(matches!(
        ...
        [
            ...
            &response_item.item,
            kodex_protocol::models::ResponseItem::Message { role, .. }
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_kodex_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let kodex_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(kodex_home.path())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(kodex_home_path.clone()),
            ..Default::default()
        })
        .await?;
