...
fn finalizer_preserves_utf8_plan_and_requires_commit_and_stdin_eof() -> Result<()> {
    ...
    let end = script[start..]
        ...
        + start;
    let home = r"C:\Users\Zoë-東京\.kodex";
    let script = format!(
        "$ErrorActionPreference = 'Stop';
         [Console]::InputEncoding = [Text.Encoding]::GetEncoding(437);
         {input_setup}
         if ($plan.record.kodex_home -cne '{home}') {{ throw 'UTF-8 plan was corrupted' }}
         [Console]::Out.WriteLine('waiting');\n{}\n[Console]::Out.WriteLine('released')",
        &script[start..end]
    );
    let mut plan = serde_json::to_vec(&serde_json::json!({ "record": { "kodex_home": home } }))?;
    plan.push(b'\n');
