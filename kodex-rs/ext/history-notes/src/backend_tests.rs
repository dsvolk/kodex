...
#[tokio::test]
async fn routes_through_kodex_backend_and_injects_trusted_session_agent_context() {
...
async fn marks_encrypted_history_and_notes_arguments_without_changing_the_json_body() {
    ...
    for (route, _) in &cases {
        Mock::given(method("POST"))
            .and(path(format!("/backend-api/kodex/alpha/{route}")))
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(KodexAuth::Headers(AuthHeaders::new(HeaderMap::new())));
