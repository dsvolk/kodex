...
use kodex_core_plugins::startup_sync::curated_plugins_repo_path;
...
const CALENDAR_NAMESPACE: &str = "mcp__kodex_apps__calendar";
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if server
                ...
                .unwrap_or_default()
                .iter()
                .any(|request| request.url.path() == "/ps/plugins/suggested/kodex")
            {
                return;
            }
...
async fn start_gated_step_preparation(test: &TestKodex, server: &MockServer) -> Result<PathUri> {
    ...
    let prior_recommendation_count = server
        ...
        .unwrap_or_default()
        .into_iter()
        .filter(|request| request.url.path() == "/ps/plugins/suggested/kodex")
        .count();
...
async fn start_install_turn(test: &TestKodex, prompt: &str) -> Result<ElicitationRequestEvent> {
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, test.config.cwd.as_path());
    test.kodex
        .start_or_steer_turn(
    ...
    Ok(wait_for_event_match(&test.kodex, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "kodex_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.5",
                "product_client_id": kodex_login::default_client::originator().value,
            }
        })
    );
