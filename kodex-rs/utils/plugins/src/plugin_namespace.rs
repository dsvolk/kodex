...
use kodex_exec_server_protocol::DISCOVERABLE_PLUGIN_MANIFEST_PATHS;
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".kodex-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".kodex-plugin/plugin.json");
    ...
    fn rejects_nonregular_legacy_plugin_manifest_before_lower_precedence_manifest() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let kodex_path = plugin_root.join(".kodex-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(&kodex_path).expect("nonregular Kodex manifest");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_before_lower_precedence_manifest() {
        ...
        let kodex_path = plugin_root.join(".kodex-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(kodex_path.parent().expect("Kodex manifest parent"))
            .expect("Kodex manifest parent");
        fs::create_dir_all(claude_path.parent().expect("Claude manifest parent"))
            .expect("Claude manifest parent");
        fs::write(plugin_root.join("benign.json"), r#"{"name":"sample"}"#)
            .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink("../benign.json", &kodex_path).expect("Kodex manifest symlink");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_directory_before_lower_precedence_manifest() {
        ...
        .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink(&manifest_directory, plugin_root.join(".kodex-plugin"))
            .expect("Kodex manifest directory symlink");
    ...
    fn preserves_kodex_claude_cursor_legacy_precedence() {
        ...
        assert_eq!(
            ...
            Some(kodex_path.clone())
        );
        fs::remove_file(kodex_path).expect("remove Kodex manifest");
