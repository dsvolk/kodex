...
const OPENAI_FILE_USE_CASE: &str = "kodex";
    ...
    Contents: Stream<Item = std::io::Result<Bytes>> + Send + 'static,
{
    ...
    let create_request = match hosted_upload {
        Some(context) => serde_json::json!({
            ...
            "file_size": file_size_bytes,
            "use_case": OPENAI_FILE_USE_CASE,
            "kodex_connector_id": context.connector_id,
            "kodex_action_name": context.action_name,
            "kodex_model": context.model,
        }),
        None => create_request,
    };
    ...
    for attempt in 1..=MAX_BLOB_UPLOAD_ATTEMPTS {
                ...
                if upload_status.is_success() {
                    if attempt > 1 {
                        tracing::event!(
                            ...
                            tracing::Level::INFO,
                            event.name = "kodex.openai_file_blob_upload_recovered",
        ...
        retry_delay = server_delay
            .unwrap_or_else(|| kodex_client::backoff(Duration::from_millis(125), attempt));
        ...
        let event_name = if will_retry {
            "kodex.openai_file_blob_upload_retry"
        } else {
            "kodex.openai_file_blob_upload_failed"
        };
        tracing::event!(
            target: "kodex_otel.log_only",
            tracing::Level::WARN,
...
mod tests {
    ...
    async fn upload_hosted_app_context_and_finalizes_reservation() {
        ...
        let create_request = serde_json::json!({
            ...
            "file_size": 8,
            "use_case": "kodex",
            "kodex_connector_id": "library",
            "kodex_action_name": "create_library_file",
            "kodex_model": "gpt-work",
        });
        Mock::given(method("POST"))
            .and(path("/backend-api/files"))
            .and(header("chatgpt-account-id", "account_id"))
