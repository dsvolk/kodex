...
async fn account_switch_reloads_telemetry_collectors_and_preserves_trace_context() -> Result<()> {
    ...
    write_otel_config(kodex_home.path(), &initial_endpoint)?;
    write_chatgpt_auth(
        kodex_home.path(),
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_json_logging("kodex_app_server::otel_reloader=info")
    ...
    write_otel_config(kodex_home.path(), &next_endpoint)?;
    ...
    timeout(
        TEST_TIMEOUT,
        app_server.wait_for_json_log_event("kodex.app_server.otel_reloaded"),
    )
    .await??;
    timeout(TEST_TIMEOUT, async {
    ...
    assert!(
        next_metrics.contains("kodex.thread.started"),
...
fn write_otel_config(kodex_home: &Path, collector_endpoint: &str) -> Result<()> {
    std::fs::write(
