...
fn client(
    ...
    let home = tempfile::tempdir().expect("Kodex home");
...
fn config(server: &MockServer) -> GatewayAuthConfig {
    GatewayAuthConfig {
        ...
        client_id: "kodex-test".to_string(),
        resource: Some("https://gateway.example.test/kodex".to_string()),
...
async fn gateway_credentials_use_a_dedicated_encrypted_file_and_reload_large_tokens() {
    let kodex_home = tempfile::tempdir().expect("Kodex home");
    let keyring = Arc::new(MockKeyringStore::default());
    ...
    assert!(!kodex_home.path().join("secrets/kodex_auth.age").exists());
...
async fn browser_authorization_exchanges_and_persists_under_the_store_lock() {
    ...
    Mock::given(method("POST"))
        ...
        .and(body_string_contains("client_id=kodex-test"))
