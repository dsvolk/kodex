...
async fn classification_retries_refreshes_but_never_switches_accounts() -> Result<()> {
    ...
    for account in ["workspace-a", "workspace-b"] {
        ...
        let changes = Arc::new(AuthAfterRouting {
            initial: KodexAuth::from_external_chatgpt_tokens(
                &format!("{token}.initial"),
                "workspace-a",
                /*chatgpt_plan_type*/ None,
            )?,
            updated: KodexAuth::from_external_chatgpt_tokens(
        ...
        auth.set_workspace_routing_resolver(Arc::downgrade(&resolver));
        let mut config = sampler_config(format!("{}/backend-api/kodex", server.uri()));
...
async fn idle_classifier_retains_the_owning_threads_workspace_binding() -> Result<()> {
    ...
    let session_store = kodex_extension_api::ExtensionData::new("session-1");
    let input = kodex_extension_api::ThreadStartInput {
        config: &config,
        session_source: &kodex_protocol::protocol::SessionSource::Exec,
        persistent_thread_state_available: false,
        environments: &[],
        mcp_resource_client: None,
        extension_metrics: None,
        session_store: &session_store,
        thread_store: test.kodex.thread_extension_data(),
    };
    let owning_context = input
        .thread_store
        .get::<kodex_model_provider::WorkspaceRoutingContext>()
