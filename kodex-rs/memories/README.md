...
Runtime orchestration for Phase 1 and Phase 2 still lives in `kodex-core` under
`kodex-rs/core/src/memories/`.

## Crates

- `kodex-rs/memories/read` (`kodex-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `kodex-rs/memories/write` (`kodex-memories-write`) owns the write path:
...
- In `kodex`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.kodex/memories/.git` by `kodex-git-utils`
