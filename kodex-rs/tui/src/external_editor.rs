...
pub(crate) async fn run_editor(
    ...
    let default_kodex_home = dirs::home_dir().map(|home| home.join(".kodex"));
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    let project_kodex_home = cwd.join(".kodex");
    let mut candidate_homes = vec![kodex_home];
    if let Some(default_kodex_home) = default_kodex_home.as_deref() {
        candidate_homes.push(default_kodex_home);
    }
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    candidate_homes.push(&project_kodex_home);
