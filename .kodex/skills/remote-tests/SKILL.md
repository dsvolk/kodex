...
Use `TestKodexBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$KODEX_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `kodex_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap kodex_remote_env_cleanup EXIT

  cd kodex-rs
  just test -p kodex-core --test all
'
```
...
bash -c '
  ...
  cd kodex-rs
  just test -p kodex-app-server --test all
'
```
...
```sh
bazel test //kodex-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `kodex` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of kodex in `~/code/kodex`. Reset files if needed. Multiple checkouts take longer to build and take up more space.
