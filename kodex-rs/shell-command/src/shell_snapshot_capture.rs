...
const SNAPSHOT_COMMAND_HELPER: &str = r#"__kodex_snapshot_command() {
        ...
        script
            ...
            .replace(
                ...
                if options.declarations && options.environment {
                    "printf '\\0KODEX_SNAPSHOT_POSIX_STARTUP\\0%s\\0' \"$__kodex_env_file\"\n    SNAPSHOT_ENVIRONMENT\n    printf '\\0'"
                } else {
                    ""
                },
            )
...
setopt_count=$(setopt | __kodex_snapshot_command wc -l | __kodex_snapshot_command tr -d ' ')
print "# setopts $setopt_count"
setopt | __kodex_snapshot_command sed 's/^/setopt /'
print ''
printf '\0'
alias_count=$(alias -L | __kodex_snapshot_command wc -l | __kodex_snapshot_command tr -d ' ')
...
echo ''
SNAPSHOT_COMMAND_HELPER
bash_opts=$(set -o | __kodex_snapshot_command awk '$2=="on"{print $1}')
bash_opt_count=$(printf '%s\n' "$bash_opts" | __kodex_snapshot_command sed '/^$/d' | __kodex_snapshot_command wc -l | __kodex_snapshot_command tr -d ' ')
...
echo ''
printf '\0'
alias_count=$(alias -p | __kodex_snapshot_command wc -l | __kodex_snapshot_command tr -d ' ')
...
fn sh_snapshot_script(shell_startup: SnapshotStartup) -> String {
    let startup = match shell_startup {
        SnapshotStartup::Interactive => [
            ...
            r##"if [ -n "${ENV-}" ]; then
  __kodex_env_file=$(__kodex_snapshot_expand_env "$ENV")
  if [ -r "$__kodex_env_file" ] && [ ! -d "$__kodex_env_file" ]; then
    SNAPSHOT_STARTUP_ENVIRONMENT
  ...
  command unset __kodex_env_file
fi
command unset -f __kodex_snapshot_expand_env
"##,
        ]
        .join("\n"),
...
if set -o >/dev/null 2>&1; then
  sh_opts=$(set -o | __kodex_snapshot_command awk '$2=="on"{print $1}')
  sh_opt_count=$(printf '%s\n' "$sh_opts" | __kodex_snapshot_command sed '/^$/d' | __kodex_snapshot_command wc -l | __kodex_snapshot_command tr -d ' ')
...
if alias >/dev/null 2>&1; then
  alias_count=$(alias | __kodex_snapshot_command wc -l | __kodex_snapshot_command tr -d ' ')
  echo "# aliases $alias_count"
  alias
  echo ''
else
...
impl<'a> CapturedSnapshot<'a> {
    ...
    pub fn parse(shell_type: ShellType, captured: &'a [u8]) -> Option<Self> {
        ...
        let startup_marker = b"\0KODEX_SNAPSHOT_POSIX_STARTUP\0";
