...
use kodex_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use kodex_protocol::approvals::GuardianAssessmentAction as CoreGuardianAssessmentAction;
...
use kodex_protocol::items::CollabAgentToolCallItem;
use kodex_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use kodex_protocol::items::CommandExecutionItem;
...
use kodex_protocol::items::FileChangeItem;
use kodex_protocol::items::ImageViewItem;
use kodex_protocol::items::McpToolCallItem;
use kodex_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use kodex_protocol::items::ReasoningItem;
use kodex_protocol::items::SubAgentActivityItem;
use kodex_protocol::items::TurnItem;
use kodex_protocol::items::UserMessageItem;
use kodex_protocol::items::WebSearchItem as CoreWebSearchItem;
use kodex_protocol::mcp::CallToolResult;
use kodex_protocol::mcp::McpServerInfo;
use kodex_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use kodex_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use kodex_protocol::models::ImageReference as CoreImageReference;
...
use kodex_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use kodex_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use kodex_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: kodex_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        kodex_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        kodex_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.kodex/state",
            "logDir": "file:///C:/Users/alice/.kodex/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.kodex/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.kodex/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.kodex/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.kodex/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        kodex_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.kodex/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![kodex_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = kodex_protocol::ThreadId::default();
    let receiver_thread_id = kodex_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: kodex_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            kodex_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(kodex_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@kodex-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "kodex-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@kodex-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "kodex-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn kodex_error_info_serializes_http_status_code_in_camel_case() {
    let value = KodexErrorInfo::ResponseTooManyFailedAttempts {
...
fn core_error_info_converts_to_camel_case() {
    for (core, expected) in [
        (CoreKodexErrorInfo::CyberPolicy, json!("cyberPolicy")),
        (CoreKodexErrorInfo::BioPolicy, json!("other")),
        (CoreKodexErrorInfo::InvalidPrompt, json!("other")),
        (
            CoreKodexErrorInfo::RateLimitExceeded,
            json!("rateLimitExceeded"),
        ),
    ] {
        assert_eq!(
            serde_json::to_value(KodexErrorInfo::from(core)).unwrap(),
            expected
        );
    }
}
...
#[test]
fn kodex_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = KodexErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_cyber_access_program_uses_separate_wire_formats() {
    for (app_server_value, core_value) in [
        ...
        .expect("params should deserialize");
        let core_program: kodex_protocol::turn_input::CyberAccessProgram = params
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(kodex_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    let params = serde_json::from_value::<ThreadRealtimeStartParams>(json!({
        "threadId": "thread_123",
        "outputModality": "audio",
        "kodexResponseHandoffChannelPrefixes": {
            "analysis": ["[THINKING]"],
    ...
    assert_eq!(
        params.kodex_response_handoff_channel_prefixes,
        Some(BTreeMap::from([
