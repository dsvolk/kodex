...
pub(crate) fn run() -> Result<()> {
    let service_name = kodex_windows_sandbox::windows_sandbox_service_name()?;
    let state = ServiceState {
        service_name: service_name.clone(),
        pipe_name: kodex_windows_sandbox::windows_sandbox_service_pipe_name()?,
...
pub(crate) fn run_foreground() -> Result<()> {
    ...
    crate::ipc::run(
        ...
        || {
            eprintln!(
                "{} listening on {}",
                kodex_windows_sandbox::windows_sandbox_service_name()?,
                kodex_windows_sandbox::windows_sandbox_service_pipe_name()?
            );
            Ok(())
        },
...
unsafe extern "system" fn service_main(_argument_count: u32, _arguments: *mut *mut u16) {
    ...
    if let Err(error) = service_main_inner(state) {
        log_error(
            ...
            &format!("The Kodex sandbox service encountered a fatal error: {error:#}"),
        );
...
fn service_main_inner(state: &ServiceState) -> Result<()> {
    ...
    log_information(
        ...
        "The Kodex sandbox service has stopped.",
    );
    ...
    match control {
        SERVICE_CONTROL_STOP | SERVICE_CONTROL_SHUTDOWN => {
            ...
            if !state.shutdown.swap(true, Ordering::AcqRel) {
                ...
                log_information(
                    ...
                    "The Kodex sandbox service was asked to stop.",
                );
                wake_listener();
            }
            NO_ERROR
        ...
        SERVICE_CONTROL_INTERROGATE => {
            ...
            if let Err(error) = state.report_status(current_status, NO_ERROR) {
                log_error(
                    EVENT_SERVICE_FAILED,
                    "The Kodex sandbox service could not report its current status.",
                );
...
fn log_event(event_type: u16, event_id: u32, message: &str) {
    let source = SERVICE_STATE
        .get()
        .map_or("KodexSandboxService", |state| state.service_name.as_str())
        .encode_utf16()
        .chain(std::iter::once(0))
        .collect::<Vec<_>>();
