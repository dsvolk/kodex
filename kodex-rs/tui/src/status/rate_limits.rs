...
use chrono::Local;
use chrono::Utc;
use kodex_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use kodex_app_server_protocol::RateLimitSnapshot;
use kodex_app_server_protocol::RateLimitWindow;
use kodex_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use kodex_protocol::num_format::format_with_separators;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `kodex` or `kodex_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "kodex".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    ordered_snapshots.sort_by_key(|snapshot| !snapshot.limit_name.eq_ignore_ascii_case("kodex"));
    for snapshot in ordered_snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("kodex");
        ...
        let combine_non_kodex_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_kodex_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_kodex_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_kodex_single_limit_renders_combined_row() {
        let now = Local::now();
        let kodex = RateLimitSnapshotDisplay {
            limit_name: "kodex".to_string(),
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "kodex-other".to_string(),
            normal_model_slug: None,
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[kodex, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "kodex-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_kodex_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "kodex-other".to_string(),
            normal_model_slug: None,
