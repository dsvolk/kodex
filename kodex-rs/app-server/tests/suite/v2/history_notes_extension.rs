...
async fn app_server_uses_configured_notes_backend_for_context_window_hints(
    ...
    let mut model = kodex_core::test_support::construct_model_info_offline("mock-model", &config);
    model.supports_experimental_context = true;
    let catalog_path = kodex_home.path().join("models.json");
    std::fs::write(
        &catalog_path,
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .with_provider_base_url(&format!("{}/backend-api/kodex", server.uri()))
    ...
    mount_analytics_capture(&backend, kodex_home.path()).await?;
    ...
    let native_requests = requests
        .iter()
        .filter(|request| request.url.path() == "/backend-api/kodex/alpha/notes/v2/thread_hint")
        .count();
    ...
    if use_history_notes_extension {
        let event = wait_for_matching_analytics_event(&backend, DEFAULT_READ_TIMEOUT, |event| {
            event["event_type"] == "kodex_thread_hint_status"
...
async fn history_notes_and_async_message_emit_control_tool_analytics() -> Result<()> {
    ...
    for (namespace, tool, arguments) in &calls[..9] {
        Mock::given(method("POST"))
            .and(path(format!(
                "/backend-api/kodex/alpha/{namespace}/v2/{tool}"
            )))
