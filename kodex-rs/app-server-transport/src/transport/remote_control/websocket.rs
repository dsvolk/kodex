...
pub(super) const REMOTE_CONTROL_INSTALLATION_ID_HEADER: &str = "x-kodex-installation-id";
const REMOTE_CONTROL_SUBSCRIBE_CURSOR_HEADER: &str = "x-kodex-subscribe-cursor";
...
enum ConnectOutcome {
    Connected(Box<kodex_websocket_client::WebSocketConnection>),
    Disabled,
    Shutdown,
}
...
impl RemoteControlWebsocket {
    ...
    async fn run_connection(
        &self,
        websocket_connection: kodex_websocket_client::WebSocketConnection,
        shutdown_token: CancellationToken,
    ) -> ConnectionEndReason {
    ...
    async fn run_server_writer(
        ...
        websocket_writer: SplitSink<
            kodex_websocket_client::WebSocketConnection,
            tungstenite::Message,
        >,
    ...
    async fn run_websocket_reader(
        client_tracker: Arc<Mutex<ClientTracker>>,
        state: Arc<Mutex<WebsocketState>>,
        websocket_reader: SplitStream<kodex_websocket_client::WebSocketConnection>,
    ...
    async fn run_websocket_reader_inner(
        client_tracker: Arc<Mutex<ClientTracker>>,
        state: Arc<Mutex<WebsocketState>>,
        mut websocket_reader: SplitStream<kodex_websocket_client::WebSocketConnection>,
        pong_timeout: std::time::Duration,
        shutdown_token: CancellationToken,
    ) -> io::Result<()> {
...
async fn build_remote_control_websocket_request(
    ...
    set_remote_control_header(headers, "x-kodex-server-id", &enrollment.server_id)?;
...
pub(super) async fn connect_remote_control_websocket(
    ...
    let websocket_connect_result =
        tokio::time::timeout(REMOTE_CONTROL_WEBSOCKET_CONNECT_TIMEOUT, async {
            ...
            let connector = tokio::task::spawn_blocking(move || {
                kodex_websocket_client::WebSocketConnector::new(&factory).or_else(|_| {
                    kodex_websocket_client::WebSocketConnector::new_with_tls_mode(
                        &factory,
                        kodex_websocket_client::WebSocketTlsMode::TungsteniteDefault,
                    )
                })
            })
            .await
    ...
    match websocket_connect_result {
        ...
        Err(err) => {
            if let Some(denied) = kodex_websocket_client::network_policy_denial(&err) {
...
mod tests {
    ...
    pub(super) async fn remote_control_state_runtime(kodex_home: &TempDir) -> Arc<StateRuntime> {
        StateRuntime::init(
    ...
    async fn connected_websocket_pair() -> (
        ...
        let connect_task = tokio::spawn(async move {
            kodex_websocket_client::WebSocketConnector::new(
