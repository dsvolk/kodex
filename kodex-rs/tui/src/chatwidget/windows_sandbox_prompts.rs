...
impl ChatWidget {
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        self.session_telemetry.counter(
            "kodex.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Kodex agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/kodex/windows>"],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Kodex agent sandbox to continue. Set it up to protect your files and control network access."],
                    line!["Learn more <https://developers.openai.com/kodex/windows>"],
                ]
            })
        ...
        let elevated_item = SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                accept_otel.counter(
                    "kodex.windows_sandbox.elevated_prompt_accept",
                    /*inc*/ 1,
                    &[],
                );
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "kodex.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                quit_otel.counter(
                    "kodex.windows_sandbox.elevated_prompt_quit",
                    /*inc*/ 1,
                    &[],
                );
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Kodex in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Kodex can continue."
            ]);
        }
        lines.push(line![
            "Learn more <https://developers.openai.com/kodex/windows>"
        ]);
        ...
        let elevated_item = SelectionItem {
            ...
            actions: vec![Box::new({
                ...
                move |tx| {
                    otel.counter(
                        "kodex.windows_sandbox.fallback_retry_elevated",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Kodex with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "kodex.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                quit_otel.counter(
                    "kodex.windows_sandbox.fallback_prompt_quit",
                    /*inc*/ 1,
                    &[],
                );
                tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
