...
impl WindowsSandboxRequestProcessor {
    ...
    pub(crate) async fn windows_sandbox_readiness(
        ...
        #[cfg(target_os = "windows")]
        if kodex_windows_sandbox::registered_core_requested()
            && matches!(
                WindowsSandboxLevel::from_config(&self.config),
                WindowsSandboxLevel::Elevated
            )
            && !kodex_login::is_workload_identity_selected()
        {
            ...
            tokio::spawn(async move {
                ...
                processor.registration_refresh.get_or_init(|| async {
                    ...
                    let result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                        if !kodex_windows_sandbox::registered_core_needs_refresh(&config.kodex_home)? {
                            return Ok(());
                        }
                        ...
                        let (settings, listeners) = match config.permissions.network.as_ref() {
                            ...
                            None => (
                                kodex_windows_sandbox::WindowsSandboxProvisioningSettings::from_environment(&policy, &env_map),
                                kodex_windows_sandbox::WindowsSandboxProxyListeners::from_environment(&policy, &env_map),
                            ),
                        };
                        kodex_windows_sandbox::refresh_registered_core_via_service(
                            &config.kodex_home, settings, listeners,
                        )?;
                        Ok(())
        ...
        tokio::spawn(async move {
            ...
            let setup_result = async {
                ...
                if setup_mode == CoreWindowsSandboxSetupMode::Elevated
                    && (kodex_windows_sandbox::registered_core_requested()
                    ...
                    && !kodex_login::is_workload_identity_selected()
                {
                    let provisioning = match config
                        ...
                        .map_or_else(
                            || {
                                Ok((
                                    kodex_windows_sandbox::WindowsSandboxProvisioningSettings::from_environment(
                                        &setup_request.permission_profile,
                                        &setup_request.env_map,
                                    ),
                                    kodex_windows_sandbox::WindowsSandboxProxyListeners::from_environment(
                                        &setup_request.permission_profile,
                                        &setup_request.env_map,
                                    ),
                                ))
                            },
                            kodex_core::config::NetworkProxySpec::windows_sandbox_proxy_listeners,
                        )
                    {
                    ...
                    if let Some((settings, listeners)) = provisioning {
                        ...
                        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                            let Ok(permissions) = kodex_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                                &service_setup_request.permission_profile,
                                &service_setup_request.workspace_roots,
                            ) else {
                                anyhow::ensure!(!kodex_windows_sandbox::registered_core_requested(),
                            ...
                            kodex_windows_sandbox::provision_windows_sandbox_via_service(
                                &service_setup_request.kodex_home,
                                settings,
                                listeners,
                            )?;
                            Ok(())
                        ...
                        .inspect_err(|error| {
                            kodex_core::windows_sandbox::emit_windows_sandbox_setup_failure_metrics(
                                setup_mode,
                ...
                kodex_core::windows_sandbox::run_windows_sandbox_setup(setup_request).await
            }
            .await;
...
fn resolve_allowed_windows_sandbox_setup_mode(
    requirements: &kodex_config::ConfigRequirements,
    ...
    let (setup_mode, config_mode) = match requested_mode {
        WindowsSandboxSetupMode::Elevated => (
            ...
            kodex_config::types::WindowsSandboxModeToml::Elevated,
        ),
        WindowsSandboxSetupMode::Unelevated => (
            CoreWindowsSandboxSetupMode::Unelevated,
            kodex_config::types::WindowsSandboxModeToml::Unelevated,
        ),
    };
    requirements
...
fn determine_windows_sandbox_readiness(config: &Config) -> WindowsSandboxReadinessResponse {
    ...
    determine_windows_sandbox_readiness_from_state(
        ...
        sandbox_setup_is_complete(config.kodex_home.as_path()),
    )
}
