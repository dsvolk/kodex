...
use kodex_utils_cli::format_env_display;
...
/// - `add`    — add a server launcher entry to `~/.kodex/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "kodex mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&kodex_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", kodex_home.display()))?;
    ...
    ConfigEditsBuilder::new(&kodex_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", kodex_home.display()))?;
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `kodex mcp login {name}` to login."
        ),
    }
...
async fn run_login(config: &Config, login_args: LoginArgs) -> Result<()> {
    ...
    if matches!(server.auth, kodex_config::types::McpServerAuth::EmaAuth) {
        bail!(
            "Enterprise MCP authorization is managed by Kodex account sign-in. Open Kodex to sign in."
        );
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `kodex mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: kodex mcp remove {}", get_args.name);
