...
RUNTIME_PKG_NAME = "openai-kodex-cli-bin"
...
def _installed_kodex_path() -> Path:
    try:
        from kodex_cli_bin import bundled_kodex_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Kodex runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set KodexConfig.kodex_bin "
            "explicitly."
        ) from exc

    return bundled_kodex_path()


def _installed_kodex_path_dirs() -> tuple[Path, ...]:
    try:
        from kodex_cli_bin import bundled_path_dir
...
class KodexBinResolverOps:
    installed_kodex_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_kodex_bin_resolver_ops() -> KodexBinResolverOps:
    return KodexBinResolverOps(
        installed_kodex_path=_installed_kodex_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_kodex_bin(config: "KodexConfig", ops: KodexBinResolverOps) -> Path:
    if config.kodex_bin is not None:
        kodex_bin = Path(config.kodex_bin)
        if not ops.path_exists(kodex_bin):
            raise FileNotFoundError(
                f"Kodex binary not found at {kodex_bin}. Set KodexConfig.kodex_bin "
                "to a valid binary path."
            )
        return kodex_bin

    return ops.installed_kodex_path()


def _resolve_kodex_bin(config: "KodexConfig") -> Path:
    return resolve_kodex_bin(config, _default_kodex_bin_resolver_ops())


@dataclass(slots=True)
class KodexConfig:
    """Configuration for launching and identifying the local Kodex runtime.

    Most callers can use ``Kodex()`` without configuration. Set ``kodex_bin``
    only when intentionally using a specific local Kodex executable.
    """

    kodex_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "kodex_python_sdk"
    client_title: str = "Kodex Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class KodexClient:
    """Synchronous typed JSON-RPC client for `kodex app-server` over stdio."""

    def __init__(
        self,
        config: KodexConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or KodexConfig()
    ...
    def __enter__(self) -> "KodexClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            kodex_bin = _resolve_kodex_bin(self.config)
            if self.config.kodex_bin is None:
                path_dirs = _installed_kodex_path_dirs()
            args = [str(kodex_bin)]
        ...
        if supplied_fields:
            try:
                ...
                raise KodexError(
                    ...
                    "Configure KodexConfig.kodex_bin with a supported CLI."
                ) from exc
        result = self._request_raw(method, params)
        if not isinstance(result, dict):
            raise KodexError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Kodex process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Kodex process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise KodexError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise KodexError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_kodex_home() -> str:
    return str(Path.home() / ".kodex")
