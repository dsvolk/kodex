...
From `kodex-rs`, preview a file, optionally specifying the available width:

```sh
cargo run -p kodex-mermaid --example render -- 180 < diagram.mmd
```

Run `just test -p kodex-mermaid --lib`. Coverage includes complex snapshots for every
