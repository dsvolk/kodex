...
use kodex_code_mode::CodeModeToolKind;
...
use kodex_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use kodex_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = kodex_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = kodex_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = kodex_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) fn take_unavailable_warning(&self, tool_mode: ToolMode) -> Option<String> {
        ...
        .then(|| {
            format!(
                "Code Mode is unavailable because {error}. {behavior}; enable `features.code_mode_host` and install `kodex-code-mode-host`."
            )
        })
    }
    ...
    pub(crate) async fn execute(
        &self,
        mut request: kodex_code_mode::ExecuteRequest,
        step_context: Arc<StepContext>,
    ) -> Result<kodex_code_mode::StartedCell, String> {
        request
            .yield_time_ms
    ...
    ) -> Result<kodex_code_mode::WaitOutcome, String> {
...
> {
    ...
    tracing::event!(
        name: "kodex.code_mode.nested_tool_dispatched",
        target: "kodex_otel.trace_safe",
        tracing::Level::INFO,
        event.name = "kodex.code_mode.nested_tool_dispatched",
    ...
    session
        ...
        .track_code_mode_tool_call(kodex_analytics::CodeModeToolCallFact::ChildStarted {
