...
fn test_api_provider_applies_current_managed_residency() {
    let info = ModelProviderInfo {
        http_headers: Some(maplit::hashmap! {
            "X-OpenAI-Internal-Kodex-Residency".to_string() => "eu".into(),
...
#[test]
fn test_personal_access_token_uses_chatgpt_kodex_base_url() {
    ...
    assert_eq!(api_provider.base_url, CHATGPT_KODEX_BASE_URL);
}

#[test]
fn test_header_auth_uses_chatgpt_kodex_base_url() {
    let api_provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None)
...
#[test]
fn kodex_backend_routes_require_kodex_base_url() {
    for (base_url, expected) in [
        (None, true),
        (Some(CHATGPT_KODEX_BASE_URL), true),
        (Some("https://chatgpt-staging.com/backend-api/kodex/"), true),
        (Some("https://proxy.example.com/v1"), false),
    ] {
        let provider = ModelProviderInfo::create_openai_provider(base_url.map(str::to_owned));
        assert_eq!(provider.supports_kodex_backend_routes(), expected);
    }
}
...
command = "aws-vault"
args = ["--profile", "kodex-bedrock"]

[aws.auth_refresh]
command = "aws"
args = ["login", "--profile", "kodex-bedrock"]
        "#;
...
fn test_merge_configured_model_providers_applies_amazon_bedrock_aws_override() {
    let credential_export = AwsCredentialExportConfig {
        command: "aws-vault".to_string(),
        args: vec!["export".into(), "kodex-bedrock".into()],
        timeout_ms: NonZeroU64::new(30_000).expect("timeout should be non-zero"),
    };
...
fn test_validate_provider_aws_credential_export_command() {
    ...
    for (command, expected) in [
        ...
        assert_eq!(provider.validate(), expected);
        if expected.is_ok() {
            provider.aws.as_mut().expect("AWS config").profile = Some("kodex-bedrock".to_string());
            assert_eq!(
                provider.validate(),
...
model_catalog_url = "https://gateway.example/kodex/catalog?token=catalog-secret"
"#,
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        ModelProviderInfo {
            ...
            model_catalog_url: Some(
                "https://gateway.example/kodex/catalog?token=catalog-secret".into()
            ),
            ..ModelProviderInfo::default()
        }
    );
