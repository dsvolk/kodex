...
use kodex_extension_api::ThreadIdleInput;
...
use kodex_history::GuardianHistoryCheckpoint;
use kodex_history::InitialHistory;
use kodex_history::ResumedHistory;
use kodex_history::RetainedContext;
use kodex_history::RetainedContextEvent;
use kodex_history::RolloutItem;
use kodex_history::VerifiedAnswer;
use kodex_history::VerifiedQuestionAnswer;
...
async fn wait_for_thread_idle(thread: &KodexThread) {
...
async fn load_context(test: &TestKodex, thread: &KodexThread) -> Result<Vec<RolloutItem>> {
    Ok(test
        .thread_store
...
async fn resume(test: &TestKodex, thread: &KodexThread) -> Result<Arc<KodexThread>> {
...
async fn retained_instructions_keep_identity_across_compaction_and_resume(
    ...
    let thread = resume(&test, &test.kodex).await?;
    ...
    let user_messages = [initial.as_str(), STEER]
        ...
        .map(|(index, text)| {
            ...
            json!({
                ...
                "text": kodex_guardian_context::truncate_text(text, /*max_tokens*/ 900),
...
async fn legacy_checkpoint_recovers_root_excerpt_before_discarding_backup(
    ...
    .await;
    let snapshot = test.kodex.conversation_history_snapshot().await;
    let source = snapshot
        .items()
        .find(|item| matches!(item, ResponseItem::Message { role, .. } if role == "user"))
        .cloned()
        .context("original instruction")?;
    let mut expected = compact_and_assert_answers(&test, &test.kodex, &[]).await?;
    let mut checkpoint = load_context(&test, &test.kodex)
        .await?
        .into_iter()
        .rev()
        .find_map(|item| match item {
    ...
    test.kodex
        ...
        .await?;
    let resumed = resume(&test, &test.kodex).await?;
    assert_eq!(
        resumed
            .conversation_history_snapshot()
            .await
...
async fn legacy_rollback_replay_retains_only_surviving_steered_answers() -> Result<()> {
    ...
    test.kodex
        ...
        .await?;
    let mut thread = resume(&test, &test.kodex).await?;
...
async fn standalone_fork_retains_inherited_user_instructions(
    ...
    let fork = if history_mode == ThreadHistoryMode::Paginated {
        ...
        test.thread_manager
            .fork_prepared_thread(
                kodex_core::StartThreadOptions::new(test.config.clone()),
                prepared,
            )
            .await?
    } else {
    ...
    let answers = kodex_guardian_context::render_verified_answers(retained);
    assert_eq!(
        (answers.fragments, answers.complete),
        (
            vec![kodex_guardian_context::GuardianRootMessage::IncompleteVerifiedAnswers.render()],
            false
        ),
    );
    assert_eq!(
        retained
            ...
            .map(|(_, entry)| match entry {
                kodex_history::RetainedContextEntry::UserMessage(message) =>
                    (message.text.clone(), message.complete),
                kodex_history::RetainedContextEntry::VerifiedAnswer(_) =>
        ...
        vec![
            ...
            (
                kodex_guardian_context::truncate_text(&local_instruction, /*max_tokens*/ 900),
                !compacted,
            ),
        ],
    );
...
async fn forked_parent_instructions_do_not_become_local_authorization(
    ...
    .await;
    wait_for_thread_idle(&test.kodex).await;
    if compact_parent {
    ...
    assert!(
        root_snapshot
            .messages
            .contains(&kodex_core::GuardianRootMessage::User(
    ...
    assert_eq!(
        expected.as_ref().map(|context| context
            .ordered_entries()
            .map(|(_, entry)| match entry {
                kodex_history::RetainedContextEntry::UserMessage(message) => message.text.as_str(),
                kodex_history::RetainedContextEntry::VerifiedAnswer(_) =>
