...
# Run a suite of smoke tests against the Windows sandbox via the Kodex CLI
...
def _resolve_kodex_cmd() -> List[str]:
    """Resolve the Kodex CLI to invoke `kodex sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Kodex.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "kodex.exe",
        ws_root / "target" / "release" / "kodex.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend(
            [
                cargo_base / "debug" / "kodex.exe",
                cargo_base / "release" / "kodex.exe",
            ]
        )
    ...
    if shutil.which("kodex"):
        return ["kodex"]

    raise FileNotFoundError(
        "Kodex CLI not found. Build it first, e.g.\n"
        "  cargo build -p kodex-cli --release\n"
        "or for debug:\n"
        "  cargo build -p kodex-cli\n"
    )


KODEX_CMD = _resolve_kodex_cmd()
print(KODEX_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to kodex CLI overrides.
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"KODEX_HOME": str(proxy_home)},
            )
            add(
    ...
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", "echo hi > \\\\.\\pipe\\kodex_testpipe"],
        WS_ROOT,
    )
    ...
    # 34. WS: policy tamper (.kodex artifacts) denied
    kodex_home = Path(os.environ["USERPROFILE"]) / ".kodex"
    cap_sid_target = kodex_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
    ...
    rc2, out2, err2 = run_sbx(
        "workspace-write", ["cmd", "/c", "echo tamper > .kodex\\policy.json"], WS_ROOT
    )
    add("WS: .kodex cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .kodex policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo owned > kodex_escape.txt"], fake_root
        )
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://kodex-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
