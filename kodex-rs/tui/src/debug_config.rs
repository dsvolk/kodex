...
use kodex_config::NetworkUnixSocketPermissionToml;
use kodex_config::RequirementSource;
use kodex_config::ResidencyRequirement;
use kodex_config::SandboxModeRequirement;
use kodex_config::WebSearchModeRequirement;
use kodex_config::format_config_layer_source;
...
mod tests {
    ...
    use kodex_config::ConstraintError;
    use kodex_config::FeatureRequirementsToml;
    use kodex_config::FilesystemConstraints;
    use kodex_config::HookEventsToml;
    use kodex_config::HookHandlerConfig;
    use kodex_config::LoaderOverrides;
    use kodex_config::ManagedHooksRequirementsToml;
    use kodex_config::MatcherGroup;
    use kodex_config::McpServerIdentity;
    use kodex_config::McpServerRequirement;
    use kodex_config::NetworkConstraints;
    use kodex_config::NetworkDomainPermissionToml;
    use kodex_config::NetworkDomainPermissionsToml;
    use kodex_config::NetworkHeaderInjectionToml;
    use kodex_config::NetworkUnixSocketPermissionToml;
    use kodex_config::NetworkUnixSocketPermissionsToml;
    use kodex_config::RequirementSource;
    use kodex_config::ResidencyRequirement;
    use kodex_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\kodex\\config.toml")
        } else {
            absolute_path("/etc/kodex/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.kodex")
        } else {
            absolute_path("/repo/.kodex")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_kodex_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Kodex\\requirements.toml")
        } else {
            absolute_path("/etc/kodex/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.kodex\\state")
        } else {
            absolute_path("/home/alice/.kodex/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.kodex\\logs")
        } else {
            absolute_path("/home/alice/.kodex/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.kodex\\models.json")
        } else {
            absolute_path("/home/alice/.kodex/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.kodex\\config.toml")
        } else {
            absolute_path("/home/alice/.kodex/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/kodex.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\kodex")
        } else {
            absolute_path("/var/lib/kodex")
        };
