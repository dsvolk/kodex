...
use kodex_protocol::openai_models::ModelMessages;
...
const REMOTE_MODEL_SLUG: &str = "kodex-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Kodex".to_string(),
    ...
    let manager = kodex_core::test_support::models_manager_with_provider(
        kodex_home.path().to_path_buf(),
        kodex_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-kodex-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-kodex-test");
    assert_eq!(
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let turn_started_event = wait_for_event(&kodex, |event| {
        matches!(
            event,
...
async fn remote_models_long_model_slug_is_sent_with_supported_reasoning(
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-kodex-test";
    let prefix_model = "gpt-5.3-kodex";
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "custom/gpt-5.5-kodex";
    let response_mock = mount_sse_once(
        &server,
        sse(vec![ev_response_created("resp-1"), ev_completed("resp-1")]),
    )
    ...
    loop {
        let event = wait_for_event(&kodex, |_| true).await;
        match event {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "kodex-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "kodex-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(); "chatgpt")]
#[test_case(KodexAuth::from_api_key("test-api-key"); "api key")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn remote_models_apply_legacy_instructions(auth: KodexAuth) -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));
    ...
    let mut builder = test_kodex().with_auth(auth).with_config(|config| {
        config
            .features
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "kodex-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    kodex_core::test_support::all_model_presets()
        .iter()
...
async fn model_catalog_url_supplies_conversation_model_and_instructions() -> Result<()> {
    ...
    let mut model = kodex_models_manager::model_info::model_info_from_slug("gateway-conversation");
    ...
    .await;
    let catalog_url = format!("{}/kodex/models", server.uri());
    let test = test_kodex()
        .with_auth(KodexAuth::from_api_key("gateway-api-key"))
        ...
        .with_config(move |config| {
            ...
            config.model_verbosity = Some(kodex_protocol::config_types::Verbosity::High);
        })
        .build_with_auto_env(&server)
        .await?;
    let received = test
