...
use kodex_config::permissions_toml::NetworkUnixSocketPermissionToml;
use kodex_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use kodex_features::NetworkProxyDomainPermissionToml;
use kodex_features::NetworkProxyModeToml;
use kodex_features::NetworkProxyUnixSocketPermissionToml;
use kodex_network_proxy::NetworkMode;
use kodex_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use kodex_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
/// for Kodex to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_kodex_runtime(
    kodex_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(kodex_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Kodex versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Kodex. Filesystem access will remain restricted. Upgrade Kodex if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Kodex and will be ignored. Upgrade Kodex if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Kodex and will be ignored. Upgrade Kodex if this path is required."
            ),
        },
    );
}
