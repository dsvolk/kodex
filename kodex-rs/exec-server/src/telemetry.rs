...
impl ExecServerTelemetry {
    ...
    pub(crate) fn shell_snapshot_captured(
        ...
        let _ = metrics.record_duration("kodex.shell_snapshot.duration_ms", duration, &tags);
        let _ = metrics.counter("kodex.shell_snapshot", /*inc*/ 1, &tags);
    }
    ...
    pub(crate) fn process_started(&self, process_id: &str) -> ProcessMetricGuard {
        ...
        let parent = kodex_otel::current_span_w3c_trace_context();
        ...
        let span = tracing::info_span!(
            parent: None,
            "kodex.exec_server.process",
            otel.kind = "internal",
            process.id = process_id,
            result = tracing::field::Empty,
        );
        if let Some(parent) = parent {
            kodex_otel::set_parent_from_w3c_trace_context(&span, &parent);
        }
        ProcessMetricGuard {
            telemetry: self.clone(),
            span,
