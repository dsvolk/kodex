...
use kodex_app_server_protocol::ThreadEnvironment;
use kodex_app_server_transport::daemon_recovery::InterruptedTurn;
...
impl ThreadRequestProcessor {
    pub(crate) async fn continue_daemon_turn(&self, thread_id: &str, saved: InterruptedTurn) {
        ...
        let history = match kodex_rollout::RolloutRecorder::load_rollout_items(&path).await {
        ...
        if environment.environment_id != kodex_exec_server::LOCAL_ENVIRONMENT_ID
