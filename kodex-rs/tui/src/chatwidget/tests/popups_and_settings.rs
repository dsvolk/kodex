...
use kodex_app_server_protocol::HookErrorInfo;
use kodex_app_server_protocol::HooksListEntry;
use kodex_app_server_protocol::HooksListResponse;
use kodex_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (kodex_app_server_protocol::HookEventName::PreToolUse, 1),
            (kodex_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn plugin_detail_popup_shows_local_share_context_as_read_only_snapshot() {
    ...
    let summary = PluginSummary {
        share_context: Some(PluginShareContext {
            ...
            share_url: Some("https://chatgpt.com/kodex/plugins/share/docs".to_string()),
...
async fn experimental_features_popup_snapshot() {
    ...
    let features = vec![
        ...
        ExperimentalFeatureItem {
            ...
            description: "Talk with Kodex using /voice.".to_string(),
            enabled: false,
        },
    ];
    ...
    let mut config = kodex_config::types::TuiKeymap::default();
    config.list.accept = Some(kodex_config::types::KeybindingsSpec::One(
        kodex_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn experimental_popup_available_snapshot() {
    ...
    let features = [
        ...
        ("prevent_idle_sleep", "Prevent sleep while running", "Keep your computer awake while Kodex is running a thread."),
    ]
    .into_iter()
    .map(|(name, display_name, description)| kodex_app_server_protocol::ExperimentalFeature {
        name: name.to_string(),
        stage: kodex_app_server_protocol::ExperimentalFeatureStage::Beta,
...
async fn model_picker_refreshes_startup_catalog() {
    for (explicit_all_models, hidden_startup) in [(false, false), (true, false), (false, true)] {
        ...
        auto.model = "kodex-auto-test".to_string();
...
async fn model_picker_refresh_dismisses_empty_choices() {
        ...
        [(false, false), (true, false), (false, true), (true, true)]
    {
        ...
        let before = render_bottom_popup(&chat, /*width*/ 80);
        if explicit_all_models {
            preset.model = "kodex-auto-test".to_string();
        } else {
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(KodexErrorInfo::ServerOverloaded),
    );
...
async fn reasoning_up_shortcut_does_not_silently_enter_advanced_effort() {
    for (model, model_path) in [
        ...
        ("kodex-auto-test", "kodex-auto-test"),
    ] {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.5")).await;
        chat.thread_id = Some(ThreadId::new());
        let mut preset = get_available_model(&chat, "gpt-5.5");
...
async fn auto_model_advertising_advanced_effort_opens_reasoning_picker() {
    ...
    preset.id = "kodex-auto-test".to_string();
    preset.model = "kodex-auto-test".to_string();
    preset.display_name = "kodex-auto-test".to_string();
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &kodex_feedback::FeedbackDiagnostics::new(vec![kodex_feedback::FeedbackDiagnostic {
