...
pub fn clean_up_packaged_windows_sandbox(
    ...
    prepare_packaged_windows_sandbox_cleanup()?.finish(kodex_home, report, clean_up_desktop)
}
    ...
    pub fn finish(
        ...
        if let Some(kodex_home) = kodex_home {
            ...
            for (directory, path) in [
                (".sandbox", crate::setup::sandbox_dir(kodex_home)),
                (
                    ".sandbox-secrets",
                    crate::setup::sandbox_secrets_dir(kodex_home),
                ),
                (".sandbox-bin", crate::setup::sandbox_bin_dir(kodex_home)),
            ] {
        ...
        {
            match principals::remove_sandbox_principal("KodexSandboxUsers") {
