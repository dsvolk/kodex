...
pub(super) async fn fetch_account_rate_limits(
    ...
    if matches!(
        &result,
        Err(kodex_app_server_client::TypedRequestError::Server { source, .. })
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<kodex_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<kodex_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: kodex_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    } else if err.contains("kodex plugins are disabled")
        || err.contains("plugin sharing is disabled")
        || err.contains("plugin sharing is not enabled")
        || err.contains("feature disabled")
    {
        "Ask a workspace admin to enable Kodex plugins or plugin sharing"
    ...
    } else if err.contains("old build") || err.contains("update kodex") || err.contains("stale") {
        "Update Kodex, then try opening the shared plugin again"
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Enable plugin sharing for this Kodex session to load shared plugins".to_string(),
    }
}
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![kodex_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, kodex_protocol::mcp::Tool>,
    HashMap<String, Vec<kodex_protocol::mcp::Resource>>,
    HashMap<String, Vec<kodex_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                kodex_app_server_protocol::McpAuthStatus::Unknown => McpAuthStatus::Unknown,
                kodex_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                kodex_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                kodex_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                kodex_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    async fn fetch_thread_usage_uses_app_server_auth_after_persisted_account_changes() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/api/kodex/config/bundle"))
        ...
        wiremock::Mock::given(wiremock::matchers::method("POST"))
            .and(wiremock::matchers::path(
                "/api/kodex/usage/thread_usage/query",
            ))
    ...
    fn plugin_sharing_disabled_remote_section_error_targets_shared_with_me() {
        assert_eq!(
            ...
            PluginRemoteSectionError {
                section_id: "shared-with-me".to_string(),
                label: "Shared with me".to_string(),
                message: "Enable plugin sharing for this Kodex session to load shared plugins"
                    .to_string(),
            }
        );
    }
