        ...
        for source_kinds in source_kind_filters {
            ...
            loop {
                ...
                for thread in response.data {
                    ...
                    {
                        let expected_root = kodex_home.join(match collection {
                            SessionCollection::Active => kodex_rollout::SESSIONS_SUBDIR,
                            SessionCollection::Archived => kodex_rollout::ARCHIVED_SESSIONS_SUBDIR,
                        });
                        if !path.starts_with(expected_root)
                            || (thread.history_mode == ThreadHistoryMode::Legacy
                                && kodex_rollout::existing_rollout_path(path).await.is_none())
                        {
                            continue;
                        }
                    }
