...
use kodex_analytics::GuardianApprovalRequestSource;
...
use kodex_guardian_context::ConversationTranscriptEntry;
use kodex_guardian_context::ConversationTranscriptEntryKind;
...
use kodex_models_manager::manager::StaticModelsManager;
...
use kodex_protocol::approvals::GuardianAssessmentAction;
...
// The memories extension depends on kodex-core, so this probe verifies the nested Guardian config
...
impl kodex_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
...
impl kodex_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a kodex_extension_api::ExtensionData,
        thread_store: &'a kodex_extension_api::ExtensionData,
    ) -> kodex_extension_api::ExtensionFuture<'a, Vec<kodex_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![kodex_extension_api::PromptFragment::developer_policy(
                    GUARDIAN_MEMORY_CONTEXT_PROBE,
                    kodex_extension_api::ContentItemKind(
...
fn guardian_exec_command_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::ExecCommand {
        ...
        cwd: test_path_buf("/repo/kodex-rs/core").abs().into(),
        guardian_cwd: native_guardian_cwd("/repo/kodex-rs/core"),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/kodex\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    ...
                    name: None,
                    namespace: None,
                    output: kodex_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/kodex-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[kodex_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            kodex_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = kodex_utils_output_truncation::truncate_text(
        &approval_reason,
        kodex_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = items
        ...
        .find_map(|item| match item {
            kodex_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: kodex_protocol::permissions::FileSystemSpecialPath::Root,
                },
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: kodex_protocol::permissions::FileSystemSpecialPath::project_roots(Some(
                        "private".to_string(),
                    )),
                },
...
async fn background_approval_permissions_use_the_owning_environment() -> anyhow::Result<()> {
    ...
    windows.environment = Arc::new(kodex_exec_server::Environment::create_for_tests(Some(
        "ws://127.0.0.1:1".into(),
    ))?);
    ...
    let requests = [
        ...
        GuardianApprovalRequest::Execve {
            ...
            source: kodex_protocol::approvals::GuardianCommandSource::UnifiedExec,
...
-> anyhow::Result<()> {
    ...
    selection.config = kodex_protocol::protocol::EnvironmentConfigState::Failed("offline".into());
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Kodex agent has requested the following next action:\n"));
...
fn collect_guardian_transcript_entries(
    history: &dyn kodex_guardian_context::SectionHistory,
...
fn collect_guardian_transcript_entries_includes_recent_tool_calls_and_output() {
    ...
    if let ResponseItem::FunctionCallOutput { output, .. } = &mut items[2] {
        *output =
            kodex_protocol::models::FunctionCallOutputPayload::from_text(oversized_result.clone());
    }
    for token_cap in [
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    assert!(!text.contains("The Kodex agent has requested the following action:"));
    assert!(!text.contains("Planned action JSON:"));
    assert!(!text.contains("Retry reason:"));
    ...
    settings.bind(|| {
        assert_snapshot!(
            "kodex_core__guardian__tests__network_access_guardian_prompt_layout",
...
#[test_case::test_case(kodex_utils_path_uri::PathConvention::Windows, r"C:\workspace"; "windows")]
#[test_case::test_case(kodex_utils_path_uri::PathConvention::Posix, "/C:/workspace"; "posix")]
fn guardian_exec_command_uses_executor_cwd_convention(
    cwd_convention: kodex_utils_path_uri::PathConvention,
    expected_cwd: &str,
) -> serde_json::Result<()> {
    let cwd = PathUri::parse("file:///C:/workspace").expect("valid executor cwd");
    let action = crate::tools::sandboxing::ApprovalAction::ExecCommand {
        id: "command-1".to_string(),
        environment_id: kodex_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
...
async fn cancelled_guardian_review_emits_terminal_abort_without_warning(
    ...
    match moment {
        ...
        ApprovalCancellation::BeforeCachedResult => {
            ...
            impl kodex_extension_api::ApprovalReviewContributor for CancelWhileApproving {
                fn decide<'a>(
                    &'a self,
                    _input: &'a kodex_extension_api::ApprovalDecisionInput<'_>,
                ) -> kodex_extension_api::ExtensionFuture<
                    'a,
                    Option<kodex_extension_api::ApprovalDecision>,
                > {
                    self.0.cancel();
                    Box::pin(async { Some(kodex_extension_api::ApprovalDecision::Allow) })
                }
            }
            let mut extensions = kodex_extension_api::ExtensionRegistryBuilder::<Config>::new();
            extensions
...
async fn guardian_reuse_respects_effective_policy_and_personality(
    ...
    captured.personality = Some(kodex_protocol::config_types::Personality::Friendly);
    ...
    different_personality.personality = Some(kodex_protocol::config_types::Personality::Pragmatic);
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    kodex_analytics::GuardianReviewAnalyticsResult,
)> {
    let server = start_mock_server().await;
...
-> anyhow::Result<()> {
    ...
    update_turn_settings_for_test(&mut turn, |settings| {
        Arc::make_mut(&mut settings.model_info).auto_review_model_override =
            Some("kodex-auto-review".to_string());
    });
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    seed_guardian_parent_history(&session, &turn).await;
    ...
    assert!(matches!(
        ...
        Some(kodex_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    ...
    assert!(guardian_user_text.contains("The Kodex agent has requested the following action:\n"));
    assert!(guardian_user_text.contains(&format!(
        ">>> TRANSCRIPT END\nReviewed Kodex session id: {}\n",
    ...
    settings.bind(|| {
        assert_snapshot!(
            "kodex_core__guardian__tests__guardian_review_request_layout",
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    let mut reviewer_model = session
        .services
        .models_manager
        .get_model_info("kodex-auto-review", &turn.config.to_models_manager_config())
        .await;
        ...
        .replace_compacted_history(
            vec![
                ResponseItem::Compaction {
                    id: Some(kodex_protocol::ResponseItemId::from_server(
            ...
            .into_iter()
            .map(kodex_history::ResponseItemEnvelope::new)
            .collect(),
            /*reference_context_item*/ None,
    ...
    assert!(matches!(
        ...
        Some(kodex_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "kodex_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_routes_required_actions(
    ...
    impl kodex_extension_api::ApprovalReviewContributor for AutoApprovingReviewContributor {
        fn decide<'a>(
            &'a self,
            _input: &'a kodex_extension_api::ApprovalDecisionInput<'_>,
        ) -> kodex_extension_api::ExtensionFuture<'a, Option<kodex_extension_api::ApprovalDecision>>
        {
            Box::pin(async { Some(kodex_extension_api::ApprovalDecision::Allow) })
        }
    }
...
async fn guardian_ephemeral_retry_preserves_parallel_trunk_and_fork_history() -> anyhow::Result<()>
{
    ...
    let handle = std::thread::Builder::new()
        ...
        let feedback = kodex_feedback::guardian_review_failures(&[session.thread_id()])
            .attachment
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(kodex_protocol::openai_models::ReasoningEffort::Low),
...
async fn guardian_review_session_config_clears_context_overrides_for_distinct_effective_model() {
    ...
    let mut config = (*turn.config).clone();
    config.model = Some("kodex-auto-review".to_string());
