...
//!
//! The history is stored at `~/.kodex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.kodex`.
...
impl HistoryConfig {
    pub fn new(kodex_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            kodex_home: kodex_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.kodex_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.kodex/history.jsonl` and ensure the parent directory exists.
