...
async fn mcp_requests_preserve_trace_context_across_workers_and_continuations() -> anyhow::Result<()>
{
    ...
    for mode in [McpProtocolMode::Legacy, McpProtocolMode::V20260728] {
        ...
        let first_trace = kodex_otel::span_w3c_trace_context(&first).unwrap();
        let second_trace = kodex_otel::span_w3c_trace_context(&second).unwrap();
        ...
        let tool_trace = kodex_otel::span_w3c_trace_context(&tool).unwrap();
        client
            .call_tool(
                "test".to_string(),
                /*arguments*/ None,
        ...
        let custom_trace = kodex_otel::span_w3c_trace_context(&custom).unwrap();
        ...
        for (request, body) in requests {
            ...
            let http = spans
                ...
                .unwrap();
            assert_eq!(http.name, "kodex.exec_server.http_request");
            let bridge = spans
                .iter()
