...
pub async fn cloud_config_bundle_loader_for_storage(
    ...
    let service =
        cloud_config_bundle_service_for_storage(auth_config, enable_kodex_api_key_env).await?;
    let (loader, refresh_task) = cloud_config_bundle_loader_for_service(service);
    replace_refresh_task(refresher_task_slot(), refresh_task);
    Ok(loader)
}
...
pub async fn cloud_config_bundle_loader_for_storage_without_cache(
    ...
    let service = Arc::new(
        cloud_config_bundle_service_for_storage(auth_config, enable_kodex_api_key_env)
            .await?
            .without_cache(),
    );
...
async fn cloud_config_bundle_service_for_storage(
    ...
    let auth_manager =
        AuthManager::shared_from_auth_config(auth_config.clone(), enable_kodex_api_key_env).await?;
