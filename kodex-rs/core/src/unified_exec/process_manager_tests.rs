...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("KODEX_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("KODEX_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            KODEX_VERSION_ENV_VAR.to_string(),
            "client-version".to_string(),
        ),
...
fn exec_env_policy_excludes_non_inheritable_and_runtime_variables() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            ("kodex_version".to_string(), "stale-version".to_string()),
            (
                "kodex_permission_profile".to_string(),
            ...
            (
                "kodex_apply_patch_preserve_line_endings".to_string(),
                "1".to_string(),
            ),
            (
                "kodex_plugin_metrics_output".to_string(),
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: kodex_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = kodex_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        env: HashMap::from([
            ...
            ("KODEX_NETWORK_PROXY_ACTIVE".to_string(), "1".to_string()),
            (
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: kodex_exec_server::ExecEnvPolicy {
                inherit: kodex_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: kodex_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = kodex_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_shell_snapshot = Some(kodex_exec_server::ShellSnapshotRequest {
        scope_id: "attachment-1".to_string(),
        shell: kodex_exec_server::ShellInfo {
            name: "bash".to_string(),
    ...
    request.exec_server_sandbox = Some(
        kodex_exec_server::FileSystemSandboxContext::from_permission_profile(
            permission_profile,
            request.cwd.clone(),
        ),
    );
    ...
    assert_eq!(
        ...
        Some(kodex_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: kodex_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let kodex_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let kodex_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        kodex_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    ...
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            kodex_exec_server::WriteStatus::Accepted,
