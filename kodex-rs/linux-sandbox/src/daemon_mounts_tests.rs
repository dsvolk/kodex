...
#[test_case("/tmp/kodex-daemon-1000", "/alias", false; "directory alias")]
#[test_case("/tmp/kodex-daemon-1000/rpc.sock", "/alias.sock", false; "socket alias")]
...
#[test_case("/other", "/tmp/kodex-daemon-1000/nested", false; "nested mount")]
fn accepts_only_mounts_that_can_keep_the_directory_masked(
    ...
    for mount_id in [Some(visible_mount), None] {
        assert_eq!(
            check_mounts(
                Path::new("/tmp/kodex-daemon-1000"),
                "0:1",
                mount_id,
                mounts.as_bytes()
            )
...
#[test_case("0:2", "mnt:[4026532835]", "/tmp/kodex-daemon-1000/ns", Err(io::ErrorKind::PermissionDenied); "nested namespace mount")]
...
fn unrelated_namespace_mount_does_not_hide_a_socket_alias() {
    let mounts = b"1 0 0:1 / / rw - ext4 disk rw\n\
                   ...
                   3 1 0:1 /tmp/kodex-daemon-1000 /alias rw - ext4 disk rw\n";
    for mount_id in [Some("1"), None] {
        assert_eq!(
            check_mounts(Path::new("/tmp/kodex-daemon-1000"), "0:1", mount_id, mounts)
                .map_err(|error| error.kind()),
...
fn masks_alias_when_tmp_is_itself_a_bind_mount() {
    ...
    let expected = BTreeSet::from([
        PathBuf::from("/tmp/kodex-daemon-1000"),
        PathBuf::from("/backing/tmp/kodex-daemon-1000"),
    ]);
    assert_eq!(
        check_mounts(
            Path::new("/tmp/kodex-daemon-1000"),
    ...
    let hidden = [
        mounts.as_slice(),
        b"3 1 0:1 /tmp/kodex-daemon-1000 /tmp/kodex-daemon-1000 rw - ext4 disk rw\n",
    ]
    .concat();
    assert_eq!(
        check_mounts(
...
fn accepts_private_tmp_filesystem_and_resolves_stacked_mounts() {
    ...
    let directory = Path::new("/tmp/kodex-daemon-1000");
...
fn rejects_open_mount_that_has_been_covered() {
    ...
    let exposed = format!(
        "{mounts}4 1 0:1 /tmp/private-new/tmp/kodex-daemon-1000 /outside rw - ext4 disk rw\n"
    );
...
fn rejects_mount_hidden_by_an_ancestor_overmount() {
    let directory = Path::new("/tmp/private/kodex-daemon-1000");
    let mounts = "1 0 0:1 / / rw - ext4 disk rw\n\
                  2 1 0:2 / /tmp rw - tmpfs tmpfs rw\n\
...
fn accepts_private_tmp_bind_and_maskable_aliases(root: &str, parent: &str) {
    ...
    for (alias_root, destination, can_mask) in [
        ...
        (
            format!("{root}/kodex-daemon-1000/rpc.sock"),
            "/tmp/alias.sock",
            false,
        ),
        ("/".to_owned(), "/host", true),
    ...
    for mount_id in [Some("1"), None] {
        assert_eq!(
            ...
            BTreeSet::from([
                ...
                PathBuf::from("/mnt/wslg/distro/tmp/kodex-daemon-1000"),
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            BTreeSet::from([
                directory.to_path_buf(),
                PathBuf::from("/host-tmp/kodex-daemon-1000"),
            ])
        );
    }
}
