...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run kodex doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.kodex_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.kodex_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: kodex"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
...
Notes
   ...
   ✗ auth         token expired - Run `kodex login`.
─────────────────────────────────────────────────────────────
...
",
    ...
    fn render_human_report_snapshot_covers_environment_rows() {
        ...
        *targets = "exclusion targets: verified Kodex app and required helpers".into();
        ...
        report.checks.extend([
            ...
            .detail("running: true")
            .detail("log directory: $HOME/Library/Logs/com.openai.kodex"),
            DoctorCheck::new(
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `kodex login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project kodex
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/kodex/target/debug/kodex · <redacted>",
            DisplayStatus::Ok,
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/kodex/target/debug/kodex"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, KODEX_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, KODEX_API_KEY"
        );
    }
    ...
    fn copyable_items_use_check_status() {
        let description =
            "/tmp/logs_2.sqlite, ~/goals_1.sqlite integrity check; try --summary or `kodex doctor`";
        let details = r"see ./logs, ../goals. C:\logs, D:/goals. c:\logs, \\server\share\logs, \\?\C:\logs, \\?\UNC\server\share\logs, \\.\pipe\kodex, \??\C:\logs, \DosDevices\C:\logs, \rooted. http://localhost:8080: https://example.com; wss://example.com) and `/tmp/my data/logs_2.sqlite`";
