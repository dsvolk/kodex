...
use kodex_rollout::ThreadItem;
use kodex_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(kodex_home: &Path, path: &Path) -> bool {
    path.starts_with(kodex_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn validated_rollout_file_name(
    ...
    if kodex_rollout::rollout_id_from_path(rollout_path).is_some() {
        Ok(file_name)
    } else {
        Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = kodex_rollout::plain_rollout_path(item.path.as_path());
    ...
    Some(StoredThread {
        ...
        preview,
        name: kodex_state::is_guardian_review_source(&source)
            .then(|| kodex_state::GUARDIAN_THREAD_TITLE.to_string()),
        model_provider: item
            .model_provider
...
pub(super) async fn resolve_thread_names(
    ...
    let legacy_thread_ids = thread_history_modes
        ...
        .collect::<HashSet<_>>();
    let mut names = find_thread_names_by_ids(store.config.kodex_home.as_path(), &legacy_thread_ids)
        .await
        .unwrap_or_default();
...
pub(super) fn has_guardian_default_title(metadata: &ThreadMetadata) -> bool {
    metadata.title.trim() == kodex_state::GUARDIAN_THREAD_TITLE
