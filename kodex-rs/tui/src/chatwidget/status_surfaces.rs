...
impl ChatWidget {
    ...
    /// Clears the terminal title Kodex most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_kodex_folder } => {
                    dot_kodex_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::KodexVersion => Some(KODEX_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("kodex".to_string()),
            ...
            StatusSurfacePreviewItem::KodexVersion => StatusLineItem::KodexVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("kodex".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::KodexVersion => self
                .status_line_value_for_item(StatusLineItem::KodexVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_kodex_window(snapshot, "5h")
...
fn find_primary_kodex_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {
