...
impl RemoteAppServerClient {
    ...
    pub fn kodex_home(&self) -> Option<&str> {
        self.metadata.kodex_home.as_deref()
    }
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            metadata.kodex_home = response
                                .result
                                .get("kodexHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|kodex_home| !kodex_home.is_empty())
                                .map(str::to_string);
