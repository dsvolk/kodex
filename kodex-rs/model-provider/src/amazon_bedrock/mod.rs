...
use kodex_model_provider_info::AMAZON_BEDROCK_GPT_5_6_TERRA_MODEL_ID;
use kodex_model_provider_info::AMAZON_BEDROCK_RUNTIME_GLOBAL_GPT_5_6_LUNA_MODEL_ID;
use kodex_model_provider_info::AMAZON_BEDROCK_RUNTIME_GLOBAL_GPT_5_6_TERRA_MODEL_ID;
...
impl AmazonBedrockModelProvider {
    ...
    fn managed_auth(&self) -> Option<KodexAuth> {
    ...
    fn recover_from_unauthorized(
        ...
        Box::pin(async move {
            ...
            let permit = network_policy
                ...
                .map_err(|error| KodexErr::Fatal(error.to_string()))?;
    ...
    fn account_state(&self) -> ProviderAccountResult {
        Ok(ProviderAccountState {
            account: Some(ProviderAccount::AmazonBedrock {
                uses_kodex_managed_credentials: matches!(
                    self.auth_source(),
                    auth::BedrockAuthSource::ManagedBearerToken
                        | auth::BedrockAuthSource::ManagedAccessKeys
                ),
...
mod tests {
    ...
    async fn configured_profile_takes_precedence_over_managed_auth() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(KodexAuth::BedrockApiKey(managed_auth.clone()));
        ...
        assert_eq!(
            provider.auth().await,
            Some(KodexAuth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
            provider.account_state(),
            Ok(ProviderAccountState {
        ...
        let access_keys_auth_manager = AuthManager::from_auth_for_testing(
            KodexAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
    ...
    async fn runtime_managed_auth_resolves_runtime_endpoint() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(KodexAuth::BedrockApiKey(managed_auth));
        let provider = AmazonBedrockModelProvider::new(
            ModelProviderInfo::create_amazon_bedrock_runtime_provider(/*aws*/ None),
            Some(auth_manager),
        );
