...
use kodex_core::compact::SUMMARY_PREFIX;
...
fn replacement_history_from_rollout(path: &Path) -> Result<Vec<Value>> {
    ...
    {
        let entry = kodex_rollout::parse_rollout_line(line)?;
...
async fn assert_compaction_uses_turn_lifecycle_id(kodex: &std::sync::Arc<kodex_core::KodexThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = kodex.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions(
    ...
    // Build config pointing to the mock server and spawn Kodex.
    ...
    kodex.submit(Op::Compact).await?;
    let warning_event = wait_for_event(&kodex, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Kodex to flush rollout entries before inspecting the file.
    kodex.submit(Op::Shutdown).await?;
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    ...
    for line in text.lines() {
        ...
        let Ok(entry) = kodex_rollout::parse_rollout_line(trimmed) else {
            continue;
        };
        match entry.item {
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    kodex.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&kodex, |ev| match ev {
    ...
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.kodex_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.kodex_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn reasoning_effort_override_remote_v2_compaction_resets_pinned_effort(
    ...
    if fail_compaction {
        wait_for_event(&test.kodex, |event| matches!(event, EventMsg::Error(_))).await;
    }
...
async fn manual_compact_records_durable_and_local_token_usage() {
    ...
    kodex.submit(Op::Compact).await.unwrap();

    // First TokenCount: from the compact API call (usage.total_tokens = 0).
    let first = wait_for_event_match(&kodex, |ev| match ev {
    ...
    let last = wait_for_event_match(&kodex, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    let kodex = builder.build(&server).await.unwrap().kodex;
    ...
    {
        let event = kodex.next_event().await.unwrap();
        match event.msg {
            EventMsg::ItemStarted(ItemStartedEvent {
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    wait_for_event(&test.kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let mut resume_builder = test_kodex().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.kodex, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder.build(&server).await.expect("build test kodex");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.kodex).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using Kodex with a ChatGPT account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    let mut initial_builder = test_kodex()
        .with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test kodex");
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_kodex()
        .with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.kodex).await;
...
async fn auto_compact_persists_rollout_entries() {
    ...
    let expected_settings = kodex.thread_settings_snapshot().await;
    kodex.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&kodex, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&kodex, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-kodex-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-kodex-window-id").as_deref()
    );
    assert_ne!(
...
async fn paginated_compaction_cold_resume_from_bounded_suffix() -> Result<()> {
    ...
    let test = test_kodex()
        .with_model_info_override("gpt-5.4", |model| {
            model.multi_agent_version = Some(kodex_protocol::protocol::MultiAgentVersion::V2);
        })
        .with_history_mode(kodex_protocol::protocol::ThreadHistoryMode::Paginated)
        .with_config(move |config| {
            config
                .features
    ...
    .await?;
    let kodex_core::TurnInputSubmission::Started { turn_id } = test
        .kodex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
    ...
    .await;
    let expected_settings = test.kodex.thread_settings_snapshot().await;
    let path = test.kodex.rollout_path().expect("rollout path");
    ...
    let auth_manager = kodex_core::test_support::auth_manager_from_auth_with_home(
        KodexAuth::from_api_key("test"),
        test.config.kodex_home.to_path_buf(),
    );
    let resumed = test
        .thread_manager
        .resume_thread_with_history(
            test.config.clone(),
            kodex_history::InitialHistory::Resumed(kodex_history::ResumedHistory {
            ...
            auth_manager,
            /*parent_trace*/ None,
            kodex_protocol::mcp::ClientMcpExtensions::default(),
        )
        .await?;
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(kodex_protocol::protocol::MultiAgentVersion::V2)
    );
    let submission = resumed
        .thread
        .continue_turn_if_idle(
            TurnInputRequest::new(kodex_core::TurnInput::ResponseItem(
    ...
    assert!(matches!(
        submission,
        kodex_core::TurnInputSubmission::Started { .. }
    ));
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let kodex = builder.build(&server).await.unwrap();

    kodex.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    kodex.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_pre_turn_compaction_context_window_exceeded() {
    ...
    let error_message = wait_for_event_match(&kodex, |event| match event {
        EventMsg::Error(err) => Some(err.message.clone()),
        _ => None,
    })
    .await;
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    kodex.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
-> Result<()> {
    ...
    test.kodex.flush_rollout().await?;
    ...
    let mut resume_builder = test_kodex()
        .with_home(Arc::clone(&home))
        .with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    assert_eq!(
        resumed.kodex.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
