...
use crate::test_kodex::TestKodex;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = kodex_utils_cargo_bin::cargo_bin("kodex-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `kodex-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_kodex().with_config(move |config| {
