...
async fn ordinary_usage_permission_comes_from_backend_not_display_percent() {
    for (allowed, used_percent) in [(Some(true), 100), (Some(false), 0), (None, 0)] {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let response = test_client(&server.uri(), PathStyle::KodexApi)
...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::KodexApi).rate_limit_status_url(),
        "https://example.test/api/kodex/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::KodexApi).rate_limit_reset_credits_url(),
        "https://example.test/api/kodex/rate-limit-reset-credits"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::KodexApi)
            .consume_rate_limit_reset_credit_url(),
        "https://example.test/api/kodex/rate-limit-reset-credits/consume"
    );
    assert_eq!(
        ...
        RateLimitResetCreditsDetails {
            credits: vec![
                RateLimitResetCreditDetails {
                    ...
                    reset_type: "kodex_rate_limits".to_string(),
...
fn test_client(base_url: &str, path_style: PathStyle) -> Client {
    Client {
        ...
        http: kodex_http_client::RouteAwareClientPool::new(
