use kodex_app_server_protocol::ConfigLayer as ApiConfigLayer;
use kodex_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use kodex_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use kodex_config::ConfigLayer;
use kodex_config::ConfigLayerMetadata;
use kodex_config::ConfigLayerSource;

/// Converts a config-layer source owned by `kodex-config` into the app-server wire type owned by
/// `kodex-app-server-protocol`.
///
...
pub(crate) fn config_layer_source_to_api(source: ConfigLayerSource) -> ApiConfigLayerSource {
    match source {
        ...
        ConfigLayerSource::Project { dot_kodex_folder } => {
            ApiConfigLayerSource::Project { dot_kodex_folder }
        }
...
/// Converts config-layer metadata owned by `kodex-config` into the app-server wire type owned by
/// `kodex-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `kodex-config` into the app-server wire type owned by
/// `kodex-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
