...
async fn guardians_retain_evidence_after_compaction_and_resume(
    ...
    let models = [
        ...
        ("kodex-auto-review", reviewer_hash),
        ("resumed-parent", Some("matching")),
    ]
    .into_iter()
    .map(|(model, hash)| {
        let mut info = kodex_core::test_support::construct_model_info_offline(model, &config);
        info.comp_hash = hash.map(str::to_owned);
        if model == MODEL || model == "resumed-parent" {
            info.auto_review_model_override = Some("kodex-auto-review".to_owned());
        }
        info
    })
    .collect();
    ...
    write_models_cache_with_models(kodex_home.path(), models).await?;
    ...
    .enumerate()
    {
        ...
        if reject_sync_checkpoint && index > 0 {
            ...
            assert_eq!(reviews[0]["model"], "kodex-auto-review");
    ...
    for line in &items {
        if let RolloutItem::Compacted(checkpoint) = &line.item {
            for envelope in checkpoint.replacement_history.iter().flatten() {
                if matches!(
                    envelope.item,
                    kodex_protocol::models::ResponseItem::Compaction { .. }
                ) {
                    assert_eq!(
                        envelope
                            .metadata
