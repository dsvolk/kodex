...
const KEYRING_SERVICE: &str = "kodex";
...
impl SecretsManager {
    pub fn new(kodex_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(kodex_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(kodex_home: &Path, namespace: LocalSecretsNamespace) -> String {
