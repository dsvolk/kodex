...
async fn thread_creator_survives_resume_and_forks_use_current_auth() -> Result<()> {
    ...
    for auth in [
        ...
        let meta = kodex_rollout::read_session_meta_line(&rollout_path)
            .await?
            .meta;
        let stored = test
            .kodex
        ...
        let fork = resumed
            .thread_manager
            .fork_thread(
                kodex_core::ForkSnapshot::Interrupted,
        ...
        let meta = kodex_rollout::read_session_meta_line(&fork_path)
            .await?
            .meta;
        let stored = fork
            .thread
...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db = test.kodex.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn resume_restores_dynamic_tools_from_rollout_with_sqlite_enabled() -> Result<()> {
    ...
    let mut resume_builder = test_kodex().with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_kodex()
        .with_pre_build_hook(move |kodex_home| {
            let rollout_path = kodex_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.sqlite.state_db_path();
    let rollout_path = test.config.kodex_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = kodex_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = kodex_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
