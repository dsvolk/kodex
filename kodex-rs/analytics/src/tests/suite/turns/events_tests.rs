...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(KodexTurnEventRequest {
        event_type: "kodex_turn_event",
        event_params: crate::events::KodexTurnEventParams {
            ...
            kodex_turn_source: Some("composer".to_string()),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "root_turn_id": "turn-2",
                "turn_trigger": "user",
                "kodex_turn_source": "composer",
                "submission_type": null,
                "app_server_client": {
                    "product_client_id": "kodex_cli_rs",
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("kodex_turn_event"));
    ...
    assert_eq!(
        (
            ...
            payload["event_params"].get("kodex_turn_source"),
        ),
        (
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        [
            ("kodex_command_execution_event", None),
            ("kodex_file_change_event", None),
            ("kodex_mcp_tool_call_event", None),
            ("kodex_dynamic_tool_call_event", None),
            ("kodex_web_search_event", None),
            ("kodex_image_generation_event", None),
            ("kodex_collab_agent_tool_call_event", None),
            ("kodex_control_tool_call_event", None),
        ]
...
async fn turn_lifecycle_emits_failed_turn_event() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnKodexError(Box::new(
                TurnKodexErrorFact::from_kodex_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &KodexErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(kodex_app_server_protocol::KodexErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["kodex_error_kind"],
