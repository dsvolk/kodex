...
async fn prepare_startup_tooltip_override_persists_model_availability_nux_count() {
    let kodex_home = tempdir().expect("temp kodex home");
...
async fn accepted_model_migration_persists_target_default_reasoning_effort() -> Result<()> {
    ...
    Kodex now uses GPT-5.6 Terra in place of GPT-5.4. Switch to GPT-5.6 Terra to continue.

    GPT-5.4 Mini is no longer available

    Kodex now uses GPT-5.6 Luna in place of GPT-5.4 Mini. Switch to GPT-5.6 Luna to continue.
    ");
    Ok(())
}
