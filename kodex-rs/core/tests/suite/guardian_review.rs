...
use kodex_core::SleepFuture;
use kodex_core::TimeFuture;
use kodex_core::TimeProvider;
...
use kodex_extension_api::ToolFinishInput;
use kodex_extension_api::ToolLifecycleContributor;
use kodex_extension_api::ToolLifecycleFuture;
...
use kodex_protocol::openai_models::AutoReviewMessages;
use kodex_protocol::openai_models::MODEL_SPECIALTY_CYBER;
use kodex_protocol::openai_models::ModelTokenBudgetConfig;
...
#[test_case(KodexAuth::from_api_key("test-api-key"), "OpenAI", "/v1", true, "/v1/responses", true; "api_key_uses_responses")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/kodex", false, "/backend-api/kodex/responses", true; "chatgpt_marks_guardian_by_default")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/kodex", true, "/backend-api/kodex/responses", true; "legacy_opt_in_still_accepted")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/v1", true, "/v1/responses", true; "custom_openai_url_uses_responses")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), "Custom", "/backend-api/kodex", true, "/backend-api/kodex/responses", true; "custom_provider_uses_responses")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/kodex", true, "/backend-api/kodex/responses", false; "retry_without_response_id_keeps_last_parent")]
async fn guardian_session_inherits_parent_http_fallback(
    ...
    let credits_enabled =
        auth.uses_kodex_backend() && provider_name == "OpenAI" && base_path == "/backend-api/kodex";
    ...
    assert_eq!(
        guardian_request.header("x-kodex-guardian").as_deref(),
    ...
    for request in responses.requests() {
        ...
        if body["client_metadata"]["x-openai-subagent"] != "guardian" {
            assert_eq!(request.header("x-kodex-guardian"), None);
            assert_eq!(
                (
...
"#;
    ...
    let server = start_mock_server().await;
    let base_url = format!("{}/backend-api/kodex", streaming.uri());
    let test = test_kodex()
...
async fn guardian_review_compacts_with_summary_despite_parent_token_budget(
    ...
    let mut builder = test_kodex()
        ...
        .with_history_mode(kodex_protocol::protocol::ThreadHistoryMode::Legacy)
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[test_case(KodexAuth::from_api_key("test-api-key"), true, "gpt-5.6-luna"; "api_key_uses_luna_with_responses_lite")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), true, "kodex-auto-review"; "chatgpt_uses_kodex_auto_review")]
#[test_case(KodexAuth::create_dummy_chatgpt_auth_for_testing(), false, "kodex-auto-review"; "chatgpt_without_free_guardian")]
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let uses_kodex_backend = auth.uses_kodex_backend();
    let credits_enabled = uses_kodex_backend;
    let bundled_models = kodex_models_manager::bundled_models_response()?.models;
    ...
    let mut builder = test_kodex()
        ...
        .with_config(move |config| {
            ...
            let rules_dir = config.kodex_home.join("rules");
    ...
    assert!(
        test.kodex
            ...
            .get::<kodex_guardian_reviewer::ReviewerTasks>()
    ...
    assert!(matches!(
        ...
        Err(kodex_thread_store::ThreadStoreError::ThreadNotFound { .. })
    ));
    ...
    for handshake in server.handshakes() {
        ...
        assert_eq!(
            handshake.uri(),
            if uses_kodex_backend {
                "/backend-api/kodex/responses"
            } else {
                "/v1/responses"
            }
        );
        assert_eq!(
            handshake.header("x-kodex-guardian").as_deref(),
            is_guardian_request.then_some("reviewer")
        );
        if is_guardian_request {
            assert_eq!(handshake.header("x-kodex-routing-hint"), None);
        }
    }
...
async fn guardian_node_repl_policy_follows_production_approval_path(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.kodex, repl_server).await?;
    ...
    let bundled_policy = kodex_prompts::ResolvedModelMessages::bundled()
        .auto_review()
        .node_repl_policy;
...
async fn guardian_reviews_delayed_and_new_actions_after_catalog_refresh() -> Result<()> {
    ...
    preferred.slug = "kodex-auto-review".to_string();
    ...
    let test = test_kodex()
        ...
        .with_config(|config| {
            ...
            config.model_reasoning_effort =
                Some(kodex_protocol::openai_models::ReasoningEffort::High);
            config.model_reasoning_summary =
                Some(kodex_protocol::config_types::ReasoningSummary::Detailed);
        })
        .build_with_auto_env(&server)
        .await?;
    ...
    test.kodex
        ...
        .await?;
    let turn_id = wait_for_event_match(&test.kodex, |event| match event {
    ...
    test.kodex
        .submit(Op::TurnSettings {
            turn_id,
            update: kodex_protocol::protocol::TurnSettingsUpdate {
                model: Some("guardian-parent-b".to_string()),
                effort: Some(Some(kodex_protocol::openai_models::ReasoningEffort::Medium)),
                summary: Some(kodex_protocol::config_types::ReasoningSummary::Concise),
                ..Default::default()
            },
            reply,
        })
    ...
    catalog
        .models
        .retain(|model| model.slug != "kodex-auto-review");
    catalog.models[0]
        .model_messages
        .as_mut()
        .unwrap()
...
async fn guardian_session_is_reused_for_consecutive_tool_reviews_without_prewarm() -> Result<()> {
    ...
    let mut builder = test_kodex()
        ...
        .with_workspace_setup(|cwd, fs| async move {
            fs.write_file(
                &kodex_utils_path_uri::PathUri::from_abs_path(&cwd.join("guardian-secret.txt")),
...
async fn guardian_denial_rejects_tool_call_with_rationale(
    ...
    let feedback = kodex_feedback::guardian_review_failures(&[test.session_configured.thread_id])
        .attachment
...
async fn guardian_timeout_rejects_tool_call_with_acting_model_instructions(
    ...
    impl kodex_extension_api::ApprovalReviewContributor for TimedOutReviewContributor {
        fn decide<'a>(
            &'a self,
            input: &'a kodex_extension_api::ApprovalDecisionInput<'_>,
        ) -> kodex_extension_api::ExtensionFuture<'a, Option<kodex_extension_api::ApprovalDecision>>
        {
            ...
            Box::pin(async {
                Some(kodex_extension_api::ApprovalDecision::Reviewed(
...
impl kodex_extension_api::ApprovalReviewContributor for AttemptCachedApproval {
    fn decide<'a>(
        &'a self,
...
async fn cyber_model_guardian_denial_interrupts_turn_immediately(
    ...
    let warning = wait_for_event(&test.kodex, |event| {
        match event {
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the Kodex agent history whose request action you are assessing."
        ),
...
async fn yielded_code_mode_denials_interrupt_the_servicing_turn() -> Result<()> {
    ...
    let mut builder = test_kodex()
        .with_model_info_override("gpt-5.4", |model| {
            model.tool_mode = Some(kodex_protocol::openai_models::ToolMode::CodeMode);
...
"#
    ...
    loop {
        let event =
            tokio::time::timeout(Duration::from_secs(60), test.kodex.next_event()).await??;
        match event.msg {
