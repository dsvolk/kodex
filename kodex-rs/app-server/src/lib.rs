...
use kodex_code_mode::GrpcCodeModeSessionProvider;
use kodex_config::LoaderOverrides;
use kodex_config::NoopThreadConfigLoader;
use kodex_core::config::Config;
use kodex_core::config::UnsupportedUntrustedApprovalPolicyError;
...
use kodex_app_server_protocol::TextPosition as AppTextPosition;
use kodex_app_server_protocol::TextRange as AppTextRange;
use kodex_app_server_transport::daemon_recovery_file_path;
use kodex_config::ConfigLayerSource;
use kodex_config::ConfigLoadError;
use kodex_config::TextRange as CoreTextRange;
use kodex_core::ExecPolicyError;
...
use kodex_rollout::state_db as rollout_state_db;
use kodex_state::log_db;
use kodex_websocket_auth::WebsocketAuthSettings;
use kodex_websocket_auth::policy_from_settings;
use tokio::sync::mpsc;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Kodex rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "kodex-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "KODEX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.all_layers_low_to_high() {
        let ConfigLayerSource::Project { dot_kodex_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_kodex_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let _registered_core = kodex_windows_sandbox::registered_core_requested();
    ...
    let bootstrap_config = config_manager
        ...
        .await?;
    let bootstrap_auth =
        AuthManager::shared_from_config(&bootstrap_config, /*enable_kodex_api_key_env*/ false)
            .await
    ...
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_kodex_api_key_env*/ false)
            .await
            .map_err(std::io::Error::other)?;
    config_manager.replace_cloud_config_bundle_loader(
        auth_manager.clone(),
    ...
    let otel = kodex_core::otel_init::build_provider(
        &config,
    ...
    kodex_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    kodex_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { .. } => {
            let startup_lock_path = app_server_startup_lock_path(&kodex_home)?;
    ...
    match &transport {
        ...
        AppServerTransport::UnixSocket { socket_path } => {
            let accept_handle = start_control_socket_acceptor(
                ...
                if cfg!(windows)
                    && std::env::var_os(kodex_app_server_transport::DAEMON_SHUTDOWN_SOCKET_ENV)
                        .is_some()
                {
    ...
    if let Some(metrics) = otel.as_ref().and_then(kodex_otel::OtelProvider::metrics) {
        kodex_home_metrics::spawn(&config, metrics.clone(), transport_shutdown_token.clone());
    }
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !kodex_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Kodex local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = kodex_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Kodex local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("kodex-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
