//! Implements the `kodex doctor` diagnostic report.
//!
...
use kodex_api::ResponsesWebsocketClient;
use kodex_api::is_azure_responses_provider;
...
use kodex_install_context::StandalonePlatform;
...
use kodex_login::load_auth_dot_json;
...
/// Options for building a local Kodex diagnostic report.
///
...
///
/// This is the CLI entry point for kodex doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("auth", progress.clone(), || match &auth_manager_result {
                        ...
                        .remediation(
                            "Fix the reported authentication error, then rerun kodex doctor.",
                        ),
                    })
                },
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun kodex doctor.");
    ...
    DoctorReport {
        ...
        overall_status,
        kodex_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    .build()
    .await
    .context("failed to load Kodex config")
}
...
fn config_overrides_from_interactive(
    ...
    ConfigOverrides {
        ...
        model_provider: interactive
            ...
            .flatten(),
        kodex_self_exe: arg0_paths.kodex_self_exe.clone(),
        kodex_linux_sandbox_exe: arg0_paths.kodex_linux_sandbox_exe.clone(),
...
/// JSON support report emitted by `kodex doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        kodex_version: report.kodex_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("KODEX_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by Vite+: {}",
        env::var_os("KODEX_MANAGED_BY_VITE_PLUS").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("KODEX_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "KODEX_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = kodex_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH kodex entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH kodex #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        ...
        if env::var_os("KODEX_MANAGED_PACKAGE_ROOT").is_none() {
            status = status.max(CheckStatus::Warning);
            summary = "npm-managed launch is missing package-root provenance".to_string();
            remediation = Some(
                "Reinstall or update Kodex so the JS shim provides KODEX_MANAGED_PACKAGE_ROOT."
                    .to_string(),
            );
        }
    }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("KODEX_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("KODEX_MANAGED_BY_NPM").is_none()
        && env::var_os("KODEX_MANAGED_BY_BUN").is_none()
        && env::var_os("KODEX_MANAGED_BY_VITE_PLUS").is_none()
        && env::var_os("KODEX_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&KodexPackageLayout>,
) -> String {
...
fn kodex_path_entries() -> Vec<String> {
    let Ok(candidates) = which::which_all("kodex") else {
        return Vec::new();
    };
...
fn config_check(config: &Config) -> DoctorCheck {
    ...
    details.push(format!("KODEX_HOME: {}", config.kodex_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.kodex_home.join(kodex_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.kodex_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        KODEX_API_KEY_ENV_VAR,
        KODEX_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run kodex login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Kodex credentials were found",
        )
        .details(details)
        .remediation("Run kodex login or provide an API key through a supported auth env var."),
        ...
        .remediation("Fix auth storage access or run kodex login again."),
    }
}
...
fn stored_auth_mode(auth: &kodex_login::AuthDotJson) -> &'static str {
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(KODEX_API_KEY_ENV_VAR);
...
async fn state_check(config: &Config, command: &DoctorCommand) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "KODEX_HOME", &config.kodex_home);
    ...
    rollout_stats_details(&mut details, &config.kodex_home);
...
async fn sqlite_integrity_detail(
    sqlite: &kodex_state::SqliteConfig,
    details: &mut Vec<String>,
    label: &str,
    path: &Path,
    ...
    let (rows, timed_out) = match kodex_state::sqlite_integrity_check(sqlite, path, deadline).await
    {
        Ok(kodex_state::SqliteIntegrityCheck::Complete(rows)) => (rows, false),
        Ok(kodex_state::SqliteIntegrityCheck::TimedOut(rows)) => (rows, true),
        Err(err) => {
...
fn rollout_stats_details(details: &mut Vec<String>, kodex_home: &Path) {
    let active = collect_rollout_stats(&kodex_home.join("sessions"));
    let archived = collect_rollout_stats(&kodex_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &KodexAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match kodex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "KODEX_HOME was resolved without config",
        )
        .detail(format!("KODEX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "KODEX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if provider_base_url.is_some_and(|url| !url.trim().is_empty())
        ...
        || env_var_present(KODEX_API_KEY_ENV_VAR)
    {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(KODEX_ACCESS_TOKEN_ENV_VAR) {
...
fn provider_reachability_plan_from_parts(
    ...
    let endpoints = match (mode, provider_base_url) {
        ...
        (ProviderAuthReachabilityMode::Chatgpt, None) => vec![ReachabilityEndpoint {
            ...
            url: provider_url_for_path(chatgpt_base_url, "kodex/responses", provider_query_params),
            required: true,
...
mod tests {
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].kodex_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            kodex_self_exe: Some(PathBuf::from("/bin/kodex")),
            kodex_linux_sandbox_exe: Some(PathBuf::from("/bin/kodex-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/kodex-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.kodex_self_exe, arg0_paths.kodex_self_exe);
        assert_eq!(
            overrides.kodex_linux_sandbox_exe,
            arg0_paths.kodex_linux_sandbox_exe
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "KODEX_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("KODEX_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Kodex."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Kodex.".to_string())
        );
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                ...
                /*provider_env_key*/ None,
                /*provider_base_url*/ None,
                |name| name == KODEX_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
    ...
    fn provider_reachability_chatgpt_uses_inference_endpoint() {
        assert_eq!(
            ...
            vec![ReachabilityEndpoint {
                label: "ChatGPT".to_string(),
                url: "https://chatgpt.com/backend-api/kodex/responses".to_string(),
                required: true,
                route_probe_url: None,
            }]
        );
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-kodex-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-kodex-doctor-mcp\" is not resolvable",
            )
        }));
    }
