...
use kodex_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use kodex_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use kodex_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use kodex_arg0::Arg0DispatchPaths;
use kodex_arg0::arg0_dispatch_or_else;
use kodex_chatgpt::apply_command::ApplyCommand;
use kodex_chatgpt::apply_command::run_apply_command;
use kodex_cli::read_access_token_from_stdin;
use kodex_cli::read_api_key_from_stdin;
use kodex_cli::run_login_status;
use kodex_cli::run_login_with_access_token;
use kodex_cli::run_login_with_api_key;
use kodex_cli::run_login_with_chatgpt;
use kodex_cli::run_login_with_device_code;
use kodex_cli::run_logout;
use kodex_cloud_config::cloud_config_bundle_loader_for_storage;
use kodex_cloud_tasks::Cli as CloudTasksCli;
use kodex_exec::Cli as ExecCli;
use kodex_exec::Command as ExecCommand;
use kodex_exec::ReviewArgs;
use kodex_exec_server::ExecServerRuntimePaths;
use kodex_execpolicy::ExecPolicyCheckCommand;
...
use kodex_rollout_trace::REDUCED_STATE_FILE_NAME;
use kodex_rollout_trace::replay_bundle;
use kodex_state::StateRuntime;
use kodex_tui::AppExitInfo;
use kodex_tui::Cli as TuiCli;
use kodex_tui::ExitReason;
use kodex_tui::UpdateAction;
use kodex_utils_absolute_path::AbsolutePathBuf;
use kodex_utils_cli::CliConfigOverrides;
use kodex_utils_cli::ProfileV2Name;
use kodex_utils_cli::SharedCliOptions;
...
use kodex_core::build_models_manager;
use kodex_core::config::Config;
use kodex_core::config::ConfigBuilder;
use kodex_core::config::ConfigLoadOptions;
...
use kodex_models_manager::manager::RefreshStrategy;
...
enum Subcommand {
    ...
    #[clap(hide = true)]
    TcpTunnel(kodex_tcp_tunnel::Args),
    /// Run Kodex non-interactively.
    ...
    /// Manage external MCP servers for Kodex.
    Mcp(McpCli),

    /// Manage Kodex plugins.
    Plugin(PluginCli),
    ...
    /// Update Kodex to the latest version.
    Update,

    /// Diagnose local Kodex installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Kodex-provided sandbox.
    ...
    /// [EXPERIMENTAL] Browse tasks from Kodex Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Kodex.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = kodex_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = kodex_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = kodex_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $KODEX_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        ...
        help = "Read the access token from stdin (e.g. `printenv KODEX_ACCESS_TOKEN | kodex login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: kodex_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = kodex_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: kodex_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openai.com/kodex/config-advanced/#metrics for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: kodex_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: kodex_exec_server::RequestDispatchMode,
    ...
    /// Use Agent Identity auth from KODEX_ACCESS_TOKEN for remote registration.
    #[arg(
    ...
    #[arg(
        ...
        env = kodex_exec_server::KODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Kodex tooling.
    #[clap(hide = true)]
...
fn run_update_action(
    ...
    if let UpdateAction::Daemon(source) = action {
        let executable = cli_executable
            .ok_or_else(|| anyhow::anyhow!("Cannot locate the launching Kodex CLI"))?;
        println!("Updating the local background server...");
        let status = std::process::Command::new(executable)
            .args(source.command_args())
            .env(kodex_app_server_daemon::telemetry::HANDOFF_ENV, "1")
            .status()?;
        anyhow::ensure!(
            status.success(),
            "Daemon update failed with status {status}"
        );
        println!("Relaunch Kodex to reconnect.");
        return Ok(());
    }
    println!();
    let cmd_str = action.command_str();
    println!("Updating Kodex via `{cmd_str}`...");
    let status = {
        #[cfg(windows)]
        {
    ...
    println!("\n🎉 Update ran successfully! Please restart Kodex.");
    Ok(())
}
...
fn resolve_windows_update_command_from_path(
    ...
    if path_env.is_empty() {
        anyhow::bail!(
            "Could not find an absolute update command `{command}` on PATH. Please update manually: https://developers.openai.com/kodex/cli/"
        );
    }
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`kodex update` is not available in debug builds. Install a release build of Kodex to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = kodex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Kodex installation method. Please update manually: https://developers.openai.com/kodex/cli/"
            );
        };
...
async fn run_session_archive_cli_command(
    action: kodex_tui::SessionArchiveAction,
    ...
    kodex_tui::run_session_archive_command(
        action,
        target,
        kodex_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<kodex_tui::SessionArchiveAction> {
    if force && kodex_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => kodex_tui::DeleteConfirmation::Skip,
        false => kodex_tui::DeleteConfirmation::Prompt,
    };
    Ok(kodex_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let kodex_bin = std::env::current_exe()?;
            kodex_app_server_test_client::send_message_v2(&kodex_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    kodex_build_info::initialize!();
    let remote_control_disabled = kodex_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    {
        anyhow::bail!("`kodex agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!("`kodex agents` does not accept an initial prompt or images");
                }
                ...
                {
                    anyhow::bail!(
                        "`kodex agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
                if is_workload_identity_selected() {
                    anyhow::bail!(
                        "`kodex agents` is unavailable while workload identity is active"
                    );
                }
                if root_remote.is_none() {
                    ...
                    anyhow::bail!("`kodex agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::TcpTunnel(args)) => {
            return kodex_tcp_tunnel::run(args).await;
        }
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            kodex_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        kodex_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = kodex_app_server::AppServerRuntimeOptions {
                        code_mode_host_transport: code_mode_host.into(),
                        managed_daemon,
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                kodex_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                kodex_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                kodex_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    let exit = kodex_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        ...
                        analytics_default_enabled,
                        transport,
                        kodex_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                    if exit == kodex_app_server::AppServerExit::Forced {
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            kodex_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::Update {
                        ...
                        let result = kodex_app_server_daemon::update_from_cli(|request| {
                    ...
                    | AppServerDaemonSubcommand::PidUpdateLoop {
                        ...
                        if matches!(
                            ...
                            let result = kodex_app_server_daemon::update(http_client_factory)
                                .await
                                .map(Some);
                            daemon_telemetry::record_command(
                                &root_config_overrides,
                            ...
                            kodex_app_server_daemon::run_pid_update_loop(
                                http_client_factory,
                                restore_release,
                            )
                            .await?;
                ...
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            ...
                            kodex_app_server::app_server_control_socket_path(&kodex_home)?
                        }
                    };
                    kodex_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = kodex_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    kodex_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    kodex_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    kodex_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                kodex_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                kodex_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            kodex_cloud_tasks::run_main(cloud_cli, arg0_paths.kodex_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            kodex_cli::run_command_under_seatbelt(
                sandbox_cli,
            ...
            .await?;
            #[cfg(target_os = "linux")]
            kodex_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.kodex_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            kodex_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.kodex_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`kodex sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || kodex_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `kodex mcp`: `kodex`, `kodex exec`, `kodex review`, `kodex resume`, `kodex queue`, `kodex archive`, `kodex delete`, `kodex unarchive`, `kodex fork`, `kodex mcp`, `kodex sandbox`, and `kodex debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let runtime_paths =
        ExecServerRuntimePaths::new(kodex_self_exe, arg0_paths.kodex_linux_sandbox_exe.clone())?;
    if let Some(base_url) = cmd.remote.take() {
        ...
        let auth_provider = if cmd.aws_sigv4 {
            exec_server_auth::aws_sigv4_auth_provider(kodex_aws_auth::AwsAuthConfig {
        ...
        let mut remote_config = kodex_exec_server::RemoteEnvironmentConfig::new_with_transport(
            base_url,
            environment_id,
            if direct_transport {
                kodex_exec_server::RemoteEnvironmentTransport::Direct
            } else {
                kodex_exec_server::RemoteEnvironmentTransport::Noise
            },
            auth_provider,
        ...
        #[cfg(target_os = "macos")]
        let runtime_paths = runtime_paths.with_allowed_symlinked_kodex_home(
            kodex_config::allowed_symlinked_kodex_home(
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        kodex_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
        ...
        #[cfg(target_os = "macos")]
        let runtime_paths =
            runtime_paths.with_allowed_symlinked_kodex_home(config.as_ref().and_then(|config| {
                kodex_config::allowed_symlinked_kodex_home(
                    &config.config_layer_stack,
                    &config.kodex_home,
                )
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| kodex_exec_server::DEFAULT_LISTEN_URL.to_string());
        let run = exec_server_telemetry::run_until_shutdown(
            kodex_exec_server::run_main_with_telemetry(
                &listen_url,
                runtime_paths,
                telemetry,
                http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    config: &kodex_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<kodex_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        read_kodex_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("KODEX_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth = AuthManager::shared_from_config(config, /*enable_kodex_api_key_env*/ false)
            .await?
            .auth()
            .await
            .ok_or_else(|| anyhow::anyhow!("Agent Identity authentication is unavailable"))?;
        if !matches!(auth, KodexAuth::AgentIdentity(_)) {
            anyhow::bail!(
                "KODEX_ACCESS_TOKEN did not provide permitted Agent Identity authentication"
            );
        }
        return Ok(kodex_model_provider::auth_provider_from_auth(&auth));
    }

    let (auth_manager, auth) = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `kodex login` or set KODEX_API_KEY",
    )
    .await?;
    ...
    if auth_manager.is_workload_identity_selected() {
        Ok(kodex_model_provider::auth_provider_from_auth_manager(
            auth_manager,
            &auth,
        ))
    } else {
        Ok(kodex_model_provider::auth_provider_from_auth(&auth))
    }
}

fn is_supported_exec_server_remote_auth(auth: &KodexAuth) -> bool {
...
) -> anyhow::Result<kodex_core::config::Config> {
...
) -> anyhow::Result<(Arc<AuthManager>, kodex_login::KodexAuth)> {
    let auth_manager =
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let kodex_home = find_kodex_home()?;
    ConfigEditsBuilder::new(&kodex_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&kodex_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let kodex_home = find_kodex_home()?;
            Ok(loader_overrides_for_profile_at_kodex_home(
                Some(profile_v2),
                &kodex_home,
            ))
        }
...
fn loader_overrides_for_profile_at_kodex_home(
    profile_v2: Option<&ProfileV2Name>,
    kodex_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(kodex_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(kodex_home: &std::path::Path, feature: &str) {
    ...
    let config_path = kodex_home.join(kodex_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(kodex_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        kodex_self_exe: arg0_paths.kodex_self_exe,
        kodex_linux_sandbox_exe: arg0_paths.kodex_linux_sandbox_exe,
    ...
    let user_instructions_provider = Arc::new(KodexHomeUserInstructionsProvider::new(
        config.kodex_home.clone(),
    ));
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_kodex_api_key_env*/ false).await?;
    let mut extensions = kodex_extension_api::ExtensionRegistryBuilder::new();
    kodex_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.chatgpt_base_url.clone(),
        config.http_client_factory(),
    );
    kodex_skills_extension::install(&mut extensions, |config: &Config| {
        kodex_skills_extension::SkillsExtensionConfig {
            ...
            shadow_selection_enabled: config
                .features
                .enabled(kodex_features::Feature::SkillSearch),
        }
    });
    let prompt_input = kodex_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.kodex_home).await?;
...
fn reject_unsupported_worktree_for_subcommand(
    ...
    match subcommand {
        ...
        Some(Subcommand::Fork(_)) => {
            anyhow::bail!("`kodex fork --worktree` requires an explicit session ID")
        }
        Some(Subcommand::Exec(command)) => match &command.command {
            None | Some(ExecCommand::Fork(_)) => Ok(()),
            Some(ExecCommand::Resume(_)) => anyhow::bail!(
                "`--worktree` cannot resume an existing session; use `kodex exec fork --worktree`"
            ),
        ...
        _ => {
            anyhow::bail!(
                "`--worktree` supports new interactive sessions, `kodex fork`, `kodex exec`, and `kodex exec fork`"
            )
        }
    }
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = kodex_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<kodex_core::config::Config>,
) -> kodex_http_client::HttpClientFactory {
    match config {
        Ok(config) => config.http_client_factory(),
        Err(error) => {
            eprintln!("warning: failed to load updater network configuration: {error}");
            kodex_http_client::HttpClientFactory::new(
...
async fn print_app_server_remote_control_output(
    ...
    let output = kodex_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    if interactive.no_daemon {
        if interactive.agents_overview {
            return Ok(AppExitInfo::fatal(
                "--no-daemon cannot be used with kodex agents. The agents overview requires a shared server. Use kodex --no-daemon to work without it.",
            ));
        }
        if remote.is_some() {
            return Ok(AppExitInfo::fatal(
    ...
    let terminal_info = kodex_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Kodex's interactive TUI may not work in this terminal."
        );
    ...
    if interactive.agents_overview && remote.is_none() {
        ...
        kodex_app_server_daemon::run(AppServerLifecycleCommand::Start)
            .await
            .map_err(|err| std::io::Error::other(format!(
                "{err:#}\nThe agents overview requires a shared server. Use kodex --no-daemon to work without it."
            )))?;
    }
    ...
    let start_tui = || {
        kodex_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            kodex_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Kodex local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<kodex_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(kodex_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !kodex_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let kodex_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let kodex_home = tempfile::tempdir().expect("temporary Kodex home");
        let config = ConfigBuilder::default()
        ...
        assert_eq!(
            ...
            kodex_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = KodexAuth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_kodex_home() -> anyhow::Result<()> {
        let kodex_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_kodex_home(Some(&profile), kodex_home.path());

        assert_eq!(
            overrides.user_config_path,
            Some(resolve_profile_v2_config_path(kodex_home.path(), &profile))
        );
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["kodex", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["kodex", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["kodex", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["kodex", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["kodex", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["kodex", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["kodex", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn worktree_flag_supports_interactive_exec_and_explicit_fork_positions() {
        let arguments = [
            vec!["kodex", "--worktree"],
            vec!["kodex", "--worktree", "hello"],
            vec!["kodex", "--worktree", "exec", "hello"],
            vec!["kodex", "exec", "--worktree", "hello"],
            vec![
                "kodex",
                "fork",
                "--worktree",
    ...
    fn worktree_flag_rejects_unsupported_session_and_management_commands() {
        let arguments = [
            vec!["kodex", "--worktree", "login"],
            vec!["kodex", "fork", "--worktree"],
            vec!["kodex", "fork", "--worktree", "--last"],
            vec!["kodex", "exec", "resume", "--worktree", "session"],
            vec!["kodex", "exec", "review", "--worktree"],
            vec!["kodex", "resume", "--worktree", "session"],
            vec!["kodex", "archive", "session", "--worktree"],
            vec![
                "kodex",
                "queue",
                "--thread",
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(kodex_exec::Command::Resume(args)) = exec.command else {
    ...
    fn approve_for_me_configures_interactive_mode() {
        for flag in ["--approve-for-me", "--not-so-yolo"] {
            let mut cli = MultitoolCli::try_parse_from(["kodex", flag]).expect("parse flag");
    ...
    fn not_so_yolo_alias_is_hidden_from_help() {
        for args in [&["kodex", "--help"][..], &["kodex", "exec", "--help"][..]] {
    ...
    fn approve_for_me_defaults_propagate_from_root_to_exec() {
        let exec = finalize_exec_from_args(&["kodex", "--approve-for-me", "exec", "summarize"]);
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(kodex_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(kodex_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn approve_for_me_conflicts_with_explicit_interactive_permissions() {
        for conflicting_args in [
            ...
            let mut args = vec!["kodex", "--approve-for-me"];
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let kodex_home = find_kodex_home().expect("kodex home");
        kodex_app_server::app_server_control_socket_path(&kodex_home)
    ...
    fn tcp_tunnel_is_hidden_and_accepts_its_control_input_contract() {
        let root_help = help_from_args(&["kodex", "--help"]);
        assert!(!root_help.contains("tcp-tunnel"), "{root_help}");
        let help = help_from_args(&["kodex", "tcp-tunnel", "--help"]);
        for option in [
            "--target",
            "--auth-token-stdin",
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        for (subcommand, usage) in [
            ...
            ("list", "Usage: kodex plugin marketplace list"),
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["kodex", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["kodex", "sandbox", "--help"]);
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["kodex", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["kodex", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["kodex", "sandbox", "-C", "/tmp"])
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: thread_id.map(|thread_id| kodex_tui::ResumableThread {
                thread_id,
    ...
    async fn format_exit_messages_preserves_auth_env_through_tui_runner() {
        let exit_info = run_tui_with_recovery(
            || async {
                ...
                exit_info.disconnect_info = Some(kodex_tui::DisconnectInfo {
                    command: vec![
                        "kodex".to_string(),
                        "--remote".to_string(),
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  kodex resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  kodex resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36mkodex resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          kodex resume 123e4567-e89b-12d3-a456-426614174000
        Or run kodex resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36mkodex resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36mkodex resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
    ...
    fn resume_and_fork_preserve_no_daemon() {
        for (command, finalize) in [
            ...
            for args in [
                ["kodex", "--no-daemon", command, "--last"],
                ["kodex", command, "--last", "--no-daemon"],
            ] {
    ...
    fn resume_and_fork_preserve_no_alt_screen() {
        for (command, finalize) in [
            ...
            assert!(finalize(&["kodex", command, "--no-alt-screen"]).no_alt_screen);
            assert!(finalize(&["kodex", "--no-alt-screen", command]).no_alt_screen);
            assert!(!finalize(&["kodex", command]).no_alt_screen);
        }
    }
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["kodex", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["kodex", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["kodex", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(kodex_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(kodex_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["kodex", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["kodex", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["kodex", "fork", "1234", "continue here"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["kodex", "app-server", "--remote-control"].as_ref());
    ...
    fn strict_config_parses_for_supported_commands() {
        ...
        let cli = MultitoolCli::try_parse_from(["kodex", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn exec_server_forward_requires_registration_and_destination() {
        for args in [
            ...
            assert!(
                MultitoolCli::try_parse_from(["kodex", "exec-server"].into_iter().chain(args))
                    .is_err()
            );
        }
    }
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["kodex", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn remote_control_pair_parses() {
        let cli = MultitoolCli::try_parse_from(["kodex", "remote-control", "pair"]).expect("parse");
        let Some(Subcommand::RemoteControl(remote_control)) = &cli.subcommand else {
            panic!("expected remote-control subcommand");
        };
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["kodex", "--remote", "unix://kodex.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://kodex.sock"));
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["kodex", "resume", "--remote", "unix://kodex.sock"])
                .expect("parse");
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://kodex.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("KODEX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("KODEX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_rejects_invalid_code_mode_host_urls() {
        for endpoint in [
            ...
            let error =
                MultitoolCli::try_parse_from(["kodex", "app-server", "--code-mode-host", endpoint])
    ...
    fn app_server_listen_websocket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            kodex_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["kodex", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            kodex_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["kodex", "app-server", "--listen", "unix:///tmp/kodex.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            kodex_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/kodex.sock")
    ...
    fn app_server_listen_off_parses() {
        ...
        assert_eq!(app_server.listen, kodex_app_server::AppServerTransport::Off);
    }
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["kodex", "app-server", "proxy", "--sock", "kodex.sock"].as_ref());
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("kodex.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(kodex_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/kodex-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(kodex_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/kodex-secret"))
        );
