...
fn test_config(kodex_home: &Path) -> RolloutConfig {
    RolloutConfig {
        kodex_home: kodex_home.to_path_buf(),
        sqlite: kodex_state::SqliteConfig::new_for_testing(kodex_home.abs()),
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        kodex_state::BackfillStatus::Complete
    );
...
-> std::io::Result<()> {
    ...
    let valid_created_at = chrono::Utc
        ...
        .single()
        .expect("valid datetime");
    let mut valid_builder = kodex_state::ThreadMetadataBuilder::new(
        valid_thread_id,
        valid_path.clone(),
        valid_created_at,
        SessionSource::Cli,
...
fn fill_missing_thread_item_metadata_preserves_identity_and_prefers_state_git_fields() {
    ...
    let state_item = ThreadItem {
        ...
        section: Some(kodex_state::ThreadSection {
