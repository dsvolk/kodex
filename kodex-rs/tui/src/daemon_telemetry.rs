...
pub(super) async fn record_start(
    config: &Config,
    result: &anyhow::Result<kodex_app_server_daemon::LifecycleOutput>,
) {
    use kodex_app_server_daemon::LifecycleStatus;
    ...
    if let Some(metrics) = otel.metrics() {
        ...
        let _ = metrics.counter("kodex.daemon.start", /*inc*/ 1, &tags);
    }
    let _ = otel
