...
use kodex_protocol::models::ConfigurationReasoning;
...
fn conversation_history_snapshot_binds_review_mode_and_hash_to_the_latest_item(
    ...
    unknown.metadata = latest_hash.map(|hash| KodexHarnessMetadata {
...
fn drop_last_n_user_turns_preserves_prefix() {
    ...
    history.record_retained_context(&kodex_history::RetainedContextEvent::VerifiedAnswer {
...
fn passthrough_metadata_does_not_change_context_estimates() {
    for output in [
        ...
        for turn_metadata in [
            ...
            let mut call = kodex_protocol::models::ExecutedToolCall::new(
                "test_tool".to_string(),
                serde_json::json!({"input": "x".repeat(8 * 1024)}),
            );
            call.set_tool_result_metadata(kodex_protocol::models::ToolResultMetadata::new(
