...
async fn rejected_start(
    ...
    let spawned = kodex_utils_pty::spawn_pty_process(
        &program.to_string_lossy(),
        args,
        cwd,
        env,
        /*arg0*/ &None,
        kodex_utils_pty::TerminalSize {
            rows: 40,
            cols: 120,
        },
        &[],
...
async fn worktree_start_and_fork(backend: &str) -> anyhow::Result<()> {
    ...
    let root = kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(root.path())?
        .canonicalize()?
        .into_path_buf();
    ...
    fs::create_dir(launcher.join(".kodex"))?;
    fs::write(
        launcher.join(".kodex/config.toml"),
    ...
    fs::create_dir(source.join(".kodex"))?;
    fs::write(
...
"#,
    ...
    let program = kodex_utils_cargo_bin::cargo_bin("kodex")?;
    let mut env: HashMap<String, String> = std::env::vars().collect();
    env.insert("KODEX_HOME".into(), home.display().to_string());
    env.insert("KODEX_SQLITE_HOME".into(), home.display().to_string());
    ...
    let _daemon = if backend == "daemon" {
        ...
        let managed = bin.join(if cfg!(windows) { "kodex.exe" } else { "kodex" });
    ...
    for (fork, explicit_cd, analytics, auth_failure) in [
        ...
        if auth_failure {
            ...
            fs::write(source.join(".kodex/config.toml"), "")?;
        }
        ...
        if explicit_cd {
            args.extend(["--cd".into(), source.join(".kodex").display().to_string()]);
        }
        if analytics {
        ...
        let renamed = if !fork && let Ok(Ok((_, _, metadata))) = &observed {
            tokio::time::timeout(Duration::from_secs(/*secs*/ 10), async {
                session
                    ...
                    .await?;
                let thread_id = kodex_protocol::ThreadId::from_string(
                    metadata["ownerThreadId"].as_str().context("owner id")?,
                )?;
                loop {
                    if kodex_rollout::find_thread_name_by_id(&home, &thread_id)
                        .await?
                        .as_deref()
                        == Some("managed-source")
                    {
        ...
        if analytics {
            ...
            if backend == "embedded" {
                let update = exported
                    .iter()
                    .find(|metric| metric["name"] == "kodex.daemon.update")
            ...
            let point = exported
                .iter()
                .filter(|metric| metric["name"] == "kodex.tui.start")
                .flat_map(|metric| metric["sum"]["dataPoints"].as_array().into_iter().flatten())
                .next()
                .context("kodex.tui.start data point")?;
            let tags = point["attributes"]
                .as_array()
                .context("kodex.tui.start attributes")?
                .iter()
                .map(|attribute| {
                    Ok((
        ...
        let context = body["input"]
            ...
            .join("\n");
        if explicit_cd {
            let expected = format!("{checkout}/.kodex").replace('\\', "/");
        ...
        if previous.is_empty() {
            ...
            let sqlite = kodex_state::SqliteConfig::from_sqlite_home(
                kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&home)?,
            );
            ...
            Mock::given(wiremock::matchers::path("/source/backend-api/wham/config/bundle"))
                .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "config_toml": {"enterprise_managed": [{"id":"distrust", "name":"distrust",
                        "contents": format!("[projects.{}]\ntrust_level = \"untrusted\"\n", serde_json::to_string(&kodex_config::loader::project_trust_key(Path::new(&checkout)))?)}]}
            ...
            Mock::given(wiremock::matchers::path("/source/backend-api/wham/config/bundle"))
                .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "config_toml": {"enterprise_managed": [{"id":"distrust-source", "name":"distrust-source",
                        "contents": format!("[projects.{}]\ntrust_level = \"untrusted\"\n", serde_json::to_string(&kodex_config::loader::project_trust_key(&source.join(".kodex")))?)}]}
                }))).mount(&server).await;
            let output = rejected_start(
                ...
                &[
                    ...
                    "--no-alt-screen".into(),
                    "--cd".into(),
                    source.join(".kodex").display().to_string(),
                ],
                &launcher,
                &env,
                "cannot create a checkout from an explicitly untrusted source",
    ...
    fs::write(source.join(".kodex/config.toml"), "not = [valid toml")?;
    git(
        &source,
        &[
            "commit",
