...
use kodex_extension_api::ApprovalReviewContributor;
...
use kodex_extension_api::TokenUsageContributor;
...
use kodex_extension_api::TurnInputContributor;
use kodex_extension_api::TurnItemContributor;
use kodex_extension_api::TurnLifecycleContributor;
use kodex_protocol::items::HookPromptItem;
...
impl ApprovalReviewContributor for AllContributors {
    ...
    ) -> ExtensionFuture<'a, Option<kodex_extension_api::ApprovalDecision>> {
        Box::pin(async { Some(kodex_extension_api::ApprovalDecision::AskUser) })
    }
}

impl kodex_extension_api::SynchronousApprovalReviewer for AllContributors {
    fn review(
        &self,
        _reason: kodex_protocol::approvals::GuardianReviewReason,
    ) -> ExtensionFuture<'_, Option<kodex_protocol::protocol::ReviewDecision>> {
        Box::pin(std::future::ready(Some(
            kodex_protocol::protocol::ReviewDecision::Approved,
        )))
    }
}
...
async fn build_round_trips_every_contributor_category() {
    ...
    let input = kodex_extension_api::ApprovalDecisionInput {
        ...
        category: kodex_protocol::openai_models::GuardianScope::Shell,
        approval_policy: kodex_protocol::protocol::AskForApproval::OnRequest,
        approvals_reviewer: kodex_protocol::config_types::ApprovalsReviewer::AutoReview,
    ...
    assert_eq!(
        ...
        Some(kodex_extension_api::ApprovalDecision::AskUser)
    );
}
        ...
        fragments.extend(
            contributor
                .contribute_turn_context(TurnContextContributionInput {
                    thread_id: kodex_protocol::ThreadId::default(),
