//! Registry of model providers supported by Kodex.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Kodex works out-of-the-box.
//!   2. User-defined entries inside `~/.kodex/config.toml` under the `model_providers`
...
use kodex_api::RetryConfig as ApiRetryConfig;
...
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-kodex-residency";
...
pub const CHATGPT_KODEX_BASE_URL: &str = "https://chatgpt.com/backend-api/kodex";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "kodex";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/openai/kodex/discussions/7782";
...
pub struct ModelProviderInfo {
    ...
    /// Optional full URL for a Kodex-native model catalog. When unset, OpenAI discovery
    /// uses the Kodex backend unless `base_url` overrides the inference endpoint.
...
other non-default provider fields are not supported"
    ...
    fn build_header_map(&self) -> KodexResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> KodexResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> KodexResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
    ...
    pub fn supports_kodex_backend_routes(&self) -> bool {
        self.is_openai()
            && self.base_url.as_deref().is_none_or(|base_url| {
                base_url
                    .trim_end_matches('/')
                    .ends_with("/backend-api/kodex")
            })
    }
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Kodex CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These KODEX_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_kodex_oss_base_url = format!(
        "http://localhost:{kodex_oss_port}/v1",
        kodex_oss_port = std::env::var("KODEX_OSS_PORT")
            .ok()
    ...
    let kodex_oss_base_url = std::env::var("KODEX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_kodex_oss_base_url);
    create_oss_provider_with_base_url(&kodex_oss_base_url, wire_api)
}
