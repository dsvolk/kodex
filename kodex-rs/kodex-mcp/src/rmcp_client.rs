...
//! and exposing cached Kodex Apps tools while a client is still connecting.
...
use crate::kodex_apps::normalize_kodex_apps_callable_name;
use crate::kodex_apps::normalize_kodex_apps_callable_namespace;
use crate::kodex_apps::normalize_kodex_apps_tool_title;
use crate::kodex_apps::prepare_openai_file_params_for_model;
use crate::mcp::KODEX_APPS_MCP_SERVER_NAME;
...
use kodex_login::AuthChangeState;
...
use kodex_protocol::protocol::McpStartupUpdateEvent;
use kodex_rmcp_client::ExecutorStdioServerLauncher;
...
use kodex_rmcp_client::StreamableHttpBearerToken;
use kodex_rmcp_client::StreamableHttpRedirectMode;
use kodex_rmcp_client::ToolWithConnectorId;
use kodex_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that Kodex should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "kodex/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "kodex/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "kodex.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "kodex.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const KODEX_APPS_REFRESH_DURATION_METRIC: &str = "kodex.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const KODEX_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const KODEX_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) kodex_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
        ...
        self.tool_catalog
            .read(|catalog| {
                ...
                if catalog.revision == 0
                    && let Some(cache_context) = &self.kodex_apps_tools_cache_context
                {
...
#[derive(Default)]
struct KodexAppsStartupReconnectState {
...
#[derive(Clone)]
struct KodexAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct KodexAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<KodexAppsStartupReconnectState>,
    startup_status_context: Option<KodexAppsStartupStatusContext>,
}

impl KodexAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(KodexAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = tx_event.map(|tx_event| KodexAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = kodex_apps_reconnect_backoff(state.consecutive_failures);
...
fn kodex_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    KODEX_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(KODEX_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    kodex_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
    ...
    fn start(&self) -> ManagedClientFuture {
        ...
        *self
        ...
        let Self {
            ...
            kodex_apps_tools_cache_context,
        ...
        let is_kodex_apps_mcp_server = server_name == KODEX_APPS_MCP_SERVER_NAME;
        ...
        async move {
            ...
            let refresh_start = is_kodex_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    KODEX_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_kodex_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<KodexAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let cached_server_info = if is_kodex_apps_mcp_server {
            kodex_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            kodex_apps_tools_cache_context: kodex_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_kodex_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                KodexAppsStartupReconnect::new(Arc::new(move || startup.start()))
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.kodex_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Result<Vec<ToolInfo>, StartupOutcomeError> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(error) if self.is_kodex_apps_mcp_server => self.cached_tools().ok_or(error),
                Err(error) => Err(error),
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_kodex_apps_mcp_server: bool,
    kodex_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_kodex_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", kodex_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Filters disabled connectors, presents declared Kodex Apps file parameters to the model as
...
pub(crate) fn prepare_kodex_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_kodex_apps_mcp_server {
        kodex_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Kodex Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn kodex_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_kodex_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_kodex_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_kodex_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    *server_capabilities
    ...
    let server_supports_sandbox_state_meta_capability = initialize_result
        ...
        .is_some();
    let kodex_apps_tools_cache_context = kodex_apps_tools_cache_context.map(|context| {
        if server_disables_tool_catalog_cache {
    ...
    let fetch_ticket = kodex_apps_tools_cache_context
        .as_ref()
        .map(|context| context.begin_fetch(ConnectorRuntimeFetchSource::Startup));
    let tools = list_tools_for_client_uncached(
        &server_name,
        is_kodex_apps_mcp_server,
        /*kodex_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let client_tools: Arc<[ToolInfo]> =
        match (kodex_apps_tools_cache_context.as_ref(), fetch_ticket) {
            ...
            _ => unreachable!("Kodex Apps fetch ticket requires cache context"),
        };
    ...
    if is_kodex_apps_mcp_server || tool_catalog_cache_context.is_some() {
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
            list_start.elapsed(),
            &[("cache", "miss")],
...
fn record_protocol_discovery_metrics(
    ...
    if is_kodex_apps_mcp_server {
        tags.push(("server_kind", "openai_kodex_apps"));
    }
    let _ = metrics.counter("kodex.mcp.protocol_discovery", /*inc*/ 1, &tags);
    let _ = metrics.record_duration(
        "kodex.mcp.protocol_discovery.duration_ms",
        started_at.elapsed(),
        &tags,
    );
}
...
pub(crate) fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("kodex-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Kodex"),
    )
...
pub(crate) async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(kodex_utils_path_uri::LegacyAppPathString::into_string);
                ...
                && let Some(env_var) = bearer_token_env_var.as_ref()
            {
                (
                    Arc::new(ExecutorEnvironmentHttpClient {
                        ...
                        http_client,
                    }) as Arc<dyn kodex_exec_server::HttpClient>,
...
mod tests {
    ...
    #[test]
    fn kodex_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: KODEX_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "kodex_apps__gmail".to_string(),
