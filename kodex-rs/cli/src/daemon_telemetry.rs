...
pub(crate) async fn record_command(
    ...
    target: &'static str,
    result: &anyhow::Result<Option<kodex_app_server_daemon::UpdateOutput>>,
) {
    if std::env::var_os(kodex_app_server_daemon::telemetry::HANDOFF_ENV).is_some() {
        return;
    }
    // Skip telemetry if consent-aware configuration cannot be loaded promptly.
    let Ok(Ok(config)) = timeout(
        Duration::from_secs(/*secs*/ 2),
        crate::cloud_config::load_config(overrides, kodex_config::LoaderOverrides::default()),
    )
    .await
    else {
        return;
    };
    use kodex_app_server_daemon::UpdateStatus;
    ...
    if let Some(metrics) = otel.metrics() {
        let mut tags = kodex_app_server_daemon::telemetry::settings_tags(&config.kodex_home)
            .await
            .to_vec();
        tags.extend([
