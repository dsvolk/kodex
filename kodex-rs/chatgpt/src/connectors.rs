...
use kodex_connectors::ConnectorMetadata;
use kodex_connectors::ConnectorMetadataStore;
use kodex_connectors::ConnectorToolSummary;
use kodex_connectors::DirectoryListResponse;
use kodex_connectors::merge::merge_connectors;
use kodex_connectors::merge::merge_plugin_connectors;
use kodex_core::config::Config;
pub use kodex_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use kodex_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use kodex_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use kodex_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use kodex_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use kodex_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "kodex";
...
async fn connector_auth(config: &Config) -> anyhow::Result<KodexAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_kodex_api_key_env*/ false).await?;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Kodex backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = kodex_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = kodex_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_kodex_backend(),
        "ChatGPT backend requests require Kodex backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run kodex login"
    );
...
mod tests {
    use super::*;
    use kodex_connectors::metadata::connector_install_url;
    use kodex_plugin::AppConnectorId;
