...
use kodex_network_proxy::build_config_state;
...
use kodex_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn process_platform_defaults_allow_scratch_without_granting_it_to_filesystem_helpers() {
    let workspace = tempfile::Builder::new()
        .prefix("kodex-seatbelt-approved-project-")
    ...
    for scratch_root in ["/private/tmp", "/private/var/tmp"] {
        let scratch = tempfile::Builder::new()
            .prefix("kodex-seatbelt-process-scratch-")
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/kodex-unreadable");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.kodex".to_string(),
...
-> anyhow::Result<()> {
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .build()
        .await?;
    let prepared_socket = "/tmp/kodex-prepared-use";
    let explicit_socket = "/tmp/kodex-browser-use";
    let managed_network = ManagedNetworkSandboxContext {
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/kodex-readable");
    let unreadable = absolute_path("/tmp/kodex-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
            path: root.into(),
...
fn preferences_access_requires_unrestricted_reads() {
    ...
    denied_path.entries.push(FileSystemSandboxEntry::new(
        absolute_path("/tmp/kodex-private").into(),
        FileSystemAccessMode::Deny,
    ));
...
fn restricted_reads_cannot_read_preferences_outside_allowed_roots() {
    ...
    let marker = "kodex-preferences-read-canary";
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/kodex-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec!["/usr/bin/true".to_string()],
        file_system_sandbox_policy: &file_system_policy,
        network_sandbox_policy: NetworkSandboxPolicy::Restricted,
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/kodex-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/kodex-proxy-use";
    let explicit_socket = "/tmp/kodex-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_kodex_subpaths() {
    ...
    // write to .kodex/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.kodex carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_kodex_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .kodex/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_kodex_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
    ...
    // Verify that writing a file to the folder containing .git and .kodex is allowed.
...
fn seatbelt_protects_resolved_target_of_symlinked_metadata_directory() {
    ...
    let dot_kodex = writable_root.join(".kodex");
    ...
    symlink(&actual_config, &dot_kodex).expect("create .kodex symlink");
    let policy = restricted_write_policy(&[writable_root.as_path()]);
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec![
            "/bin/sh".to_string(),
    ...
    assert!(
        !output.status.success(),
        "resolved .kodex target should remain read-only"
    );
    assert_eq!(
...
#[test]
fn create_seatbelt_args_block_first_time_dot_kodex_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_kodex = repo_root.join(".kodex");
    let config_toml = dot_kodex.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_kodex.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_kodex = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_kodex_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_kodex),
        "missing {expected_dot_kodex}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .kodex/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".kodex")).expect("create .kodex");
    fs::write(
        vulnerable_root.join(".kodex").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .kodex/config.toml");
    ...
    let dot_kodex_canonical = vulnerable_root_canonical.join(".kodex");
