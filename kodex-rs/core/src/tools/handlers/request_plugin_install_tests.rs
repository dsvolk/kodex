...
async fn request_plugin_install_rejects_subagent_threads(presentation: ToolSuggestPresentation) {
    ...
    let result = RequestPluginInstallHandler::new(Vec::new(), presentation)
        .handle(ToolInvocation {
            ...
            tool_name: kodex_tools::ToolName::plain(REQUEST_PLUGIN_INSTALL_TOOL_NAME),
...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    write_curated_plugin_sha(kodex_home.path());
    write_plugins_feature_config(kodex_home.path());

    let config = load_plugins_config(kodex_home.path()).await;
    let plugins_manager =
    ...
    let refreshed_config = load_plugins_config(kodex_home.path()).await;
    assert!(verified_plugin_install_completed(
        "sample@openai-api-curated",
        &refreshed_config,
        &plugins_manager,
...
async fn persist_disabled_install_request_writes_connector_config() {
    ...
    persist_disabled_install_request(&kodex_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(kodex_home.path().join(CONFIG_TOML_FILE)).expect("read config");
