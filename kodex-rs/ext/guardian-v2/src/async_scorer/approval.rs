...
use kodex_extension_api::ApprovalDecisionInput;
use kodex_extension_api::ApprovalReviewContributor;
use kodex_extension_api::ExtensionFuture;
use kodex_protocol::approvals::GuardianReviewReason;
        ...
        let mut policy = guardian_config.as_ref().map_or_else(
            || {
                kodex_config::GuardianPolicyLoader::new(
                    Some(&kodex_features::FeatureToml::Enabled(true)),
...
async fn cached_evidence(
    ...
    let action = kodex_guardian_context::action_for_review(input.action.clone());
