            ...
            self.chat_widget.show_selection_view(SelectionViewParams {
                ...
                items: [
                    ...
                    (!workload_identity_selected).then(|| SelectionItem {
                        ...
                        description: Some(
                            "Open `kodex agents` in another terminal afterward.".to_string(),
                        ),
    ...
    async fn attach_agents_overview_thread(
        ...
        if self.primary_thread_id != Some(root_thread_id) {
            ...
            let resume_model_settings = match target_thread.status {
                ...
                kodex_app_server_protocol::ThreadStatus::Idle
                | kodex_app_server_protocol::ThreadStatus::Active { .. }
                | kodex_app_server_protocol::ThreadStatus::SystemError => {
    ...
    pub(super) async fn replay_agents_overview_requests(
        ...
        for request in requests {
            self.handle_app_server_event(
                app_server,
                kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(request)),
            )
            .await;
        }
    }
        ...
        tokio::spawn(async move {
            let result = async {
                ...
                let executable = if current_executable
                    ...
                    .is_some_and(|name| {
                        if cfg!(windows) {
                            name.eq_ignore_ascii_case("kodex-tui")
                        } else {
                            name == "kodex-tui"
                        }
                    }) {
