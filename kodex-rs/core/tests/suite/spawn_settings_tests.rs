...
use kodex_protocol::protocol::CollabAgentRef;
...
type RemovedChild = (ThreadId, Arc<KodexThread>);
...
async fn spawn_reports_effective_settings_after_child_runtime_is_removed() -> Result<()> {
    ...
    assert!(matches!(
        ...
        Err(error) if matches!(error.details(), kodex_protocol::error::KodexErrorDetails::ThreadNotFound(id) if *id == child_id)
    ));
    let spawn = wait_for_event_match(&test.kodex, |event| match event {
    ...
    .await;
    for thread in [test.kodex.as_ref(), child.as_ref()] {
