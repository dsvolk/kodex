    ...
    wiremock::Mock::given(wiremock::matchers::method("GET"))
        ...
        .respond_with(|request: &wiremock::Request| {
            ...
            let data = start.iter_days().take_while(|date| *date <= end).map(|date| {
                ...
                match route {
                    "daily-token-usage-breakdown" | "daily-workspace-user-token-usage-breakdown" => json!({
                        "date":date, "product_surface_usage_values":{"kodex_cli":10.0},
                        "premium_usage_values":{
                            "credit_usage_credits":{"kodex_cli":10.0}, "total_usage_credits":{},
                    ...
                    "daily-workspace-user-credit-usage" => json!({"date":date,"values":{"kodex":10.0}}),
            ...
            let response = if route == "daily-workspace-user-credit-usage" {
                json!({"breakdown":params["breakdown"],"series":[{"key":"kodex","label":"Kodex","total":data.len() as f64 * 10.0}],"data":data})
            } else {
                json!({"data":data})
            };
