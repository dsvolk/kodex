...
use core_test_support::test_kodex::run_test_with_large_stack;
...
use kodex_core_plugins::ExecutorPluginProvider;
use kodex_core_plugins::PluginCatalog;
use kodex_core_plugins::PluginCatalogEntry;
use kodex_core_plugins::PluginIdentity;
use kodex_core_plugins::PluginListQuery;
use kodex_core_plugins::PluginProvider;
use kodex_core_plugins::PluginProviderError;
use kodex_core_plugins::PluginProviderFuture;
use kodex_core_plugins::PluginSourceLocation;
...
use kodex_mcp_extension::PluginProviders;
use kodex_mcp_extension::PluginsThreadState;
use kodex_mcp_extension::install_plugin_providers;
...
async fn selected_plugin_servers_use_managed_requirements_for_the_selected_root_id() -> TestResult {
    ...
    std::fs::create_dir_all(plugin_root.path().join(".kodex-plugin"))?;
    std::fs::write(
        plugin_root.path().join(".kodex-plugin/plugin.json"),
...
async fn raw_selected_plugin_contributions(
    ...
    kodex_mcp_extension::install_plugins(&mut builder, Arc::clone(&environment_manager));
...
async fn run_cloud_plugin_projection_scenarios() -> anyhow::Result<()> {
    ...
    for (case, installed, cloud_enabled, cloud_available, cloud_empty) in [
        ...
        let declaration =
            json!({"url": format!("{}/api/kodex/ps/mcp", plugin_mcp.chatgpt_base_url)});
        ...
        if installed {
            ...
            fs::create_dir_all(root_path.join(".kodex-plugin"))?;
            fs::write(root_path.join(".kodex-plugin/plugin.json"), json!({
        ...
        let calendar = requests[0]
            .tool_by_name("mcp__kodex_apps__calendar", "_list_events")
