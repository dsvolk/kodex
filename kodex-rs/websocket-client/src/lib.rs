//! Proxy-aware WebSocket connection setup shared by Kodex API clients, reusing the HTTP factory's
...
use futures::Sink;
use futures::Stream;
use kodex_http_client::BuildCustomCaTransportError;
...
/// Selects whether WebSocket TLS follows Kodex custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Kodex custom CAs.
    ExplicitKodexTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Kodex custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ) -> Result<Self, BuildCustomCaTransportError> {
        Self::new_with_tls_mode(http_client_factory, WebSocketTlsMode::ExplicitKodexTls)
    }

    /// Creates a connector with explicit Kodex TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build Kodex TLS configuration when they establish their
    ...
    pub fn new_with_tls_mode(
        ...
        let tls_config = match tls_mode {
            WebSocketTlsMode::ExplicitKodexTls => {
