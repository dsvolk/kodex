...
pub(super) async fn windows_sandbox_ready(app_server: &mut AppServerSession) -> bool {
    ...
    matches!(
        ...
        .await,
        Ok(Ok(
            kodex_app_server_protocol::WindowsSandboxReadinessResponse {
                status: kodex_app_server_protocol::WindowsSandboxReadiness::Ready,
            }
        ))
    )
}
...
impl App {
        ...
        if self.windows_sandbox.pending_setup.is_some() {
            if self.windows_sandbox.setup_started_at.is_none() {
                self.chat_widget.add_info_message(
                    "Windows sandbox setup is still running. Restart Kodex to retry.".to_string(),
                    /*hint*/ None,
                );
            }
            return;
        ...
        let response = tokio::time::timeout(
            ...
            app_server
                ...
                .request_typed(ClientRequest::WindowsSandboxSetupStart {
                    request_id,
                    params: kodex_app_server_protocol::WindowsSandboxSetupStartParams {
                        mode: setup_mode,
    ...
    pub(super) fn finish_windows_sandbox_setup_start(
        ...
        response: std::result::Result<
            std::result::Result<
                kodex_app_server_protocol::WindowsSandboxSetupStartResponse,
                TypedRequestError,
            >,
        ...
        match response {
            Ok(Ok(kodex_app_server_protocol::WindowsSandboxSetupStartResponse {
                started: true,
            })) => {}
            Err(_) => {
                self.chat_widget.add_error_message(
                    "Windows sandbox setup request timed out. Waiting for completion; restart Kodex if it does not finish."
                        .to_string(),
                );
            }
            Ok(Err(
