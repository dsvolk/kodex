...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/kodex/kodex-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "kodex-manual.md";
const OUTLINE_FILE_NAME = "kodex-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "kodex-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-kodex-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Kodex Manual Outline\n\n${outline.text}\n`;
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchKodexManual(args);
...
export { DEFAULT_MANUAL_URL, fetchKodexManual };
