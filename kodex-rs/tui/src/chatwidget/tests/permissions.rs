...
async fn remote_windows_agent_permission_uses_server_selection() {
    ...
    assert!(matches!(events.try_recv(), Ok(AppEvent::KodexOp(_))));
    assert!(matches!(
        events.try_recv(),
...
async fn permission_discovery_uses_server_catalog_for_remote_custom_selection() {
    ...
    discovery
        .profiles
        .push(kodex_app_server_protocol::PermissionProfileSummary {
...
pub(super) fn requirements_stack(
    requirements_toml: kodex_config::ConfigRequirementsToml,
) -> ConfigLayerStack {
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.explicit_permission_profile_mode = true;
    chat.config.config_layer_stack = requirements_stack(kodex_config::ConfigRequirementsToml {
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default Kodex agent sandbox"),
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let op = std::iter::from_fn(|| rx.try_recv().ok())
        .find_map(|event| match event {
            AppEvent::KodexOp(op @ Op::OverrideTurnContext { .. }) => Some(op),
            _ => None,
        })
