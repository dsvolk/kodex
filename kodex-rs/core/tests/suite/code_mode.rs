...
use kodex_extension_api::ToolCallOutcome;
...
use kodex_mcp::kodex_apps_mcp_server_config;
...
use kodex_tools::FunctionCallError;
...
async fn run_code_mode_turn_with_config(
    ...
    run_code_mode_turn_with_model_and_config(server, prompt, code, "test-gpt-5.1-kodex", configure)
        .await
}
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_kodex().with_model(model).with_config(move |config| {
...
async fn run_unavailable_code_mode_turn(
    ...
    loop {
        match wait_for_event(&test.kodex, |_| true).await {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_kodex()
        .with_model("test-gpt-5.1-kodex")
        .with_code_mode_host_program("kodex-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("kodex-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("kodex-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth = KodexAuth::from_api_key("dummy");
    let auth_manager = kodex_core::test_support::auth_manager_from_auth(auth.clone());
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-kodex")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-kodex").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let mut builder = test_kodex().with_model(model).with_config(move |config| {
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.kodex, "rmcp").await?;
...
async fn code_mode_exec_holds_captured_result_during_elicitation() -> Result<()> {
    ...
    assert_eq!(
        test.kodex.increment_out_of_band_elicitation_count().await?,
        1
    );
    assert_eq!(
        test.kodex.increment_out_of_band_elicitation_count().await?,
    ...
    let release_elicitation = async {
        ...
        assert_eq!(
            test.kodex.decrement_out_of_band_elicitation_count().await?,
            1
        );
        ...
        assert!(
            metadata["executed_tool_calls"]
                ...
                .is_some_and(|calls| calls.iter().any(|call| call["arguments"]
                    .get("_kodex_executed_tool_call_truncated")
                    .is_some()))
        );
    } else {
        assert_eq!(
...
fn result_metadata_apps_builder(base_url: String, account_email: &str) -> TestKodexBuilder {
    ...
    let auth = KodexAuth::from_external_chatgpt_tokens(
...
async fn code_mode_mcp_metadata_keeps_originating_window_after_compaction() -> Result<()> {
    ...
    let control = Arc::new(ResultMetadataTestControl {
        server: Mutex::new(McpServerContribution::HostedApps {
            config: Box::new(kodex_apps_mcp_server_config(
...
text("done");"#,
    ...
    let originating_window_id =
        started.single_request().body_json()["client_metadata"]["x-kodex-window-id"].clone();
    ...
    assert_ne!(
        resumed_request.body_json()["client_metadata"]["x-kodex-window-id"],
...
async fn result_metadata_follows_call_binding(
    ...
    let (test, follow_up) = if direct {
        ...
        responses::mount_sse_once(
            ...
            sse(vec![
                responses::ev_function_call_with_namespace(
                    "call-1",
                    "mcp__kodex_apps__messagesearch",
    ...
    assert_eq!(test.kodex.analytics_enabled(), effective_analytics_enabled);
    let request = follow_up.single_request();
    ...
    let captured = kodex_core::test_support::history_with_tool_call_metadata(&test.kodex).await;
    if direct {
        let result = captured
            .iter()
            .find_map(|item| match item {
                kodex_protocol::models::ResponseItem::FunctionCallOutput {
                    call_id,
                    output,
                    ..
...
async fn code_mode_resumed_wait_does_not_certify_a_reused_runtime_cell() -> Result<()> {
    ...
    assert_eq!(metadata["tool_calls_complete"], true);
    resumed.kodex.shutdown_and_wait().await?;
    Ok(())
}
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__kodex_apps__calendar_timezone_option_99"
);
if (!tool) {
        ...
        apps_tool_calls.iter().any(|call| {
            call.pointer("/params/_meta/itemId") == Some(&serde_json::json!(originating_item_id))
                ...
                && call.pointer("/params/_meta/windowId")
                    == Some(&first_body["client_metadata"]["x-kodex-window-id"])
        }),
...
"#,
        visible_tool_name = "mcp__kodex_apps__calendar_timezone_option_99",
...
struct ResponseIdObserver {
    ...
    originating_items: Mutex<Vec<(String, Option<kodex_protocol::ResponseItemId>)>>,
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let kodex_extension_api::ToolCallSource::CodeMode { cell_id, .. } = input.source else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    kodex_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
"#,
    ...
    assert!(!exec_description.contains("kodex/imageDetail"));
...
async fn code_mode_node_repl_screenshots_can_be_captured_without_guardian_transcript_flags(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    core_test_support::wait_for_mcp_server(&test.kodex, repl_server).await?;
    let evidence = test
        .kodex
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if reviewer_compaction {
                    ...
                    reviewer.input_modalities =
                        vec![kodex_protocol::openai_models::InputModality::Text];
                } else {
...
"#,
        ...
        test.kodex
            .start_or_steer_turn(
                ...
                .with_thread_settings(ThreadSettingsOverrides {
                    environments: Some(kodex_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
        ...
        if use_responses_lite {
            let metadata: Value = serde_json::from_str(
                first_body["client_metadata"]["x-kodex-turn-metadata"]
                    .as_str()
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let new_thread = base_test
        ...
        .start_thread(StartThreadOptions {
            dynamic_tools: vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
                name: "kodex_app".to_string(),
                description: "Kodex app tools.".to_string(),
                tools: vec![DynamicToolNamespaceTool::Function(
                    DynamicToolFunctionSpec {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "kodex_app__hidden_dynamic_tool");
const out = await tools.kodex_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("kodex_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("kodex_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("Kodex app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("kodex_app__hidden_dynamic_tool(args:")
            })
    );
