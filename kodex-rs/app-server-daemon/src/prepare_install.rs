...
pub async fn update_from_cli(
    ...
    let managed_kodex_path = daemon.current_managed_kodex_bin()?;
    Ok(Some(crate::UpdateOutput {
        status: crate::UpdateStatus::Updated,
        installed_version: Some(managed_install::managed_kodex_version(&managed_kodex_path).await?),
        running_version: crate::client::probe(&daemon.socket_path)
            .await
            .ok()
            .map(|info| info.app_server_version),
        managed_kodex_path,
        message: "The CLI package is selected and pinned. Run `kodex app-server daemon update` to return to production updates.".to_string(),
    }))
}
...
async fn prepare_from_package(
    ...
    anyhow::ensure!(
        daemon.managed_kodex_bin.starts_with(&previous_root),
    ...
    if mode == InstallMode::Missing {
        ...
        {
            daemon.ensure_managed_kodex_bin()?;
            return Ok(true);
        }
    } else {
        anyhow::ensure!(
            previous_root.join("current").symlink_metadata().is_ok(),
            "no daemon package is selected; run `kodex app-server daemon start` first"
        );
    }
    ...
    let selected = managed_install::managed_kodex_bin(home);
    ...
    let source = source.context(
        "this CLI has no complete local package; install a packaged Kodex CLI or use the standalone installer",
    )?;
    anyhow::ensure!(
        !root.canonicalize()?.starts_with(source.canonicalize()?),
        "KODEX_HOME must be outside the source CLI package"
    );
    let manifest_bytes = std::fs::read(source.join("kodex-package.json"))?;
    let manifest: KodexPackageManifest = serde_json::from_slice(&manifest_bytes)?;
    ...
    if mode == InstallMode::Replace {
        if !confirm(&InstallRequest {
            ...
            installed_version: tokio::time::timeout(
                ...
                managed_install::managed_kodex_version(&selected),
            )
            .await
            .ok()
            .and_then(Result::ok),
    ...
    if mode == InstallMode::Missing && managed_install::managed_kodex_bin(home).is_file() {
        return Ok(true);
    }
    anyhow::ensure!(
    ...
    let staged_exe = if cfg!(target_os = "macos") && stage.path().join("KodexCLI.app").is_dir() {
        stage.path().join("KodexCLI.app/Contents/MacOS/kodex")
    } else {
        stage.path().join(entrypoint)
    };
    anyhow::ensure!(
        package_tree(source, /*destination*/ None)? == digest
            && std::fs::read(stage.path().join("kodex-package.json"))? == manifest_bytes
    ...
    let binary_version =
        managed_install::managed_kodex_version(&stage.path().join(entrypoint)).await?;
    anyhow::ensure!(
    ...
    if release.try_exists()? {
        ...
        #[cfg(unix)]
        if !stage.path().join("kodex").exists() {
            std::os::unix::fs::symlink("bin/kodex", stage.path().join("kodex"))?;
        }
    ...
    if backend.is_some() {
        let selected = Daemon {
            ...
            managed_kodex_bin: root.join("current").join(entrypoint),
            ..daemon.clone()
        };
        selected.start_managed_backend(settings).await.context(
            "daemon package selected but could not start; retry with `kodex app-server daemon start`",
        )?;
...
fn package_tree(root: &Path, destination: Option<&Path>) -> Result<String> {
    ...
    while let Some(path) = paths.pop() {
        ...
        if cfg!(unix)
            && relative == Path::new("kodex")
            && std::fs::read_link(&path).ok().as_deref() == Some(Path::new("bin/kodex"))
        {
            continue;
        }
        anyhow::ensure!(
...
fn validate_package(root: &Path) -> Result<()> {
    ...
    if cfg!(windows) {
        ...
        names.push("kodex-resources/bwrap");
    }
    for name in names {
        if !root.join(name).is_file() {
            return Err(anyhow!(
                "local Kodex package is missing {name}; reinstall the CLI or use the standalone installer"
            ));
        }
        #[cfg(unix)]
        if name != "kodex-package.json" {
            use std::os::unix::fs::PermissionsExt;
            if std::fs::metadata(root.join(name))?.permissions().mode() & 0o111 == 0 {
                return Err(anyhow!(
                    "local Kodex package file {name} is not executable; reinstall the CLI or use the standalone installer"
                ));
            }
        }
    }
