//! Ambient terminal rendering for the Kodex companion.
//!
...
impl AmbientPet {
    ...
    /// KODEX_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_kodex_home(
            ...
            /*kodex_home*/ Some(kodex_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = kodex_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_kodex_app_vocabulary() {
