...
fn local_directory_path_accepts_drive_and_verbatim_drive_paths() {
    assert!(validate_local_directory_path(Path::new(r"C:\Users\alice\.kodex")).is_ok());
    assert!(validate_local_directory_path(Path::new(r"\\?\D:\Kodex Data\home")).is_ok());
}
...
fn local_directory_path_rejects_relative_network_parent_and_stream_paths() {
    for path in [
        ...
        r"\\?\UNC\server\share\.kodex",
        r"\\.\C:\Users\alice",
        r"C:\Users\alice\..\other",
        r"C:\Users\alice\.kodex:stream",
    ] {
        assert!(
