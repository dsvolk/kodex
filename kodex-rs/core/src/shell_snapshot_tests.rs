...
async fn inactive_profiles_keep_snapshots_but_active_brokers_require_sandbox() -> Result<()> {
    ...
    let dir = tempdir()?;
    std::fs::create_dir(dir.path().join(".kodex"))?;
    std::fs::write(
        dir.path().join(".kodex/startup.sh"),
    ...
    config.shell_environment_policy.r#set.insert(
        "BASH_ENV".into(),
        dir.path().join(".kodex/startup.sh").display().to_string(),
    );
    ...
    let mut tool_config = crate::config::ConfigBuilder::without_managed_config_for_tests()
        .kodex_home(dir.path().to_path_buf())
        .build()
        .await?;
    tool_config
        .features
        .enable(kodex_features::Feature::ShellSnapshot)?;
    ...
    for (config, remains_valid) in [
        ...
        proxy
            .replace_config_state(kodex_network_proxy::build_config_state(
                config,
                kodex_network_proxy::NetworkProxyConstraints::default(),
...
async fn snapshot_discovers_and_redacts_shell_initialized_credentials() -> Result<()> {
    ...
    assert!(!snapshot.contains("KODEX_NETWORK_PROXY_BROKERED_CREDENTIALS"));
    ...
    fs::write(
        ...
        "test \"${HOME-missing}\" = missing && \
         test \"${KODEX_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE-}\" = 1\n",
    )
    .await?;
    validate_snapshot(
        &shell,
    ...
    let explicit_vendor_policy = ShellEnvironmentPolicy {
        inherit: kodex_protocol::config_types::ShellEnvironmentPolicyInherit::None,
...
async fn write_rollout_stub(kodex_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
    let dir = kodex_home
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    ...
    write_rollout_stub(&kodex_home, live_session).await?;
    fs::write(&live_snapshot, "live").await?;
    fs::write(&orphan_snapshot, "orphan").await?;
    fs::write(&invalid_snapshot, "invalid").await?;

    cleanup_stale_snapshots(&kodex_home, ThreadId::new(), /*state_db*/ None).await?;
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&kodex_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&kodex_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(&kodex_home, active_session, /*state_db*/ None).await?;
