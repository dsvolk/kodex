use kodex_config::GuardianPolicyLoader;
...
use kodex_features::GuardianV2ConfigToml;
use kodex_features::GuardianV2TranscriptConfigToml;
...
impl GuardianV2Config {
    pub(crate) fn resolve(config: &Config) -> Result<Self, String> {
        ...
        let legacy = FeatureToml::Config(GuardianV2ConfigToml {
            enabled: Some(config.features.enabled(kodex_features::Feature::GuardianV2)),
            ..configured
        });
        resolved.policy =
    ...
    pub(super) fn policy_for_model(
        &self,
        model: Option<&kodex_protocol::openai_models::ModelInfo>,
    ) -> GuardianModelPolicy {
