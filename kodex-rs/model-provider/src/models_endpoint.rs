...
use kodex_model_provider_info::CHATGPT_KODEX_BASE_URL;
use kodex_model_provider_info::ModelProviderInfo;
use kodex_models_manager::manager::ModelsEndpointClient;
use kodex_models_manager::manager::ModelsEndpointFuture;
use kodex_models_manager::manager::ModelsEndpointResponse;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<KodexAuth> {
    ...
    async fn uses_kodex_backend(&self) -> bool {
        self.auth()
            .await
            .as_ref()
            .is_some_and(KodexAuth::uses_kodex_backend)
    }
    ...
    async fn list_models(
        ...
        let metric_auth_mode = if self.has_provider_api_key()
            || auth.as_ref().is_some_and(KodexAuth::is_api_key_auth)
        {
            "api_key"
        ...
        let _timer = kodex_otel::start_global_timer(
            "kodex.remote_models.fetch_update.duration_ms",
            &[("auth_mode", metric_auth_mode)],
        );
        let identity = crate::models_identity::identity(&self.provider_info, auth.as_ref())?;
        let auth_mode = auth.as_ref().map(KodexAuth::auth_mode);
        let mut api_provider = self.provider_info.to_api_provider(auth_mode)?;
        if (auth.as_ref().is_some_and(KodexAuth::is_api_key_auth) || self.has_provider_api_key())
            && self.supports_api_key_models()
            && self.provider_info.base_url.is_none()
            && self.provider_info.model_catalog_url.is_none()
        {
            // Kodex metadata is served by the Kodex backend, not the public /v1/models API.
            api_provider.base_url = CHATGPT_KODEX_BASE_URL.to_string();
        }
        ...
        let agent_identity_telemetry = if let Some(KodexAuth::AgentIdentity(auth)) = auth.as_ref() {
            ...
            client
                ...
                .map_err(|mut error| {
                    if self.provider_info.model_catalog_url.is_some()
                        && let kodex_api::ApiError::Transport(TransportError::Http {
                            url,
                            headers,
                            body,
                            ..
        ...
        .await
        .map_err(|_| KodexErr::RequestTimeout)??;
    ...
    fn auth_env(&self) -> AuthEnvTelemetry {
        let kodex_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.kodex_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, kodex_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_kodex_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_kodex_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "kodex_otel.log_only",
            tracing::Level::INFO,
            event.name = "kodex.api_request",
            ...
            auth.env_kodex_api_key_present = self.auth_env.kodex_api_key_env_present,
            auth.env_kodex_api_key_enabled = self.auth_env.kodex_api_key_env_enabled,
...
mod tests {
    ...
    use kodex_login::default_client::ResidencyRequirement;
    use kodex_login::default_client::create_client;
    use kodex_login::default_client::set_default_client_residency_requirement;
    use kodex_models_manager::manager::ModelsManager;
    use kodex_models_manager::manager::OpenAiModelsManager;
    ...
    async fn api_key_discovery_respects_provider_routing() {
        let client_version = kodex_models_manager::client_version_to_whole();
        for (name, base_url, models_url, inference_url) in [
            (
                "OpenAI",
                None,
                Some("https://chatgpt.com/backend-api/kodex/models"),
                "https://api.openai.com/v1",
            ),
            (
                "OpenAI",
                Some("https://example.com/kodex"),
                None,
                "https://example.com/kodex",
            ),
            (
                "Azure",
            ...
            let auth = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("test-api-key"));
    ...
    #[tokio::test]
    async fn provider_api_key_without_endpoint_overrides_uses_kodex_backend() {
        for auth in [
            None,
            ...
            assert_eq!(
                *capture.0.lock().unwrap(),
                Some(format!(
                    "{CHATGPT_KODEX_BASE_URL}/models?client_version={}",
                    kodex_models_manager::client_version_to_whole(),
                ))
            );
        }
    }
    ...
    impl kodex_login::ExternalAuth for RotatingAuth {
        fn resolve(&self) -> kodex_login::ExternalAuthFuture<'_, KodexAuth> {
            Box::pin(async move {
                let generation = self.0.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                Ok(KodexAuth::from_api_key(&format!("token-{generation}")))
            })
        }

        fn refresh(
            &self,
            _context: kodex_login::ExternalAuthRefreshContext,
        ) -> kodex_login::ExternalAuthFuture<'_, KodexAuth> {
            self.resolve()
        }
    }
    ...
    async fn command_auth_refresh_fetches_a_catalog_for_the_current_credentials() {
        ...
        let server = MockServer::start().await;
        let auth = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("initial"));
        auth.set_external_auth(Arc::new(RotatingAuth(std::sync::atomic::AtomicUsize::new(
            0,
        ))))
        .await
        .unwrap();
        let model = kodex_protocol::openai_models::ModelInfo {
            used_fallback_model_metadata: false,
            ..kodex_models_manager::model_info::model_info_from_slug("command-auth-model")
        };
        Mock::given(method("GET"))
            .and(path("/models"))
        ...
        auth.auth().await;
        let bundled = kodex_models_manager::bundled_models_response().unwrap();
    ...
    async fn explicit_catalog_reuses_auth_headers_and_query_parameters() {
        for auth in [
            ...
            KodexAuth::from_api_key("test-key"),
        ] {
            let server = MockServer::start().await;
            let mut model =
                kodex_models_manager::model_info::model_info_from_slug("provider-model");
            ...
            let provider = ModelProviderInfo {
                model_catalog_url: Some(
                    format!("{}/kodex/models?deployment=one", server.uri()).into(),
                ),
                ...
                http_headers: Some(std::collections::HashMap::from([
                    ...
                    (
                        kodex_login::default_client::RESIDENCY_HEADER_NAME.to_string(),
                        "us".into(),
                    ),
                ])),
            ...
            manager.set_api_key_model_discovery_enabled(
                auth_manager.auth_mode() == Some(kodex_protocol::auth::AuthMode::ApiKey),
            );
            for _ in 0..2 {
                assert_eq!(
                    manager
    ...
    async fn explicit_catalog_rejects_oversized_response() {
        ...
        let endpoint = OpenAiModelsEndpoint::new(
            ModelProviderInfo {
                model_catalog_url: Some(format!("{}/kodex/models", server.uri()).into()),
        ...
        let KodexErrorDetails::InvalidRequest(message) = error.details() else {
    ...
    async fn explicit_catalog_rejects_redirects_without_forwarding_provider_credentials() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(/*s*/ 302)
                    .insert_header("location", format!("{}/kodex/models", destination.uri()))
        ...
        let endpoint = OpenAiModelsEndpoint::new(
            ModelProviderInfo {
                ...
                model_catalog_url: Some(
                    format!("{}/kodex/models?token=catalog-secret", server.uri()).into(),
                ),
                experimental_bearer_token: Some("provider-key".into()),
                ..ModelProviderInfo::default()
            },
