...
fn external_chatgpt_auth(access_token: &str) -> std::io::Result<KodexAuth> {
    KodexAuth::from_external_chatgpt_tokens(access_token, CHATGPT_ACCOUNT_ID, Some("enterprise"))
}
...
async fn header_auth_is_attached_to_responses_requests() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex().with_auth(KodexAuth::Headers(AuthHeaders::new(headers)));
...
async fn amazon_bedrock_managed_access_keys_sign_requests() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex()
        ...
        .with_auth(KodexAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
...
async fn amazon_bedrock_aws_auth_refresh_resigns() -> anyhow::Result<()> {
    ...
    const SUBPROCESS_ENV: &str = "KODEX_BEDROCK_AWS_REFRESH_TEST";
    const HELPER_ARG: &str = "KODEX_BEDROCK_AWS_REFRESH_COMMAND";
    ...
    if persistent_export_failure {
        test.kodex
            ...
            .await?;
        let EventMsg::TurnComplete(completed) = wait_for_event(&test.kodex, |event| {
...
async fn external_auth_401_retry_uses_refreshed_chatgpt_headers() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex().with_auth(KodexAuth::from_api_key("seed"));
    let test = builder.build_with_auto_env(&server).await?;
