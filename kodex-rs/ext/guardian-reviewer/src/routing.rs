...
use kodex_extension_api::SynchronousApprovalReviewer;
...
impl<H: ReviewHost> ReviewRequest<'_, H> {
    ...
    async fn cached_approval(&self) -> ReviewDecision {
        ...
        if self.thread_store.get::<GuardianV2Enabled>().is_some() {
            self.analytics
                .track_guardian_v2_event(kodex_analytics::GuardianV2Event {
                    thread_id: self.thread_id.to_string(),
                    turn_id: turn_id.to_owned(),
                    item_id: item_id.map(str::to_owned),
                    model: Some(self.model.slug.clone()),
                    occurred_at_ms: kodex_analytics::now_unix_millis(),
                    kind: kodex_analytics::GuardianV2EventKind::FastDecision {
                        decision: "approved",
                    },
                });
        }
