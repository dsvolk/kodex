...
use kodex_app_server_transport::daemon_recovery;
...
async fn managed_shutdown_skips_nonpersistent_threads_and_tolerates_save_failure(
    ...
    let source = if matches!(scenario, SnapshotScenario::Child) {
        kodex_protocol::protocol::SessionSource::SubAgent(
            kodex_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id: kodex_protocol::ThreadId::new(),
                depth: 1,
                agent_path: None,
                agent_nickname: None,
                agent_role: None,
...
async fn managed_shutdown_preserves_admitted_resume() -> Result<()> {
    ...
    std::fs::write(
        ...
        format!(
            ...
            format!("{}/api/kodex/ps/mcp", mcp.chatgpt_base_url),
        ),
    )?;
...
fn spawn_server(home: &Path, socket_path: &Path) -> Result<Child> {
    let binary = kodex_utils_cargo_bin::cargo_bin("kodex-app-server")?;
    Ok(Command::new(binary)
        ...
        .arg("--managed-daemon")
        .env(
            kodex_app_server_transport::DAEMON_SHUTDOWN_SOCKET_ENV,
            socket_path,
        )
        .stdin(Stdio::null())
        .stdout(Stdio::null())
