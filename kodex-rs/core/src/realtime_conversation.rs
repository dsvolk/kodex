...
use crate::client::X_KODEX_TURN_METADATA_HEADER;
...
use kodex_api::RealtimeAudioFrame;
use kodex_api::RealtimeContextAppendChannel;
use kodex_api::RealtimeEvent;
use kodex_api::RealtimeEventParser;
use kodex_api::RealtimeSessionConfig;
use kodex_api::RealtimeSessionMode;
use kodex_api::RealtimeWebsocketClient;
use kodex_api::RealtimeWebsocketConnection;
use kodex_api::RealtimeWebsocketEvents;
use kodex_api::RealtimeWebsocketWriter;
...
use kodex_protocol::protocol::RealtimeConversationClosedEvent;
use kodex_protocol::protocol::RealtimeConversationRealtimeEvent;
use kodex_protocol::protocol::RealtimeConversationSdpEvent;
use kodex_protocol::protocol::RealtimeConversationStartedEvent;
...
use kodex_utils_output_truncation::approx_bytes_for_tokens;
...
const STANDALONE_HANDOFF_ID: &str = "kodex";
const DEFAULT_REALTIME_MODEL: &str = "gpt-realtime-1.5";
const DEFAULT_FRAMELESS_REALTIME_MODEL: &str = "gpt-live-1-kodex";
...
struct RealtimeHandoffState {
    ...
    kodex_responses_as_items: bool,
    kodex_response_item_prefix: Option<String>,
    kodex_response_handoff_mode: KodexResponseHandoffMode,
    backend_reasoning_status: bool,
    kodex_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.kodex_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.kodex_response_handoff_mode == KodexResponseHandoffMode::BemTags
    }
}
...
struct RealtimeStart {
    ...
    kodex_response_handoff_mode: KodexResponseHandoffMode,
    backend_reasoning_status: bool,
    kodex_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    ) -> KodexResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> KodexResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            http_client_factory,
            ...
            kodex_responses_as_items,
            kodex_response_item_prefix,
            kodex_response_handoff_mode,
            backend_reasoning_status,
            kodex_response_handoff_channel_prefixes,
        ...
        let handoff = RealtimeHandoffState {
            ...
            kodex_response_handoff_mode,
            backend_reasoning_status,
            kodex_response_handoff_channel_prefixes: Arc::new(
                kodex_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> KodexResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(KodexErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> KodexResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| KodexErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn send_reasoning_status(&self, text: &str) -> KodexResult<()> {
        let handoff = self
            .state
            .lock()
            .await
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.kodex_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.kodex_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.kodex_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> KodexResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> KodexResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.kodex_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> KodexResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> KodexResult<PreparedRealtimeConversationStart> {
    ...
    {
        extra_headers.insert(X_KODEX_TURN_METADATA_HEADER, metadata);
    }
    Ok(PreparedRealtimeConversationStart {
        ...
        kodex_responses_as_items: params.kodex_responses_as_items,
        kodex_response_item_prefix: params.kodex_response_item_prefix,
        kodex_response_handoff_mode: params.kodex_response_handoff_mode,
        backend_reasoning_status: params.backend_reasoning_status,
        kodex_response_handoff_channel_prefixes: params.kodex_response_handoff_channel_prefixes,
...
) -> KodexResult<RealtimeSessionConfig> {
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> KodexResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), KodexErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&KodexAuth>, provider: &ModelProviderInfo) -> KodexResult<String> {
    ...
    if let Some(api_key) = auth.and_then(KodexAuth::api_key) {
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            KodexErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: KodexResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        KodexResponseHandoffMode::Thinking => None,
        KodexResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        KodexResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            ...
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.kodex_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    kodex_error_info: KodexErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
