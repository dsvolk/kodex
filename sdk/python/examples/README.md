...
- `sync.py` (public sync surface: `Kodex`)
- `async.py` (public async surface: `AsyncKodex`)

All examples intentionally use only public SDK exports from `openai_kodex`
and `openai_kodex.types`.
...
```bash
python -m pip install openai-kodex
```

The SDK installs its pinned `openai-kodex-cli-bin` runtime dependency.
