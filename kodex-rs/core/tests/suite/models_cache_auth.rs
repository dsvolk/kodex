...
fn header_auth(token: &'static str) -> KodexAuth {
    KodexAuth::Headers(AuthHeaders::new(HeaderMap::from_iter([
...
async fn auth_and_provider_switches_do_not_reuse_chatgpt_catalog() -> Result<()> {
    ...
    let api = test_kodex()
        .with_home(home.clone())
        .with_auth(KodexAuth::from_api_key("api-key"))
        .with_config(|config| {
            ...
            config
                .features
                .enable(kodex_features::Feature::ApiKeyModelDiscovery)
