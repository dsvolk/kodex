...
/// Filename used for the redacted `kodex doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "kodex-doctor-report.json";
/// Filename used for the raw Kodex Apps MCP tools cache feedback attachment.
pub const KODEX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "kodex-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const KODEX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "kodex-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_kodex_api_key_present = tracing::field::debug(auth_env.kodex_api_key_env_present),
        auth_env_kodex_api_key_enabled = tracing::field::debug(auth_env.kodex_api_key_env_enabled),
...
#[derive(Clone)]
pub struct KodexFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for KodexFeedback {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("kodex_http_client::transport", LevelFilter::DEBUG)
                    .with_target("kodex_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target("kodex_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("kodex_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
impl FeedbackAttachmentPath {
    ...
    pub fn read_attachment(&self, max_bytes: usize) -> io::Result<Option<FeedbackAttachment>> {
        let rollout_path = kodex_rollout::rollout_id_from_path(&self.path)
            .map(|_| kodex_rollout::plain_rollout_path(&self.path));
        let buffer = if rollout_path.is_some() {
            let Some(buffer) =
                kodex_rollout::read_rollout_prefix(&self.path, max_bytes.saturating_add(1))?
            else {
                return Ok(None);
            };
            buffer
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `kodex-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn log_attachment(&self, logs_override: Option<Vec<u8>>) -> FeedbackAttachment {
        FeedbackAttachment {
            filename: "kodex-logs.log".to_string(),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = KodexFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        ...
        tracing::trace!(target: "kodex_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "kodex_http_client::transport", "transport-trace");
        tracing::trace!(target: "kodex_api::sse", "sse-trace");
        tracing::trace!(target: "kodex_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "kodex_http_client::transport", "transport-debug");
        tracing::debug!(target: "kodex_api::sse::responses", "sse-debug");
        tracing::trace!(target: "kodex_feedback_test", "unrelated-trace");
        log::trace!(target: "kodex_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    async fn feedback_upload_allows_slow_reports() {
        ...
        upload_test_feedback(&KodexFeedback::new(), &dsn, &[attachment])
            .await
    ...
    async fn feedback_upload_deadline_stops_retries_and_later_attachments() {
        ...
        let snapshot = KodexFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default());
    ...
    async fn feedback_upload_retries_diagnostics_without_replaying_core() {
        ...
        upload_test_feedback(&KodexFeedback::new(), &dsn, &attachments)
            .await
    ...
    async fn feedback_upload_stops_diagnostics_for_sentry_rate_limits() {
        for (accepted_before_limit, rate_limit) in [
            ...
            upload_test_feedback(&KodexFeedback::new(), &dsn, &[later])
                .await
    ...
    async fn feedback_upload_does_not_forward_private_data_to_redirect_target() {
        ...
        let error = upload_test_feedback(&KodexFeedback::new(), &dsn, &[])
            .await
    ...
    async fn feedback_upload_reports_transport_failures() {
        ...
        assert!(
            error
                .downcast_ref::<kodex_http_client::RouteAwareRequestError>()
                .is_some_and(kodex_http_client::RouteAwareRequestError::is_connect)
        );
    }

    #[test]
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("kodex-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = KodexFeedback::new()
            .snapshot(/*session_id*/ None)
        ...
        let attachments_without_diagnostics = KodexFeedback::new()
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics::default())
    ...
    fn path_backed_attachments_use_binary_content_types() {
        let suffix = ThreadId::new();
        let gzip_filename = format!("kodex-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("kodex-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = KodexFeedback::new()
            .snapshot(/*session_id*/ None)
            .feedback_attachments(
                /*include_logs*/ false,
                &[],
