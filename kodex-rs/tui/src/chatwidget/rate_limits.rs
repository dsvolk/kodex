...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerKodexErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerKodexErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerKodexErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerKodexErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerKodexErrorInfo) -> bool {
    matches!(info, AppServerKodexErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    pub(crate) fn rate_limit_refresh_interval(&self) -> Option<std::time::Duration> {
        ...
        let used_percent = self
            ...
            .filter(|(limit_id, snapshot)| {
                limit_id.as_str() == "kodex" || snapshot.limit_name == self.current_model()
            })
    ...
    pub(crate) fn on_rolling_rate_limit_snapshot(&mut self, snapshot: RateLimitSnapshot) {
        if let (Some(previous), Some(current)) = (
            self.kodex_rate_limit_reached_type,
    ...
    fn on_rate_limit_snapshot_from(
        ...
        let usage_notice_blocked = self.kodex_rate_limit_reached_type.is_some()
            || self.kodex_spend_control_reached == Some(true);
        if let Some(mut snapshot) = snapshot {
            let limit_id = snapshot
                .limit_id
                .clone()
                .unwrap_or_else(|| "kodex".to_string());
            let is_kodex_limit = limit_id.eq_ignore_ascii_case("kodex");
            if is_kodex_limit
                && self
                    .usage_notice_state
            ...
            {
                self.kodex_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_kodex_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.kodex_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.kodex_spend_control_reached == Some(true)
                || matches!(
                    self.kodex_rate_limit_reached_type,
                    Some(
            ...
            if is_kodex_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_kodex_limit && !has_workspace_credits;
            ...
            self.kodex_rate_limit_reached_type = None;
            self.kodex_spend_control_reached = None;
        }
        if usage_notice_blocked
            != (self.kodex_rate_limit_reached_type.is_some()
                || self.kodex_spend_control_reached == Some(true))
        {
            self.request_redraw();
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::KodexOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using kodex. Request increase?",
            ),
        };
