...
//! inputs and the mutable [`kodex_mcp::McpRuntime`]. Background scheduling
//! belongs elsewhere.
...
pub(super) struct McpDesiredState {
    ...
    pub(super) auth: Option<KodexAuth>,
...
impl Session {
    ...
    pub(super) fn project_selected_environment_mcp_servers<'a>(
        ...
        Box::pin(async move {
            if self.isolation == kodex_extension_api::SessionIsolation::Isolated {
                return projection;
            }
            ...
            for selected in environments.turn_environments() {
                ...
                for (name, mut server) in servers {
                    ...
                    self.services.session_telemetry.counter(
                        "kodex.mcp.executor_discovery.server",
                        /*inc*/ 1,
                        &[("server_name", name.as_str()), ("outcome", outcome)],
                    );
                    if name == KODEX_APPS_MCP_SERVER_NAME
    ...
    pub(super) fn build_mcp_runtime_input(
        ...
        config
            .environment_cwds
            .entry(kodex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string())
        ...
        McpRuntimeInput {
            ...
            runtime_context,
            kodex_apps_tools_cache: self.services.mcp_manager.kodex_apps_tools_cache(),
