...
use kodex_protocol::protocol::COLLABORATION_MODE_CLOSE_TAG;
...
fn model_with_collaboration_messages(
    ...
    default: Option<&str>,
    plan: Option<&str>,
) -> kodex_protocol::openai_models::ModelInfo {
            ...
            .with_thread_settings(ThreadSettingsOverrides {
                ...
                summary: Some(
                    test.config
                        ...
                        .unwrap_or(kodex_protocol::config_types::ReasoningSummary::Auto),
                ),
                collaboration_mode: Some(collaboration_mode),
                ..Default::default()
            }),
...
async fn resume_replays_collaboration_instructions() -> Result<()> {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn cold_resume_refreshes_legacy_collaboration_snapshot_once(
    ...
    let builder_with_instructions = |instructions: &str| {
        ...
        test_kodex().with_model(MODEL).with_config(move |config| {
            config.model_catalog = Some(ModelsResponse {
                models: vec![model],
            });
        })
    ...
    let legacy_rollout = std::fs::read_to_string(&rollout_path)?
        .lines()
        .map(|original_line| {
            let mut line = kodex_rollout::parse_rollout_line(original_line)?;
