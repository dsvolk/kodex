use kodex_backend_client::ApiKeyTurnCost;
use kodex_backend_client::ApiKeyTurnCostStatus;
use kodex_backend_client::Client as BackendClient;
use kodex_backend_client::RequestError;
        ...
        match &self.backend {
            ...
            TurnCostBackend::ModelProvider(model_provider) => {
                ...
                let auth = model_provider
                    ...
                    .await
                    .map_err(|error| RequestError::Other(error.into()))?;
                let endpoint = provider.url_for_path("analytics/kodex/turn-costs");
