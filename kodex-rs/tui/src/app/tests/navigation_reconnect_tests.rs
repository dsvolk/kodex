...
-> Result<()> {
    ...
    for (previous_thread, changed_child_permissions, overview_initialized) in [
        ...
        let endpoint = crate::RemoteAppServerEndpoint::UnixSocket {
            socket_path: kodex_utils_absolute_path::AbsolutePathBuf::try_from(socket_path.clone())?,
        };
        ...
        if interrupted_setup {
            let preset = kodex_utils_approval_presets::builtin_approval_presets()
                .into_iter()
            ...
            while let Ok(event) = events.try_recv() {
                if let AppEvent::InsertHistoryCell(cell) = &event {
                    ...
                    if rendered.contains("Windows sandbox setup was interrupted") {
                        insta::assert_snapshot!(rendered, @"■ Windows sandbox setup was interrupted. Restart Kodex before using Agent mode.");
                        saw_warning = true;
                        continue;
                    }
                }
        ...
        if let Some(id) = previous_thread {
            ...
            .await?;
            while let Ok(event) = events.try_recv() {
                if matches!(event, AppEvent::KodexOp(AppCommand::UserTurn { .. })) {
                    app.handle_event(&mut tui, &mut session, event).await?;
                }
            }
