...
use core_test_support::test_kodex::TestEnv;
...
use kodex_app_server_protocol::FsCopyParams;
use kodex_app_server_protocol::FsCreateDirectoryParams;
use kodex_app_server_protocol::FsGetMetadataParams;
use kodex_app_server_protocol::FsReadDirectoryParams;
use kodex_app_server_protocol::FsReadFileParams;
use kodex_app_server_protocol::FsRemoveParams;
use kodex_app_server_protocol::FsUnwatchParams;
use kodex_app_server_protocol::FsWatchParams;
use kodex_app_server_protocol::FsWriteFileParams;
...
use kodex_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use kodex_app_server_protocol::PluginSearchParams;
use kodex_app_server_protocol::PluginSkillReadParams;
use kodex_app_server_protocol::PluginUninstallParams;
use kodex_app_server_protocol::ProcessKillParams;
use kodex_app_server_protocol::ProcessSpawnParams;
use kodex_app_server_protocol::ProjectImportParams;
use kodex_app_server_protocol::ProjectListParams;
use kodex_app_server_protocol::ProjectReadParams;
use kodex_app_server_protocol::RemoteControlClientsListParams;
use kodex_app_server_protocol::RemoteControlClientsRevokeParams;
use kodex_app_server_protocol::RemoteControlPairingStartParams;
use kodex_app_server_protocol::RemoteControlPairingStatusParams;
...
use kodex_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use kodex_exec_server::KODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use kodex_exec_server::KODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use kodex_exec_server::KODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use kodex_exec_server::KODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use kodex_exec_server::KODEX_EXEC_SERVER_URL_ENV_VAR;
...
pub struct TestAppServer {
    ...
    // removing an owned KODEX_HOME that may still be its cwd on Windows.
    ...
    _owned_kodex_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "kodex-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "KODEX_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary KODEX_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(kodex_home);
        cmd.env("KODEX_HOME", kodex_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "KODEX_APP_SERVER_MANAGED_CONFIG_PATH",
            kodex_home.join("managed_config.toml"),
        );
        cmd.env_remove(KODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("kodex app-server proc should start")?;
            }
            retries += 1;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: kodex_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    ...
    /// Uses this existing KODEX_HOME instead of a temporary one.
    pub fn with_kodex_home(mut self, kodex_home: &Path) -> Self {
        self.kodex_home = Some(kodex_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary KODEX_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (kodex_home, owned_kodex_home) = match kodex_home {
            Some(kodex_home) => (kodex_home, None),
            None => {
                let owned_kodex_home = TempDir::new()?;
                (
                    owned_kodex_home.path().to_path_buf(),
                    Some(owned_kodex_home),
                )
            }
        };
        let attribution_settings_server = if mock_chatgpt_backend
            || kodex_home.join("auth.json").is_file()
        {
            let config_path = kodex_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = kodex_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            kodex_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&kodex_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        KODEX_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        KODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        KODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (KODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        KODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => kodex_utils_cargo_bin::cargo_bin("kodex-app-server")
                .context("should find binary for kodex-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && kodex_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                kodex_utils_cargo_bin::cargo_bin("kodex-code-mode-host")
        {
        ...
        app_server._owned_kodex_home = owned_kodex_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `kodex-app-server` child process.
        //
