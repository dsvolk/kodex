...
use kodex_config::McpServerIdpOAuthConfig;
use kodex_config::McpServerToolConfig;
use kodex_config::McpServerTransportConfig;
use kodex_config::types::PluginMcpServerEmaAuthConfig;
use kodex_protocol::mcp_policy::EnvironmentMcpPolicy;
...
fn rejected_plugin_ema_registration_does_not_veto_hosted_apps() {
    ...
    for (url, resource) in [
        ...
        for initially_enabled in [true, false] {
            ...
            assert_eq!(
                catalog.server(KODEX_APPS_MCP_SERVER_NAME).unwrap().config(),
                &rejected,
            );
            ...
            for catalog in [catalog, materialized] {
                ...
                assert_eq!(
                    builder.build().server(KODEX_APPS_MCP_SERVER_NAME),
