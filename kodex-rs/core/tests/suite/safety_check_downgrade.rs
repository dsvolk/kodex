...
const REQUESTED_MODEL: &str = "gpt-5.3-kodex";
...
fn disabled_text_turn(test: &TestKodex, text: &str) -> TurnInputRequest {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_kodex().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.kodex, |event| {
    ...
    let warning = wait_for_event(&test.kodex, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.kodex, |event| {
...
#[test_case::test_case("cyber_policy", CYBER_POLICY_MESSAGE, KodexErrorInfo::CyberPolicy; "cyber")]
#[test_case::test_case("bio_policy", BIO_POLICY_MESSAGE, KodexErrorInfo::BioPolicy; "bio")]
#[test_case::test_case("invalid_prompt", INVALID_PROMPT_MESSAGE, KodexErrorInfo::InvalidPrompt; "invalid_prompt")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn error_response_emits_typed_error_without_retry(
    code: &str,
    message: &str,
    error_info: KodexErrorInfo,
) -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    for response in [
        ...
        let error = wait_for_event(&test.kodex, |event| matches!(event, EventMsg::Error(_))).await;
        let EventMsg::Error(error) = error else {
            panic!("expected error event");
        };
        assert_eq!(error.message, message);
        assert_eq!(error.kodex_error_info.as_ref(), Some(&error_info));
        assert_eq!(
            serde_json::to_value(&error.kodex_error_info)?,
...
async fn model_verification_only_emits_once_per_turn() -> Result<()> {
    ...
    let mut builder = test_kodex().with_model(SERVER_MODEL);
    let test = builder.build(&server).await?;
