...
use kodex_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "kodex-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/kodex-backend/agent-identity";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"kodex-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://chatgpt.com/kodex"
            | "https://chatgpt.com/backend-api/kodex"
            | "https://chat.openai.com"
            | "https://chat.openai.com/backend-api"
            | "https://chat.openai.com/kodex"
            | "https://chat.openai.com/backend-api/kodex" => Ok(Self::Production),
            "https://chatgpt-staging.com"
            | "https://chatgpt-staging.com/backend-api"
            | "https://chatgpt-staging.com/kodex"
            | "https://chatgpt-staging.com/backend-api/kodex" => Ok(Self::Staging),
            _ => anyhow::bail!(
...
/// single Kodex run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "kodex-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "kodex-cli".to_string(),
        },
...
mod tests {
    ...
    use kodex_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn chatgpt_environment_maps_known_urls_to_authapi() -> anyhow::Result<()> {
        assert_eq!(
            ChatGptEnvironment::from_chatgpt_base_url("https://chatgpt.com/backend-api/kodex")?,
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/kodex"),
            "http://localhost:8080/api/kodex/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/kodex/"),
            "http://localhost:8080/api/kodex/agent-identities/jwks"
