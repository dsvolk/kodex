...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "kodex.exe".to_string(),
            "--kodex-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
    ...
    assert_eq!(args[0], KODEX_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&KODEX_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["kodex.exe", "--kodex-run-as-fs-helper"]
    );
