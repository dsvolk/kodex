...
use kodex_api::ResponseStream;
use kodex_api::ResponsesApiRequest;
use kodex_api::ResponsesClient;
use kodex_api::ResponsesOptions;
use kodex_api::ResponsesWebsocketClient;
use kodex_api::ResponsesWebsocketConnection;
...
use kodex_model_provider::ACCOUNT_ROUTING_HEADER;
use kodex_model_provider::AgentIdentitySessionFallback;
use kodex_model_provider::ProviderAuthScope;
use kodex_model_provider::ResolvedResponsesProvider;
...
impl ConnectionPool {
    ...
    fn headers(
        ...
        if request_kind == RequestMode::GuardianClassifier {
            headers.insert("x-kodex-guardian", HeaderValue::from_static("classifier"));
        }
    ...
    async fn responses_request_kind(&self) -> RequestMode {
        ...
        if self
            ...
            && provider.supports_kodex_backend_routes()
    ...
    async fn open_connection(
        ...
        if let Some(metrics) = self.config.metrics.as_deref() {
            ...
            metrics.histogram(
                "kodex.guardian_v2.connection.duration_ms",
