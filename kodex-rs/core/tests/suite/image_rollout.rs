...
use kodex_attachment_store::ResolveFuture;
use kodex_attachment_store::ResolveRequest;
use kodex_attachment_store::UploadFuture;
use kodex_attachment_store::UploadRequest;
use kodex_attachment_store::UploadResult;
...
fn find_user_message_with_image(text: &str) -> Option<ResponseItem> {
    for line in text.lines() {
        ...
        let rollout = match kodex_rollout::parse_rollout_line(trimmed) {
            Ok(rollout) => rollout,
            Err(_) => continue,
        };
    ...
    let expected = ResponseItem::Message {
        ...
        content: vec![
            ContentItem::InputText {
                text: kodex_protocol::models::local_image_open_tag_text_with_path(
            ...
            ContentItem::InputText {
                text: kodex_protocol::models::image_close_tag_text(),
            },
            ContentItem::InputText {
                text: "pasted image".to_string(),
            },
...
async fn uploaded_images_preserve_user_message_display_history() -> anyhow::Result<()> {
    ...
    let started = wait_for_event_match(&test.kodex, |event| match event {
    ...
    .await;
    let completed = wait_for_event_match(&test.kodex, |event| match event {
    ...
    for line in rollout_text.lines() {
        if let RolloutItem::EventMsg(EventMsg::ItemCompleted(event)) =
            kodex_rollout::parse_rollout_line(line)?.item
...
async fn resumed_history_only_emits_resize_notices_for_new_images() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let initial = test_kodex().build_with_auto_env(&server).await?;
    ...
    let persisted_developer_metadata = fs::read_to_string(&rollout_path)?
        ...
        .filter_map(|line| {
            let RolloutItem::ResponseItem(envelope) = kodex_rollout::parse_rollout_line(line)
