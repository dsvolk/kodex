...
fn next_user_turn_event(
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        if let AppEvent::KodexOp(turn @ AppCommand::UserTurn { .. }) = event {
            return turn;
        }
    }
...
"#,
    ...
    assert!(
        std::iter::from_fn(|| app_event_rx.try_recv().ok())
            .all(|event| !matches!(event, AppEvent::KodexOp(AppCommand::UserTurn { .. })))
    );
...
async fn run_safety_retry(
    ...
    let mut model_catalog = kodex_models_manager::bundled_models_response()?;
    ...
    let model_catalog_path = kodex_home.path().join("models.json");
