...
fn auth_identity(auth: &KodexAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &KodexAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let kodex_home = AbsolutePathBuf::resolve_path_against_base(kodex_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(kodex_home.clone()),
            cache_enabled: true,
            kodex_home,
            timeout,
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            kodex_otel::start_global_timer("kodex.cloud_config_bundle.fetch.duration_ms", &[]);
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.kodex_home) {
                    tracing::warn!(
