...
use kodex_network_proxy::NetworkProxyState;
...
async fn test_network_proxy_with_config(
    ...
    let state = kodex_network_proxy::build_config_state(
        config,
        NetworkProxyConstraints::default(),
        kodex_utils_path_uri::Platform::native(),
    )?;
    NetworkProxy::builder()
        ...
        .managed_by_kodex(/*managed_by_kodex*/ false)
...
async fn explicit_escalation_prepares_exec_without_managed_network() -> anyhow::Result<()> {
    ...
    let command = kodex_sandboxing::SandboxCommand {
...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/kodex-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/kodex-path:/usr/bin:/bin"),
        "runtime PATH prepend should update the live exec environment"
    );
    assert_eq!(
        runtime_path_prepends.entries,
        vec!["/package/kodex-path"],
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/kodex-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/kodex-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/kodex-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/kodex-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/kodex-resources/zsh/bin:/bin:/package/kodex-resources/zsh/bin"
            .to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/kodex-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_kodex_marker() {
    let env = HashMap::from([
        (
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_kodex_metadata_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport KODEX_THREAD_ID='parent-thread'\nexport KODEX_VERSION='old-version'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s|%s' \"$KODEX_THREAD_ID\" \"$KODEX_VERSION\"".to_string(),
    ];
    let env = HashMap::from([
        ("KODEX_THREAD_ID".to_string(), "nested-thread".to_string()),
        (
...
fn maybe_wrap_shell_lc_with_snapshot_restores_permission_profile_from_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport KODEX_PERMISSION_PROFILE='parent-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv KODEX_PERMISSION_PROFILE".to_string(),
    ];
    let env = HashMap::from([(
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(KODEX_PERMISSION_PROFILE_ENV_VAR, "current-profile")
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_unsets_absent_permission_profile() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport KODEX_PERMISSION_PROFILE='stale-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(KODEX_PERMISSION_PROFILE_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_apply_patch_rollout_state() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS='stale'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS".to_string(),
    ];
    let env = HashMap::from([(
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR, "1")
        .output()
        .expect("run rewritten command");
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_reserved_metrics_output_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport KODEX_PLUGIN_METRICS_OUTPUT='/stale/path'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"${KODEX_PLUGIN_METRICS_OUTPUT-unset}\"".to_string(),
    ];
...
async fn snapshot_wrapper_replays_dummy_and_preserves_unbrokered_credentials() -> anyhow::Result<()>
{
    ...
    let injection = "\"; printf KODEX_ENV_INJECTED; __kodex_env_file=\"";
    ...
    if std::path::Path::new("/bin/zsh").exists() {
        ...
        for (mut startup_env, expected_canonical, expected_alias) in [
            ...
            assert!(
                !exported_env
                    .split('\0')
                    .any(|entry| entry.starts_with("__KODEX_SNAPSHOT_PROXY_OVERRIDE_"))
            );
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_kodex_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{KODEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{KODEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_kodex_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{KODEX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("kodex-path");
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport KODEX_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${KODEX_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$KODEX_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "KODEX_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("KODEX_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
