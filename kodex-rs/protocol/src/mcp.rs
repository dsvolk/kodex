...
//! `schemars`) so they can be embedded in Kodex's own protocol structures.
...
/// One source and the first runtime turn in which Kodex recorded it.
///
...
/// MCP extensions supplied by the client that created a Kodex session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `kodex-protocol` so other crates can convert
