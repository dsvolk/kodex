...
use kodex_login::GatewayAuthConfig;
...
pub fn seed_gateway_auth(
    ...
    // Repeated fixtures sharing a Kodex home must use the same encryption key.
    ...
    digest.update(runtime.kodex_home.to_string_lossy().as_bytes());
    digest.update([0]);
    for value in [
