...
use kodex_otel::ToolDecisionSource;
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let sandbox_manager = sandbox_manager.with_allowed_symlinked_kodex_home(
            environment
                .environment
                .local_runtime_paths()
                .and_then(|paths| paths.allowed_symlinked_kodex_home.clone()),
        );
        ...
        let windows_sandbox_type = kodex_protocol::sandbox::effective_windows_sandbox_type(
        ...
        let kodex_sandbox_exe = if cfg!(windows) {
            turn_ctx.config.kodex_self_exe.as_ref()
        } else {
            turn_ctx.config.kodex_linux_sandbox_exe.as_ref()
        };
        ...
        match first_result {
            ...
            Err(ToolError::Kodex(err)) => {
                let KodexErrorDetails::Sandbox(SandboxErr::Denied {
                    output,
                    network_policy_decision,
                }) = err.details()
                else {
                    let err = ToolError::Kodex(err);
                ...
                if network_policy_decision.is_some() && network_approval_context.is_none() {
                    ...
                    return Err(ToolError::Kodex(err));
                }
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::Kodex(err) => match err.details() {
            KodexErrorDetails::Sandbox(SandboxErr::Denied { .. }) => Some("denied"),
            KodexErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => Some("timed_out"),
            KodexErrorDetails::Sandbox(SandboxErr::Signal(_)) => Some("signal"),
            _ => None,
        },
