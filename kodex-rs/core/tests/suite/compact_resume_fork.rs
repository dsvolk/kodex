...
//! request payload that Kodex would send to the model and assert that the
...
fn seed_first_checkpoint_harness_metadata(path: &Path, retained_text: &str) -> Result<()> {
    ...
    for envelope in replacement_history {
        envelope.metadata = Some(KodexHarnessMetadata::default());
    }
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<KodexThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.kodex)
}

async fn user_turn(conversation: &Arc<KodexThread>, text: &str) {
    conversation
...
async fn compact_conversation(conversation: &Arc<KodexThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<KodexThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<KodexThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = kodex_core::test_support::auth_manager_from_auth(
