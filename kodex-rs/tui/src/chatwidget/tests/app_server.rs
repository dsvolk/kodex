...
fn thread_settings_for_test(
    model: &str,
    thread_id: ThreadId,
) -> kodex_app_server_protocol::ThreadSettingsUpdatedNotification {
    kodex_app_server_protocol::ThreadSettingsUpdatedNotification {
        thread_id: thread_id.to_string(),
        thread_settings: kodex_app_server_protocol::ThreadSettings {
...
async fn session_header_uses_catalog_display_name_without_changing_model() {
    ...
    for (name, first_event, display_name) in [
        ...
        let rendered = drain_insert_history_with(&mut events, HistoryCell::raw_lines)
            ...
            .collect::<String>()
            .replace(KODEX_CLI_VERSION, "<VERSION>")
...
async fn invalid_url_elicitation_is_declined() {
    ...
    chat.handle_elicitation_request_now(
        kodex_app_server_protocol::RequestId::Integer(9),
        kodex_app_server_protocol::McpServerElicitationRequestParams {
            thread_id: request_thread_id.to_string(),
            turn_id: Some("turn-auth".to_string()),
            server_name: "payments".to_string(),
            request: kodex_app_server_protocol::McpServerElicitationRequest::Url {
                meta: None,
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: kodex_app_server_protocol::RequestId::Integer(9),
...
async fn live_app_server_turn_completed_clears_working_status_after_answer_item() {
    ...
    chat.handle_server_notification(
        ServerNotification::TurnCompleted(TurnCompletedNotification {
            ...
            turn: AppServerTurn {
                id: "turn-1".to_string(),
                items_view: kodex_app_server_protocol::TurnItemsView::Summary,
                items: vec![item],
...
async fn live_app_server_strict_review_required_notification_renders_message() {
    ...
    chat.handle_server_notification(
        ServerNotification::StrictReviewRequired(
            kodex_app_server_protocol::StrictReviewRequiredNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                started_at_ms: 1_000,
            },
...
async fn startup_config_warning_is_not_repeated_by_thread() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let message = "Kodex is ignoring 1 unrecognized configuration setting. Check for typos or deprecated settings.";
    ...
    ⚠ Kodex is ignoring 1 unrecognized configuration setting. Check for typos or
      deprecated settings.
    ");
}
...
async fn live_app_server_sub_agent_activity_renders_once() {
    ...
    let activity = AppServerThreadItem::SubAgentActivity {
        ...
        kind: kodex_app_server_protocol::SubAgentActivityKind::Completed,
...
async fn live_app_server_turn_completion_repairs_dropped_message_deltas() {
    ...
    completed_turn.items_view = kodex_app_server_protocol::TurnItemsView::Summary;
    completed_turn.items = vec![AppServerThreadItem::AgentMessage {
        ...
        memory_citation: None,
        delivery: None,
        questions: Some(vec![kodex_protocol::items::AsyncUserInputQuestion {
...
async fn live_app_server_rate_limit_error_renders_upstream_message() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let error = KodexErr::from(KodexErrorDetails::RateLimitExceeded(
    ...
    handle_error(
        &mut chat,
        error.to_string(),
        Some(error.to_kodex_protocol_error().into()),
    );
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                kodex_error_info: Some(KodexErrorInfo::CyberPolicy),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
...
async fn app_server_safety_access_errors_render_dedicated_notice() {
    ...
    for (case, message) in cases {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
        chat.handle_non_retry_error(message, /*kodex_error_info*/ None);
