...
use kodex_config::types::Personality;
...
fn read_only_user_turn(test: &TestKodex, items: Vec<UserInput>, model: String) -> TurnInputRequest {
...
async fn first_turn_model_change_appends_model_instructions_developer_message(
    ...
    let mut builder =
        test_kodex().with_model_info_override("gpt-5.6-terra", configure_model_switching_fixture);
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_rollout_path = test.kodex.rollout_path().expect("rollout path");
    let source_history =
        kodex_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
-> Result<()> {
    ...
    let mut builder = test_kodex().with_model("gpt-5.5");
    let test = builder.build(&server).await?;
    let next_model = "exp-kodex-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_kodex().build(&server).await?;
...
async fn model_change_projects_media_without_changing_live_or_replayed_history(
    ...
    let image_url = kodex_utils_image::data_url_from_bytes("image/png", &png.into_inner());
    ...
    let thread = match source {
        MediaHistorySource::Live => Arc::clone(&test.kodex),
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
