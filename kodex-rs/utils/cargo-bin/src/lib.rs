...
/// site.)
///
/// This is expected to be used exclusively in test code because Kodex CLI is a
...
pub fn repo_root() -> io::Result<PathBuf> {
    let marker = if runfiles_available() {
        ...
        let marker_path = option_env!("KODEX_REPO_ROOT_MARKER")
            .map(PathBuf::from)
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::NotFound,
                    "KODEX_REPO_ROOT_MARKER was not set at compile time",
                )
            })?;
