...
use kodex_login::auth::AgentIdentityAuth;
use kodex_login::auth::AgentIdentityAuthError;
...
struct AuthManagerAuthProvider {
    ...
    expected_auth: KodexAuth,
}

impl AuthManagerAuthProvider {
    fn is_expected_auth(&self, auth: &KodexAuth) -> bool {
        auth.uses_kodex_backend()
    ...
    fn current_auth(&self) -> Option<KodexAuth> {
        self.auth_manager
            .auth_cached()
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> kodex_protocol::error::Result<SharedAuthProvider> {
    ...
    if matches!(
        auth,
        Some(KodexAuth::BedrockApiKey(_) | KodexAuth::BedrockAccessKeys(_))
    ) {
...
) -> kodex_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(KodexAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(KodexAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> kodex_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Kodex auth snapshot.
pub fn auth_provider_from_auth(auth: &KodexAuth) -> SharedAuthProvider {
    match auth {
        KodexAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        KodexAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        ...
        | KodexAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
mod tests {
    ...
    use kodex_login::auth::AgentIdentityAuthRecord;
    use kodex_login::auth::BedrockApiKeyAuth;
    use kodex_login::auth::login_with_chatgpt_auth_tokens;
    ...
    fn test_kodex_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "kodex-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp kodex home");
        path
    }

    fn write_chatgpt_auth_json(kodex_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    ) -> (PathBuf, Arc<AuthManager>, KodexAuth) {
        let kodex_home = test_kodex_home();
        write_chatgpt_auth_json(&kodex_home);
    ...
    fn custom_provider_does_not_inherit_ambient_auth_headers() {
        ...
        let ambient_auth = KodexAuth::Headers(AuthHeaders::new(ambient_headers));
    ...
    fn custom_provider_does_not_inherit_ambient_bedrock_auth() {
        let provider =
            create_oss_provider_with_base_url("http://localhost:11434/v1", WireApi::Responses);
        let ambient_auth = KodexAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn custom_provider_uses_command_resolved_auth() {
        ...
        let command_auth = KodexAuth::from_api_key("command-token");
    ...
    fn openai_provider_preserves_ambient_auth_headers() {
        ...
        let ambient_auth = KodexAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = KodexAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = KodexAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&KodexAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_kodex_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
