#!/usr/bin/env python3
"""Black-box MCP compliance runner for a supplied Kodex binary."""
...
class AppServerClient:
    """Minimal JSONL client for the Kodex app-server protocol."""
    ...
    def __init__(
        ...
        *,
        ...
        self._stdout_thread = threading.Thread(
            ...
            name="kodex-app-server-stdout",
            daemon=True,
        )
        self._stderr_thread = threading.Thread(
            target=self._read_stderr,
            name="kodex-app-server-stderr",
            daemon=True,
        )
    ...
    def _send(self, message: Mapping[str, object]) -> None:
        if self.process.poll() is not None:
            raise AppServerError(
                f"Kodex app-server exited with code {self.process.returncode}"
            )
    ...
    def request(
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Kodex app-server closed stdout while handling {method}"
                )
    ...
    def wait_for_notification(
        ...
        *,
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Kodex app-server closed stdout while waiting for {method}"
                )
            if "id" in message and isinstance(message.get("method"), str):
                self._handle_server_request(message)
                continue
...
def _isolated_environment(kodex_home: Path) -> dict[str, str]:
    env = dict(os.environ)
    env["KODEX_HOME"] = str(kodex_home)
    # Direct app-server MCP calls do not need a model or credentials.
    for name in ("KODEX_API_KEY", "KODEX_ACCESS_TOKEN", "OPENAI_API_KEY"):
        env.pop(name, None)
    return env
...
def _registration_command(
    ...
    *,
    ...
    command = [str(kodex_binary), "mcp", "add", TEST_SERVER_NAME]
    if transport == "stdio":
        if mode == MODERN_VERSION:
            command.extend(["--env", f"KODEX_MCP_PROTOCOL_VERSION={MODERN_VERSION}"])
        return [
            *command,
            "--",
            sys.executable,
...
def _validate_registration(
    ...
    *,
    ...
    if transport == "stdio":
        ...
        modern_opt_in = (
            isinstance(env, dict)
            and env.get("KODEX_MCP_PROTOCOL_VERSION") == MODERN_VERSION
        )
...
def _exercise_app_server(
    ...
    *,
    ...
    sentinel = f"kodex-mcp-{transport}-{mode}"
...
def _run_case(
    ...
    *,
    ...
    try:
        ...
        if registered:
            remove = _run_command(
                [str(kodex_binary), "mcp", "remove", TEST_SERVER_NAME],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
            ...
            listed = _run_command(
                [str(kodex_binary), "mcp", "list", "--json"],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
...
def _run_official_case(
    ...
    *,
    ...
    case_home.mkdir(parents=True)
    env = _isolated_environment(case_home)
    env["KODEX_CONFORMANCE_TIMEOUT"] = str(timeout_seconds)
    env["KODEX_CONFORMANCE_ENABLE_MODERN_FEATURE"] = (
        "1" if enable_modern_feature else "0"
    )
    env["KODEX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = (
    ...
    for result in results:
        ...
        case.checks.append(
            CheckResult(
                name=f"harness/{result.scenario}/kodex-adapter",
...
def _cimd_registration_checks(
    ...
    *,
    ...
    for check_name, client_registration in (
        ...
        env["KODEX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = "1"
        if client_registration is not None:
            env["KODEX_CONFORMANCE_CLIENT_REGISTRATION"] = client_registration
        results = run_official_mode(
            conformance_command=conformance_command,
            adapter_script=adapter_script,
            kodex_binary=kodex_binary,
...
def run_compliance(
    ...
    *,
    ...
    report: dict[str, object] = {
        ...
        "kodexBinary": str(kodex_binary),
        "kodexVersion": version.stdout or None,
...
def _required_regression_checks(
    ...
    *,
    ...
    if not isinstance(version_check, dict) or version_check.get("success") is not True:
        errors.append(f"{label} does not contain a successful Kodex version check")
...
def _print_human_report(report: Mapping[str, object]) -> None:
    ...
    print(f"Kodex MCP compliance: {'PASS' if success else 'FAIL'}")
    print(f"Binary: {report.get('kodexBinary')}")
    print(f"Version: {report.get('kodexVersion') or 'unknown'}")
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "supplied Kodex binary, plus the supplemental local stdio suite."
        )
    )
    parser.add_argument("kodex_binary", type=Path)
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--adapter-script",
        type=Path,
        default=Path(__file__).resolve().with_name("kodex_conformance_adapter.py"),
        help="Path to the Kodex adapter used by the official client suite.",
    )
    parser.add_argument(
        "--conformance-cli",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require Kodex to recover from OAuth scope escalation and "
    ...
    parser.add_argument(
        "--artifact-parent",
        type=Path,
        help="Parent directory for isolated temporary Kodex homes.",
    )
    parser.add_argument(
        "--keep-artifacts",
        action="store_true",
        help="Keep isolated Kodex homes and include their path in the report.",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    kodex_binary = args.kodex_binary.expanduser().resolve()
    ...
    if not kodex_binary.is_file() or not os.access(kodex_binary, os.X_OK):
        print(f"error: Kodex binary is not executable: {kodex_binary}", file=sys.stderr)
        return 2
