...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(kodex_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
async fn file_storage_loads_empty_agent_identity_email_as_none() -> anyhow::Result<()> {
    let kodex_home = tempdir()?;
    let storage = FileAuthStorage::new(kodex_home.path().to_path_buf());
    let auth_file = get_auth_file(kodex_home.path());
    std::fs::write(
        &auth_file,
...
fn encrypted_auth_file(kodex_home: &Path) -> PathBuf {
    kodex_home.join("secrets").join("kodex_auth.age")
}
...
fn secrets_keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_secrets_backend_with_auth(&mock_keyring, kodex_home.path(), &expected)?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let kodex_home = PathBuf::from("~/.kodex");

    let key = compute_store_key(kodex_home.as_path())?;
...
fn direct_keyring_auth_storage_delete_removes_keyring_and_file() -> anyhow::Result<()> {
    ...
    assert!(
        mock_keyring
            .saved_value(&compute_store_key(kodex_home.path())?)
            .is_none(),
...
fn secrets_keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, kodex_home.path(), &auth)?;
    Ok(())
}
...
fn auto_auth_storage_load_prefers_keyring_value() -> anyhow::Result<()> {
    ...
    seed_secrets_backend_with_auth(&mock_keyring, kodex_home.path(), &keyring_auth)?;
...
fn auto_auth_storage_load_falls_back_when_keyring_errors() -> anyhow::Result<()> {
    ...
    let key = compute_keyring_account(kodex_home.path(), LocalSecretsNamespace::KodexAuth);

    let encrypted = auth_with_prefix("encrypted");
    seed_secrets_backend_with_auth(&mock_keyring, kodex_home.path(), &encrypted)?;
...
fn auto_auth_storage_save_prefers_keyring() -> anyhow::Result<()> {
    ...
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, kodex_home.path(), &expected)?;
    Ok(())
}
