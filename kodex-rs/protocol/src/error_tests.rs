...
#[test]
fn kodex_err_debug_preserves_legacy_shape() {
    let actual = [
        KodexErr::Timeout,
        KodexErr::Stream("disconnected".to_string()),
        KodexErr::Stream("retry later".to_string()).with_retry_delay(Duration::from_secs(2)),
        KodexErr::InternalServerError.with_retry_delay(Duration::from_secs(3)),
    ]
    .map(|err| format!("{err:?}"));
...
fn retryability_preserves_error_details_distinctions() {
    let errors = [
        (KodexErr::ServerOverloaded, false),
        (
            KodexErr::new(KodexErrorDetails::RateLimitExceeded("retry later".into())),
            true,
        ),
        (
        ...
        (
            KodexErrorDetails::ToolCollision("functions.update_plan".to_string()).into(),
            false,
        ),
        (KodexErr::InternalServerError, true),
    ];
...
fn retry_delay_distinguishes_server_advice_backoff_and_terminal_errors() {
    let error = KodexErr::InternalServerError;
    ...
    let error = KodexErr::QuotaExceeded.with_retry_delay(advice);
    assert_eq!(
        (
            error.retry_delay(/*retry_count*/ 1),
            error.server_retry_delay(),
...
fn usage_limit_reached_error_formats_plus_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You’ve hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/kodex/settings/usage to purchase more credits or try again later."
    );
}
...
fn server_overloaded_maps_to_protocol() {
    let err = KodexErr::ServerOverloaded;
    assert_eq!(
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = KodexErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    *source.url_mut().unwrap() = "http://example.com".parse().unwrap();
    let err = KodexErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.kodex_error_info,
        Some(KodexErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You’ve hit your usage limit. Upgrade to Plus to continue using Kodex (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
fn usage_limit_reached_error_formats_pro_plan_with_reset() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You’ve hit your usage limit. Visit https://chatgpt.com/kodex/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_kodex_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        let err = UsageLimitReachedError {
            ...
            promo_message: Some(
                "Visit https://chatgpt.com/kodex/settings/usage to purchase more credits"
                    .to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You’ve hit your usage limit for kodex_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn unexpected_status_prefers_error_message_when_present() {
    let err = UnexpectedResponseError {
        ...
        body: r#"{"error":{"message":"Workspace is not authorized in this region."},"status":401}"#
            .to_string(),
        user_message: None,
        url: Some("https://chatgpt.com/backend-api/kodex/responses".to_string()),
        cf_ray: None,
        request_id: Some("req-123".to_string()),
        identity_authorization_error: None,
        identity_error_code: None,
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: Workspace is not authorized in this region., url: https://chatgpt.com/backend-api/kodex/responses, request id: req-123"
        )
    );
}
...
fn unexpected_status_includes_cf_ray_and_request_id() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/kodex/responses, cf-ray: 9c81f9f18f2fa49d-LHR, request id: req-xyz"
        )
    );
}
...
fn unexpected_status_includes_identity_auth_details() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/kodex/models, cf-ray: cf-ray-auth-401-test, request id: req-auth, auth error: missing_authorization_header, auth error code: token_expired"
        )
    );
}
...
fn usage_limit_reached_includes_hours_and_minutes() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You’ve hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/kodex/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Kodex, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You’ve hit your usage limit. To continue using Kodex, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
