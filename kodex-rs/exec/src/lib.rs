...
pub use event_processor_with_jsonl_output::KodexStatus;
...
use kodex_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use kodex_core::config::ConfigTomlLoadResult;
...
use kodex_core::config::resolve_oss_provider;
use kodex_core::config::resolve_profile_v2_config_path;
use kodex_core::find_thread_meta_by_name_str;
use kodex_core::format_exec_policy_error_with_source;
use kodex_core::path_utils;
use kodex_core::read_session_meta_line;
...
use kodex_login::default_client::set_default_originator;
...
use kodex_otel::set_parent_from_context;
use kodex_otel::traceparent_context_from_env;
...
enum StdinPromptBehavior {
    ...
    /// `kodex exec` behavior for `kodex exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `kodex exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<kodex_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("kodex_exec".to_string()) {
        tracing::warn!(?err, "Failed to set kodex exec originator override {err:?}");
    }
    ...
    if worktree {
        ...
        match command.as_ref() {
            Some(ExecCommand::Resume(_)) => {
                anyhow::bail!("--worktree is not supported with `kodex exec resume`");
            }
            Some(ExecCommand::Review(_)) => {
                anyhow::bail!("--worktree is not supported with `kodex exec review`");
            }
    ...
    #[allow(clippy::print_stderr)]
    let kodex_home = match find_kodex_home() {
        Ok(kodex_home) => kodex_home,
        Err(err) => {
            eprintln!("Error finding kodex home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&kodex_home, profile_v2));
    ...
    let managed_worktree = if worktree {
        ...
        .await;
        let settings =
            WorktreeSettings::for_cli(&kodex_home, host_config.config_toml.desktop.as_ref())?;
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        kodex_core::otel_init::build_provider(
            &config,
    ...
    kodex_core::otel_init::record_process_start(otel.as_ref(), "kodex_exec");
    kodex_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "kodex_exec");
    ...
    #[cfg(target_os = "macos")]
    let local_runtime_paths = local_runtime_paths.with_allowed_symlinked_kodex_home(
        kodex_config::allowed_symlinked_kodex_home(&config.config_layer_stack, &config.kodex_home),
    );
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "kodex_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = kodex_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Kodex
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            kodex_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Kodex initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        kodex_app_server_protocol::TurnStatus::Failed
                            | kodex_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<kodex_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<kodex_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != kodex_app_server_protocol::TurnItemsView::Full
}
...
async fn parse_latest_turn_context_cwd(path: &Path) -> Option<PathBuf> {
    ...
    tokio::task::spawn_blocking(move || {
        let reader = kodex_rollout::open_rollout_seekable_reader(&path).ok()?;
        let mut scanner = kodex_rollout::ReverseJsonlScanner::new(reader).ok()?;
        while let Some(outcome) = scanner.scan_next_rollout_line().ok()? {
            if let kodex_rollout::ScanOutcome::Parsed(RolloutLine {
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.kodex_home, session_id, Some(state_db.as_ref()))
                .await?
