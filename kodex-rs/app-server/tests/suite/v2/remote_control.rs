...
use kodex_app_server::RemoteControlStartupMode;
...
use kodex_app_server_protocol::RemoteControlClient;
use kodex_app_server_protocol::RemoteControlClientsListOrder;
use kodex_app_server_protocol::RemoteControlClientsListParams;
use kodex_app_server_protocol::RemoteControlClientsListResponse;
use kodex_app_server_protocol::RemoteControlClientsRevokeParams;
use kodex_app_server_protocol::RemoteControlClientsRevokeResponse;
use kodex_app_server_protocol::RemoteControlConnectionStatus;
use kodex_app_server_protocol::RemoteControlDisableResponse;
use kodex_app_server_protocol::RemoteControlEnableResponse;
...
use kodex_app_server_protocol::RemoteControlPairingStatusResponse;
use kodex_app_server_protocol::RemoteControlStatusChangedNotification;
use kodex_app_server_protocol::RemoteControlStatusReadResponse;
...
use kodex_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = kodex_home.path().join("managed_config.toml");
    let socket_path = kodex_home
        .path()
    ...
    let result = timeout(
        ...
        run_main_with_transport_options(
            Arg0DispatchPaths {
                kodex_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let listener = configured_remote_control_listener(kodex_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(kodex_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(kodex_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(kodex_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(kodex_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(kodex_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(kodex_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(kodex_home).await?;
...
async fn configured_remote_control_listener(kodex_home: &std::path::Path) -> Result<TcpListener> {
    let listener = TcpListener::bind("127.0.0.1:0").await?;
    let remote_control_url = format!("http://{}/backend-api/", listener.local_addr()?);
    let catalog_path = kodex_home.join("models.json");
    std::fs::write(
        &catalog_path,
        serde_json::to_vec(&kodex_models_manager::bundled_models_response()?)?,
    )?;
    MockResponsesConfig::new(&remote_control_url)
        ...
        .disable_feature(kodex_features::Feature::Plugins)
        .write(kodex_home)?;
    write_chatgpt_auth(
        kodex_home,
