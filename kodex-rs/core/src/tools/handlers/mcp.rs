...
use kodex_tools::ToolSearchSourceInfo;
use kodex_tools::ToolSpec;
use kodex_tools::agent_plugin_mcp_tool_to_responses_api_tool;
use kodex_tools::mcp_tool_to_responses_api_tool;
use kodex_utils_image::PromptImageMode;
...
pub struct McpHandler {
    tool_info: ToolInfo,
    spec: Arc<ToolSpec>,
    code_mode_tool_definitions: OnceLock<Vec<kodex_code_mode::ToolDefinition>>,
}
...
impl McpHandler {
    async fn handle_call(
        ...
        let result_metadata_capture_allowed = invocation
            ...
            && prepared_mcp_call
                .as_ref()
                .is_some_and(kodex_mcp::PreparedMcpCall::is_host_owned_apps);
        ...
        let truncation_policy = prepared_mcp_call
            .as_ref()
            .and_then(kodex_mcp::PreparedMcpCall::output_token_limit)
...
impl CoreToolRuntime for McpHandler {
    ...
    fn cached_code_mode_definitions(&self) -> Option<&[kodex_code_mode::ToolDefinition]> {
        Some(
            self.code_mode_tool_definitions
                .get_or_init(|| {
                    let mut definitions = kodex_tools::collect_code_mode_tool_definitions(
    ...
    fn on_tool_result_accepted(&self, invocation: &ToolInvocation, result: &dyn ToolOutput) {
        ...
        let is_encrypted = |item: &Value| {
            item.get("_meta")
                .and_then(|meta| meta.get("kodex/encryptedContent"))
            ...
            .filter_map(|item| {
                ...
                match item.get("type").and_then(Value::as_str) {
                            ...
                            || image_capture_enabled =>
                    {
                        ...
                        let detail = item
                            .get("_meta")
                            .and_then(|meta| meta.get("kodex/imageDetail"))
                        ...
                        Some(UserInput::Image {
                            image: kodex_protocol::models::ImageReference::Inline { image_url },
                            detail,
                        })
                    }
                    _ => None,
...
mod tests {
    ...
    async fn mcp_pre_tool_use_payload_uses_prefixed_tool_name_and_raw_args() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: kodex_tools::ToolName::namespaced("memory", "create_entities"),
    ...
    async fn mcp_pre_tool_use_payload_keeps_builtin_like_tool_names_namespaced() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: kodex_tools::ToolName::namespaced("mcp__foo", "exec_command"),
                source: ToolCallSource::Direct,
                payload,
            }),
            Some(PreToolUsePayload {
    ...
    async fn mcp_post_tool_use_payload_uses_prefixed_tool_name_args_and_result() {
        ...
        let output = McpToolOutput {
            ...
            truncation_policy: kodex_utils_output_truncation::TruncationPolicy::Bytes(1024),
        };
        let (session, turn) = make_session_and_context().await;
        ...
        let recorder = crate::tools::executed_tool_calls::ExecutedToolCalls::new(
            ...
            &kodex_history::InitialHistory::New,
        );
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: kodex_tools::ToolName::namespaced("filesystem", "read_file"),
            source: ToolCallSource::Direct,
            payload,
        };
        assert_eq!(
        ...
        let cell_id = kodex_code_mode::CellId::new("mcp-cell".to_string());
        ...
        assert_eq!(
            ...
            json!([{
                "name": kodex_tools::code_mode_name_for_tool_name(&invocation.tool_name),
