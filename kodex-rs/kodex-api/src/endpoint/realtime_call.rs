...
const MULTIPART_BOUNDARY: &str = "kodex-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=kodex-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "kodex_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/kodex"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/kodex/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
    ...
    async fn sends_backend_session_call_as_json_body() {
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/kodex/realtime/calls?intent=quicksilver&architecture=avas"
        );
