...
const KODEX_AUTH_SECRETS_FILENAME: &str = "kodex_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Kodex authentication credentials used by the CLI, TUI, app server, and other clients.
    KodexAuth,
...
impl LocalSecretsBackend {
    pub fn new(kodex_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn decrypted_cache(&self) -> Option<&Mutex<Option<CachedSecrets>>> {
        match self.namespace {
            ...
            LocalSecretsNamespace::ManagedSecrets | LocalSecretsNamespace::KodexAuth => None,
        }
    }
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.kodex_home, self.namespace);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(kodex_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        ...
        let keyring = Arc::new(MockKeyringStore::default());
        let account =
            compute_keyring_account(kodex_home.path(), LocalSecretsNamespace::ManagedSecrets);
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        ...
        let kodex_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let kodex_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        kodex_auth_backend.set(&scope, &name, "kodex-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;
        gateway_backend.set(&scope, &name, "gateway-value")?;

        assert_eq!(
            kodex_auth_backend.get(&scope, &name)?,
            Some("kodex-auth-value".to_string())
        );
        assert!(
            MCP_OAUTH_CACHE
                .lock()
                .unwrap_or_else(PoisonError::into_inner)
                .as_ref()
                .is_none_or(|cached| cached.path != kodex_auth_backend.secrets_path())
        );
        assert_eq!(
        ...
        assert!(!kodex_home.path().join("secrets").join("local.age").exists());
        assert!(
            kodex_home
                .path()
        ...
        keyring
            .delete(
                keyring_service(),
                &compute_keyring_account(kodex_home.path(), LocalSecretsNamespace::KodexAuth),
            )
