...
`kodex-http-client` is the low-level HTTP transport shared by Kodex crates. It is the intended
...
- custom CA handling through `KODEX_CA_CERTIFICATE` and `SSL_CERT_FILE`;
...
feature requires more than enabling reqwest's default proxy behavior: Kodex must resolve platform
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `kodex-client`.
...
would skip Kodex's route selection for the redirect target. In `RespectSystemProxy` mode the pool
...
If the existing wrapper surface cannot support a use case, extend `kodex-http-client` rather than
