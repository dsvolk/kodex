...
This repository uses Bazel to build the Rust workspace under `kodex-rs`.
...
- `rules_rs` imports third-party crates from `kodex-rs/Cargo.toml` and
  `kodex-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `kodex_rust_crate`, which wraps `rust_library`,
...
- Each crate in `kodex-rs/*/BUILD.bazel` typically uses `kodex_rust_crate` and
...
Kodex uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/kodex` | Yes | Yes |
| `workflow_dispatch` in `openai/kodex` | Yes | Yes |
| Same-repository pull request in `openai/kodex` | Yes | Yes |
| Fork pull request into `openai/kodex` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/kodex \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //kodex-rs/cli:kodex
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `openai/kodex`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `kodex_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `kodex_rust_crate` to configure it.
