...
use kodex_http_client::DestinationPolicy::Restricted;
use kodex_http_client::NetworkPolicyDenied::Destination;
use kodex_http_client::RouteAwareRequestError::Policy;
...
fn pretrust_startup_sync_uses_installed_git_with_hostile_path() {
    const CHILD_HOME: &str = "KODEX_TEST_CURATED_SYNC_HOME";
        ...
        for plugin in plugins {
            assert!(
                curated_plugins_repo_path(&home)
                    .join(format!("plugins/{plugin}/.kodex-plugin/plugin.json"))
                    .is_file()
            );
        }
    }
...
async fn backup_archive_routes_metadata_and_backend_supplied_download_urls() {
    ...
    let controller = kodex_http_client::NetworkPolicyController::default();
...
fn git_command_sanitizes_ambient_repository_environment() {
    ...
    assert_eq!(
        ...
        [
            OsStr::new("-c"),
            OsStr::new(kodex_git_utils::SAFE_BARE_REPOSITORY_CONFIG),
        ]
    );
...
fn pretrust_git_sync_ignores_repository_local_transport_config() {
    ...
    let kodex_home = fixture.path().join("kodex-home");
    ...
    let err = sync_openai_plugins_repo_via_git(&kodex_home, &git_wrapper)
...
async fn ordinary_clone_rejects_tracked_embedded_bare_repository() {
    ...
    run_setup_git(
        ...
        &[
            ...
            "-c",
            "user.email=kodex-tests@example.com",
            "commit",
            "--quiet",
            "-m",
    ...
    let apply_error = kodex_git_utils::apply_git_patch(&kodex_git_utils::ApplyGitRequest {
        cwd: nested.clone(),
        diff: String::new(),
        revert: false,
        preflight: true,
    ...
    assert!(kodex_git_utils::collect_git_info(&nested).await.is_none());
    assert!(kodex_git_utils::git_diff_to_remote(&nested).await.is_none());
    assert!(
        !marker.exists(),
    ...
    let explicit_environment = Command::new("git")
        .args(["-c", kodex_git_utils::SAFE_BARE_REPOSITORY_CONFIG])
        .args(["rev-parse", "--git-dir"])
        .env("GIT_DIR", &nested)
        .current_dir(&clone)
        .output()
...
fn write_curated_plugin_sha(kodex_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(kodex_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(kodex_home.join(".tmp")) else {
        return false;
    };
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.kodex-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_kodex_home_tmp_dir() {
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.kodex-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.kodex-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.kodex-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.kodex-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.kodex-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.kodex-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.kodex-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.kodex-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");
