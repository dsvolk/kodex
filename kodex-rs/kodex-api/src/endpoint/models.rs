...
impl<T: HttpTransport> ModelsClient<T> {
    ...
    fn append_client_version_query(req: &mut kodex_client::Request, client_version: &str) {
...
mod tests {
    ...
    fn catalog_request_url_preserves_routing_and_encodes_queries() {
        ...
        let url = ModelsClient::<CapturingTransport>::catalog_request_url(
            &provider,
            "https://catalog.example/kodex/models?deployment=one",
            "1.2.3",
        )
        .unwrap();
        assert_eq!(
            url,
            "https://catalog.example/kodex/models?deployment=one&api-version=2026+09&client_version=1.2.3"
        );
    }
    ...
    fn catalog_request_url_rejects_relative_urls() {
        ...
        for catalog_url in ["/kodex/models", "kodex/models"] {
    ...
    async fn response_body_limit_survives_auth_retry_without_limiting_other_models_requests() {
        ...
        let mut provider = provider("https://example.com/api/kodex");
    ...
    async fn appends_client_version_query() {
        ...
        let provider = provider("https://example.com/api/kodex");
        let request_url = ModelsClient::<CapturingTransport>::request_url(&provider, "0.99.0");
        ...
        assert_eq!(
            url,
            "https://example.com/api/kodex/models?client_version=0.99.0"
        );
    }
