...
pub fn sandbox_setup_is_complete(kodex_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(kodex_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    if crate::registered_core_requested()
        && !crate::app_package::registered_setup_is_ready(kodex_home).unwrap_or(false)
    {
        return false;
    }
    matches!(load_users(kodex_home), Ok(Some(users)) if users.version_matches())
}
...
pub fn sandbox_setup_is_complete_with_settings(
    ...
    let Ok(Some(mut marker)) = load_marker(kodex_home) else {
        return false;
    };
...
fn load_marker(kodex_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(kodex_home);
...
fn load_users(kodex_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(kodex_home);
...
fn remove_sandbox_users_file(kodex_home: &Path, reason: &str) -> Result<()> {
    let path = sandbox_users_path(kodex_home);
    debug_log(
...
pub fn logon_existing_sandbox_account(
    ...
    let marker: SetupMarker = serde_json::from_slice(
        &fs::read(setup_marker_path(kodex_home)).context("read sandbox setup marker")?,
    )
    ...
    let users: SandboxUsersFile = serde_json::from_slice(
        &fs::read(sandbox_users_path(kodex_home)).context("read sandbox accounts")?,
    )
...
fn select_identity(
    ...
    let _marker = match load_marker(kodex_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(kodex_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let needed_read = read_roots_override
        .map(<[PathBuf]>::to_vec)
        .unwrap_or_else(|| {
            gather_read_roots(command_cwd, permissions, env_map, kodex_home, runtime)
        });
    let needed_write = write_roots_override
        .map(<[PathBuf]>::to_vec)
        .unwrap_or_else(|| gather_write_roots_for_permissions(permissions, command_cwd, env_map));
    // Do not grant the capability token write access to KODEX_HOME/.sandbox; the setup helper
...
fn require_sandbox_account_with_setup(
    ...
    let mut identity = match marker {
        Some(marker) if marker.version_matches() => {
                ...
                marker.request_mismatch_reason(network_identity, &desired_offline_proxy_settings)
            {
                ...
                let selected = select_identity(network_identity, kodex_home)?;
                if selected.is_none() {
                    setup_reason = Some(
    ...
    if identity.is_some() {
        ...
        for username in [OFFLINE_USERNAME, ONLINE_USERNAME] {
            ...
            if needs_repair {
                ...
                remove_sandbox_users_file(kodex_home, reason)?;
    ...
    if identity.is_none() {
        ...
        identity = select_identity(network_identity, kodex_home)?;
    }
...
pub(crate) fn refresh_logon_sandbox_creds(
    ...
    remove_sandbox_users_file(kodex_home, "sandbox user login failed")?;
...
mod tests {
    ...
    fn remove_sandbox_users_file_deletes_existing_file() {
        ...
        let users_path = sandbox_users_path(kodex_home.path());
        ...
        remove_sandbox_users_file(kodex_home.path(), "stale creds").expect("remove users");
