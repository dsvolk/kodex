...
#[test]
fn kodex_apps_mcp_protocol_can_be_enabled_independently_of_generic_mcp() {
    let features_toml = FeaturesToml::from(BTreeMap::from([
        (Feature::KodexAppsMcp20260728.key().to_string(), true),
    ...
    assert_eq!(
        (
            features.enabled(Feature::KodexAppsMcp20260728),
...
"#,
    ...
    assert_eq!(
        ...
        Some(FeatureToml::Config(crate::GuardianV2ConfigToml {
            ...
            reasoning_effort: Some(kodex_protocol::openai_models::ReasoningEffort::Minimal),
...
#[test]
fn kodex_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::KodexHooks));
    assert_eq!(feature_for_key("kodex_hooks"), Some(Feature::KodexHooks));
}
