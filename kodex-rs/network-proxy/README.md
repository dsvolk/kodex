...
`kodex-network-proxy` is Kodex's local network policy enforcement proxy. It runs:
...
`kodex-network-proxy` reads from Kodex's merged `config.toml` (via `kodex-core` config loading).
...
# bundle env vars pointing at immutable public files under $KODEX_HOME/proxy/ so common HTTPS
...
The proxy can also run without a full Kodex permissions profile. Put the network policy in a
...
```bash
cargo run -p kodex-network-proxy -- --config /path/to/network-proxy.json
```
...
`kodex-network-proxy` can be embedded as a library with a thin API:

```rust
use kodex_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `kodex-network-proxy` is embedded in managed Kodex runtime, policy decisions emit structured
OTEL-compatible events with `target=kodex_otel.network_proxy`.
...
This section documents the protections implemented by `kodex-network-proxy`, and the boundaries of
