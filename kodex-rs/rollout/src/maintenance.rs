...
//! existing rollout path. They must not do that at the same time for one Kodex home, so they share
...
/// Try to exclude rollout compression and migration for one Kodex home.
pub fn try_acquire_rollout_maintenance_lock(
    kodex_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = kodex_home.join(".tmp");
