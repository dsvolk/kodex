...
use kodex_windows_sandbox::prepare_packaged_windows_sandbox_cleanup;
...
impl PackageLifecycle {
    ...
    pub(crate) fn register_authenticated_user(
        ...
        if let Some(previous) = previous {
            // A missing watcher does not retire its owner; other clients can use elevated setup.
                ...
                previous.user_sid == record.user_sid && previous.kodex_home == record.kodex_home,
        ...
        if let Some(installation) = active.as_mut()
            && installation.kodex_home.is_some()
        {
        ...
        with_owner_impersonation(user_token.0, || {
            ...
            let kodex_home = match crate::ipc::pin_existing_ancestors(
                &record.kodex_home,
            ...
            .and_then(|()| {
                ...
                Ok(()) => Some(record.kodex_home.clone()),
                Err(error) => {
            ...
            if let Some(installation) = active.as_mut() {
                installation.kodex_home = kodex_home;
    ...
    pub(crate) fn clean_up(&self) -> Result<()> {
        let _setup_lock =
            kodex_windows_sandbox::acquire_sandbox_setup_lock(/*timeout_ms*/ 5_000)?;
