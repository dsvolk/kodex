...
pub(crate) fn test_mcp_config(kodex_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: kodex_config::ConfigLayerStack::default(),
        approvals_reviewer: kodex_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: kodex_connectors::ConnectorSnapshot::default(),
    }
}
...
fn ema_catalog_supports_configured_installed_and_selected_plugins_without_widening_policy() {
    ...
    let mut config = test_mcp_config(PathBuf::new());
    let idp = kodex_config::McpServerIdpOAuthConfig {
        issuer: "https://idp.example".into(),
        client_id: "enterprise-client".into(),
    };
    let deny_all = kodex_protocol::mcp_policy::EnvironmentMcpPolicy {
...
fn mcp_server_permissions_handle_unattached_and_threadless_servers() {
    ...
    let mut missing_server = kodex_apps_mcp_server_config(
        "https://example.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
...
#[test]
fn kodex_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        kodex_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        kodex_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        kodex_apps_mcp_url_for_base_url("http://localhost:8080/api/kodex"),
        "http://localhost:8080/api/kodex/ps/mcp"
    );
    assert_eq!(
        kodex_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/kodex/ps/mcp"
    );
}

#[test]
fn kodex_apps_server_config_uses_plugin_service_path() {
    let config = kodex_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for kodex apps"),
    };
...
#[test]
fn kodex_apps_server_config_forwards_thread_originator_header() {
    let config = kodex_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "kodex".to_string()),
                ]))
            );
...
#[test]
fn kodex_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "kodex"), (Some("tpp"), "tpp")] {
        let config = kodex_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn kodex_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = kodex_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
fn effective_mcp_servers_preserve_chatgpt_auth_for_staging() {
    for url in [
        ...
        let server = kodex_apps_mcp_server_config(
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(kodex_home.path().to_path_buf());
    ...
    let kodex_apps = effective
        .get(KODEX_APPS_MCP_SERVER_NAME)
        .expect("kodex apps server should exist");

    let sample = sample.config();
    let docs = docs.config();
    let kodex_apps = kodex_apps.config();
