...
#[derive(Debug, Parser)]
#[command(name = "kodex-state-logs")]
#[command(about = "Tail Kodex logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to KODEX_HOME. Defaults to $KODEX_HOME or ~/.kodex.
    #[arg(long, env = "KODEX_HOME")]
    kodex_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --kodex-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(kodex_home) = args.kodex_home.as_ref() {
        config_builder = config_builder.kodex_home(kodex_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["kodex-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["kodex-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["kodex-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["kodex-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden Kodex home.
    #[tokio::test]
    async fn direct_db_skips_kodex_home_config() {
        let kodex_home = tempfile::tempdir().expect("create Kodex home");
        std::fs::write(kodex_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("kodex-state-logs"),
            OsString::from("--kodex-home"),
            kodex_home.path().as_os_str().to_owned(),
            OsString::from("--db"),
