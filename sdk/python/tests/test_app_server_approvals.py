...
from openai_kodex import ApprovalMode, AsyncKodex, Kodex
from openai_kodex.generated.v2_all import AskForApprovalValue, ThreadResumeParams
...
def test_thread_resume_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            source = kodex.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            resumed = kodex.thread_resume(source.id)
            resumed_state = kodex._client.thread_resume(  # noqa: SLF001
                resumed.id,
...
def test_thread_fork_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            source = kodex.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            forked = kodex.thread_fork(source.id)
            forked_state = kodex._client.thread_resume(  # noqa: SLF001
                forked.id,
...
def test_turn_approval_mode_persists_until_next_turn(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            ...
            after_turn_override = kodex._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            second_result = thread.run("inherit previous approval mode")
            after_omitted_turn = kodex._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_thread_run_approval_mode_persists_until_explicit_override(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            thread = kodex.thread_start(approval_mode=ApprovalMode.deny_all)

            first_result = thread.run("keep approvals denied")
            after_default_run = kodex._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            ...
            after_override_run = kodex._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncKodex(config=harness.app_server_config()) as kodex:
                thread = await kodex.thread_start(approval_mode=ApprovalMode.deny_all)
                first_result = await thread.run("keep async approvals denied")
                after_default_run = await kodex._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
                ...
                after_override_run = await kodex._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
