...
async fn save_remote_plugin_share_creates_workspace_plugin() {
    ...
    assert_eq!(
        local_paths::load_plugin_share_local_paths(kodex_home.path()).unwrap(),
...
fn archive_plugin_for_upload_places_manifest_at_archive_root() {
    ...
    assert_eq!(
        ...
        vec![
            ".kodex-plugin/plugin.json".to_string(),
...
fn archive_plugin_for_upload_round_trips_through_plugin_bundle_archive_with_long_paths() {
    ...
    assert_eq!(
        fs::read_to_string(destination.path().join(".kodex-plugin/plugin.json")).unwrap(),
        r#"{"name":"demo-plugin"}"#
    );
    assert_eq!(
...
async fn list_remote_plugin_shares_fetches_created_workspace_plugins() {
    ...
    let local_plugin_path =
        AbsolutePathBuf::try_from(kodex_home.path().join("local-plugin")).unwrap();
    ...
    let result = list_remote_plugin_shares(&config, Some(&auth), kodex_home.path())
        .await
        .unwrap();
...
async fn delete_remote_plugin_share_deletes_workspace_plugin() {
    ...
    delete_remote_plugin_share(&config, Some(&auth), kodex_home.path(), "plugins_123")
        .await
        .unwrap();
    assert_eq!(
        local_paths::load_plugin_share_local_paths(kodex_home.path()).unwrap(),
    ...
    for method in [Method::PUT, Method::POST] {
        ...
        let server = async {
            ...
            controller.publish(
                ...
                kodex_http_client::DestinationPolicy::Restricted {
        ...
        assert!(
            matches!(
                ...
                Err(RemotePluginCatalogError::Request {
                    source: kodex_http_client::RouteAwareRequestError::Policy(
                        kodex_http_client::NetworkPolicyDenied::Revoked
                    ),
                    ..
                })
            ),
