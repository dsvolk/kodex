...
use kodex_exec_server::FsCloseParams;
use kodex_exec_server::FsOpenParams;
use kodex_exec_server::FsReadBlockParams;
use kodex_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\kodex-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\kodex-fs-stream-{}", Uuid::new_v4());
...
#[cfg(unix)]
fn read_only_sandbox(path: std::path::PathBuf) -> kodex_exec_server::FileSystemSandboxContext {
