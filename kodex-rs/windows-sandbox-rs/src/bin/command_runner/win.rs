...
use kodex_windows_sandbox::ConsoleMode;
use kodex_windows_sandbox::ErrorPayload;
use kodex_windows_sandbox::ErrorStage;
use kodex_windows_sandbox::ExitPayload;
use kodex_windows_sandbox::FramedMessage;
use kodex_windows_sandbox::IPC_PROTOCOL_VERSION;
use kodex_windows_sandbox::LaunchDesktop;
use kodex_windows_sandbox::LocalSid;
use kodex_windows_sandbox::Message;
use kodex_windows_sandbox::OutputPayload;
use kodex_windows_sandbox::OutputStream;
use kodex_windows_sandbox::PipeSpawnHandles;
use kodex_windows_sandbox::ResizePayload;
use kodex_windows_sandbox::SpawnReady;
use kodex_windows_sandbox::SpawnRequest;
use kodex_windows_sandbox::StderrMode;
use kodex_windows_sandbox::StdinMode;
use kodex_windows_sandbox::WindowsSandboxTokenMode;
use kodex_windows_sandbox::allow_null_device;
use kodex_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use kodex_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use kodex_windows_sandbox::decode_bytes;
use kodex_windows_sandbox::encode_bytes;
use kodex_windows_sandbox::get_current_token_for_restriction;
use kodex_windows_sandbox::hide_current_user_profile_dir;
use kodex_windows_sandbox::log_note;
use kodex_windows_sandbox::read_frame;
use kodex_windows_sandbox::read_handle_loop;
use kodex_windows_sandbox::spawn_process_with_pipes;
use kodex_windows_sandbox::to_wide;
use kodex_windows_sandbox::token_mode_for_permission_profile;
use kodex_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
const READ_ACL_MUTEX_NAME: &str = "Local\\KodexSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<kodex_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.kodex_home.clone();
    hide_current_user_profile_dir(req.kodex_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = kodex_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
