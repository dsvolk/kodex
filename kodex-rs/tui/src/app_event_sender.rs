...
impl AppEventSender {
    ...
    pub(crate) fn interrupt(&self) {
        self.send(AppEvent::KodexOp(AppCommand::interrupt()));
    }

    pub(crate) fn compact(&self) {
        self.send(AppEvent::KodexOp(AppCommand::compact()));
    }

    pub(crate) fn set_thread_name(&self, name: String) {
        self.send(AppEvent::KodexOp(AppCommand::set_thread_name(name)));
    }

    pub(crate) fn review(&self, target: ReviewTarget) {
        self.send(AppEvent::KodexOp(AppCommand::review(target)));
    }

    pub(crate) fn list_skills(&self, cwds: Vec<PathBuf>, force_reload: bool) {
        self.send(AppEvent::KodexOp(AppCommand::list_skills(
            cwds,
            force_reload,
        )));
    }
    ...
    pub(crate) fn user_input_answer(&self, id: String, response: ToolRequestUserInputResponse) {
        self.send(AppEvent::KodexOp(AppCommand::user_input_answer(
            id, response,
        )));
    }
