...
use kodex_protocol::protocol::EnvironmentConnectionEvent;
...
impl TurnEnvironmentSnapshot {
    pub(crate) fn has_full_access(
        ...
        kodex_protocol::protocol::has_full_access(
            approval_policy,
            thread_profile,
    ...
    #[cfg(test)]
    pub(crate) fn primary_environment(&self) -> Option<Arc<kodex_exec_server::Environment>> {
        self.primary()
...
"#,
        ...
        let cwd = AbsolutePathBuf::current_dir().expect("cwd");
        let cwd_uri = PathUri::from_abs_path(&cwd);
        let manager = EnvironmentManager::from_kodex_home(
            temp_dir.path(),
            Some(test_runtime_paths()),
