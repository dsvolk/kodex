...
pub struct McpServerOAuthConfig {
    ...
    /// Fixed callback port that takes precedence over Kodex's global OAuth callback port.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Kodex should start this MCP server.
    pub environment_id: String,

    /// When `false`, Kodex skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `kodex exec` exits with an error if this MCP server fails to initialize.
