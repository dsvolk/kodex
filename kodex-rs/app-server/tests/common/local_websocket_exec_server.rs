...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(kodex_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(kodex_home);
        command.env("KODEX_HOME", kodex_home);
        #[cfg(target_os = "linux")]
        command.env(
            KODEX_LINUX_SANDBOX_EXE_ENV_VAR,
