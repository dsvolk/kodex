...
use kodex_execpolicy::Decision::Allow;
use kodex_execpolicy::NetworkRuleProtocol::Https;
use kodex_network_proxy::LocalBindingPolicy::DefaultFalse;
use kodex_network_proxy::NetworkDomainPermission;
use kodex_network_proxy::NetworkUnixSocketPermission;
use kodex_network_proxy::NetworkUnixSocketPermissions;
...
-> anyhow::Result<()> {
    ...
    for (name, toml, required, expected) in [
        ...
        let configured: kodex_config::permissions_toml::NetworkToml = toml::from_str(toml)?;
