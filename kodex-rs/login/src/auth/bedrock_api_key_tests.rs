...
fn bedrock_api_key_debug_redacts_secret() {
    ...
    assert_eq!(
        format!("{:?}", KodexAuth::BedrockApiKey(auth)),
...
async fn login_with_bedrock_api_key_replaces_openai_auth() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            KodexAuth::BedrockApiKey(auth) => Some(auth),
            KodexAuth::ApiKey(_)
            | KodexAuth::Chatgpt(_)
            ...
            | KodexAuth::BedrockAccessKeys(_) => None,
        }),
        Some(bedrock_auth())
    );
    Ok(())
...
async fn access_keys_auth_round_trips_and_logs_out() -> anyhow::Result<()> {
    ...
    assert_eq!(
        auth_manager.auth_cached().and_then(|auth| match auth {
            KodexAuth::BedrockAccessKeys(auth) => Some(auth),
            KodexAuth::ApiKey(_)
            | KodexAuth::Chatgpt(_)
            | KodexAuth::ChatgptAuthTokens(_)
            | KodexAuth::Headers(_)
            | KodexAuth::AgentIdentity(_)
            | KodexAuth::PersonalAccessToken(_)
            | KodexAuth::BedrockApiKey(_) => None,
        }),
