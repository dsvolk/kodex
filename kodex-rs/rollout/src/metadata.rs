...
use kodex_state::BackfillState;
use kodex_state::BackfillStats;
use kodex_state::BackfillStatus;
use kodex_state::DB_ERROR_METRIC;
use kodex_state::DB_METRIC_BACKFILL;
use kodex_state::DB_METRIC_BACKFILL_DURATION_MS;
use kodex_state::ExtractionOutcome;
use kodex_state::ThreadMetadataBuilder;
use kodex_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = kodex_otel::global();
    ...
    let sessions_root = kodex_home.join(SESSIONS_SUBDIR);
    let archived_root = kodex_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(kodex_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(kodex_home: &Path, path: &Path) -> String {
    path.strip_prefix(kodex_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
