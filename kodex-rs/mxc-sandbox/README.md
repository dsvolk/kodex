...
This crate routes a command through the current Kodex executable and directly
...
elevation. The existing Kodex Windows sandboxes remain separate backends.

Windows executors record `kodex.windows_mxc.available` once per process with an
...
from the canonical Kodex permission profile, including protected metadata
...
executor's Kodex executable as the native SDK helper. It carries one typed
...
`kodex sandbox windows` to debug through the same preparation path as execution.
