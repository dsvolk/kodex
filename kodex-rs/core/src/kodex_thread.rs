...
use kodex_exec_server::SelectedCapabilityRootsStatus;
...
use kodex_protocol::turn_input::RecoverTurnRequest;
use kodex_protocol::turn_input::StartIfIdleSubmission;
use kodex_protocol::turn_input::SteerSubmission;
use kodex_protocol::turn_input::SuspendTurnOutcome;
...
use kodex_thread_store::StoredThreadHistory;
use kodex_thread_store::ThreadMetadataPatch;
use kodex_thread_store::ThreadStoreError;
use kodex_thread_store::ThreadStoreResult;
...
pub struct KodexThreadSettingsOverrides {
...
/// (formerly called a conversation) in Kodex.
impl KodexThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> KodexResult<String> {
    ...
    pub fn thread_extension_data(&self) -> &kodex_extension_api::ExtensionData {
        &self.session.services.thread_extension_data
    }

    pub async fn shutdown_and_wait(&self) -> KodexResult<()> {
    ...
    pub(crate) async fn emit_thread_ready_lifecycle(&self) {
        ...
        for contributor in contributors {
            contributor
                .on_thread_ready(kodex_extension_api::ThreadReadyInput {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(kodex_extension_api::ThreadResumeInput {
                    session_store: &self.session.services.session_extension_data,
                    thread_store: &self.session.services.thread_extension_data,
                })
                .await;
    ...
    pub async fn start_turn_if_idle(
        &self,
        request: TurnInputRequest,
    ) -> KodexResult<StartIfIdleSubmission> {
        match self
    ...
    pub async fn suspend_turn_and_shutdown(&self) -> KodexResult<SuspendTurnOutcome> {
        ...
        self.io
            ...
            .await
            .map_err(|_| KodexErr::Fatal("thread session has stopped".to_string()))?;
        let outcome = result
            .await
            .map_err(|_| KodexErr::Fatal("thread suspension reply was lost".to_string()))??;
    ...
    /// callers that only hold a `KodexThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: KodexThreadSettingsOverrides,
    ...
    fn thread_settings_update(overrides: KodexThreadSettingsOverrides) -> SessionSettingsUpdate {
        let KodexThreadSettingsOverrides {
            environments,
    ...
    pub async fn next_event(&self) -> KodexResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> KodexResult<()> {
    ...
    pub async fn restorable_thread_settings(&self) -> KodexThreadSettingsOverrides {
    ...
    ) -> std::collections::HashMap<String, kodex_protocol::mcp::McpServerConnectionStatus> {
        self.session
            .services
            .mcp_runtime
    ...
    ) -> (kodex_mcp::McpConfig, kodex_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<kodex_mcp::McpConfig>, kodex_mcp::McpRuntimeContext) {
    ...
    pub async fn refresh_kodex_apps_tools(
        &self,
    ) -> anyhow::Result<kodex_mcp::KodexAppsToolSnapshot> {
        self.session.refresh_kodex_apps_tools().await
    }
    ...
    pub async fn start_mcp_event_stream(
    ...
    ) -> anyhow::Result<kodex_mcp::McpEventStream> {
        ...
        self.session.refresh_mcp_if_dirty().await;
        kodex_mcp::McpResourceClient::new(Arc::clone(&self.session.services.mcp_runtime))
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> KodexResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            KodexErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> KodexResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(KodexErr::InvalidRequest(
