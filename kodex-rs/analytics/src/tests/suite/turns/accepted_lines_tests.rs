    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "kodex",
                "model_slug": "gpt-5.1-kodex",
                "completed_at": 1710000000,
...
async fn analytics_flush_delivers_completed_turn_with_file_diff() {
    ...
    let capture_path = std::env::temp_dir().join(format!(
        "kodex-analytics-turn-flush-{}-{nonce}.jsonl",
        std::process::id()
    ));
...
"
    ...
    let event_types = contents
        ...
        .collect::<Vec<_>>();
    assert!(event_types.iter().any(|event| event == "kodex_turn_event"));
    assert!(
        event_types
            .iter()
            .any(|event| event == "kodex_accepted_line_fingerprints")
    );
