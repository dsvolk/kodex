...
pub async fn append_thread_name(
    ...
    append_session_index_entry(kodex_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(kodex_home);
...
fn session_index_path(kodex_home: &Path) -> PathBuf {
    kodex_home.join(SESSION_INDEX_FILE)
}
