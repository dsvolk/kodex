...
async fn disabling_plugins_clears_cloud_catalog_and_skips_discovery() -> anyhow::Result<()> {
    ...
    let mut config = kodex_core::config::ConfigBuilder::default()
        .kodex_home(config_home.path().to_path_buf())
    ...
    let thread = ExtensionData::new("thread");
    let thread_init = kodex_extension_api::ExtensionDataInit::default();
    for (turn_id, plugins_enabled, reply) in [
        ...
        registry.mcp_server_contributors()[0]
            .contribute(kodex_extension_api::McpServerContributionContext::for_step(
                &config,
                &thread_init,
                &thread,
                "test",
...
fn cloud_catalog() -> PluginCatalog {
    PluginCatalog {
        entries: vec![PluginCatalogEntry {
            ...
            locations: vec![kodex_core_plugins::PluginSourceLocation::Cloud {
...
async fn run_cloud_catalog_refresh_lifecycle() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex()
        ...
        .with_auth(kodex_login::KodexAuth::from_external_chatgpt_tokens(
            "header.e30.initial",
            "account-a",
    ...
    for (turn, reply, expected_plugin, new_account, tool_call) in [
        ...
        if let Some(account) = new_account {
            ...
            *apps_tools
            ...
            let previous_auth =
                resources.auth_cache_key_for_server(kodex_mcp::KODEX_APPS_MCP_SERVER_NAME);
            ...
            tokio::time::timeout(std::time::Duration::from_secs(10), async {
                loop {
                    if resources.auth_cache_key_for_server(kodex_mcp::KODEX_APPS_MCP_SERVER_NAME)
                        != previous_auth
                        && test
                            .kodex
        ...
        if let Some((connector, action, _)) = tool_call {
            events.push(ev_function_call_with_namespace(
                &call_id,
                &format!("mcp__kodex_apps__{connector}"),
                &format!("_{action}"),
                "{}",
            ));
        }
        ...
        *step_packages
                ...
                tokio::try_join!(
                    ...
                    async {
                        ...
                        let previous_auth = resources
                            .auth_cache_key_for_server(kodex_mcp::KODEX_APPS_MCP_SERVER_NAME);
        ...
        if let Some((connector, action, plugin_name)) = tool_call {
            ...
            let (other_namespace, other_tool) = if connector == "calendar" {
                ("mcp__kodex_apps__gmail", "_search_email")
            } else {
                ("mcp__kodex_apps__calendar", "_list_events")
            };
