...
use kodex_app_server_protocol::GrantedPermissionProfile;
...
"#,
    ...
    let manifest_dir = plugin_dir.join(".kodex-plugin")?;
    ...
    #[cfg(unix)]
    if scenario == ExecutorSkillScenario::RestrictedDeniedReference {
        let external_reference_dir = kodex_home.path().join("external-reference");
    ...
    if scenario == ExecutorSkillScenario::RestrictedVisible && !auto_env.environment().is_remote() {
        let denied_skill_dir = kodex_home.path().join("denied-skill");
