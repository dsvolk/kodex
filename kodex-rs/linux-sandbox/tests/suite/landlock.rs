...
fn kodex_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_kodex-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> kodex_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<kodex_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let kodex_linux_sandbox_exe = Some(kodex_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &kodex_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            KodexErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<kodex_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> kodex_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn wsl_no_proc_masks_inherited_procfs_and_windows_interop() {
    ...
    let proc_check = tokio::time::timeout(
        Duration::from_millis(NETWORK_TIMEOUT_MS),
        tokio::process::Command::new(kodex_linux_sandbox_exe())
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let kodex_linux_sandbox_exe: Option<PathBuf> = Some(kodex_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_kodex_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_kodex = tmpdir.path().join(".kodex");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_kodex).expect("create .kodex");

    let git_target = dot_git.join("config");
    let kodex_target = dot_kodex.join("config.toml");
    ...
    let kodex_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", kodex_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".kodex write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(kodex_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_kodex_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-kodex");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_kodex = tmpdir.path().join(".kodex");
    symlink(&decoy, &dot_kodex).expect("create .kodex symlink");
    ...
    let kodex_output = expect_denied(
        ...
        .await,
        ".kodex symlink replacement should be denied",
    );
    assert_ne!(kodex_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_kodex_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".kodex symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".kodex symlink build failure should deny: {output:?}"),
    };
...
async fn sandbox_rejects_symlinked_synthetic_mount_registry() {
    ...
    let registry = temp.path().join(format!(
        "kodex-bwrap-synthetic-mount-targets-{effective_uid}"
    ));
...
registry="${{TMPDIR:-/tmp}}/kodex-bwrap-synthetic-mount-targets-$(id -u)"
if touch "$registry/forged-marker" 2>/dev/null; then
  exit 22
fi
redirected_registry='{redirected_tmp}'/kodex-bwrap-synthetic-mount-targets-$(id -u)
...
if grep -E '(^|[[:space:]])\.(git|kodex|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_kodex_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".kodex"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .kodex directory creation should be denied",
    );
    assert_ne!(mkdir_kodex_output.exit_code, 0);
    assert!(!subdir.join(".kodex").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = kodex_linux_sandbox_exe()
        .parent()
...
async fn sandbox_starts_with_denied_tmp_without_exposing_registry() {
    ...
    let sandbox_helper = kodex_linux_sandbox_exe();
...
registry="$TMPDIR/kodex-bwrap-synthetic-mount-targets-$(id -u)"
test ! -e "$registry"
"#,
                ],
                cwd.clone(),
