...
use kodex_exec_server::CapabilityRootDiscoverRequest;
...
use kodex_exec_server::discover_capability_roots;
use kodex_protocol::protocol::Product;
use kodex_skills::EnvironmentSkillMetadata;
use kodex_skills::SkillDependencies;
use kodex_skills::SkillPolicy;
...
async fn direct_environment_loader_preserves_plugin_dependencies_and_product_policy() {
    ...
    fs::create_dir_all(root.path().join(".kodex-plugin")).expect("manifest dir");
    fs::create_dir_all(skill_dir.join("agents")).expect("metadata dir");
    fs::write(
        root.path().join(".kodex-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )
    .expect("manifest");
    fs::write(
...
"#,
    ...
    let outcome =
        load_environment_skills_from_root(&file_system, &root_uri, Some(Product::Kodex)).await;
    ...
    assert_eq!(
        ...
        vec![EnvironmentSkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::Kodex, Product::Atlas],
            }),
        }]
    );
    let atlas =
...
async fn executor_bundle_parser_matches_direct_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".kodex-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".kodex-plugin/plugin.json", "kodex-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.kodex-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "kodex-name:search");
}
