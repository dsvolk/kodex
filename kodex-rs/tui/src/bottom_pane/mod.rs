...
pub(crate) struct BottomPane {
    ...
    effects: kodex_config::types::TuiEffects,
...
pub(crate) struct BottomPaneParams {
    ...
    pub(crate) effects: kodex_config::types::TuiEffects,
...
mod tests {
    ...
    fn flush_buffered_typing_in_background_and_covering_editors() {
        ...
        let request = ToolRequestUserInputParams {
            ...
            questions: vec![kodex_app_server_protocol::ToolRequestUserInputQuestion {
                id: "question".into(),
                header: String::new(),
        ...
        for (kind, action, label) in [
            ...
            assert_eq!(
                ...
                if action == "view_usage" {
                    "https://chatgpt.com/kodex/settings/usage"
                } else {
                    "Credits"
                }
            );
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::KodexOp(Op::Interrupt)),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::KodexOp(Op::Interrupt))),
        ...
        assert!(matches!(
            rx.try_recv(),
            Ok(AppEvent::KodexOp(Op::Interrupt))
        ));
    }
