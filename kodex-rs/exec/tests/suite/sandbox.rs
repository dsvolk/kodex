#![cfg(unix)]
use kodex_core::spawn::StdioPolicy;
...
pub(super) async fn spawn_command_under_sandbox(
    ...
    use kodex_core::exec::build_exec_request;
    ...
    let kodex_linux_sandbox_exe = Some(
        core_test_support::find_kodex_linux_sandbox_exe()
    ...
    let exec_request = build_exec_request(
        ...
        /*kodex_self_exe*/ &None,
        kodex_protocol::sandbox::SandboxType::None,
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_kodex_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .kodex && echo 'sandbox_mode = \"danger-full-access\"' > .kodex/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .kodex");

    let status = child.wait().await.expect("should wait for .kodex command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .kodex creation: {status:?}"
    );
    let dot_kodex_metadata = tokio::fs::symlink_metadata(&dot_kodex).await;
    if let Ok(metadata) = dot_kodex_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_kodex.display()
        );
    } else if let Err(err) = &dot_kodex_metadata {
        assert_eq!(
            err.kind(),
