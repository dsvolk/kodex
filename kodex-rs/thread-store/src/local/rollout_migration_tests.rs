...
use kodex_app_server_protocol::build_turns_from_rollout_items;
...
use kodex_protocol::mcp::McpResourceOrigin;
...
fn move_to_archived(home: &Path, path: PathBuf) -> PathBuf {
    let directory = home.join(kodex_rollout::ARCHIVED_SESSIONS_SUBDIR);
...
fn read_rollout(path: &Path) -> Vec<RolloutLine> {
    fs::read_to_string(path)
        ...
        .lines()
        .map(|line| kodex_rollout::parse_rollout_line(line).expect("parse migrated rollout"))
        .collect()
}
...
async fn indexed_store(home: &Path) -> LocalThreadStore {
    ...
    let state_db = kodex_rollout::state_db::try_init(&rollout_config)
        .await
...
async fn migration_publishes_canonical_projected_history_and_is_idempotent() {
    let home = TempDir::new().expect("create Kodex home");
...
async fn migration_preserves_answers_before_a_rolled_back_steer() {
    ...
    for call_id in ["before-steer", "after-steer"] {
        ...
        let answer: kodex_rollout::RetainedContextEvent = serde_json::from_value(json!({
    ...
    assert_eq!(
        checkpoint
            ...
            .cloned()
            .map(
                |answer| kodex_rollout::RetainedContextEvent::VerifiedAnswer {
                    answer,
...
async fn assert_migrated_evidence_order(steer_order: Option<u64>) {
    ...
    let answers = checkpoint
        ...
        .map(|answer| {
            ...
            kodex_rollout::RetainedContextEvent::VerifiedAnswer {
                answer,
                acceptance_order,
            }
        })
    ...
    let path = write_rollout(
        ...
        vec![
            ...
            RolloutItem::ResponseItem(kodex_rollout::ResponseItemEnvelope {
                item: steer,
    ...
    let replacement_histories = read_rollout(&path)
        ...
        .filter_map(|line| match line.item {
            RolloutItem::Compacted(item) => item.replacement_history.map(|items| {
                items
                    .into_iter()
                    .map(kodex_rollout::ResponseItemEnvelope::into_item)
                    .collect::<Vec<_>>()
            }),
            _ => None,
        })
...
async fn migration_preserves_compressed_rollouts_during_publish_and_recovery() {
    ...
    assert_eq!(
        kodex_rollout::read_session_meta_line(&compressed_path)
            .await
...
async fn migration_preserves_legacy_displayed_thread_names() {
    ...
    kodex_rollout::append_thread_name(home.path(), title_thread_id, "stale index title")
        .await
        .expect("write stale legacy index name");
    kodex_rollout::append_thread_name(home.path(), index_thread_id, "indexed title")
        .await
    ...
    guardian_metadata.name = Some(kodex_state::GUARDIAN_THREAD_TITLE.to_string());
    state_db
        .upsert_thread(&guardian_metadata)
        .await
        .expect("seed legacy Guardian name");
    kodex_rollout::append_thread_name(home.path(), guardian_thread_id, "indexed Guardian name")
        .await
...
async fn migration_apply_conflicts_with_rollout_maintenance() {
    ...
    let _maintenance_guard = kodex_rollout::try_acquire_rollout_maintenance_lock(home.path())
