...
use kodex_protocol::items::ContextCompactionItem;
...
use kodex_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = KodexErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
                    kodex_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    let kodex_error = result.as_ref().err();
    if result.is_ok() {
        let post_compact_outcome = run_post_compact_hooks(sess, turn_context, trigger).await;
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, kodex_error, analytics_details)
                .await;
    ...
    match result {
        ...
        Err(err) => {
            sess.track_turn_kodex_error(turn_context, &err);
...
) -> KodexResult<RemoteCompactionV2Output> {
    let turn_context = &step_context.turn;
...
fn build_v2_compacted_history(
    ...
    prompt_input_metadata: Vec<Option<KodexHarnessMetadata>>,
    compaction_output: ResponseItem,
...
mod tests {
    ...
    fn response_stream(events: Vec<KodexResult<ResponseEvent>>) -> ResponseStream {
    ...
    fn build_v2_compacted_history_filters_to_installed_retention_shape() {
        let hook = kodex_protocol::items::build_hook_prompt_message(&[
            kodex_protocol::items::HookPromptFragment::from_single_hook("hook", "hook-run"),
        ])
        .expect("hook prompt");
        let input = vec![
    ...
    fn retained_history_truncation_preserves_metadata() {
        ...
        assert_eq!(truncated[0].metadata, Some(KodexHarnessMetadata::default()));
    }
    ...
    async fn collect_compaction_output_accepts_additional_output_items() {
        ...
        let stream = response_stream(vec![
            ...
            Ok(ResponseEvent::Completed {
                ...
                usage_metadata: Some(kodex_protocol::ResponseUsageMetadata {
