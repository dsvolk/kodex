...
-> anyhow::Result<()> {
    ...
    mount_response_once(
        ...
        .insert_header("x-kodex-safety-buffering-enabled", "true")
        .insert_header("x-kodex-safety-buffering-faster-model", FASTER_MODEL),
    )
    .await;
    ...
    let event = wait_for_event_match(&test.kodex, |event| match event {
