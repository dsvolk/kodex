...
"#,
        ...
        let output = Command::new(kodex_utils_cargo_bin::cargo_bin("kodex")?)
...
async fn invalid_custom_ca_falls_back_to_system_roots() -> Result<()> {
    ...
    let kodex_home = TempDir::new()?;
    let certificate = kodex_home.path().join("invalid-ca.pem");
    ...
    for sandbox in [None, Some("seatbelt")] {
        ...
        command
            .args(["doctor", "--json"])
            .env("KODEX_HOME", kodex_home.path())
            .env("KODEX_CA_CERTIFICATE", &certificate)
            .stdin(Stdio::null());
...
fn doctor_reports_macos_system_proxy_configuration_and_policy() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let report = doctor_report(kodex_home.path())?;
...
#[cfg(target_os = "macos")]
fn doctor_report(kodex_home: &Path) -> Result<Value> {
    let output = Command::new(kodex_utils_cargo_bin::cargo_bin("kodex")?)
        .args(["doctor", "--json"])
