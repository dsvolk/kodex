...
use kodex_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use kodex_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use kodex_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use kodex_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use kodex_protocol::protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub enum Account {
    ...
    AmazonBedrock {
        #[serde(default)]
        uses_kodex_managed_credentials: bool,
    },
}
    ...
    fn from(account: ProviderAccount) -> Self {
        match account {
            ...
            ProviderAccount::AmazonBedrock {
                uses_kodex_managed_credentials,
            } => Self::AmazonBedrock {
                uses_kodex_managed_credentials,
            },
        }
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Kodex-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, Kodex attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Kodex attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Kodex was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `kodex`).
