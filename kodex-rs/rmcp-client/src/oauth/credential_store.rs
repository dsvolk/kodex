//! Adapts Kodex's pinned credential storage to RMCP-owned OAuth refreshes.
//!
...
//! the pinned store before RMCP exchanges the token and saves the result. Kodex
        ...
        Self: 'async_trait,
    {
        Box::pin(async move {
            let mut token_response = credentials
                .token_response
                .ok_or(AuthError::AuthorizationRequired)?;
            // Kodex stores granted scopes in the token response rather than a separate field.
