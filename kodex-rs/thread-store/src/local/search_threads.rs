...
use kodex_rollout::first_rollout_content_match_snippet;
use kodex_rollout::parse_cursor;
use kodex_rollout::search_rollout_matches;
use kodex_utils_absolute_path::normalize_windows_device_path;
...
fn rollout_search_path(path: &Path) -> PathBuf {
    let path = kodex_rollout::plain_rollout_path(path);
