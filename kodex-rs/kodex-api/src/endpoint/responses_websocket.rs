...
const X_KODEX_TURN_STATE_HEADER: &str = "x-kodex-turn-state";
...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "kodex_api::responses_websocket_timing";
...
const WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str = "x-kodex-ws-stream-request-start-ms";
...
fn map_ws_stream_error(error: WsError) -> ApiError {
    match kodex_websocket_client::network_policy_denial(&error) {
    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                if event.kind() == "kodex.response.metadata"
                    && let Some(etag) =
                        event
                            .headers
                            .as_ref()
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='kodex_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn direct_serialization_preserves_websocket_request_payload() {
        let api_request = ResponsesApiRequest {
            ...
            access_programs: Some(
                kodex_protocol::turn_input::CyberAccessProgram::DaybreakBlue.into(),
            ),
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-kodex-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-kodex-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            ...
            "headers": {
                "x-kodex-safety-buffering-enabled": "true",
                "x-kodex-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "kodex.response.metadata",
            "headers": {
                "x-kodex-safety-buffering-enabled": "false",
                "x-kodex-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");
