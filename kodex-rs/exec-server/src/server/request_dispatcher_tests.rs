...
fn request_span_uses_bounded_name_wire_method_and_inbound_trace_parent() {
    ...
    let trace = kodex_protocol::protocol::W3cTraceContext {
    ...
    router.request(
        ...
        move |_handler: Arc<ExecServerHandler>, _params: ()| {
            ...
            async move {
                ...
                Ok::<_, kodex_exec_server_protocol::JSONRPCErrorError>(())
            }
        },
    );
...
async fn total_duration_records_disconnection_during_execution() {
    ...
    router.request(
        ...
        move |_handler: Arc<ExecServerHandler>, _params: ()| {
            ...
            async move {
                execution_started.notify_one();
                std::future::pending::<Result<(), kodex_exec_server_protocol::JSONRPCErrorError>>()
                    .await
            }
        },
    );
