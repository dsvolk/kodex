...
use kodex_context_fragments::to_annotated_content;
use kodex_extension_api::ConversationHistorySnapshot;
use kodex_guardian_context::SectionHistory;
use kodex_guardian_context::TranscriptHistory;
...
use kodex_history::RetainedInputSource;
...
use kodex_utils_audio::estimate_audio_token_count;
use kodex_utils_cache::BlockingLruCache;
use kodex_utils_cache::sha1_digest;
...
use kodex_utils_output_truncation::approx_tokens_from_byte_count_i64;
use kodex_utils_output_truncation::truncate_function_output_payload;
...
impl ConversationHistorySnapshot for SharedConversationHistory {
    fn latest_compaction(&self) -> Option<kodex_history::CompactionCheckpoint<'_>> {
        kodex_history::CompactionCheckpoint::latest(&self.items)
    }
...
impl ContextManager {
                    ...
                    || context.ordered_entries().any(|(_, entry)| match entry {
                        ...
                        RetainedContextEntry::UserMessage(message) => {
                            !self.raw_items().any(|item| {
                                if item.id().map(kodex_protocol::ResponseItemId::as_str)
    ...
    where
        I: IntoIterator<Item = (T, Option<&'a KodexHarnessMetadata>)>,
    ...
    pub(crate) fn drop_last_n_user_turns(&mut self, num_turns: u32) {
        ...
        let first_removed_message_id = snapshot[cut_idx]
            .id()
            .map(kodex_protocol::ResponseItemId::as_str);
...
fn is_api_message(message: &ResponseItem, metadata: Option<&KodexHarnessMetadata>) -> bool {
    match message {
