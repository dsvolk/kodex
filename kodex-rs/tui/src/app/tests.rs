...
use kodex_app_server_protocol::AdditionalFileSystemPermissions;
use kodex_app_server_protocol::AdditionalNetworkPermissions;
use kodex_app_server_protocol::AdditionalPermissionProfile;
...
use kodex_app_server_protocol::FileUpdateChange;
...
use kodex_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use kodex_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use kodex_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use kodex_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use kodex_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use kodex_app_server_protocol::PatchChangeKind;
use kodex_app_server_protocol::PermissionsRequestApprovalParams;
use kodex_app_server_protocol::RequestId as AppServerRequestId;
...
use kodex_app_server_protocol::ThreadAttachmentOperation;
use kodex_app_server_protocol::ThreadAttachmentUpdatedNotification;
...
use kodex_app_server_protocol::TokenUsageBreakdown;
...
use kodex_app_server_protocol::TurnError as AppServerTurnError;
...
use kodex_app_server_protocol::UserInput as AppServerUserInput;
...
use kodex_protocol::models::NetworkPermissions;
use kodex_protocol::models::PermissionProfile;
use kodex_protocol::openai_models::MODEL_SPECIALTY_CYBER;
use kodex_protocol::protocol::EventMsg;
use kodex_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
use kodex_protocol::protocol::MultiAgentVersion;
use kodex_protocol::protocol::SessionMeta;
use kodex_protocol::protocol::SessionSource as RolloutSessionSource;
...
async fn duplicate_named_resume_renders_error_without_replacing_current_session() -> Result<()> {
    ...
    let runtime = kodex_state::StateRuntime::init(
    ...
    for _ in 0..2 {
        ...
        let mut metadata = kodex_state::ThreadMetadataBuilder::new(
            thread_id,
            path.to_path_buf(),
            created_at,
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let kodex_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: kodex_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn external_editor_writable_directory_rejected_snapshot() -> Result<()> {
    ...
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let kodex_home = app.chat_widget.config_ref().kodex_home.clone();
    let fallback_home = dirs::home_dir()
        .expect("home directory")
        .join(".kodex")
        .abs();
    let workspace_kodex_home = app.chat_widget.config_ref().cwd.join(".kodex");
    let permission_profile = PermissionProfile::workspace_write_with(
        &[kodex_home, fallback_home, workspace_kodex_home],
        kodex_protocol::permissions::NetworkSandboxPolicy::Restricted,
...
async fn enqueue_primary_thread_session_replays_buffered_approval_after_attach() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        kodex_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::ServerRequestResolved(
                kodex_app_server_protocol::ServerRequestResolvedNotification {
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::KodexOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    app.local_settings.kodex_home = AbsolutePathBuf::from_absolute_path(local_home.path())?;
    let history_config = kodex_message_history::HistoryConfig::new(
        app.local_settings.kodex_home.clone(),
        &app.local_settings.history,
    );
    kodex_message_history::append_entry("local prompt", thread_id, &history_config).await?;
    let (log_id, _) = kodex_message_history::history_metadata(&history_config).await;
    ...
    let cursor = kodex_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(kodex_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: kodex_app_server_protocol::CollabAgentTool::Wait,
                status: kodex_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemCompleted(kodex_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: kodex_app_server_protocol::CollabAgentTool::SendInput,
                status: kodex_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    kodex_app_server_protocol::CollabAgentState {
                        status: kodex_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
    ...
    app.handle_app_server_event(
        ...
        kodex_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::ThreadAttachmentUpdated(ThreadAttachmentUpdatedNotification {
                ...
                identity_key: r#"["github.com","openai","kodex",123]"#.to_string(),
...
fn selected_and_resumed_threads_use_server_capability_for_v1_and_v2_children() -> Result<()> {
    ...
    runtime.block_on(Box::pin(async {
        ...
        let root_session_meta = SessionMeta {
            ...
            originator: "kodex-tui-test".to_string(),
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::KodexOp(Op::UserTurn { .. })))
        );
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    let config = std::fs::read_to_string(kodex_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let kodex_home = tempdir()?;
        app.config.kodex_home = kodex_home.path().to_path_buf().abs();
        app.config.sqlite = kodex_state::SqliteConfig::new_for_testing(kodex_home.path().abs());
        ...
        let state_db = kodex_state::StateRuntime::init(
            kodex_state::SqliteConfig::new_for_testing(kodex_home.path().abs()),
            app.config.model_provider_id.clone(),
        )
        .await
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = kodex_home.path().join("memories");
...
async fn resume_rejects_selected_config_profile_permission_definitions() -> Result<()> {
    ...
    let mut loader = kodex_config::LoaderOverrides::without_managed_config_for_tests();
...
async fn remote_resume_rejects_explicit_permission_override() -> Result<()> {
    ...
    app.config.kodex_home = home.path().to_path_buf().abs();
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::KodexOp(op)) => op,
        other => panic!("expected KodexOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let kodex_home = tempdir()?;
    app.config.kodex_home = kodex_home.path().to_path_buf().abs();
    let config_toml_path = kodex_home.path().join("config.toml").abs();
...
async fn side_defers_subagent_approval_overlay_until_side_exits() -> Result<()> {
    ...
    app.enqueue_thread_request(
        ...
        ServerRequest::DynamicToolCall {
            ...
            params: kodex_app_server_protocol::DynamicToolCallParams {
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            kodex_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            kodex_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            kodex_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            kodex_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: kodex_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: kodex_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: kodex_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
...
async fn inactive_thread_started_notification_initializes_replay_session() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                originator: None,
                environments: Some(vec![kodex_app_server_protocol::ThreadEnvironment {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: kodex_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: kodex_app_server_protocol::SessionSource::Unknown,
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
    ...
    let resolution = app
        ...
        .take_resolution(
            ...
            Op::ExecApproval {
                ...
                turn_id: None,
                decision: kodex_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            },
        )
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            kodex_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn ctrl_l_clears_owned_history_and_preserves_the_draft() -> Result<()> {
    let (mut app, _kodex_home, thread_id) =
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        kodex_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let kodex_home = app_server
        .kodex_home_path(&app.chat_widget.config_ref().kodex_home)
        .expect("kodex home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&kodex_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&kodex_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{kodex_home}/../other/{suffix}"),
        format!("{kodex_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&kodex_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/kodex\\home"),
        &format!("/tmp/kodex/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/kodex\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/kodex\\/a");
    let attachments_dir = app.chat_widget.config_ref().kodex_home.join("attachments");
...
fn test_turn(turn_id: &str, status: TurnStatus, items: Vec<ThreadItem>) -> Turn {
    Turn {
        ...
        items_view: kodex_app_server_protocol::TurnItemsView::Full,
        items,
        status,
        error: None,
        started_at: None,
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        kodex_error_info: Some(AppServerKodexErrorInfo::ActiveTurnNotSteerable {
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(kodex_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
    ...
    for (configured_mode, has_explicit_cwd, has_remote_exec, has_runtime_cwd, expected_directory) in [
        ...
        for cwd in [
            ...
            crate::legacy_core::config::set_project_trust_level(
                &kodex_home,
                cwd,
                kodex_protocol::config_types::TrustLevel::Trusted,
            )
            .map_err(std::io::Error::other)?;
        }
        let config = ConfigBuilder::default()
            .kodex_home(kodex_home.clone())
            .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        ...
        .expect("materialized rollout should be created");
        let rollout_path =
            app_test_support::rollout_path(&kodex_home, filename_timestamp, &thread_id);
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(kodex_config::types::ResumeCwdMode::Current)
        .apply()
        .await
        .map_err(std::io::Error::other)?;
...
async fn prompt_edit_reverts_earlier_and_first_visible_prompts_in_place() -> Result<()> {
    ...
    use kodex_protocol::items::UserMessageItem;
    ...
    .expect("materialized rollout should be created");
    let source_path =
        app_test_support::rollout_path(config.kodex_home.as_path(), filename_ts, &source_thread_id);
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            kodex_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
    ...
    local_settings.tui.alternate_screen = kodex_config::types::AltScreenMode::Never;
    ...
    app.scrollback_has_older_history = app_server.has_older_history(source_thread_id);
    {
        ...
        for item in selected_turn.items {
            store.push_notification(ServerNotification::ItemCompleted(
                kodex_app_server_protocol::ItemCompletedNotification {
    ...
    let goal = app_server
        .thread_goal_set(
            ...
            Some(kodex_app_server_protocol::ThreadGoalStatus::Paused),
            /*token_budget*/ None,
        )
        .await?
        .goal;
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_kodex_backend_auth: app.chat_widget.has_kodex_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(Box::new(
                ServerNotification::ItemStarted(
                    kodex_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: kodex_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                kodex_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                kodex_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
...
async fn changing_cyber_model_reasoning_preserves_selected_permissions() {
    Box::pin(async {
        ...
        assert!(
            app.apply_permission_profile_selection(PermissionProfileSelection {
                profile_id: kodex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
                    .to_string(),
        ...
        for (effort, session_only) in [
            ...
            assert_eq!(
                settings
                    ...
                    .expect("active permission profile")
                    .id,
                kodex_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
            );
            assert_eq!(
...
async fn selecting_cyber_model_falls_back_to_user_when_auto_review_is_unavailable() {
    ...
    assert_eq!(
        ...
        Some(kodex_app_server_protocol::AskForApproval::OnRequest)
    );
    assert_eq!(
        params.approvals_reviewer,
        Some(kodex_app_server_protocol::ApprovalsReviewer::User)
    );
}
...
async fn selecting_cyber_model_respects_auto_review_requirements() {
    Box::pin(async {
        ...
        let mut requirements_with_sources = kodex_config::ConfigRequirementsWithSources::default();
        requirements_with_sources.merge_unset_fields(
            kodex_config::RequirementSource::Unknown,
            requirements_toml.clone(),
        );
        let requirements = kodex_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: kodex_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: kodex_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                kodex_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.kodex/skills/abc/SKILL.md"),
