"""Command-line interface for building Kodex package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical Kodex package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        help="Semantic version to record in kodex-package.json.",
    )
    parser.add_argument(
        "--package-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt kodex-code-mode-host executable. If omitted, "
    ...
    zsh_source.add_argument(
        ...
        help=(
            ...
            "scripts/kodex_package/kodex-zsh."
        ),
    )
    zsh_source.add_argument(
        "--zsh-bin",
    ...
    parser.add_argument(
        "--kodex-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows kodex-command-runner.exe executable. "
            "If omitted for Windows targets, kodex-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--kodex-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows kodex-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "kodex-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="kodex-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        kodex_command_runner_bin=resolve_optional_input_path(
            args.kodex_command_runner_bin,
            "prebuilt Windows kodex-command-runner.exe executable",
            "--kodex-command-runner-bin",
        ),
        kodex_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.kodex_windows_sandbox_setup_bin,
            "prebuilt Windows kodex-windows-sandbox-setup.exe executable",
            "--kodex-windows-sandbox-setup-bin",
        ),
    )
    inputs = PackageInputs(
        ...
        kodex_command_runner_bin=source_outputs.kodex_command_runner_bin,
        kodex_windows_sandbox_setup_bin=source_outputs.kodex_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built Kodex package archive at {archive_path}")

    print(f"Built Kodex package directory at {package_dir}")
    return 0
