...
impl App {
    ...
    pub(super) fn open_desktop_thread(&mut self, thread_id: ThreadId) {
        let url = format!("kodex://threads/{thread_id}");
    ...
    pub(super) fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, KODEX_CLI_VERSION)
    }
        ...
        if tui.is_owned_screen() {
            if !self.transcript_cells.iter().any(|cell| {
                ...
                let header: Arc<dyn HistoryCell> =
                    Arc::new(self.clear_ui_header_cell(KODEX_CLI_VERSION));
...
$package = Get-AppxPackage -Name OpenAI.Kodex -ErrorAction SilentlyContinue
if ($null -eq $package) {{
...
$application = $manifest.Package.Applications.Application |
    Where-Object {{
        @($_.Extensions.Extension) | Where-Object {{
            $_.Category -eq 'windows.protocol' -and $_.Protocol.Name -eq 'kodex'
        }}
    }} |
    Select-Object -First 1
if ($null -eq $application -or [string]::IsNullOrWhiteSpace($application.Executable)) {{
    Write-Error 'Desktop app package does not declare a kodex protocol executable'
    exit 1
}}
