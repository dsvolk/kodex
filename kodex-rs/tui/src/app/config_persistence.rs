...
async fn build_config_on_runtime_worker(
    ...
    application_network_policy: kodex_http_client::NetworkPolicy,
...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::KodexOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn refresh_in_memory_config_from_disk(&mut self) -> Result<()> {
        ...
        self.refresh_server_version_overview_notice(KODEX_CLI_VERSION);
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but Kodex could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("kodex-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.local_settings.tui.theme.as_deref()
            && let Some(theme) = crate::render::highlight::resolve_theme_by_name(
                name,
                Some(&self.local_settings.kodex_home),
            )
        {
    ...
    pub(super) async fn verify_windows_sandbox_mode_after_setup(
        &mut self,
        app_server: &mut AppServerSession,
        requested_mode: kodex_app_server_protocol::WindowsSandboxSetupMode,
    ) -> bool {
...
mod tests {
    ...
    async fn resume_model_settings_preserves_only_explicit_model_overrides() {
        ...
        let profile = "work"
            .parse::<kodex_config::ProfileV2Name>()
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let kodex_home = tempdir()?;
        let required_policy = kodex_protocol::protocol::AskForApproval::Never;
...
"#
    ...
    async fn new_session_and_config_rebuilds_keep_the_live_application_network_policy() -> Result<()>
    {
        ...
        let permission_config = app
            ...
            .await?;
        let pet_url =
            "https://persistent.oaistatic.com/kodex/pets/v1/dewey-spritesheet-v4.webp".parse()?;
    ...
    -> Result<()> {
        ...
        let kodex_home = tempdir()?;
        app.config.kodex_home = kodex_home.path().to_path_buf().abs();
        std::fs::write(kodex_home.path().join("config.toml"), "[broken")?;
