...
use anyhow::bail;
use clap::Parser;
use kodex_core_api::AbsolutePathBuf;
use kodex_core_api::AltScreenMode;
...
use kodex_core_api::KodexAppsToolsCache;
use kodex_core_api::KodexHomeUserInstructionsProvider;
use kodex_core_api::KodexThread;
use kodex_core_api::MemoriesConfig;
use kodex_core_api::ModelAvailabilityNuxConfig;
...
use kodex_core_api::Permissions;
use kodex_core_api::ProjectConfig;
use kodex_core_api::RealtimeAudioConfig;
use kodex_core_api::RealtimeConfig;
use kodex_core_api::SessionPickerViewMode;
use kodex_core_api::SessionSource;
use kodex_core_api::SqliteConfig;
use kodex_core_api::StartIfIdleSubmission;
use kodex_core_api::StartThreadOptions;
use kodex_core_api::TerminalResizeReflowConfig;
use kodex_core_api::ThreadManager;
use kodex_core_api::ThreadStoreConfig;
use kodex_core_api::ToolSuggestConfig;
use kodex_core_api::TuiKeymap;
use kodex_core_api::TuiNotificationSettings;
use kodex_core_api::TuiPetAnchor;
use kodex_core_api::TurnInputRequest;
use kodex_core_api::UriBasedFileOpener;
...
#[derive(Debug, Parser)]
#[command(
    name = "kodex-thread-manager-sample",
    about = "Run one Kodex turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("kodex_thread_manager_sample".to_string()) {
    ...
    install_image_generation_extension(&mut extensions, auth_manager.clone(), |config: &Config| {
        Some(config.kodex_home.clone())
    });
    let thread_manager = ThreadManager::new(
        &config,
        Arc::clone(&auth_manager),
        build_models_manager(&config, auth_manager),
        KodexAppsToolsCache::default(),
        SessionSource::Exec,
        environment_manager,
    ...
    turn_output?;
    shutdown_result.context("shut down Kodex thread")?;
...
fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    ...
    let mut config = Config {
        ...
        sqlite: SqliteConfig::from_sqlite_home(kodex_home.clone()),
        log_dir: kodex_home.join("log").to_path_buf(),
        kodex_home,
...
async fn run_turn(thread: &KodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread.next_event().await.context("read Kodex event")?;
