...
impl VoiceHost {
    ...
    pub async fn connect(package: &KodexPackageLayout, build_commit: &str) -> Result<Self> {
        ...
        let path = root.join("kodex-resources/voice/bin").join(name);
        ensure!(
        ...
        let SpawnedProcess {
            ...
            stderr_rx,
            exit_rx,
        } = kodex_utils_pty::spawn_pipe_process(
            &path,
            &[],
            &root,
            &environment,
