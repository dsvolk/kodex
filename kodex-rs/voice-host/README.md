...
`kodex-voice-host` establishes the inherited-pipe lifecycle for the proposed
...
`kodex-resources/voice/bin/kodex-voice-host[.exe]`, filters the child environment,
and owns process cleanup through `kodex-utils-pty`. Its runtime must remain alive
...
into ordinary Kodex. The existing `libloading` dependency supplies OS loading.
...
the host platform. From `kodex-rs`, run
`KODEX_TEST_VOICE_RUNTIME=/absolute/prepared/runtime just test -p kodex-voice-host --test packaged_runtime --run-ignored all`.
