...
fn list_tasks_url_omits_empty_query_and_encodes_all_parameters() {
    ...
    assert_eq!(
        client
            ...
            .unwrap(),
        "https://example.test/api/kodex/tasks/list"
    );
    assert_eq!(
        client
            .list_tasks_url(
            ...
            .unwrap(),
        "https://example.test/api/kodex/tasks/list?limit=10&task_filter=mine+%2F+shared&cursor=next%3Dpage&environment_id=env%26one"
    );
}
...
async fn migrated_requests_preserve_query_auth_and_json_body() {
    ...
    .with_auth_provider(Arc::new(kodex_model_provider::BearerAuthProvider::new(
    ...
    assert!(requests[0].starts_with(
        "GET /api/kodex/tasks/list?limit=10&task_filter=mine+%2F+shared&cursor=next%3Dpage&environment_id=env%26one HTTP/1.1\r\n"
    ));
    assert!(
        requests[0]
            .to_ascii_lowercase()
    ...
    assert!(requests[1].starts_with("POST /api/kodex/tasks HTTP/1.1\r\n"));
    assert!(
        requests[1]
            .to_ascii_lowercase()
            .contains("authorization: bearer request-token\r\n")
...
fn run_without_environment_proxies(test_name: &str) -> bool {
    const CHILD: &str = "KODEX_BOOTSTRAP_PROXY_TEST_CHILD";
...
async fn bootstrap_gets_resolve_each_fallback_destination_and_preserve_headers() {
    ...
    for (base_suffix, api_prefix) in [("", "/api/kodex"), ("/backend-api", "/wham")] {
...
async fn bootstrap_gets_keep_default_responses_without_proxy_retry() {
    ...
    for status in [200, 401] {
        ...
        for endpoint in ["config/bundle", "accounts/check"] {
            cache_system_proxy_route_for_test(
                &format!("{}/api/kodex/{endpoint}", issuer.uri()),
                proxy.uri(),
            );
        }
        Mock::given(method("GET"))
...
async fn bootstrap_gets_honor_disabled_fallback() {
    ...
    for endpoint in ["config/bundle", "accounts/check"] {
        cache_system_proxy_route_for_test(&format!("{base_url}/api/kodex/{endpoint}"), proxy.uri());
    }
    let client = Client::new(
        base_url,
        HttpClientFactory::new(OutboundProxyPolicy::ReqwestDefault),
...
async fn bootstrap_get_recovers_from_stalled_body_before_cloud_startup_timeout() {
    ...
    let proxy = MockServer::start().await;
    cache_system_proxy_route_for_test(
        &format!("{issuer_url}/api/kodex/config/bundle"),
        proxy.uri(),
    );
    Mock::given(method("GET"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({})))
