...
pub use kodex_core::test_support::TestKodexResponsesRequestKind;
pub use kodex_core::test_support::responses_metadata;
use kodex_utils_absolute_path::AbsolutePathBuf;
pub use kodex_utils_absolute_path::test_support::PathBufExt;
pub use kodex_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    kodex_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    kodex_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(kodex_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = kodex_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("kodex-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\kodex\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.kodex`.
pub async fn load_default_config_for_test(kodex_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        kodex_linux_sandbox_exe: Some(
            find_kodex_linux_sandbox_exe().expect("should find binary for kodex-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_kodex_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    kodex_utils_cargo_bin::cargo_bin("kodex-linux-sandbox")
}

pub async fn wait_for_event<F>(
    kodex: &KodexThread,
    predicate: F,
) -> kodex_protocol::protocol::EventMsg
where
    F: FnMut(&kodex_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(kodex, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(kodex: &KodexThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    kodex: &KodexThread,
    thread_settings: kodex_protocol::protocol::ThreadSettingsOverrides,
) -> anyhow::Result<()> {
    use kodex_protocol::protocol::EventMsg;
    ...
    let submission_id = kodex.submit(Op::ThreadSettings { thread_settings }).await?;
    loop {
        let ev = timeout(Duration::from_secs(10), kodex.next_event())
            .await
...
impl kodex_extension_api::ThreadLifecycleContributor<Config> for ThreadIdle {
    fn on_thread_idle<'a>(
        &'a self,
...
impl ThreadIdle {
    pub async fn wait(thread: &KodexThread) {
...
pub async fn wait_for_event_match<T, F>(kodex: &KodexThread, matcher: F) -> T
where
    F: Fn(&kodex_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(kodex, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&kodex_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), kodex.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    kodex_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    kodex_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    kodex_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    kodex_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    kodex_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a Kodex sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! kodex_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_kodex_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("kodex-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
