...
use crate::rmcp_client::KODEX_APPS_RECONNECT_INITIAL_BACKOFF;
use crate::rmcp_client::KodexAppsStartupReconnect;
...
use kodex_config::McpServerEnvVar;
use kodex_config::McpServerToolConfig;
use kodex_config::types::AuthKeyringBackendKind;
use kodex_config::types::OAuthCredentialsStoreMode;
use kodex_connectors::ConnectorRuntimeContext;
use kodex_connectors::ConnectorRuntimeContextKey;
use kodex_connectors::ConnectorRuntimeFetchSource;
use kodex_connectors::ConnectorRuntimeManager;
...
use kodex_protocol::approvals::ElicitationRequest;
use kodex_protocol::mcp::ClientMcpExtensions;
use kodex_protocol::mcp::McpServerInfo;
...
use kodex_protocol::protocol::McpStartupFailureReason;
use kodex_rmcp_client::ElicitationResponse;
use kodex_rmcp_client::InProcessTransportFactory;
use kodex_rmcp_client::McpAuthState;
use kodex_rmcp_client::McpLoginRequirement;
...
fn create_kodex_apps_tools_cache_context(
    kodex_home: PathBuf,
...
fn store_current_tools(cache_context: &ConnectorRuntimeContext<ToolInfo>, tools: Vec<ToolInfo>) {
    let _ = cache_context.publish_if_newest_accepted(
        ...
        &create_test_server_info("Kodex Apps"),
        tools,
    );
}
...
async fn legacy_tool_catalog_does_not_follow_pagination_cursor() -> anyhow::Result<()> {
    ...
    client
        .initialize(
            InitializeRequestParams::new(
                ...
                Implementation::new("kodex-test", "0.0.0-test"),
            )
    ...
    let tools = list_tools_for_client_uncached(
        "legacy",
        /*is_kodex_apps_mcp_server*/ false,
        "test",
        &client,
        Some(Duration::from_secs(5)),
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        kodex_apps_tools_cache_context: None,
    }
}
...
async fn connection_statuses_observe_clients_without_starting_them() {
    use kodex_protocol::mcp::McpServerConnectionStatus as Status;
...
async fn connection_statuses_follow_latest_reconnect_outcome() {
    ...
    let client = manager.test_client(KODEX_APPS_MCP_SERVER_NAME);
    assert!(client.client().await.is_err());
    let expected = |status| HashMap::from([(KODEX_APPS_MCP_SERVER_NAME.to_string(), status)]);
    assert_eq!(
    ...
    for status in [
        ...
        tokio::time::advance(KODEX_APPS_RECONNECT_INITIAL_BACKOFF * 2).await;
    }
...
pub(crate) async fn create_test_manager_with_ready_apps_client(
    ...
    let tool = create_test_tool(KODEX_APPS_MCP_SERVER_NAME, tool_name);
    let client = Arc::new(
        RmcpClient::new_in_process_client(Arc::new(RefreshTestTransportFactory {
    ...
    manager.insert_test_client(
        ...
        AsyncManagedClient {
            ...
            .boxed()
            .shared(),
            is_kodex_apps_mcp_server: true,
            server_capabilities: Arc::new(std::sync::Mutex::new(None)),
            cached_server_info: Some(create_test_server_info("Kodex Apps")),
            kodex_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: None,
            cancel_token: CancellationToken::new(),
...
fn create_test_manager_with_failed_apps_startup(
    ...
    .boxed()
    .shared();
    let kodex_home = tempdir().expect("tempdir");
    let cache_context = create_kodex_apps_tools_cache_context(
        kodex_home.path().to_path_buf(),
    ...
    manager.insert_test_client(
        ...
        AsyncManagedClient {
            ...
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: Some(Arc::new(KodexAppsStartupReconnect::new(reconnect_factory))),
            cancel_token: CancellationToken::new(),
        },
    );
    manager
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        kodex_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn assert_elicitation_declined_with_reviewer_calls(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            kodex_rmcp_client::Elicitation::Mcp(elicitation),
...
async fn assert_requested_user_input_is_declined(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            kodex_rmcp_client::Elicitation::Mcp(
    ...
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "kodex_approval_kind": "tool_suggestion",
            })),
...
#[tokio::test]
async fn disabled_permissions_surface_requested_user_input_with_non_kodex_approval_metadata() {
...
async fn assert_disabled_permissions_surface_requested_user_input(
    ...
    let kodex_protocol::mcp::RequestId::String(request_id) = request.id else {
        panic!("expected Kodex-owned string request ID");
    };
...
async fn disabled_permissions_decline_requested_user_input_with_approval_metadata() {
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "kodex_approval_kind": "mcp_tool_call",
...
async fn on_request_approval_forms_remain_with_the_reviewer() {
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "kodex_request_type": "approval_request",
                "kodex_approval_kind": "mcp_tool_call",
                "tool_name": "test_tool",
            })),
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        kodex_protocol::mcp::RequestId::String(request_a_id),
        kodex_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected Kodex-owned string request IDs");
    };
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        kodex_protocol::mcp::RequestId::String(cancelled_id),
        kodex_protocol::mcp::RequestId::String(pending_id),
...
fn test_normalize_tools_selects_raw_server_name() {
    let mut tool = create_test_tool("kodex_apps", "search");
    tool.callable_namespace = "kodex_apps__calendar".to_string();
    ...
    assert_eq!(
        model_tool_names(&model_tools),
        HashSet::from([ToolName::namespaced("kodex_apps__calendar", "search")])
    );
}
...
#[test]
fn kodex_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_kodex_apps_tools_cache(
        KODEX_APPS_MCP_SERVER_NAME,
...
async fn read_only_apps_discovery_never_uses_a_shared_writable_catalog() -> anyhow::Result<()> {
    ...
    let cache_context = cache.context(kodex_home.path().to_path_buf(), cache_key.clone());
    store_current_tools(
        &cache_context,
        vec![create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "write")],
    );
    ...
    for read_only in [false, true] {
        let mut config = crate::mcp::tests::test_mcp_config(kodex_home.path().to_path_buf());
        ...
        let manager = McpConnectionSet::new(
            ...
            McpRuntimeInput {
                ...
                kodex_apps_tools_cache: cache.clone(),
                tool_catalog_cache: McpToolCatalogCache::default(),
                kodex_apps_tools_cache_key: cache_key.clone(),
    ...
    for (generic_mode, hosted_mode) in [
        ...
        let manager = McpConnectionSet::new(
            ...
            McpRuntimeInput {
                ...
                kodex_apps_tools_cache: ConnectorRuntimeManager::default(),
                tool_catalog_cache: McpToolCatalogCache::default(),
                kodex_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                    /*account_id*/ None, /*chatgpt_user_id*/ None,
                ),
                client_mcp_extensions: ClientMcpExtensions::default(),
                auth: None,
        ...
        assert_eq!(
            manager.servers[KODEX_APPS_MCP_SERVER_NAME].protocol_mode,
            hosted_mode
        );
...
#[tokio::test]
async fn kodex_apps_extension_does_not_share_host_owned_tools_cache() -> anyhow::Result<()> {
    let kodex_home = tempdir()?;
    let cache_key = ConnectorRuntimeContextKey::personal(
        /*account_id*/ None, /*chatgpt_user_id*/ None,
    );
    let kodex_apps_tools_cache = ConnectorRuntimeManager::<ToolInfo>::default();
    let cache_context =
        kodex_apps_tools_cache.context(kodex_home.path().to_path_buf(), cache_key.clone());
    store_current_tools(
        &cache_context,
    ...
    for (hosted_mode, extension_mode, expected_mode) in [
        ...
        let manager = McpConnectionSet::new(
            ...
            McpRuntimeInput {
                ...
                kodex_apps_tools_cache: kodex_apps_tools_cache.clone(),
                tool_catalog_cache: McpToolCatalogCache::default(),
                kodex_apps_tools_cache_key: cache_key.clone(),
                client_mcp_extensions: ClientMcpExtensions::default(),
                auth: None,
        ...
        let client = manager.test_client(KODEX_APPS_MCP_SERVER_NAME);
        assert_eq!(
            manager.servers[KODEX_APPS_MCP_SERVER_NAME].protocol_mode, expected_mode,
            "an ordinary extension must use its own mode, not the hosted protocol default"
        );
        assert!(
            client.kodex_apps_tools_cache_context.is_none(),
...
#[tokio::test]
async fn list_all_tools_uses_shared_kodex_apps_cache_while_client_is_pending() {
    let kodex_home = tempdir().expect("tempdir");
    let cache_context = create_kodex_apps_tools_cache_context(
        kodex_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__kodex_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, KODEX_APPS_MCP_SERVER_NAME);
...
async fn capture_binding_uses_the_ready_clients_own_tools() {
    ...
    let mut ready_client = create_test_managed_client(vec![
        create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "client_local_tool"),
        create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "client_local_blocked"),
    ])
    .await;
    ...
    ready_client.kodex_apps_tools_cache_context = Some(cache_context.clone());
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionSet::new_uninitialized(
        &approval_policy,
    ...
    manager
        .servers
        .get_mut(KODEX_APPS_MCP_SERVER_NAME)
        .expect("test server exists")
    ...
    assert!(
        step.prepare_call(KODEX_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        step.prepare_call(KODEX_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    assert!(
        step.prepare_call(KODEX_APPS_MCP_SERVER_NAME, "client_local_blocked")
            .is_none()
    );
}
...
async fn hard_refresh_keeps_client_catalog_local_when_shared_cache_loses_race() -> anyhow::Result<()>
{
    ...
    let cache_context_a = shared_cache.context(kodex_home.path().to_path_buf(), cache_key.clone());
    let cache_context_b = shared_cache.context(kodex_home.path().to_path_buf(), cache_key);
    ...
    let refresh_a = tokio::spawn(async move {
        manager_a_for_refresh
            .refresh_kodex_apps_tools_for_discovery()
            .await
    });
    list_started.notified().await;
    let tools_b = manager_b.refresh_kodex_apps_tools_for_discovery().await?;
    ...
    let refresh_a = tokio::spawn(async move {
        manager_a_for_refresh
            .refresh_kodex_apps_client_catalog(&config_for_refresh)
            .await
    });
    list_started.notified().await;
    manager_b.refresh_kodex_apps_tools_for_discovery().await?;
    release_list.notify_one();
    ...
    let snapshot_b = manager_b.refresh_kodex_apps_client_catalog(&config).await?;
    assert_eq!(
        (
            snapshot_b
                .tools
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<kodex_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    let server_info = create_test_server_info("Kodex Apps");
    manager.insert_test_client(
        KODEX_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
            client: pending_client,
    ...
    assert_eq!(
        server_infos.get(KODEX_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn capture_binding_exposes_cached_tools_before_startup() {
    ...
    let mut cached_tool = create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "shared_cached_tool");
    ...
    assert!(
        cached_binding
            .prepare_call(KODEX_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    ...
    let startup = tokio::spawn(async move {
        manager_for_startup
            .wait_for_server_startup(KODEX_APPS_MCP_SERVER_NAME)
            .await
    });
...
async fn capture_binding_resolves_concurrently_and_rechecks_cached_clients() {
    ...
    apps_client.is_kodex_apps_mcp_server = true;
    apps_client.kodex_apps_tools_cache_context = Some(cache_context);
    let first_client =
    ...
    manager.insert_test_client(KODEX_APPS_MCP_SERVER_NAME, apps_client);
    ...
    tokio::time::timeout(Duration::from_secs(1), apps_started)
        .await
        .expect("Kodex Apps startup should begin")
        .expect("signal Kodex Apps startup");
    ...
    release_apps.send(()).expect("release Kodex Apps startup");
    assert!(startup.await.expect("Kodex Apps startup task"));
    ...
    assert!(
        binding
            .prepare_call(KODEX_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        binding
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_kodex_apps_cache_is_empty() {
    let kodex_home = tempdir().expect("tempdir");
    let cache_context = create_kodex_apps_tools_cache_context(
        kodex_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_kodex_apps_cache_when_client_startup_fails() {
    let kodex_home = tempdir().expect("tempdir");
    let cache_context = create_kodex_apps_tools_cache_context(
        kodex_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_kodex_apps_startup_and_reuses_client() {
    ...
    let step = capture_binding(&manager).await;
    let prepared = step
        .prepare_call(KODEX_APPS_MCP_SERVER_NAME, "drive_search")
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(KODEX_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_kodex_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        KODEX_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::kodex_apps_mcp_server_config(
        "https://docs.example",
...
fn hosted_actor_credentials_are_only_available_to_host_owned_mcp_servers() {
    let bootstrap_auth = KodexAuth::create_dummy_chatgpt_auth_for_testing();
    let mut actor_headers = Default::default();
    kodex_model_provider::auth_provider_from_auth(&bootstrap_auth)
    ...
    let hosted_auth = KodexAuth::Headers(AuthHeaders::new(actor_headers));
    let provider = kodex_model_provider::auth_provider_from_auth(&hosted_auth);
    let mut local_config = crate::kodex_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    ...
    let local_provider =
        chatgpt_auth_provider_for_server(&local_server, Some(Arc::clone(&provider)))
            .expect("host-owned Kodex Apps must retain hosted authentication");
    assert_eq!(
        local_provider
            .to_auth_headers()
...
async fn executor_owned_chatgpt_mcp_accepts_only_safe_explicit_authorization() -> anyhow::Result<()>
{
    ...
    let hosted_auth = KodexAuth::Headers(AuthHeaders::new(actor_headers));
    let runtime_config = crate::mcp::tests::test_mcp_config(kodex_home.path().to_path_buf());
    let cases = [
        ...
        (
            ...
            None,
            Some(("aUtHoRiZaTiOn", "KODEX_TEST_HOSTED_SECRET")),
            false,
        ),
        (
            "environment bearer",
            Some(("Authorization", "Bearer executor-secret")),
            Some("KODEX_TEST_HOSTED_SECRET"),
            None,
            false,
        ),
        (
    ...
    for (case, static_header, bearer_env_var, env_header, allows_executor_auth) in cases {
        ...
        assert!(
            chatgpt_auth_provider_for_server(
                remote_server,
                Some(kodex_model_provider::auth_provider_from_auth(&hosted_auth)),
            )
            .is_none(),
        ...
        let connection_identity = |keyring_backend_kind| {
            McpServerConnectionIdentity::new(
                ...
                Some(&hosted_auth),
                /*kodex_apps_cache_identity*/ None,
                ElicitationCapability::default(),
                ClientMcpExtensions::default(),
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            kodex_apps_tools_cache: ConnectorRuntimeManager::<ToolInfo>::default(),
            tool_catalog_cache: McpToolCatalogCache::default(),
            kodex_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                /*account_id*/ None, /*chatgpt_user_id*/ None,
            ),
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = KODEX_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `kodex mcp login {server_name}`."
    );
...
fn mcp_init_error_display_identifies_oauth_reauthentication() {
    ...
    for (config, recovery_hint) in [
        (None, "Run `kodex mcp login example`."),
        (
            Some(&executor_config),
...
fn reusable_server_config(url: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: Some("KODEX_MCP_REUSE_TEST_TOKEN".to_string()),
            http_headers: None,
            env_http_headers: None,
            http_headers_helper: None,
        },
...
async fn reconcile_reusable_server(
    ...
    reconcile_reusable_server_with_mcp_config(
        ...
        config,
        runtime_context,
        crate::mcp::tests::test_mcp_config(kodex_home.path().to_path_buf()),
    )
    .await
}
...
async fn read_only_policy_does_not_reuse_a_writable_connection() {
    ...
    .await;
    for read_only in [false, true] {
        let mut mcp_config = crate::mcp::tests::test_mcp_config(kodex_home.path().to_path_buf());
...
-> anyhow::Result<()> {
    let kodex_home = tempdir()?;
    let context = create_kodex_apps_tools_cache_context(
        kodex_home.path().to_path_buf(),
        /*account_id*/ None,
        /*chatgpt_user_id*/ None,
    )
    .with_live_scope("apps".to_string());
    let original = vec![create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "original")];
    let updated = vec![create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "updated")];
...
async fn apps_catalog_broadcast_restores_equivalent_calls() -> anyhow::Result<()> {
    ...
    .with_live_scope("apps".to_string());
    let original = vec![
        create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "first"),
        create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "second"),
    ];
    store_current_tools(&context, original.clone());
    let catalog = ClientToolCatalog::new(original.clone(), context.subscribe());
    let snapshot = catalog.read(Arc::new).await;
...
fn idle_apps_clients_do_not_retain_replaced_tools() {
    ...
    let tool = create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "original");
    ...
    store_current_tools(
        &context,
        vec![create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "updated")],
    );
    assert!(
...
async fn apps_catalog_broadcast_survives_an_older_local_refresh() -> anyhow::Result<()> {
    ...
    let newer = vec![create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "newer")];
...
-> anyhow::Result<()> {
    ...
    let startup_tools = vec![create_test_tool(KODEX_APPS_MCP_SERVER_NAME, "startup")];
    client
        .tool_catalog
    ...
    let old_call = old_binding
        .prepare_call(KODEX_APPS_MCP_SERVER_NAME, "startup")
        .expect("startup call");
    let unrelated_call = old_binding
        .prepare_call("docs", "unrelated")
        .expect("unrelated call");

    previous.refresh_kodex_apps_tools_for_discovery().await?;
    ...
    assert!(previous.shares_test_connection_with(&republished, KODEX_APPS_MCP_SERVER_NAME));
    assert_eq!(
        model_tool_names(capture_binding(&republished).await.tools()),
        HashSet::from([ToolName::namespaced("mcp__kodex_apps", "refreshed")]),
    );
...
async fn reconciliation_reuses_connection_without_relisting_regular_tools() -> anyhow::Result<()> {
    ...
    let initial_tools = list_tools_for_client_uncached(
        "docs",
        /*is_kodex_apps_mcp_server*/ false,
        /*kodex_apps_refresh_trigger*/ "test",
        &client,
        /*timeout*/ None,
        crate::pagination::MAX_MCP_CATALOG_ITEMS,
...
fn connection_identity_uses_effective_authorization_headers() {
    let runtime_context = reusable_server_runtime_context();
    let missing_env_var = format!("KODEX_TEST_UNSET_MCP_AUTHORIZATION_{}", std::process::id());
...
async fn reconciliation_reuses_legacy_stdio_server_with_existing_protocol_marker() {
    ...
    config.transport = McpServerTransportConfig::Stdio {
        ...
        env: Some(HashMap::from([(
            "KODEX_MCP_PROTOCOL_VERSION".to_string(),
...
async fn reconciliation_replaces_connection_when_auth_mode_changes() -> anyhow::Result<()> {
    ...
    let kodex_home = tempdir()?;
    let mcp_config = crate::mcp::tests::test_mcp_config(kodex_home.path().to_path_buf());
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    let kodex_home = tempdir().expect("tempdir");
    let mut mcp_config = crate::mcp::tests::test_mcp_config(kodex_home.path().to_path_buf());
    mcp_config.protocol_mode = kodex_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let previous_auth = KodexAuth::from_external_chatgpt_tokens(
        access_token,
        "account-a",
        /*chatgpt_plan_type*/ None,
    )?;
    let changed_auth = KodexAuth::from_external_chatgpt_tokens(
        access_token,
        "account-b",
        /*chatgpt_plan_type*/ None,
    )?;
    let connection_identity = |auth: &KodexAuth| {
        let provider = kodex_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            /*host_plugin_root*/ None,
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = kodex_login::auth::AgentIdentityAuthRecord {
        ...
        plan_type: kodex_protocol::account::PlanType::Plus,
        chatgpt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = kodex_login::test_support::transport_default_auth_route_config();
    let previous_auth = KodexAuth::AgentIdentity(
        kodex_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        kodex_login::auth::AgentIdentityAuthRecord {
        ...
        let changed_auth = KodexAuth::AgentIdentity(
            kodex_login::auth::AgentIdentityAuth::from_record(
                changed_record,
                "https://auth.openai.com/api/accounts",
                &auth_route_config,
