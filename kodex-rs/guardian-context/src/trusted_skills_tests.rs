...
fn bounds_escaped_skill_paths_without_corrupting_json_or_utf8() {
    let paths = (0..16)
        .map(|index| {
            format!(
                "/home/user/.kodex/skills/{index:03}/{}SKILL.md",
                "\u{0001}é".repeat(80)
            )
        })
        .collect::<Vec<_>>();
...
fn preserves_multiple_invoked_skill_paths() {
    assert_eq!(
        rendered_paths(vec![
            "/home/user/.kodex/skills/first/SKILL.md".to_owned(),
            "/home/user/.kodex/skills/second/SKILL.md".to_owned(),
        ]),
        vec![
            "/home/user/.kodex/skills/first/SKILL.md",
            "/home/user/.kodex/skills/second/SKILL.md",
        ],
    );
}
