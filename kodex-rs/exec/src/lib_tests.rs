...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("kodex-exec-tests");
...
fn exec_default_stderr_filter_suppresses_otel_self_diagnostics() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::error!(target: "kodex_exec_test", "real exec error");
    });
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = kodex_protocol::protocol::W3cTraceContext {
...
fn runtime_warnings_are_filtered_to_the_primary_thread() {
    ...
    let turn_id = "turn-1";
    let outcomes = [
        kodex_app_server_protocol::WarningNotification {
            thread_id: None,
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let kodex_home = tempdir().expect("create temp kodex home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        originator: None,
        source: kodex_app_server_protocol::SessionSource::Exec,
        ...
        name: None,
        turns: vec![
            kodex_app_server_protocol::Turn {
                id: "turn-1".to_string(),
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(kodex_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
async fn thread_start_params_match_history_to_persistence() {
    ...
    assert_eq!(
        ...
        Some(kodex_app_server_protocol::ThreadSource::User)
    );
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(kodex_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        ...
        approval_policy: kodex_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: kodex_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: kodex_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
