set working-directory := "kodex-rs"
set positional-arguments
export KODEX_REPO_ROOT := justfile_directory()
...
# Start `kodex exec-server` and run kodex-tui.
[no-cd]
...
file-search *args:
    cargo run --bin kodex-file-search -- {args}

# Run the standalone code-mode host from source.
code-mode-host *args:
    cargo run --bin kodex-code-mode-host -- {args}

# Assemble a local Kodex package.
[no-cd]
assemble-kodex-package *args:
    {{ python }} {{ justfile_directory() }}/scripts/build_kodex_package.py {args}

# Build the CLI and run the app-server test client
app-server-test-client *args:
    cargo build -p kodex-cli
    cargo run -p kodex-app-server-test-client -- --kodex-bin ./target/debug/kodex {args}
...
bench-e2e:
    ...
    bazel test --compilation_mode=opt --cache_test_results=no --test_output=streamed //kodex-rs:e2e-benchmarks
...
bench-e2e-smoke:
    ...
    bazel test --compilation_mode=fastbuild --@rules_rust//rust/settings:extra_rustc_flag=-Cdebug-assertions=no --@rules_rust//rust/settings:extra_exec_rustc_flag=-Cdebug-assertions=no --cache_test_results=no --test_output=streamed --test_arg=--test //kodex-rs:e2e-benchmarks

# Build and run Kodex from source using Bazel.
# On Unix, use `[no-cd]` and `--run_under="cd $PWD &&"` to ensure Bazel runs
# the command in the current working directory.
[no-cd]
[unix]
bazel-kodex *args:
    bazel run //kodex-rs/cli:kodex --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-kodex *args:
    bazel run //kodex-rs/cli:kodex --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)

# Build and run the standalone code-mode host from source using Bazel.
[no-cd]
[unix]
bazel-code-mode-host *args:
    bazel run //kodex-rs/code-mode-host:kodex-code-mode-host --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-code-mode-host *args:
    bazel run //kodex-rs/code-mode-host:kodex-code-mode-host --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)
...
build-for-release:
    bazel build //kodex-rs/cli:release_binaries

# Regenerate the json schema for config.toml from the current config types.
write-config-schema:
    cargo run -p kodex-config-schema --bin kodex-write-config-schema
...
[no-cd]
write-hooks-schema:
    cargo run --manifest-path {{ justfile_directory() }}/kodex-rs/Cargo.toml -p kodex-hooks --bin write_hooks_schema_fixtures

# Run the argument-comment Dylint checks across kodex-rs.
[no-cd]
[unix]
...
[unix]
log *args:
    if [ "${1:-}" = "--" ]; then shift; fi; cargo run -p kodex-cli --bin logs_client -- "$@"

[windows]
log *args:
    $forwarded_args = @($args | Select-Object -Skip 1); if ($forwarded_args.Count -gt 0 -and $forwarded_args[0] -eq "--") { $forwarded_args = @($forwarded_args | Select-Object -Skip 1) }; cargo run -p kodex-cli --bin logs_client -- @forwarded_args
