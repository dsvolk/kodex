...
fn invalid(message: impl Into<String>) -> KodexErr {
    KodexErr::InvalidRequest(message.into())
}

fn storage_error(error: impl std::fmt::Display) -> KodexErr {
    KodexErr::Io(std::io::Error::other(error.to_string()))
}
