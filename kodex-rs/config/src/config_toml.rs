//! Schema-heavy configuration TOML types used by Kodex.
...
use kodex_utils_path_uri::Platform;
...
/// Base config deserialized from ~/.kodex/config.toml.
...
pub struct ConfigToml {
    ...
    /// Allow macOS sandbox writable roots at or beneath KODEX_HOME to traverse
    ...
    /// they change between commands or lie outside KODEX_HOME.
    /// This setting has no effect on Linux or Windows.
    pub allow_symlinked_kodex_home: Option<bool>,
    ...
    /// sanctioned by Kodex will likely degrade model performance.
    ...
    /// file: Use a file in the Kodex home directory.
    ...
    /// Directory where Kodex stores the SQLite state DB.
    /// Defaults to `$KODEX_SQLITE_HOME` when set. Otherwise uses `$KODEX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Kodex writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$KODEX_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Kodex Apps MCP requests.
    ...
    /// directories for `.kodex` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use kodex_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use kodex_protocol::protocol::RealtimeVoice;
