...
pub(crate) async fn check_pending(session: &Session, turn: &TurnContext) -> KodexResult<()> {
    ...
    let minimum_prefix =
        kodex_protocol::protocol::TruncationPolicy::Bytes(base.text.len()).token_budget();
...
pub(crate) async fn finalize(
    ...
    let request = session.services.model_client.build_responses_request(
        ...
        /*service_tier*/ None,
        &session
            .responses_metadata(step, KodexResponsesRequestKind::Turn)
            .await,
    )?;
