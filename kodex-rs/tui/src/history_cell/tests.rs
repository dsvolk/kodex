...
use kodex_otel::RuntimeMetricTotals;
    ...
    let target = crate::AppServerTarget::Remote {
        endpoint: crate::RemoteAppServerEndpoint::UnixSocket {
            socket_path: AbsolutePathBuf::from_absolute_path(
                std::env::temp_dir().join("kodex.sock"),
            )
            .expect("absolute socket path"),
        },
    };
    let settings = kodex_config::types::Tui {
...
async fn test_config() -> Config {
    let kodex_home = std::env::temp_dir();
...
fn composite_cell_preserves_child_web_links() {
    let destination = "https://chatgpt.com/kodex/settings/usage";
...
fn empty_mcp_output_preserves_docs_hyperlink() {
    let destination = "https://developers.openai.com/kodex/mcp";
...
async fn session_info_preserves_styled_tooltip_links() {
    ...
    let cell = new_session_info(
        ...
        /*is_first_event*/ false,
        Some(
            "Use **/copy** or `ctrl+y`; visit the [Kodex community forum](https://example.com)."
                .to_string(),
        ),
        Some(PlanType::Free),
        /*show_fast_status*/ false,
...
fn mcp_tools_output_from_statuses_renders_status_only_servers() {
    let statuses = vec![McpServerStatus {
        ...
        auth_status: kodex_app_server_protocol::McpAuthStatus::Unknown,
    }];
...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved kodex to".to_string(),
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p kodex-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),
