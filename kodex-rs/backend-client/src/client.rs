...
use crate::types::KodexUserSettingsResponse;
use crate::types::KodexWorkspaceMessagesResponse;
...
use kodex_protocol::protocol::SpendControlLimitSnapshot;
use serde::Serialize;
...
pub enum RequestError {
    Policy(kodex_http_client::NetworkPolicyDenied),
    UnexpectedStatus {
        method: String,
        url: String,
        status: StatusCode,
...
impl From<kodex_http_client::HttpError> for RequestError {
    fn from(error: kodex_http_client::HttpError) -> Self {
        match error {
            kodex_http_client::HttpError::Policy(denied) => Self::Policy(denied),
            error @ (kodex_http_client::HttpError::Request(_)
            | kodex_http_client::HttpError::Route(_)
            | kodex_http_client::HttpError::Build(_)
            | kodex_http_client::HttpError::UnsupportedRedirectScheme(_)
            | kodex_http_client::HttpError::TooManyRedirects
            | kodex_http_client::HttpError::Timeout) => Self::Other(error.into()),
        }
    }
}
...
impl Client {
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_kodex_user_agent())
            .with_auth_provider(kodex_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("kodex-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("kodex"))
            .cloned();
    ...
    pub async fn get_accounts_check(
        ...
        let url = match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::KodexApi => format!(
                "{}/api/kodex/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from kodex-backend.
    ///
    /// `GET /api/kodex/config/bundle` (Kodex API style) or
    ...
    pub fn config_bundle_url(&self) -> String {
        match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/config/bundle", self.base_url),
            PathStyle::ChatGptApi => format!("{}/wham/config/bundle", self.base_url),
        }
    }

    /// Fetch authenticated Kodex user settings from the active backend route.
    ///
    /// Uses `GET /api/kodex/settings/user` for Kodex API hosts and
    /// `GET /wham/settings/user` for ChatGPT `backend-api` hosts.
    pub async fn get_user_settings(
        &self,
    ) -> std::result::Result<KodexUserSettingsResponse, RequestError> {
        ...
        self.decode_json::<KodexUserSettingsResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }

    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<KodexWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<KodexWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/tasks", self.base_url),
    ...
    fn make_additional_rate_limit_snapshot(
        details: kodex_backend_openapi_models::models::AdditionalRateLimitDetails,
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<kodex_backend_openapi_models::models::SpendControlStatusDetails>,
        plan_type: Option<AccountPlanType>,
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::KodexApi => format!(
                "{}/api/kodex/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/workspace-messages", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::KodexApi => format!("{}/api/kodex/settings/user", self.base_url),
...
mod tests {
    use super::*;
    use kodex_backend_openapi_models::models::AdditionalRateLimitDetails;
    use kodex_backend_openapi_models::models::RateLimitReachedKind;
    use kodex_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "kodex_other".to_string(),
                metered_feature: "kodex_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                kodex_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("kodex"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("kodex_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("kodex_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("kodex"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let kodex_client = test_client("https://example.test", PathStyle::KodexApi);
        assert_eq!(
            kodex_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/kodex/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let kodex_client = test_client("https://example.test", PathStyle::KodexApi);
        assert_eq!(
            kodex_client.token_usage_profile_url(),
            "https://example.test/api/kodex/profiles/me"
        );
    ...
    fn workspace_messages_uses_expected_paths() {
        let kodex_client = test_client("https://example.test", PathStyle::KodexApi);
        assert_eq!(
            kodex_client.workspace_messages_url(),
            "https://example.test/api/kodex/workspace-messages"
        );
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        let kodex_response = Client::new(
            server.uri(),
            HttpClientFactory::new(kodex_http_client::OutboundProxyPolicy::ReqwestDefault),
        )
        .get_user_settings()
        .await
        .unwrap();
        ...
        assert_eq!(
            [kodex_response, chatgpt_response],
            [
                KodexUserSettingsResponse {
    ...
    fn user_settings_missing_attribution_policy_defaults_to_disabled() {
        assert_eq!(
            serde_json::from_value::<KodexUserSettingsResponse>(serde_json::json!({})).unwrap(),
            KodexUserSettingsResponse {
                commit_attribution_enabled: false,
            }
        );
