...
use anyhow::Result;
use anyhow::ensure;
use kodex_windows_sandbox::APP_CORE_RUNNER_ALIAS;
use kodex_windows_sandbox::SandboxRuntimeAccount;
use kodex_windows_sandbox::logon_existing_sandbox_account;
use kodex_windows_sandbox::resolve_sid as lookup_sid;
use kodex_windows_sandbox::sandbox_secrets_dir;
use kodex_windows_sandbox::string_from_sid_bytes;
use kodex_windows_sandbox::to_wide;
use kodex_windows_sandbox::token_groups;
    ...
    for account in [
        ...
        let token = with_owner_impersonation(identity.token.0, || {
            ...
            crate::ipc::pin_existing_ancestors(
                &sandbox_secrets_dir(&identity.kodex_home),
                &mut pins,
            )?;
            logon_existing_sandbox_account(&identity.kodex_home, account)
        })?;
        let expected = record
            .runtime()?
            .accounts
...
pub(crate) fn prepare_cleanup(record: InstallationRecord) -> Result<InstallationRecord> {
    ...
    for account in &record.runtime()?.accounts {
        if kodex_windows_sandbox::local_user_flags(account.account.username())?.is_some() {
...
fn validate_target(
    ...
    let user = unsafe { kodex_windows_sandbox::get_user_sid_bytes(token) }?;
    ...
    let sandbox_group = lookup_sid(kodex_windows_sandbox::SANDBOX_USERS_GROUP)?;
    ...
    for group in groups {
        ...
        sandbox_member,
        "runtime account is not in KodexSandboxUsers"
    );
    Ok(user_sid)
}
