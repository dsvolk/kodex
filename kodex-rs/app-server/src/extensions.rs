...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn kodex_skills_extension::SkillProvider>,
...
pub(crate) fn thread_extensions(
    ...
    if let Some(queue_service) = queue_service {
        kodex_queue_extension::install(&mut builder, queue_service);
    }
    kodex_history_notes_extension::install(&mut builder, auth_manager.clone());
    kodex_core::install_agent_message_board(&mut builder, thread_manager.clone());
    if let Some(state_db) = state_db {
        kodex_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            kodex_otel::global(),
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config.features.enabled(kodex_features::Feature::Goals),
    ...
    kodex_guardian_v2::install(&mut builder, auth_manager.clone(), thread_manager);
    kodex_memories_extension::install(&mut builder, kodex_otel::global());
    kodex_mcp_extension::install(&mut builder);
    kodex_mcp_extension::install_plugins(&mut builder, environment_manager);
    kodex_web_search_extension::install(&mut builder, auth_manager.clone());
    kodex_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.kodex_home.clone())
    });
    let skill_providers = kodex_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_host_provider(Arc::new(kodex_skills_extension::HostSkillProvider::new()));
    kodex_skills_extension::install_with_providers_and_metrics(
        &mut builder,
        skill_providers,
        kodex_otel::global(),
        |config: &Config| kodex_skills_extension::SkillsExtensionConfig {
...
#[cfg(test)]
mod tests {
    use kodex_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use kodex_protocol::protocol::ThreadGoalStatus;
    use kodex_protocol::protocol::ThreadGoalUpdatedEvent;
