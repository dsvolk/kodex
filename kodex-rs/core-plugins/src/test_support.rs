...
use kodex_skills::SkillLoadFuture;
...
pub(crate) fn test_plugins_manager(kodex_home: PathBuf) -> PluginsManager {
    PluginsManager::new(
        kodex_home,
...
pub(crate) fn test_auth_manager(auth_mode: Option<AuthMode>) -> Arc<AuthManager> {
    auth_manager_from_optional_auth(test_kodex_auth(auth_mode))
}

pub(crate) async fn set_test_auth_mode(auth_manager: &AuthManager, auth_mode: Option<AuthMode>) {
    set_test_auth(auth_manager, test_kodex_auth(auth_mode)).await;
}

pub(crate) async fn set_test_auth(auth_manager: &AuthManager, auth: Option<KodexAuth>) {
...
fn test_kodex_auth(auth_mode: Option<AuthMode>) -> Option<KodexAuth> {
    auth_mode.map(|auth_mode| match auth_mode {
        AuthMode::ApiKey => KodexAuth::from_api_key("test-api-key"),
        AuthMode::Chatgpt => KodexAuth::create_dummy_chatgpt_auth_for_testing(),
        AuthMode::ChatgptAuthTokens => KodexAuth::from_external_chatgpt_tokens(
            "header.e30.test",
            "test-account",
            /*chatgpt_plan_type*/ None,
        )
        .expect("test ChatGPT tokens should parse"),
        AuthMode::Headers => KodexAuth::Headers(AuthHeaders::new(http::HeaderMap::new())),
        AuthMode::BedrockApiKey => KodexAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "test-api-key".to_string(),
            region: "us-east-1".to_string(),
        }),
        AuthMode::BedrockAccessKeys => KodexAuth::BedrockAccessKeys(BedrockAccessKeysAuth {
        ...
        AuthMode::AgentIdentity | AuthMode::PersonalAccessToken => {
            panic!("test auth mode requires a purpose-built KodexAuth")
        }
    })
}
...
pub(crate) fn write_curated_plugin_sha_with(kodex_home: &Path, sha: &str) {
    write_file(&kodex_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(kodex_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let kodex_home = AbsolutePathBuf::try_from(kodex_home).expect("kodex home should be absolute");
