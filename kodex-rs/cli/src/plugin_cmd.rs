...
use kodex_core_plugins::ConfiguredMarketplace;
use kodex_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use kodex_core_plugins::PluginInstallOutcome;
...
use kodex_core_plugins::allowed_configured_marketplace_names;
use kodex_core_plugins::installed_marketplaces::marketplace_install_root;
use kodex_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use kodex_core_plugins::marketplace::MarketplaceListError;
use kodex_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use kodex_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use kodex_core_plugins::marketplace::MarketplacePluginSource;
use kodex_core_plugins::marketplace::find_marketplace_manifest_path;
use kodex_core_plugins::remote;
...
use kodex_core_plugins::remote::RemotePluginSummary;
use kodex_login::AuthManager;
use kodex_login::KodexAuth;
use kodex_plugin::PluginId;
use kodex_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "kodex plugin add",
    after_help = "Examples:\n  kodex plugin add sample@debug\n  kodex plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "kodex plugin list",
    after_help = "Examples:\n  kodex plugin list\n  kodex plugin list --marketplace debug\n  kodex plugin list --json\n  kodex plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "kodex plugin remove",
    after_help = "Examples:\n  kodex plugin remove sample@debug\n  kodex plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources = configured_marketplace_sources(&plugins_input, kodex_home.as_path());
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, kodex_home);
...
pub(crate) async fn load_cli_auth_manager(config: &Config) -> Result<Arc<AuthManager>> {
    Ok(AuthManager::shared_from_config(config, /*enable_kodex_api_key_env*/ true).await?)
}
...
async fn fetch_remote_marketplaces(
    ...
    let auth = context.auth.as_ref();
    if !auth.is_some_and(KodexAuth::uses_kodex_backend) {
        ensure!(
    ...
    let result = if context.plugins_input.remote_plugin_enabled {
        remote::fetch_remote_marketplaces(
            ...
            /*catalog_cache_root*/ Some(context.kodex_home.as_path()),
            cache_mode,
        )
        .await
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(kodex_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, kodex_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
