...
const X_KODEX_IMAGEGEN_REQUEST_ID_HEADER: &str = "x-kodex-imagegen-request-id";
...
fn imagegen_request_id_from_headers(headers: &HeaderMap) -> Option<String> {
    headers
        .get(X_KODEX_IMAGEGEN_REQUEST_ID_HEADER)
...
mod tests {
    ...
    fn provider() -> Provider {
        Provider {
            name: "test".to_string(),
            base_url: "https://example.com/api/kodex".to_string(),
            query_params: None,
    ...
    async fn generate_posts_typed_request_and_parses_image_response() {
        ...
        response_headers.insert(
            X_KODEX_IMAGEGEN_REQUEST_ID_HEADER,
        ...
        assert_eq!(
            request.url,
            "https://example.com/api/kodex/images/generations"
        );
        assert_eq!(
    ...
    async fn edit_posts_typed_request_and_parses_image_response() {
        ...
        let request = captured_request(&transport);
        assert_eq!(request.url, "https://example.com/api/kodex/images/edits");
        assert_eq!(
            request.body.as_ref().and_then(RequestBody::json),
            Some(&json!({
