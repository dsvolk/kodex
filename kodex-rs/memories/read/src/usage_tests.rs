...
fn classifies_reads_in_both_memory_versions() {
    for version in [MemoryVersion::V1, MemoryVersion::V2] {
        ...
        for (reader, root) in [
            ("cat", format!("/tmp/.kodex/{root}")),
            ("Get-Content", format!(r"C:\Users\test\.kodex\{root}")),
        ] {
            let command = format!(
...
fn attributes_mixed_version_reads_to_their_roots() {
    let command = "cat /tmp/.kodex/memories/MEMORY.md \
        && sed -n '1,20p' /tmp/.kodex/memories_v2/rollout_summaries/thread.md \
        && cat /tmp/.kodex/memories/raw_memories.md \
        && cat /tmp/.kodex/memories_v2/skills/testing/SKILL.md";
    assert_eq!(
        memories_usage_from_command(command),
        vec![
