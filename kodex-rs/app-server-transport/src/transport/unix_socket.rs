...
const MAX_UNFRAGMENTED_MESSAGE_BYTES_HEADER: &str =
    "x-kodex-websocket-max-unfragmented-message-bytes";
...
pub async fn start_control_socket_acceptor(
    ...
    let (socket_path, rendezvous_path, _startup_lock) = {
        ...
        kodex_uds::prepare_shared_daemon_socket_directory()?;
    ...
    let (socket_path, directory_guard) = {
        if let Some(parent) = socket_path.as_path().parent() {
            kodex_uds::prepare_private_socket_directory(parent).await?;
        }
        let (path, guard) = kodex_uds::validate_private_socket_path(socket_path.as_path())?;
...
async fn prepare_control_socket_path(socket_path: &Path) -> IoResult<()> {
    #[cfg(windows)]
    let (socket_path, _directory_guard) = kodex_uds::validate_private_socket_path(socket_path)?;
    #[cfg(windows)]
    ...
    if !kodex_uds::is_stale_socket_path(socket_path).await? {
        return Err(std::io::Error::new(
...
fn protected_socket_path(rendezvous_path: &Path) -> IoResult<std::path::PathBuf> {
    ...
    Ok(kodex_uds::shared_daemon_socket_directory()?.join(format!("{hash:x}")))
}
