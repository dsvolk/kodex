...
async fn host_drain_rejects_turn_start_paths_without_recording_input() -> anyhow::Result<()> {
    ...
    for request in [
        ...
        assert_eq!(
            test.kodex.start_turn_if_idle(request).await?,
...
async fn host_drain_closes_realtime_after_handoff_error() -> anyhow::Result<()> {
    ...
    let test = test_kodex()
        ...
        .with_config({
            ...
            move |config| {
                ...
                config.realtime.version = kodex_config::config_toml::RealtimeWsVersion::V1;
            }
        })
        .build_with_auto_env(&server)
        .await?;
    ...
    for expected in [
        ...
        let event = wait_for_event(&test.kodex, |event| match event {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn submit_user_message(
    kodex: &kodex_core::KodexThread,
    text: &str,
) -> kodex_protocol::error::Result<TurnInputSubmission> {
    kodex.start_or_steer_turn(user_message_request(text)).await
}
...
async fn start_turn_if_idle_keeps_automatic_plan_rejections_atomic(
    ...
    let mut collaboration_mode = test.kodex.config_snapshot().await.collaboration_mode;
    ...
    let current_settings = test.kodex.thread_settings_snapshot().await;
    ...
    .await;
    assert_eq!(
        test.kodex.config_snapshot().await.collaboration_mode,
        collaboration_mode
    );
...
async fn recover_turn_if_idle_preserves_id_and_resumes_plan_mode() {
    ...
    assert_eq!(
        test.kodex.config_snapshot().await.collaboration_mode.mode,
        ModeKind::Plan
    );

    let started = wait_for_event(&test.kodex, |event| {
...
async fn continue_turn_if_idle_starts_new_turn_with_internal_input() {
    ...
    let test = test_kodex()
        ...
        .with_config(|config| {
            config
                .features
                .enable(kodex_features::Feature::FastMode)
                .unwrap();
        })
        .build_with_auto_env(&server)
        .await
    ...
    wait_for_event(&test.kodex, |event| {
        ...
        assert!(!matches!(event, EventMsg::ItemCompleted(event)
            if matches!(event.item, kodex_protocol::items::TurnItem::UserMessage(_))));
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_input_submission_reports_started_and_steered_for_concurrent_submissions() {
    ...
    let first_submission = tokio::spawn({
        let kodex = Arc::clone(&kodex);
        let barrier = Arc::clone(&barrier);
        async move {
            barrier.wait().await;
            submit_user_message(kodex.as_ref(), "first message").await
        }
    });
    let second_submission = tokio::spawn({
        ...
        async move {
            barrier.wait().await;
            submit_user_message(kodex.as_ref(), "second message").await
        }
    });
    barrier.wait().await;
    ...
    wait_for_event(kodex.as_ref(), |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_input_submission_applies_thread_settings_only_after_accepted_input() {
    ...
    let started = submit_user_message(kodex, "start turn")
        .await
    ...
    assert_eq!(
        kodex.config_snapshot().await.approval_policy,
        AskForApproval::Never
    );
    assert_eq!(
        kodex.environment_selections().await,
...
async fn sampling_is_ready_for_daemon_recovery(
    ...
    .await?;
    let active = test.kodex.interrupted_turn().await;
    assert_eq!(
