...
./scripts/stage_npm_packages.py \
  ...
  --package kodex \
  --package kodex-responses-api-proxy \
  --package kodex-sdk
```
...
When `--package kodex` is provided, the staging helper builds the lightweight
`@openai/kodex` meta package plus all platform-native `@openai/kodex` variants
