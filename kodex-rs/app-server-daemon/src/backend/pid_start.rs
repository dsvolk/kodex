...
impl PidBackend {
    pub(super) async fn start_inner(&self, replacement: Option<PidRecord>) -> Result<Option<u32>> {
        ...
        let kodex_bin = fs::canonicalize(&self.kodex_bin)
            .await
            .unwrap_or_else(|_| self.kodex_bin.clone());
        let launched_identity =
            if matches!(self.command_kind, super::PidCommandKind::AppServer { .. }) {
                executable_identity(&kodex_bin).await.ok()
            } else {
                None
            };
            ...
            )
        {
            ...
            let kodex_home = self
                ...
                .context("daemon pid path has no Kodex home")?;
            let recovery_file = kodex_app_server_transport::daemon_recovery_file_path(kodex_home);
        ...
        #[cfg(windows)]
        {
            ...
            for name in [
                ...
                if matches!(name, "KODEX_SQLITE_HOME" | "npm_config_cafile") {
                ...
                let expands_home = if name == "npm_config_cafile" {
                    ...
                    matches!(
                        name,
                        "KODEX_SQLITE_HOME" | "AWS_CONFIG_FILE" | "AWS_SHARED_CREDENTIALS_FILE"
            ...
            match self.command_kind {
                PidCommandKind::AppServer { .. } => {
                    command.env(kodex_app_server_transport::DAEMON_SHUTDOWN_SOCKET_ENV, "1");
                }
                PidCommandKind::UpdateLoop { .. } => {
                    ...
                    command.env(
                        kodex_app_server_transport::DAEMON_SHUTDOWN_FILE_ENV,
                        shutdown_file,
                    );
                }
            }
