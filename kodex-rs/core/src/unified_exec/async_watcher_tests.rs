...
async fn streaming_output_harness() -> anyhow::Result<StreamingOutputHarness> {
    ...
    let spawned = kodex_utils_pty::spawn_from_driver(kodex_utils_pty::ProcessDriver {
        writer_tx,
        stdout_rx,
        stderr_rx: None,
        exit_rx,
...
async fn exit_watcher_waits_for_late_network_denial_before_classifying_end() -> anyhow::Result<()> {
    ...
    model_info.truncation_policy = kodex_protocol::openai_models::TruncationPolicyConfig {
    ...
    assert_eq!(
        ...
        Some(kodex_utils_output_truncation::formatted_truncate_text(
            "LATE_DENIAL",
            kodex_utils_output_truncation::TruncationPolicy::Bytes(4),
        ))
    );
    assert!(
