...
use kodex_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR;
...
async fn start_retries_stale_empty_pid_file_under_its_own_lock() {
    ...
    kodex_uds::prepare_private_socket_directory(&state_dir)
        .await
    ...
    tokio::fs::write(&pid_file, "")
        ...
        .expect("write pid file");
    let backend = PidBackend::new(
        temp_dir.path().join("missing-kodex"),
        pid_file,
        /*remote_control_enabled*/ false,
    );
...
async fn legacy_launch_clears_recovery_best_effort() {
    for snapshot_is_directory in [false, true] {
        ...
        let recovery_file = kodex_app_server_transport::daemon_recovery_file_path(home.path());
...
async fn pid_record_captures_the_resolved_launch_binary() {
    ...
    let original = temp.path().join("original-kodex");
    let replacement = temp.path().join("replacement-kodex");
    for (path, bytes) in [
        (&original, b"#!/bin/sh\nexec sleep 30\n".as_slice()),
        (
            &replacement,
    ...
    let selected = temp.path().join("current-kodex");
...
async fn shutdown_grace_child() {
    let Some(ready) = std::env::var_os("KODEX_TEST_SHUTDOWN_GRACE_READY") else {
        return;
    };
    ...
    let _ = tokio::time::timeout(Duration::from_secs(8), async {
        ...
        if std::env::var_os("KODEX_TEST_SHUTDOWN_GRACE_EXIT").is_some() {
...
async fn shutdown_grace_handles_process_exit() {
    ...
    for (name, grace_seconds, exits) in [
        ...
        let mut child = std::process::Command::new(std::env::current_exe().expect("test binary"))
            ...
            .env("KODEX_TEST_SHUTDOWN_GRACE_READY", &ready)
            .envs(exits.then_some(("KODEX_TEST_SHUTDOWN_GRACE_EXIT", "1")))
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
...
async fn failed_updater_handoff_preserves_predecessor_record() {
    ...
    kodex_uds::prepare_private_socket_directory(&state_dir)
        ...
        .expect("private state directory");
    let backend = PidBackend::new_update_loop(
        temp.path().join("missing-kodex.exe"),
