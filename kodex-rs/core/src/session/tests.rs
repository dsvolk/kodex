...
use core_test_support::test_kodex::TurnInputRequest as ExternalTurnInputRequest;
...
use kodex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use kodex_config::LoaderOverrides;
use kodex_config::NetworkConstraints;
use kodex_config::NetworkDomainPermissionToml;
use kodex_config::NetworkDomainPermissionsToml;
use kodex_config::RequirementSource;
use kodex_config::Sourced;
...
use kodex_login::auth::AgentIdentityAuthPolicy;
...
use kodex_models_manager::model_info;
...
use kodex_prompts::render_model_instructions;
use kodex_protocol::AgentPath;
use kodex_protocol::ResponseItemId;
use kodex_protocol::SessionId;
use kodex_protocol::ThreadId;
use kodex_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
...
use kodex_protocol::models::AgentMessageInputContent;
use kodex_protocol::models::FileSystemPermissions;
use kodex_protocol::models::FunctionCallOutputBody;
use kodex_protocol::models::FunctionCallOutputContentItem;
use kodex_protocol::models::FunctionCallOutputPayload;
use kodex_protocol::models::ImageDetail;
use kodex_protocol::models::ImageReference;
use kodex_protocol::models::PermissionProfile;
use kodex_protocol::models::SandboxEnforcement;
use kodex_protocol::openai_models::ModelServiceTier;
...
use kodex_protocol::protocol::InternalSessionSource;
use kodex_protocol::protocol::SandboxPolicy;
use kodex_protocol::protocol::TurnEnvironmentSelections;
use kodex_protocol::request_permissions::PermissionGrantScope;
use kodex_protocol::request_permissions::RequestPermissionProfile;
use kodex_protocol::turn_input::TurnInput as SubmittedTurnInput;
use kodex_protocol::turn_input::TurnInputMode;
use kodex_protocol::turn_input::TurnInputRequest;
...
use core_test_support::test_kodex::local;
use core_test_support::test_kodex::test_kodex;
use core_test_support::test_path_buf;
use core_test_support::tracing::install_test_tracing;
use core_test_support::wait_for_event;
use kodex_config::config_toml::ConfigToml;
use kodex_config::config_toml::ProjectConfig;
use kodex_config::permissions_toml::FilesystemPermissionToml;
use kodex_config::permissions_toml::FilesystemPermissionsToml;
use kodex_config::permissions_toml::NetworkToml;
use kodex_config::permissions_toml::PermissionProfileToml;
use kodex_config::permissions_toml::PermissionsToml;
use kodex_execpolicy::Decision;
use kodex_execpolicy::NetworkRuleProtocol;
use kodex_execpolicy::Policy;
...
use kodex_protocol::items::HookPromptFragment;
use kodex_protocol::items::build_hook_prompt_message;
use kodex_protocol::mcp::McpAttribution;
use kodex_protocol::mcp::McpAttributionSource;
use kodex_protocol::mcp::McpAttributionStatus;
...
use kodex_protocol::protocol::HistoryPosition;
...
use kodex_protocol::protocol::NetworkApprovalProtocol;
use kodex_protocol::protocol::RateLimitSnapshot;
use kodex_protocol::protocol::RateLimitWindow;
use kodex_protocol::protocol::RealtimeAudioFrame;
use kodex_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use kodex_protocol::protocol::RealtimeVoice;
use kodex_protocol::protocol::RealtimeVoicesList;
use kodex_protocol::protocol::SessionMeta;
...
use kodex_protocol::protocol::TurnAbortedEvent;
...
use kodex_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<kodex_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &kodex_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(kodex_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    extension_metrics.counter(
        "kodex.test.extension.counter",
        /*inc*/ 2,
        &[("component", "skills"), ("model", "extension-model")],
    );

    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "kodex.test.extension");
    let counter = find_metric(&snapshot, "kodex.test.extension.counter");
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl kodex_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: kodex_extension_api::WorldStateContributionInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<
            'a,
            Vec<kodex_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("kodex.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let mut builder = kodex_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "kodex.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("kodex-core-tests");
...
async fn request_mcp_server_elicitation_waits_for_user_response(source: SessionSource) {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        event,
        kodex_protocol::approvals::ElicitationRequestEvent {
            turn_id: Some(turn_context.sub_id.clone()),
            server_name: "kodex_apps".to_string(),
            id: kodex_protocol::mcp::RequestId::String("request-1".to_string()),
            request,
        }
    );
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        kodex_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_kodex: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_kodex)?;
    std::fs::write(
        dot_kodex.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        kodex_home.join(kodex_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<kodex_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&kodex_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: kodex_hooks::StartHookTarget::SessionStart {
                source: kodex_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn kodex_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { kodex_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn new_turn_refreshes_managed_network_proxy_for_sandbox_change() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn kodex_network_proxy::NetworkPolicyDecider> =
        Arc::new(|_request| async { kodex_network_proxy::NetworkDecision::ask("not_allowed") });
    let (started_proxy, _) = Session::start_managed_network_proxy(
        &spec,
        &Policy::empty(),
        &initial_permission_profile,
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::ExecCommand {
                ...
                environment_id: kodex_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> kodex_sandboxing::SandboxablePreference {
            kodex_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(kodex_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: kodex_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let kodex_home = session.kodex_home().await;
    std::fs::create_dir_all(&kodex_home).expect("create kodex home");
    let config_toml_path = kodex_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = kodex_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let kodex_home = session.kodex_home().await;
    std::fs::create_dir_all(&kodex_home).expect("create kodex home");
    let base_config_path = kodex_home.join(CONFIG_TOML_FILE);
    let profile_config_path = kodex_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(kodex_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            .features
            .enable(Feature::KodexHooks)
            .expect("enable Kodex hooks");
    })
    .await?;
    let kodex_home = session.kodex_home().await;
    std::fs::create_dir_all(&kodex_home)?;
    let config_toml_path = kodex_home.join(CONFIG_TOML_FILE);
    let user_config: kodex_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = kodex_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = kodex_hooks::list_hooks(kodex_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..kodex_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        kodex_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: kodex_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: kodex_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: kodex_config::MatcherGroup {
                matcher: None,
                hooks: vec![kodex_config::HookHandlerConfig::Command {
        ...
        let identity = kodex_config::TomlValue::try_from(identity)?;
        kodex_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&kodex_home).expect("create kodex home");
    std::fs::write(
        kodex_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    let requirement = serde_json::from_value::<kodex_config::McpServerRequirement>(json!({
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let replacement_history = vec![
        ResponseItemEnvelope {
            ...
            metadata: Some(KodexHarnessMetadata::default()),
        },
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = kodex_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: kodex_protocol::protocol::SubAgentActivityKind::Started,
...
async fn annotated_history_uses_explicit_model_without_a_step(
    ...
    model_info.truncation_policy = if byte_policy {
        kodex_protocol::openai_models::TruncationPolicyConfig::bytes(/*limit*/ 101)
    } else {
        kodex_protocol::openai_models::TruncationPolicyConfig::tokens(/*limit*/ 4)
    };
    ...
    let expected = ResponseItemEnvelope {
        item,
        metadata: retain_metadata.then_some(KodexHarnessMetadata {
    ...
    for envelope in &mut expected {
        ...
        output.body = FunctionCallOutputBody::Text(kodex_utils_output_truncation::truncate_text(
            &text,
            kodex_utils_output_truncation::TruncationPolicy::Tokens(expected_budget),
        ));
    }
...
async fn prepares_resumed_history_before_installing_it() {
    ...
    assert_eq!(
        ...
        Some(KodexHarnessMetadata::default())
    );
}
...
async fn record_initial_history_seeds_token_info_from_rollout() {
    ...
    let info1 = TokenUsageInfo {
        total_token_usage: TokenUsage {
            ...
            total_tokens: 30,
            kodex_rollout_budget_units: None,
        },
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl kodex_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a kodex_extension_api::ExtensionData,
            thread_store: &'a kodex_extension_api::ExtensionData,
            turn_store: &'a kodex_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl kodex_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: kodex_extension_api::TurnStartInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl kodex_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: kodex_extension_api::TurnErrorInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl kodex_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &kodex_extension_api::ExtensionData,
            thread_store: &kodex_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: kodex_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex().with_config(|config| {
        config.update_plan_enabled = true;
        config.permissions.approval_policy =
            kodex_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.kodex.ensure_rollout_materialized().await;
    initial
        .kodex
        .flush_rollout()
        .await
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "kodex_core__kodex_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: kodex_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            kodex_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            kodex_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            kodex_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    let kodex_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(kodex_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        kodex_home: config.kodex_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(kodex_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("kodex_other".to_string()),
        limit_name: Some("kodex_other".to_string()),
        normal_model_slug: None,
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(kodex_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
...
async fn build_test_config(kodex_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/kodex/analytics-events/events"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&server)
        .await;

    let auth_manager =
        AuthManager::from_auth_for_testing(KodexAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("kodex-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "kodex_thread_initialized")
                    }) {
...
async fn session_configuration_apply_client_metadata_preserves_permissions() {
    ...
    let updated = configuration
        .apply(
            &SessionSettingsUpdate {
                app_server_client_name: Some("kodex-tui".to_string()),
    ...
    assert_eq!(
        ...
        (Some("kodex-tui".to_string()), Some("1.0.0".to_string())),
    );
}
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = kodex_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        kodex_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config(
    ...
    let kodex_home = tempfile::tempdir().expect("create kodex home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        kodex_home.path().join(kodex_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let kodex_home = parent_config.kodex_home.clone();
    let skill_dir = kodex_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&kodex_exec_server::LOCAL_FS));
    ...
    let role_path = kodex_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
    ...
    session
        ...
        .send(Event {
            ...
            msg: EventMsg::ThreadSettingsApplied(
                kodex_protocol::protocol::ThreadSettingsAppliedEvent {
...
async fn compaction_persists_resume_metadata_and_companion_records() {
    ...
    let expected_settings = kodex_protocol::protocol::ThreadSettingsAppliedEvent {
        thread_id: Some(session.thread_id()),
        thread_settings: session.thread_settings_snapshot().await,
    };
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let kodex_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(kodex_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("Test API Key"));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(kodex_code_mode::DisabledCodeModeSessionProvider),
        ...
        Arc::new(kodex_thread_store::LocalThreadStore::new(
            kodex_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        kodex_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        kodex_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
async fn response_metadata_builders_capture_fresh_mcp_attribution() {
    ...
    let step_context = StepContext::for_test(Arc::clone(&turn_context));
    let before = session
        .responses_metadata(&step_context, KodexResponsesRequestKind::Turn)
        .await;
    let source = McpAttributionSource {
        connector_id: None,
        plugin_id: None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(kodex_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: kodex_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(kodex_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: kodex_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: kodex_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        ...
        thread_store: Arc::new(kodex_thread_store::LocalThreadStore::new(
            kodex_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .kodex_home(config.kodex_home.to_path_buf())
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            kodex_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(kodex_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(kodex_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            kodex_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(kodex_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = kodex_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(kodex_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    ...
                    call_id,
                    kodex_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(kodex_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: kodex_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_tool_rejects_invalid_requests(
    ...
    Arc::make_mut(&mut turn_context.config)
        .permissions
        .approval_policy = kodex_config::Constrained::allow_any(AskForApproval::Never);
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(kodex_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = kodex_utils_path_uri::PathUri::from_abs_path(&second_cwd);
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        kodex_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = kodex_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(kodex_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn kodex_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<kodex_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: kodex_extension_api::ThreadStopInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: kodex_extension_api::ThreadStopInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl kodex_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: kodex_extension_api::TurnAbortInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(kodex_home).await;
    configure_config(&mut config);
    let state_db = None;
    let config = Arc::new(config);
    let thread_id = ThreadId::default();
    let auth_manager = AuthManager::from_auth_for_testing_with_home(auth, kodex_home.to_path_buf());
    let models_manager = models_manager_with_provider(
        config.kodex_home.to_path_buf(),
        auth_manager.clone(),
        config.model_provider.clone(),
...
async fn cancelled_step_capture_finishes_warning_delivery() {
    ...
    impl kodex_extension_api::ThreadInstructionsProvider for WarningProvider {
        fn load_thread_instructions(&self) -> kodex_extension_api::LoadInstructionsFuture<'_> {
            Box::pin(async {
                kodex_extension_api::LoadedUserInstructions {
                    instructions: None,
    ...
    assert!(matches!(error.details(), KodexErrorDetails::TurnAborted));
    assert_eq!(
        warning_fields(
            rx.try_recv()
    ...
    assert!(
        turn.extension_data
            .get::<kodex_extension_api::SelectedPluginSnapshot>()
            .is_none()
    );
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            kodex_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = kodex_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !kodex_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        kodex_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        kodex_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_active_reviewer_and_latest_runtime_policy(
    ...
    if live_update {
        assert_eq!(
            session
                .apply_turn_settings(
                    &old_turn.sub_id,
                    kodex_protocol::protocol::TurnSettingsUpdate {
                        approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                        ..Default::default()
                    },
                )
                .await,
            kodex_protocol::protocol::TurnSettingsUpdateOutcome::Applied
        );
    } else {
        session
            .update_settings(SessionSettingsUpdate {
    ...
    let request = kodex_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: kodex_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        ("kodex_approval_kind".to_string(), json!("mcp_tool_call")),
                        ("kodex_request_type".to_string(), json!("approval_request")),
...
async fn mcp_refresh_detects_shared_auth_manager_changes() {
    ...
    assert_eq!(
        ...
        Some(kodex_protocol::auth::AuthMode::ApiKey)
    );
    session.refresh_mcp_if_dirty().await;
    assert!(
        session
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    let (session, turn_context) = make_session_and_context().await;
    let selected_root =
        |environment_id: &str, path: &str| kodex_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: kodex_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let kodex_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        turn_environments.push(TurnEnvironment::new(
            ...
            Arc::new(
                kodex_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
async fn capability_discovery_uses_environment_permission_profile() {
    ...
    let expected_sandbox = FileSystemSandboxContext {
        ...
        windows_sandbox_selection: if cfg!(windows) {
            kodex_file_system::WindowsSandboxSelection::Elevated
        } else {
            kodex_file_system::WindowsSandboxSelection::Disabled
        },
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(kodex_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(kodex_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
async fn build_initial_context_reuses_in_flight_recommendation_prewarm() {
    ...
    Mock::given(method("GET"))
        .and(path("/ps/plugins/suggested/kodex"))
...
impl kodex_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a kodex_extension_api::ExtensionData,
        thread_store: &'a kodex_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<kodex_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    kodex_extension_api::PromptFragment::developer_policy(
                        "prompt extension enabled",
                        kodex_extension_api::ContentItemKind("test.prompt_extension".to_string()),
                    )
                })
                .into_iter()
                .collect()
...
-> Arc<kodex_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = kodex_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl kodex_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: kodex_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<kodex_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            ...
            .then(|| {
                kodex_extension_api::PromptFragment::developer_policy(
                    ...
                    kodex_extension_api::ContentItemKind("test.turn_context".to_string()),
                )
            })
            .into_iter()
            .collect()
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<kodex_thread_store::InMemoryThreadStore> {
    let store = Arc::new(kodex_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn kodex_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &kodex_thread_store::InMemoryThreadStore,
...
async fn extension_interrupt_emits_thread_idle() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: kodex_extension_api::ThreadIdleInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    image::DynamicImage::new_rgba8(/*w*/ 8, /*h*/ 8)
        ...
        .expect("encode image");
    let image_url = kodex_utils_image::data_url_from_bytes("image/png", &encoded.into_inner());
    let text_element = kodex_protocol::user_input::TextElement::new(
        kodex_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl kodex_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: kodex_extension_api::ThreadIdleInput<'a>,
        ) -> kodex_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(kodex_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn active_turn_keeps_first_root_when_mail_coalesces(inherited_root: Option<&str>) {
    ...
    .enumerate()
    {
        sess.input_queue
            .enqueue_mailbox_communication(
                communication,
                kodex_protocol::turn_input::TurnStartOptions {
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(kodex_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: kodex_tools::ToolName::plain("exec_command"),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let kodex_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_kodex = project_root.join(".kodex");
    let nested_dot_kodex = nested.join(".kodex");

    std::fs::create_dir_all(&kodex_home)?;
    std::fs::create_dir_all(&nested_dot_kodex)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_kodex)?;
    write_project_hooks(&nested_dot_kodex)?;
    write_project_trust_config(&kodex_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .kodex_home(kodex_home)
    ...
    let expected_source_path = kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_kodex.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_kodex = project_root.join(".kodex");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_kodex)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let kodex_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&kodex_home)?;
        write_project_trust_config(&kodex_home, &trust_entries).await?;
