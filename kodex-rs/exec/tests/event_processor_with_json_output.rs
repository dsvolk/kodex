use kodex_app_server_protocol::CollabAgentState as ApiCollabAgentState;
use kodex_app_server_protocol::CollabAgentStatus as ApiCollabAgentStatus;
use kodex_app_server_protocol::CollabAgentTool;
use kodex_app_server_protocol::CollabAgentToolCallStatus as ApiCollabAgentToolCallStatus;
use kodex_app_server_protocol::CommandAction;
use kodex_app_server_protocol::CommandExecutionSource;
use kodex_app_server_protocol::CommandExecutionStatus as ApiCommandExecutionStatus;
use kodex_app_server_protocol::ErrorNotification;
use kodex_app_server_protocol::FileUpdateChange as ApiFileUpdateChange;
use kodex_app_server_protocol::ItemCompletedNotification;
use kodex_app_server_protocol::ItemStartedNotification;
use kodex_app_server_protocol::McpToolCallError;
use kodex_app_server_protocol::McpToolCallResult;
use kodex_app_server_protocol::McpToolCallStatus as ApiMcpToolCallStatus;
use kodex_app_server_protocol::PatchApplyStatus as ApiPatchApplyStatus;
use kodex_app_server_protocol::PatchChangeKind as ApiPatchChangeKind;
...
use kodex_app_server_protocol::WebSearchAction as ApiWebSearchAction;
use kodex_app_server_protocol::WebSearchItem as ApiWebSearchItem;
...
use kodex_exec::CollabToolCallItem;
use kodex_exec::CollabToolCallStatus;
use kodex_exec::CollectedThreadEvents;
use kodex_exec::CommandExecutionItem;
use kodex_exec::CommandExecutionStatus;
use kodex_exec::ErrorItem;
use kodex_exec::EventProcessorWithJsonOutput;
use kodex_exec::ExecThreadItem;
use kodex_exec::FileChangeItem;
use kodex_exec::FileUpdateChange as ExecFileUpdateChange;
...
use kodex_exec::McpToolCallStatus;
use kodex_exec::PatchApplyStatus;
use kodex_exec::PatchChangeKind;
use kodex_exec::ReasoningItem;
use kodex_exec::ThreadErrorEvent;
use kodex_exec::ThreadEvent;
use kodex_exec::ThreadItemDetails;
use kodex_exec::ThreadStartedEvent;
use kodex_exec::TodoItem;
use kodex_exec::TodoListItem;
use kodex_exec::TurnCompletedEvent;
use kodex_exec::TurnFailedEvent;
use kodex_exec::TurnStartedEvent;
use kodex_exec::Usage;
use kodex_exec::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_configured = SessionConfiguredEvent {
        ...
        approvals_reviewer: kodex_protocol::config_types::ApprovalsReviewer::User,
...
fn plan_update_emits_started_then_updated_then_completed() {
    ...
    assert_eq!(
        ...
        CollectedThreadEvents {
            ...
            status: KodexStatus::InitiateShutdown,
        }
    );
}
...
fn token_usage_update_is_emitted_on_turn_completion() {
    ...
    let usage_update =
        processor.collect_thread_events(ServerNotification::ThreadTokenUsageUpdated(
            kodex_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn failed_turn_clears_stale_final_message() {
    ...
    assert_eq!(collected.status, KodexStatus::Running);
    ...
    assert_eq!(collected.status, KodexStatus::InitiateShutdown);
...
fn model_reroute_surfaces_as_error_item() {
    ...
    let collected = processor.collect_thread_events(ServerNotification::ModelRerouted(
        kodex_app_server_protocol::ModelReroutedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            from_model: "gpt-5".to_string(),
            to_model: "gpt-5-mini".to_string(),
            reason: kodex_app_server_protocol::ModelRerouteReason::HighRiskCyberActivity,
        },
    ));
