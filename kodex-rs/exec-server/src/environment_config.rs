...
use kodex_config::McpServerDisabledReason;
use kodex_config::RequirementSource;
use kodex_config::RequirementsLayerEntry;
use kodex_config::compose_requirements_for_hostname;
use kodex_config::format_config_layer_source;
use kodex_config::host_name;
use kodex_config::loader::LocalTomlLayerStack;
use kodex_config::loader::load_local_config_layers;
use kodex_exec_server_protocol::EnvironmentConfigLayer;
use kodex_exec_server_protocol::EnvironmentConfigLayerStack;
use kodex_exec_server_protocol::EnvironmentConfigReadParams;
use kodex_exec_server_protocol::EnvironmentConfigReadResponse;
...
pub(crate) async fn read_environment_config(
    ...
    let kodex_home = find_kodex_home().map_err(|error| {
        ReadEnvironmentConfigError::Internal(format!("failed to find Kodex home: {error}"))
    })?;
    let layers = load_local_config_layers(file_system, kodex_home.as_path(), &cwd)
        .await
        .map_err(|error| {
    ...
    Ok(EnvironmentConfigReadResponse {
        ...
        kodex_home_dir: PathUri::from_abs_path(&kodex_home),
        hostname: host_name(),
...
impl Environment {
    ...
    pub async fn discover_http_mcp_servers(
        ...
        for layer in response.config.layers {
            ...
            kodex_config::merge_toml_values(&mut merged, &config);
        }
