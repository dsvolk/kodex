...
/// Start an interactive sub-Kodex thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_kodex_thread_interactive(
    mut config: Config,
    ...
    let extensions = if isolation == kodex_extension_api::SessionIsolation::Isolated {
        kodex_extension_api::empty_extension_registry()
    } else {
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_kodex_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let (session, io) = Box::pin(run_kodex_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
