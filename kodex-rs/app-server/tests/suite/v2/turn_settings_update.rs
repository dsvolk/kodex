...
-> Result<()> {
    ...
    .await;
    let kodex_home = TempDir::new()?;
    mock_config(kodex_home.path(), &server.uri())?.write(kodex_home.path())?;
    let mut app = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn mock_config(kodex_home: &Path, server_uri: &str) -> Result<MockResponsesConfig> {
    ...
    let models = [MODEL_A, MODEL_B]
        ...
        .collect();
    let catalog_path = kodex_home.join("step-models.json");
    std::fs::write(
        &catalog_path,
