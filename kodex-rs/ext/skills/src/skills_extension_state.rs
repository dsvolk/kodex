...
use kodex_mcp::McpResourceClientAuthKey;
use kodex_mcp::McpResourceServerCacheKey;
...
impl CloudSkillGeneration {
    pub(crate) fn current_resource_cache_key(&self) -> Option<McpResourceServerCacheKey> {
        self.mcp_resources
            .as_ref()
            .and_then(|client| client.server_cache_key(kodex_mcp::KODEX_APPS_MCP_SERVER_NAME))
    }

    pub(crate) fn is_current(&self) -> bool {
        self.auth_cache_key
            == self.mcp_resources.as_ref().map(|client| {
                client.auth_cache_key_for_server(kodex_mcp::KODEX_APPS_MCP_SERVER_NAME)
            })
    }
}
