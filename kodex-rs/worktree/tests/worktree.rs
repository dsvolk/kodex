...
use kodex_worktree::ManagedWorktree;
use kodex_worktree::WorktreeManager;
use kodex_worktree::WorktreeSettings;
use kodex_worktree::default_worktree_base;
...
impl RepositoryFixture {
    fn new() -> Self {
        ...
        let kodex_home = temp_root.join("kodex-home");
    ...
    fn manager(&self) -> WorktreeManager {
        let settings =
            WorktreeSettings::from_desktop_config(&self.kodex_home, /*desktop*/ None)
...
fn git_output(repository: &Path, args: &[&str]) -> std::process::Output {
    Command::new("git")
        ...
        .args([
            "-c",
            "user.name=Kodex Worktree Test",
            "-c",
            "user.email=kodex-worktree-test@example.invalid",
            "-c",
...
fn cli_creation_shares_default_and_custom_desktop_pools_without_enabling_cleanup() {
    let fixture = RepositoryFixture::new();
    for root in [None, Some(fixture.kodex_home.join("custom-desktop"))] {
        let desktop =
            root.map(|root| HashMap::from([("git-worktree-root".to_owned(), json!(root))]));
        let desktop_settings =
            WorktreeSettings::from_desktop_config(&fixture.kodex_home, desktop.as_ref())
        ...
        let manager = WorktreeManager::new(
            WorktreeSettings::for_cli(&fixture.kodex_home, Some(&cli_desktop))
...
fn thread_binding_rejects_primary_and_unmanaged_worktrees() {
    ...
    assert!(
        !fixture.repository.join(".git/kodex-thread.json").exists(),
...
fn thread_binding_writes_the_exact_desktop_owner_schema_into_git_metadata() {
    ...
    let metadata_path = checkout.join(run_git(
        &checkout,
        &["rev-parse", "--git-path", "kodex-thread.json"],
    ));
...
fn creation_uses_desktop_hash_bucket_layout_and_detached_head() {
    ...
    assert_eq!(
        ...
        Some(fixture.kodex_home.join("worktrees").as_path())
    );
    assert_eq!(
...
fn creation_rejects_a_working_directory_that_escapes_through_a_base_symlink() {
    ...
    let outside = fixture.kodex_home.join("outside-worktree");
...
fn creation_ignores_inherited_git_environment() {
    const CHILD_ROOT: &str = "KODEX_WORKTREE_ENV_TEST_ROOT";
    if let Some(root) = std::env::var_os(CHILD_ROOT) {
        let root = PathBuf::from(root);
        let source = root.join("project");
        let source_cwd = source.join("nested/component");
        let settings =
            WorktreeSettings::from_desktop_config(&root.join("kodex-home"), /*desktop*/ None)
        ...
        assert_eq!(
            worktree.head_sha,
            std::env::var("KODEX_WORKTREE_ENV_TEST_HEAD").expect("fixture operation succeeds")
        );
        return;
    }

    let fixture = RepositoryFixture::new();
    let other = fixture.kodex_home.join("other-repository");
    ...
    child
        ...
        .env("KODEX_WORKTREE_ENV_TEST_HEAD", run_git(&fixture.repository, &["rev-parse", "HEAD"]))
        ...
        .env("GIT_CONFIG_PARAMETERS", "'filter.review.process=kodex-worktree-nonexistent-filter' 'filter.review.required=true'");
...
fn creation_disables_filters_from_destination_conditional_config() {
    ...
    let config = fixture.kodex_home.join("destination.gitconfig");
    fs::write(
        &config,
        "[filter \"late\"]\nsmudge = kodex-worktree-nonexistent-filter\nrequired = true\n",
    )
...
fn creation_fails_for_a_directory_outside_a_git_repository() {
    let fixture = RepositoryFixture::new();
    let manager = fixture.manager();
    let non_repository = fixture.kodex_home.join("not-a-repository");
...
fn listing_rejects_a_primary_checkout_even_when_its_path_matches_the_layout() {
    ...
    let git_dir = fixture.kodex_home.join("separate-git-dir");
    run_git(
        &primary,
        &[
            "init",
...
fn listing_rejects_checkout_and_bucket_aliases_but_allows_a_root_alias() {
    ...
    let aliased_root = fixture.kodex_home.join("aliased-worktrees");
...
fn listing_only_includes_managed_worktrees_for_the_requested_repository() {
    ...
    let other_repository = fixture.kodex_home.join("other-project");
...
fn creation_listing_and_thread_binding_preserve_native_repository_paths() {
    ...
    for name in [
        ...
        let repository = fixture.kodex_home.join(&name);
        initialize_repository(&repository);
