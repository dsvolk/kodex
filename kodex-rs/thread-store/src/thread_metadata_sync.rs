...
use kodex_protocol::protocol::user_message_preview;
...
impl ThreadMetadataSync {
    pub(crate) async fn for_create(params: &CreateThreadParams) -> Self {
        ...
        let guardian_review = kodex_state::is_guardian_review_source(&params.source);
        let update = ThreadMetadataPatch {
            name: (guardian_review && params.history_mode == ThreadHistoryMode::Paginated)
                .then(|| Some(kodex_state::GUARDIAN_THREAD_TITLE.to_string())),
            title: guardian_review.then(|| kodex_state::GUARDIAN_THREAD_TITLE.to_string()),
            preview: guardian_review.then(|| kodex_state::GUARDIAN_THREAD_PREVIEW.to_string()),
    ...
    pub(crate) fn for_resume(
        ...
        let guardian_review = metadata.is_some_and(|metadata| {
            serde_json::from_str::<SessionSource>(&metadata.source)
                .as_ref()
                .is_ok_and(kodex_state::is_guardian_review_source)
        });
        let mut sync = Self {
            thread_id: params.thread_id,
            cwd_seen: params
    ...
    pub(crate) fn observe_appended_items(
        ...
        let affects_metadata = items
            .iter()
            .any(kodex_state::rollout_item_affects_thread_metadata);
        ...
        for item in items {
            match item {
                RolloutItem::SessionMeta(meta_line) if meta_line.meta.id == self.thread_id => {
                    ...
                    if kodex_state::is_guardian_review_source(&meta_line.meta.source) {
                        self.preview_seen = true;
                        self.first_user_message_seen = true;
                        self.title_seen = true;
                        update.preview = Some(kodex_state::GUARDIAN_THREAD_PREVIEW.to_string());
                    }
...
mod tests {
    ...
    async fn create_metadata_records_originator_without_project() {
        ...
        let sync = ThreadMetadataSync::for_create(&CreateThreadParams {
            ...
            base_instructions: kodex_protocol::models::BaseInstructions::default(),
    ...
    fn thread_settings_applied_updates_live_metadata() {
        ...
        assert_eq!(update.patch.model.as_deref(), Some("gpt-5.2-kodex"));
        assert_eq!(
