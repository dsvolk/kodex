//! Diagnoses whether Kodex update paths target the running installation.
//!
...
const DESKTOP_UPDATE_URL: &str = "https://persistent.oaistatic.com/kodex-app-prod/appcast.xml";
#[cfg(target_os = "macos")]
const BACKEND_DESKTOP_UPDATE_URL: &str = "https://chatgpt.com/backend-api/wham/app/appcast";
#[cfg(target_os = "windows")]
const DESKTOP_UPDATE_URL: &str =
    "https://persistent.oaistatic.com/kodex-app-prod/windows-store-update.json";
...
pub(super) async fn updates_check(config: &Config) -> DoctorCheck {
    ...
    let version_file = config.kodex_home.join(VERSION_FILE_NAME);
...
fn windows_store_update(
    ...
    if manifest.schema_version == 0
        ...
        || manifest.package_identity != "OpenAI.Kodex"
    {
...
fn update_action_label(context: &InstallContext) -> &'static str {
    match &context.method {
        InstallMethod::Npm => "npm install -g @openai/kodex",
    ...
    tokio::time::timeout(Duration::from_secs(/*secs*/ 5), async {
        let mut response = client
            ...
            .header(
                ...
                concat!("kodex-doctor/", env!("CARGO_PKG_VERSION")),
            )
            .send()
            .await
...
mod tests {
    ...
    fn macos_update_probe_uses_the_persisted_production_appcast_feed() {
        ...
        let state_directory = home
            .path()
            .join("Library/Application Support/com.openai.kodex");
    ...
    async fn macos_staged_updates_require_a_newer_matching_extracted_bundle() {
        ...
        for (name, identity, build) in [
            ("newest", "com.openai.kodex", "6268"),
            ("newer", "com.openai.kodex", "6168"),
            ("older", "com.openai.kodex", "6138"),
            ("different", "com.example.other", "9999"),
            ("invalid", "com.openai.kodex", "invalid"),
        ] {
        ...
        write_macos_bundle(&linked, "com.openai.kodex", "9999");
