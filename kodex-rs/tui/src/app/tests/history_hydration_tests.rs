...
async fn history_fixture(
    ...
    let path = rollout_path(kodex_home.path(), filename_timestamp, &id);
    ...
    let target = SessionTarget {
        ...
        thread_id,
        cwd: None,
        history_mode: Some(kodex_app_server_protocol::ThreadHistoryMode::Paginated),
    };
    Ok((app, kodex_home, target))
}

#[tokio::test]
async fn history_hydration_metadata_tracks_missing_turns_and_stale_completions_keep_new_request()
-> Result<()> {
    let (mut app, _kodex_home, target) =
...
async fn history_hydration_archived_retry_uses_first_attempt_runtime_settings() -> Result<()> {
    for action in [
        ...
        for mode in [TranscriptMode::Terminal, TranscriptMode::Owned] {
            let (mut app, kodex_home, mut target) = history_fixture(&[500]).await?;
            app.config.tui_fullscreen_transcript = !mode.is_owned();
            app.config.tui_alternate_screen = kodex_config::types::AltScreenMode::Always;
            ...
            let archived = kodex_home.path().join("archived_sessions");
