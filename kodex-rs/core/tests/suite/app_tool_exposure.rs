...
impl McpServerContributor<Config> for AppsServer {
    ...
    fn contribute<'a>(
        ...
        Box::pin(async move {
            let mut config = kodex_mcp::hosted_plugin_runtime_mcp_server_config(
...
pub(super) async fn connector_exposure_requests(
    ...
    let namespace = "mcp__kodex_apps__calendar";
    ...
    assert!(
        requests[0]
            .tool_by_name("mcp__kodex_apps__notes", "read")
            .is_none()
    );
    ...
    assert!(!exec_description.contains("mcp__kodex_apps__notes__read("));
    if calls_tool {
