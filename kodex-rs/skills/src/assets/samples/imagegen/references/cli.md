...
Set a stable path to the skill CLI (default `KODEX_HOME` is `~/.kodex`):

```
export KODEX_HOME="${KODEX_HOME:-$HOME/.kodex}"
export IMAGE_GEN="$KODEX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/kodex-network.md`
