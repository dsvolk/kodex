...
use kodex_core::EnvironmentNetworkPolicy;
...
use kodex_core::WaitForEnvironmentToolConfig;
...
use kodex_exec_server::NoiseRendezvousConnectBundle;
use kodex_exec_server::NoiseRendezvousConnectProvider;
use kodex_exec_server::REMOTE_ENVIRONMENT_ID;
...
use kodex_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_kodex_with_wait_for_environment() -> TestKodexBuilder {
    let mut extensions = ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(Arc::new(WaitForEnvironmentTestExtension));
    test_kodex().with_extensions(Arc::new(extensions.build()))
}

async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestKodex> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = kodex_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_patch_approval(test: &TestKodex) {
    let event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn approved_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/kodex-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\kodex-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn environment_permissions_follow_configuration_ownership() -> Result<()> {
    ...
    .await;
    let snapshot = test.kodex.config_snapshot().await;
    ...
    test.kodex
        .restore_thread_settings(test.kodex.restorable_thread_settings().await)
        .await?;
    let (mcp_config, _) = test.kodex.current_mcp_config_and_runtime_context().await;
    assert_eq!(
...
async fn executor_profile_roots_survive_settings_restore_and_turn_recording() -> Result<()> {
    ...
    let test = test_kodex()
        ...
        .await?;
    let runtime_roots = test.kodex.config_snapshot().await.workspace_roots;
    ...
    .await?;
    let saved = test.kodex.restorable_thread_settings().await;
    ...
    test.kodex.restore_thread_settings(saved).await?;
    let snapshot = test.kodex.config_snapshot().await;
    assert_eq!(
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_kodex_with_wait_for_environment()
            } else {
                test_kodex()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    let initial_environments = test.kodex.environment_selections().await;
    ...
    let preview = test
        .kodex
        .preview_thread_settings_overrides(KodexThreadSettingsOverrides {
    ...
    assert_eq!(
        test.kodex.environment_selections().await,
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    let mcp_url = format!("{}/api/kodex/ps/mcp", apps.chatgpt_base_url);
    let response_mock = mount_sse_sequence(
        &server,
        vec![
            sse(vec![
    ...
    core_test_support::wait_for_mcp_server(&test.kodex, "deferred").await?;
    test.kodex
        .submit(Op::UserInputAnswer {
            id: request.turn_id,
            response: RequestUserInputResponse {
    ...
    test.kodex.flush_rollout().await?;
    let rollout = fs::read_to_string(test.kodex.rollout_path().context("rollout path")?)?;
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    let mut builder = test_kodex().with_extensions(Arc::new(extensions.build()));
    let test = builder.build_with_auto_env(&server).await?;
    let selection = test
        .kodex
    ...
    assert_eq!(
        test.kodex.inspect_selected_capability_roots().ready_roots,
...
async fn owner_network_policy_rejects_unsupported_environment_authority() -> Result<()> {
    ...
    let selections = test.kodex.environment_selections().await;
    ...
    assert_eq!(test.kodex.environment_selections().await, selections);
    Ok(())
}
...
async fn future_pending_environment_can_finish_without_retargeting_the_active_turn() -> Result<()> {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    let selection = test.kodex.environment_selections().await.remove(0);
...
async fn active_environment_update_wakes_the_old_wait_with_the_new_selection() -> Result<()> {
    let server = start_mock_server().await;
    let mut builder = test_kodex_with_wait_for_environment().with_config(|config| {
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        assert!(config.features.disable(Feature::StepModelSwitching).is_ok());
    });
    let test = builder.build_with_auto_env(&server).await?;
    let mut original = test.kodex.environment_selections().await.remove(0);
...
async fn ready_before_selection_resolves_resumed_thread_capability_root_after_wait(
    ...
    let remote_environment = tokio::spawn(kodex_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_kodex_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
    ...
    assert_eq!(test.kodex.instruction_sources().await, vec![agents_path]);
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/kodex-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/kodex-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/kodex-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/kodex-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/kodex-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/kodex-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/kodex-remote-dotdot-{}", std::process::id()));
