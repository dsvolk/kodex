...
use kodex_protocol::items::DynamicToolCallStatus;
use kodex_protocol::items::ImageGenerationItem;
...
fn promotes_backing_agent_artifacts_once_without_a_client_request() {
    let mut state = started_state();
    let first_delta = assistant_delta("message-1", "[analysis] ::kodex-realtime-inline{}");
    ...
    let completed = completed_item(TurnItem::AgentMessage(AgentMessageItem {
        ...
        content: vec![AgentMessageContent::Text {
            text: "[analysis] ::kodex-realtime-inline{}\nVisible explanation".to_string(),
        }],
        phase: None,
    ...
    let next_message = assistant_delta("message-2", "::kodex-realtime-inline{}\nNext message");
    assert_eq!(
...
fn promotes_distinct_visualizations_once_and_ignores_markdown_fences() {
    let mut state = started_state();
    let text = "```\n::kodex-inline-vis{file=hidden}\n```\n~~~\nvisualize{file=also-hidden}\n~~~\n::kodex-inline-vis{file=first}\nvisualize{file=second}";
...
#[test]
fn promotes_successful_kodex_app_mcp_calls_and_splits_active_transcripts() {
    let mut state = started_state();
    observe_realtime(
        &mut state,
        RealtimeEvent::OutputTranscriptDelta(RealtimeTranscriptDelta {
    ...
    for (server, status) in [
        ...
        ("kodex_app", McpToolCallStatus::InProgress),
        ("kodex_app", McpToolCallStatus::Failed),
    ] {
        assert!(
            state
