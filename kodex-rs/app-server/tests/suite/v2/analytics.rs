...
use kodex_config::types::OtelHttpProtocol;
use kodex_core::config::ConfigBuilder;
use kodex_core_plugins::loader::curated_plugin_cache_version;
...
async fn guardian_review_turns_and_tools_reach_analytics() -> Result<()> {
    ...
    let parent_command = events
        .iter()
        .find(|event| {
            event["event_type"] == "kodex_command_execution_event"
    ...
    let turns = children
        .iter()
        .filter(|event| event["event_type"] == "kodex_turn_event")
        .map(|event| {
            let params = &event["event_params"];
    ...
    let tools = children
        .iter()
        .filter(|event| event["event_type"] == "kodex_web_search_event")
        .collect::<Vec<_>>();
...
fn set_metrics_exporter(config: &mut kodex_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = kodex_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("kodex-app-server"),
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, kodex_home: &Path) -> Result<()> {
    Mock::given(method("POST"))
    ...
    let body = timeout(read_timeout, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/kodex/analytics-events/events"
            }) {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "kodex_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("kodex_thread_initialized event should be present"))
}
...
fn write_curated_metrics_plugin(kodex_home: &Path) -> Result<PathBuf> {
    let plugin_id = PluginId::parse(METRICS_PLUGIN_ID)?;
    let plugin_root = PluginStore::new(kodex_home.to_path_buf()).plugin_root(
        &plugin_id,
...
"#,
    ...
    std::fs::write(
        &script_path,
        r#"test -n "$KODEX_PLUGIN_METRICS_OUTPUT"
sleep "${1:-0.3}"
while [ -n "${2:-}" ] && [ ! -f "$2" ]; do sleep 0.01; done
printf '%s' '{"version":1,"measurements":[{"name":"findings","value":3,"dimensions":{"severity":"high"}},{"name":"files_scanned","value":17}]}' > "$KODEX_PLUGIN_METRICS_OUTPUT"
"#,
    )?;

    let curated_repo = kodex_home.join(".tmp/plugins");
...
async fn assert_plugin_measurement_analytics(remote: bool, background: bool) -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "trusted plugin metrics fixture uses a local Kodex home cache"
    );
    skip_if_wine_exec!(Ok(()), "plugin metrics fixture is Unix-only");

    let kodex_home = TempDir::new()?;
    let script_path = write_curated_metrics_plugin(kodex_home.path())?.canonicalize()?;
    let release_path = kodex_home.path().join("release-command");
    let mut command = vec![
        "/bin/sh".to_string(),
    ...
    .collect();
    let catalog_path = kodex_home.path().join("measurement-models.json");
    std::fs::write(
        &catalog_path,
...
"#,
    ...
    mount_analytics_capture(&analytics_server, kodex_home.path()).await?;
    ...
    if remote {
        assert_eq!(
            ...
            kodex_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
    ...
    for measurement_name in ["findings", "files_scanned"] {
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "kodex_plugin_measurement_event"
    ...
    for request in analytics_server
        ...
        .unwrap_or_default()
    {
        if request.method != "POST" || request.url.path() != "/kodex/analytics-events/events" {
            continue;
        }
        let payload: Value = serde_json::from_slice(&request.body)?;
        let Some(events) = payload["events"].as_array() else {
    ...
    for event in &measurement_events {
        ...
        assert_eq!(event_params["originator"], "kodex_work_desktop");
