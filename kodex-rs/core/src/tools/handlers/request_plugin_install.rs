...
use kodex_analytics::PluginInstallRequestSource;
use kodex_analytics::PluginInstallRequested;
use kodex_analytics::PluginInstallRequestedPlugin;
...
use kodex_tools::DiscoverableToolAction;
use kodex_tools::DiscoverableToolType;
use kodex_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use kodex_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use kodex_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use kodex_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use kodex_tools::RequestPluginInstallArgs;
use kodex_tools::RequestPluginInstallResult;
use kodex_tools::ToolName;
use kodex_tools::ToolSpec;
use kodex_tools::all_requested_connectors_picked_up;
use kodex_tools::build_request_plugin_install_elicitation_request;
use kodex_tools::filter_request_plugin_install_discoverable_tools_for_client;
use kodex_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("kodex-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in kodex-tui yet".to_string(),
            ));
        }
        ...
        let tool = if self.presentation == ToolSuggestPresentation::RecommendationContext {
            ...
            match kodex_core_plugins::hydrate_selected_recommended_plugin_install_metadata(
                &plugins_config,
                auth.as_ref(),
                tool,
            )
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.kodex_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    kodex_home: &kodex_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&kodex_login::KodexAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_kodex_apps_tools().await {
        Ok(mcp_tools) => {
        ...
        Err(err) => {
            warn!(
                "failed to refresh kodex apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
