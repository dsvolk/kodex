...
pub(crate) struct ManagedTuiWorktree {
    manager: kodex_worktree::WorktreeManager,
    checkout: kodex_worktree::ManagedWorktree,
...
async fn latest_thread_cwd(path: Option<PathBuf>, fallback: PathBuf) -> PathBuf {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        while let Some(outcome) = scanner.scan_next_rollout_line().ok()? {
            if let kodex_rollout::ScanOutcome::Parsed(kodex_rollout::RolloutLine {
                item: kodex_rollout::RolloutItem::TurnContext(item),
                ..
            }) = outcome
            {
...
pub(super) async fn prepare(
    ...
    if let Some(id_or_name) = cli.fork_session_id.as_deref() {
        let prepared = if should_load_configured_environments(&loader_overrides, target) {
            EnvironmentManager::prepare_from_kodex_home(&source.kodex_home).await
        } else {
        ...
        if cli.cwd.is_none() {
            ...
            .await;
            let source_bundle =
                cloud_config_bundle_for_app_server_target(target, &bootstrap, &source.kodex_home)
                    .await?;
    ...
    if !source.features.enabled(kodex_features::Feature::Worktrees) {
    ...
    let manager = kodex_worktree::WorktreeManager::new(
        kodex_worktree::WorktreeSettings::for_cli(
            &source.kodex_home,
    ...
    .await?;
    let bundle =
        cloud_config_bundle_for_app_server_target(target, &bootstrap, &source.kodex_home).await?;
    managed
        .check_source_policy(
            &cli_overrides,
            overrides,
