...
use kodex_app_server_protocol::BedrockDiscoverParams;
use kodex_app_server_protocol::BedrockDiscoverResponse;
use kodex_app_server_protocol::BedrockSetupParams;
use kodex_app_server_protocol::BedrockSetupResponse;
...
async fn bedrock_app_server(
    ...
    let aws_config = kodex_home.join("aws-config");
    let aws_credentials = kodex_home.join("aws-credentials");
...
async fn setup_bedrock_profile_and_environment() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "Kodex-managed Bedrock credentials are already configured and take priority over AWS environment credentials. Run `kodex logout` and try again."
    );
...
async fn setup_bedrock_rejects_invalid_or_conflicting_credentials() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let mut app_server = bedrock_app_server(kodex_home.path(), &[]).await?;
    ...
    for source_file in ["selected.config.toml", "config.toml", "managed_config.toml"] {
        ...
        let mut app_server = bedrock_app_server(
            &layered_home_path,
            &[(
                "KODEX_APP_SERVER_TEST_USER_CONFIG_FILE",
