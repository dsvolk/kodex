...
impl LocalAgentControl {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> KodexResult<String> {
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> KodexResult<AgentInfo> {
        let state = self.upgrade()?;
        ...
        let snapshot = match state.get_thread(agent_id).await {
                ...
                if !config.ephemeral
                    ...
                    && let Err(err) = agent_graph_store
                        .set_thread_spawn_edge_status(
                            agent_id,
                            kodex_agent_graph_store::ThreadSpawnEdgeStatus::Closed,
                        )
                        .await
                {
            ...
            Err(err)
                if known_agent && matches!(err.details(), KodexErrorDetails::ThreadNotFound(_)) =>
            {
    ...
    pub(crate) async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> KodexResult<String> {
