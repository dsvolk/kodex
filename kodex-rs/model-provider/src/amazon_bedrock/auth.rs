...
pub(super) fn auth_source(
        ...
        .is_some_and(|aws| aws.profile.is_some())
    {
        ...
        Some(KodexAuth::BedrockApiKey(_))
    ) {
        BedrockAuthSource::ManagedBearerToken
    } else if matches!(
        auth_manager.and_then(AuthManager::auth_cached),
        Some(KodexAuth::BedrockAccessKeys(_))
    ) {
...
pub(super) async fn resolve_auth_method(
    ...
    match source {
        BedrockAuthSource::CommandBearerToken => Err(KodexErr::Fatal(
        ...
        BedrockAuthSource::CredentialExport => {
            ...
            let context = AwsAuthContext::load_with_credentials_provider(config, credential_export)
                .await
                .map_err(aws_auth_error_to_kodex_error)?;
            Ok(BedrockAuthMethod::AwsSdkAuth { context })
        }
        BedrockAuthSource::ManagedBearerToken => {
            let Some(KodexAuth::BedrockApiKey(auth)) = managed_auth else {
                return Err(KodexErr::Fatal(
                    "selected Kodex-managed Amazon Bedrock API key is no longer available"
                        .to_string(),
                ));
            };
        ...
        BedrockAuthSource::ManagedAccessKeys => {
            let Some(KodexAuth::BedrockAccessKeys(auth)) = managed_auth else {
                return Err(KodexErr::Fatal(
                    "selected Kodex-managed Amazon Bedrock access keys are no longer available"
                        .to_string(),
                ));
            };
...
fn aws_auth_error_to_kodex_error(error: AwsAuthError) -> KodexErr {
    KodexErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Kodex agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> kodex_api::AuthProviderFuture<'_> {
...
mod tests {
    ...
    fn bedrock_auth_source_distinguishes_static_environment_credentials() {
        ...
        let managed_auth =
            AuthManager::from_auth_for_testing(KodexAuth::BedrockApiKey(BedrockApiKeyAuth {
                api_key: "managed-bedrock-api-key".to_string(),
                region: "us-east-1".to_string(),
            }));
        let managed_access_keys = AuthManager::from_auth_for_testing(KodexAuth::BedrockAccessKeys(
