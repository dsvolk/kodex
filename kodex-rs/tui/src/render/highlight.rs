...
//! | `KODEX_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static KODEX_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and kodex home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `kodex_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), kodex_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let kodex_home_set_ok = KODEX_HOME.set(kodex_home.clone()).is_ok();
    #[cfg(not(test))]
    ...
    if !override_set_ok || !kodex_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, kodex_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = kodex_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$KODEX_HOME/themes/{name}.tmTheme"));
    if resolve_theme_by_name(name, kodex_home).is_some() {
        return None;
    }
...
fn custom_theme_path(name: &str, kodex_home: &Path) -> PathBuf {
    kodex_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{kodex_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, kodex_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, kodex_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, kodex_home: Option<&Path>) -> Theme {
    let ts = two_face::theme::extra();

    // Honor user-configured theme if valid.
    if let Some(name) = name {
        if let Some(theme) = resolve_theme_by_name(name, kodex_home) {
            return theme;
        }
...
fn build_default_theme() -> Theme {
    ...
    let kodex_home = KODEX_HOME
        .get()
        .and_then(|kodex_home| kodex_home.as_deref());
    resolve_theme_with_override(name, kodex_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        let home = KODEX_HOME.get().and_then(|home| home.as_deref());
...
pub(crate) fn resolve_theme_by_name(name: &str, kodex_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{KODEX_HOME}/themes/`.
...
/// found in `{kodex_home}/themes/`.
pub(crate) fn list_available_themes(kodex_home: Option<&Path>) -> Vec<ThemeEntry> {
    ...
    for (name, _) in model_themes::THEMES {
        if !entries.iter().any(|entry| entry.name == *name)
            && resolve_theme_by_name(name, kodex_home).is_some()
        {
            entries.push(ThemeEntry {
                name: (*name).to_string(),
                is_custom: kodex_home.is_some_and(|home| custom_theme_path(name, home).exists()),
            });
        }
    }
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*kodex_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*kodex_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*kodex_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*kodex_home*/ None).is_none());
    }
