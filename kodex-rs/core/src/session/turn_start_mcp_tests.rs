...
use kodex_mcp::McpResourceClientCacheKey;
...
impl kodex_extension_api::TurnLifecycleContributor for Recorder {
    fn turn_start_phase(
        &self,
        _thread_store: &kodex_extension_api::ExtensionData,
    ) -> kodex_extension_api::TurnStartPhase {
        kodex_extension_api::TurnStartPhase::RegularTaskStart
    }

    fn requires_mcp_runtime(&self, _thread_store: &kodex_extension_api::ExtensionData) -> bool {
        self.requires_mcp.load(std::sync::atomic::Ordering::Relaxed)
    }

    fn on_turn_start<'a>(
        &'a self,
        _input: kodex_extension_api::TurnStartInput<'a>,
