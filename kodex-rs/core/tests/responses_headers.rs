...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let kodex_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-kodex-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-kodex-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-kodex-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-kodex-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-kodex-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = initial_request
        ...
        .expect("x-kodex-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-kodex-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-kodex-turn-metadata"
    );
    ...
    let body_metadata: serde_json::Value = serde_json::from_str(
        initial_request.body_json()["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
            .expect("request body should include x-kodex-turn-metadata"),
    )
    .expect("body x-kodex-turn-metadata should be valid JSON");
    assert_eq!(
        (
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/kodex.git",
    ]);
...
async fn responses_stream_redacts_git_remote_credentials_from_turn_metadata() {
    ...
    let credential_bearing_origin =
        "https://alice:test-git-secret-token@example.invalid/openai/kodex.git";
    ...
    let body_metadata = request_body["client_metadata"]["x-kodex-turn-metadata"]
        .as_str()
    ...
    for (location, metadata) in [
        ...
        assert_eq!(
            remotes,
            &serde_json::json!({ "origin": "https://example.invalid/openai/kodex.git" }),
