...
use kodex_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    submit_queue_only_agent_mail(test.kodex.as_ref(), "pending mailbox input").await;
    let submission = test
        .kodex
    ...
    wait_for_turn_complete(test.kodex.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message = core_test_support::wait_for_event_match(test.kodex.as_ref(), |event| {
...
async fn build_kodex(server: &StreamingSseServer) -> Arc<KodexThread> {
    test_kodex()
        .with_config(|config| config.update_plan_enabled = true)
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming Kodex test session")
        .kodex
}

async fn submit_user_input(kodex: &KodexThread, text: &str) {
    kodex
...
async fn submit_danger_full_access_user_turn(test: &TestKodex, text: &str) {
...
async fn steer_user_input(kodex: &KodexThread, text: &str) {
...
async fn enqueue_queue_only_agent_mail(kodex: &KodexThread, text: &str) {
    kodex
...
async fn submit_queue_only_agent_mail(kodex: &KodexThread, text: &str) {
    enqueue_queue_only_agent_mail(kodex, text).await;
    kodex
        .submit(Op::RealtimeConversationListVoices)
        .await
        .expect("submit list-voices barrier");
    wait_for_event(kodex, |event| {
        matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(kodex: &KodexThread) {
    wait_for_event(kodex, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(kodex: &KodexThread, text: &str) {
...
async fn wait_for_turn_complete(kodex: &KodexThread) {
    wait_for_event(kodex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(kodex: &KodexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(kodex, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(kodex: &KodexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(kodex, |event| {
        matches!(
            event,
...
impl kodex_extension_api::ThreadLifecycleContributor<kodex_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: kodex_extension_api::ThreadStartInput<'a, kodex_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_with_previous_turn_context() {
    ...
    .await;
    let mut extensions =
        kodex_extension_api::ExtensionRegistryBuilder::<kodex_core::config::Config>::new();
    ...
    let kodex = test_kodex()
        ...
        .await
        .expect("build Kodex test session")
        .kodex;
    ...
    wait_for_turn_complete(&kodex).await;
    enqueue_queue_only_agent_mail(&kodex, CHILD_MESSAGE).await;
    wait_for_turn_complete(&kodex).await;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    kodex_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            kodex_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&kodex, INITIAL_PROMPT).await;
    wait_for_event(&kodex, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&kodex, STEER_PROMPT).await;
    wait_for_turn_complete(&kodex).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&kodex, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&kodex, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&kodex, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&kodex, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&kodex, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&kodex, "new mailbox input").await;
    wait_for_sleep_item_completed(&kodex, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&kodex).await;
    ...
    kodex.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&kodex, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = kodex.rollout_path().expect("rollout path");
    let rollout = tokio::fs::read_to_string(rollout_path)
        .await
        .expect("read rollout");
    let persisted_sleep_items = rollout
        .lines()
        .filter_map(|line| kodex_rollout::parse_rollout_line(line).ok())
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let kodex = build_kodex(&server).await;

    submit_user_input(&kodex, "first prompt").await;

    wait_for_reasoning_item_started(&kodex).await;

    submit_queue_only_agent_mail(&kodex, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&kodex, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&kodex, "second prompt").await;
    wait_for_turn_complete(&kodex).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&kodex, "second prompt").await;
...
async fn interrupt_if_no_pending_input_checks_turn_and_queue(
    ...
    let test = test_kodex()
        ...
        .await?;
    let kodex = &test.kodex;
    let TurnInputSubmission::Started { turn_id } = kodex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
    ...
    if matches!(case, ConditionalInterruptCase::PendingUserInput) {
        steer_user_input(kodex, PENDING_PROMPT).await;
    }
    if matches!(case, ConditionalInterruptCase::PendingMailbox) {
        submit_queue_only_agent_mail(kodex, PENDING_PROMPT).await;
    }
    ...
    assert_eq!(
        tokio::time::timeout(
            ...
            kodex.interrupt_if_no_pending_input(&expected_turn_id),
        )
        .await??,
        should_abort,
    );
...
async fn terminal_compaction_error_does_not_retry_pending_input(
    ...
    if failure_point == CompactionFailurePoint::PreTurn {
        submit_user_input(kodex, "initial prompt").await;
        wait_for_turn_complete(kodex).await;
    }
    if pending_input == PendingInputAfterFailure::QueuedMail {
        submit_queue_only_agent_mail(kodex, PENDING_MESSAGE).await;
    }
    submit_user_input(kodex, "prompt that needs compaction").await;
    tokio::time::timeout(
        std::time::Duration::from_secs(/*secs*/ 10),
    ...
    .await?;
    match pending_input {
        PendingInputAfterFailure::Steer => steer_user_input(kodex, PENDING_MESSAGE).await,
        ...
        PendingInputAfterFailure::TriggeringMail => {
            kodex
                ...
                .await?;
            kodex.submit(Op::RealtimeConversationListVoices).await?;
            wait_for_event(kodex, |event| {
                matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
            })
            .await;
    ...
    kodex.flush_rollout().await?;
    let history = kodex.load_history(/*include_archived*/ false).await?;
    ...
    submit_user_input(kodex, "retry after quota resets").await;
    wait_for_agent_message(kodex, "recovered answer").await;
    let completed = wait_for_event(kodex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&kodex, "resumed old task").await;
    wait_for_turn_complete(&kodex).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&kodex, "processed steered prompt").await;
    wait_for_turn_complete(&kodex).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&kodex, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&kodex, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
