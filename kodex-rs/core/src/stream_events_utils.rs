...
use futures::Future;
use kodex_memories_read::citations::parse_memory_citation;
use kodex_memories_read::citations::thread_ids_from_memory_citation;
use kodex_protocol::error::KodexErr;
use kodex_protocol::error::Result;
use kodex_protocol::memory_citation::MemoryCitation;
...
use kodex_utils_stream_parser::strip_proposed_plan_blocks;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<kodex_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                kodex_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn record_stage1_output_usage_and_detect_memory_citation(
    state_db_ctx: Option<&kodex_state::MemoryStore>,
    item: &ResponseItem,
) -> bool {
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(KodexErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![kodex_protocol::items::AgentMessageContent::Text { text: stripped }];
