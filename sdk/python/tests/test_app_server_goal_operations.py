...
from openai_kodex._goal import _GoalNotificationStream
...
from openai_kodex.generated.v2_all import TurnStatus
...
def test_private_goal_operation_coalesces_runtime_continuations(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            ...
            state, turn_id = kodex._client.start_goal_operation(  # noqa: SLF001
                thread.id,
            ...
            stream = _GoalNotificationStream(
                ...
                lambda: kodex._client.unregister_goal_operation(state),  # noqa: SLF001
                lambda: kodex._client.cancel_goal_operation(state),  # noqa: SLF001
            )
            events = list(stream)
            result = _collect_turn_result(iter(events), turn_id=turn_id)
            routes = kodex._client._router._goal_operations.copy()  # noqa: SLF001
