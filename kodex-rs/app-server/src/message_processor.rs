...
use kodex_app_server_protocol::ExperimentalApi;
...
use kodex_app_server_protocol::UserVerificationCancelResponse;
...
use kodex_extension_api::TurnStartAdmission;
...
use kodex_queue_extension::QueuedItemService;
use kodex_rollout::StateDbHandle;
use kodex_state::log_db::LogDbLayer;
use kodex_thread_store::LocalQueueStore;
use kodex_thread_store::QueueStore;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: KodexFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let gateway_login_control =
            kodex_login::GatewayLoginControl::for_runtime(&auth_manager.runtime_config());
        ...
        let thread_store = kodex_core::thread_store_from_config(config.as_ref(), state_db.clone());
        ...
        let executor_skill_provider: Arc<dyn kodex_skills_extension::SkillProvider> = Arc::new(
            kodex_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ...
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                kodex_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let kodex_request = deserialize_client_request(request);
                let result = match kodex_request {
                    Ok(kodex_request) => {
    ...
    pub(crate) async fn daemon_recovery_snapshot(
        &self,
    ) -> kodex_app_server_transport::daemon_recovery::RecoverySnapshot {
        self.thread_processor.daemon_recovery_snapshot().await
    }

    pub(crate) async fn restore_daemon_threads(
        &self,
        mut snapshot: kodex_app_server_transport::daemon_recovery::RecoverySnapshot,
    ) {
        for thread_id in snapshot.loaded {
            ...
            if let Err(err) = self
                ...
                .thread_resume(
                    ...
                    kodex_app_server_protocol::ThreadResumeParams {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = kodex_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = kodex_request.experimental_reason()
        ...
        let (turn_admission, recheck_turn_admission) = match &kodex_request {
        ...
        let event_stream_ready = match &kodex_request {
        ...
        let serialization_scope = kodex_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: kodex_request.id().clone(),
        };
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match kodex_request {
            ...
            ClientRequest::McpServerEventStreamStart { params, .. } => {
                ...
                session
                    ...
                    .await?;
                Ok(Some(
                    kodex_app_server_protocol::McpServerEventStreamStartResponse {}.into(),
                ))
            }
            ClientRequest::McpServerEventStreamStop { params, .. } => {
                session
                    ...
                    .await;
                Ok(Some(
                    kodex_app_server_protocol::McpServerEventStreamStopResponse {}.into(),
                ))
            }
