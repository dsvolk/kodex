...
async fn daemon_version_notice_preserves_manual_update_guidance() {
    ...
    app.app_server_target = AppServerTarget::LocalDaemon {
        ...
        endpoint: crate::RemoteAppServerEndpoint::UnixSocket {
            socket_path: AbsolutePathBuf::relative_to_current_dir("kodex.sock").unwrap(),
        },
    };
    ...
    for (client, server, comparison) in [
        ...
        assert_eq!(
            ...
            Some(format!(
                "A background Kodex service is running v{server}, {comparison} your Kodex CLI v{client}."
            ))
        );
    ...
    Service v0.155.0-alpha.22 < Kodex CLI v0.155.0-alpha.23 · /daemon
    Service v0.156.0 ≠ Kodex CLI v0.155.0-alpha.23 · /daemon
    Service v0.156.0 ≠ Kodex CLI v0.0.0 · /daemon
    ");
}
...
async fn daemon_menu_is_read_only_and_confirmation_can_cancel_or_handoff() {
    ...
    std::fs::write(package.path().join("bin/kodex"), "CLI").unwrap();
    std::fs::write(package.path().join("kodex-package.json"), "{}").unwrap();
    app.daemon_cli_executable =
        Some(AbsolutePathBuf::from_absolute_path(package.path().join("bin/kodex")).unwrap());
    app.app_server_target = AppServerTarget::LocalDaemon {
        allow_embedded_fallback: true,
        endpoint: crate::RemoteAppServerEndpoint::UnixSocket {
            socket_path: AbsolutePathBuf::relative_to_current_dir("kodex.sock").unwrap(),
    ...
    insta::assert_snapshot!(overview.lines().find(|line| line.contains("Service v")).unwrap().trim(), @"Service v0.153.0 < Kodex CLI v0.154.0 · /daemon");
    ...
    for (width, source, snapshot) in [
        ...
        app.daemon_cli_executable = Some(
            AbsolutePathBuf::from_absolute_path(if cfg!(windows) {
                r"C:\cli-build\bin\kodex"
            } else {
                "/x/cli-build/bin/kodex"
            })
            .unwrap(),
        );
        ...
        insta::assert_snapshot!(
            ...
            render_bottom_popup(&app.chat_widget, width)
                .replace(r"C:\cli-build\bin\kodex", "/x/cli-build/bin/kodex")
        );
    ...
    std::fs::remove_file(package.path().join("kodex-package.json")).unwrap();
    // The full CLI can have any filename; capability comes from its entry point.
    app.daemon_cli_executable =
        Some(AbsolutePathBuf::from_absolute_path(package.path().join("bin/kodex-tui")).unwrap());
    app.open_daemon_menu();
    insta::assert_snapshot!(
