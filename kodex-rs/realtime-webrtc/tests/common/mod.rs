...
use anyhow::Result;
use anyhow::ensure;
use kodex_realtime_webrtc::AudioState;
use kodex_realtime_webrtc::Message;
use kodex_realtime_webrtc::encode_frame;
use kodex_realtime_webrtc::read_message;
use std::fs;
use std::io::Write;
...
const APP: &str = if cfg!(windows) { "kodex.exe" } else { "kodex" };
const HELPER: &str = if cfg!(windows) {
    "kodex-voice-host.exe"
} else {
    "kodex-voice-host"
};
...
pub fn package(test: &str) -> Result<Option<PathBuf>> {
    ...
    if source.file_name().is_some_and(|name| name == APP) {
        kodex_build_info::BuildInfo::initialize(BUILD_COMMIT);
        return Ok(Some(
            source
                .parent()
                .context("bin")?
    ...
    fs::create_dir_all(root.join("kodex-resources/voice/bin"))?;
    fs::write(root.join("kodex-package.json"), "{}")?;
    fs::copy(&source, root.join("bin").join(APP))?;
    fs::copy(&source, root.join("kodex-resources/voice/bin").join(HELPER))?;
