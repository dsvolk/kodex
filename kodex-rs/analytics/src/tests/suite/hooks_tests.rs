...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(KodexHookRunEventRequest {
        event_type: "kodex_hook_run",
        event_params: kodex_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(kodex_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(kodex_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(kodex_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(kodex_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(kodex_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
...
async fn reducer_ingests_hook_run_fact() {
    ...
    assert_eq!(payload[0]["event_type"], "kodex_hook_run");
