//! The main Kodex TUI chat surface.
//!
...
use crate::version::KODEX_CLI_VERSION;
...
use kodex_app_server_protocol::KodexErrorInfo as AppServerKodexErrorInfo;
...
use kodex_app_server_protocol::ModelVerification as AppServerModelVerification;
...
use kodex_app_server_protocol::ThreadGoal as AppThreadGoal;
use kodex_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use kodex_config::ConstraintResult;
...
use kodex_git_utils::local_git_branches;
use kodex_git_utils::recent_commits;
use kodex_otel::RuntimeMetricsSummary;
...
use kodex_protocol::approvals::GuardianAssessmentDecisionSource;
use kodex_protocol::approvals::GuardianAssessmentEvent;
use kodex_protocol::approvals::GuardianAssessmentStatus;
...
use kodex_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use kodex_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
const MEMORIES_DOC_URL: &str = "https://developers.openai.com/kodex/memories";
...
use kodex_file_search::FileMatch;
...
use kodex_utils_approval_presets::ApprovalPreset;
use kodex_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_kodex_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: kodex_feedback::KodexFeedback,
...
/// and by sending requests back to kodex-core.
///
...
pub(crate) struct ChatWidget {
    ...
    kodex_rate_limit_reached_type: Option<RateLimitReachedType>,
    kodex_spend_control_reached: Option<bool>,
    ...
    pub(crate) pet_http_client: kodex_http_client::RouteAwareClientPool,
...
fn request_permissions_from_params(
    params: kodex_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            kodex_windows_sandbox::current_log_file_path_for_kodex_home(
                &self.local_settings.kodex_home,
            )
            .is_file();
    ...
    /// Forward a command directly to kodex.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.kodex_op_target {
            KodexOpTarget::Direct(kodex_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                let is_review = op.is_review();
                if let Err(e) = kodex_op_tx.send(op) {
            ...
            KodexOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::KodexOp(op));
            }
        }
        true
    }
...
const PLACEHOLDER: &str = "Ask Kodex to do anything";
