//! Mapping from Kodex protocol events into raw rollout-trace events.
//!
...
//! `kodex-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `kodex-rollout-trace`.
//!
...
use kodex_protocol::protocol::PatchApplyStatus;
use kodex_protocol::protocol::SubAgentActivityEvent;
...
pub(crate) struct KodexTurnTraceEvent {
    pub context_turn_id: KodexTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn kodex_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<KodexTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let kodex_turn_id = event.turn_id.clone();
            Some(KodexTurnTraceEvent {
                context_turn_id: kodex_turn_id.clone(),
                payload: RawTraceEventPayload::KodexTurnStarted {
                    kodex_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(KodexTurnTraceEvent {
                context_turn_id: kodex_turn_id.clone(),
                payload: RawTraceEventPayload::KodexTurnEnded {
                    kodex_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a kodex_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a kodex_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a kodex_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a kodex_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a kodex_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a kodex_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a kodex_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a kodex_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [kodex_protocol::parse_command::ParsedCommand],
