...
use kodex_context_fragments::AnsweredQuestion;
use kodex_context_fragments::ContextualUserFragment;
...
use kodex_protocol::items::AgentMessageDelivery;
...
fn write_scenario_capabilities(home: &TempDir) -> Result<ScenarioSkills> {
    ...
    for (name, description) in [
        ...
        let manifest = plugin_cache
            .join(name)
            .join("local/.kodex-plugin/plugin.json");
