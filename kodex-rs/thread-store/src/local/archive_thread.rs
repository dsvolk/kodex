...
) -> ThreadStoreResult<Vec<kodex_protocol::ThreadId>> {
...
async fn archive_thread_with_paths(
    ...
    for rollout_path in rollout_paths {
        if rollout_path_is_archived(store.config.kodex_home.as_path(), rollout_path.as_path()) {
            continue;
        }
        let canonical_rollout_path = scoped_rollout_path(
            store.config.kodex_home.join(kodex_rollout::SESSIONS_SUBDIR),
