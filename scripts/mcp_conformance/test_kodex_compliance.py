...
import pytest
from run_kodex_compliance import (
...
def test_registration_commands_select_stdio_then_http_shapes() -> None:
    ...
    assert stdio == [
        ...
        TEST_SERVER_NAME,
        "--env",
        f"KODEX_MCP_PROTOCOL_VERSION={MODERN_VERSION}",
        "--",
        sys.executable,
        "/src/server.py",
        "--mode",
    ...
    valid, _ = _validate_registration(
        {
            ...
            "transport": {
                ...
                "env": {"KODEX_MCP_PROTOCOL_VERSION": MODERN_VERSION},
            },
        },
        transport="stdio",
        mode=MODERN_VERSION,
...
def test_isolated_environment_drops_model_credentials(
    ...
    monkeypatch.setenv("KODEX_API_KEY", "secret")
    monkeypatch.setenv("KODEX_ACCESS_TOKEN", "secret")
    monkeypatch.setenv("OPENAI_API_KEY", "secret")

    env = _isolated_environment(tmp_path)

    assert env["KODEX_HOME"] == str(tmp_path)
    assert "KODEX_API_KEY" not in env
    assert "KODEX_ACCESS_TOKEN" not in env
...
def test_scenario_ratio_includes_adapter_failures() -> None:
    checks = [
        ...
        CheckResult(
            "harness/auth/example/kodex-adapter",
            False,
            "adapter failed",
            source="harness",
            scenario="auth/example",
...
def test_official_case_preserves_adapter_check_identity_on_failure_and_success(
    ...
    monkeypatch.setattr(
        "run_kodex_compliance.run_official_mode",
        lambda **_kwargs: [result],
    )

    case = _run_official_case(
        Path("/opt/kodex"),
        Path("/src/kodex_conformance_adapter.py"),
    ...
    assert case.checks == [
        CheckResult(
            name=f"harness/{scenario}/kodex-adapter",
            success=adapter_success,
...
def test_official_case_keeps_cimd_registration_checks_in_regression_gated_http_case(
    ...
    assert auto_env.get("KODEX_CONFORMANCE_CLIENT_REGISTRATION") is None
    assert forced_env["KODEX_CONFORMANCE_CLIENT_REGISTRATION"] == "cimd"
    ...
    assert gate["fixedChecks"] == [
        {
            ...
            "source": "harness",
            "scenario": scenario,
            "check_id": f"harness/{scenario}/kodex-adapter",
        },
        {
            "mode": MODERN_VERSION,
            "transport": "official-http",
...
def test_regression_cli_accepts_a_baseline_report() -> None:
    args = _parse_args(
        ["/opt/kodex", "--baseline-report", "/tmp/kodex-mcp-baseline.json"]
    )

    assert args.baseline_report == Path("/tmp/kodex-mcp-baseline.json")
...
def test_regression_cli_extracts_compact_baseline_without_starting_conformance(
    ...
    kodex = tmp_path / "kodex"
    kodex.write_text("#!/bin/sh\nexit 99\n", encoding="utf-8")
    kodex.chmod(0o755)
...
def test_regression_cli_rejects_extraction_without_a_source_report(
    ...
    assert main([str(kodex), "--extract-baseline", str(tmp_path / "compact.json")]) == 2
...
def test_regression_cli_rejects_malformed_baselines(
    ...
    assert main([str(kodex), "--baseline-report", str(baseline)]) == 2
...
def test_regression_cli_rejects_missing_baseline_before_starting_conformance(
    ...
    assert main([str(kodex), "--baseline-report", str(tmp_path / "missing.json")]) == 2
