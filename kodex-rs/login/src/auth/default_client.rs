//! Default Kodex HTTP client: shared `User-Agent`, `originator`, optional residency header, and
...
/// See https://github.com/openai/kodex/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Kodex. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "kodex_cli_rs";
pub const KODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "KODEX_INTERNAL_ORIGINATOR_OVERRIDE";
pub use kodex_model_provider_info::RESIDENCY_HEADER_NAME;
pub use kodex_model_provider_info::ResidencyRequirement;
pub use kodex_model_provider_info::read_managed_residency_requirement as read_default_client_residency_requirement;
pub use kodex_model_provider_info::set_managed_residency_requirement as set_default_client_residency_requirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(KODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(KODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "kodex-tui"
        || originator_value == "kodex_vscode"
        || originator_value.starts_with("Kodex ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "kodex_atlas" || originator_value == "kodex_chatgpt_desktop"
}

pub fn get_kodex_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Kodex user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Kodex user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Kodex originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Kodex's route-aware system/PAC resolution.
...
/// Builds the default Kodex HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Kodex
...
fn create_client_for_route_with_builder(
    ...
    if http_client_factory.network_policy().is_managed() {
        let mut pool = kodex_http_client::RouteAwareClientPool::with_builder(
...
/// Builds the default Kodex HTTP client for a concrete outbound route without blocking the
...
/// Builds the default Kodex transport without blocking the async runtime worker.
///
/// Route-aware proxy handling resolves each request and redirect destination. When it is disabled,
/// or the client is running inside the Kodex sandbox, this preserves the default client's behavior.
...
/// Builds an HTTP client for an auth endpoint without Kodex default headers.
...
/// Builds the default Kodex HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_kodex_user_agent()) {
