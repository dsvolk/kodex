...
use kodex_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use kodex_core_plugins::marketplace::marketplace_root_dir;
use kodex_core_plugins::marketplace_add::MarketplaceAddOutcome;
use kodex_core_plugins::marketplace_add::MarketplaceAddRequest;
use kodex_core_plugins::marketplace_add::add_marketplace;
use kodex_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use kodex_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "kodex plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Kodex is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "kodex plugin marketplace add",
    after_help = "Examples:\n  kodex plugin marketplace add ./path/to/marketplace\n  kodex plugin marketplace add owner/repo --ref main\n  kodex plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "kodex plugin marketplace upgrade",
    after_help = "Examples:\n  kodex plugin marketplace upgrade\n  kodex plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "kodex plugin marketplace remove",
    after_help = "Example:\n  kodex plugin marketplace remove debug"
)]
...
async fn run_list(config: Config, args: ListMarketplaceArgs) -> Result<()> {
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.kodex_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<kodex_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, kodex_home);
...
async fn run_upgrade(
    ...
    builder: kodex_core::config::ConfigBuilder,
) -> Result<()> {
    ...
    let reload_config: kodex_core_plugins::ConfigLayerReload = std::sync::Arc::new(move || {
        runtime
