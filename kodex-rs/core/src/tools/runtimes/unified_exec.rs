...
use kodex_shell_command::powershell::prefix_powershell_script_with_utf8;
...
pub struct UnifiedExecRequest {
    ...
    pub shell_snapshot: Option<kodex_exec_server::ShellSnapshotRequest>,
...
impl<'a> ToolRuntime<UnifiedExecRequest, UnifiedExecAttempt> for UnifiedExecRuntime<'a> {
    ...
    async fn run(
        ...
        let (mut env, managed_network_context, network_proxy_launch) = match managed_network {
            Some(network) if environment_is_remote => {
                let mut launch = network
                    ...
                    .await
                    .map_err(|err| {
                        ToolError::Kodex(KodexErr::Io(io::Error::other(err.to_string())))
                    })?;
                ...
                if !launch.proxy.enabled {
                    ...
                    let environment_info =
                        req.turn_environment
                            ...
                            .await
                            .map_err(|err| {
                                ToolError::Kodex(KodexErr::Io(io::Error::other(format!(
            ...
            && !environment_is_remote
        {
            ...
            {
                if !kodex_network_proxy::is_credential_broker_provider_env_key(&key)
        ...
        let brokered_shell_snapshot_missing = !environment_is_remote
            && managed_network.is_some()
            && env
                .get(kodex_network_proxy::CREDENTIAL_BROKER_ACTIVE_ENV_KEY)
                .is_some_and(|active| active == "1")
            && command
                .get(1)
        ...
        let snapshot_permissions = shell_snapshot_location
            .as_ref()
            .filter(|_| {
                env.get(kodex_network_proxy::CREDENTIAL_BROKER_ACTIVE_ENV_KEY)
                    .is_some_and(|active| active == "1")
            })
            .and_then(|path| {
        ...
        TurnEnvironment::new(
            TurnEnvironmentSelection {
                ...
                config: EnvironmentConfigState::Ready(EnvironmentConfig {
                    ...
                    windows_sandbox_type: kodex_sandboxing::SandboxType::None,
