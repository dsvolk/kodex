...
async fn real_imds_credentials_stop_after_policy_revocation() -> Result<(), Box<dyn Error>> {
    ...
    assert!(
        matches!(
            &denied,
            Err(Policy(kodex_http_client::NetworkPolicyDenied::Destination))
        ),
        "{denied:?}"
    );
    assert!(
...
-> Result<(), Box<dyn Error>> {
    ...
    let trust = kodex_http_client::HttpClientTlsConfig::default()
    ...
    let client = kodex_http_client::HttpClientBuilder::new().build_with_tls(
        &factory,
...
-> Result<(), Box<dyn Error>> {
    for case in [
        ...
        child
            ...
            .env("KODEX_AWS_PROFILE_PROBE_CASE", case)
...
fn sdk_profile_probe() -> Result<(), Box<dyn Error>> {
    let case = std::env::var("KODEX_AWS_PROFILE_PROBE_CASE")?;
