...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "kodex.external_agent_config.import";

#[derive(Clone)]
pub struct ExternalAgentConfigService {
    pub(super) kodex_home: PathBuf,
...
impl ExternalAgentConfigService {
    ...
    pub fn kodex_home(&self) -> &Path {
        &self.kodex_home
    }

    #[cfg(test)]
    fn new_for_test(kodex_home: PathBuf, external_agent_home: PathBuf) -> Self {
        let source = ExternalAgentSource::default();
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            kodex_home,
            ...
            auth_manager: kodex_login::test_support::auth_manager_from_optional_auth(
                /*auth*/ None,
            ),
            source,
            session_import_limits: ExternalAgentSessionImportLimits::default(),
            ...
            let import_result = match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Memory if self.source.supports_memory() => {
                    async {
                        ...
                        let target_path = memory_import::resources_root(&self.kodex_home);
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let target_config = match &scope {
            MigrationScope::Home => self.kodex_home.join("config.toml"),
            MigrationScope::Repository { root } => root.join(".kodex").join("config.toml"),
        };
    ...
    fn import_hooks(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        let target_hooks = match &scope {
            MigrationScope::Home => self.kodex_home.join("hooks.json"),
            MigrationScope::Repository { root } => root.join(".kodex").join("hooks.json"),
        };
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            ...
            MigrationScope::Home => {
                ...
                (source_agents_md, self.kodex_home.join("AGENTS.md"))
            }
        };
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Kodex.matches_product_restriction(products))
            })
