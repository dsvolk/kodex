...
pub fn registered_core_requested() -> bool {
    ...
    *REQUESTED.get_or_init(|| {
        requested_value(std::env::var_os("KODEX_WINDOWS_REGISTERED_CORE").as_deref())
    })
}
...
pub(crate) fn registered_setup_is_ready(kodex_home: &Path) -> Result<bool> {
    for account in [
    ...
    Ok(record.kodex_home == kodex_home.canonicalize()?
...
#[doc(hidden)]
pub fn registered_core_needs_refresh(kodex_home: &Path) -> Result<bool> {
    let Some(package) = current_package_full_name()? else {
        return Ok(false);
    };
    let Some(record) = crate::runtime_ownership::load_installation()? else {
...
pub(crate) fn registered_runner_alias(
    ...
    anyhow::ensure!(
        record.user_sid
            ...
            && record.kodex_home == kodex_home.canonicalize()?
            && record.runtime()?.ready_for_package(&package),
