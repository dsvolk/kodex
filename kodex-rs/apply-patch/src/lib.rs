...
use kodex_utils_path_uri::PathUriParseError;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the Kodex executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `kodex-apply-patch` (to avoid forcing
/// `kodex-arg0` to depend on `kodex-core`), it remains part of the "kodex core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const KODEX_CORE_APPLY_PATCH_ARG1: &str = "--kodex-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {
