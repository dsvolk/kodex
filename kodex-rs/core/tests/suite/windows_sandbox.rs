...
use kodex_core::exec::process_exec_tool_call;
use kodex_core::sandboxing::SandboxPermissions;
use kodex_core::windows_sandbox::WindowsSandboxSetupMode;
use kodex_core::windows_sandbox::WindowsSandboxSetupRequest;
use kodex_core::windows_sandbox::run_windows_sandbox_setup;
...
fn kodex_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestKodexHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep KODEX_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let kodex_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&kodex_home)
            .with_context(|| format!("create stable test KODEX_HOME {}", kodex_home.display()))?;
        return Ok(TestKodexHome::Persistent(kodex_home));
    }

    Ok(TestKodexHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("kodex-resources");
    ...
    for helper_name in ["kodex-windows-sandbox-setup", "kodex-command-runner"] {
        let helper = kodex_utils_cargo_bin::cargo_bin(helper_name)?;
...
fn stage_windows_sandbox_cli(fixture_bin: &Path) -> anyhow::Result<(PathBuf, PathBuf)> {
    ...
    let resources_dir = fixture_bin.join("kodex-resources");
    std::fs::create_dir_all(&resources_dir)?;

    let kodex_source = kodex_utils_cargo_bin::cargo_bin("kodex")?;
    let kodex = fixture_bin.join("kodex.exe");
    std::fs::copy(&kodex_source, &kodex)
        .with_context(|| format!("copy {} to {}", kodex_source.display(), kodex.display()))?;
    for helper_name in ["kodex-windows-sandbox-setup", "kodex-command-runner"] {
        let helper = kodex_utils_cargo_bin::cargo_bin(helper_name)?;
    ...
    let probe_source = kodex_utils_cargo_bin::cargo_bin("kodex-windows-managed-deny-probe")?;
...
fn windows_sandbox_cli_preserves_managed_deny_reads_across_launches() -> anyhow::Result<()> {
    let kodex_home =
        kodex_home_for_windows_sandbox_test("windows-cli-managed-deny-read-kodex-home")?;
    ...
    let (kodex, probe) = stage_windows_sandbox_cli(&bin)?;
    ...
    for launch in 1..=2 {
        let output = Command::new(&kodex)
            .current_dir(&work)
            .env("KODEX_HOME", kodex_home.path())
            .env("KODEX_WINDOWS_ALLOWED_TEXT", &allowed_text)
            .env("KODEX_WINDOWS_DENIED_TEXT", &denied_text)
            .env("KODEX_WINDOWS_ALLOWED_MODULE", &allowed_module)
            .env("KODEX_WINDOWS_DENIED_MODULE", &denied_module)
...
async fn windows_elevated_setup_rejects_default_root_deny() -> anyhow::Result<()> {
    let kodex_home = kodex_home_for_windows_sandbox_test("windows-elevated-root-deny-kodex-home")?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let kodex_home =
        kodex_home_for_windows_sandbox_test("windows-restricted-token-deny-read-kodex-home")?;
    let _kodex_home_guard = EnvVarGuard::set("KODEX_HOME", kodex_home.path().as_os_str());
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let kodex_home =
        kodex_home_for_windows_sandbox_test("windows-elevated-missing-metadata-kodex-home")?;
    let _kodex_home_guard = EnvVarGuard::set("KODEX_HOME", kodex_home.path().as_os_str());
    ...
    for name in kodex_protocol::permissions::PROTECTED_METADATA_PATH_NAMES {
...
$sid = (Get-LocalUser -Name 'KodexSandboxOffline').SID.Value
$rules = foreach ($name in @('kodex_sandbox_offline_block_inbound', 'kodex_sandbox_offline_block_outbound')) {
...
"#,
    ...
    let expected_rules: Vec<_> = [
        ("kodex_sandbox_offline_block_inbound", "Inbound"),
        ("kodex_sandbox_offline_block_outbound", "Outbound"),
    ]
    .into_iter()
    .map(|(name, direction)| {
        serde_json::json!({
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let kodex_home = kodex_home_for_windows_sandbox_test("windows-elevated-deny-read-kodex-home")?;
    let _kodex_home_guard = EnvVarGuard::set("KODEX_HOME", kodex_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
    ...
    let bundled_skill_dir = user_profile.path().join(".kodex/plugins/cache");
    std::fs::create_dir_all(&bundled_skill_dir)?;
    let bundled_skill = bundled_skill_dir.join("SKILL.md");
    let setup_marker = kodex_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(kodex_home.path()),
...
async fn windows_elevated_unified_exec_enforces_managed_deny_reads() -> anyhow::Result<()> {
    let kodex_home =
        kodex_home_for_windows_sandbox_test("windows-elevated-tool-runtime-deny-read-kodex-home")?;
    let _kodex_home_guard = EnvVarGuard::set("KODEX_HOME", kodex_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;

    let configured_kodex_home = dunce::canonicalize(kodex_home.path())?.abs();
    let builder = test_kodex()
        .with_windows_cmd_shell()
        .with_config(move |config| {
            config.kodex_home = configured_kodex_home;
