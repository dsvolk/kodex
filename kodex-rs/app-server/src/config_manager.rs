...
use kodex_cloud_config::cloud_config_bundle_loader;
...
use kodex_config::loader::load_managed_requirements_state;
...
use kodex_utils_json_to_toml::json_to_toml;
...
#[error(
    "Your organization's required model provider settings changed. Restart Kodex to apply them; this request was not sent"
)]
pub(crate) struct ModelProviderRequirementsChanged;

/// Shared app-server entry point for loading effective Kodex configuration.
#[derive(Clone)]
pub(crate) struct ConfigManager {
    ...
    network_policy: kodex_http_client::NetworkPolicyController,
    local_network_policy: kodex_http_client::NetworkPolicyController,
...
struct ApplicationPolicySnapshot {
    revision: kodex_http_client::NetworkPolicyRevision,
    policy: kodex_http_client::DestinationPolicy,
    cloud: Option<kodex_config::CloudConfigBundle>,
}
...
impl ConfigManager {
    pub(crate) fn new(
        ...
        let network_policy = kodex_http_client::NetworkPolicyController::default();
        Self {
            kodex_home,
    ...
    pub(crate) fn kodex_home(&self) -> &Path {
    ...
    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.kodex_home())
    }
    ...
    pub(crate) fn replace_cloud_config_bundle_loader(
        ...
        let endpoint = kodex_backend_client::Client::new(
    ...
    pub(crate) async fn load_retained_session_config(
        ...
        let mut manager = self.clone();
        manager.thread_config_loader = Arc::new(kodex_config::NoopThreadConfigLoader);
        ...
        let mut config = Config::rebuild_with_session_layers(
            ...
            cwd.to_path_buf(),
            &refreshed_layers,
            AbsolutePathBuf::from_absolute_path(&self.kodex_home)?,
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            ...
            .fallback_cwd(Some(self.kodex_home.clone()))
    ...
    pub(crate) async fn load_permission_config_for_thread(
        ...
        for layer in thread_config.config_layer_stack.layers_low_to_high() {
            if matches!(layer.name, kodex_config::ConfigLayerSource::SessionFlags) {
                kodex_config::merge_toml_values(&mut session_flags, &layer.config);
            }
        }
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let result = kodex_core::config::ConfigBuilder::default()
            .kodex_home(self.kodex_home.clone())
    ...
    fn apply_network_policy(&self, config: &mut Config) {
        ...
        config.application_auth_route_config = Some(
            kodex_login::AuthRouteConfig::from_http_client_factory(
                config
                    .http_client_factory()
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.kodex_self_exe = self.arg0_paths.kodex_self_exe.clone();
        config.kodex_linux_sandbox_exe = self.arg0_paths.kodex_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(kodex_home: PathBuf) -> Self {
        Self::new_for_tests(
            kodex_home,
            Vec::new(),
