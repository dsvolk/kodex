//! Bridges Apps SDK-style `openai/fileParams` metadata into Kodex's MCP flow.
//!
//! Strategy:
...
//!
//! The model-facing local-path schema is owned by `kodex-mcp` alongside MCP tool inventory, so this
...
use kodex_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use kodex_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("ChatGPT auth is required to upload files for Kodex Apps tools".to_string());
    };
    ...
    let upload_auth = kodex_model_provider::auth_provider_from_auth(auth);
