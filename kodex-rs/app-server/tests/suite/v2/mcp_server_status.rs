...
use kodex_app_server_protocol::McpServerConnectionStatus;
...
-> Result<()> {
    ...
    let client_id = format!("https://chatgpt.com/oauth/kodex/{callback_id}/client.json");
...
async fn mcp_server_status_list_returns_raw_server_and_tool_names(plugin: bool) -> Result<()> {
    ...
    if plugin {
        ...
        std::fs::create_dir_all(plugin_root.path().join("sample/.kodex-plugin"))?;
        std::fs::write(
            plugin_root.path().join("sample/.kodex-plugin/plugin.json"),
...
-> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let kodex_home = TempDir::new()?;
    let barrier_file = kodex_home.path().join("allow-initialize");
    let pid_file = kodex_home.path().join("mcp.pid");
...
async fn mcp_server_status_list_reports_disconnected_stdio_transport() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let kodex_home = TempDir::new()?;
    let exit_file = kodex_home.path().join("exit-mcp-server");
    mock_responses_config(&server.uri())
        .with_extra_config(&format!(
