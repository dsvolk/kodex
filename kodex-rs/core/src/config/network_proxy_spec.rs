...
use kodex_network_proxy::BlockedRequestObserver;
use kodex_network_proxy::ConfigReloader;
use kodex_network_proxy::ConfigReloaderFuture;
use kodex_network_proxy::ConfigState;
use kodex_network_proxy::EnvironmentNetworkPolicy;
use kodex_network_proxy::LocalBindingPolicy;
...
use kodex_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use kodex_network_proxy::managed_proxy_ports;
use kodex_network_proxy::normalize_host;
use kodex_network_proxy::validate_policy_against_constraints;
...
impl NetworkProxySpec {
    ...
    pub fn windows_sandbox_proxy_listeners(
        ...
        kodex_windows_sandbox::WindowsSandboxProvisioningSettings,
        kodex_windows_sandbox::WindowsSandboxProxyListeners,
    )> {
        if !self.config.enabled {
            return Ok((
                kodex_windows_sandbox::WindowsSandboxProvisioningSettings::default(),
                kodex_windows_sandbox::WindowsSandboxProxyListeners::default(),
            ));
        }
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    kodex_network_proxy::NetworkDomainPermission::Allow
                } else {
                    kodex_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
