...
- From the Kodex CLI, run `kodex execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
kodex execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
kodex execpolicy check \
...
```bash
cargo run -p kodex-execpolicy -- check --rules path/to/policy.rules git status
```
