...
const LOCK_CONTENTION_EVENT_TARGET: &str = "kodex_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are KODEX_HOME-backed today: if KODEX_HOME is unset they use
        // the default home (`~/.kodex`), and if an embedder has no local home/filesystem authority
        ...
        let kodex_home = find_kodex_home()
            .map_err(|source| OAuthStoreLockFailure::KodexHome { store, source })?;
        Self::acquire_in_with_mode(&kodex_home, store, acquire_timeout, mode)
    }
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(kodex_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve KODEX_HOME for MCP OAuth {store} aggregate-store lock")]
    KodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(kodex_home: &Path, store: OAuthStore) -> PathBuf {
    kodex_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}
