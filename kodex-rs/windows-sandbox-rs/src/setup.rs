...
pub const OFFLINE_USERNAME: &str = "KodexSandboxOffline";
pub const ONLINE_USERNAME: &str = "KodexSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "kodex-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(kodex_home: &Path) -> PathBuf {
    kodex_home.join(".sandbox")
}

pub fn sandbox_bin_dir(kodex_home: &Path) -> PathBuf {
    kodex_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(kodex_home: &Path) -> PathBuf {
    kodex_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(kodex_home: &Path) -> PathBuf {
    sandbox_dir(kodex_home).join("setup_marker.json")
}

pub fn sandbox_users_path(kodex_home: &Path) -> PathBuf {
    sandbox_secrets_dir(kodex_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, kodex_home, runtime);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        kodex_home: request.kodex_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.kodex_home)
    })
}

fn run_setup_refresh_payload(b64: &str, kodex_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(kodex_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(kodex_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    cmd.arg(b64)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let cwd = std::env::current_dir().unwrap_or_else(|_| kodex_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(kodex_home) {
        log_note(
...
fn gather_helper_read_roots(kodex_home: &Path, runtime: SetupRuntime) -> Vec<PathBuf> {
    if runtime == SetupRuntime::Registered {
        return Vec::new();
    }
    let helper_dir = helper_bin_dir(kodex_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(kodex_home, runtime);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, kodex_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<kodex_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "KODEX_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "KODEX_WINDOWS_SANDBOX_PROXY_PORTS";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(kodex_home) {
...
fn verify_setup_completed(kodex_home: &Path) -> Result<()> {
...
fn run_setup_exe(
    ...
    if !retained_handles.is_empty() {
        ...
        return run_setup_exe_payload(&payload_b64, needs_elevation, kodex_home, retained_handles);
    }
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, kodex_home, retained_handles)
    })
}
...
fn run_setup_exe_payload(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(kodex_home)?;
        if let Err(err) = clear_setup_error_report(kodex_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.kodex_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.kodex_home, &[])
}
...
fn elevated_provisioning_payload(
    ...
    ElevationPayload {
        ...
        online_username: ONLINE_USERNAME.to_string(),
        kodex_home: request.kodex_home.to_path_buf(),
        command_cwd: request.kodex_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user,
        otel: kodex_otel::global_statsig_metrics_settings(),
...
pub fn run_elevated_provisioning_setup_with_retained_handles(
    ...
    if !kodex_home.is_absolute()
        || !matches!(
            kodex_home.components().next(),
        ...
        )
    {
        return Err(failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!(
                "sandbox provisioning KODEX_HOME must be an absolute local disk path: {}",
                kodex_home.display()
            ),
        ));
    }
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: kodex_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
        deny_read_paths: Vec::new(),
        deny_write_paths: Vec::new(),
...
pub(crate) fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.kodex_home, runtime);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, kodex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to KODEX_HOME or anything under KODEX_HOME/.sandbox,
    // KODEX_HOME/.sandbox-bin, or KODEX_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let kodex_home_key = canonical_path_key(kodex_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(kodex_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(kodex_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(kodex_home));
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let kodex_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(kodex_home.path())
    ...
    fn elevated_setup_payload_contains_no_caller_acl_roots() {
        ...
        assert_eq!(payload.command_cwd, kodex_home);
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("kodex.exe");
        let setup_exe = resources_dir.join("kodex-windows-sandbox-setup.exe");
        fs::write(&exe, b"kodex").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "KODEX_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_kodex_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let kodex_home = user_profile.join("KodexHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&kodex_home).expect("create kodex home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &kodex_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let expected =
            dunce::canonicalize(helper_bin_dir(&kodex_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&kodex_home);
        fs::create_dir_all(&kodex_home).expect("create kodex home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_kodex_home = dunce::canonicalize(&kodex_home).expect("canonical kodex home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_kodex_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_kodex).expect("canonical extra .kodex"),
                explicit_deny,
            ]
            .into_iter()
