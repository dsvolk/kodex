...
const BEFORE_PANIC: &str = "\x1b]1337;kodex-before-panic\x07";
const BEFORE_RECOVERY: &str = "\x1b]1337;kodex-before-recovery\x07";
const BEFORE_VISIBLE_FRAME: &str = "\x1b]1337;kodex-before-visible-frame\x07";
const AFTER_VISIBLE_FRAME: &str = "\x1b]1337;kodex-after-visible-frame\x07";
...
fn caught_panic_rehides_cursor_before_repainting_and_restores_its_shape() {
    ...
    let child = Command::new(std::env::current_exe().expect("current test binary"))
        ...
        .env("KODEX_TUI_CAUGHT_PANIC_PTY_CHILD", "1")
...
async fn caught_panic_pty_child() {
    if std::env::var_os("KODEX_TUI_CAUGHT_PANIC_PTY_CHILD").is_none() {
        return;
    }
