...
fn executed_tool_call_recorder_bounds_pending_calls_and_preserves_overflow() {
    ...
    for index in 0..MAX_PENDING_EXECUTED_TOOL_CALLS + 2 {
        recorder.record_call(
            &ToolCall {
                tool_name: kodex_tools::ToolName::plain(crate::tools::code_mode::PUBLIC_TOOL_NAME),
...
fn executed_tool_call_recorder_bounds_retained_history_and_keeps_latest_calls() {
    ...
    for index in 0..512 {
        ...
        kodex_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt);
    ...
    assert!(metadata.iter().all(|call| {
        call["arguments"]["_kodex_executed_tool_call_truncated"]["omitted_calls"].is_null()
    }));
}
    ...
    for truncated in [false, true] {
            ...
            .enumerate()
        {
            if index > 0 {
                recorder.record_call(
                    &ToolCall {
                        tool_name: kodex_tools::ToolName::plain(
...
fn completeness_rejects_direct_id_collision_and_late_calls() {
    ...
    recorder.record_call(
        &ToolCall {
            tool_name: kodex_tools::ToolName::plain("direct_tool"),
