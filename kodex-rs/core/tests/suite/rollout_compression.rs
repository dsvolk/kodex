...
async fn compressed_shared_fork_resume_preserves_checkpoint_and_frozen_history() -> Result<()> {
    ...
    let parent_path = test.kodex.rollout_path().context("parent rollout")?;
    ...
    assert_eq!(
        kodex_rollout::read_session_meta_line(&child_path)
            .await?
            .meta
            .history_base
    ...
    let store =
        kodex_core::thread_store_from_config(&config, kodex_core::init_state_db(&config).await);
    ...
    let resumed = test
        ...
        .resume_thread_with_history(
            ...
            kodex_core::test_support::auth_manager_from_auth(kodex_login::KodexAuth::from_api_key(
                "dummy",
            )),
            /*parent_trace*/ None,
