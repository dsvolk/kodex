...
async fn daemon_startup(command: &str) -> Result<()> {
    let kodex = kodex_utils_cargo_bin::cargo_bin("kodex")?.canonicalize()?;
    ...
    env.insert("KODEX_HOME".into(), home.path().display().to_string());
    env.insert(
        "KODEX_SQLITE_HOME".into(),
    ...
    if matches!(command, "start" | "bedrock-running") || mismatch {
        ...
        let managed = home
            .path()
            .join("packages/app-server-daemon/current/bin/kodex");
        fs::create_dir_all(home.path().join("packages/app-server-daemon/current/bin"))?;
        fs::hard_link(&kodex, &managed).or_else(|_| fs::copy(&kodex, &managed).map(|_| ()))?;
