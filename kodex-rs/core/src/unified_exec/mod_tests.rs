...
use core_test_support::test_kodex::test_env as remote_test_env;
use kodex_exec_server::ExecProcess;
use kodex_exec_server::ExecProcessEventReceiver;
use kodex_exec_server::ExecProcessFuture;
...
use kodex_exec_server::WriteResponse;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, kodex_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, kodex_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), kodex_exec_server::ExecServerError> {
...
async fn blocking_terminate_unified_process(
    ...
    Ok(Arc::new(
        UnifiedExecProcess::from_exec_server_started(StartedExecProcess {
            ...
            sandbox_type: Some(kodex_sandboxing::SandboxType::None),
        })
        .await?,
    ))
}
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        ...
        &turn,
        process_id,
        "export KODEX_INTERACTIVE_SHELL_VAR=kodex\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        ...
        &turn,
        process_id,
        "echo $KODEX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        ...
        &turn,
        process_id,
        format!("export KODEX_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        ...
        &turn,
        process_id,
        "sleep 5 && echo $KODEX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = kodex_exec_server::Environment::default_for_tests();
...
async fn stdin_approval_preserves_the_reviewed_terminal() -> anyhow::Result<()> {
    ...
    let (session, mut turn, events) = make_session_and_context_with_auth_and_config_and_rx(
        kodex_login::KodexAuth::from_api_key("Test API Key"),
        Vec::new(),
        |config| {
