...
const RATE_LIMIT_RESET_REQUEST_TIMEOUT_ENV_VAR: &str =
    "KODEX_TEST_RATE_LIMIT_RESET_REQUEST_TIMEOUT_MS";
...
async fn consume_rate_limit_reset_credit_requires_chatgpt_auth() -> Result<()> {
    ...
    let mut mcp = initialized_app_server(kodex_home.path()).await?;
    ...
    assert_eq!(
        ...
        "kodex account authentication required for rate limit reset credits"
    );
...
async fn consume_account_rate_limit_reset_credit_maps_backend_outcomes() -> Result<()> {
    let (kodex_home, server) = chatgpt_test_context().await?;
    let cases = [
        (
            "request-reset",
            "reset",
    ...
    for (idempotency_key, backend_code, _, windows_reset) in cases {
        Mock::given(method("POST"))
            .and(path("/api/kodex/rate-limit-reset-credits/consume"))
...
async fn consume_account_rate_limit_reset_credit_rejects_empty_idempotency_key() -> Result<()> {
    let (kodex_home, _server) = chatgpt_test_context().await?;
    let mut mcp = initialized_app_server(kodex_home.path()).await?;
...
async fn consume_timeout_releases_account_auth_queue() -> Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/api/kodex/accounts/check"))
        .respond_with(
...
async fn chatgpt_test_context() -> Result<(TempDir, MockServer)> {
    ...
    Ok((kodex_home, server))
}

async fn initialized_app_server(kodex_home: &Path) -> Result<TestAppServer> {
