...
use kodex_protocol::protocol::NetworkPolicyRuleAction;
...
use kodex_shell_escalation::EscalateServer;
use kodex_shell_escalation::EscalationDecision;
use kodex_shell_escalation::EscalationExecution;
use kodex_shell_escalation::EscalationPermissions;
use kodex_shell_escalation::EscalationPolicy;
use kodex_shell_escalation::EscalationPolicyFuture;
use kodex_shell_escalation::EscalationSession;
use kodex_shell_escalation::ExecResult;
use kodex_shell_escalation::PreparedExec;
use kodex_shell_escalation::ResolvedPermissionProfile;
use kodex_shell_escalation::ShellCommandExecutor;
use kodex_shell_escalation::ShellCommandExecutorFuture;
use kodex_shell_escalation::Stopwatch;
...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        kodex_linux_sandbox_exe: ctx.step_context.turn.config.kodex_linux_sandbox_exe.clone(),
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::Kodex(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<kodex_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["KODEX_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            sandbox_exe: self.kodex_linux_sandbox_exe.as_deref(),
