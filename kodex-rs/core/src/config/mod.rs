...
use kodex_agent_roles::load_agent_roles;
...
use kodex_config::ConfigRequirements;
use kodex_config::ConfigRequirementsToml;
use kodex_config::ConstrainedWithSource;
use kodex_config::FeatureRequirementsToml;
use kodex_config::ManagedAuthPolicy;
use kodex_config::McpEnterpriseManagedAuthConfig;
use kodex_config::McpServerRequirement;
use kodex_config::PluginRequirementsToml;
use kodex_config::ProfileV2Name;
use kodex_config::ResidencyRequirement;
use kodex_config::SandboxModeRequirement;
use kodex_config::Sourced;
use kodex_config::ThreadConfigLoader;
use kodex_config::config_toml::ConfigToml;
use kodex_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use kodex_config::config_toml::ProjectConfig;
use kodex_config::config_toml::RealtimeAudioConfig;
use kodex_config::config_toml::RealtimeConfig;
use kodex_config::config_toml::ThreadStoreToml;
use kodex_config::config_toml::validate_model_providers;
...
use kodex_config::sandbox_mode_requirement_for_permission_profile;
...
use kodex_config::types::History;
use kodex_config::types::McpServerConfig;
use kodex_config::types::McpServerDisabledReason;
...
use kodex_config::types::ToolSuggestConfig;
use kodex_config::types::ToolSuggestDisabledTool;
use kodex_config::types::ToolSuggestDiscoverable;
use kodex_config::types::TuiKeymap;
use kodex_config::types::TuiNotificationSettings;
use kodex_config::types::TuiPetAnchor;
use kodex_config::types::UriBasedFileOpener;
use kodex_config::types::WindowsSandboxModeToml;
use kodex_core_plugins::PluginLoadOutcome;
use kodex_core_plugins::PluginsConfigInput;
use kodex_exec_server::ExecutorFileSystem;
use kodex_exec_server::LOCAL_FS;
use kodex_exec_server::ReadFileOptions;
use kodex_features::CodeModeConfigToml;
use kodex_features::CurrentTimeReminderConfigToml;
use kodex_features::CurrentTimeReminderDeliveryMode;
use kodex_features::CurrentTimeSource;
use kodex_features::Feature;
use kodex_features::FeatureConfigSource;
use kodex_features::FeatureOverrides;
use kodex_features::FeatureToml;
use kodex_features::Features;
use kodex_features::FeaturesToml;
use kodex_features::MultiAgentV2ConfigToml;
use kodex_features::NetworkProxyConfigToml;
use kodex_features::SleepToolMode;
use kodex_features::TokenBudgetConfigToml;
...
use kodex_login::AuthManagerConfig;
use kodex_login::AuthRouteConfig;
use kodex_mcp::DEFAULT_OPTIONAL_MCP_STARTUP_GRACE;
use kodex_mcp::McpConfig;
use kodex_mcp::McpPluginAttribution;
use kodex_mcp::McpProtocolMode;
use kodex_mcp::McpServerRegistration;
use kodex_mcp::ResolvedMcpCatalog;
use kodex_model_provider::ProviderCapabilities;
use kodex_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use kodex_model_provider_info::ModelProviderInfo;
use kodex_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use kodex_model_provider_info::built_in_model_providers;
use kodex_model_provider_info::merge_configured_model_providers;
use kodex_models_manager::ModelsManagerConfig;
use kodex_prompts::ResolvedModelMessages;
use kodex_protocol::config_types::AltScreenMode;
use kodex_protocol::config_types::AutoCompactTokenLimitScope;
...
use kodex_protocol::config_types::SandboxMode;
...
use kodex_protocol::config_types::WebSearchConfig;
...
use kodex_protocol::models::BaseInstructionsProvenance;
use kodex_protocol::models::PermissionProfile;
pub use kodex_protocol::models::PermissionProfileSnapshot;
use kodex_protocol::models::ProfileWorkspaceRoot;
...
use kodex_protocol::permissions::DenyReadValidator;
use kodex_protocol::permissions::DenyReadViolation;
...
pub use kodex_thread_store::ExtraConfig;
...
use crate::config::permissions::get_readable_roots_required_for_kodex_runtime;
...
pub use kodex_agent_roles::AgentRoleConfig;
pub use kodex_config::ConfigLoadOptions;
pub use kodex_config::Constrained;
pub use kodex_config::ConstraintError;
pub use kodex_config::ConstraintResult;
pub use kodex_config::LoaderOverrides;
pub use kodex_network_proxy::NetworkProxyAuditMetadata;
use kodex_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use kodex_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(kodex_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(kodex_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Kodex-only writable roots are intentionally excluded.
...
// when Kodex is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<kodex_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    pub application_network_policy: kodex_http_client::NetworkPolicy,
    ...
    /// Optional external notifier command. When set, Kodex will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Kodex
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.kodex/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Kodex"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Kodex '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    pub tui_effects: kodex_config::types::TuiEffects,

    /// Rich content rendering preferences, independent of animations.
    pub tui_rendering: kodex_config::types::TuiRendering,
    ...
    /// file (default): Use a file in the Kodex home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Kodex can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Kodex unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/kodex/blob/main/kodex-rs/rmcp-client/src/oauth.rs#L2
    /// file: KODEX_HOME/.credentials.json
    ///       This file will be readable to Kodex and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Kodex will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Kodex state (defaults to `~/.kodex` but can be
    /// overridden by the `KODEX_HOME` environment variable).
    pub kodex_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Kodex SQLite databases.
    pub sqlite: kodex_state::SqliteConfig,

    /// Directory where Kodex writes log files (defaults to `$KODEX_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.kodex/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Kodex executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub kodex_self_exe: Option<PathBuf>,

    /// Path to the `kodex-linux-sandbox` executable. This must be set if
    /// [`kodex_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `kodex-linux-sandbox`.
    pub kodex_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `kodex-execve-wrapper` executable used for shell
    ...
    /// Whether Kodex-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Kodex updates on startup and surfaces update prompts.
    /// Set to `false` only if your Kodex updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Kodex product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Kodex product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: kodex_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn kodex_home(&self) -> PathBuf {
        self.kodex_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn kodex_home(mut self, kodex_home: PathBuf) -> Self {
        self.kodex_home = Some(kodex_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let kodex_home = match kodex_home {
            Some(kodex_home) => AbsolutePathBuf::from_absolute_path(kodex_home)?,
            None => find_kodex_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&kodex_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = kodex_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    kodex_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(kodex_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &kodex_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &kodex_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            kodex_home: self.kodex_home.to_path_buf(),
            ...
            kodex_linux_sandbox_exe: self.kodex_linux_sandbox_exe.clone(),
            ...
            host_owned_apps_protocol_mode: if self.features.enabled(Feature::KodexAppsMcp20260728) {
            ...
            connector_snapshot:
                kodex_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub fn workspace_routing_context(&self) -> kodex_model_provider::WorkspaceRoutingContext {
        kodex_model_provider::WorkspaceRoutingContext::new(self.chatgpt_base_url.clone())
            .with_session(kodex_login::WorkspaceRoutingSession {
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_kodex_home(
            kodex_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Kodex home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_kodex_home(
        kodex_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = kodex_config::build_cli_overrides_layer(&cli_overrides);
        kodex_config::merge_toml_values(&mut merged, &cli_layer);
        let kodex_home = AbsolutePathBuf::from_absolute_path_checked(kodex_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &kodex_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `kodex exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `kodex_self_exe`, `kodex_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(kodex_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(kodex_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, kodex_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .kodex/ folders.
...
/// Patch `KODEX_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(kodex_home: &Path, provider: &str) -> std::io::Result<()> {
    kodex_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn resolve_feature_enabled(feature: Option<&kodex_config::config_toml::FeatureToggleToml>) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = kodex_home.join(memories_config.version.directory_name());
        ...
        let prefer_mxc = features.enabled(Feature::PreferMxc)
            ...
            && kodex_sandboxing::windows_mxc_available();
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(kodex_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| kodex_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| kodex_home.clone());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_kodex_runtime(
            &kodex_home,
            zsh_path.as_ref(),
        ...
        let managed_deny_read_policy = filesystem_requirements
            ...
            .map(|Sourced { value, .. }| -> std::io::Result<_> {
                ...
                value.apply_to_policy(&mut policy, kodex_utils_path_uri::PathConvention::native())?;
                Ok(Arc::new(policy))
            })
            .transpose()?;
        ...
        let config = Self {
            ...
            kodex_home,
            sqlite: kodex_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        kodex_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
pub fn find_kodex_home() -> std::io::Result<AbsolutePathBuf> {
    kodex_utils_home_dir::find_kodex_home()
}

/// Returns the path to the folder where Kodex logs are stored. Does not verify
