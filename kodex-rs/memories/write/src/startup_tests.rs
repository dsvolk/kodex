...
use kodex_git_utils::diff_since_latest_init;
use kodex_git_utils::reset_git_repository;
...
async fn dual_write_prepares_both_roots_without_importing_old_notes() -> anyhow::Result<()> {
    ...
    let test = build_test_kodex_with_memories_config(&server, Arc::clone(&home), memories).await?;
    trigger_memories_startup(&test).await;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_kodex(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Kodex before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    core_test_support::submit_thread_settings(
        ...
        kodex_protocol::protocol::ThreadSettingsOverrides {
            ...
            permission_profile: Some(kodex_protocol::models::PermissionProfile::workspace_write()),
            ..Default::default()
        },
    )
    .await?;
...
async fn run_memory_phase_one_model_request_test(
    ...
    .await?;
    if version == kodex_protocol::MemoryVersion::V2 {
...
async fn run_memory_phase_two_model_request_test(
    ...
    let version = memories.version;
    let test = build_test_kodex_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    if version == kodex_protocol::MemoryVersion::V2 {
        ...
        let phase1_claim = store
            ...
            .await?;
        let kodex_state::Stage1JobClaimOutcome::Claimed { ownership_token } = phase1_claim else {
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = config.kodex_home.join(version.directory_name());
    tokio::fs::create_dir_all(&root).await?;
    seed_extension_instructions(&root).await?;
    match version {
        kodex_protocol::MemoryVersion::V1 => seed_required_memory_artifacts(&root).await?,
        kodex_protocol::MemoryVersion::V2 => {
...
async fn build_test_kodex(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestKodex> {
    build_test_kodex_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_kodex_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestKodex> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<kodex_state::StateRuntime>> {
    let db = kodex_state::StateRuntime::init(
        kodex_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestKodex) {
    let config_snapshot = test.kodex.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<kodex_core::config::Config>) {
    let config_snapshot = test.kodex.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> kodex_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(kodex_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &kodex_state::StateRuntime,
    kodex_home: &Path,
    ...
    let mut metadata_builder = kodex_state::ThreadMetadataBuilder::new(
        thread_id,
        kodex_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = kodex_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = kodex_home.join(format!("rollout-{thread_id}.jsonl"));
    let line = RolloutLine {
...
) -> anyhow::Result<kodex_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.kodex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        kodex_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_kodex(test: &TestKodex) -> anyhow::Result<()> {
    test.kodex.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.kodex, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
...
-> anyhow::Result<()> {
    ...
    let home = Arc::new(TempDir::new()?);
    let mut memories = startup_test_memories_config();
    memories.version = kodex_protocol::MemoryVersion::V2;
    let request = run_memory_phase_one_model_request_test(&server, home, memories).await?;
