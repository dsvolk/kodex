...
use kodex_app_server_protocol::NetworkAccess;
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.kodex_home =
        kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local KODEX_HOME"
    );
...
async fn replayed_mcp_tool_call_stays_active_until_completion() {
    ...
    if let AppServerThreadItem::McpToolCall { status, .. } = &mut item {
        *status = kodex_app_server_protocol::McpToolCallStatus::Completed;
    }
        ...
        chat.handle_server_notification(
            ServerNotification::ItemCompleted(ItemCompletedNotification {
                ...
                item: AppServerThreadItem::McpToolCall {
                    ...
                    tool: "js".to_string(),
                    status: kodex_app_server_protocol::McpToolCallStatus::Failed,
                    ...
                    plugin_id: None,
                    read_only_hint: None,
                    result: Some(Box::new(kodex_app_server_protocol::McpToolCallResult {
                        content: vec![
...
async fn replayed_in_progress_turn_marks_task_running() {
    for replay_kind in [
        ...
        for items_view in [
            kodex_app_server_protocol::TurnItemsView::Full,
            kodex_app_server_protocol::TurnItemsView::NotLoaded,
        ] {
