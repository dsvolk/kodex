...
impl TestDaemon {
    fn new() -> Result<Self> {
        ...
        let kodex_source = std::fs::canonicalize(&kodex)?;
        ...
        std::fs::hard_link(&kodex_source, &managed)
            .or_else(|_| std::fs::copy(&kodex_source, managed).map(|_| ()))?;
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(&self.kodex);
        command.env("KODEX_HOME", self.home.path());
        command
    }
...
fn manual_update_rejects_an_unowned_installation() -> Result<()> {
    ...
    std::fs::remove_file(
        daemon
            .home
            .path()
            .join("packages/standalone/current/bin/kodex"),
    )?;
...
fn packaged_daemon_launch(action: &str, initial: InitialDaemon) -> Result<()> {
    ...
    for directory in ["bin", "kodex-path", "kodex-resources"] {
        std::fs::create_dir_all(package.join(directory))?;
    }
    std::fs::copy(&daemon.kodex, package.join("bin/kodex"))?;
    daemon.kodex = package.join("bin/kodex");
    for helper in [
        "bin/kodex-code-mode-host",
        "kodex-path/rg",
        "kodex-resources/bwrap",
    ] {
    ...
    std::fs::write(
        package.join("kodex-package.json"),
        serde_json::to_vec(&serde_json::json!({
            "version": env!("CARGO_PKG_VERSION"), "target": target, "entrypoint": "bin/kodex"
        }))?,
    )?;
    ...
    let cli_before = daemon.kodex.canonicalize()?;
    ...
    assert_eq!(
        output["managedKodexPath"],
        initial_root
            .canonicalize()?
            .join("current/bin/kodex")
            .to_str()
            .context("managed path is not UTF-8")?
    );
    assert_eq!(daemon.kodex.canonicalize()?, cli_before);
    ...
    if action == "start" {
        ...
        std::fs::remove_file(package.join("bin/kodex-code-mode-host"))?;
        let invalid = daemon
            .command()
        ...
        std::fs::write(
            package.join("bin/kodex-code-mode-host"),
        ...
        assert_eq!(
            std::fs::read(current.join("bin/kodex-code-mode-host"))?,
            b"replacement helper"
        );
        if initial == InitialDaemon::Missing {
            assert_eq!(
                std::fs::read(original.join("bin/kodex-code-mode-host"))?,
                b"runtime fixture"
            );
        }
        assert_eq!(daemon.lifecycle("start")?["status"], "alreadyRunning");
