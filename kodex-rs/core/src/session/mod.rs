...
use kodex_analytics::ImagePreparationFact;
use kodex_analytics::ImagePreparationMetadata;
use kodex_analytics::SubAgentThreadStartedInput;
use kodex_analytics::TurnKodexErrorFact;
use kodex_async_utils::OrCancelExt;
use kodex_attachment_store::AttachmentStore;
use kodex_attachment_store::InlineAttachmentStore;
use kodex_connectors::connector_runtime_context_key;
use kodex_context_fragments::RenderedFragment;
use kodex_exec_server::Environment;
use kodex_exec_server::EnvironmentManager;
use kodex_execpolicy::prefix_rule_migration;
use kodex_extension_api::ConversationHistorySnapshot;
use kodex_extension_api::ExtensionDataInit;
use kodex_extension_api::PromptSlot;
use kodex_extension_api::TurnContextContributionInput;
use kodex_features::FEATURES;
use kodex_features::Feature;
use kodex_features::unstable_features_warning_event;
use kodex_history::RolloutItem;
use kodex_hooks::Hooks;
use kodex_hooks::HooksConfig;
use kodex_login::AuthManager;
use kodex_login::KodexAuth;
use kodex_login::auth_env_telemetry::collect_auth_env_telemetry;
use kodex_mcp::McpResourceClient;
...
use kodex_network_proxy::normalize_host;
use kodex_otel::current_span_trace_id;
use kodex_otel::current_span_w3c_trace_context;
use kodex_otel::set_parent_from_w3c_trace_context;
...
use kodex_protocol::approvals::ElicitationRequestEvent;
use kodex_protocol::approvals::ExecPolicyAmendment;
use kodex_protocol::approvals::NetworkPolicyAmendment;
use kodex_protocol::approvals::NetworkPolicyRuleAction;
...
use kodex_protocol::dynamic_tools::DynamicToolResponse;
use kodex_protocol::dynamic_tools::DynamicToolSpec;
use kodex_protocol::items::EnteredReviewModeItem;
...
use kodex_protocol::protocol::HasLegacyEvent;
...
use kodex_protocol::protocol::RawResponseItemEvent;
...
use kodex_protocol::protocol::TurnContextNetworkItem;
...
use kodex_protocol::protocol::WorldStateItem;
...
use kodex_protocol::request_permissions::RequestPermissionsArgs;
use kodex_protocol::request_permissions::RequestPermissionsEvent;
use kodex_protocol::request_permissions::RequestPermissionsResponse;
use kodex_protocol::request_user_input::RequestUserInputArgs;
use kodex_protocol::request_user_input::RequestUserInputResponse;
use kodex_rmcp_client::ElicitationResponse;
use kodex_rollout::should_persist_response_item;
use kodex_rollout::state_db;
use kodex_rollout_trace::ThreadStartedTraceMetadata;
use kodex_rollout_trace::ThreadTraceContext;
use kodex_sandboxing::SandboxType;
use kodex_sandboxing::policy_transforms::intersect_permission_profiles_with_context;
use kodex_shell_command::parse_command::parse_command;
use kodex_terminal_detection::user_agent;
use kodex_thread_store::CreateThreadParams;
use kodex_thread_store::LiveThread;
use kodex_thread_store::LiveThreadInitGuard;
use kodex_thread_store::LocalThreadStore;
use kodex_thread_store::PersistContext;
use kodex_thread_store::ReadThreadParams;
use kodex_thread_store::ResumeThreadParams;
use kodex_thread_store::ThreadPersistenceMetadata;
use kodex_thread_store::ThreadStore;
use kodex_utils_audio::prepare_response_items as prepare_audio_response_items;
use kodex_utils_git_discovery::GitRootDiscovery;
use kodex_utils_output_truncation::with_serialization_allowance;
...
use crate::kodex_thread::KodexThreadSettingsOverrides;
...
use kodex_protocol::models::LocalImagePreparation;
...
use kodex_protocol::protocol::DeprecationNoticeEvent;
...
use kodex_protocol::protocol::ModelRerouteEvent;
use kodex_protocol::protocol::ModelRerouteReason;
use kodex_protocol::protocol::ModelVerification;
use kodex_protocol::protocol::ModelVerificationEvent;
use kodex_protocol::protocol::NetworkApprovalContext;
use kodex_protocol::protocol::Op;
use kodex_protocol::protocol::RateLimitSnapshot;
use kodex_protocol::protocol::RawResponseCompletedEvent;
use kodex_protocol::protocol::RequestUserInputEvent;
...
use kodex_protocol::protocol::StreamErrorEvent;
use kodex_protocol::protocol::ThreadMemoryMode;
...
use kodex_skills_extension::HostSkillsService;
use kodex_tools::ToolName;
use kodex_tools::UnifiedExecShellMode;
use kodex_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use kodex_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn kodex_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<kodex_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        kodex_sandboxing::WindowsSandboxProxySettingsMode,
}
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/kodex/concepts/cyber-safety";
...
impl Session {
    ...
    ) -> BoxFuture<'static, KodexResult<(Arc<Self>, SessionIo)>> {
        Box::pin(async move {
            let parent_trace = match args.parent_trace {
                Some(trace) => {
                    if kodex_otel::context_from_w3c_trace_context(&trace).is_some() {
                        Some(trace)
                    } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> KodexResult<(Arc<Self>, SessionIo)> {
        ...
        let isolation = thread_extension_init
            .get::<kodex_extension_api::SessionIsolation>()
            .map(|policy| *policy)
            .unwrap_or_default();
        // Enforce snapshot-only instructions for both managed and inline isolated sessions.
        let instructions = if isolation == kodex_extension_api::SessionIsolation::Isolated {
            SessionInstructions {
        ...
        let exec_policy = if isolation == kodex_extension_api::SessionIsolation::Isolated {
            let managed_policy = config
                .config_layer_stack
                .requirements()
                .exec_policy
                .as_deref()
                .map_or_else(kodex_execpolicy::Policy::empty, |policy| {
                    policy.as_ref().clone()
                });
            ...
            {
                let kodex_home = config.kodex_home.clone();
                let policy_path = default_policy_path(kodex_home.as_path());
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| KodexErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let mut config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            kodex_models_manager::manager::RefreshStrategy::Offline
        } else {
            kodex_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        let disabled_plugin_ids = disabled_plugin_ids.unwrap_or_else(|| {
            ...
            settings_owner
                .and_then(|thread_id| {
                    kodex_history::latest_disabled_plugin_ids(
        ...
        .map_err(|e| {
            ...
            match e.downcast::<KodexErr>() {
                Ok(error) => error,
                Err(error) => map_session_init_error(&error, &config.kodex_home),
            }
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> KodexResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> KodexResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| KodexErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub(crate) async fn submit_turn_input(
        ...
        .await?;
        reply_rx.await.unwrap_or(Err(KodexErr::InternalAgentDied))
    }
    ...
    pub(crate) async fn shutdown_and_wait(&self) -> KodexResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(err) if matches!(err.details(), KodexErrorDetails::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> KodexResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> KodexResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-kodex-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &kodex_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        windows_sandbox_type: SandboxType,
        network_policy_decider: Option<Arc<dyn kodex_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn kodex_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn kodex_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.kodex_home().clone()
    }
    ...
    pub(crate) async fn get_prompt_base_instructions(&self) -> BaseInstructions {
        ...
        {
            BaseInstructions {
                text: kodex_prompts::without_update_plan_instructions(&instructions.text),
                ..instructions
            }
        } else {
            instructions
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Kodex performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn restorable_thread_settings(&self) -> KodexThreadSettingsOverrides {
        let state = self.state.lock().await;
        state
            .session_configuration
    ...
    pub(crate) fn track_turn_kodex_error(&self, turn_context: &TurnContext, error: &KodexErr) {
        self.services
            .analytics_events_client
            .track_turn_kodex_error(TurnKodexErrorFact::from_kodex_err(
                self.thread_id.to_string(),
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .kodex_error_info
                .as_ref()
                .is_some_and(KodexErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_kodex_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => kodex_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&kodex_home, amendment)
            .await?;
    ...
    async fn record_prepared_conversation_items(
        ...
        let policy: kodex_utils_output_truncation::TruncationPolicy =
    ...
    async fn capture_step_context_inner(
        ...
        if cancellation_token.is_cancelled() {
            return Err(KodexErr::TurnAborted);
        }
        let (
            selected_capability_roots,
            executor_capability_discovery,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-kodex, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(kodex_hooks::SessionStartSource::Compact);
        }
    }
                ...
                .is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(kodex_protocol::AgentPath::root),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = kodex_error.to_string();
        let kodex_error_info = KodexErrorInfo::ResponseStreamDisconnected {
            http_status_code: kodex_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            kodex_error_info: Some(kodex_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<kodex_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_config(
    ...
    HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::KodexHooks),
