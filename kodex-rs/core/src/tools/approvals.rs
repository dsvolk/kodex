...
#[cfg(unix)]
use kodex_protocol::approvals::GuardianCommandSource;
    ...
    pub(crate) fn permission_request_payload(&self) -> PermissionRequestPayload {
        match self {
            ...
            Self::Execve { command, .. } => PermissionRequestPayload::bash(
                kodex_shell_command::parse_command::shlex_join(command),
                /*description*/ None,
            ),
    ...
    pub(crate) fn into_guardian_request(
        ...
        Ok(match self {
            Self::ExecCommand {
                ...
                environment_id,
                command,
                guardian_cwd: kodex_utils_path_uri::LegacyAppPathString::from_path_uri(
                    &cwd,
...
impl ApprovalResolution {
    fn into_tool_result(self, model_info: &ModelInfo) -> Result<ReviewDecision, ToolError> {
        ...
        match self.decision {
            ...
            ReviewDecision::Abort => Err(ToolError::Kodex(KodexErr::TurnAborted)),
        ...
        match action {
            ApprovalAction::ExecCommand {
                ...
                let policy_fingerprint = ctx
                    ...
                    .map(kodex_execpolicy::RequirementsExecPolicy::fingerprint);
