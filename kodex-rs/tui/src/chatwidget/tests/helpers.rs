use super::*;
use kodex_app_server_protocol::ImageGenerationItem;
...
pub(super) async fn test_config() -> Config {
    ...
    let kodex_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_kodex_home(kodex_home.clone(), Vec::new())
            .await
            .expect("config");
    // Keep generic UI snapshots stable when the bundled catalog default changes.
    config.model = Some("gpt-5.6-sol".to_string());
    config.kodex_home = kodex_home.abs();
    config.sqlite = kodex_state::SqliteConfig::new_for_testing(kodex_home.as_path().abs());
    config.log_dir = kodex_home.join("log");
...
pub(super) fn test_model_catalog(_config: &Config) -> Arc<ModelCatalog> {
    Arc::new(
        ModelCatalog::new(crate::test_support::TEST_MODEL_PRESETS.clone())
            ...
            kodex_models_manager::collaboration_mode_presets::builtin_collaboration_mode_presets(),
        ),
    )
}
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::KodexOpTarget::Direct(op_tx));
...
fn token_usage_breakdown(usage: TokenUsage) -> kodex_app_server_protocol::TokenUsageBreakdown {
    kodex_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    kodex_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: kodex_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    kodex_error_info: Option<KodexErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = kodex_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            kodex_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(kodex_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    let mut entries = vec![kodex_app_server_protocol::HookOutputEntry {
        kind: kodex_app_server_protocol::HookOutputEntryKind::Warning,
        text: "Heads up from the hook".to_string(),
    }];
    if event_name != kodex_app_server_protocol::HookEventName::Interrupt {
        entries.push(kodex_app_server_protocol::HookOutputEntry {
