...
use kodex_windows_sandbox::create_directory_guard;
use kodex_windows_sandbox::string_from_sid_bytes;
...
fn authenticate_impersonated_client(
    ...
    let _setup_lock = kodex_windows_sandbox::acquire_sandbox_setup_lock(/*timeout_ms*/ 5_000)?;
    anyhow::ensure!(
        kodex_windows_sandbox::resolve_sid(kodex_windows_sandbox::SANDBOX_USERS_GROUP)
            .is_ok_and(|current| current == sandbox_sid),
        kodex_windows_sandbox::SANDBOX_GROUP_CHANGED
    );
    ...
    crate::package_identity::authorize_client(authorized_process, token.0)
        .context("authorize the packaged Kodex provisioning client")?;
    ...
    let user = unsafe { kodex_windows_sandbox::get_user_sid_bytes(token.0) }
        .context("read provisioning client identity")?;
    let user_sid = string_from_sid_bytes(&user).map_err(anyhow::Error::msg)?;
    let account = unsafe { kodex_windows_sandbox::account_name_from_sid(user.as_ptr() as _) }
    ...
    let requested_home = match request {
        ServiceRequest::RegisterInstallation { kodex_home } => kodex_home,
        ServiceRequest::ProvisionSandbox(request) => &request.kodex_home,
    };
    let (kodex_home, handles) = match request {
        ServiceRequest::ProvisionSandbox(request) => prepare_kodex_home(
            requested_home,
            runtime,
        ...
        ServiceRequest::RegisterInstallation { .. } => {
            ...
            kodex_windows_sandbox::validate_local_directory_path(requested_home)?;
    ...
    let desktop_installation =
        crate::installation_record::read_desktop_installation(&kodex_home, token.0)
            .inspect_err(|_| {
