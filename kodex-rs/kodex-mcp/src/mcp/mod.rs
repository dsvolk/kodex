...
use kodex_model_provider::CHATGPT_KODEX_BASE_URL;
use kodex_protocol::mcp::ClientMcpExtensions;
use kodex_protocol::mcp::McpServerInfo;
use kodex_protocol::mcp::Resource;
use kodex_protocol::mcp::ResourceTemplate;
use kodex_protocol::mcp::Tool;
...
pub const KODEX_APPS_MCP_SERVER_NAME: &str = "kodex_apps";
const DEFAULT_KODEX_APPS_MCP_PRODUCT_SKU: &str = "kodex";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const KODEX_CONNECTORS_TOKEN_ENV_VAR: &str = "KODEX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `kodex_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// Kodex home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `kodex-linux-sandbox` for sandboxed MCP tool execution.
    ...
    /// Protocol mode for servers other than the host-owned Kodex Apps registration.
    pub protocol_mode: McpProtocolMode,
    /// Independent protocol mode for the trusted, HTTP Kodex Apps registration.
...
pub fn host_owned_kodex_apps_enabled(config: &McpConfig, auth: Option<&KodexAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(KodexAuth::uses_kodex_backend)
}
...
fn is_trusted_chatgpt_mcp_server(
    ...
    if url::Url::parse(CHATGPT_KODEX_BASE_URL)
        .ok()
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_kodex_apps_enabled(config, auth) {
        servers.remove(KODEX_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    kodex_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            tool_catalog_cache,
            kodex_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn kodex_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(KODEX_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(KODEX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(KODEX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_kodex_apps_base_url(base_url: &str) -> String {
...
fn kodex_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_kodex_apps_base_url(base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/kodex") {
        base_url
    } else {
        format!("{base_url}/api/kodex")
    };
    format!("{base_url}/ps/mcp")
}

pub fn kodex_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        kodex_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    kodex_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_KODEX_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: kodex_apps_mcp_bearer_token_env_var(),
