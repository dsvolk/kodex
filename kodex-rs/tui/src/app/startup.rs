...
impl App {
    ...
    pub async fn run(
        ...
        let has_kodex_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            kodex_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("kodex.status_line", /*inc*/ 1, &[]);
        }
...
See the Kodex keymap documentation for supported actions and examples."
                )
            })?;
        ...
        let _ =
            app.initialize_server_version_notice(KODEX_CLI_VERSION, app_server.server_version());
            ...
            loop {
                ...
                let control = select! {
                    ...
                    result = async { match reconnect.as_mut() { Some(future) => future.await, None => std::future::pending().await } }, if reconnect.is_some() && !has_pending_app_events => {
                        ...
                        match result {
                            Ok(connected) => {
                                app.finish_reconnect(tui, &mut app_server, &mut app_event_rx, connected, KODEX_CLI_VERSION).await?;
