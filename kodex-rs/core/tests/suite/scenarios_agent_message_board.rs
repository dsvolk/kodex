...
impl kodex_core::TimeProvider for BoardClock {
    fn current_time(&self, _thread_id: kodex_protocol::ThreadId) -> kodex_core::TimeFuture<'_> {
        Box::pin(async move {
            match self {
    ...
    fn sleep(
        &self,
        _thread_id: kodex_protocol::ThreadId,
...
fn configure(config: &mut kodex_core::config::Config) {
...
async fn board_post_and_reads_reach_model_context_without_self_notices() -> anyhow::Result<()> {
    ...
    let test = test_kodex()
        .with_config(|config| {
            configure(config);
            config.current_time_reminder = Some(kodex_core::config::CurrentTimeReminderConfig {
...
{
    let server = responses::start_mock_server().await;
    let mut builder = test_kodex().with_config(configure);
    ...
    assert!(matches!(
        root.kodex.agent_status().await,
        kodex_protocol::protocol::AgentStatus::Completed(_)
    ));
...
async fn board_unsubscribe_survives_post_and_resume_until_resubscribed() -> anyhow::Result<()> {
    ...
    resumed
        ...
        .await?;
    let child_id = wait_for_event_match(&resumed.kodex, |event| match event {
    ...
    for (phase_index, (phase, subscribed, release)) in phases.into_iter().enumerate() {
        resumed
            ...
            .await?;
        let turn_id = wait_for_event_match(&resumed.kodex, |event| match event {
