    ...
    let mut builder = test_kodex().with_config(move |config| {
        ...
        servers.insert(
            ...
            kodex_config::types::McpServerConfig {
                ...
                transport: kodex_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_tool_output_limit_preserves_output_that_fits(
    ...
    let server = start_mock_server().await;
    let builder = test_kodex().with_config(|config| config.tool_output_token_limit = Some(50));
    let (_fixture, output) =
...
async fn mcp_tool_output_limit_survives_resume(output_token_limit: Option<usize>) -> Result<()> {
    ...
    let server = start_mock_server().await;
    let builder = test_kodex().with_config(|config| config.tool_output_token_limit = Some(50_000));
    ...
    fixture.kodex.ensure_rollout_materialized().await;
    fixture.kodex.flush_rollout().await?;
