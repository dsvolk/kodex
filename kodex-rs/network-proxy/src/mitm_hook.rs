...
mod tests {
        ...
        MitmHookConfig {
            ...
            actions: MitmHookActionsConfig {
                ...
                inject_request_headers: vec![InjectedHeaderConfig {
                    ...
                    secret_env_var: Some("KODEX_GITHUB_TOKEN".to_string()),
                    secret_file: None,
    ...
    fn compile_resolves_env_backed_injected_headers() {
        ...
        let hooks = compile_mitm_hooks_with_resolvers(
            &config,
            |name| (name == "KODEX_GITHUB_TOKEN").then(|| "ghp-secret".to_string()),
        ...
        assert_eq!(
            ...
            SecretSource::EnvVar("KODEX_GITHUB_TOKEN".to_string())
        );
        assert_eq!(
    ...
    fn evaluate_matches_query_and_header_constraints() {
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/kodex/issues?state=open&per_page=10")
    ...
    fn evaluate_matches_wildcard_path_query_and_header_constraints() {
        let mut config = base_config();
        let mut hook = github_hook();
        hook.matcher.path_prefixes = vec!["pattern:/repos/*/kodex/issues*".to_string()];
        hook.matcher.query =
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/kodex/issues?state=open")
    ...
    fn evaluate_path_wildcard_does_not_cross_segment_boundaries() {
        ...
        let nested_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/private/kodex/issues")
            .body(Body::empty())
            .unwrap();
    ...
    fn evaluate_rejects_paths_that_upstream_may_normalize() {
        ...
        let paths = [
            "/openai/openai/../kodex",
            "/openai/openai/%2e%2e/kodex",
            "/openai/openai/%2E%2E/kodex",
            "/openai/openai/.%2e/kodex",
            "/openai/openai/%2e./kodex",
            "/openai/openai/%252e%252e/kodex",
            "/openai/openai/%2f..%2fkodex",
            "/openai/openai/%5c..%5ckodex",
    ...
    fn evaluate_treats_glob_metacharacters_as_literal_without_glob_prefix() {
        ...
        let exact_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/[draft]/kodex/issues?state=op*")
            .header("x-github-api-version", "2022-11-28[preview]")
            .body(Body::empty())
            .unwrap();
        let non_literal_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/draft/kodex/issues?state=open")
            .header("x-github-api-version", "2022-11-28-preview")
            .body(Body::empty())
            .unwrap();
    ...
    fn evaluate_allows_literal_values_with_reserved_prefixes() {
        ...
        let exact_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/kodex/issues?state=pattern%3A%2A")
            .header("x-github-api-version", "pattern:*")
            .body(Body::empty())
            .unwrap();
        let non_literal_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/kodex/issues?state=pattern%3Aopen")
    ...
    fn evaluate_returns_hooked_host_no_match_when_query_constraint_fails() {
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/kodex/issues?state=closed")
            .body(Body::empty())
            .unwrap();
