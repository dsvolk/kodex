...
use kodex_app_server_protocol::GetAccountRateLimitsParams;
use kodex_login::LoginOnboardingEntrypoint;
use kodex_login::login_with_bedrock_access_keys;
use kodex_model_provider::is_supported_amazon_bedrock_region;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "KODEX_APP_SERVER_LOGIN_ISSUER";
// The development success-page redirect remains debug-only.
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "KODEX_APP_SERVER_DEV_OPEN_APP_URL";
...
pub(crate) struct AccountRequestProcessor {
    ...
    gateway_client: Arc<std::sync::Mutex<Option<Arc<kodex_login::GatewayAuthManager>>>>,
...
impl AccountRequestProcessor {
    pub(crate) fn new(
        ...
        let resolver: Arc<dyn kodex_login::WorkspaceRoutingResolver> = processor.clone();
        processor
            .auth_manager
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(KodexAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(KodexAuth::account_plan_type),
        }
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::Kodex => LoginSuccessPageBrand::Kodex,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: KODEX_OPEN_APP_URL.parse().map_err(|err| {
                            internal_error(format!("invalid Kodex open app URL: {err}"))
                        })?,
                        app_brand,
                    }
                } else {
                ...
                self.login_chatgpt_v2(request_id, kodex_streamlined_login, login_success_page)
                    .await;
            }
    ...
    async fn login_chatgpt_common(
            ...
            && !open_app_url.trim().is_empty()
        {
            *url = open_app_url
                .parse()
                .map_err(|err| internal_error(format!("invalid Kodex open app URL: {err}")))?;
        }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(kodex_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(kodex_streamlined_login, login_success_page)
            .await?;
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some((auth, http_client_factory)) =
            ...
            return Err(invalid_request(
                "kodex account authentication required to read rate limits",
            ));
        };
        ...
        let usage_request = async {
            if params.supports_luna_reserve
                && auth.auth_mode() == kodex_protocol::auth::AuthMode::Chatgpt
        ...
        let response = response
            .map_err(|err| internal_error(format!("failed to fetch kodex rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch kodex rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<_, _> = response
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "kodex".to_string());
                (limit_id, snapshot)
            })
            .collect();
    ...
    async fn get_account_token_usage_response(
        ...
        let Some((auth, http_client_factory)) =
            ...
            return Err(invalid_request(
                "kodex account authentication required to read token usage",
            ));
        };
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(kodex_app_server_protocol::ThreadUsage {
                    ...
                    groups: usage
                        ...
                        .into_iter()
                        .map(
                            |group| kodex_app_server_protocol::ThreadUsageBreakdownGroup {
                                model: group.model,
    ...
    async fn get_workspace_messages_response(
        ...
        let Some((auth, http_client_factory)) =
            ...
            return Err(invalid_request(
                "kodex account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some((auth, http_client_factory)) =
            ...
            return Err(invalid_request(
                "kodex account authentication required to notify workspace owner",
            ));
        };
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/kodex/workspace-messages".to_string(),
                status,
