...
async fn stateless_app_and_plugin_facts_preserve_arrival_order() {
    ...
    assert_eq!(
        ...
        [
            ("kodex_app_mentioned", Some(TEST_PRODUCT_CLIENT_ID)),
            ("kodex_app_used", Some(TEST_PRODUCT_CLIENT_ID)),
            ("kodex_plugin_used", Some(TEST_PRODUCT_CLIENT_ID)),
        ]
    );
    assert_eq!(
    ...
    *turn_metadata.root_turn_id.lock().expect("root turn ID") = None;
    ...
    assert_eq!(
        payload
            ...
            .collect::<Vec<_>>(),
        vec![
            json!({"event_type": "kodex_command_execution_event", "turn_id": "turn-1", "item_id": "before-conflict", "root_turn_id": "root-ancestor"}),
            json!({"event_type": "kodex_command_execution_event", "turn_id": "turn-1", "item_id": "after-conflict", "root_turn_id": null}),
            json!({"event_type": "kodex_control_tool_call_event", "turn_id": "turn-1", "item_id": "queued-control", "root_turn_id": null}),
            json!({"event_type": "kodex_dynamic_tool_call_event", "turn_id": "turn-1", "item_id": "queued-code-mode", "root_turn_id": null}),
            json!({"event_type": "kodex_turn_event", "turn_id": "turn-1", "item_id": null, "root_turn_id": null}),
        ]
    );
}
...
async fn completed_background_tool_item_emits_after_turn_event() {
    ...
    assert_eq!(
        out.iter()
            ...
            .collect::<Vec<_>>(),
        vec![
            json!({"event_type": "kodex_turn_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "kodex_command_execution_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "kodex_control_tool_call_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "kodex_dynamic_tool_call_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
        ]
    );
}
...
async fn voice_handoff_attributes_plugin_events_after_realtime_closes() {
    ...
    for event_type in ["kodex_app_used", "skill_invocation", "kodex_turn_event"] {
        let event = events
            .iter()
