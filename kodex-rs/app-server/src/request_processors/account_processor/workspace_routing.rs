...
use super::*;
use kodex_app_server_protocol::AccountRoutingOverride;
use kodex_backend_client::AccountEntry;
use kodex_login::WorkspaceRouting;
use kodex_login::WorkspaceRoutingRequest;
use kodex_login::WorkspaceRoutingResolver;
use kodex_model_provider::ProviderAccount;
use kodex_model_provider::ProviderAccountError;
use kodex_model_provider::ProviderAccountState;
        ...
        Ok(Some(
            GetAccountResponse {
                ...
                workspace_routing: read.workspace_routing.map(|routing| {
                    kodex_app_server_protocol::WorkspaceRouting {
        ...
        let read = Box::pin(async {
            ...
            let config = match load_config().await {
                ...
                Err(_)
                    if !self
                        ...
                        .auth_cached()
                        .as_ref()
                        .is_some_and(KodexAuth::is_chatgpt_auth) =>
                {
            ...
            if let Some(request) = request.filter(|_| workspace_routing.is_some()) {
                ...
                let workspace_bound = request.previously_routed
                    || request.provider_base_url == kodex_model_provider::CHATGPT_KODEX_BASE_URL
