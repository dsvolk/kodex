...
const COMMAND_RUNNER_EXE: &str = "kodex-command-runner.exe";
pub(crate) const BIN_DIRNAME: &str = "bin";
pub(crate) const RESOURCES_DIRNAME: &str = "kodex-resources";

pub(crate) fn helper_bin_dir(kodex_home: &Path) -> PathBuf {
    sandbox_bin_dir(kodex_home)
}
...
pub(crate) fn resolve_command_runner(kodex_home: &Path, log_dir: Option<&Path>) -> Result<PathBuf> {
    ...
    Ok(match copy_runner_if_needed(kodex_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_exe_for_launch(source: &Path, kodex_home: &Path) -> PathBuf {
    let runtime = crate::setup::current_setup_runtime();
    resolve_exe_for_runtime(source, kodex_home, runtime)
}

fn resolve_exe_for_runtime(source: &Path, kodex_home: &Path, runtime: SetupRuntime) -> PathBuf {
    let sandbox_log_dir = crate::sandbox_dir(kodex_home);
    ...
    let destination = helper_bin_dir(kodex_home).join(file_name);
...
fn copy_runner_if_needed(kodex_home: &Path, log_dir: Option<&Path>) -> Result<PathBuf> {
    let source = sibling_source_path()?;
    let suffix = helper_version_suffix(&source)?;
    let destination = helper_bin_dir(kodex_home).join(materialized_file_name(&suffix));
    log_note(
        &format!(
...
fn materialized_file_name(suffix: &str) -> String {
    format!("kodex-command-runner-{suffix}.exe")
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let kodex_home = Path::new(r"C:\Users\example\.kodex");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.kodex\.sandbox-bin"),
            helper_bin_dir(kodex_home)
        );
    }
    ...
    fn registered_request_does_not_materialize_or_replace_a_missing_source() {
        ...
        let executable = tmp.path().join("kodex.exe");
    ...
    fn legacy_request_materializes_the_same_source() {
        ...
        let destination = helper_bin_dir(&home).join("kodex.exe");
        assert_eq!(
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("kodex-command-runner.exe");
        ...
        let runner_destination =
            helper_bin_dir(&kodex_home).join(materialized_file_name(&runner_suffix));
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("kodex.exe");
        let helper = resources_dir.join("kodex-command-runner.exe");
        fs::write(&exe, b"kodex").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "kodex-command-runner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_resolves_bin_junctions() {
        ...
        fs::write(bin_dir.join("kodex.exe"), b"kodex").expect("write exe");
        let helper = resources_dir.join("kodex-windows-sandbox-setup.exe");
        ...
        assert_eq!(
            bundled_executable_path_for_exe(
                &junction.join("kodex.exe"),
                /*file_name*/ "kodex-windows-sandbox-setup.exe"
            ),
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("kodex.exe");
        let package_helper = package_resources_dir.join("kodex-command-runner.exe");
        let bin_helper = bin_resources_dir.join("kodex-command-runner.exe");
        fs::write(&exe, b"kodex").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("kodex.exe");
        let sibling_helper = release_dir.join("kodex-command-runner.exe");
        let resource_helper = resources_dir.join("kodex-command-runner.exe");
        fs::write(&exe, b"kodex").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "kodex-command-runner-test-suffix.exe");
    }
}
