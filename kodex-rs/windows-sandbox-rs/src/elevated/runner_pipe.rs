...
pub fn find_runner_exe(kodex_home: &Path, log_dir: Option<&Path>) -> anyhow::Result<PathBuf> {
    resolve_command_runner(kodex_home, log_dir)
}
...
pub fn pipe_pair() -> (String, String) {
    ...
    let base = format!(r"\\.\pipe\kodex-runner-{nonce:x}");
