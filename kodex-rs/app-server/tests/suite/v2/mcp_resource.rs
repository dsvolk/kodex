...
use futures::poll;
use kodex_app_server::in_process;
use kodex_app_server::in_process::InProcessServerEvent;
...
use kodex_app_server_protocol::ProjectCreateParams;
use kodex_app_server_protocol::ProjectCreateResponse;
use kodex_app_server_protocol::ProjectDeleteParams;
...
use kodex_config::ThreadConfigContext;
use kodex_config::ThreadConfigLoader;
use kodex_config::ThreadConfigLoaderFuture;
...
use kodex_state::PINNED_THREAD_SECTION_ID;
use kodex_state::PINNED_THREAD_SECTION_NAME;
...
const TEST_RESOURCE_URI: &str = "test://kodex/resource";
pub(super) const TEST_WIDGET_RESOURCE_URI: &str = "ui://widget/checkout-session.html";
const TEST_BLOB_RESOURCE_URI: &str = "test://kodex/resource.bin";
const TEST_RESOURCE_BLOB: &str = "YmluYXJ5LXJlc291cmNl";
const TEST_RESOURCE_TEXT: &str = "Resource body from the MCP server.";
const TEST_ERROR_RESOURCE_URI: &str = "test://kodex/error";
const TEST_ELICITATION_RESOURCE_URI: &str = "test://kodex/elicitation";
...
async fn mcp_resource_read_returns_resource_contents() -> Result<()> {
    ...
    let (_kodex_home, mut mcp) = start_resource_test_app_server(
        &apps_server_url,
...
async fn mcp_resource_read_preserves_protocol_errors(protocol: ProtocolVersion) -> Result<()> {
    ...
    let config = MockResponsesConfig::new(&responses_server.uri()).with_extra_config(&format!(
        "[mcp_servers.resource_server]\nurl = \"{apps_server_url}/api/kodex/ps/mcp\""
    ));
        ...
        (
            ...
            json!({
                ...
                "data": {
                    ...
                    "_meta": {"_kodex_apps": {"connector_auth_failure": {
...
async fn orchestrator_skill_can_read_referenced_resource_without_an_executor() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "warnings": ["Orchestrator skill discovery stopped after 2 resource pages: failed to list orchestrator skill resources: resources/list failed for `kodex_apps`: Mcp error: -32603: simulated later-page failure"],
            "next_cursor": null,
        })
    );
...
async fn disabled_orchestrator_skills_do_not_expose_skills_namespace() -> Result<()> {
    ...
    let (apps_server_url, apps_server_calls, apps_server_handle) =
        start_resource_apps_mcp_server().await?;
    let responses_server_uri = responses_server.uri();
    let (_kodex_home, mut mcp) = start_resource_test_app_server_with_extra_config(
        &apps_server_url,
        &responses_server_uri,
        r#"
[orchestrator.skills]
...
async fn mcp_resource_read_returns_error_for_unknown_thread() -> Result<()> {
    ...
    let client = start_resource_in_process_client(
        kodex_home.path(),
        Arc::new(kodex_config::NoopThreadConfigLoader),
    )
    .await?;
...
async fn metadata_and_mcp_requests_complete_while_unrelated_resume_loads_config() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses_server.uri())
        .with_extra_config(&format!(
            "[mcp_servers.resource_server]\nurl = \"{mcp_server_url}/api/kodex/ps/mcp\""
        ))
    ...
    let client =
        start_resource_in_process_client(kodex_home.path(), blocked_resume.clone()).await?;
        ...
        timeout(
            ...
            sender.request(ClientRequest::ThreadResume {
                ...
                params: ThreadResumeParams {
                    thread_id,
                    cwd: Some(kodex_home.path().to_string_lossy().into_owned()),
                    ..Default::default()
                },
            }),
        )
...
async fn resume_revalidates_persisted_thread_after_config_load(
    ...
    let kodex_home = TempDir::new()?;
    MockResponsesConfig::new(&responses_server.uri()).write(kodex_home.path())?;
    ...
    let thread_id = create_fake_rollout(
        ...
        matches!(mutation, ResumeMutation::GitMetadata).then(|| {
            kodex_protocol::protocol::GitInfo {
                commit_hash: None,
    ...
    let path = rollout_path(kodex_home.path(), filename_timestamp, &thread_id);
    let resume_cwd = TempDir::new()?;
    let blocked_resume = Arc::new(BlockedResumeConfig {
        cwd: AbsolutePathBuf::from_absolute_path(resume_cwd.path())?,
        matching_loads: AtomicUsize::new(0),
...
async fn start_resource_in_process_client(
    ...
    let config = ConfigBuilder::default()
        ...
        .fallback_cwd(Some(kodex_home.to_path_buf()))
        .cli_overrides(cli_overrides.clone())
        .loader_overrides(loader_overrides.clone())
        .build()
        .await?;
    let state_db = kodex_rollout::state_db::try_init(&config).await?;
    ...
    Ok(in_process::start(InProcessStartArgs {
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "kodex-app-server-tests".to_string(),
                title: None,
...
async fn start_resource_test_app_server_with_extra_config(
    ...
    let builder = TestAppServer::builder().with_kodex_home(kodex_home.path());
...
-> Result<(String, Arc<ResourceAppsMcpCalls>, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/kodex/ps/mcp", mcp_service);
