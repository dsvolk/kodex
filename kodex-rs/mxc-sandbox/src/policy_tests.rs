...
fn wrapper_preserves_exact_argv_and_separate_command_cwd() -> Result<()> {
    ...
    assert_eq!(wrapped, vec![crate::KODEX_WINDOWS_MXC_ARG1]);
...
fn symbolic_root_preserves_equal_path_precedence_and_denies() -> Result<()> {
    ...
    assert_eq!(
        ...
        volumes[..2]
            .iter()
            .flat_map(|volume| [".agents", ".kodex", ".git"].map(|name| volume
                .join(name)
                .to_str()
                .unwrap()
                .to_owned()))
