//! Defines the protocol for a Kodex session between a client and an agent.
//!
...
use crate::error::Result as KodexResult;
...
use kodex_extension_items::image_generation::ImageGenerationFailure;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Kodex:";
...
pub struct ConversationStartParams {
    /// Whether Kodex response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Kodex when the session ends.
    ...
    /// Sends automatic Kodex responses as realtime conversation items instead of handoff appends.
    pub kodex_responses_as_items: bool,
    /// Optional prefix added to automatic Kodex response items when `kodex_responses_as_items` is set.
    pub kodex_response_item_prefix: Option<String>,
    /// Selects how automatic Kodex handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub kodex_response_handoff_mode: KodexResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub kodex_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Kodex's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Kodex when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Kodex when this realtime session ends.
...
pub enum Op {
    ...
    TurnInput {
        ...
        mode: TurnInputMode,
        reply: oneshot::Sender<KodexResult<TurnInputSubmission>>,
    },
    ...
    SuspendTurnAndShutdown {
        reply: oneshot::Sender<KodexResult<SuspendTurnOutcome>>,
    },
    ...
    /// Request to shut down kodex instance.
    Shutdown,
...
/// running the command proposed by Kodex.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.kodex`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum KodexResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// Kodex errors that we expose to clients.
#[derive(Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[schemars(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum KodexErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub kodex_error_info: Option<KodexErrorInfo>,
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.kodex_error_info
            .as_ref()
            .is_none_or(KodexErrorInfo::affects_turn_status)
    }
}
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub kodex_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Kodex),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Kodex
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(kodex_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                kodex_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_kodex() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("kodex".to_string()).restriction_product(),
            Some(Product::Kodex)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Kodex])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Kodex]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = kodex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            kodex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".kodex")).expect("create .kodex");
        let canonical_cwd = kodex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_kodex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".kodex"))
            .expect("canonical .kodex");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_kodex.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_kodex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".kodex"))
            .expect("canonical .kodex");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_kodex.to_path_buf(),
    ...
    fn active_turn_not_steerable_error_does_not_affect_turn_status() {
        let event = ErrorEvent {
            ...
            kodex_error_info: Some(KodexErrorInfo::ActiveTurnNotSteerable {
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            kodex_error_info: Some(KodexErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
    ...
    fn misalignment_explanation_and_steer_are_never_serialized_into_error_events() {
        let event = ErrorEvent {
            ...
            kodex_error_info: Some(KodexErrorInfo::MisalignmentPolicyViolation),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "kodex_error_info": "misalignment_policy_violation"
            })
        );
    ...
    fn serialize_event() -> Result<()> {
        ...
        let event = Event {
            ...
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                ...
                thread_source: None,
                thread_name: None,
                model: "kodex-mini-latest".to_string(),
