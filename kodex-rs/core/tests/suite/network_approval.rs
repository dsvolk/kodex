...
use kodex_core::config::NetworkProxySpec;
use kodex_core::shell::ShellType;
use kodex_core::shell::get_shell;
...
use kodex_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "kodex-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://kodex-network-test.invalid:80";
...
-> Result<()> {
    ...
    .await?;
    let EventMsg::RequestPermissions(request) = wait_for_event(&test.kodex, |event| {
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.kodex.next_event())
            .await
...
#[test_case("GET", "http://kodex-network-test.invalid/"; "plain_http")]
#[test_case("CONNECT", "kodex-network-test.invalid:443"; "connect")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg_attr(
    not(target_os = "linux"),
    ignore = "requires the trusted Linux proxy bridge"
...
async fn background_network_approval_uses_current_review_settings_and_original_execution(
    ...
    .await?;
    let EventMsg::TurnStarted(first_turn) = wait_for_event(&test.kodex, |event| {
    ...
    let active_turn_id = if same_turn {
        ...
        wait_for_turn_complete(&test).await;
        assert_eq!(test.kodex.list_background_terminals().await.len(), 1);
        test.kodex
            .start_or_steer_turn(
                    ...
                    collaboration_mode: Some(CollaborationMode {
                        ...
                        settings: Settings {
                            ...
                            reasoning_effort: Some(
                                kodex_protocol::openai_models::ReasoningEffort::Medium,
                            ),
            ...
            .await?;
        let EventMsg::TurnStarted(active_turn) = wait_for_event(&test.kodex, |event| {
            matches!(event, EventMsg::TurnStarted(_))
        })
        .await
        else {
    ...
    test.kodex
        ...
        .await?;
    let assessment = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::GuardianAssessment(assessment)
    ...
    let metadata: Value = serde_json::from_str(
        final_parent.body_json()["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
    ...
    assert_eq!(
        test.kodex.list_background_terminals().await.len(),
        1,
        "approved network access must not terminate the background process"
    );
    test.kodex.submit(Op::CleanBackgroundTerminals).await?;
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#kodex-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("kodex-network-test.invalid is not in the allowed_domains")
    );
    test.kodex
...
PY"#
    ...
    assert!(test.kodex.list_background_terminals().await.is_empty());
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="kodex-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): kodex-network-test.invalid",
        )
    }));
    mount_exec_network_turn(
        &server,
...
async fn denying_network_policy_amendment_persists_and_blocks_request() -> Result<()> {
    ...
    let policy = fs::read_to_string(test.home.path().join("rules/default.rules"))?;
    assert!(policy.contains(
        r#"network_rule(host="kodex-network-test.invalid", protocol="http", decision="deny""#
    ));
    let output = responses
        .requests()
        .iter()
...
async fn unattributed_network_request_uses_active_turn_environment_fallback(
    ...
    if approvals_reviewer == ApprovalsReviewer::AutoReview {
        ...
        let thread_id = kodex_protocol::ThreadId::from_string(thread_id)?;
        ...
        tokio::time::timeout(Duration::from_secs(5), test.kodex.shutdown_and_wait())
            .await
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.kodex.list_background_terminals().await.is_empty() {
                break;
            }
            tokio::time::sleep(Duration::from_millis(10)).await;
        }
...
async fn thread_turnover_closes_managed_proxy_tunnels() -> Result<()> {
    ...
    let mut network = NetworkProxyConfig {
        enabled: true,
        mode: kodex_network_proxy::NetworkMode::Full,
...
pub(super) fn guardian_parent_catalog() -> ModelsResponse {
    let template = kodex_models_manager::bundled_models_response()
        ...
        models: ["guardian-parent-a", "guardian-parent-b"]
            ...
            .map(|slug| {
                ...
                model.supported_reasoning_levels.retain(|level| {
                    level.effort != kodex_protocol::openai_models::ReasoningEffort::Low
                });
                model
                    .model_messages
                    .as_mut()
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestKodex> {
...
fn managed_network_unified_exec_builder(features: &[Feature]) -> Result<TestKodexBuilder> {
...
async fn wait_for_completion_without_network_prompt(test: &TestKodex) {
    let event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_turn_complete(test: &TestKodex) {
    wait_for_event(&test.kodex, |event| {
