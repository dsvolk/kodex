...
use kodex_utils_template::Template;
...
// Keep in sync with the Kodex CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub kodex_streamlined_login: bool,
    ...
    match path.as_str() {
                ...
                Ok((tokens, client)) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Kodex failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "kodex_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut extra_parameters = vec![
        ...
        ("kodex_cli_simplified_flow", "true"),
...
/// Returns true when the OAuth callback represents a missing Kodex entitlement.
fn is_missing_kodex_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_kodex_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_kodex_entitlement_error(error_code, error_description) {
        return "Kodex is not enabled for your workspace. Contact your workspace administrator to request access to Kodex.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_kodex_entitlement_error(code, error_description) {
            (
                "You do not have access to Kodex".to_string(),
                "This account is not currently authorized to use Kodex in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Kodex.".to_string(),
                "Contact your workspace administrator to get access to Kodex, then return to Kodex and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Kodex to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_kodex_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_kodex_entitlement");
        assert!(is_missing_kodex_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Kodex"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_kodex_entitlement"));
    }
}
