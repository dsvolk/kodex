...
use kodex_models_manager::manager::ModelsManagerFuture;
use kodex_models_manager::manager::RefreshStrategy;
use kodex_models_manager::manager::StaticModelsManager;
use kodex_models_manager::model_info::with_config_overrides;
...
async fn instruction_refresh_serializes_reads_and_releases_on_cancellation() {
    ...
    let source = turn.config.kodex_home.join("AGENTS.md");
...
async fn next_step_waits_for_environment_publication_before_capturing() {
    ...
    let next_cwd = kodex_utils_path_uri::PathUri::from_abs_path(&next_cwd);
...
async fn thread_instruction_refresh_serializes_reads_and_releases_on_cancellation(
    ...
    let global = global_text.map(|text| Instructions {
        text: text.to_string(),
        source: Some(turn.config.kodex_home.join("AGENTS.md")),
    });
    let instructions = |text: &str| {
        Some(Instructions {
            text: text.to_string(),
