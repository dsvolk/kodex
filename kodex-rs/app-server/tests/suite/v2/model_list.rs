...
use kodex_app_server_protocol::Model;
use kodex_app_server_protocol::ModelListParams;
use kodex_app_server_protocol::ModelListResponse;
use kodex_app_server_protocol::ModelServiceTier;
use kodex_app_server_protocol::ModelUpgradeInfo;
use kodex_app_server_protocol::ReasoningEffortOption;
...
async fn api_key_model_discovery_startup_enablement_respects_user_config(
    ...
    let mut remote_model = kodex_models_manager::bundled_models_response()?
        .models
        .remove(0);
    remote_model.slug = "rollout-model".into();
    remote_model.visibility = kodex_protocol::openai_models::ModelVisibility::List;
...
"#
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[("OPENAI_API_KEY", None), ("KODEX_API_KEY", None)])
        .build_initialized()
        .await?;
    let mut bundled = kodex_models_manager::bundled_models_response()?.models;
...
fn expected_visible_models() -> Vec<Model> {
    ...
    let mut presets = ModelPreset::filter_by_auth(
        kodex_core::test_support::all_model_presets().clone(),
        /*chatgpt_mode*/ false,
    );
