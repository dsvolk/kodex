...
async fn check_remote_voice_start(
    ...
    Box::pin(app.handle_event(
        ...
        AppEvent::PersistRealtimeVoiceSelection {
            voice: kodex_protocol::protocol::RealtimeVoice::Juniper,
        },
    ))
    .await?;
    if capabilities == super::session_lifecycle_requests::HistoryCapabilities::Current {
        assert_eq!(
            ...
            (
                Some(kodex_protocol::protocol::RealtimeVoice::Juniper),
                Some(kodex_protocol::protocol::RealtimeVoice::Juniper)
            )
        );
    }
    ...
    app.config.realtime.voice = Some(kodex_protocol::protocol::RealtimeVoice::Maple);
    app.chat_widget
        .on_realtime_voice_saved(kodex_protocol::protocol::RealtimeVoice::Maple);
    Box::pin(app.handle_event(
        &mut tui,
        &mut app_server,
        AppEvent::KodexOp(Op::RealtimeConversationStart {
            thread_id,
...
async fn switching_agent_threads_preserves_backend_voice_and_routes_pending_speech() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadRealtimeTranscriptDone(
            kodex_app_server_protocol::ThreadRealtimeTranscriptDoneNotification {
                thread_id: source.to_string(),
                role: "user".into(),
    ...
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::KodexOp(speech))).await?;
    ...
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::KodexOp(stop))).await?;
    assert_eq!(
        recorded_params(&requests, "thread/realtime/stop"),
...
async fn switching_threads_keeps_the_source_voice_partial_only_on_reattach() {
    ...
    app.chat_widget.handle_server_notification(
        ServerNotification::ThreadRealtimeTranscriptDelta(
            kodex_app_server_protocol::ThreadRealtimeTranscriptDeltaNotification {
                thread_id: source.to_string(),
                role: "user".into(),
...
async fn queued_voice_caption_after_switch_returns_once_to_its_source_thread() {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadRealtimeClosed(
            kodex_app_server_protocol::ThreadRealtimeClosedNotification {
                thread_id: source.to_string(),
...
async fn delegated_final_speech_reaches_app_server_once_and_stale_speech_is_rejected() -> Result<()>
{
    ...
    assert!(ops.try_recv().is_err());
    let mut tui = crate::tui::test_support::make_test_tui()?;
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::KodexOp(speech.clone())))
        .await?;
    assert!(!app.chat_widget.has_pending_realtime_speech(delivery_id));
    assert_eq!(
    ...
    Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::KodexOp(queued))).await?;
    assert_eq!(
        recorded_params(&requests, "thread/realtime/appendSpeech").len(),
        1
    );
...
async fn rejected_voice_setting_preserves_current_selection() -> Result<()> {
    ...
    std::fs::write(app.config.kodex_home.join("config.toml"), "realtime = [")?;
    let mut tui = crate::tui::test_support::make_test_tui()?;
    Box::pin(app.handle_event(
        &mut tui,
        &mut app_server,
...
async fn remote_voice_picker_reads_server_preference() -> Result<()> {
    ...
    app.chat_widget
        .set_realtime_voice(Some(kodex_protocol::protocol::RealtimeVoice::Maple));
...
async fn overridden_voice_save_keeps_effective_voice() -> Result<()> {
    for project_override in [false, true] {
        ...
        let client = crate::start_embedded_app_server(
            ...
            app.config.clone(),
            overrides,
            kodex_config::LoaderOverrides::without_managed_config_for_tests(),
            /*strict_config*/ false,
        ...
        let server = AppServerSession::new(
            kodex_app_server_client::AppServerClient::InProcess(client),
            crate::app_server_session::ThreadParamsMode::Embedded,
        );
        while events.try_recv().is_ok() {}
        app.persist_realtime_voice(&server, kodex_protocol::protocol::RealtimeVoice::Juniper)
            .await;
        assert_eq!(
            (
                app.config.realtime.voice,
            ...
            (
                Some(kodex_protocol::protocol::RealtimeVoice::Maple),
                Some(kodex_protocol::protocol::RealtimeVoice::Maple)
            ),
        );
