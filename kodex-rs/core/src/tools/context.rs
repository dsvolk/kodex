...
use kodex_protocol::models::function_call_output_content_items_to_text;
use kodex_tools::LoadableToolSpec;
...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the Kodex tool call id
    ...
    pub(crate) async fn originating_call(&self) -> Option<ToolCallOrigin> {
        if let ToolCallSource::CodeMode { cell_id, .. } = &self.source {
            return self
                ...
                .services
                .code_mode_service
                .cell_originating_call(&kodex_code_mode::CellId::new(cell_id.clone()));
        }
