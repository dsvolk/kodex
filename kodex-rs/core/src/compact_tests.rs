...
fn collect_annotated_user_messages_extracts_user_text_only() {
    ...
    assert_eq!(
        vec![CompactedUserMessage {
            ...
            internal_chat_message_metadata_passthrough: None,
            harness_metadata: Some(KodexHarnessMetadata::default()),
        }],
        collected
    );
}
...
fn collect_user_messages_filters_legacy_warnings() {
    let items = vec![
        ...
        user_message(
            "Warning: Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-kodex, apply for trusted access: https://chatgpt.com/cyber or learn more: https://developers.openai.com/kodex/concepts/cyber-safety",
        ),
...
fn build_token_limited_compacted_history_truncates_overlong_user_messages() {
    ...
    assert_eq!(history[0].metadata, Some(KodexHarnessMetadata::default()));
