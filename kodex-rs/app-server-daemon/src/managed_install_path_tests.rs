...
fn discovers_package_and_legacy_installs() {
    ...
    let legacy = current.join(super::managed_kodex_file_name());
    assert_eq!(
        super::managed_kodex_bin(home.path()),
        home.path()
            .join("packages/app-server-daemon/current/bin")
            .join(super::managed_kodex_file_name())
    );
    ...
    assert_eq!(super::managed_kodex_bin(home.path()), legacy);
    let packaged = current.join("bin").join(super::managed_kodex_file_name());
    std::fs::create_dir(current.join("bin")).expect("bin directory");
    std::fs::write(&packaged, b"packaged").expect("packaged executable");
    assert_eq!(super::managed_kodex_bin(home.path()), packaged);
...
fn updater_only_runs_for_stable_installer_owned_releases() {
    ...
    let managed = release.join("bin/kodex");
    ...
    let alpha_managed = alpha.join("bin/kodex");
    ...
    let local_managed = local.join("bin/kodex");
...
async fn older_managed_binary_does_not_claim_updater_support() {
    ...
    let binary = temp.path().join("kodex");
