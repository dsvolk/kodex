...
async fn sandbox_fetches_and_enforces_cloud_managed_permission_profile() -> Result<()> {
    ...
    if !nested_macos_sandbox_unavailable {
        assert!(
            String::from_utf8(output.stdout)?.starts_with("kodex"),
            "expected the sandboxed Kodex version command to run",
        );
    }

    let cache: Value = serde_json::from_slice(&std::fs::read(
        kodex_home.path().join("cloud-config-bundle-cache.json"),
    )?)?;
    assert_eq!(
        json!({
