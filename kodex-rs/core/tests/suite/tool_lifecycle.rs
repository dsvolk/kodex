...
async fn mcp_result_processing_precedes_completion(
    ...
    let call = match mode {
        ...
        McpCallMode::CodeMode => responses::ev_custom_tool_call(
            call_id,
            "exec",
            r#"text(await tools.mcp__kodex_apps__calendar_list_events({query: "callback test"}));"#,
        ),
    };
...
async fn tool_start_receives_frozen_host_plugin_root() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let kodex_home = Arc::new(tempfile::tempdir()?);
    let plugin_root = super::plugins::write_sample_plugin_manifest_and_config(kodex_home.as_ref());
    ...
    let test = test_kodex()
        ...
        .await?;
    wait_for_mcp_server(&test.kodex, "sample").await?;
    responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
