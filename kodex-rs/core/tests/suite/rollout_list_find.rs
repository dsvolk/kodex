...
use chrono::Utc;
use kodex_core::RolloutRecorder;
use kodex_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(kodex_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = kodex_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(kodex_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(kodex_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = kodex_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_kodex_home_directory() {
    let repo = TempDir::new().unwrap();
    let kodex_home = repo.path().join(".kodex");
    std::fs::create_dir_all(&kodex_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".kodex/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&kodex_home, id);

    let found =
        find_thread_path_by_id_str(&kodex_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
