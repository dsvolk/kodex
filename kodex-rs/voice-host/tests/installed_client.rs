...
async fn build_commit() -> Result<String> {
    let output = timeout(
        DEADLINE,
        Command::new(cargo_bin("kodex-voice-host")?)
            .arg("--build-commit")
            .kill_on_drop(true)
            .output(),
    )
...
fn install_startup_libraries(runtime: &Path) -> Result<()> {
    ...
    let source = if kodex_utils_cargo_bin::runfiles_available() {
        let resource = format!(
            "../../third_party/voice/native_link_{}_{}/runtime.json",
            std::env::consts::OS,
            std::env::consts::ARCH
        );
        kodex_utils_cargo_bin::find_resource!(resource)?
            .parent()
            .context("runtime parent")?
            .to_owned()
    } else {
        std::env::var_os("KODEX_TEST_VOICE_RUNTIME")
            .map(PathBuf::from)
...
fn install_helper(root: &Path) -> Result<KodexPackageLayout> {
    let bin = root.join("bin");
    let helper_dir = root.join("kodex-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(root.join("kodex-package.json"), "{}")?;
    let app = bin.join(if cfg!(windows) { "kodex.exe" } else { "kodex" });
    fs::write(&app, [])?;
    let source = cargo_bin("kodex-voice-host")?;
...
async fn installed_client_rejects_mixed_builds_and_missing_helper() -> Result<()> {
    ...
    let helper = directory
        .path()
        .join("kodex-resources/voice/bin")
