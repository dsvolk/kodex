...
use kodex_app_server_protocol::GatewayOAuthCancelResponse;
...
impl AccountRequestProcessor {
    async fn gateway_client(
        ...
        let manager = kodex_model_provider::create_model_provider(
    ...
    pub(crate) async fn gateway_oauth_login(
        ...
        let result = tokio::select! {
            ...
            url = url_rx => {
                if let Ok(url) = url {
                    self.outgoing.send_server_notification_to_connections(&[owner], ServerNotification::GatewayOAuthChanged(kodex_app_server_protocol::GatewayOAuthChangedNotification {
                        auth_url: Some(url), provider_id: config.model_provider_id.clone(),
                        status: kodex_app_server_protocol::GatewayOAuthStatus::Started, error: None,
                    })).await;
                }
                login.await
            }
