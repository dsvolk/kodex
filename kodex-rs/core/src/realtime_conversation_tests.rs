...
async fn clears_active_handoff_explicitly() {
    ...
    let state = RealtimeHandoffState {
        ...
        kodex_response_handoff_channel_prefixes: Arc::new(BTreeMap::new()),
...
fn realtime_headers_include_only_non_default_originator() {
    let default_originator = kodex_login::default_client::originator();
    for (originator, expected_header) in [
        ("kodex_work_desktop", Some("kodex_work_desktop")),
