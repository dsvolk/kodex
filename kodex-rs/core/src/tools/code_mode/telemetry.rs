...
use kodex_analytics::CodeModeToolCallFact;
use kodex_analytics::CodeModeToolCallStatus;
...
impl CodeModeToolCallGuard {
    pub(super) fn new(
        ...
        Self {
            ...
            cell_id: None,
            tool_name,
            started_at_ms: kodex_analytics::now_unix_millis(),
    ...
    pub(super) fn record_code_mode_host_duration(&self, duration: Duration) {
        ...
        tracing::info!(
            target: "kodex_code_mode::timing",
            {
                event.name = "kodex.code_mode.host_timing",
    ...
    fn drop(&mut self) {
        self.analytics
            .track_code_mode_tool_call(CodeModeToolCallFact::Completed {
                ...
                completed_at_ms: kodex_analytics::now_unix_millis(),
                status: self.status,
            });
    }
}
...
impl Drop for NestedToolDispatchTrace {
    fn drop(&mut self) {
        ...
        tracing::event!(
            name: "kodex.code_mode.nested_tool_dispatch_interrupted",
            target: "kodex_otel.trace_safe",
            parent: &self.span,
            tracing::Level::INFO,
            event.name = "kodex.code_mode.nested_tool_dispatch_interrupted",
