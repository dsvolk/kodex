...
const TEST_MODE: &str = "KODEX_APP_SERVER_SYSTEM_PROXY_TEST_MODE";
...
async fn browser_login_bootstraps_through_system_proxy() -> Result<()> {
        ...
        for mode in ["bootstrap-only", "cloud-enables-proxy"] {
            ...
            let output = timeout(
                ...
                command
                    .kill_on_drop(true)
                    .env(TEST_MODE, mode)
                    .env("KODEX_APP_SERVER_LOGIN_ISSUER", BLOCKED_ORIGIN)
    ...
    let model_catalog = kodex_models_manager::bundled_models_response()?;
    ...
    let saved_auth: serde_json::Value =
        serde_json::from_slice(&std::fs::read(kodex_home.path().join("auth.json"))?)?;
