...
use kodex_mcp::ResolvedMcpServer;
...
pub enum McpToolSource {
    /// A connector routed through the host-owned Kodex Apps MCP server.
    Connector,
    /// An MCP server whose frozen registration matches the active Kodex configuration.
    Config,
...
pub struct ToolStartInput<'a> {
    ...
    pub originating_item_id: Option<&'a kodex_protocol::ResponseItemId>,
