...
async fn resume_restores_windows_sandbox_override() -> Result<()> {
    ...
    let settings = initial.kodex.restorable_thread_settings().await;

    let resumed = builder.restart(&server, &initial).await?;
    resumed.kodex.restore_thread_settings(settings).await?;
...
async fn resume_includes_initial_messages_from_rollout_events() -> Result<()> {
    ...
    let mut builder = test_kodex();
    let initial = builder.build(&server).await?;
    let kodex = Arc::clone(&initial.kodex);
    ...
    resumed.kodex.flush_rollout().await?;
    let mut rejoined = resumed
        .thread_manager
        .resume_thread_from_rollout(
            resumed.config.clone(),
            resumed.kodex.rollout_path().expect("resumed rollout path"),
            resumed.thread_manager.auth_manager(),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
        )
        .await?;
    assert!(Arc::ptr_eq(&rejoined.thread, &resumed.kodex));
