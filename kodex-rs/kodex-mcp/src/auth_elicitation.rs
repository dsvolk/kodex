...
//! Session orchestration stays in `kodex-core`.

use kodex_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_KODEX_APPS_META_KEY: &str = "_kodex_apps";
...
pub struct KodexAppsConnectorAuthFailure {
...
pub struct KodexAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct KodexAppsAuthElicitationPlan {
    pub auth_failure: KodexAppsConnectorAuthFailure,
    pub elicitation: KodexAppsAuthElicitation,
}

#[derive(Serialize)]
struct KodexAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<KodexAppsConnectorAuthFailure> {
    ...
    Some(KodexAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<KodexAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(KodexAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &KodexAppsConnectorAuthFailure,
) -> KodexAppsAuthElicitation {
    KodexAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_KODEX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: KodexAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("kodex_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &KodexAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Kodex.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn auth_failure_detection_requires_trusted_connector_identity_and_auth_flag() {
        ...
        ordinary_error.meta.as_mut().expect("auth metadata")[MCP_TOOL_KODEX_APPS_META_KEY]
    ...
    fn detects_each_supported_connector_auth_reason() {
        for auth_reason in [
            ...
            result.meta.as_mut().expect("auth metadata")[MCP_TOOL_KODEX_APPS_META_KEY]
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "kodex_apps_auth_call_123");
    }
}
