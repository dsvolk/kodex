    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                kodex_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                KodexErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        Arc<dyn for<'call> kodex_extension_api::ToolExecutor<kodex_extension_api::ToolCall<'call>>>,
    > {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
