...
pub(crate) fn fetch(
    ...
    tokio::spawn(async move {
        let result = async {
            let mut entries = list(kodex_home.clone(), request.cwd.clone()).await?;
            let archive_root = kodex_home.join(kodex_rollout::ARCHIVED_SESSIONS_SUBDIR);
            for entry in &mut entries {
                ...
                if let Ok(response) = response {
                    ...
                    let Some(path) = kodex_rollout::existing_rollout_path(path).await else {
                        continue;
                    };
...
pub(crate) async fn list(kodex_home: PathBuf, cwd: PathBuf) -> anyhow::Result<Vec<Entry>> {
    let host = crate::legacy_core::config::load_config_toml_with_layer_stack(
        &kodex_home,
        /*cwd*/ None,
        Vec::new(),
        kodex_config::ConfigLoadOptions::default(),
    )
    .await?;
    let settings =
        kodex_worktree::WorktreeSettings::for_cli(&kodex_home, host.config_toml.desktop.as_ref())?;
    // Closing the popup discards its result; an already-running blocking Git call still finishes.
    tokio::task::spawn_blocking(move || {
        let cwd = kodex_git_utils::get_git_repo_root(&cwd).unwrap_or(cwd);
        let manager = kodex_worktree::WorktreeManager::new(settings);
        Ok(manager
            .list(&cwd)?
            .into_iter()
            .map(|checkout| Entry {
...
pub(crate) async fn remove(
    ...
    let settings =
        kodex_worktree::WorktreeSettings::for_cli(&kodex_home, host.config_toml.desktop.as_ref())?;
    tokio::task::spawn_blocking(move || {
        kodex_worktree::WorktreeManager::new(settings).remove(&source_cwd, &root)
    })
    .await?
}
