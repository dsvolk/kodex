...
async fn picker_side_worktree_fork_and_cd_run_on_the_production_stack() -> Result<()> {
    let start = if kodex_utils_cargo_bin::cargo_bin("kodex").is_ok() {
        PtyKodex::start_cli
    } else {
        PtyKodex::start
    };
    ...
    write_test_config(kodex_home.path(), &root)?;
    ...
    // Both launchers give kodex-main and Tokio workers explicit 16 MiB stacks.
    ...
    terminal.wait_for_screen("Ask Kodex to do anything")?;
...
fn submit(terminal: &mut PtyKodex, input: &str) -> Result<()> {
