...
The TypeScript SDK wraps the `kodex` CLI from `@openai/kodex`. It spawns the CLI and exchanges JSONL events over stdin/stdout.

## Installation

```bash
npm install @openai/kodex-sdk
```
...
Threads are persisted in `~/.kodex/sessions`. If you lose the in-memory `Thread` object, reconstruct it with `resumeThread()` and keep going.

```typescript
const savedThreadId = process.env.KODEX_THREAD_ID!;
const thread = kodex.resumeThread(savedThreadId);
await thread.run("Implement the fix");
```

### Working directory controls

Kodex runs in the current working directory by default. To avoid unrecoverable errors, Kodex requires the working directory to be a Git repository. You can skip the Git repository check by passing the `skipGitRepoCheck` option when creating a thread.
...
`Kodex` client to fully control which variables the CLI receives—useful for sandboxed hosts like Electron apps.
...
The SDK still injects its required variables (such as `KODEX_API_KEY`) on top of the environment you provide. If you set
...
is forwarded unchanged as a separate `--config` argument, without modifying `KODEX_HOME`:
