...
use kodex_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(kodex_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<KodexThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<KodexThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &KodexThread) -> String {
...
async fn wait_for_recorded_user_message(thread: &KodexThread, needle: &str) {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<KodexThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<KodexThread>, message: &str) {
...
async fn mcp_attribution_in_constructed_request(thread: &KodexThread) -> McpAttribution {
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(err) => match err.details() {
            KodexErrorDetails::ThreadNotFound(id) => assert_eq!(*id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(
        err.details(),
        KodexErrorDetails::ThreadNotFound(id) if *id == thread_id
    );
}
...
async fn check_v2_agent_reload(route: V2ReloadRoute) {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(err) => match err.details() {
            KodexErrorDetails::ThreadNotFound(id) => assert_eq!(*id, spawned_agent.thread_id),
            _ => panic!("expected ThreadNotFound, got {err:?}"),
        },
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        KodexAuth::from_api_key("dummy"),
        harness.config.model_provider.clone(),
        harness.config.kodex_home.to_path_buf(),
        std::sync::Arc::new(kodex_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_fork_from_paginated_parent_uses_model_context_prefix() {
    ...
    let lines = std::fs::read_to_string(&rollout_path)
        ...
        .lines()
        .map(|line| kodex_rollout::parse_rollout_line(line).expect("parse rollout line"))
        .collect::<Vec<_>>();
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = kodex_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
...
async fn spawn_agent_fork_sanitizes_inherited_compaction_metadata(thread_context_enabled: bool) {
    ...
    let (parent_thread_id, parent_thread) = harness.start_paginated_thread().await;
    let parent_resume_metadata = kodex_history::CompactionResumeMetadata {
        multi_agent_version: Some(MultiAgentVersion::V2),
        last_started_turn_id: Some("parent-turn".into()),
        previous_turn_settings: Some(kodex_history::PreviousTurnSettings {
    ...
    assert_eq!(
        ...
        &kodex_history::CompactionResumeMetadata {
...
async fn spawn_agent_fork_strips_parent_usage_hints_from_compacted_history(
    ...
    let answer_event: kodex_history::RetainedContextEvent = serde_json::from_value(serde_json::json!({
    ...
    let delivery = kodex_history::ResponseItemEnvelope {
        ...
        .unwrap(),
        metadata: Some(kodex_history::KodexHarnessMetadata {
            user_input_order: Some(7),
            sender_user_messages: Some(Box::new(kodex_history::SenderUserMessages {
    ...
    let mut retained_context = kodex_history::RetainedContext::default();
    ...
    parent_thread
        ...
        .persist_rollout_items(&[
            RolloutItem::Compacted(CompactedItem {
                ...
                guardian_history: Some(kodex_history::GuardianHistoryCheckpoint(vec![
    ...
    let mut inherited_context = kodex_history::RetainedContext::default();
...
async fn spawn_agent_full_fork_legacy_compaction_rebuilds_child_instructions_once() {
    ...
    for (case, parent_developer_instructions) in [
        ...
        parent_config.config_layer_stack = kodex_config::ConfigLayerStack::new(
            parent_config
                .config_layer_stack
        ...
        let mut rollout_items = vec![
            ...
            RolloutItem::Compacted(CompactedItem {
                ...
                compaction_response_id: None,
                latest_token_usage_record: None,
                resume_metadata: Some(kodex_history::CompactionResumeMetadata {
                    multi_agent_version: Some(MultiAgentVersion::V2),
        ...
        let (items, _, _) = kodex_rollout::RolloutRecorder::load_rollout_items(
        ...
        assert_eq!(
            inherited_state,
            Some(kodex_history::CompactionResumeMetadata {
                multi_agent_version: Some(MultiAgentVersion::V2),
                last_started_turn_id: None,
...
async fn resume_and_cold_fork_restore_mcp_attribution_in_constructed_requests() {
    ...
    let resumed = harness
        ...
        .resume_thread_from_rollout(
            harness.config.clone(),
            rollout_path,
            AuthManager::from_auth_for_testing(KodexAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
...
async fn spawn_agent_respects_legacy_max_threads_alias() {
    ...
    let KodexErrorDetails::AgentLimitReached {
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let KodexErrorDetails::AgentLimitReached { max_threads } = err.details() else {
        panic!("expected AgentLimitReached");
    };
...
async fn spawn_thread_subagents_persist_parent_originator_across_new_and_truncated_fork() {
    ...
    assert_eq!(parent_originator, "kodex_work_desktop");
