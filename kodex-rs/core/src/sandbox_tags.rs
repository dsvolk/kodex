...
use kodex_sandboxing::policy_transforms::should_require_platform_sandbox;
use std::path::Path;
...
impl SandboxTags {
    ...
    pub(crate) fn record_metadata(&self, metadata: &mut KodexResponsesMetadata) {
...
pub(crate) fn record_policy_metadata(
    profile: &PermissionProfile,
    cwd: &Path,
    metadata: &mut KodexResponsesMetadata,
) {
