...
#[cfg(debug_assertions)]
use crate::events::KodexPluginMetadata;
#[cfg(debug_assertions)]
use crate::events::KodexPluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::KodexPluginUsedMetadata;
#[cfg(debug_assertions)]
...
use kodex_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use kodex_app_server_protocol::AskForApproval as AppServerAskForApproval;
use kodex_app_server_protocol::ClientRequest;
use kodex_app_server_protocol::ClientResponsePayload;
use kodex_app_server_protocol::CommandExecutionOutputDeltaNotification;
use kodex_app_server_protocol::RequestId;
use kodex_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
...
use kodex_app_server_protocol::ThreadStatus as AppServerThreadStatus;
...
fn sample_accepted_line_fingerprint_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::AcceptedLineFingerprints(Box::new(
        KodexAcceptedLineFingerprintsEventRequest {
            event_type: "kodex_accepted_line_fingerprints",
            event_params: KodexAcceptedLineFingerprintsEventParams {
                event_type: "kodex.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: thread_id.to_string(),
                product_surface: Some("kodex".to_string()),
                model_slug: Some("gpt-5.1-kodex".to_string()),
                completed_at: 1,
                repo_hash: None,
...
fn sample_artifact_operation_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::ArtifactOperation(kodex_artifact_operation_event_request(
        TrackEventsContext {
            turn_metadata: None,
            model_slug: "gpt-5.1-kodex".to_string(),
            thread_id: thread_id.to_string(),
            turn_id: "turn-1".to_string(),
            product_client_id: "kodex_desktop".to_string(),
        },
        ArtifactOperation {
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(KodexMcpToolCallEventRequest {
        event_type: "kodex_mcp_tool_call_event",
        event_params: KodexMcpToolCallEventParams {
            base: KodexToolItemEventBase {
                thread_id: thread_id.to_string(),
                ...
                runtime: KodexRuntimeMetadata {
                    kodex_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(KodexPluginUsedEventRequest {
        event_type: "kodex_plugin_used",
        event_params: KodexPluginUsedMetadata {
            plugin: KodexPluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                remote_plugin_id: None,
                ...
                product_client_id: Some("kodex_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "kodex-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://chatgpt.com/backend-api/kodex/analytics-events/events".to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = kodex_login::KodexAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "kodex_accepted_line_fingerprints"
    );
...
#[tokio::test]
#[cfg(debug_assertions)]
async fn api_key_auth_sends_only_plugin_events_to_kodex_backend() {
    let capture_path = unique_capture_path("api-key-plugin-events");
    let destination = AnalyticsEventsDestination::CaptureFile {
        path: capture_path.clone(),
    };
    let auth_manager = kodex_login::AuthManager::from_auth_for_testing(
        kodex_login::KodexAuth::from_api_key("sk-test"),
    );
    let plugin_measurement = |thread_id: &str, plugin_id: &str| {
        TrackEventRequest::PluginMeasurement(KodexPluginMeasurementEventRequest {
            ...
            event_params: KodexPluginMeasurementEventParams {
                ...
                originator: "kodex_cli_rs".to_string(),
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn flush_is_noop_when_analytics_is_disabled() {
    let client = AnalyticsEventsClient::new(
        kodex_login::AuthManager::from_auth_for_testing(
