    ...
    pub(crate) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::RemoveManagedWorktree { request, root } => {
                ...
                {
                    let kodex_home = self.config.kodex_home.to_path_buf();
            ...
            AppEvent::RevertSessionForPromptEdit {
                ...
                let reverted = match app_server.revert_thread(thread_id, before_turn_id, &retained_turns).await {
                    ...
                    if matches!(&err, kodex_app_server_client::TypedRequestError::Server { source, .. }
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt {
                ...
                self.session_telemetry.counter(
                    "kodex.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                self.chat_widget.clear_windows_sandbox_setup_status();
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "kodex.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
                ...
                #[cfg(target_os = "windows")]
                {
                        ...
                        .await
                    {
                            ...
                            if let Some(selection) = profile_selection {
                                ...
                                self.app_event_tx.send(AppEvent::KodexOp(
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Kodex can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                    } else {
            ...
            AppEvent::AgentsDaemonStarted { result } => match result {
                Ok(()) => self.chat_widget.add_info_message(
                    "Background server started. Run `kodex agents` in another terminal; this session remains unchanged."
                        .to_string(),
                    /*hint*/ None,
                ),
                Err(error) => self
        ...
        Ok(match result {
            ...
            Ok(()) => {
                self.track_agents_overview_notification(&ServerNotification::ThreadArchived(
                    kodex_app_server_protocol::ThreadArchivedNotification {
        ...
        Ok(match result {
            ...
            Ok(()) => {
                self.track_agents_overview_notification(&ServerNotification::ThreadDeleted(
                    kodex_app_server_protocol::ThreadDeletedNotification {
                        thread_id: thread_id.to_string(),
                    },
                ));
                self.discard_thread_local_state(thread_id).await;
