...
use anyhow::anyhow;
use kodex_agent_roles::parse_agent_role_file_contents;
use kodex_config::ConfigLayerEntry;
use kodex_config::ConfigLayerSource;
use kodex_config::ConfigLayerStack;
use kodex_config::SkillsConfig;
use kodex_config::loader::resolve_relative_paths_in_config_toml;
use kodex_exec_server::read_sensitive_file_to_string;
...
async fn apply_role_to_config_inner(
    ...
    let role_config = deserialize_config_toml_with_base(role_layer_toml, &config.kodex_home)?;
...
async fn load_role_layer_toml(
    ...
    let (role_config_toml, role_config_base) = if is_built_in {
        ...
        (role_config_toml, config.kodex_home.as_path())
    } else {
