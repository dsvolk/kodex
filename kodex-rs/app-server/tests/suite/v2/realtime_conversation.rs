...
use kodex_app_server_protocol::ThreadRealtimeAudioChunk;
...
use kodex_app_server_protocol::ThreadRealtimeInitialItem;
use kodex_app_server_protocol::ThreadRealtimeItem;
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime(&mut self, offer_sdp: &str) -> Result<StartedWebrtcRealtime> {
        self.start_webrtc_realtime_with_kodex_response_routing(
            offer_sdp,
    ...
    async fn start_webrtc_realtime_with_kodex_response_items(
        &mut self,
        offer_sdp: &str,
        ...
        self.start_webrtc_realtime_with_kodex_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*kodex_responses_as_items*/ Some(true),
            /*kodex_response_handoff_mode*/ None,
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
    ...
    async fn start_webrtc_realtime_with_kodex_response_routing(
        &mut self,
        offer_sdp: &str,
        client_managed_handoffs: Option<bool>,
        kodex_responses_as_items: Option<bool>,
        kodex_response_handoff_mode: Option<KodexResponseHandoffMode>,
        ...
        let start_request_id = self
            ...
            .send_thread_realtime_start_request(ThreadRealtimeStartParams {
                ...
                kodex_response_item_prefix: kodex_responses_as_items
                    .unwrap_or(false)
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_kodex_responses_as_items(
            /*kodex_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_kodex_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_kodex_responses_as_items(
        &mut self,
        kodex_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
const PROVIDER_RESIDENCY_ENV_VAR: &str = "KODEX_TEST_REALTIME_RESIDENCY_HEADER";
    ...
    let mut config = MockResponsesConfig::new(&responses_uri)
        ...
        .with_extra_config(
            "[model_providers.mock_provider.http_headers]\n\
             \"X-OpenAI-Internal-Kodex-Residency\" = \"eu-static\"\n\
        ...
        ProviderHeaders::StaticAndEnvironment => {
            config = config.with_extra_config(&format!(
                "[model_providers.mock_provider.env_http_headers]\n\
                 \"x-openai-internal-kodex-residency\" = \"{PROVIDER_RESIDENCY_ENV_VAR}\""
            ));
            "eu-environment"
        }
    };
...
async fn webrtc_v3_start_posts_live_session_and_joins_without_session_update() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_kodex_response_routing(
            "v=offer\r\n",
            /*client_managed_handoffs*/ None,
            /*kodex_responses_as_items*/ None,
            /*kodex_response_handoff_mode*/ None,
...
async fn webrtc_v1_client_managed_handoffs_disable_automatic_output() -> Result<()> {
    ...
    assert!(
        ...
        "automatic Kodex output should not reach realtime in client-managed handoff mode"
    );
...
#[tokio::test]
async fn webrtc_v1_ignores_kodex_response_handoff_mode() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let started = harness
        .start_webrtc_realtime_with_kodex_response_routing(
            ...
            /*client_managed_handoffs*/ None,
            /*kodex_responses_as_items*/ None,
            /*kodex_response_handoff_mode*/ Some(KodexResponseHandoffMode::BemTags),
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
        .await?;
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_kodex_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_kodex_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v3_routes_handoffs_by_session_mode() -> Result<()> {
    ...
    for (mode, channel_prefixes, texts, expected_channels) in [
        ...
        (
            Some(KodexResponseHandoffMode::Commentary),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
        ...
        (
            Some(KodexResponseHandoffMode::BemTags),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Kodex should then continue
...
async fn realtime_starts_on_forked_and_resumed_threads(
    ...
    if let RealtimeThreadLoad::ColdResume = load {
        ...
        harness.mcp = TestAppServer::builder()
            .with_kodex_home(harness._kodex_home.path())
            .build_initialized_with_timeout(DEFAULT_TIMEOUT)
            .await?;
