...
fn event_target(value: &str) -> Option<&'static str> {
    ...
    let name = *parts.last()?;
    let package = parts.iter().any(|part| part.starts_with("openai.kodex_"));
    let trusted = package || parts.windows(2).any(|pair| pair == ["openai", "kodex"]);
    match name {
        "kodex-windows-sandbox-setup.exe" => Some("sandbox_setup"),
        "kodex-command-runner.exe" => Some("command_runner"),
        "kodex.exe" => Some("kodex"),
        "kodex-desktop.exe" => Some("kodex_desktop"),
        "chatgpt.exe" | "electron.exe" if trusted => Some("kodex_desktop"),
        "rg.exe" if trusted => Some("ripgrep"),
        _ if package => Some("kodex_desktop_package"),
        _ => None,
    }
}
...
fn classify(channels: &[Option<Vec<Evidence>>]) -> DoctorCheck {
    ...
    let summary = match (status, visible) {
        ...
        (CheckStatus::Ok, _) => "no locally visible recent Kodex security enforcement was found",
        (CheckStatus::Warning, false) => "security event channels could not be inspected",
        (CheckStatus::Warning, true) => "recent Kodex security audit or detection requires review",
        (CheckStatus::Fail, _) => "endpoint security blocked or quarantined a Kodex executable",
    };
    let mut check = desktop_check("desktop.security.enforcement", status, summary).details(details);
    if status != CheckStatus::Ok {
        check = check.remediation(
            "ask your organization's security administrator to review endpoint security events and the approved Kodex application policy",
        );
    }
    check
}
