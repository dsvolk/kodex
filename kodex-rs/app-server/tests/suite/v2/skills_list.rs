...
use kodex_app_server_protocol::SkillScope;
use kodex_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = kodex_home.join("plugins/cache/openai-curated-remote/linear/local");
...
fn write_cached_local_curated_plugin_with_skill(
    ...
    let plugin_root = kodex_home.join(format!(
...
async fn skills_list_uses_each_cwds_bundled_skills_configuration() -> Result<()> {
    ...
    for (cwd, enabled) in [(disabled_cwd.path(), false), (enabled_cwd.path(), true)] {
        ...
        std::fs::create_dir_all(cwd.join(".kodex"))?;
        std::fs::write(
            cwd.join(".kodex/config.toml"),
            format!("[skills.bundled]\nenabled = {enabled}\n"),
        )?;
        set_project_trust_level(kodex_home.path(), cwd, TrustLevel::Trusted)?;
    }
...
async fn skills_list_runtime_enable_refreshes_shared_system_skill_cache() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    let stale_skill_path = kodex_home
        .path()
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(kodex_home.path(), "openai-curated")?;
    std::fs::write(
        kodex_home.path().join("config.toml"),
        format!(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(kodex_home.path())?)?;
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_skill(&kodex_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".kodex/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(kodex_home.path().join(&relative_cwd))?;
...
async fn skills_list_preserves_requested_cwd_order(home_plugins_enabled: bool) -> Result<()> {
    ...
    write_skill(&kodex_home, "shared-skill")?;
    write_cached_local_curated_plugin_with_skill(kodex_home.path(), "openai-api-curated")?;
    std::fs::write(
        kodex_home.path().join("config.toml"),
        format!(
            r#"[features]
...
async fn skills_list_refreshes_externally_updated_plugin_versions() -> Result<()> {
    ...
    std::fs::create_dir_all(source.path().join(".kodex-plugin"))?;
    std::fs::create_dir_all(source.path().join("skills"))?;
    std::fs::write(
        source.path().join(".kodex-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
        kodex_home.path().join("config.toml"),
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".kodex/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
