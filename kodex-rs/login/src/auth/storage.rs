...
use kodex_agent_identity::AgentIdentityJwtClaims;
use kodex_agent_identity::decode_agent_identity_jwt;
...
use kodex_secrets::LocalSecretsNamespace;
use kodex_secrets::SecretName;
use kodex_secrets::SecretScope;
use kodex_secrets::SecretsBackendKind;
use kodex_secrets::SecretsManager;
use once_cell::sync::Lazy;

/// Expected structure for $KODEX_HOME/auth.json.
...
pub(super) fn get_auth_file(kodex_home: &Path) -> PathBuf {
    kodex_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(kodex_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(kodex_home: PathBuf) -> Self {
        Self { kodex_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `KODEX_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.kodex_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.kodex_home)
    }
}

static KODEX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("KODEX_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("KODEX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Kodex Auth";

// turns kodex_home path into a stable, short key string
fn compute_store_key(kodex_home: &Path) -> std::io::Result<String> {
    let canonical = kodex_home
        .canonicalize()
        .unwrap_or_else(|_| kodex_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(kodex_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            kodex_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.kodex_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.kodex_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.kodex_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(kodex_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(kodex_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            kodex_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::KodexAuth,
        );
        Self {
            kodex_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &KODEX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &KODEX_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &KODEX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(kodex_home)),
        }
    }
}
...
// A global in-memory store for mapping kodex_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(kodex_home: PathBuf) -> Self {
        Self { kodex_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(kodex_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(kodex_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(kodex_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(kodex_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(kodex_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(kodex_home, keyring_store))
        }
    }
}
