...
pub fn current_log_file_path_for_kodex_home(kodex_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(kodex_home))
}
...
mod tests {
    ...
    fn setup_log_releases_handles_without_path_fallback() {
        const CHILD_ENV: &str = "KODEX_TEST_SETUP_LOG_RELEASE_CHILD";
    ...
    #[test]
    fn current_log_file_path_for_kodex_home_uses_sandbox_dir() {
        let kodex_home = Path::new("kodex-home");

        assert_eq!(
            current_log_file_path_for_kodex_home(kodex_home),
            current_log_file_path(&kodex_home.join(".sandbox"))
        );
    }
}
