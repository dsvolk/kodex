...
use kodex_extension_api::CommandStartInput;
...
fn configure_command_test_permissions(config: &mut Config) {
    // Match TestKodex::submit_turn: these fixtures exercise execution callbacks
...
async fn start_command_turn(test: &TestKodex) -> Result<()> {
    test.kodex
