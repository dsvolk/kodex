        ...
        original
            .start_process(
                crate::protocol::ExecParams {
                    ...
                    cwd: kodex_utils_path_uri::PathUri::from_host_native_path(
                        std::env::current_dir()?,
                    )?,
                    shell_snapshot: None,
                    env_policy: None,
...
struct ControlledRpc {
    ...
    requests: tokio::sync::mpsc::Receiver<kodex_exec_server_protocol::JSONRPCMessage>,
...
async fn application_policy_registry_denial_does_not_poison_later_noise_startup() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(kodex_http_client::NetworkPolicyDenied::Unavailable)
    );
