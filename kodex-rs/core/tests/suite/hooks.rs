...
use kodex_config::HookStateToml;
...
use kodex_core::config::ThreadStoreConfig;
...
use kodex_plugin::PluginHookSource;
use kodex_plugin::PluginId;
use kodex_protocol::items::parse_hook_prompt_fragment;
...
async fn managed_hook_discovery_failure_rejects_session_startup_when_hooks_enabled() -> Result<()> {
    ...
    let mut builder = test_kodex().with_cloud_config_bundle(
...
fn trust_plugin_hooks(config: &mut Config, plugin_hook_sources: Vec<PluginHookSource>) {
    ...
    let listed = kodex_hooks::list_hooks(kodex_hooks::HooksConfig {
...
fn rollout_hook_prompt_texts(text: &str) -> Result<Vec<String>> {
    ...
    for line in text.lines() {
        ...
        let rollout = kodex_rollout::parse_rollout_line(trimmed).context("parse rollout line")?;
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.kodex_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.kodex_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.kodex_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
...
"#,
    ...
    let forked = test
        ...
        .fork_thread(
            ...
            StartThreadOptions::new(test.config.clone()),
            test.kodex.rollout_path().expect("parent rollout path"),
        )
        .await?
        .thread;
    forked
    ...
    test.kodex
        .start_or_steer_turn(
            ...
            .with_thread_settings(
                kodex_protocol::protocol::ThreadSettingsOverrides {
...
async fn async_hook_finishing_while_idle_waits_for_the_next_turn(
    ...
    assert!(
        timeout(Duration::from_millis(150), test.kodex.next_event())
            .await
            .is_err(),
    ...
    test.kodex.start_turn_if_idle(next_turn).await?;
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.kodex_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
            .filter_map(|input| input.get("source").and_then(Value::as_str))
...
async fn resumed_thread_keeps_stop_continuation_prompt_in_history() -> Result<()> {
    ...
    let mut resume_builder = test_kodex().with_config(trust_discovered_hooks);
    let resumed = resume_builder.restart(&server, &initial).await?;
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.kodex_home_path())?;
...
async fn blocked_queued_prompt_does_not_strand_earlier_accepted_prompt(
    ...
    let history = test.kodex.conversation_history_snapshot().await;
...
async fn permission_request_hook_allow_bypasses_strict_auto_review() -> Result<()> {
    ...
    let request = wait_for_event(&test.kodex, |event| {
...
async fn permission_request_hook_allows_network_approval_without_prompt() -> Result<()> {
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://kodex-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
...
async fn permission_request_hook_denies_network_approval_with_custom_message() -> Result<()> {
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); opener.open('http://kodex-network-test.invalid', timeout=2).read()""#;
    run_network_permission_hook_test(
        "deny",
        denial,
...
"#,
    ...
    let test = test_kodex()
        ...
        .with_config(move |config| {
            trust_discovered_hooks(config);
            config.approvals_reviewer = kodex_config::types::ApprovalsReviewer::AutoReview;
    ...
    if expected_denial.is_none() {
        ...
        assert!(
            timeout(
                ...
                wait_for_event(&test.kodex, |event| matches!(
                    event,
    ...
    assert_single_permission_request_hook_input(
        test.kodex_home_path(),
        command,
        Some("network-access http://kodex-network-test.invalid:80"),
    )?;
    let requests = responses.requests();
    assert_eq!(
        requests
...
async fn pre_tool_use_json_deny_blocks_exec_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.kodex_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
"#
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.kodex_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(hook_inputs[0]["tool_input"]["command"], command);
    assert_eq!(
...
async fn local_bundled_cleanup_hook_runs_without_saved_trust(
    ...
    fs::create_dir_all(plugin_root.join(".kodex-plugin"))?;
    ...
    let hook_key = kodex_hooks::hook_key(
    ...
    let mut builder = test_kodex().with_home(home).with_config(move |config| {
        for feature in [Feature::Plugins, Feature::KodexHooks] {
    ...
    assert_eq!(
        test.config.features.enabled(Feature::KodexHooks),
        hooks_enabled
    );
    ...
    if !hooks_enabled {
        let regular_hooks = kodex_hooks::list_hooks(kodex_hooks::HooksConfig {
            feature_enabled: true,
    ...
    assert_eq!(
        kodex_hooks::hook_states_from_stack(Some(&test.config.config_layer_stack)),
        expected_hook_states
    );
    wait_for_mcp_server(&test.kodex, mcp_server_name).await?;
    test.kodex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<kodex_config::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.kodex_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.kodex_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.kodex_home_path())?;
