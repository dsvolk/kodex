...
use kodex_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# Kodex user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# Kodex user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
async fn psp_feature_configures_first_party_routing() -> Result<()> {
    ...
    assert!(config.features.enabled(kodex_features::Feature::Psp));
    assert_eq!(
...
async fn batch_write_rejects_legacy_profile_selector() -> Result<()> {
    ...
    let error = service
        .batch_write(ConfigBatchWriteParams {
            edits: vec![
                kodex_app_server_protocol::ConfigEdit {
            ...
            apps: std::collections::HashMap::from([(
                ...
                AppConfig {
                    ...
                    omit_tools_from: Some(vec![
                        kodex_protocol::config_types::ToolExposureSurface::Deferred
                    ]),
...
async fn load_default_config_preserves_managed_requirements_and_selected_user_config_path() {
    ...
    assert_eq!(
        config
            ...
            .requirements()
            .managed_auth_policy(),
        kodex_config::ManagedAuthPolicy {
            allowed_login_methods: Some(vec![kodex_protocol::config_types::ForcedLoginMethod::Api]),
...
async fn managed_auth_policy_survives_unusable_requirements_file_changes() -> Result<()> {
    ...
    let auth_manager = kodex_login::AuthManager::shared_from_config(
        &startup, /*enable_kodex_api_key_env*/ false,
    )
    .await?;
    std::fs::write(
        &requirements_path,
    ...
    assert!(
        auth_manager.is_login_method_allowed(kodex_protocol::config_types::ForcedLoginMethod::Api)
    );
    assert!(
        !auth_manager
            .is_login_method_allowed(kodex_protocol::config_types::ForcedLoginMethod::Chatgpt)
    );
    assert_eq!(
...
async fn allowed_login_methods_follow_current_forced_workspaces() -> Result<()> {
    ...
    let auth = kodex_login::AuthManager::shared_from_config(
        &config, /*enable_kodex_api_key_env*/ false,
    )
    .await?;
...
async fn permission_config_reload_merges_session_layers() -> Result<()> {
    ...
    let wrapper = wrapper_dir.join("kodex-execve-wrapper");
    ...
    let service = ConfigManager::new(
        ...
        /*strict_config*/ false,
        CloudConfigBundleLoader::default(),
        kodex_arg0::Arg0DispatchPaths {
            main_execve_wrapper_exe: Some(wrapper),
            ..Default::default()
        },
        std::sync::Arc::new(kodex_config::NoopThreadConfigLoader),
    );

    let mut config = service
        .load_with_overrides(
            /*request_overrides*/ None,
            kodex_core::config::ConfigOverrides {
