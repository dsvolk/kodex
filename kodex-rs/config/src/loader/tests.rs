...
use kodex_file_system::CopyOptions;
use kodex_file_system::CreateDirectoryOptions;
use kodex_file_system::ExecutorFileSystemFuture;
use kodex_file_system::FileMetadata;
use kodex_file_system::FileSystemReadStream;
use kodex_file_system::FileSystemSandboxContext;
use kodex_file_system::GetMetadataOptions;
use kodex_file_system::ReadDirectoryEntry;
use kodex_file_system::ReadFileOptions;
use kodex_file_system::RemoveOptions;
use kodex_file_system::WalkOptions;
use kodex_file_system::WalkOutcome;
use kodex_file_system::WriteFileOptions;
...
fn project_environment_filters_keep_excluded_hosts_out_of_children() {
    ...
    for project_policy in [
        ...
        let actual = kodex_protocol::shell_environment::populate_env(
            [
                ("GH_HOST", host),
...
async fn default_windows_managed_config_is_ignored_with_warning() {
    ...
    let managed_config_path = kodex_home.join("managed_config.toml");
    std::fs::write(
        &managed_config_path,
        r#"
model = "legacy-model"
...
"#,
    ...
    std::fs::write(kodex_home.join(CONFIG_TOML_FILE), r#"model = "user-model""#)
    ...
    let expected_warnings = vec![format!(
        "Ignoring deprecated managed config file at {}; KODEX_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\Kodex\\requirements.toml for enforced settings or config.toml for defaults.",
...
fn windows_local_managed_configuration_ignores_legacy_file_but_detects_requirements() {
    ...
    let kodex_home = tmp.path().join("kodex-home");
    std::fs::create_dir_all(&kodex_home).expect("create kodex home");
    std::fs::write(kodex_home.join("managed_config.toml"), "")
        .expect("write default legacy managed config");
...
"#,
    ...
    assert!(
        message.contains("https://developers.openai.com/kodex/config-advanced#profiles"),
        "unexpected error message: {message}"
    );
}
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    let dot_kodex = project.join(".kodex");
    let system_dir = tmp.path().join("system");
    let managed_dir = tmp.path().join("managed");
    for dir in [&kodex_home, &dot_kodex, &system_dir, &managed_dir] {
    ...
    let user_file = kodex_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &user_file,
        format!(
