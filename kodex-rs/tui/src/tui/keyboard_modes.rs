...
fn read_windows_vscode_detection() -> VscodeDetection {
    ...
    read_windows_vscode_detection_with_timeout(
        || {
            let executable = kodex_utils_path::system_executable("cmd.exe")?;
...
fn read_tmux_extended_keys_format() -> Option<String> {
    let executable = kodex_utils_path::system_executable("tmux")?;
    let path = kodex_utils_path::system_path().ok()?;
    for args in [
