...
use kodex_protocol::protocol::ExecCommandOutputDeltaEvent;
use kodex_protocol::protocol::ExecOutputStream;
...
pub(crate) use kodex_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use kodex_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use kodex_sandboxing::record_filesystem_sandbox_violation;
#[cfg(test)]
...
#[cfg(test)]
use kodex_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
...
use kodex_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: kodex_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    KodexErr::Io(io::Error::other(format!(
...
pub async fn process_exec_tool_call(
    ...
    let windows_sandbox_type = if params.windows_sandbox_level
        == kodex_protocol::config_types::WindowsSandboxLevel::Disabled
    {
        SandboxType::None
    } else {
    ...
    let request = manager
        .transform(SandboxTransformRequest {
            ...
            sandbox_exe: if cfg!(windows) {
                kodex_self_exe.as_deref()
            } else {
                kodex_linux_sandbox_exe.as_deref()
            },
            use_legacy_landlock,
            windows_sandbox_level,
        })
        .map_err(KodexErr::from)?;
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| KodexErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    windows_sandbox_level: kodex_protocol::config_types::WindowsSandboxLevel,
    err: &str,
) {
    ...
    let level = if matches!(
        windows_sandbox_level,
        kodex_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = kodex_otel::global() {
        let _ = metrics.counter(
            "kodex.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_kodex_home;
    use kodex_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use kodex_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            kodex_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let kodex_home = find_kodex_home().map_err(|err| {
        KodexErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve kodex_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                kodex_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    kodex_home: kodex_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(KodexErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, KodexErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(KodexErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(KodexErr::Sandbox(SandboxErr::Timeout {
            ...
            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                ...
                return Err(KodexErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
