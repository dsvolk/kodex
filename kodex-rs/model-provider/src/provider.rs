...
use kodex_models_manager::cache::ModelsCache;
...
use kodex_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Kodex may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "kodex-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Kodex should have a unified provider-specific auth
    ...
    fn map_api_error(&self, error: ApiError) -> KodexErr {
        kodex_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, kodex_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            self.info()
                .to_api_provider(auth.as_ref().map(KodexAuth::auth_mode))
        })
    }
    ...
    ) -> ModelProviderFuture<'a, kodex_protocol::error::Result<ResolvedResponsesProvider>> {
        Box::pin(async move {
            ...
            if provider_uses_first_party_auth_path(self.info())
                && self.info().supports_kodex_backend_routes()
                && let Some(auth) = self.auth().await.filter(KodexAuth::is_chatgpt_auth)
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, kodex_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, kodex_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Kodex session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, kodex_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
            if !provider_uses_first_party_auth_path(self.info()) {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| kodex_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, KodexAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    KodexAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    KodexAuth::BedrockApiKey(_) | KodexAuth::BedrockAccessKeys(_) => {
...
mod tests {
    ...
    fn test_kodex_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("kodex-model-provider-test-{}", std::process::id()))
    }
    ...
    fn bedrock_api_key_auth() -> KodexAuth {
        KodexAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("kodex-bedrock".to_string()),
                region: None,
    ...
    async fn shared_bedrock_auth_refresh_is_reused_only_for_matching_configuration() {
        ...
        const HELPER_ARG: &str = "KODEX_BEDROCK_SHARED_AUTH_REFRESH_COMMAND";
        const SUBPROCESS_ARG: &str = "KODEX_BEDROCK_SHARED_AUTH_REFRESH_SUBPROCESS";
...
"#
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_kodex_home(), /*config_model_catalog*/ None);
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = kodex_models_manager::bundled_models_response()
