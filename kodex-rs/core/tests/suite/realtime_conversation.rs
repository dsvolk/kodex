...
use kodex_app_server_protocol::ThreadRealtimeItemContent;
use kodex_app_server_protocol::ThreadRealtimeSessionOutcome;
use kodex_app_server_protocol::ThreadRealtimeTranscriptRole;
use kodex_app_server_protocol::ThreadTimelineEntry;
...
use kodex_login::OPENAI_API_KEY_ENV_VAR;
...
use kodex_protocol::protocol::RealtimeHandoffRequested;
use kodex_protocol::protocol::RealtimeNoopRequested;
use kodex_protocol::protocol::RealtimeOutputModality;
use kodex_protocol::protocol::RealtimeTranscriptEntry;
...
use kodex_thread_store::ListTimelineParams;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Kodex.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = kodex_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "KODEX_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
async fn seed_recent_thread(
    ...
    let db = test.kodex.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = kodex_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.kodex, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.kodex, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.kodex, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    let closed = wait_for_event_match(&test.kodex, |msg| match msg {
    ...
    let history = test.kodex.load_history(/*include_archived*/ false).await?;
    assert!(
        !history
            .items
            .iter()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_websocket_transports_send_kodex_headers_without_creating_a_call(
    ...
    let conversation = test
        ...
        .start_thread(StartThreadOptions {
            ...
            environments: Some(test.kodex.config_snapshot().await.environments.environments),
            ..StartThreadOptions::new(test.config.clone())
        })
        .await?;
    let kodex = &conversation.thread;
    ...
    let started = wait_for_event_match(kodex, |msg| match msg {
        EventMsg::RealtimeConversationStarted(started) => Some(Ok(started.clone())),
        EventMsg::Error(err) => Some(Err(err.clone())),
        _ => None,
    })
    ...
    let (expected_uri, expected_realtime_session_id) = match transport {
        ConversationStartTransport::Websocket => (
            "/v1/live?model=gpt-live-1-kodex",
    ...
    kodex.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(kodex, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
    kodex.shutdown_and_wait().await?;
    test.kodex.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}

#[test_case(None, "gpt-live-1-kodex", None, None; "default model without source")]
#[test_case(Some("session-override-model"), "session-override-model", None, None; "explicit model without source")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::User), Some("user".to_string()); "user source")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::Feature("voice_chat".to_string())), Some("voice_chat".to_string()); "voice chat source")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::Feature("realtime_voice".to_string())), Some("realtime_voice".to_string()); "realtime voice source")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::GuardianReview), Some("guardian_review".to_string()); "child shares parent session")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::Feature("x".repeat(/*n*/ 256))), Some("x".repeat(/*n*/ 256)); "source at byte limit")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::Feature("x".repeat(/*n*/ 257))), None; "oversized source omitted")]
#[test_case(None, "gpt-live-1-kodex", Some(ThreadSource::Feature("é\r\n\"".to_string())), Some("é\r\n\"".to_string()); "source escaped for headers")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_chatgpt_sends_kodex_headers_to_backend(
    model: Option<&str>,
    expected_model: &str,
    thread_source: Option<ThreadSource>,
    ...
    Mock::given(method("POST"))
        .and(path_regex(".*/backend-api/kodex/realtime/calls$"))
        .and(capture.clone())
        .respond_with(
    ...
    let kodex = &conversation.thread;
    assert_eq!(kodex.config_snapshot().await.thread_source, thread_source);
    ...
    let created = wait_for_event_match(kodex, |msg| match msg {
    ...
    assert_eq!(
        ...
        (
            "/backend-api/kodex/realtime/calls",
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.kodex, |msg| match msg {
        EventMsg::RealtimeConversationSdp(created) => Some(Ok(created.clone())),
        EventMsg::Error(err) => Some(Err(err.clone())),
        _ => None,
    })
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=kodex-realtime-call-boundary")
    );
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.kodex, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.kodex, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        ...
        test.kodex.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.kodex, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    let mut builder = test_kodex().with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing());
    let test = builder.build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
    ...
    test.kodex.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.kodex, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn assert_transport_close_tail_flush(
    ...
    let mut builder = test_kodex().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.kodex_error_info, Some(KodexErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.kodex, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.kodex
        ...
        .await?;
    let _ = wait_for_event_match(&test.kodex, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_kodex().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(KodexAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
        )
        .await?;
    let kodex = resumed_thread.thread;
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("kodex-rs/core"))?;
    ...
    assert!(startup_context.contains("kodex-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime(ephemeral: bool) -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.kodex, |event| match event {
        ...
        assert_eq!(
            timeline.items.iter().filter_map(|entry| match entry {
                ...
                ThreadTimelineEntry::Item { item, .. }
                    if matches!(item.as_ref(), kodex_app_server_protocol::ThreadItem::UserMessage { .. }) => Some("typed input"),
                _ => None,
            }).collect::<Vec<_>>(),
...
async fn conversation_relays_only_public_reasoning_as_quiet_status(enabled: bool) -> Result<()> {
    ...
    let mut builder = test_kodex().with_config({
        ...
        move |config| {
            ...
            config.model_reasoning_summary =
                Some(kodex_protocol::config_types::ReasoningSummary::None);
        }
    });
    ...
    let _ = item_tx.send(());
    wait_for_event(&test.kodex, |event| matches!(event,
        EventMsg::ItemCompleted(item) if matches!(&item.item, kodex_protocol::items::TurnItem::Reasoning(reasoning) if reasoning.id == "reasoning")
    )).await;
    if enabled {
        assert_eq!(
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.kodex
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            kodex_response_item_prefix: None,
            kodex_response_handoff_mode:
                kodex_protocol::protocol::KodexResponseHandoffMode::BemTags,
            backend_reasoning_status: false,
            kodex_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn_and_promotes_its_artifact() -> Result<()> {
    ...
    let response_mock = responses::mount_sse_once(
        ...
        responses::sse(vec![
            responses::ev_response_created("resp-1"),
            responses::ev_assistant_message("msg-1", "::kodex-realtime-inline{}\nShared artifact"),
            responses::ev_completed("resp-1"),
        ]),
    )
    .await;
    ...
    let turn_id = loop {
        let event = test.kodex.next_event().await?;
    ...
    assert_eq!(
        ...
        vec![ThreadRealtimeItemContent::BemItemPromoted {
            ...
            presentation:
                kodex_app_server_protocol::ThreadRealtimeBemItemPresentation::InlineMarkdown,
        }]
    );
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_kodex().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
