...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    core_test_support::wait_for_event(test.kodex.as_ref(), |event| {
        matches!(event, kodex_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn history_injection_skips_skill_discovery_after_initial_context() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_kodex().with_workspace_setup(|cwd, fs| async move {
        write_repo_skill(cwd, fs, "demo", "demo skill", SKILL_BODY).await
    });
    let test = builder.build_with_auto_env(&server).await?;
...
async fn idle_user_turn_includes_skill_instructions_in_the_first_request() -> Result<()> {
    ...
    let mut builder = test_kodex().with_workspace_setup(move |cwd, fs| async move {
