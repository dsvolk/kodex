...
impl GatewayAuthManager {
    ...
    pub fn new(
        ...
        Ok(Self {
            control: GatewayLoginControl::for_host(&kodex_home, http_client_factory),
            state: Arc::new(GatewayAuthState {
                config,
                status_tx: login::status_sender_for_host(&kodex_home, http_client_factory),
                storage: GatewayAuthStorage::new(kodex_home.clone(), keyring),
                kodex_home,
                http_client,
    ...
    async fn refresh(
        ...
        let credential_lock = Arc::new(storage::lock_credentials(&self.state.kodex_home).await?);
    ...
    fn credential_id(&self) -> String {
        ...
        digest.update(self.state.kodex_home.to_string_lossy().as_bytes());
        digest.update([0]);
        for value in [
