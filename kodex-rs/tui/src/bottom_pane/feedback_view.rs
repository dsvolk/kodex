...
use kodex_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use kodex_feedback::FeedbackDiagnostics;
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} You can share this in #kodex-feedback:")
        }
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "  Sentry URL: ".into(),
                    format!("https://go/kodex-feedback/{thread_id}")
...
fn slack_feedback_url(_thread_id: &str) -> String {
    KODEX_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec![
            "  • ".into(),
            format!("{KODEX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
        Line::from(vec![
            "  • ".into(),
            format!("{KODEX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Kodex session logs and diagnostics with the team for troubleshooting"
                        .to_string(),
                ),
...
mod tests {
    ...
    use kodex_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/kodex-feedback-internal".to_string();
