...
fn map_api_error_preserves_bio_policy() {
    ...
    assert_eq!(err.to_kodex_protocol_error(), KodexErrorInfo::BioPolicy);
...
fn map_api_error_maps_http_and_wrapped_websocket_bio_policy() {
    for wrapped in [false, true] {
        for message in [
            ...
            let KodexErrorDetails::BioPolicy { message } = err.details() else {
                panic!("expected KodexErrorDetails::BioPolicy, got {err:?}");
            };
