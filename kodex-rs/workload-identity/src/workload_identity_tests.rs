...
fn success(access_token: &str, expires_in: u64) -> ResponseTemplate {
    ResponseTemplate::new(200).set_body_json(serde_json::json!({
        ...
        "scope": "openid profile email chatgpt.workspace.feature.allow-kodex-local-access.access",
...
async fn managed_exchange_denies_before_reading_assertion_or_sending() {
    ...
    assert!(matches!(
        ...
        Err(WorkloadIdentityError::Policy(
            kodex_http_client::NetworkPolicyDenied::Unavailable
        ))
    ));
    ...
    assert!(matches!(
        exchange.resolve().await,
        Err(WorkloadIdentityError::Policy(
            kodex_http_client::NetworkPolicyDenied::Destination
        ))
    ));
    assert!(server.received_requests().await.unwrap().is_empty());
}
