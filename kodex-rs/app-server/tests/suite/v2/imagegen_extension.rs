...
use kodex_app_server_protocol::ImageGenerationFailure;
...
async fn standalone_image_generation_returns_saved_path_hint_to_model(
    ...
    Mock::given(method("POST"))
        .and(path("/api/kodex/images/generations"))
        .respond_with(
            ResponseTemplate::new(200)
                .insert_header("x-kodex-imagegen-request-id", "req-imagegen-123")
                .set_body_json(json!({
                    "created": 1,
                    "background": "opaque",
                    "data": [
    ...
    create_config_toml(kodex_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
    ...
    let image_request = server
        ...
        .into_iter()
        .find(|request| request.url.path() == "/api/kodex/images/generations")
...
async fn invalid_transparent_background_does_not_dispatch(
    ...
    assert!(
        server
            ...
            .context("failed to fetch received requests")?
            .iter()
            .all(|request| !request.url.path().starts_with("/api/kodex/images/"))
    );
    Ok(())
}
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(500)
                .insert_header("x-kodex-imagegen-request-id", "req-imagegen-failed-123")
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(429)
                .insert_header("x-kodex-active-limit", "image_gen")
...
async fn standalone_image_edit_uses_attached_model_visible_image(
    ...
    let (edit_request, _) = run_image_edit_test(|kodex_home| {
        let image_path = kodex_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let kodex_home = TempDir::new()?;
    let (arguments, input) = input(kodex_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
    ...
    let image_request = requests
        .iter()
        .find(|request| request.url.path() == "/api/kodex/images/edits")
...
async fn mount_image_edit_response(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/api/kodex/images/edits"))
...
fn create_config_toml(
    ...
    let mut config = MockResponsesConfig::new(server_uri)
        ...
        .with_provider_base_url(&format!("{server_uri}/api/kodex"))
