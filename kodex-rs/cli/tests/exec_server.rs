...
use kodex_exec_server::NoiseRendezvousConnectArgs;
...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let kodex_home = TempDir::new()?;
    std::fs::write(kodex_home.path().join("config.toml"), "not valid toml = [")?;
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = kodex_command(kodex_home.path())?;
    cmd.env(
        kodex_exec_server::KODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    command
        ...
        .env("KODEX_API_KEY", "test-api-key")
        .env(
            kodex_exec_server::KODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined KODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${KODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            shell_snapshot: None,
            env_policy: Some(kodex_exec_server::ExecEnvPolicy {
                inherit: kodex_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let kodex_bin = kodex_utils_cargo_bin::cargo_bin("kodex")?;
    let kodex_home = kodex_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(kodex_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let kodex_home = TempDir::new()?;
    let mut child = std::process::Command::new(kodex_utils_cargo_bin::cargo_bin("kodex")?)
        .env("KODEX_HOME", kodex_home.path())
