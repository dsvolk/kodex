...
use kodex_sandboxing::SandboxDirectSpawnTransformRequest;
...
use kodex_utils_pty::Child;
use kodex_utils_pty::ChildStdin;
use kodex_utils_pty::Command;
#[cfg(target_os = "macos")]
use kodex_utils_pty::DescriptorPolicy;
use kodex_utils_pty::SpawnFallback;
#[cfg(any(windows, test))]
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.kodex_self_exe;
        let sandbox_manager = SandboxManager::for_file_system_helpers();
        #[cfg(target_os = "macos")]
        let sandbox_manager = sandbox_manager.with_allowed_symlinked_kodex_home(
            self.runtime_paths.allowed_symlinked_kodex_home.clone(),
        );
        ...
        let command = SandboxCommand {
            ...
            args: vec![KODEX_FS_HELPER_ARG1.to_string()],
            ...
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    sandbox_exe: if cfg!(windows) {
                        Some(self.runtime_paths.kodex_self_exe.as_path())
                    } else {
                        self.runtime_paths.kodex_linux_sandbox_exe.as_deref()
                    },
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.kodex_self_exe.clone()];
    if let Some(path) = &runtime_paths.kodex_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| !kodex_protocol::shell_environment::is_non_inheritable_env_var(name));
    command.envs(env);
    command.stdin(stdin);
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let kodex_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(kodex_self_exe, /*kodex_linux_sandbox_exe*/ None)
        ...
        let readable = runtime_paths.kodex_self_exe.clone();
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/kodex".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let kodex_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(kodex_self_exe.clone(), Some(kodex_self_exe))
                .expect("runtime paths");
        ...
        let sandbox_context = {
            ...
            crate::FileSystemSandboxContext {
                windows_sandbox_selection:
                    kodex_file_system::WindowsSandboxSelection::RestrictedToken,
                ..sandbox_context
            }
        };
    ...
    fn sandbox_exec_request_uses_filesystem_root_and_preserves_policy_cwd() {
        ...
        let sandbox_context = crate::FileSystemSandboxContext {
            windows_sandbox_selection: kodex_file_system::WindowsSandboxSelection::RestrictedToken,
            ..sandbox_context
        };
    ...
    fn helper_permissions_include_only_the_helper_executable() {
        ...
        assert!(
            policy.can_read_local_path_with_cwd(
                runtime_paths.kodex_self_exe.as_path(),
                cwd.as_path(),
            )
        );
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let kodex_self_exe = root.path().join("bin").join("kodex");
        let kodex_linux_sandbox_exe = root.path().join("aliases").join("kodex-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(kodex_self_exe, Some(kodex_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let kodex_parent = runtime_paths.kodex_self_exe.parent().expect("kodex parent");
        ...
        assert!(!policy.can_read_local_path_with_cwd(kodex_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        kodex_file_system::FileSystemSandboxContext::from_permission_profile(
