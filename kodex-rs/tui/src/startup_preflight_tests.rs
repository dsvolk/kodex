...
fn startup_delays_composer_for_homes_without_authentication_state() -> std::io::Result<()> {
    ...
    let kodex_home = temporary_directory.path().join("kodex-home");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == kodex_login::KODEX_ACCESS_TOKEN_ENV_VAR).then(|| "  ".into()),
    ));

    std::fs::create_dir_all(kodex_home.join("tmp").join("arg0"))?;
    assert!(should_delay_startup_composer_for_first_login(
        &kodex_home,
        Ok(system_config_path.clone()),
        || Ok(false),
    ...
    let helper_directory = kodex_home
        .join("tmp")
        .join("arg0")
        .join("kodex-arg0-session");
    ...
    assert!(should_delay_startup_composer_for_first_login(
        ...
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| (name == "KODEX_HOME").then(|| "/custom/home".into()),
    ));
    assert!(!should_delay_startup_composer_for_first_login(
        &kodex_home,
        Ok(system_config_path.clone()),
        || Ok(false),
        |name| { (name == kodex_login::KODEX_ACCESS_TOKEN_ENV_VAR).then(|| "access-token".into()) },
    ));
    for disabled_credential in [
        kodex_login::OPENAI_API_KEY_ENV_VAR,
        kodex_login::KODEX_API_KEY_ENV_VAR,
    ] {
        assert!(should_delay_startup_composer_for_first_login(
            &kodex_home,
            Ok(system_config_path.clone()),
    ...
    for existing_state in ["history.jsonl", "log", "sessions"] {
        let state_path = kodex_home.join(existing_state);
    ...
    assert!(!should_delay_startup_composer_for_first_login(
        ...
        |name| match name {
            "KODEX_HOME" => Some("/custom/home".into()),
            kodex_login::KODEX_ACCESS_TOKEN_ENV_VAR => Some("access-token".into()),
            _ => None,
        },
    ));
    ...
    let daemon_directory = kodex_home.join("app-server-control");
    ...
    let additional_temporary_state = kodex_home.join("tmp").join("other");
...
fn startup_keeps_composer_when_home_state_cannot_be_confirmed() -> std::io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            &blocking_parent.join("kodex-home"),
            Ok(system_config_path),
...
fn startup_keeps_composer_for_workload_identity_markers() -> std::io::Result<()> {
    ...
    for marker in [
        kodex_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR,
        kodex_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR,
    ] {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            ...
            || panic!("workload identity should not probe managed configuration"),
            |name| {
                (name == kodex_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR)
