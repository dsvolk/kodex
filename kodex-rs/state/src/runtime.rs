...
impl StateRuntime {
    ...
    async fn init_inner(
        ...
        if has_memories_v2
            && let Err(err) = runtime
                .memories_for_version(kodex_protocol::MemoryVersion::V2)
                .await
        {
            runtime.close().await;
            return Err(err);
    ...
    pub async fn clear_memory_data_in_sqlite_home(sqlite: &SqliteConfig) -> anyhow::Result<bool> {
        ...
        for version in [
            kodex_protocol::MemoryVersion::V1,
            kodex_protocol::MemoryVersion::V2,
        ] {
            let path = match version {
                kodex_protocol::MemoryVersion::V1 => sqlite.memories_db_path(),
                kodex_protocol::MemoryVersion::V2 => sqlite.memories_v2_db_path(),
            };
            ...
            let pool = match version {
                ...
                kodex_protocol::MemoryVersion::V2 => sqlite.open_memories_v2_db().await?,
            };
...
mod tests {
    ...
    async fn sqlite_integrity_check_can_be_interrupted_and_retried() {
        ...
        let sqlite = crate::SqliteConfig::new_for_testing(kodex_home.as_path().abs());
