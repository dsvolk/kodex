...
async fn kodex_home_symlink_opt_out_uses_loaded_user_config() -> anyhow::Result<()> {
    ...
    for (home_env, root_alias, enabled, ignore_user_config) in [
        ...
        ("home-link/../.kodex", Some(".kodex"), false, false),
        ("home-link/../.kodex", None, true, false),
        ("user-home", Some("user-home"), true, true),
    ] {
        let test = test_kodex_exec();
        let home = test.home_path().join(".kodex");
        let project_home = test.cwd_path().join(".kodex");
        ...
        command
            ...
            .args(["-c", "allow_symlinked_kodex_home=true"])
