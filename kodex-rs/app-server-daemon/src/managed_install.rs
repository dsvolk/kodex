...
pub(crate) fn package_root(kodex_home: &Path) -> PathBuf {
    let dedicated = kodex_home.join("packages/app-server-daemon");
    if !matches!(dedicated.join("current").symlink_metadata(),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound)
    {
        return dedicated;
    }
    let state = kodex_home.join("app-server-daemon");
    for (package, artifacts) in [
        ...
        if artifacts.iter().any(|name| {
            ...
            return kodex_home.join("packages").join(package);
        }
    }
    dedicated
}
...
pub(crate) fn managed_kodex_bin(kodex_home: &Path) -> PathBuf {
    let root = package_root(kodex_home);
    let current = root.join("current");
    let packaged = current.join("bin").join(managed_kodex_file_name());
    let legacy = current.join(managed_kodex_file_name());
    if packaged.is_file()
...
pub(crate) fn is_stable_standalone_release(kodex_home: &Path, kodex_bin: &Path) -> bool {
    let standalone = package_root(kodex_home);
    ...
    components.len() == 3
        ...
        && std::fs::canonicalize(kodex_bin).is_ok_and(|bin| bin.starts_with(&release))
}

/// Older managed binaries can serve app-server requests without owning an updater.
pub(crate) async fn supports_daemon_update_loop(kodex_bin: &Path) -> bool {
    supports_daemon_command(kodex_bin, &["pid-update-loop", "--help"]).await
}

/// Probe an internal daemon command without running a long-lived process.
pub(crate) async fn supports_daemon_command(kodex_bin: &Path, args: &[&str]) -> bool {
    let mut command = Command::new(kodex_bin);
    #[cfg(windows)]
...
pub(crate) async fn resolved_managed_kodex_bin(kodex_bin: &Path) -> Result<PathBuf> {
    fs::canonicalize(kodex_bin).await.with_context(|| {
        format!(
            "failed to resolve managed Kodex binary {}",
            kodex_bin.display()
        )
    })
}

pub(crate) async fn managed_kodex_version(kodex_bin: &Path) -> Result<String> {
    let mut command = Command::new(kodex_bin);
    #[cfg(windows)]
    command.creation_flags(windows_sys::Win32::System::Threading::CREATE_NO_WINDOW);
    let output = command
        ...
        .with_context(|| {
            format!(
                "failed to invoke managed Kodex binary {}",
                kodex_bin.display()
            )
        })?;
    if !output.status.success() {
        return Err(anyhow!(
            "managed Kodex binary {} exited with status {}",
            kodex_bin.display(),
            output.status
        ));
    }
    ...
    let stdout = String::from_utf8(output.stdout).with_context(|| {
        format!(
            "managed Kodex version was not utf-8: {}",
            kodex_bin.display()
        )
    })?;
    parse_kodex_version(&stdout)
}
...
fn managed_kodex_file_name() -> &'static str {
    if cfg!(windows) { "kodex.exe" } else { "kodex" }
}

fn parse_kodex_version(output: &str) -> Result<String> {
    let version = output
        .split_whitespace()
        .nth(1)
        .filter(|version| !version.is_empty())
        .ok_or_else(|| anyhow!("managed Kodex version output was malformed"))?;
