...
fn is_workspace_crate_name(name: &str) -> bool {
    name.starts_with("kodex_") || matches!(name, "app_test_support" | "core_test_support")
}
...
fn workspace_crate_filter_accepts_first_party_names_only() {
    assert!(is_workspace_crate_name("kodex_core"));
    assert!(is_workspace_crate_name("kodex_tui"));
