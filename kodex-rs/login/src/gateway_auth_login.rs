...
impl GatewayLoginControl {
    ...
    pub(super) fn for_host(kodex_home: &std::path::Path, factory: &HttpClientFactory) -> Arc<Self> {
        ...
        if let Some((_, _, control)) = controls
            .iter()
            .find(|(home, routes, _)| home == kodex_home && routes == factory)
        {
...
pub(super) fn status_sender_for_host(
    ...
    GatewayLoginControl::for_host(kodex_home, factory)
        .status_tx
        .clone()
}
