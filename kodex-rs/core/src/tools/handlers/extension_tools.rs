...
pub(crate) struct ExtensionToolAdapter(
    Arc<dyn for<'call> kodex_tools::ToolExecutor<ExtensionToolCall<'call>>>,
);

impl ExtensionToolAdapter {
    pub(crate) fn new(
        executor: Arc<dyn for<'call> kodex_tools::ToolExecutor<ExtensionToolCall<'call>>>,
    ) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall<'_> {
    ...
    let kodex_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use kodex_tools::ExtensionTurnItem;
    use kodex_tools::ToolCallSource as ExtensionToolCallSource;
    ...
    impl<'call> kodex_extension_api::ToolExecutor<kodex_tools::ToolCall<'call>>
        for StubExtensionExecutor
    {
        fn tool_name(&self) -> kodex_tools::ToolName {
            kodex_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> kodex_tools::ToolSpec {
            kodex_tools::ToolSpec::Function(kodex_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: kodex_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle<'a>(
            &'a self,
            _call: kodex_tools::ToolCall<'call>,
        ) -> kodex_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
            Box::pin(async {
                Ok(
                    Box::new(kodex_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn kodex_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<kodex_tools::ToolCall<'static>>>>,
        ...
        for CapturingExtensionExecutor
    {
        ...
        fn handle<'a>(
            &'a self,
            call: kodex_tools::ToolCall<'call>,
        ) -> kodex_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: kodex_tools::ToolCall<'_>,
        ) -> Result<Box<dyn kodex_tools::ToolOutput>, kodex_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: kodex_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = kodex_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        kodex_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        kodex_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
