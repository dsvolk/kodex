...
async fn stdio_sigterm_times_out_with_blocked_stderr() -> Result<()> {
    ...
    let mut process = tokio::process::Command::new(cargo_bin("kodex-app-server")?)
        ...
        .env("RUST_LOG", "kodex_app_server_transport=error")
...
async fn stdio_shutdown_times_out_with_blocked_file_write(shutdown: Shutdown) -> Result<()> {
    let kodex_home = TempDir::new()?;
    let path = kodex_home.path().join("blocked-write");
