...
use kodex_state::RuntimeDbBackup;
use kodex_tui::LocalStateDbStartupError;
...
pub(crate) fn is_locked(detail: &str) -> bool {
    kodex_state::sqlite_error_detail_is_lock(detail)
}

pub(crate) fn is_corruption(detail: &str) -> bool {
    kodex_state::sqlite_error_detail_is_corruption(detail)
}
...
pub(crate) fn print_auto_backup_start(startup_error: &LocalStateDbStartupError) {
    eprintln!("Kodex couldn't start because its local database appears to be damaged.");
    eprintln!("Moving the damaged local database aside so Kodex can rebuild it from saved data.");
...
pub(crate) async fn backup_files_for_fresh_start(
    ...
    kodex_state::backup_runtime_db_for_fresh_start(startup_error.database_path()).await
}
...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("Kodex rebuilt its local database.");
    eprintln!(
        "Kodex detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Kodex couldn't start because its local database appears to be damaged.");
    eprintln!("Run `kodex doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Kodex couldn't start because another Kodex process is using its local data.");
    eprintln!("Quit any other copies of Kodex that may still be running, then try again.");
    print_technical_details(startup_error);
}
...
mod tests {
    ...
    async fn backup_backs_up_only_failed_database_file() -> std::io::Result<()> {
        ...
        let sqlite = kodex_state::SqliteConfig::new_for_testing(temp_dir.path().abs());
