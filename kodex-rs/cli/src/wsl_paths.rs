...
pub use kodex_utils_path::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\kodex.zip").as_deref(),
            Some("/mnt/c/Temp/kodex.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/kodex.tgz").as_deref(),
            Some("/mnt/d/Work/kodex.tgz")
        );
        assert!(win_path_to_wsl("/home/user/kodex").is_none());
    }
