...
use kodex_protocol::items::CollabAgentToolCallStatus;
...
impl AnalyticsEventsDestination {
    ...
    fn from_base_url_and_capture_file(base_url: String, capture_file: Option<PathBuf>) -> Self {
        ...
        Self::Http {
            url: format!("{base_url}/kodex/analytics-events/events"),
        }
    }
}
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::KodexCompactionEvent) {
    ...
    pub fn track_goal_event(&self, event: KodexGoalEvent) {
    ...
    pub fn track_turn_kodex_error(&self, fact: TurnKodexErrorFact) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::TurnKodexError(
            Box::new(fact),
        )));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_kodex_backend() {
        return;
    }
    if events.is_empty() {
        return;
