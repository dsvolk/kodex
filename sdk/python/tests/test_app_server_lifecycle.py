...
from openai_kodex import AsyncKodex, Kodex
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncKodex(config=harness.app_server_config()) as kodex:
            ...
            server = kodex.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": kodex.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            active_thread = kodex.thread_start()
            archived_thread = kodex.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            kodex.thread_archive(archived_thread.id)
            active_list = kodex.thread_list(archived=False)
            archived_list = kodex.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncKodex(config=harness.app_server_config()) as kodex:
                ...
                resumed = await kodex.thread_resume(thread.id)
                forked = await kodex.thread_fork(thread.id)
                archive_response = await kodex.thread_archive(thread.id)
                unarchived = await kodex.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            ...
            forked = kodex.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            ...
            archived = kodex.thread_archive(thread.id)
            unarchived = kodex.thread_unarchive(thread.id)
