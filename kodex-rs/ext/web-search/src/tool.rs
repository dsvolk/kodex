...
use kodex_api::SearchRequest;
use kodex_api::SearchSettings;
use kodex_core::X_KODEX_TURN_METADATA_HEADER;
use kodex_core::web_search_action_detail;
...
use kodex_extension_api::parse_tool_input_schema_without_compaction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "kodex.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(
        ...
        let extra_headers = search_request_headers(
            ...
            call.kodex_turn_metadata.as_deref(),
        );
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_KODEX_TURN_METADATA_HEADER, header_value);
    }
