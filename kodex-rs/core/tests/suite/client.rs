...
use kodex_core::X_KODEX_ROUTING_HINT_HEADER;
use kodex_core::resolve_installation_id;
...
use kodex_protocol::models::LocalShellAction;
use kodex_protocol::models::LocalShellExecAction;
use kodex_protocol::models::LocalShellStatus;
use kodex_protocol::models::MessagePhase;
use kodex_protocol::models::ReasoningItemContent;
...
async fn responses_request_uses_implicit_default_tier_for_bedrock() -> anyhow::Result<()> {
    ...
    for provider_id in [
        ...
        let response_mock = mount_sse_once(&server, sse(vec![ev_completed("done")])).await;
        let test = test_kodex()
            .with_auth(KodexAuth::BedrockApiKey(BedrockApiKeyAuth {
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> kodex_core::KodexResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestKodexResponsesRequestKind::Turn,
    )
}
...
fn assert_kodex_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-kodex-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-kodex-turn-metadata"]
        .as_str()
        .expect("missing x-kodex-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-kodex-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-kodex-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_kodex().build(&server).await.unwrap();
...
async fn non_openai_responses_requests_include_item_ids_without_passthrough_metadata() {
    ...
    kodex
        ...
        .await
        .unwrap();
    wait_for_event(&kodex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.kodex.submit(Op::Shutdown).await?;
    wait_for_event(&initial.kodex, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let kodex_home = Arc::new(TempDir::new()?);
    let mut builder = test_kodex();
    let first = builder
        .resume(&server, Arc::clone(&kodex_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.kodex.submit(Op::Shutdown).await?;
    wait_for_event(&first.kodex, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, kodex_home, session_path).await?;
...
/// Writes an `auth.json` into the provided `kodex_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        kodex_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: kodex_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = kodex_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = kodex_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![kodex_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = kodex_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Kodex to resume from our file
    let kodex_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_kodex()
        .with_home(kodex_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, kodex_home, session_path.clone())
        .await
        .expect("resume conversation");
    let kodex = test.kodex.clone();
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let kodex_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_kodex().with_model("gpt-5.4");
    let test = builder
        .resume(&server, kodex_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_kodex().with_auth(KodexAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    assert_eq!(request.header(X_KODEX_ROUTING_HINT_HEADER), None);
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.kodex_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&kodex_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = kodex_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        kodex_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(KodexAuth::from_api_key(
            "unused-api-key",
        ))),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &kodex_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_kodex()
        .with_auth(KodexAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/kodex", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_kodex()
        .with_auth(create_dummy_kodex_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/kodex/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
    ...
    assert_eq!(
        request.header(X_KODEX_ROUTING_HINT_HEADER),
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = KodexAuth::from_auth_storage(
        kodex_home.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
        AuthKeyringBackendKind::default(),
        &kodex_login::test_support::transport_default_auth_route_config(),
    )
    .await
    .expect("Failed to load KodexAuth")
    .expect("No KodexAuth found in kodex_home");
    let auth_manager = kodex_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.kodex_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        kodex_core::build_models_manager(&config, auth_manager),
        kodex_core::KodexAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(kodex_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(kodex_core::test_support::EmptyUserInstructionsProvider),
        /*analytics_events_client*/ None,
        kodex_core::passthrough_image_store(),
    ...
    let NewThread { thread: kodex, .. } = thread_manager
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__kodex_apps"),
        "did not expect kodex_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect kodex_apps resource calls: {resource_methods:?}"
    );
}
...
async fn powershell_shell_version_is_model_visible_only_when_enabled() -> anyhow::Result<()> {
    ...
    let shell_path = kodex_shell_command::powershell::try_find_powershell_executable_blocking()
    ...
    for enabled in [false, true] {
        ...
        let user_shell = kodex_shell_command::shell_detect::DetectedShell {
            shell_type: kodex_shell_command::shell_detect::ShellType::PowerShell,
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestKodex { kodex, .. } = test_kodex()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestKodex { kodex, .. } = test_kodex().with_model("gpt-5.5").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestKodex { kodex, config, .. } = test_kodex().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        kodex_core::test_support::auth_manager_from_auth(KodexAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-kodex-primary-used-percent", "12.5")
        .insert_header("x-kodex-secondary-used-percent", "40.0")
        .insert_header("x-kodex-primary-window-minutes", "10")
        .insert_header("x-kodex-secondary-window-minutes", "60")
        .insert_header("x-kodex-primary-reset-at", "1704069000")
        .insert_header("x-kodex-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_kodex()
        .with_auth(KodexAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let kodex = builder
    ...
    wait_for_event(&kodex, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-kodex-primary-used-percent", "100.0")
        .insert_header("x-kodex-secondary-used-percent", "87.5")
        .insert_header("x-kodex-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-kodex-primary-window-minutes", "15")
        .insert_header("x-kodex-secondary-window-minutes", "60")
        .insert_header("x-kodex-credits-has-credits", "true")
        .insert_header("x-kodex-credits-unlimited", "false")
        .insert_header("x-kodex-credits-balance", "")
        .insert_header(
    ...
    let mut builder = test_kodex();
    let kodex_fixture = builder.build(&server).await?;
    let kodex = kodex_fixture.kodex.clone();
    ...
    let token_event = wait_for_event(&kodex, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&kodex, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&kodex, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&kodex, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = KodexErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn env_var_overrides_loaded_auth() {
    ...
    assert_eq!(request.headers.get(X_KODEX_ROUTING_HINT_HEADER), None);
}

fn create_dummy_kodex_auth() -> KodexAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Kodex sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
