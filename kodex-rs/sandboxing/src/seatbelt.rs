...
use kodex_network_proxy::PROXY_URL_ENV_KEYS;
use kodex_network_proxy::has_proxy_url_env_vars;
use kodex_network_proxy::proxy_url_env_value;
...
fn normalize_writable_root_for_sandbox(
    ...
    let allow_symlinks = allowed_symlinked_kodex_home.is_some_and(|home| {
    ...
    if !allow_symlinks && let Some(symlink) = nested_symlink_component(root.as_path()) {
        return Err(SeatbeltPreparationError::FileSystem(format!(
            "writable root {} contains symlink component {}; symlinked writable roots are not supported.\n\
             If this writable root is at or beneath KODEX_HOME and you trust its symlink targets, \
             set `allow_symlinked_kodex_home = true` at the top level of `$KODEX_HOME/config.toml` \
             (normally `~/.kodex/config.toml`) on the execution host, then restart Kodex or its executor. \
             This opt-out trusts targets outside KODEX_HOME and targets changed between commands. \
    ...
    let normalized = if allow_symlinks {
        ...
        // outside KODEX_HOME and links changed by an earlier command.
    ...
    for (index, access_root) in roots.into_iter().enumerate() {
            ...
            access_root.excluded_subpaths.into_iter().enumerate()
        {
            ...
            for (excluded_param, excluded_subpath) in excluded_subpaths {
                ...
                // protected directory itself, such as `mkdir .kodex`.
...
pub(crate) fn create_seatbelt_command_args_with_profile(
    ...
    let allowed_symlinked_kodex_home = allowed_symlinked_kodex_home
        .cloned()
        ...
        if file_system_sandbox_policy.has_full_disk_write_access() {
            ...
            build_seatbelt_access_policy(
                ...
                writable_roots
                    ...
                    .collect(),
                allowed_symlinked_kodex_home.as_ref(),
            )?
        };
    ...
    if !file_system_sandbox_policy.has_full_disk_write_access() {
        let directory = kodex_uds::shared_daemon_socket_directory()
