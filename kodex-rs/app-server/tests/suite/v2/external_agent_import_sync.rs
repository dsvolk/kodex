...
const NATIVE_USER: &str = "native Kodex message";
const NATIVE_ASSISTANT: &str = "native Kodex answer";
...
impl ImportFixture {
    async fn new() -> Result<Self> {
        ...
        let app_server = start_app_server(kodex_home.path()).await?;
        Ok(Self {
    ...
    async fn restart(&mut self) -> Result<()> {
        ...
        self.app_server = start_app_server(self._kodex_home.path()).await?;
        Ok(())
    }
...
async fn start_app_server(kodex_home: &Path) -> Result<TestAppServer> {
    let home = kodex_home.display().to_string();
...
async fn cold_diverged_target_is_deferred_without_checkpoint() -> Result<()> {
    ...
    let mut fixture = ImportFixture::new().await?;
    MockResponsesConfig::new(&model.uri()).write(fixture._kodex_home.path())?;
