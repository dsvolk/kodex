...
use kodex_network_proxy::NetworkPolicyAuditObserver;
use kodex_network_proxy::NetworkPolicyDecider;
use kodex_network_proxy::NetworkProxy;
use kodex_network_proxy::NetworkProxyHandle;
use kodex_network_proxy::NetworkProxyState;
use kodex_network_proxy::RemoteNetworkProxyLaunchConfig;
use kodex_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use kodex_network_proxy::strip_managed_proxy_env;
...
use kodex_sandboxing::WindowsSandboxProxySettingsMode;
use kodex_sandboxing::WindowsSandboxSpawnRequest;
...
pub(crate) async fn prepare_exec_request_with_telemetry(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.kodex_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.kodex_self_exe) {
            sandbox_helper_paths.push(runtime_paths.kodex_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.kodex_linux_sandbox_exe.iter().cloned());
    ...
    let sandbox_manager = sandbox_manager
        .with_allowed_symlinked_kodex_home(runtime_paths.allowed_symlinked_kodex_home.clone());
    ...
    let (program, args) = params.arg0.as_ref().map_or_else(
        ...
        |arg0| {
            ...
            helper_args.push(KODEX_ARG0_EXEC_HELPER_ARG1.to_string());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            sandbox_exe: if cfg!(windows) {
                Some(runtime_paths.kodex_self_exe.as_path())
            } else {
                runtime_paths.kodex_linux_sandbox_exe.as_deref()
            },
...
> {
    ...
    state.set_process_log_metadata(kodex_network_proxy::NetworkProxyProcessLogMetadata {
        thread_id: telemetry.thread_id.clone(),
        tool_call_id: telemetry.tool_call_id.clone(),
        executor_identity: telemetry
            .executor_registration
            .as_ref()
            .map(|registration| kodex_network_proxy::ExecutorLogIdentity {
