...
//! In tmux, also forward to the attached terminal so clients attached after Kodex
...
fn copy_to_clipboard_with(
    ...
    // Persistent tmux sessions may gain remote clients after Kodex starts, so still
...
fn wsl_clipboard_copy(text: &str) -> Result<(), String> {
    let executable = kodex_utils_path::system_executable("powershell.exe")
        .ok_or_else(|| "PowerShell is unavailable in the system PATH".to_string())?;
    let path = kodex_utils_path::system_path()
