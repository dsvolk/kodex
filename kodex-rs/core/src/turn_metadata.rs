...
use kodex_git_utils::get_git_remote_urls_assume_git_repo;
use kodex_git_utils::get_has_changes_in_repo;
use kodex_git_utils::get_head_commit_hash;
...
const KODEX_VERSION_KEY: &str = "kodex_version";
...
impl<'a> ExecutionMetadata<'a> {
    ...
    pub(crate) fn apply_to(&self, metadata: &mut KodexResponsesMetadata) {
...
pub async fn detached_memory_responses_metadata(
    ...
    let mut metadata = KodexResponsesMetadata {
        turn_id: Some(turn_id.clone()),
        root_turn_id: Some(turn_id),
        request_kind: Some(KodexResponsesRequestKind::Memory),
        ...
        .await,
        ..KodexResponsesMetadata::new(installation_id, session_id, thread_id, window_id)
    };
...
impl kodex_analytics::TurnAnalyticsMetadata for TurnMetadataState {
...
impl TurnMetadataState {
    ...
    fn responses_metadata_template(&self) -> KodexResponsesMetadata {
    ...
    fn mcp_metadata_template(&self) -> KodexResponsesMetadata {
        let mut extra = self
            .responsesapi_client_metadata
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
