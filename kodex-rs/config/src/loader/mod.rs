...
use kodex_network_proxy::is_credential_broker_provider_env_key;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/kodex/config.toml";
...
///
/// - system    `/etc/kodex/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Kodex\requirements.toml` (Windows)
/// - cloud:    enterprise-managed cloud config bundle requirements
/// - legacy:   `/etc/kodex/managed_config.toml` (Unix) reinterpreted as
///   requirements.toml
/// - admin:    managed preferences (*)
///
/// For backwards compatibility, Unix continues to load
/// `/etc/kodex/managed_config.toml` and map it to `requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - package:  optional default configuration supplied with the Kodex package
/// - admin:    managed preferences (*)
/// - system    `/etc/kodex/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Kodex\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${KODEX_HOME}/config.toml`
/// - profile   `${KODEX_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.kodex/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.kodex/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/kodex/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let packaged_defaults_layer = if let Some(file) = &overrides.packaged_defaults_path {
        ...
        let config = toml::from_str(raw_toml).map_err(|error| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid embedded packaged defaults; this is a Kodex build error: {error}"),
            )
        })?;
        ConfigLayerEntry::new_with_raw_toml(
            ...
            config,
            raw_toml.to_owned(),
            AbsolutePathBuf::from_absolute_path(kodex_home)?,
        )
    };
    ...
    {
        let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(kodex_home)?;
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(kodex_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, kodex_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/kodex/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(kodex_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/kodex/requirements.toml"))
}
...
pub fn has_local_managed_configuration(kodex_home: &Path) -> io::Result<bool> {
...
fn has_local_managed_configuration_with_system_requirements_path(
    ...
    #[cfg(not(windows))]
    if layer_io::managed_config_default_path(kodex_home).try_exists()? {
        return Ok(true);
    }
...
#[cfg(windows)]
fn windows_kodex_system_dir() -> PathBuf {
...
fn requirements_layers_from_legacy_scheme(
    ...
    let kodex_home = AbsolutePathBuf::from_absolute_path(kodex_home)?;
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Kodex to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".kodex"))
    }
}

fn project_layer_entry(
    dot_kodex_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_kodex_folder: dot_kodex_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_kodex_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for layer in discovered.layers {
        let config =
            resolve_relative_paths_in_config_toml(layer.config, layer.dot_kodex_folder.as_path())?;
...
async fn discover_project_layers(
    ...
    let kodex_home_abs = AbsolutePathBuf::from_absolute_path(kodex_home)?;
    let kodex_home_normalized =
        normalize_path(kodex_home_abs.as_path()).unwrap_or_else(|_| kodex_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_kodex_abs = dir.join(".kodex");
        let dot_kodex_uri = PathUri::from_abs_path(&dot_kodex_abs);
        if !fs
            .get_metadata(&dot_kodex_uri, Default::default(), /*sandbox*/ None)
            .await
        ...
        let dot_kodex_normalized =
            normalize_path(dot_kodex_abs.as_path()).unwrap_or_else(|_| dot_kodex_abs.to_path_buf());
        if dot_kodex_abs == kodex_home_abs || dot_kodex_normalized == kodex_home_normalized {
            continue;
        }
        let config_file = dot_kodex_abs.join(CONFIG_TOML_FILE);
                ...
                let config: TomlValue = match toml::from_str(&contents) {
                    ...
                    Err(e) => {
                        ...
                        layers.push(DiscoveredProjectLayer {
                            dot_kodex_folder: dot_kodex_abs,
...
/// from a file like `/etc/kodex/managed_config.toml` that has the same
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Kodex").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Kodex").join("config.toml"))
        );
    }
}
