...
impl App {
    pub(super) fn open_daemon_menu(&mut self) {
        ...
        let unavailable = if matches!(self.app_server_target, AppServerTarget::Remote { .. }) {
            ...
            Some("Run the Kodex CLI to manage the daemon from this menu.")
        } else {
            None
        };
        ...
        let has_package = self.daemon_cli_executable.as_ref().is_some_and(|path| {
            kodex_install_context::InstallContext::from_exe(
                cfg!(target_os = "macos"),
                Some(path.as_path()),
    ...
    pub(super) fn confirm_daemon_update(&mut self, source: DaemonUpdateSource) {
        ...
        let mut explanation = match source {
            ...
            DaemonUpdateSource::ThisCli => {
                let version = kodex_install_context::InstallContext::current()
                    .package_manifest()
                    .map_or_else(|| KODEX_CLI_VERSION.to_string(), |manifest| manifest.version.to_string());
        ...
        explanation.push_str("\nThis may restart the daemon and interrupt active or queued work.\nKodex exits to update in this terminal. Relaunch it afterward.");
        let mut header = vec![Line::from("Update daemon and exit Kodex?".bold())];
