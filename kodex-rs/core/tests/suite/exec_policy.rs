...
async fn submit_user_turn(
    test: &core_test_support::test_kodex::TestKodex,
    prompt: &str,
...
async fn git_status_requires_approval_under_unless_trusted() -> Result<()> {
    ...
    let mut builder = test_kodex().with_model("gpt-5.2").with_config(|config| {
        config
            .features
...
"#;
    ...
    let server = start_mock_server().await;
    let mut migrated_builder = test_kodex().with_config(|config| {
        let policy_path = config.kodex_home.join("rules/default.rules");
    ...
    let migrated_policy_path = migrated.kodex_home_path().join("rules/default.rules");
    assert_eq!(fs::read_to_string(&migrated_policy_path)?, MIGRATED_POLICY);
    assert_eq!(
        fs::read_to_string(migrated.kodex_home_path().join(MIGRATION_MARKER_FILENAME))?,
        "v1\n"
    );

    let mut ignored_builder = test_kodex().with_config(|config| {
        let policy_path = config.kodex_home.join("rules/default.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(policy_path, LEGACY_POLICY).expect("write legacy policy");
    ...
    let ignored_policy_path = ignored.kodex_home_path().join("rules/default.rules");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_kodex().with_config(|config| {
        let policy_path = config.kodex_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
...
"#,
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::UnifiedExec)
                .expect("test config should allow feature update");
            let policy_path = config.kodex_home.join("rules").join("broken.rules");
            fs::create_dir_all(
                policy_path
                    .parent()
                    .expect("policy directory must have a parent"),
...
async fn environment_command_restrictions_override_saved_prefix_approvals() -> Result<()> {
    ...
    let mut builder = test_kodex().with_config(|config| {
        config
            ...
            .enable(Feature::UnifiedExec)
            .expect("test config should allow feature update");
        let policy_path = config.kodex_home.join("rules").join("approved.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(
            policy_path,
    ...
    assert!(matches!(
        error.details(),
        KodexErrorDetails::InvalidRequest(message)
...
async fn environment_command_policy_changes_invalidate_session_approvals() -> Result<()> {
    ...
    let mut builder = test_kodex().with_config(|config| {
        ...
        config.approvals_reviewer = ApprovalsReviewer::User;
        let policy_path = config.kodex_home.join("rules").join("prompt.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(
            policy_path,
