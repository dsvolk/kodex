...
use kodex_login::TokenData;
use kodex_protocol::auth::AuthMode;
use kodex_protocol::openai_models::ModelAccessPrograms;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> kodex_login::ExternalAuthFuture<'_, KodexAuth> {
        Box::pin(async { Ok(KodexAuth::from_api_key("test-external-api-key")) })
    }
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_kodex_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_kodex_backend })
    }
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(kodex_home, endpoint_client, auth_manager)
}

async fn mutate_file_cache_for_test<F>(kodex_home: &Path, f: F)
where
    F: FnOnce(&mut ModelsCacheEntry),
{
    let client_version = crate::client_version_to_whole();
    let cache = FileModelsCache::new(kodex_home.join(MODEL_CACHE_FILE), DEFAULT_MODEL_CACHE_TTL);
    let mut entry = cache
        .load(&client_version)
        .await
...
async fn file_cache_implements_models_cache_contract() {
    let kodex_home = tempdir().expect("temp dir");
    let cache = FileModelsCache::new(
        kodex_home.path().join(MODEL_CACHE_FILE),
...
async fn chatgpt_auth_tokens_for_tests(kodex_home: &Path) -> KodexAuth {
    let auth_dot_json = kodex_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: kodex_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(kodex_home).expect("kodex home should be created");
    std::fs::write(
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = openai_manager_for_tests(kodex_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-kodex/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(kodex_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(kodex_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(kodex_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_refetches_when_cache_stale() {
    ...
    mutate_file_cache_for_test(kodex_home.path(), |cache| {
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_kodex_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_kodex_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(kodex_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        kodex_home.path().to_path_buf(),
        endpoint.clone(),
