...
use kodex_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use kodex_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use kodex_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;
use serde::Deserialize;
use serde_json::Value as JsonValue;

const KODEX_AGENT_PLUGIN_EXTENSION_NAMESPACE: &str = "com.openai";
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    let kodex_extension = object
        .get("extensions")
        .and_then(JsonValue::as_object)
        .and_then(|extensions| extensions.get(KODEX_AGENT_PLUGIN_EXTENSION_NAMESPACE))
        .and_then(|extension| {
            if extension.is_object() {
                ...
                tracing::warn!(
                    path = %manifest_path,
                    namespace = KODEX_AGENT_PLUGIN_EXTENSION_NAMESPACE,
    ...
    let onboarding_skill = resolve_openai_onboarding_skill(plugin_root, kodex_extension);
    let kodex_extension = kodex_extension.map(serde_json::to_string).transpose()?;
    ...
    if let Some(extension) = kodex_extension.as_deref() {
        apply_kodex_agent_plugin_extension(&mut resolved, plugin_root, manifest_path, extension)?;
        resolved.paths.onboarding_skill = onboarding_skill;
    } else if let Some((overlay_path, overlay_contents)) = overlay {
        apply_kodex_agent_plugin_extension(
            &mut resolved,
            plugin_root,
            overlay_path,
            overlay_contents,
...
fn apply_kodex_agent_plugin_extension(
