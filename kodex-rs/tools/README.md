...
`kodex-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `kodex-core`.
...
compatibility-sensitive orchestration in `kodex-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `kodex-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `kodex-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `kodex-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `kodex-core`.
3. Leave compatibility-sensitive adapters in `kodex-core` while downstream
...
`kodex-core`, that is a sign to revisit the extraction boundary before adding
more here.
