...
async fn finish_turn(thread: &KodexThread) {
...
async fn saved_history(test: &TestKodex, thread: &KodexThread) -> Result<Vec<RolloutItem>> {
...
pub(super) async fn migration_scenario() -> Result<Vec<responses::ResponsesRequest>> {
    ...
    test.kodex.append_rollout_items(&history).await?;
    let thread = resume(&test, &test.kodex, history).await?;
    assert_eq!(
