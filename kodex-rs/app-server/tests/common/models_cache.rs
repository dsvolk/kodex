...
/// Write a models_cache.json file to the kodex home directory.
/// This prevents ModelsManager from making network requests to refresh models.
/// The cache will be treated as fresh (within TTL) and used instead of fetching from the network.
/// Uses bundled-catalog-derived presets, converted to ModelInfo format.
pub async fn write_models_cache(kodex_home: &Path) -> std::io::Result<()> {
    ...
    write_models_cache_with_models(kodex_home, models).await
}
...
pub async fn write_models_cache_with_models(
    ...
    let config = kodex_core::config::ConfigBuilder::default()
        .loader_overrides(kodex_config::LoaderOverrides::without_managed_config_for_tests())
        .kodex_home(kodex_home.to_path_buf())
        .build()
        .await?;
    let auth = kodex_login::KodexAuth::from_auth_storage(
        kodex_home,
    ...
    .await?;
    let cache = kodex_model_provider::test_support::models_cache_entry(
