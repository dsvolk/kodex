...
impl RuntimeDbSpec {
    fn path(self, kodex_home: &Path) -> PathBuf {
        kodex_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all Kodex SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable Kodex SQLite database, creating it if necessary.
    ...
    /// Open an existing Kodex SQLite database without creating or modifying it.
