...
const BUNDLED_FRIENDLY_TEMPLATE: &str = "You have a vivid inner life as Kodex:";
...
async fn config_personality_none_sends_no_personality() -> anyhow::Result<()> {
    ...
    let mut builder = test_kodex().with_model("gpt-5.5").with_config(|config| {
...
async fn fixed_friendly_personality_ignores_pragmatic_update(
    ...
    .await;
    let mut builder = test_kodex().with_model(model).with_config(|config| {
