...
try {
    ...
    while (!$finished) {
        ...
        try {
            ...
            $created = $false
            $mutex = [Threading.Mutex]::new($false, 'Global\KodexSandboxSetup', [ref]$created, $security)
            ...
            if ($record.runtime.retiring -ne $plan.record.runtime.retiring -or
                ...
                $record.kodex_home -ne $plan.record.kodex_home -or
            ...
            foreach ($account in $plan.record.runtime.accounts) {
                ...
                [CleanupNative]::DeleteUser(('KodexSandbox' + $account.account), $sid)
                [CleanupNative]::Log($plan.service_name, 4, 3004, "Removed $($account.account) sandbox profile and account")
            }
            $groupSid = $null
            try { if ($plan.group_sid) { $groupSid = ([Security.Principal.NTAccount]::new('KodexSandboxUsers')).Translate([Security.Principal.SecurityIdentifier]).Value } }
            catch [Security.Principal.IdentityNotMappedException] { }
            if ($groupSid) {
                if ($groupSid -cne $plan.group_sid) { throw 'Sandbox group was replaced' }
                $status = [CleanupNative]::NetLocalGroupDel($null, 'KodexSandboxUsers')
            ...
            if ($legacyJson) {
                ...
                if ($owner.user_sid -eq $plan.record.user_sid -and $owner.kodex_home -eq $plan.record.kodex_home) { $legacy.DeleteValue($plan.value, $false) }
            }
