...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    kodex
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&kodex, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    assert_eq!(error.kodex_error_info, None);
...
async fn user_shell_command_honors_default_and_extended_deadlines() -> anyhow::Result<()> {
    for (timeout_ms, expected_ms) in [(None, 3_600_000), (Some(28_800_000), 28_800_000)] {
        ...
        let completed = async {
            loop {
                let event = fixture.kodex.next_event().await.expect("read shell event");
...
fn slow_user_shell_command() -> &'static str {
    match kodex_core::shell::default_user_shell().name() {
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.kodex, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.kodex.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_kodex::test_kodex().with_config(move |config| {
        config
            .features
    ...
    let begin_event = wait_for_event_match(&test.kodex, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.kodex, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.kodex, |ev| match ev {
    ...
    let _ = wait_for_event(&test.kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_kodex::test_kodex().with_config(|config| {
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.kodex, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_kodex::test_kodex();
    let test = builder
        .with_config(|config| {
