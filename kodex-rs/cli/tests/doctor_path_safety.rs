...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        let program = root
            .path()
            .join(format!("kodex{}", std::env::consts::EXE_SUFFIX));
        let source = kodex_utils_cargo_bin::cargo_bin("kodex")?;
...
"#,
        ...
        for name in [
            ...
            std::fs::write(
                ...
                "@echo helper ran>>\"%KODEX_TEST_HELPER_MARKER%\"\r\n@exit /b 0\r\n",
            )?;
        }
    ...
    fn command(&self) -> Result<assert_cmd::Command> {
        ...
        command
            ...
            .env("KODEX_HOME", &self.home)
            .env("HOME", self.root.path())
            .env("PATH", &self.path)
            .env("KODEX_TEST_HELPER_MARKER", &self.marker)
            .env("KODEX_MANAGED_BY_NPM", "1")
            .env(
                "KODEX_MANAGED_PACKAGE_ROOT",
                self.workspace.join("node_modules/@openai/kodex"),
            )
            .env(
...
fn doctor_reports_only_safe_config_error_metadata() -> Result<()> {
    ...
    let project_config_dir = fixture.workspace.join(".kodex");
...
async fn interactive_tmux_startup_does_not_execute_workspace_helpers() -> Result<()> {
    ...
    env.insert(
        "KODEX_TUI_DISABLE_KEYBOARD_ENHANCEMENT".to_string(),
        "0".to_string(),
    );
    env.insert(
...
async fn feedback_with_logs_does_not_execute_path_helpers() -> Result<()> {
    ...
    let package_root = fixture.workspace.join("node_modules/@openai/kodex");
    let package_root = package_root.to_string_lossy();
    let mut app_server = TestAppServer::builder()
        .with_program(&fixture.program)
        .with_kodex_home(&fixture.home)
        ...
        .with_env_overrides(&[
            ...
            ("KODEX_TEST_HELPER_MARKER", Some(marker.as_ref())),
            ("KODEX_MANAGED_BY_NPM", Some("1")),
            ("KODEX_MANAGED_PACKAGE_ROOT", Some(package_root.as_ref())),
            ("ZELLIJ", Some("0")),
            ("ZELLIJ_VERSION", None),
            ("TMUX", None),
            ("TMUX_PANE", None),
