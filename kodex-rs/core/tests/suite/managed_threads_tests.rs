...
use kodex_extension_api::SessionIsolation;
...
async fn dropping_startup_cleans_up_while_required_mcp_is_stalled() -> anyhow::Result<()> {
    ...
    let fixture = test_kodex().build_with_auto_env(&server).await?;
    ...
    servers.insert(
        ...
        serde_json::from_value(json!({
            "url": format!("{}/api/kodex/ps/mcp", http_server.chatgpt_base_url),
    ...
    options.environments = Some(fixture.kodex.environment_selections().await);
    options
    ...
    drop(release);
    fixture.kodex.shutdown_and_wait().await?;
    Ok(())
}
...
async fn owner_cancellation_closes_agent_and_preserves_history_and_parent() -> anyhow::Result<()> {
    ...
    duplicate.environments = Some(fixture.kodex.environment_selections().await);
    duplicate
        .thread_extension_init
        .insert(SessionIsolation::Isolated);
    ...
    assert!(history.items.iter().any(|item| matches!(
        item,
        kodex_history::RolloutItem::EventMsg(EventMsg::TurnComplete(_))
    )));
...
async fn startup_allowlist_controls_advertising_and_execution(
    ...
    let fixture = test_kodex()
        .with_config(|config| {
            ...
            config
                .features
                .disable(kodex_features::Feature::CodeMode)
...
#[test_case(None, Some(kodex_guardian_reviewer::reviewer_tool_policy()), false; "explicit_policy")]
#[test_case(None, None, true; "ordinary_defaults")]
#[test_case(Some(kodex_protocol::protocol::SessionSource::Internal(
    kodex_protocol::protocol::InternalSessionSource::Guardian)), None, false; "internal_guardian_fallback")]
#[test_case(Some(kodex_protocol::protocol::SessionSource::SubAgent(
    kodex_protocol::protocol::SubAgentSource::Other("guardian".to_owned()))), None, false; "legacy_guardian_fallback")]
#[test_case(Some(kodex_protocol::protocol::SessionSource::Internal(
    kodex_protocol::protocol::InternalSessionSource::Guardian)), Some(ToolPolicy::default()), true; "explicit_policy_overrides_identity")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn startup_tool_policy_controls_additional_permission_parameters(
    source: Option<kodex_protocol::protocol::SessionSource>,
    ...
    let fixture = test_kodex()
        .with_config(|config| {
            ...
            config
                .features
                .enable(kodex_features::Feature::ExecPermissionApprovals)
