...
fn chatgpt_auth(user: &str, workspace: &str, token: &str) -> KodexAuth {
    let claims = json!({
        "jti": token,
...
fn auth_owner_generation_distinguishes_refreshes_from_identity_changes() {
    ...
    for (auth, generation, owner_generation) in [
        ...
        (Some(KodexAuth::from_api_key("key-1")), 7, 6),
        (Some(KodexAuth::from_api_key("key-2")), 8, 7),
    ] {
...
async fn captured_credentials_are_revoked_if_account_changes_before_client_construction() {
    ...
    assert!(matches!(
        client
            ...
            .send()
            .await,
        Err(kodex_http_client::RouteAwareRequestError::Policy(
