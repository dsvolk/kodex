...
//!
//! The background scan only visits sessions and archived_sessions under KODEX_HOME.
...
const SIZE_BYTES_METRIC: &str = "kodex.app_server.kodex_home.size_bytes";
...
pub(crate) fn spawn(
    ...
    tokio::task::spawn_blocking(move || {
        let sizes = match directory_sizes(&kodex_home, &shutdown) {
            Ok(sizes) => sizes,
            Err(error) => {
                tracing::debug!(error_kind = ?error.kind(), "Skipping KODEX_HOME size metrics");
                return;
            }
        };
...
fn directory_sizes(kodex_home: &Path, shutdown: &CancellationToken) -> io::Result<DirectorySizes> {
    let sessions = kodex_home.join(SESSIONS_SUBDIR);
    let archived_sessions = kodex_home.join(ARCHIVED_SESSIONS_SUBDIR);
