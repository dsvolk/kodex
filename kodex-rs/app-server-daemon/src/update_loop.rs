...
use crate::managed_install::resolved_managed_kodex_bin;
...
#[cfg(unix)]
const INSTALL_URL: &str = "https://chatgpt.com/kodex/install.sh";
#[cfg(windows)]
const INSTALL_URL: &str = "https://chatgpt.com/kodex/install.ps1";
...
async fn run_with_http(
    ...
    let mut listener = Some(kodex_uds::UnixListener::bind(&socket_path).await?);
    #[cfg(windows)]
    updater.mark_ready().await?;
    let needs_managed_handoff =
        match resolved_managed_kodex_bin(&daemon.current_managed_kodex_bin()?).await {
            Ok(managed_bin) => {
...
async fn adopt_managed_updater(
    ...
    listener: &mut Option<kodex_uds::UnixListener>,
) -> Result<UpdateLoopControl> {
    let managed_bin = resolved_managed_kodex_bin(&daemon.current_managed_kodex_bin()?).await?;
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(err) = replacement.replace_current_updater().await {
            ...
            *listener = Some(kodex_uds::UnixListener::bind(&socket_path).await?);
            return Err(err);
        }
...
async fn update_once(
    ...
    let kodex_home = package_root
        .parent()
        .and_then(Path::parent)
        .context("daemon package root has no Kodex home")?;
    ...
    if package_root.ends_with("app-server-daemon") {
        anyhow::ensure!(
            script
                .windows(b"KODEX_INSTALL_DAEMON_ONLY".len())
                .any(|window| window == b"KODEX_INSTALL_DAEMON_ONLY"),
    ...
    anyhow::ensure!(
        crate::managed_install::package_root(kodex_home) == package_root,
    ...
    let managed_kodex_bin =
        resolved_managed_kodex_bin(&daemon.current_managed_kodex_bin()?).await?;
    let restart_mode = match trigger {
        ...
        UpdateTrigger::Scheduled
            if executable_identity(&managed_kodex_bin).await? != *running_updater_identity =>
        {
            RestartMode::Always
        }
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, &managed_kodex_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_kodex_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_kodex_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Kodex binary {}",
            managed_kodex_bin.display()
        )
    })
}
...
async fn run_installer_script(
    ...
    let mut child = command
        .env(
            ...
            package_root
                .parent()
                .and_then(Path::parent)
                .context("package root has no Kodex home")?,
        )
        .env("KODEX_INSTALL_DEFER_SELECTION", defer_selection)
        .env(
            "KODEX_INSTALL_DAEMON_ONLY",
            if package_root.ends_with("app-server-daemon") {
                "1"
            } else {
                "0"
        ...
        .env("KODEX_RELEASE", "latest")
        .env("KODEX_NON_INTERACTIVE", "1")
        .env("KODEX_INSTALL_IF_LATEST", latest_guard)
        .env("KODEX_INSTALL_IF_CURRENT", current_guard)
        .env("KODEX_UPDATE_FROM_RELEASE", previous_release)
        .kill_on_drop(true)
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Kodex updater")?;
    #[cfg(windows)]
    let _installer_job = crate::backend::windows::installer_job(&child)?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Kodex updater stdin was unavailable")?;
    #[cfg(unix)]
    ...
    write_result
        ...
        .context("failed to pass standalone Kodex updater to shell")?;
    #[cfg(unix)]
    ...
    let status = status.context("failed to wait for standalone Kodex updater")?;

    if status.success() {
        Ok(UpdateLoopControl::Continue)
    } else {
        anyhow::bail!("standalone Kodex updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Kodex updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Kodex updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Kodex updater")?
            .to_vec();
...
impl Signal {
    async fn recv(&mut self) -> Option<()> {
        ...
        let _ = kodex_app_server_transport::daemon_shutdown_signal().await;
        Some(())
    }
}
