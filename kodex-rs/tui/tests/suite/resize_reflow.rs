...
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let kodex = kodex_binary(&repo_root)?;
    let kodex_home = tempdir()?;
    let server = MockServer::start().await;
    let _response_mock = mount_resize_response(&server).await;
    write_config(kodex_home.path(), &repo_root, &server.uri())?;
    write_auth(kodex_home.path())?;

    let session_name = format!("kodex-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("KODEX_HOME={}", kodex_home.path().display()))
    ...
    let kodex_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!kodex_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&kodex_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&kodex_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("kodex-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&kodex_pane)?;
    let restored_row =
...
async fn tmux_scrolled_composer_resize_preserves_visible_draft_text() -> Result<()> {
    ...
    let session_name = format!("kodex-resize-scrolled-composer-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    let baseline =
        wait_for_capture_contains(&kodex_pane, &final_line, Duration::from_secs(/*secs*/ 15))?;
        ...
        [("narrowed", "28", "9", 2), ("restored", "44", "14", 6)]
    {
        ...
        let capture = capture_pane(&kodex_pane)?;
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("kodex-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn kodex_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = kodex_utils_cargo_bin::cargo_bin("kodex") {
        return Ok(path);
    }

    let fallback = repo_root.join("kodex-rs/target/debug/kodex");
    anyhow::ensure!(
        fallback.is_file(),
        "kodex binary is unavailable; run `cargo build -p kodex-cli` first"
    );
    Ok(fallback)
}

fn write_config(kodex_home: &Path, repo_root: &Path, server_url: &str) -> Result<()> {
...
fn write_auth(kodex_home: &Path) -> Result<()> {
    std::fs::write(
