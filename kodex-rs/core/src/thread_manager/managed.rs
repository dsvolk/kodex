...
impl ThreadManager {
    ...
    pub async fn start_thread_until(
        ...
        if matches!(
            ...
            kodex_history::InitialHistory::Resumed(_)
        ) {
        ...
        if options
            ...
            .as_deref()
            != Some(&kodex_extension_api::SessionIsolation::Isolated)
        {
            return Err(KodexErr::InvalidRequest(
        ...
        tokio::spawn(async move {
            ...
            let result = {
                ...
                tokio::select! {
                    biased;
                    _ = &mut until => Err(KodexErr::TurnAborted),
                    _ = abandoned.cancelled() => Err(KodexErr::TurnAborted),
                    result = start => result,
                }
            };
        ...
        let thread = receiver.await.map_err(|_| KodexErr::InternalAgentDied)??;
        handoff.disarm();
        Ok(thread)
    }
}
