...
fn override_builder() -> TestKodexBuilder {
    test_kodex()
...
async fn worker_reasoning_overrides_follow_effective_client_policy(
    ...
    let parent = Arc::clone(&test.kodex);
    ...
    for (thread, expected) in [(&parent, inherited_updates), (&test.kodex, worker_updates)] {
...
async fn reasoning_effort_override_recovery_reuses_trusted_tail_update() -> anyhow::Result<()> {
    ...
    .await;
    let thread_settings = test.kodex.restorable_thread_settings().await;
    assert_eq!(
        test.kodex.suspend_turn_and_shutdown().await?,
    ...
    let test = if !matches!(startup, PrewarmStartup::New) {
        ...
        if matches!(startup, PrewarmStartup::Fork) {
            ...
            let forked = previous
                ...
                .fork_thread(
                    ForkSnapshot::Interrupted,
                    kodex_core::StartThreadOptions::new(config.clone()),
                    previous.kodex.rollout_path().expect("rollout path"),
                )
                .await?;
            previous.kodex = forked.thread;
...
impl OverrideUnavailable {
    fn builder(self) -> TestKodexBuilder {
        match self {
