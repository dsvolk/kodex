...
use kodex_ansi_escape::ansi_escape_line;
...
#[cfg(test)]
use kodex_app_server_protocol::McpAuthStatus;
...
use kodex_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use kodex_app_server_protocol::SkillErrorInfo;
...
use kodex_models_manager::model_presets::HIDE_GPT_5_1_KODEX_MAX_MIGRATION_PROMPT_CONFIG;
use kodex_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
use kodex_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        kodex_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&kodex_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&kodex_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[kodex_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&kodex_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<kodex_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    kodex_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
pub(crate) struct App {
    ...
    dynamic_tool_status_updates:
        tokio::sync::broadcast::Sender<kodex_app_server_protocol::ThreadStatusChangedNotification>,
    dynamic_tool_tasks: HashMap<kodex_app_server_protocol::RequestId, (String, JoinHandle<()>)>,
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.kodex_error_info,
        Some(AppServerKodexErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_kodex_backend_auth: self.chat_widget.has_kodex_backend_auth(),
