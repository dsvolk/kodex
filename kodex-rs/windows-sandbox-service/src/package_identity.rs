//! Binds provisioning requests to the packaged Kodex client and its Windows user.
...
pub(crate) fn authorize_client_process(pipe: HANDLE) -> Result<AuthorizedClientProcess> {
    ...
    let client_family = unsafe { kodex_windows_sandbox::process_package_family(process.0) }
        .context("read provisioning client package identity")?;
    let service_family =
        unsafe { kodex_windows_sandbox::process_package_family(Threading::GetCurrentProcess()) }
    ...
    match client_family {
        Some(client_family) => match service_family.as_deref() {
            ...
            None if FOREGROUND_MODE.load(Ordering::Acquire)
                && is_known_kodex_package_family(&client_family) => {}
            #[cfg(debug_assertions)]
            None if FOREGROUND_MODE.load(Ordering::Acquire) => {
                bail!("provisioning client does not belong to a trusted Kodex package family")
            }
        ...
        #[cfg(debug_assertions)]
        None if FOREGROUND_MODE.load(Ordering::Acquire) => {}
        None => bail!("provisioning clients must run with an installed Kodex package identity"),
    }
...
pub(crate) fn authorize_client(
    ...
    let process_user = unsafe { kodex_windows_sandbox::get_user_sid_bytes(process_token.0) }
        .context("read the client process user")?;
    let impersonated_user = unsafe { kodex_windows_sandbox::get_user_sid_bytes(client_token) }
...
#[cfg(debug_assertions)]
fn is_known_kodex_package_family(package_family: &str) -> bool {
    matches!(
        package_family,
        "OpenAI.Kodex_3k8sg7r9htsxt"
            | "OpenAI.KodexAlpha_3k8sg7r9htsxt"
            | "OpenAI.KodexBeta_3k8sg7r9htsxt"
            | "OpenAI.KodexNightly_3k8sg7r9htsxt"
    )
}
