...
use kodex_app_server_protocol::JSONRPCNotification;
...
use kodex_app_server_protocol::SandboxPolicy;
...
use kodex_otel::OtelProvider;
use kodex_otel::current_span_w3c_trace_context;
use kodex_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "kodex-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $KODEX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Kodex app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Kodex", version, about = "Bootstrap Kodex app-server", long_about = None)]
struct Cli {
    /// Path to the `kodex` CLI binary. When set, requests use stdio by
    /// spawning `kodex app-server` as a child process.
    #[arg(long, env = "KODEX_BIN", global = true)]
    kodex_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--kodex-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "KODEX_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `kodex` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `kodex app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/kodex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `kodex app-server --listen`.
    ...
    /// Send a user message through the Kodex app-server.
    SendMessage {
        /// User message to send to Kodex.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Kodex-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the Kodex app-server.
    GetAccountRateLimits,
    /// List the available models from the Kodex app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Kodex app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "KODEX_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let kodex_bin = kodex_bin.unwrap_or_else(|| PathBuf::from("kodex"));
            serve(&kodex_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(kodex_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(kodex_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(kodex_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --kodex-bin and does not support --url");
            }
            let kodex_bin = kodex_bin.context("plugin-analytics-smoke requires --kodex-bin")?;
            plugin_analytics_smoke::run(&kodex_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --kodex-bin and does not support --url"
                );
            }
            let kodex_bin =
                kodex_bin.context("plugin-analytics-mutation-smoke requires --kodex-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --kodex-bin and does not support --url");
            }
            let kodex_bin = kodex_bin.context("plugin-remote-uninstall requires --kodex-bin")?;
...
fn resolve_endpoint(kodex_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if kodex_bin.is_some() && url.is_some() {
        bail!("--kodex-bin and --url are mutually exclusive");
    }
    if let Some(kodex_bin) = kodex_bin {
        return Ok(Endpoint::SpawnKodex(kodex_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if kodex_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --kodex-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(kodex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(kodex_bin);
        if let Some(kodex_bin_parent) = kodex_bin.parent() {
            let mut path = OsString::from(kodex_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", kodex_bin.display()))?;
...
fn serve(kodex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/kodex-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,kodex_=trace {}",
        shell_quote(&kodex_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started kodex app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnKodex(kodex_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: kodex_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut KodexClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = KodexClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = KodexClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (kodex_bin, url) {
        (Some(_), Some(_)) => bail!("--kodex-bin and --url are mutually exclusive"),
        (Some(kodex_bin), None) => {
            let server = BackgroundAppServer::spawn(&kodex_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve kodex-app-server-test-client binary path")?;
...
impl KodexClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnKodex(kodex_bin) => Self::spawn_stdio(kodex_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(kodex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(kodex_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let kodex_bin_display = kodex_bin.display();
        let mut cmd = Command::new(kodex_bin);
        if let Some(kodex_bin_parent) = kodex_bin.parent() {
            let mut path = OsString::from(kodex_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut kodex_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{kodex_bin_display}` app-server"))?;

        let stdin = kodex_app_server
            .stdin
            .take()
            .context("kodex app-server stdin unavailable")?;
        let stdout = kodex_app_server
            .stdout
            .take()
            .context("kodex app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: kodex_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `kodex-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "kodex-toy-app-server".to_string(),
                    title: Some("Kodex Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: kodex_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                kodex_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: kodex_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to kodex app-server")?;
                    return Ok(());
                }
                bail!("kodex app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from kodex app-server")?;
                if bytes == 0 {
                    bail!("kodex app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = kodex_core::otel_init::build_provider(
            &config,
...
impl Drop for KodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[kodex app-server exited: {status}]");
            return;
        }
