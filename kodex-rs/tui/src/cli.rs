...
use kodex_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `kodex resume` subcommand.
    // These are not exposed as user flags on the base `kodex` command.
    #[clap(skip)]
    ...
    /// top-level `kodex resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `kodex fork` subcommand.
    // These are not exposed as user flags on the base `kodex` command.
    #[clap(skip)]
    ...
    /// top-level `kodex fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
