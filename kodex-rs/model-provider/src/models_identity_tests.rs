...
fn chatgpt_auth(email: &str, user: &str, account: &str, plan: &str, signature: &str) -> KodexAuth {
...
fn cache_identity_tracks_account_email_user_plan_and_auth_mode_but_not_token_refresh() {
    ...
    for auth in [
        ...
        KodexAuth::from_api_key("api-key"),
    ] {
...
fn cache_identity_tracks_provider_routing_and_effective_api_credentials() {
    let auth = KodexAuth::from_api_key("first-key");
    ...
    let other_key = KodexAuth::from_api_key("second-key");
...
fn cache_identity_tracks_catalog_url() {
    let auth = KodexAuth::from_api_key("test-key");
    let mut provider =
