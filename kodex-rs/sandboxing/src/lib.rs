...
pub use kodex_mxc_sandbox::KODEX_WINDOWS_MXC_ARG1;
...
pub fn system_bwrap_warning(
    _permission_profile: &kodex_protocol::models::PermissionProfile,
) -> Option<String> {
    None
}

impl From<SandboxTransformError> for KodexErr {
    fn from(err: SandboxTransformError) -> Self {
        match err {
            ...
            SandboxTransformError::MissingLinuxSandboxExecutable => {
                KodexErr::LandlockSandboxExecutableNotProvided
            }
            SandboxTransformError::EnvironmentNetworkProxy(message) => {
                KodexErr::UnsupportedOperation(message)
            }
            ...
            SandboxTransformError::Wsl1UnsupportedForBubblewrap => {
                KodexErr::UnsupportedOperation(crate::bwrap::WSL1_BWRAP_WARNING.to_string())
            }
            #[cfg(not(target_os = "macos"))]
            SandboxTransformError::SeatbeltUnavailable => KodexErr::UnsupportedOperation(
