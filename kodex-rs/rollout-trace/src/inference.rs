...
const INFERENCE_CALL_ID_HEADER: &str = "x-kodex-inference-call-id";
...
///
/// A Kodex turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Kodex turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        kodex_turn_id: KodexTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                kodex_turn_id: attempt.context.kodex_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps kodex-core focused on
    ...
    /// Records a provider stream that Kodex intentionally stopped consuming.
    ///
...
/// Kodex received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        kodex_turn_id: Some(context.kodex_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.kodex_turn_id, "turn-1");
