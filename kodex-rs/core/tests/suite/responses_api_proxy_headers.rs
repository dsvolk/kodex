...
-> Result<()> {
    ...
    let mut builder = test_kodex().with_config(|config| {
        ...
        config
            ...
            .insert("kodex_security_surface".to_string(), "sdk".to_string());
    });
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-kodex-window-id"))?;
    let child_window_id = child
        .header("x-kodex-window-id")
        .ok_or_else(|| anyhow!("child request missing x-kodex-window-id"))?;
    ...
    assert_eq!(
        child.header("x-kodex-parent-thread-id").as_deref(),
        Some(parent_thread_id)
    );
    let parent_turn_metadata: serde_json::Value = serde_json::from_str(
        &parent
            .header("x-kodex-turn-metadata")
            .ok_or_else(|| anyhow!("parent request missing x-kodex-turn-metadata"))?,
    )?;
    assert_eq!(parent_turn_metadata["kodex_security_surface"], json!("sdk"));
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-kodex-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-kodex-turn-metadata"))?,
    )?;
    ...
    assert_eq!(child_turn_metadata["kodex_security_surface"], json!("sdk"));

    Ok(())
}

async fn submit_turn_with_timeout(test: &TestKodex, prompt: &str) -> Result<()> {
