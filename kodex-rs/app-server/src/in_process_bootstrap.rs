...
pub struct EmbeddedNetworkPolicy {
    pub(crate) effective: kodex_http_client::NetworkPolicyController,
    pub(crate) local: kodex_http_client::NetworkPolicyController,
}

impl EmbeddedNetworkPolicy {
    /// Loads the local rules for cloud bootstrap; all other traffic starts unavailable.
    pub async fn load(overrides: &kodex_config::LoaderOverrides) -> Self {
        let policy = Self::default();
        let local = async {
            kodex_config::loader::load_local_application_requirements(
    ...
    pub fn bind_bootstrap_auth(
        &self,
        mut auth: kodex_login::AuthConfig,
    ) -> kodex_login::AuthConfig {
        let factory = auth
            .auth_route_config
            .http_client_factory()
            .clone()
        ...
        .config_bundle_url();
        auth.auth_route_config = kodex_login::AuthRouteConfig::from_http_client_factory(
...
pub(super) async fn configure(
    ...
    let bootstrap_auth = kodex_login::AuthConfig {
    ...
    let policy_config = config_manager
        .load_startup_config(Some(config.cwd.to_path_buf()))
        .await?;
    let auth_config = kodex_login::AuthConfig {
    ...
    .await
    .map_err(IoError::other)?;
    let auth_manager = if enable_kodex_api_key_env {
        AuthManager::shared_from_auth_config(auth_config, /*enable_kodex_api_key_env*/ true)
            .await
            .map_err(IoError::other)?
    } else {
    ...
    kodex_login::default_client::set_default_client_residency_requirement(
