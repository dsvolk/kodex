...
use kodex_mcp::KodexAppsResourceListParams;
...
async fn root_and_spawned_subagent_receive_distinct_mcp_session_sources() -> Result<()> {
    ...
    let test = core_test_support::test_kodex::test_kodex()
...
async fn root_reconciliation_reuses_pending_apps_startup() -> Result<()> {
    ...
    extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
        ...
        url: format!("{}/api/kodex/ps/mcp", gated_apps_server.chatgpt_base_url),
                        ...
                        .map(|(name, server)| {
                            (
                                ...
                                serde_json::from_value(json!({
                                    "url": format!("{}/api/kodex/ps/mcp", server.chatgpt_base_url),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn kodex_apps_resource_filter_sends_mime_type_on_each_page() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    for (cursor, expected_page) in [
        ...
        let page = client
            .list_kodex_apps_resources(KodexAppsResourceListParams {
                cursor,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn kodex_apps_resource_filter_rejects_extension_name_collision() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
        id: "test-extension",
        url: format!("{}/api/kodex/ps/mcp", apps_server.chatgpt_base_url),
        root_resolved: None,
    }));
    extensions.thread_lifecycle_contributor(Arc::new(McpResourceClientCapture {
        client: Arc::clone(&captured_client),
    ...
    let client = captured_client
        ...
        .clone()
        .expect("thread start should capture the MCP resource client");
    assert!(client.has_server(KODEX_APPS_MCP_SERVER_NAME).await);
    ...
    let error = client
        ...
        .await
        .expect_err("an extension named kodex_apps must not inherit the Apps resource filter");
    assert_eq!(
        error.to_string(),
        "MCP server 'kodex_apps' is not registered by the hosted runtime"
    );
    let requests = server.received_requests().await.expect("recorded requests");
    assert!(
        ...
        "an extension named kodex_apps must not receive filtered resource requests"
    );
    test.kodex.shutdown_and_wait().await?;
    Ok(())
}
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, kodex_config::types::McpServerConfig>::deserialize)
        .transpose()?
...
async fn elevated_apps_catalog_limit_requires_host_owned_registration() -> Result<()> {
    ...
    for extension_id in [None, Some("hosted_plugin_runtime"), Some("test-extension")] {
        ...
        let test = builder.build_with_auto_env(&server).await?;
        let startup = wait_for_mcp_server(&test.kodex, KODEX_APPS_MCP_SERVER_NAME).await;
        ...
        if extension_id == Some("test-extension") {
            ...
            assert!(
                ...
                "an extension named kodex_apps must not inherit the trusted Apps limit: {error}"
            );
            continue;
        }
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__kodex_apps__calendar_create_event(args:")
        }),
...
async fn initially_empty_deferred_tool_world_state_is_not_rendered_or_persisted() -> Result<()> {
    ...
    let mut builder = core_test_support::test_kodex::test_kodex()
        .with_config(enable_deferred_tool_world_state_without_agents);
    let test = builder.build_with_auto_env(&server).await?;
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    wait_for_mcp_server(&initial.kodex, KODEX_APPS_MCP_SERVER_NAME).await?;
    initial
        .submit_turn("inspect deferred tools before resume")
        .await?;

    initial.kodex.ensure_rollout_materialized().await;
    initial.kodex.flush_rollout().await?;
    ...
    assert_eq!(
        persisted_tools,
        json!({
            "mcp__kodex_apps__calendar": "Plan events and manage your calendar."
        })
    );
    ...
    drop(initial);
    wait_for_mcp_server(&resumed.kodex, KODEX_APPS_MCP_SERVER_NAME).await?;
    resumed
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    test.kodex
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.kodex, |event| {
    ...
    wait_for_event(&test.kodex, |event| {
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == KODEX_APPS_MCP_SERVER_NAME
                    && matches!(update.status, kodex_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(test.kodex_home_path().join("cache/kodex_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.kodex.submit(Op::RefreshMcpServers).await?;
    test.kodex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
