...
use kodex_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use kodex_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use kodex_core::KodexResponsesMetadata;
use kodex_core::ModelClient;
use kodex_core::ModelClientSession;
...
use kodex_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use kodex_core::test_support::with_parent_turn;
...
use kodex_protocol::models::ExecutedToolCall;
...
use kodex_protocol::models::ToolResultMetadata;
use kodex_protocol::models::ToolResultSource;
use kodex_protocol::models::ToolResultSources;
...
use kodex_rollout_trace::ConversationPart;
use kodex_rollout_trace::InferenceTraceContext;
use kodex_rollout_trace::RawTraceEventPayload;
use kodex_rollout_trace::TraceWriter;
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> KodexResponsesMetadata {
    responses_metadata(harness, turn_id, TestKodexResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestKodexResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> KodexResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestKodexResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_preserves_credit_usage_metadata() {
    ...
    let harness = websocket_harness_for_kodex_backend(&server).await;
    ...
    assert_eq!(
        usage_metadata,
        Some(kodex_protocol::ResponseUsageMetadata {
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(X_KODEX_ROUTING_HINT_HEADER),
    ...
    assert_eq!(
        ...
        Some(kodex_login::default_client::get_kodex_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-kodex-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_KODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_omits_routing_hint_for_provider_with_own_credentials() {
    ...
    assert_eq!(handshake.header(X_KODEX_ROUTING_HINT_HEADER), None);
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-kodex-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::KodexTurnStarted {
            kodex_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
    ...
    for new_turn_on_switch in [false, true] {
                    ...
                    .map(|index| {
                        ...
                        vec![
                            ...
                            json!({
                                ...
                                "headers": {"x-kodex-turn-state": turn_state},
                            }),
                            ev_completed(&id),
                        ]
                    })
        ...
        for index in 1..=5 {
            if index == 3 {
                ...
                std::fs::write(
                    harness.kodex_home.path().join("auth.json"),
                ...
                .map(|requests| requests
                    ...
                    .map(|request| {
                        ...
                        (
                            ...
                            body["client_metadata"]["x-kodex-turn-state"].clone(),
                        )
                    })
                    .collect::<Vec<_>>())
                .collect::<Vec<_>>(),
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server.single_handshake().header("x-kodex-turn-metadata"),
        None
    );
    assert_eq!(
        server
            .single_handshake()
            .header(X_KODEX_ROUTING_HINT_HEADER),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-kodex-primary-used-percent": "100.0",
            "x-kodex-secondary-used-percent": "87.5",
            "x-kodex-primary-over-secondary-limit-percent": "95.0",
            "x-kodex-primary-window-minutes": "15",
            "x-kodex-secondary-window-minutes": "60",
            "x-kodex-credits-has-credits": "true",
            "x-kodex-credits-unlimited": "false",
            "x-kodex-credits-balance": "",
            "x-kodex-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let token_event =
        wait_for_event(&test.kodex, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.kodex, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
...
async fn websocket_harness_for_kodex_backend(server: &WebSocketTestServer) -> WebsocketTestHarness {
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = kodex_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(|auth| {
        kodex_core::test_support::auth_manager_from_auth_with_home(
            auth,
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "kodex-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
...
async fn responses_websocket_restored_history_metric(fork: bool) -> anyhow::Result<()> {
    ...
    let exporter = InMemoryMetricExporter::default();
    let config =
        MetricsConfig::in_memory("test", "kodex-core", "test", exporter).with_runtime_reader();
    ...
    let mut initial = test_kodex().build_with_websocket_server(&server).await?;
    initial.submit_text_turn("original history marker").await?;
    let rollout_path = initial.kodex.rollout_path().expect("persisted history");
    initial.kodex.shutdown_and_wait().await?;

    let manager = &initial.thread_manager;
    let mut options = kodex_core::StartThreadOptions::new(initial.config.clone());
    options.thread_extension_init.insert(metrics);
    let restored = if fork {
        manager
            .fork_thread(kodex_core::ForkSnapshot::Interrupted, options, rollout_path)
            .await?
    } else {
        options.initial_history =
            kodex_rollout::RolloutRecorder::get_rollout_history(&rollout_path).await?;
        manager.start_thread(options).await?
    };
    initial.kodex = restored.thread;
    initial.submit_text_turn("continue").await?;
    assert_continuation_metrics(
        &initial.kodex.session_telemetry(),
        &[
...
fn assert_continuation_metrics(telemetry: &SessionTelemetry, expected: &[([&str; 3], u64)]) {
    ...
    let metric = snapshot
        ...
        .find(|metric| metric.name() == kodex_otel::WEBSOCKET_CONTINUATION_COUNT_METRIC)
