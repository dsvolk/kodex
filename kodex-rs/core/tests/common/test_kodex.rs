...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-kodex";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: kodex_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let environment =
            kodex_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &kodex_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                kodex_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestKodexBuilder {
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestKodex> {
    ...
    ///
    /// With no remote test configuration, or with `KODEX_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `KODEX_TEST_ENVIRONMENT=docker` or
    /// `KODEX_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `KODEX_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `KODEX_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        ...
        previous.kodex.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let kodex_linux_sandbox_exe = Some(
            crate::find_kodex_linux_sandbox_exe()
                .context("should find binary for kodex-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let kodex_linux_sandbox_exe = None;
        let local_runtime_paths = kodex_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            kodex_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            kodex_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            kodex_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        let installation_id = resolve_installation_id(&config.kodex_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(KodexHomeUserInstructionsProvider::new(
        ...
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| kodex_core::build_models_manager(&config, auth_manager.clone()));
        ...
        let thread_manager = Arc::new_cyclic(|manager| {
            ...
            if config.features.enabled(Feature::GuardianV2) {
                ...
                kodex_guardian_v2::install_reviewer(&mut extensions, manager.clone());
            }
            let thread_manager = ThreadManager::new(
                ...
                kodex_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
                installation_id,
            ...
            {
                kodex_core::test_support::with_code_mode_host_program(
                    thread_manager,
                    code_mode_host_program,
                    &config,
                )
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    kodex_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = kodex_utils_cargo_bin::cargo_bin("kodex") {
            config.kodex_self_exe = Some(path);
        } else if let Ok(path) = kodex_utils_cargo_bin::cargo_bin("kodex-exec") {
            // `kodex-exec` also supports `--kodex-run-as-apply-patch`, so use it
        ...
        {
            let kodex = bin_dir.join("kodex");
            let kodex_exec = bin_dir.join("kodex-exec");
            if kodex.is_file() {
                config.kodex_self_exe = Some(kodex);
            } else if kodex_exec.is_file() {
                config.kodex_self_exe = Some(kodex_exec);
            }
        }
...
pub struct TestKodex {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub kodex: Arc<KodexThread>,
...
impl TestKodex {
    ...
    pub fn kodex_home_path(&self) -> &Path {
        self.config.kodex_home.as_path()
    }
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.kodex, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
        Ok(())
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.kodex, |event| match event {
...
pub struct TestKodexHarness {
    server: MockServer,
    test: TestKodex,
}

impl TestKodexHarness {
    pub async fn with_builder(mut builder: TestKodexBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestKodexBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_kodex() -> TestKodexBuilder {
    TestKodexBuilder {
        ...
        models_manager: None,
        thread_store: None,
        image_store: kodex_core::passthrough_image_store(),
    }
}
