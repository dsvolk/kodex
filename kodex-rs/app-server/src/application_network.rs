...
use kodex_config::ApplicationRequirementsToml;
...
impl ConfigManager {
    pub(super) async fn refresh_local_network_policy(
        &self,
    ) -> io::Result<kodex_config::loader::LocalApplicationRequirements> {
        let _reload = self
    ...
    async fn load_local_network_policy(
        &self,
        application_revision: kodex_http_client::NetworkPolicyRevision,
    ) -> io::Result<kodex_config::loader::LocalApplicationRequirements> {
        let local_revision = self.local_network_policy.policy().revision();
        let result = async {
            let sources = kodex_config::loader::load_local_application_requirements(
