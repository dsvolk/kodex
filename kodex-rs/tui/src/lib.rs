...
// The standalone `kodex-tui` binary prints a short help message before the
...
pub use kodex_app_server_client::RemoteAppServerEndpoint;
use kodex_app_server_protocol::Account as AppServerAccount;
...
use kodex_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use kodex_config::format_config_error_with_source;
...
use kodex_login::default_client::set_default_client_residency_requirement;
use kodex_login::enforce_login_restrictions;
...
use kodex_rollout::StateDbHandle;
use kodex_rollout::state_db;
use kodex_state::log_db;
...
use kodex_utils_oss::ensure_oss_provider_ready;
...
pub(crate) use kodex_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "kodex-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: kodex_feedback::KodexFeedback,
...
impl AppServerTarget {
    ...
    fn environment_http_client_factory(
        &self,
        config: &Config,
        policy: &kodex_app_server_client::EmbeddedNetworkPolicy,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = kodex_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(kodex_home: &Path) {
    ...
    let _ = std::fs::remove_file(kodex_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let kodex_home = find_kodex_home().wrap_err("failed to resolve KODEX_HOME")?;
            kodex_app_server_client::app_server_control_socket_path(&kodex_home)
...
async fn connect_remote_app_server(
    ...
    let app_server = RemoteAppServerClient::connect(RemoteAppServerConnectArgs {
        endpoint,
        client_name: "kodex-tui".to_string(),
...
async fn maybe_probe_default_daemon_socket(kodex_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = kodex_app_server_client::app_server_control_socket_path(kodex_home).ok()?;
    #[cfg(windows)]
    let (validated_path, _directory) =
        kodex_uds::validate_private_socket_path(socket_path.as_path()).ok()?;
    #[cfg(windows)]
    ...
    match tokio::time::timeout(
        ...
        kodex_uds::UnixStream::connect(probe_path),
    )
    .await
    {
        Ok(Ok(_stream)) => {
    ...
    ThreadListParams {
        ...
        cwd: cwd_filter.map(|cwd| {
            resume_picker::repository_cwd_filter(
                cwd,
                uses_remote_filesystem,
                config.features.enabled(kodex_features::Feature::Worktrees),
            )
        }),
    ...
    cloud_config_bundle_loader_for_storage(
        embedded_network_policy
            .bind_bootstrap_auth(app_server_target.auth_config_for_cloud_loader(
                bootstrap_auth_config(kodex_home, bootstrap_config)?,
            )),
        /*enable_kodex_api_key_env*/ false,
    )
    .await
...
async fn run_ratatui_app(
    ...
    let app_server_session = match startup_app_server {
        Ok(Ok(app_server)) => {
            AppServerSession::new(app_server, app_server_target.thread_params_mode())
                .with_local_kodex_home(&initial_config.kodex_home)
        }
        Ok(Err(err)) => {
            terminal_restore_guard.restore_silently();
            session_log::log_session_end();
    ...
    if (cli.resume_picker || cli.fork_picker)
        ...
        && let Some(theme) =
            crate::render::highlight::resolve_theme_by_name(name, Some(config.kodex_home.as_path()))
    {
    ...
    if config.analytics_enabled != Some(false)
        && config.otel.metrics_exporter != kodex_config::types::OtelExporterKind::None
        && let Some(metrics) = kodex_otel::global()
    {
        let _ = metrics.counter(
            "kodex.tui.fullscreen_transcript",
            /*inc*/ 1,
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_kodex_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, kodex_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        kodex_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
"#
    ...
    async fn bedrock_setup_wizard_requires_eligible_onboarding() -> color_eyre::Result<()> {
        ...
        for (label, config_toml, login_status, target, expected) in [
            ...
            std::fs::write(kodex_home.path().join("config.toml"), config_toml)?;
    ...
    pub(crate) fn write_session_rollout(
        ...
        let session_meta = kodex_protocol::protocol::SessionMeta {
            ...
            source: kodex_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(kodex_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_kodex_home(kodex_home.path()).await?;
            ...
            let cloud_config_bundle =
                kodex_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: kodex_app_server_client::app_server_control_socket_path(&kodex_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://kodex.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("kodex.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("kodex.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let kodex_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(kodex_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let kodex_home = TempDir::new()?;
        let socket_path =
            kodex_app_server_client::app_server_control_socket_path(kodex_home.path())?;
        #[cfg(windows)]
        {
            let parent = socket_path.as_path().parent().expect("socket parent");
            std::fs::create_dir_all(parent)?;
            let listener = kodex_uds::UnixListener::bind(socket_path.as_path()).await?;
            assert!(
                maybe_probe_default_daemon_socket(kodex_home.path())
                    .await
                    .is_none()
        ...
        kodex_uds::prepare_private_socket_directory(
            socket_path.as_path().parent().expect("socket parent"),
        )
        .await?;
        let _listener = kodex_uds::UnixListener::bind(socket_path.as_path()).await?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(kodex_home.path()).await,
            expected
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("kodex.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        config
            .features
            .set_enabled(kodex_features::Feature::Worktrees, /*enabled*/ false)?;
        ...
        let mut app_server = AppServerSession::new(
            kodex_app_server_client::AppServerClient::InProcess(
        ...
        config
            .features
            .set_enabled(kodex_features::Feature::Worktrees, /*enabled*/ true)?;
        let filter_cwd = latest_session_cwd_filter(
            /*uses_remote_workspace*/ false, /*remote_cwd_override*/ None, &config,
            /*show_all*/ false,
        );
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            kodex_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = kodex_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            kodex_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let kodex_home = temp_dir.path().to_path_buf();
