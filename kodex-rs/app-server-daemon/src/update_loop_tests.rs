...
async fn explicit_update_migrates_running_and_stopped_installations() {
    for (running, local) in [(false, false), (true, false), (false, true), (true, true)] {
        ...
        if local {
            ...
            std::fs::copy(&legacy.managed_kodex_bin, package.join("kodex")).unwrap();
        ...
        b"# KODEX_INSTALL_IF_LATEST\ntest \"$KODEX_INSTALL_DEFER_SELECTION\" = 0 && test \"$KODEX_INSTALL_DAEMON_ONLY\" = 0\n".to_vec(),
    ));
        super::update_once(
            &scheduled,
            &legacy,
            &executable_identity(&legacy.managed_kodex_bin)
                .await
                .unwrap(),
            &mut test_terminate(),
        ...
        let legacy_bin = std::fs::read(&legacy.managed_kodex_bin).unwrap();
        ...
        let script = format!(
            r#"# KODEX_INSTALL_DEFER_SELECTION
set -eu
test "$KODEX_INSTALL_DEFER_SELECTION" = 1
test "$KODEX_INSTALL_IF_CURRENT" = 0
test "$KODEX_INSTALL_DAEMON_ONLY" = 1
test "$KODEX_INSTALL_IF_LATEST" = 0
root="$KODEX_HOME/packages/app-server-daemon"
mkdir -p "$root/releases/{release}/bin"
printf '#!/bin/sh\nif [ "$1" = --version ]; then echo kodex 1.0.0; else exit 2; fi\n' > "$root/releases/{release}/bin/kodex"
chmod +x "$root/releases/{release}/bin/kodex"
...
"#
        ...
        managed_kodex_path: dedicated.join("current/bin/kodex"),
        ...
        assert_eq!(
            std::fs::read(&legacy.managed_kodex_bin).unwrap(),
            legacy_bin
        );
        assert_eq!(
        ...
        let selected = Daemon {
            ...
            managed_kodex_bin: dedicated.join("current/bin/kodex"),
            ..legacy.clone()
        };
...
fn manual_update_daemon(home: &TempDir) -> (Daemon, String) {
    ...
    let bin = standalone.join("releases").join(&release).join("kodex");
    std::fs::create_dir_all(bin.parent().expect("binary parent")).expect("release directory");
    std::fs::write(
        &bin,
        b"#!/bin/sh\nif [ \"$1\" = '--version' ]; then echo kodex 1.0.0; else exec sleep 30; fi\n",
    )
    ...
    (
        Daemon {
            ...
            managed_kodex_bin: standalone.join("current/kodex"),
        },
        release,
    )
}
...
async fn manual_request_retries_after_updater_replacement() {
    ...
    kodex_uds::prepare_private_socket_directory(socket_path.parent().expect("socket parent"))
        .await
        .expect("socket directory");
    let mut listener = kodex_uds::UnixListener::bind(&socket_path)
        .await
        .expect("old updater socket");
    let expected = UpdateOutput {
        status: UpdateStatus::NoUpdate,
        managed_kodex_path: daemon.managed_kodex_bin.clone(),
    ...
    let server = tokio::spawn(async move {
        ...
        let mut successor = kodex_uds::UnixListener::bind(&socket_path)
            .await
...
async fn manual_request_recovers_when_one_shot_updater_exits() {
    ...
    std::fs::remove_file(&daemon.managed_kodex_bin).expect("remove selected binary");
    let result = tokio::time::timeout(
...
async fn unsupported_request_preserves_updater_schedule() {
    ...
    let identity = executable_identity(&daemon.managed_kodex_bin)
        .await
    ...
    let mut pinned = kodex_uds::UnixStream::connect(&socket_path).await.unwrap();
    ...
    std::fs::remove_file(&daemon.managed_kodex_bin).expect("remove selected binary");
    let mut malformed = kodex_uds::UnixStream::connect(&socket_path)
        .await
...
async fn test_control_server(
    ...
    std::fs::create_dir_all(daemon.socket_path.parent().expect("socket parent"))
        .expect("socket directory");
    let mut listener = kodex_uds::UnixListener::bind(&daemon.socket_path)
        .await
        .expect("control listener");
    let kodex_home = home.to_path_buf();
    tokio::spawn(async move {
        loop {
            ...
            let version = if std::fs::read_to_string(
                crate::managed_install::package_root(&kodex_home).join("auto-update-version"),
            )
            .unwrap_or_default()
            .starts_with("1.1.0")
            {
            ...
            websocket.send(tokio_tungstenite::tungstenite::Message::Text(
                serde_json::json!({"id": 1, "result": {
                    "userAgent": format!("kodex_app_server_daemon/{version}"),
                    "kodexHome": kodex_home, "platformFamily": "unix", "platformOs": std::env::consts::OS,
                }}).to_string().into(),
...
async fn daemon_start_and_restart_preserve_launch_features() {
    ...
    .unwrap();
        std::fs::write(&daemon.managed_kodex_bin, format!(
        "#!/bin/sh\nif [ \"$1\" = --version ]; then echo kodex 1.0.0; exit; fi\nif [ \"$3\" = --help ]; then exit; fi\nprintf '%s\\n' \"$@\" > '{}'\nexec sleep 30\n",
        args_path.display(),
    )).unwrap();
        let control = async {
...
async fn confirmed_feature_restart_preserves_ownership_and_skips_matching_settings() {
    ...
    for managed in [true, false] {
        ...
        if managed {
            ...
            let selected_package = daemon.managed_kodex_bin.parent().unwrap();
...
async fn check_manual_update_restart(package_directory: &str) {
    ...
    if local_package {
        ...
        daemon.managed_kodex_bin = standalone.join("current/kodex");
        release = local;
    }
    let daemon = std::sync::Arc::new(daemon);
    ...
    let guard = if local_package {
        "KODEX_INSTALL_IF_CURRENT"
    } else {
        "KODEX_INSTALL_IF_LATEST"
    };
    let install_binary = if local_package {
        // Same binary and version, but a different package: it must still restart.
        format!(
            "cp '{root}/releases/{release}/kodex' '{root}/releases/{next}/bin/kodex'",
            root = standalone.display()
        )
    } else {
        format!(
            r#"printf '#!/bin/sh\nif [ "$1" = --version ]; then echo kodex 1.1.0; else exec sleep 30; fi\n' > '{root}/releases/{next}/bin/kodex'"#,
            root = standalone.display()
        )
    };
    ...
    let script = format!(
        "#!/bin/sh\n# KODEX_INSTALL_IF_LATEST KODEX_INSTALL_IF_CURRENT KODEX_INSTALL_DAEMON_ONLY\nif [ \"$KODEX_UPDATE_FROM_RELEASE\" = '{next}' ]; then exit 0; fi\ntest \"${guard}\" = 1 || exit 4\ntest \"$KODEX_UPDATE_FROM_RELEASE\" = '{release}' || exit 5\ntouch '{ready}'\nwhile [ ! -e '{proceed}' ]; do sleep .05; done\nmkdir -p '{root}/releases/{next}/bin'\n{install_binary}\nchmod +x '{root}/releases/{next}/bin/kodex'\nln -sfn 'releases/{next}' '{root}/current'\nprintf '{next}' > '{root}/auto-update-version'\n",
        root = standalone.display(),
        ready = ready.display(),
    ...
    let mut queued = kodex_uds::UnixStream::connect(&daemon.manual_update_socket_path())
        .await
    ...
    assert_eq!(
        output.managed_kodex_path,
        standalone.join("current/bin/kodex")
    );
    ...
    let no_op = FakeInstallerHttp::new(InstallerResponse::Success(
        b"#!/bin/sh\n# KODEX_INSTALL_IF_LATEST KODEX_INSTALL_DAEMON_ONLY\nexit 0\n".to_vec(),
    ));
    use std::io::Write;
...
function Test-Installer {
    if ($env:KODEX_NON_INTERACTIVE -ne '1') { throw 'interactive installer' }
    if ($env:KODEX_INSTALL_DAEMON_ONLY -ne '1') { throw 'wrong package destination' }
    if ($env:KODEX_INSTALL_IF_CURRENT -ne '1' -or $env:KODEX_INSTALL_IF_LATEST -ne '0') { throw 'wrong update guard' }
}
Test-Installer
"#
        .to_vec(),
...
async fn daemon_owned_updates_require_and_request_an_isolated_installer() {
    ...
    daemon.managed_kodex_bin = root.join("current/kodex");
    let identity = executable_identity(&daemon.managed_kodex_bin)
        .await
        .unwrap();
    ...
    let current = FakeInstallerHttp::new(InstallerResponse::Success(
        b"# KODEX_INSTALL_IF_LATEST\ntest \"$KODEX_INSTALL_IF_LATEST\" = 1 && test \"$KODEX_INSTALL_DAEMON_ONLY\" = 1\n".to_vec(),
    ));
    let output = manual_update_once(
        &current,
        &daemon,
    ...
    let no_op = FakeInstallerHttp::new(InstallerResponse::Success(
        b"# KODEX_INSTALL_IF_CURRENT KODEX_INSTALL_DAEMON_ONLY\nexit 0\n".to_vec(),
    ));
    let script = format!(
        r#"# KODEX_INSTALL_IF_CURRENT KODEX_INSTALL_DAEMON_ONLY
set -eu
test "$KODEX_INSTALL_IF_CURRENT" = 1
test "$KODEX_INSTALL_IF_LATEST" = 0
test "$KODEX_INSTALL_DAEMON_ONLY" = 1
test "$KODEX_RELEASE" = latest
test "$KODEX_UPDATE_FROM_RELEASE" = '{release}'
