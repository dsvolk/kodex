...
use kodex_models_manager::cache::ModelsCacheEntry;
use kodex_models_manager::cache::ModelsCacheError;
use kodex_models_manager::cache::ModelsCacheFuture;
...
async fn run_agent_with_model(models_manager: SharedModelsManager, model_slug: &str) -> Result<()> {
    ...
    loop {
        if matches!(
            wait_for_event(&test.kodex, |_| true).await,
...
async fn injected_cache_hit_drives_agent_model_selection() -> Result<()> {
    ...
    let cache = Arc::new(TestModelsCache {
        entry: Some(ModelsCacheEntry {
            ...
            fetched_at: Utc::now(),
            etag: None,
            client_version: Some(kodex_models_manager::client_version_to_whole()),
...
async fn account_switch_during_cache_store_preserves_new_catalog_for_next_turn() -> Result<()> {
    ...
    let initial_auth = KodexAuth::create_dummy_chatgpt_auth_for_testing();
