...
async fn http_auth_challenge_reaches_agent_tool_call_events_without_replay() -> anyhow::Result<()> {
    ...
    let fixture = test_kodex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.kodex, "reauth").await?;
    fixture
        .kodex
...
async fn oauth_mode_refresh_replaces_the_live_connection(
    ...
    Mock::given(method("GET"))
        .and(path(
            "/.well-known/oauth-authorization-server/api/kodex/ps/mcp",
        ))
    ...
    let _kodex_home_guard = EnvVarGuard::set("KODEX_HOME", home.path().as_os_str());
