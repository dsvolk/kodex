...
async fn private_tmp_mount_preserves_daemon_socket_isolation() {
    ...
    std::fs::copy(kodex_linux_sandbox_exe(), private.path().join("sandbox")).unwrap();
    ...
    let output = tokio::process::Command::new("unshare")
        ...
        .env("KODEX_TEST_PRIVATE_TMP", private.path())
        .kill_on_drop(true)
        .output()
        .await
        .unwrap();
...
fn private_tmp_fixture() {
    let Some(private) = std::env::var_os("KODEX_TEST_PRIVATE_TMP") else {
        return;
    };
    ...
    let root = kodex_uds::prepare_shared_daemon_socket_directory().unwrap();
    ...
    command
        ...
        .env("KODEX_TEST_DAEMON_SOCKET", &endpoint)
        .env("TMPDIR", "/tmp");
    ...
    command
        .env("KODEX_TEST_DAEMON_ALIAS_SOCKET", &alias_endpoint)
        .env("KODEX_TEST_OTHER_ALIAS_SOCKET", "/tmp/build/other.sock");
    let output = command.output().unwrap();
    assert_eq!(output.status.code(), Some(0), "{output:?}");
    assert!(String::from_utf8_lossy(&output.stdout).contains("private-tmp-isolated"));
    ...
    let registry_alias = PathBuf::from(format!(
        "/tmp/kodex-bwrap-synthetic-mount-targets-{}/view",
        unsafe { libc::geteuid() }
    ));
...
fn private_tmp_client() {
    let Some(endpoint) = std::env::var_os("KODEX_TEST_DAEMON_SOCKET") else {
        return;
    };
    ...
    if let Some(alias) = std::env::var_os("KODEX_TEST_DAEMON_ALIAS_SOCKET") {
        assert!(
            UnixStream::connect(alias).is_err(),
            "daemon alias reachable"
        );
        UnixStream::connect(std::env::var_os("KODEX_TEST_OTHER_ALIAS_SOCKET").unwrap())
...
"#;
    ...
    for proc_mode in ["fresh-proc", "host-proc"] {
        let mut command = tokio::process::Command::new(kodex_linux_sandbox_exe());
        command
