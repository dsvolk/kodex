...
pub(crate) const DELAYED_OUTPUT_AFTER_EXIT_PARENT_ARG: &str =
    "--kodex-test-delayed-output-after-exit-parent";
pub(crate) const SYSTEM_PROXY_REQUEST_URL_ENV: &str =
    "KODEX_EXEC_SERVER_TEST_SYSTEM_PROXY_REQUEST_URL";
pub(crate) const SYSTEM_PROXY_URL_ENV: &str = "KODEX_EXEC_SERVER_TEST_SYSTEM_PROXY_URL";

const KODEX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";
const DELAYED_OUTPUT_AFTER_EXIT_CHILD_ARG: &str = "--kodex-test-delayed-output-after-exit-child";

#[macro_export]
macro_rules! skip_if_mxc_unavailable {
    ($return_value:expr $(,)?) => {{
        if !kodex_sandboxing::windows_mxc_available() {
...
pub static TEST_BINARY_DISPATCH_GUARD: Option<TestBinaryDispatchGuard> = {
    let guard = configure_test_binary_dispatch("kodex-exec-server-tests", |exe_name, argv1| {
        if argv1 == Some(KODEX_ARG0_EXEC_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(KODEX_FS_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(KODEX_WINDOWS_SANDBOX_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(kodex_sandboxing::KODEX_WINDOWS_MXC_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
...
pub(crate) fn current_test_binary_helper_paths() -> anyhow::Result<(PathBuf, Option<PathBuf>)> {
    ...
    let kodex_linux_sandbox_exe = if cfg!(target_os = "linux") {
        TEST_BINARY_DISPATCH_GUARD
            .as_ref()
            .and_then(|guard| guard.paths().kodex_linux_sandbox_exe.clone())
            .or_else(|| Some(current_exe.clone()))
    } else {
        None
    };
    Ok((current_exe, kodex_linux_sandbox_exe))
}
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    kodex_build_info::BuildInfo::initialize(TEST_BUILD_COMMIT);
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            kodex_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(async {
        #[cfg(target_os = "macos")]
        let runtime_paths = {
            let home = kodex_utils_home_dir::find_kodex_home()?;
            let config = kodex_config::loader::load_config_layers_state(
                &kodex_exec_server::LocalFileSystem::unsandboxed(),
                home.as_path(),
                /*cwd*/ None,
                &[],
            ...
            .await?;
            runtime_paths.with_allowed_symlinked_kodex_home(
                kodex_config::allowed_symlinked_kodex_home(&config, &home),
            )
        };
