...
async fn request_permissions_auto_review_applies_guardian_decision(outcome: &str) -> Result<()> {
    ...
    skip_if_wine_exec!(
        Ok(()),
        "request_permissions requires a cwd native to the Kodex host"
    );
...
async fn partial_request_permissions_grants_do_not_preapprove_new_permissions() -> Result<()> {
    ...
    let kodex_protocol::models::LegacyReadWriteRoots {
        read: approval_reads,
...
async fn request_permissions_session_grants_carry_across_turns() -> Result<()> {
    ...
    let completion_event = wait_for_event(&test.kodex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn denied_child_permissions_require_fresh_approval(
    ...
    let harness =
        TestKodexHarness::with_auto_env_builder(test_kodex().with_config(move |config| {
    ...
    test.kodex.submit(decision).await?;
    wait_for_completion(test).await;
