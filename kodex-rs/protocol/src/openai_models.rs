//! Shared model metadata types exchanged between Kodex services and clients.
//!
...
/// Metadata describing a Kodex-supported model.
#[derive(Debug, Clone, Deserialize, Serialize, TS, JsonSchema, PartialEq)]
...
/// Model metadata returned by the Kodex backend `/models` endpoint.
...
impl ModelInfo {
    pub fn supports_service_tier(&self, service_tier: &str) -> bool {
        // Flex is an API request option, even when the Kodex catalog does not advertise it.
