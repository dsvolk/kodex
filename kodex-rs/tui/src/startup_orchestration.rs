...
pub(super) async fn run_main_inner(
    ...
    if cli.shared.worktree {
        ...
        if cli.fork_picker || cli.fork_last {
            return Err(std::io::Error::other(
                "`kodex fork --worktree` requires an explicit session ID",
            ));
        }
    }
    ...
    // `oss` model provider.
    let overrides_cli = kodex_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let kodex_home = match find_kodex_home() {
        Ok(kodex_home) => kodex_home.to_path_buf(),
        Err(err) => {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&kodex_home, profile_v2);
        launch_loader_overrides.user_config_path = Some(user_config_path);
        launch_loader_overrides.user_config_profile = Some(profile_v2.clone());
    }
    let embedded_network_policy =
        kodex_app_server_client::EmbeddedNetworkPolicy::load(&launch_loader_overrides).await;
    ...
    if !std::io::stdin().is_terminal() || !std::io::stdout().is_terminal() {
        let validation_target = app_server_target_for_launch(
            ...
            std::env::var_os(kodex_exec_server::KODEX_EXEC_SERVER_URL_ENV_VAR).as_deref(),
        )?;
        let validation_environment_manager =
            if should_load_configured_environments(&loader_overrides, &validation_target) {
                EnvironmentManager::prepare_from_kodex_home(&kodex_home).await
            } else {
    ...
    let initial_screen = if cli.resume_picker || cli.fork_picker || cli.agents_overview {
        ...
        && startup_preflight::should_delay_startup_composer_for_first_login(
            &kodex_home,
            kodex_config::loader::system_config_toml_file(),
            || kodex_config::loader::has_local_managed_configuration(&kodex_home),
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        startup_draft
            .run_until(maybe_probe_default_daemon_socket(&kodex_home))
            .await?
    } else {
        None
    };
    ...
    if auto_start_daemon && daemon_exclusion.is_none() {
        ...
        let output = startup_draft
            ...
            .with_restored(|| async {
                ...
                let result = kodex_app_server_daemon::start_with_features(&daemon_features).await;
    ...
    remove_legacy_tui_log_file(config.kodex_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        kodex_app_server_client::build_otel_provider(
            &config,
    ...
    let metrics = otel
        .as_ref()
        .and_then(kodex_otel::OtelProvider::metrics)
        .cloned();
    if let Some(metrics) = &metrics {
        let _ = kodex_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            kodex_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = kodex_state::install_process_db_telemetry(telemetry);
    }
    ...
    let daemon_settings = if metrics.is_some() {
        kodex_app_server_daemon::telemetry::settings_tags(&config.kodex_home)
            .await
            .to_vec()
    } else {
        Vec::new()
    ...
    let launch_telemetry = move |target: &AppServerTarget, connected: bool| {
        ...
        let _ = metrics.counter("kodex.tui.start", /*inc*/ 1, &launch_tags);
    };
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new(
                "kodex_core=info,kodex_tui=info,kodex_rmcp_client=info,kodex_realtime_webrtc=warn",
            )
        });
    ...
    let feedback = kodex_feedback::KodexFeedback::new();
    ...
    if let Ok(exit) = &app_result
        ...
        && let Some(metrics) = otel.as_ref().and_then(kodex_otel::OtelProvider::metrics)
    {
        ...
        let _ = metrics.counter("kodex.daemon.update", /*inc*/ 1, &tags);
    }
