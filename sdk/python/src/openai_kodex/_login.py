...
from .client import KodexClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncKodex needed by async login handles."""

    _client: AsyncKodexClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Kodex connection."""
        ...


def start_chatgpt_login(client: KodexClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: KodexClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Kodex.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Kodex.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncKodex.login_chatgpt()`."""

    _kodex: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._kodex._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._kodex._ensure_initialized()
        return await self._kodex._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncKodex.login_chatgpt_device_code()`."""
