...
async fn remote_project_trust_guards_thread_start_and_preserves_repository_decisions() -> Result<()>
{
    ...
    std::fs::create_dir(project_cwd.join(".kodex"))?;
    ...
    std::fs::write(kodex_home.join("config.toml"), &undecided_config)?;
    std::fs::write(
        project_cwd.join(".kodex/config.toml"),
    ...
    assert_eq!(
        std::fs::read_to_string(kodex_home.join("config.toml"))?,
        undecided_config
    );
    ...
    std::fs::create_dir(project_root.join(".kodex"))?;
    std::fs::write(
    ...
    std::fs::remove_dir_all(project_root.join(".kodex"))?;
    ...
    std::fs::remove_file(project_cwd.join(".kodex/config.toml"))?;
    std::fs::remove_dir(project_cwd.join(".kodex"))?;
