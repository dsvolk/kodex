...
usage() {
  cat <<'EOF'
Usage: build-kodex-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-bin <path>] \
  [--zsh-manifest <path>] \
  [--kodex-command-runner-bin <path>] \
  [--kodex-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --kodex-command-runner-bin)
      resource_args+=(
        --kodex-command-runner-bin
        "${2:?--kodex-command-runner-bin requires a value}"
      )
    ...
    --kodex-windows-sandbox-setup-bin)
      resource_args+=(
        --kodex-windows-sandbox-setup-bin
        "${2:?--kodex-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="kodex"
    entrypoint="kodex"
    archive_stem="kodex-package"
    ;;
  app-server)
    variant="kodex-app-server"
    entrypoint="kodex-app-server"
    archive_stem="kodex-app-server-package"
    ;;
  *)
    echo "No Kodex package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/kodex-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/kodex-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/kodex-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--kodex-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--kodex-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_kodex_package.py"
  --target "$target"
  --variant "$variant"
...
if [[ -n "$voice_release_dir" ]]; then
  ...
  rm -rf "$voice_package"
  voice_helper="${voice_release_dir%/}/kodex-voice-host${exe_suffix}"
...
from kodex_package.archive import write_archive
