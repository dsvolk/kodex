...
use kodex_analytics::GuardianV2Event;
use kodex_analytics::GuardianV2EventKind;
use kodex_core::GuardianAuthorizationVersion;
use kodex_core::GuardianRootSnapshot;
...
use kodex_core::context::GuardianReviewEvidenceFragment;
use kodex_core::context::GuardianReviewEvidenceRecord;
...
use kodex_guardian_context::ReviewEvidence;
use kodex_guardian_context::render_review_evidence;
...
pub(super) struct Classification {
    ...
    pub(super) thread: Arc<KodexThread>,
            ...
            for (section, cost) in transcript.section_costs() {
                for (measurement, value) in cost.measurements() {
                    metrics.histogram_with_boundaries(
                        kodex_guardian_context::SECTION_COST_METRIC,
                        i64::try_from(value).unwrap_or(i64::MAX),
                        kodex_guardian_context::SECTION_COST_BOUNDARIES,
                        &[
                            ("target", "async"),
                            ("section", section),
