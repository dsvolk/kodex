...
use anyhow::Result;
use anyhow::ensure;
use kodex_windows_sandbox::PreparedWindowsSandboxCleanup;
use kodex_windows_sandbox::resolve_sid;
use kodex_windows_sandbox::revoke_ace;
...
pub(super) fn clean_up(
    ...
    let kodex_home = installation.kodex_home.clone();
        ...
        with_owner_impersonation(installation.user_token.0, || {
            ...
            if let Some(home) = &kodex_home {
                if desktop.created_kodex_home {
                    ...
                    record_result(
                        "remove desktop-created kodex home",
                        std::fs::remove_dir_all(home),
                    );
                } else {
                    prune_kodex_home = true;
                    log_cleanup("skipping recursive kodex home removal: existing CLI home");
                    // Preserve CLI data without leaving inherited permissions for the deleted group.
                    record_result(
                        "remove kodex home sandbox permissions",
                        resolve_sid("KodexSandboxUsers")
                ...
                log_cleanup("skipping kodex home: no pinned home");
            }
            ...
            if desktop.cache_home.is_dir() {
                match crate::ipc::pin_existing_ancestors(
                    ...
                    Ok(()) => {
                        record_result(
                            "remove kodex runtime cache",
                            std::fs::remove_dir_all(desktop.cache_home.join("kodex-runtimes")),
                        );
    ...
    if prune_kodex_home && let Some(home) = &kodex_home {
        ...
        if let Err(error) = with_owner_impersonation(installation.user_token.0, || {
            ...
            remove_empty_directory(home, "kodex home");
            Ok(())
        }) {
            log_error(
                EVENT_CLEANUP_DETAIL,
                &format!("remove empty kodex home: failed, {error:#}"),
            );
        }
    }
    crate::service::log_information(
