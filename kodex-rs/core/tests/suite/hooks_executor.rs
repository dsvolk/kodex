...
async fn executor_stop_hook_runs_after_attachment() -> Result<()> {
    ...
    let call = &calls[0];
    let turn_metadata = &call["params"]["_meta"]["x-kodex-turn-metadata"];
...
async fn executor_interrupt_hook_runs_after_attachment() -> Result<()> {
    ...
    fixture.test.kodex.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.test.kodex, |event| {
    ...
    assert_eq!(call["_meta"]["x-kodex-turn-metadata"]["turn_id"], *turn_id);
    Ok(())
}
...
async fn executor_browser_and_computer_use_cleanup_hooks_use_separate_mcp_routes(
    ...
    if hook_event == "SubagentStop" {
        let child = fixture
            ...
            .start_thread(StartThreadOptions {
                ...
                environments: Some(fixture.test.kodex.environment_selections().await),
                ..StartThreadOptions::new(fixture.test.config.clone())
            })
            .await?;
        fixture.test.kodex = child.thread;
    ...
    assert!(node_params["_meta"].get("_kodex_apps").is_none());
    ...
    assert_eq!(browser_params["_meta"]["_kodex_apps"], routing);
    let turn_metadata = &browser_params["_meta"]["x-kodex-turn-metadata"];
    assert_eq!(
        node_params["_meta"]["x-kodex-turn-metadata"],
        *turn_metadata
    );
...
async fn executor_plugin_hook_fixture(
    ...
    for (plugin_id, manifest) in plugins {
        ...
        let plugin_directory = plugin_root.join(".kodex-plugin")?;
...
impl ExecutorHookFixture {
    ...
    async fn interrupt_running_command(&self) -> Result<()> {
        wait_for_event(&self.test.kodex, |event| {
            matches!(event, EventMsg::ExecCommandBegin(_))
        })
        .await;
        self.test.kodex.submit(Op::Interrupt).await?;
        wait_for_event(&self.test.kodex, |event| {
            matches!(event, EventMsg::TurnAborted(_))
        })
        .await;
