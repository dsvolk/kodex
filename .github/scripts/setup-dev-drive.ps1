...
if ((Test-Path "D:\") -and (Test-DevDrive "D:")) {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "kodex-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "KodexDevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop
