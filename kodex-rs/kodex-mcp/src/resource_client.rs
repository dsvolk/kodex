...
use kodex_protocol::mcp::ResourceContent;
use kodex_rmcp_client::CancellableEventStreamRequest;
...
impl McpResourceClient {
    ...
    /// Lists one Kodex Apps resource page using plugin-service's top-level `mimeType` parameter.
    pub async fn list_kodex_apps_resources(
        &self,
        params: KodexAppsResourceListParams,
    ) -> Result<McpResourcePage> {
        let params = serde_json::to_value(params)
            .context("failed to serialize Kodex Apps resource params")?;
        let connections = self.runtime.latest_host_owned_kodex_apps_connections()?;
        let (managed, timeout) = connections
            .client_by_name(KODEX_APPS_MCP_SERVER_NAME)
            .await?;
        let result = managed
            .client
            .send_custom_request_with_timeout("resources/list", Some(params), timeout)
            .await
            .context("resources/list failed for `kodex_apps`")?;
    ...
    pub async fn list_events(&self) -> Result<McpEventCatalogSnapshot> {
        let (connections, _) = self
            .runtime
            .latest_connections_for_event_server(KODEX_APPS_MCP_SERVER_NAME)?;
