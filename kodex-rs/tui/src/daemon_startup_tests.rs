...
fn audited_overrides_allow_daemon_without_allowing_arbitrary_config() {
    for (raw, eligible) in [
        ...
        let overrides = kodex_utils_cli::CliConfigOverrides {
...
async fn daemon_connection_rejects_unprotected_socket_before_handshake() -> color_eyre::Result<()> {
    ...
    let mut listener = kodex_uds::UnixListener::bind(socket_path.as_path()).await?;
...
fn daemon_eligibility_preserves_launch_options_and_explains_exclusions() {
    ...
    for (args, expected) in [
        ...
        let cli = Cli::parse_from(std::iter::once("kodex").chain(args.split_whitespace()));
        assert_eq!(
    ...
    let mut cli = Cli::parse_from(["kodex"]);
    ...
    for (kv, loader, workload, executor, expected) in [
        ...
        (
            ...
            "executor selection (KODEX_EXEC_SERVER_URL)",
        ),
    ] {
        assert_eq!(
