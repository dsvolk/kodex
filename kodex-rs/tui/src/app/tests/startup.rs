...
async fn windows_sandbox_setup_uses_observed_thread_host() {
    ...
    for (ids, expected) in [
        ...
        (
            Some(vec![kodex_exec_server::LOCAL_ENVIRONMENT_ID]),
            WindowsSandboxHost::Local,
        ),
        (Some(vec!["remote"]), WindowsSandboxHost::Remote),
        (
            Some(vec!["remote", kodex_exec_server::LOCAL_ENVIRONMENT_ID]),
            WindowsSandboxHost::Mixed,
        ),
    ] {
        let environments = ids.map(|ids| {
            ids.into_iter()
                .map(|id| kodex_app_server_protocol::ThreadEnvironment {
                    environment_id: id.to_string(),
                    cwd: kodex_utils_absolute_path::AbsolutePathBuf::try_from(std::env::temp_dir())
                        .unwrap()
                        .into(),
...
async fn uncertain_windows_sandbox_setup_keeps_intent_and_input_locked() {
    ...
    for response in [
        ...
        if transport_error {
            ...
            insta::assert_snapshot!(rendered, @"■ Windows sandbox setup request timed out. Waiting for completion; restart Kodex if it does not finish.");
        }
    }
}
...
fn startup_bottom_pane() -> (BottomPane, UnboundedReceiver<AppEvent>) {
    ...
    (
        BottomPane::new(BottomPaneParams {
            ...
            placeholder_text: "Ask Kodex to do anything".to_string(),
...
async fn startup_draft_handoff_recognizes_late_user_input_as_new_protected_view() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        assert!(
            !matches!(event, AppEvent::KodexOp(Op::UserInputAnswer { .. })),
...
async fn remote_overview_startup_hides_disabled_older_server_notice() -> Result<()> {
    ...
    let rendered = render_bottom_popup(&app.chat_widget, /*width*/ 80);
    assert!(rendered.contains("Service v2.0.0 < Kodex CLI v2.1.0"));
    app.pending_server_version_notice =
        Some(crate::status::remote_connection::ServerVersionNotice {
    ...
    app.handle_app_server_event(
        ...
        kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::CurrentTimeRead {
                ...
                params: kodex_app_server_protocol::CurrentTimeReadParams {
                    thread_id: other_thread_id.to_string(),
                },
            },
        )),
...
async fn startup_thread_started_does_not_replay_resolved_approval() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(request.clone())),
    )
    .await;
    app.handle_app_server_event(
        &app_server,
...
async fn windows_sandbox_config_refresh_uses_connected_server() -> Result<()> {
    ...
    app.chat_widget.windows_sandbox_config.mode =
        Some(kodex_app_server_protocol::WindowsSandboxSetupMode::Elevated);
    assert!(app.refresh_windows_sandbox_config(&server).await);
    let loaded_config = crate::windows_sandbox::WindowsSandboxConfig {
        mxc_selected: false,
        mode: Some(kodex_app_server_protocol::WindowsSandboxSetupMode::Unelevated),
