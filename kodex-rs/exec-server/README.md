...
`kodex-exec-server` is the library backing `kodex exec-server`, a small
...
filesystem operations and `kodex-linux-sandbox`.

## Transport

The server speaks the exec-specific `kodex-exec-server-protocol` message
...
It uses the standard Kodex ChatGPT sign-in state; run `kodex login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `KODEX_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Kodex then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `KODEX_API_KEY`;
Kodex sends it as a bearer token on the registration request. For example:

```sh
KODEX_API_KEY="$OPENAI_API_KEY" \
kodex exec-server \
  --remote ... \
...
exec-server JSON-RPC over the authenticated WebSocket. The existing Kodex Noise
...
`kodex.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/kodex.exec_server.relay.v1.proto`. The relay frame carries stream
...
`kodex_build_info::build_id(commit, target)`, which CI can also call for an
...
- `arg0`: optional argv0 override forwarded to `kodex-utils-pty`.
...
provider from Kodex auth state before starting remote mode.
