...
use kodex_execpolicy::AmendError;
use kodex_execpolicy::Decision;
use kodex_execpolicy::Error as ExecPolicyRuleError;
use kodex_execpolicy::Evaluation;
use kodex_execpolicy::MatchOptions;
use kodex_execpolicy::NetworkRuleProtocol;
use kodex_execpolicy::Policy;
use kodex_execpolicy::PolicyParser;
use kodex_execpolicy::RequirementsExecPolicy;
use kodex_execpolicy::RuleMatch;
use kodex_execpolicy::blocking_append_allow_prefix_rule;
use kodex_execpolicy::blocking_append_network_rule;
...
use kodex_protocol::permissions::FileSystemSandboxKind;
use kodex_protocol::protocol::AskForApproval;
use kodex_shell_command::is_dangerous_command::DangerousCommandMatch;
use kodex_shell_command::is_dangerous_command::DangerousCommandPlatform;
use kodex_shell_command::is_dangerous_command::dangerous_command_match_for_platform;
use thiserror::Error;
use tokio::fs;
...
use kodex_shell_command::bash::parse_shell_lc_plain_commands;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(kodex_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(kodex_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &kodex_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            kodex_shell_command::is_dangerous_command::dangerous_powershell_words_match(
                command,
                command_platform,
            )
        }
...
pub(crate) fn default_policy_path(kodex_home: &Path) -> PathBuf {
    kodex_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy_for_platform(
    ...
    if command_platform == DangerousCommandPlatform::Windows
        && let Some(commands) =
            kodex_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
        && !commands.is_empty()
    {
        return ExecPolicyCommands {
            commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and kodex prompts the user to run outside the sandbox
