...
fn user_layer(kodex_home: &TempDir, config: &str) -> ConfigLayerEntry {
...
fn stack(kodex_home: &TempDir, user: &str, session: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![
            user_layer(kodex_home, user),
            ConfigLayerEntry::new(
...
fn session_flags_can_reenable_user_disabled_path() {
    let kodex_home = TempDir::new().expect("temp dir");
    let skill_path = kodex_home.path().join("skills/demo/SKILL.md");
...
fn path_rule_disables_selected_path() {
    let kodex_home = TempDir::new().expect("temp dir");
    let path = kodex_home.path().join("disable-by-path/SKILL.md").abs();
...
fn later_name_rule_reenables_path_disabled_skill() {
    let kodex_home = TempDir::new().expect("temp dir");
    let path = kodex_home.path().join("reenable-by-name/SKILL.md").abs();
    let rules = SkillConfigRules {
        entries: vec![
            SkillConfigRule {
                selector: SkillConfigRuleSelector::Path(path.clone()),
...
fn later_path_rule_reenables_one_skill_disabled_by_name() {
    let kodex_home = TempDir::new().expect("temp dir");
    let root = kodex_home.path().join("reenable-by-path");
