...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter. In explicit CLI/API fallback mode, keep `gpt-image-2` when a flat chroma-key background plus local alpha extraction with `python "${KODEX_HOME:-$HOME/.kodex}/skills/.system/imagegen/scripts/remove_chroma_key.py"` is acceptable.
