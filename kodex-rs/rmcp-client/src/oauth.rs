...
//!
//! If the keyring is not available or fails, we fall back to KODEX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Kodex MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let kodex_home = find_kodex_home().map_err(anyhow::Error::from)?;
...
fn compute_store_key(server_name: &str, server_url: &str) -> Result<String> {
    ...
    let payload = if enterprise_owned {
        ...
        // isolated by Kodex profile as well as authenticated user and workspace.
        let kodex_home = find_kodex_home()?;
        fs::create_dir_all(&kodex_home)?;
        payload.insert(
            "kodex_home".to_string(),
            serde_json::to_value(kodex_home.as_path().canonicalize()?)?,
        );
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_kodex_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    fn fallback_file_is_private_at_creation() -> Result<()> {
        use std::os::unix::fs::PermissionsExt;
        const CHILD: &str = "KODEX_TEST_OAUTH_PERMISSIVE_UMASK";
