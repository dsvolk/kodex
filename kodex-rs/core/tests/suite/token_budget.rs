...
use kodex_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
...
use kodex_protocol::protocol::CONTEXT_WINDOW_OPEN_TAG;
...
async fn assert_context_compaction_item_lifecycle(kodex: &std::sync::Arc<kodex_core::KodexThread>) {
...
async fn token_budget_guidance_precedes_standalone_context_window(
    ...
    let backend_url = format!("{}/backend-api/kodex", server.uri());
    let test = test_kodex()
        .with_model_info_override("gpt-5.2", |model| {
            model.supports_experimental_context = true;
        })
        .with_auth(KodexAuth::from_external_chatgpt_tokens(
...
#[test_case("OpenAI", "/backend-api/kodex", None, true, true; "kodex_backend")]
#[test_case("OpenAI", "/backend-api/kodex", None, false, false; "unsupported_model")]
#[test_case("Custom", "/backend-api/kodex", None, true, false; "custom_provider")]
#[test_case("OpenAI", "/v1", None, true, false; "non_kodex_endpoint")]
#[test_case("OpenAI", "/backend-api/kodex", Some("test-provider-token"), true, false; "provider_credentials")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn experimental_context_requires_capable_model_and_kodex_backend(
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_kodex()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.kodex, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert_context_compaction_item_lifecycle(&test.kodex).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.kodex, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.kodex, |event| match event {
        EventMsg::HookCompleted(completed)
