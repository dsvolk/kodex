...
const RESOURCE_URI: &str = "memo://kodex/example-note";

fn stdio_server_bin() -> Result<PathBuf, CargoBinError> {
    kodex_utils_cargo_bin::cargo_bin("test_stdio_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("kodex-test", "0.0.0-test").with_title("Kodex rmcp resource test"),
    )
...
async fn rmcp_client_can_list_and_read_resources() -> anyhow::Result<()> {
    ...
    let mut expected_templates = ListResourceTemplatesResult::with_all_items(vec![
        rmcp::model::ResourceTemplate::new("memo://kodex/{slug}", "kodex-memo")
...
async fn rmcp_client_preserves_each_resource_error() -> anyhow::Result<()> {
    let client = resource_client().await?;
    for uri in ["memo://kodex/missing-first", "memo://kodex/missing-second"] {
        let error = client
            .read_resource(
