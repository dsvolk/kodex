...
fn managed_ca_dir() -> Result<PathBuf> {
    let kodex_home =
        find_kodex_home().context("failed to resolve KODEX_HOME for managed MITM CA")?;
    Ok(kodex_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Kodex-generated MITM CA bundle.
