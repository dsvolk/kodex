    ...
    fn resolve_agent(&self, path: AgentPath) -> BoxFuture<'_, Result<ThreadId>> {
        Box::pin(async move {
            self.members
                ...
                .ok_or_else(|| KodexErr::InvalidRequest("unknown agent".into()))
        })
    }
    ...
    fn current_time(&self, _caller: ThreadId) -> BoxFuture<'_, Result<DateTime<Utc>>> {
        Box::pin(async move {
            Ok(DateTime::parse_from_rfc3339("2026-09-18T12:00:00Z")
                .map_err(|error| KodexErr::Io(std::io::Error::other(error)))?
                .with_timezone(&Utc)
    ...
    fn notify(
        ...
        Box::pin(async move {
            if self.fail_notifications.load(Ordering::SeqCst) {
                return Err(KodexErr::Io(std::io::Error::other(
...
fn board_tool_call(name: &str, args: serde_json::Value) -> kodex_tools::ToolCall<'static> {
    kodex_tools::ToolCall {
        turn_id: "turn-1".into(),
        call_id: name.into(),
        tool_name: kodex_tools::ToolName::namespaced("collaboration", name),
        model: "test".into(),
        kodex_turn_metadata: None,
        truncation_policy: kodex_utils_output_truncation::TruncationPolicy::Bytes(20_000),
        source: kodex_tools::ToolCallSource::Direct,
        conversation_history: kodex_tools::ConversationHistory::default(),
        turn_item_emitter: Arc::new(kodex_tools::NoopTurnItemEmitter),
        environments: vec![],
        payload: kodex_tools::ToolPayload::Function {
...
async fn tools_validate_arguments_deduplicate_calls_and_bound_unicode_results() {
    use kodex_tools::ToolCallSource;
    use kodex_tools::ToolName;
