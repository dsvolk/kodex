...
pub(super) fn run(
    ...
    on_authenticated_user: &dyn Fn(
        InstallationRecord,
        OwnedHandle,
        kodex_windows_sandbox::SetupRuntime,
    ...
    if request.refresh_only {
        ...
        // revoked. Resume only the same provisioned accounts; never repair setup.
            ...
            previous.user_sid == identity.user_sid
                && previous.kodex_home == identity.kodex_home
        ...
        for entry in &runtime.accounts {
            let account = entry.account.username();
                ...
                kodex_windows_sandbox::local_user_flags(account)?
                    .is_some_and(|flags| flags & (UF_ACCOUNTDISABLE | UF_PASSWORD_EXPIRED) == 0)
                    && string_from_sid_bytes(&kodex_windows_sandbox::resolve_sid(account)?)
    ...
    let result = (|| -> Result<()> {
        ...
        for account in [
            ...
            setup_complete &= kodex_windows_sandbox::local_user_flags(account)?
                .is_some_and(|flags| flags & (UF_ACCOUNTDISABLE | UF_PASSWORD_EXPIRED) == 0);
        }
        if !setup_complete {
            ...
            let setup_owner_sid = kodex_windows_sandbox::resolve_sid(&identity.account)?;
            ensure!(
            ...
            kodex_windows_sandbox::provision_sandbox_in_process(
                &identity.kodex_home,
                &identity.account,
                settings,
                identity.runtime,
