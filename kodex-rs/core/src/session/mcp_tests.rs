...
fn plugin_install_elicitation_telemetry_metadata_requires_install_tool_suggestion() {
    let event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        ...
        request: kodex_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
    ...
    let enable_event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        turn_id: Some("turn-1".to_string()),
        server_name: "kodex_apps".to_string(),
        id: kodex_protocol::mcp::RequestId::String("request-2".to_string()),
        request: kodex_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
                "kodex_approval_kind": "tool_suggestion",
                "suggest_type": "enable",
...
fn guardian_decisions_map_to_elicitation_responses_without_session_state() {
    let model = kodex_models_manager::model_info::model_info_from_slug("acting-model");
    assert_eq!(
    ...
    assert_eq!(
        ...
        ElicitationResponse {
            ...
            meta: Some(json!({
                "approvals_reviewer": ApprovalsReviewer::AutoReview,
                "message": kodex_prompts::ResolvedModelMessages::from_model(&model).auto_review().timeout_instructions,
            })),
        }
    );
    assert_eq!(
...
fn guardian_elicitation_timeout_uses_acting_model_instructions() {
    let mut model = kodex_models_manager::model_info::model_info_from_slug("acting-model");
