...
async fn daybreak_metadata_follows_the_actual_plugin_call(case: AccessCase) -> Result<()> {
    ...
    let tool_name = if matches!(
        ...
        "get_kodex_security_daybreak_access"
    };
    ...
    if remote_url.is_some() {
        ...
        let (config, _) = test.kodex.current_mcp_config_and_runtime_context().await;
        assert_eq!(
    ...
    test.kodex
        ...
        .await?;
    let end = wait_for_event(&test.kodex, |event| {
...
async fn daybreak_discards_in_flight_account_changes_without_apps() -> Result<()> {
        ...
        vec![
            responses::sse(vec![
                responses::ev_tool_search_call(
                    "search",
                    &json!({"query":"get_kodex_security_daybreak_access"}),
                ),
                responses::ev_completed("resp-1"),
            ]),
            responses::sse(vec![
    ...
    test.kodex
        ...
        .await?;
    let call = wait_for_event(&test.kodex, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    });
    let switch = async {
        let request =
