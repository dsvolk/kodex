...
fn isolated_bundle_test(name: &str) -> Result<bool> {
    const CHILD: &str = "KODEX_MEASUREMENT_REFERENCE_TEST";
...
fn archive(plugin_name: &str) -> Result<Vec<u8>> {
    ...
    for (name, bytes) in [
        (".kodex-plugin/plugin.json", manifest.as_str()),
...
impl Fixture {
    async fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            executor,
            config,
            auth: KodexAuth::create_dummy_chatgpt_auth_for_testing(),
...
async fn narrow_preparation_reuses_versions_and_keeps_old_leases_alive() -> Result<()> {
    ...
    catalog(&f.server, vec![installed(&f.server, "2.0.0")]).await;
    bundle(&f.server, "2.0.0", /*count*/ 1).await?;
    f.auth = KodexAuth::from_external_chatgpt_tokens(
        "header.e30.other",
        "other-account",
...
async fn preparation_requires_enabled_matching_installed_global_plugin() -> Result<()> {
    ...
    for (enabled, auth) in [
        (false, f.auth.clone()),
        (true, KodexAuth::from_api_key("test")),
    ] {
    ...
    f.auth = KodexAuth::create_dummy_chatgpt_auth_for_testing();
