//! Shared implementation for `kodex archive`, `kodex delete`, and `kodex unarchive`.
//!
...
pub async fn run_session_archive_command(
    ...
    let kodex_home = find_kodex_home().wrap_err("failed to find Kodex home")?;
    let mut app_server =
        start_app_server_for_session_command(options, kodex_home.to_path_buf()).await?;
...
async fn run_session_archive_action_with_app_server(
    ...
    let resolved = resolve_session_target(app_server, kodex_home, action, target).await?;
...
pub(super) async fn start_app_server_for_session_command(
    ...
    let workload_identity_selected = kodex_login::is_workload_identity_selected();
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(kodex_home.as_path()).await
    } else {
        None
    };
