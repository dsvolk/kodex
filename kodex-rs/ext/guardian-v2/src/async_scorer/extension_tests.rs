...
use kodex_core::context::NodeReplReviewEvidence;
...
use kodex_extension_api::ToolCallSource;
use kodex_extension_api::ToolName;
use kodex_extension_api::ToolPayload;
use kodex_extension_api::ToolStartInput;
use kodex_features::Feature;
use kodex_guardian_context::truncate_text as truncate_entry;
...
use kodex_protocol::openai_models::GuardianV2ModelConfig;
use kodex_protocol::openai_models::GuardianV2TranscriptModelConfig;
...
use kodex_protocol::security_risk::SecurityRiskScore;
...
use kodex_features::GuardianV2ReviewScopeConfigToml;
use kodex_protocol::openai_models::GuardianModelPolicy;
...
impl ExternalAuth for RefreshableAuth {
    fn resolve(&self) -> ExternalAuthFuture<'_, KodexAuth> {
        Box::pin(async { Ok(KodexAuth::from_api_key(*self.0.lock().expect("auth"))) })
    }

    fn refresh(&self, _: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, KodexAuth> {
...
) -> kodex_config::GuardianPolicyLoader {
    kodex_config::GuardianPolicyLoader::new(
        Some(&kodex_features::FeatureToml::Config(
            kodex_features::GuardianV2ConfigToml {
                enabled: Some(true),
                review_scope: scope.cloned(),
                ..Default::default()
            },
        )),
        &kodex_config::ConfigRequirements::default(),
    )
}
...
async fn installed_extension_warms_connections_without_blocking_thread_start() -> Result<()> {
    ...
    let thread_store = test.kodex.thread_extension_data();
...
async fn installed_extension_uses_http_after_warm_socket_auth_expires() -> Result<()> {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("original"));
    auth_manager
...
impl RecordingMetrics {
    fn classification_samples(&self) -> Vec<RecordedMetric> {
        self.0.lock().unwrap().iter().filter(|sample| {
            !matches!(sample, RecordedMetric::Histogram(name, _, _) if name == kodex_guardian_context::SECTION_COST_METRIC || name == kodex_guardian_context::REQUEST_TOKENS_METRIC)
        }).cloned().collect()
    }
}
...
impl ExtensionMetrics for RecordingMetrics {
    ...
    fn histogram(&self, name: &str, value: i64, tags: &[(&str, &str)]) {
        if name == "kodex.guardian_v2.connection.duration_ms" {
            return;
        }
...
fn user_instruction(text: &str) -> ResponseItem {
    ResponseItem::Message {
        ...
        internal_chat_message_metadata_passthrough: Some(InternalChatMessageMetadataPassthrough {
            content_item_kinds: Some(vec![kodex_protocol::models::ContentItemKind(
                "user.text".to_owned(),
            )]),
            ..Default::default()
        }),
...
struct TestRetainedHistory {
    ...
    retained_context: Option<kodex_history::RetainedContext>,
}

impl ConversationHistorySnapshot for TestRetainedHistory {
    fn retained_context(&self) -> Option<&kodex_history::RetainedContext> {
        self.retained_context.as_ref()
    }

    fn latest_compaction(&self) -> Option<kodex_history::CompactionCheckpoint<'_>> {
        self.items()
            .filter_map(|item| {
                kodex_history::CompactionCheckpoint::from_item(
                    item,
...
async fn sandboxed_shell_classification_respects_review_scope() -> Result<()> {
    ...
    let thread_store = fixture.test.kodex.thread_extension_data();
...
async fn computer_use_only_scores_cannot_approve_other_actions() -> Result<()> {
    ...
    let authorization =
        super::super::authorization::ScoreAuthorization::current(&fixture.test.kodex).await;
    ...
    let mut changed_model = fixture
        ...
        .await;
    changed_model.guardian = Some(kodex_protocol::openai_models::GuardianModelPolicy {
        computer_use: Some(kodex_protocol::openai_models::GuardianReviewMode::Adaptive),
        ..Default::default()
    });
...
) -> Result<(serde_json::Value, TestKodex, ExtensionRegistry<Config>)> {
...
async fn sample_configured_conversation_history_with_source(
    ...
    let builder = test_kodex()
        .with_extensions(Arc::new(extensions.build()))
        .with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model_info_override("kodex-auto-review", |model_info| {
            model_info
                .model_messages
                .as_mut()
    ...
    let conversation_history = test.kodex.conversation_history_snapshot().await;
...
async fn unscored_tools_invalidate_cached_scores() -> Result<()> {
    ...
    let mut model = thread_store
        ...
        .as_ref()
        .clone();
    model.guardian = Some(kodex_protocol::openai_models::GuardianModelPolicy {
        computer_use: Some(kodex_protocol::openai_models::GuardianReviewMode::Adaptive),
        ..Default::default()
    });
    thread_store.insert(model);
...
impl GuardianFailureFixture {
    ...
    async fn score_tool(&self, tool_name: ToolName) {
        let thread_store = self.test.kodex.thread_extension_data();
        set_cached_score(
            thread_store,
            SecurityRiskScore {
                scores: BTreeMap::from([("action_risk".to_owned(), 0.25)]),
...
async fn contributor_fails_closed_when_luna_classification_fails() -> Result<()> {
    ...
    fixture.registry.thread_lifecycle_contributors()[0]
        .on_thread_start(ThreadStartInput {
            ...
            extension_metrics: None,
            session_store: &fixture.session_store,
            thread_store: fixture.test.kodex.thread_extension_data(),
        })
        .await;
    fixture
        .test
...
"#;
    ...
    assert!(metrics.0.lock().unwrap().iter().any(|sample| {
        ...
        if name == kodex_guardian_context::SECTION_COST_METRIC
            && *value > 0
            && tags == &[
    ...
    seed_cached_score(
        ...
        thread_store,
        first_unscored,
        ScoreAuthorization::current(&test.kodex).await,
    );
    assert_eq!(
        cached_approval(
            &registry,
...
async fn assert_luna_pool_context(thread_context_enabled: bool) -> Result<()> {
    ...
    assert_eq!(
        request["client_metadata"]["x-kodex-window-id"],
    ...
    let mut expected_content = json!([
        ...
        {
            "type": "input_text",
            "text": "The Kodex agent has requested the following action:\n"
        },
...
async fn contributor_skips_required_models_in_standard_scope() -> Result<()> {
    ...
    let _ = kodex_core::guardian_review::prepare_review_prewarm(&test.kodex).await?;
    let authorization = ScoreAuthorization::current(&test.kodex).await;
    let progress = thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("Guardian v2 should track score progress per thread");
...
async fn assert_compaction_approval_policy(thread_context_enabled: bool) -> Result<()> {
    ...
    let authorization = fixture.test.kodex.guardian_authorization_version().await;
    fixture
        .test
        .kodex
    ...
    thread_store.insert(model);
    assert_eq!(
        fixture.test.kodex.guardian_authorization_version().await,
        authorization
    );
    let score_authorization = ScoreAuthorization::current(&fixture.test.kodex).await;
    let progress = thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("score progress");
    seed_cached_score(
...
async fn assert_parent_compaction_reuse(thread_context_enabled: bool) -> Result<()> {
    ...
    let conversation_history = TestRetainedHistory {
        ...
        retained_context: thread_context_enabled.then(kodex_history::RetainedContext::default),
    };
    thread_store.insert(parent_model);
        ...
        .on_tool_start(ToolStartInput {
            ...
            conversation_history: Arc::new(TestRetainedHistory {
                ...
                retained_context: thread_context_enabled
                    .then(kodex_history::RetainedContext::default),
...
struct CacheMiss;
impl kodex_extension_api::SynchronousApprovalReviewer for CacheMiss {
    fn review(
        &self,
        _reason: kodex_protocol::approvals::GuardianReviewReason,
    ) -> kodex_extension_api::ExtensionFuture<'_, Option<ReviewDecision>> {
...
async fn cached_approval_discounts_only_its_own_unscored_wrapper() -> Result<()> {
    skip_if_no_network!(Ok(()));
    let fixture = GuardianFailureFixture::new().await?;
    let store = fixture.test.kodex.thread_extension_data();
    ...
    seed_cached_score(
        ...
        store,
        wrapper,
        ScoreAuthorization::current(&fixture.test.kodex).await,
    );
...
async fn cached_approval(
    registry: &kodex_extension_api::ExtensionRegistry<Config>,
    store: &ExtensionData,
    action: &str,
    ...
    let category = match review_scope(&action) {
        ...
        None if store
            ...
            .other_tools
            == kodex_protocol::openai_models::GuardianReviewMode::Adaptive =>
        {
            kodex_protocol::openai_models::GuardianScope::Shell
        }
        None => {
    ...
    let input = kodex_extension_api::ApprovalDecisionInput {
        approval_id: "cache-probe",
        tool_call_id: action.get("id").and_then(serde_json::Value::as_str),
        action: &action,
        thread_id: kodex_protocol::ThreadId::from_string(store.level_id()).unwrap(),
        thread_store: store,
        category,
        approval_policy: kodex_protocol::protocol::AskForApproval::OnRequest,
    ...
    match registry.decide_approval(&input).await {
        Some(kodex_extension_api::ApprovalDecision::Allow) => Some(ReviewDecision::Approved),
        _ => None,
    }
}
...
async fn cached_score_publication_rejects_delayed_results_without_changing_coverage() -> Result<()>
{
    ...
    let store = fixture.test.kodex.thread_extension_data();
    let progress = store.get::<GuardianV2ScoreProgress>().unwrap();
    let authorization = ScoreAuthorization::current(&fixture.test.kodex).await;
