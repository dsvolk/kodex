...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("kodex.turns", /*inc*/ 1, &[("source", "test")])?;
    metrics.counter("kodex.api_request", /*inc*/ 1, &[("status", "200")])?;
    ...
    metrics.counter("kodex.tool.call", /*inc*/ 1, &[("tool", "test")])?;
    metrics.record_duration(
        "kodex.tool.call.duration_ms",
        Duration::from_millis(42),
        &[("tool", "test")],
    )?;
    metrics.histogram(
        "kodex.turn.token_usage",
        /*value*/ 100,
        &[("token_type", "total")],
    )?;
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "kodex.test.log_exported",
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "kodex.test.trace_event",
