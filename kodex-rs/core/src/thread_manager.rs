...
use kodex_agent_graph_store::AgentGraphStore;
use kodex_agent_graph_store::LocalAgentGraphStore;
...
use kodex_code_mode::CodeModeSessionProvider;
use kodex_code_mode::DisabledCodeModeSessionProvider;
use kodex_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use kodex_extension_api::ThreadInstructionsProvider;
...
use kodex_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use kodex_thread_store::LocalThreadStoreConfig;
use kodex_thread_store::MoveThreadToSectionParams;
use kodex_thread_store::PreparedFork;
use kodex_thread_store::ReadThreadByRolloutPathParams;
use kodex_thread_store::ReadThreadParams;
use kodex_thread_store::StoredModelContext;
use kodex_thread_store::StoredThread;
use kodex_thread_store::ThreadMetadataPatch;
...
struct TempKodexHomeGuard {
    path: PathBuf,
}

impl Drop for TempKodexHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Kodex thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<KodexThread>,
...
pub struct ThreadManager {
    ...
    _test_kodex_home_guard: Option<TempKodexHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<kodex_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    ...
    pub(crate) threads: Arc<RwLock<HashMap<ThreadId, Arc<KodexThread>>>>,
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            if has_state_db && background_migration_enabled {
                ...
                let kodex_home = config.kodex_home.to_path_buf();
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        kodex_apps_tools_cache: KodexAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    /// Construct with a dummy AuthManager containing the provided KodexAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let kodex_home = std::env::temp_dir().join(format!(
            "kodex-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&kodex_home)
            .unwrap_or_else(|err| panic!("temp kodex home dir create failed: {err}"));
        ...
        manager._test_kodex_home_guard = Some(TempKodexHomeGuard { path: kodex_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided KodexAuth and kodex home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_kodex_home = match AbsolutePathBuf::from_absolute_path_checked(&kodex_home) {
            Ok(kodex_home) => kodex_home,
            Err(err) => panic!("test kodex_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        let restriction_product = SessionSource::Exec.restriction_product();
        let skills_service = Arc::new(HostSkillsService::new_with_restriction_product(
            absolute_kodex_home.clone(),
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: kodex_state::SqliteConfig::new_for_testing(absolute_kodex_home),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(kodex_home, /*config_model_catalog*/ None),
                git_root_discovery: Arc::default(),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> KodexResult<Arc<KodexThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `KodexThread`/`LiveThread`, so metadata changes stay ordered
        ...
        let updated = self
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    KodexErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    KodexErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> KodexResult<NewThread> {
    ...
    pub async fn ensure_multi_agent_v2_child_loaded(
        ...
        let parent = self.get_thread(parent_thread_id).await.map_err(|_| {
            KodexErr::InvalidRequest(format!(
    ...
    /// as `Arc<KodexThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<KodexThread>> {
    ...
    pub async fn remove_thread_for_client(
        &self,
        thread_id: &ThreadId,
    ) -> KodexResult<Option<Arc<KodexThread>>> {
    ...
    pub async fn remove_thread_if_matches(
        &self,
        thread_id: &ThreadId,
        expected: &Arc<KodexThread>,
    ) -> Option<Arc<KodexThread>> {
        let mut threads = self.state.threads.write().await;
        if threads
            .get(thread_id)
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> KodexResult<Arc<KodexThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(KodexErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => KodexErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> KodexResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<KodexThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(KODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> KodexResult<NewThread> {
        ...
        let isolation = thread_extension_init
            ...
            .unwrap_or_else(|| {
                if crate::guardian::is_basic_session_source(&session_source) {
                    kodex_extension_api::SessionIsolation::Isolated
                } else {
                    kodex_extension_api::SessionIsolation::Inherit
                }
            });
        ...
        let (instructions, inherited_exec_policy, extensions, mcp_manager, multi_agent_version) =
            if isolation == kodex_extension_api::SessionIsolation::Isolated {
                (
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(KodexErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(KodexThread::new(
                    session,
                    io,
        ...
        Err(KodexErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> kodex_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return kodex_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(kodex_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> KodexErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => KodexErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => KodexErr::InvalidRequest(message),
        err => KodexErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> KodexErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => KodexErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => KodexErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => KodexErr::UnsupportedOperation(format!(
