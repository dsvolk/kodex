...
const MEMO_URI: &str = "memo://kodex/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "kodex/sandbox-state-meta";
...
impl TestToolServer {
    fn new() -> Self {
        ...
        let entitlement_tools = std::env::var("MCP_TEST_DAYBREAK_READ_ONLY")
            ...
            .into_iter()
            .flat_map(|read_only| {
                ["get_kodex_security_daybreak_access", "get_daybreak_access"].map(|name| {
                    ...
                    if name == "get_kodex_security_daybreak_access" {
    ...
    /// Tool intended for manual testing of Kodex TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Kodex TUI):
    /// - Build this binary: `cargo build -p kodex-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `kodex mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Kodex TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://kodex/{slug}", "kodex-memo")
            .with_title("Kodex Memo")
            .with_description("Template for memo://kodex/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise Kodex TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("kodex-rmcp-test-server", env!("CARGO_PKG_VERSION"))
        ...
        match request.name.as_ref() {
            "js" => {
                ...
                if args.code == "nodeRepl.fail()" {
                    ...
                    let mut meta = MetaObject::new();
                    meta.insert("kodex/imageDetail".to_string(), json!("low"));
                    Ok(CallToolResult::success(vec![
            ...
            "sandbox_meta" | "get_kodex_security_daybreak_access" | "get_daybreak_access" => Ok(
            ...
            "encrypted_output" => {
                let mut meta = MetaObject::new();
                meta.insert("kodex/encryptedContent".to_string(), json!(true));
