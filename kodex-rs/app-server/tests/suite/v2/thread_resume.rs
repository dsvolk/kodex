...
use kodex_app_server_protocol::ActivePermissionProfile;
use kodex_app_server_protocol::ApprovalsReviewer;
...
use kodex_app_server_protocol::McpToolCallAppContext;
...
use kodex_app_server_protocol::SandboxPolicy as AppSandboxPolicy;
...
use kodex_app_server_protocol::ThreadActiveFlag;
...
use kodex_core::ARCHIVED_SESSIONS_SUBDIR;
use kodex_exec_server::LOCAL_ENVIRONMENT_ID;
use kodex_features::Feature;
use kodex_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use kodex_protocol::protocol::ThreadSettingsAppliedEvent;
...
use kodex_rollout::append_rollout_item_to_path;
use kodex_rollout::read_session_meta_line;
...
async fn thread_resume_paginated_model_context_preserves_original_metadata() -> Result<()> {
    ...
    let saved_cwd = normalized_existing_path(kodex_home.path())?;
    mock_responses_config(&server.uri()).write(kodex_home.path())?;
    ...
    let path = rollout_path(kodex_home.path(), "2025-01-05T12-00-00", &conversation_id);
...
async fn cold_resume_workspace_roots_honor_owned_snapshot_and_caller_overrides() -> Result<()> {
    ...
    let kodex_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(kodex_home.path())?;
    let saved_cwd = normalized_existing_path(kodex_home.path())?.abs();
    ...
    let path = rollout_path(kodex_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn cold_resume_workspace_roots_validate_foreign_startup_paths() -> Result<()> {
    ...
    let kodex_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(kodex_home.path())?;
    let local_cwd = normalized_existing_path(kodex_home.path())?.abs();
    let extra_workspace = TempDir::new()?;
...
async fn thread_resume_preserves_goal_first_and_fork_approvals_reviewer() -> Result<()> {
    ...
    let (thread_id, fork_thread_id) = {
        ...
        let start_id = mcp
            .send_thread_start_request_with_auto_env(ThreadStartParams {
                model: Some("gpt-5.2-kodex".to_string()),
                approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
...
-> Result<()> {
    ...
    let live_cwd = normalized_existing_path(kodex_home.path())?;
    mock_responses_config(&server.uri()).write(kodex_home.path())?;
    let config_path = kodex_home.path().join("config.toml");
...
async fn thread_resume_restores_collaboration_mode_from_legacy_turn_context() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let kodex_home = TempDir::new()?;
    let fixture = setup_rollout_fixture(kodex_home.path(), &server.uri()).await?;
    mock_responses_config(&server.uri())
...
async fn cold_resume_reresolves_persisted_active_permission_profile() -> Result<()> {
    ...
    for history_mode in [ThreadHistoryMode::Legacy, ThreadHistoryMode::Paginated] {
        ...
        write_dev_permission_config(&server.uri(), kodex_home.path(), ":workspace")?;
        let profile_root = serde_json::to_string(profile_workspace.path())?;
        writeln!(
            std::fs::OpenOptions::new()
                .append(true)
                .open(kodex_home.path().join("config.toml"))?,
        ...
        write_dev_permission_config(&server.uri(), kodex_home.path(), ":read-only")?;
        let mut mcp = TestAppServer::builder()
            .with_kodex_home(kodex_home.path())
            .without_managed_config()
            .build_initialized()
...
async fn unloaded_thread_goal_mutations_respect_parent_ownership() -> Result<()> {
    ...
    for (source, version) in [
        ...
        let path = rollout_path(kodex_home.path(), TIMESTAMP, &thread_id);
        let params = json!({
            "threadId": thread_id,
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let kodex_home = TempDir::new()?;
    let kodex_home_path = normalized_existing_path(kodex_home.path())?;
    mock_responses_config(&server.uri())
        .enable_feature(Feature::Goals)
        .write(&kodex_home_path)?;

    let sqlite_home = kodex_home_path
        .as_path()
        .to_str()
        .expect("test kodex home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_kodex_home(&kodex_home_path)
        .without_managed_config()
        .with_env_overrides(&[("KODEX_SQLITE_HOME", Some(sqlite_home))])
        .build_initialized()
        .await?;
...
async fn thread_resume_running_thread_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "kodex_thread_initialized"
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(kodex_home, filename_ts, thread_id);
    let contents = std::fs::read_to_string(&path)?;
...
async fn thread_resume_returns_rollout_history() -> Result<()> {
    ...
    append_rollout_item_to_path(
        &rollout_path(kodex_home.path(), "2025-01-05T12-00-00", &conversation_id),
        &RolloutItem::EventMsg(EventMsg::ThreadSettingsApplied(settings)),
    )
    .await?;
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in ["kodex_chatgpt_android_remote", "kodex_chatgpt_ios_remote"] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(kodex_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(kodex_home.path(), filename_ts, &conversation_id);
    let archived_dir = kodex_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "kodex unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            kodex_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_keeps_original_root_until_external_objective_edit() -> Result<()> {
    ...
    let get_id = mcp
        .send_raw_request("thread/goal/get", Some(json!({ "threadId": thread.id })))
        .await?;
    let _: kodex_app_server_protocol::ThreadGoalGetResponse =
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let get: kodex_app_server_protocol::ThreadGoalGetResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(get_id)).await??;
...
async fn cold_paginated_resume_omits_usage_when_its_turn_is_ambiguous() -> Result<()> {
    ...
    let path = rollout_path(kodex_home.path(), filename_ts, &conversation_id);
    append_rollout_item_to_path(
        &path,
        &RolloutItem::EventMsg(EventMsg::TokenCount(TokenCountEvent {
            info: Some(TokenUsageInfo {
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(kodex_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    ...
    let repo_path = kodex_home.path().join("repo");
    ...
    let rollout_path = rollout_path(kodex_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_checkpoints_settings_without_advancing_recency() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let kodex_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(kodex_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(kodex_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let ThreadResumeResponse { cwd, .. } =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(resume_id)).await??;
    assert_eq!(cwd, AbsolutePathBuf::from_absolute_path(kodex_home.path())?);
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(kodex_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
"#;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![kodex_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_preserves_recency_and_checkpoints_model() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        recency_at,
    } = start_materialized_thread_and_restart(kodex_home.path(), "materialize").await?;
...
command = "kodex-definitely-not-a-real-binary"
required = true"#,
        )
        .write(kodex_home.path())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(kodex_home.path(), "seed history").await?;
...
async fn setup_rollout_fixture(kodex_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    mock_responses_config(server_uri).write(kodex_home)?;
