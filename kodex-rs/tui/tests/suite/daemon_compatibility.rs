...
async fn incompatible_daemon_falls_back_for_default_and_explicit_features() -> Result<()> {
    for scenario in ["default", "explicit", "host policy"] {
        let cwd = kodex_utils_cargo_bin::repo_root()?;
        ...
        let socket = kodex_app_server_client::app_server_control_socket_path(home.path())?;
        ...
        let mut terminal = PtyKodex::start(&cwd, home, &args)?;
