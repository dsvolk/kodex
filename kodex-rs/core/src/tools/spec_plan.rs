...
use kodex_tools::TOOL_SEARCH_TOOL_NAME;
use kodex_tools::ToolCall as ExtensionToolCall;
use kodex_tools::ToolEnvironmentMode;
use kodex_tools::ToolExecutor;
use kodex_tools::ToolExposures;
use kodex_tools::ToolName;
use kodex_tools::ToolSpec;
use kodex_tools::UnifiedExecShellMode;
use kodex_tools::can_request_original_image_detail;
use kodex_tools::collect_code_mode_exec_prompt_tool_definitions;
use kodex_tools::collect_request_plugin_install_entries;
use kodex_tools::default_namespace_description;
use kodex_tools::request_user_input_available_modes;
...
struct CoreToolPlanContext<'a> {
    tool_policy: &'a kodex_extension_api::ToolPolicy,
    turn_context: &'a TurnContext,
    model_info: &'a ModelInfo,
    environments: &'a TurnEnvironmentSnapshot,
    mcp: &'a kodex_mcp::McpBinding,
...
fn apply_mcp_tool_exposure_policy(
    ...
    for tool in mcp.tools() {
        ...
        omitted_exposures_by_tool
            .entry(tool_name)
            .or_insert_with(|| {
                let connector_omissions = (tool.server_name == KODEX_APPS_MCP_SERVER_NAME)
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(kodex_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(kodex_code_mode::WAIT_TOOL_NAME),
        ] {
    ...
    if turn_context.config.tool_registry.error_on_tool_collisions {
        if let Some(tool_name) = registry.first_collision() {
            ...
            return Err(KodexErrorDetails::ToolCollision(name).into());
        }
        ...
        for tool in registry.entries() {
            ...
            if include_tool_namespaces_info && tool.exposure != ToolExposure::Hidden {
                ...
                match namespace_owners.get(namespace_name) {
                    Some(existing_owner) if existing_owner != &owner => {
                        return Err(
                            KodexErrorDetails::ToolCollision(namespace_name.to_string()).into()
                        );
                    }
                    Some(_) => {}
                    None => {
            ...
            match namespace_descriptions.entry(namespace.name.clone()) {
                ...
                Entry::Occupied(entry) if entry.get() != &namespace.description => {
                    return Err(KodexErrorDetails::ToolCollision(entry.key().clone()).into());
                }
                Entry::Occupied(_) => {}
            }
        }
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && kodex_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&kodex_code_mode::normalize_code_mode_identifier(
                &kodex_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        kodex_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_kodex_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && kodex_code_mode::is_code_mode_nested_tool(&kodex_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !kodex_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(kodex_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            if unified_image_budget_enabled(&turn_context.config.features, model_info) {
                kodex_code_mode::ImageDetailVisibility::Hidden
            } else {
                kodex_code_mode::ImageDetailVisibility::Visible
            },
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, kodex_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| kodex_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &kodex_code_mode::ToolDefinition,
    right: &kodex_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, kodex_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &kodex_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, kodex_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
