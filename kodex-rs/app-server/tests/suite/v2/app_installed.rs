...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let kodex_home = configured_kodex_home(fixture.base_url())?;
    let mut app_server = start_app_server(kodex_home.path()).await?;
        ...
        for candidate in ["alpha", "beta"] {
            assert_eq!(
                responses::namespace_child_tool(
                    &body,
                    &format!("mcp__kodex_apps__{candidate}"),
...
async fn installed_apps_thread_refresh_updates_live_tools_and_retains_them_on_failure() -> Result<()>
{
    ...
    assert!(
        initial_model_request
            .single_request()
            .tool_by_name("mcp__kodex_apps__alpha", "connector_alpha")
            .is_some()
    );
    ...
    for (call_id, refresh_fails) in [("after-refresh", false), ("after-failed-refresh", true)] {
        ...
        assert!(
            requests[0]
                .tool_by_name("mcp__kodex_apps__beta", "connector_beta")
                .is_some()
        );
        assert!(
            requests[0]
...
fn configured_kodex_home(base_url: &str) -> Result<TempDir> {
...
impl InstalledAppsFixture {
    async fn start() -> Result<Self> {
        ...
        synthetic_link
            ...
            .0
            .insert("_kodex_apps".to_string(), json!({ "synthetic_link": true }));
