...
async fn gateway_credentials_accompany_primary_auth_in_models_and_responses() {
    ...
    for delivery in [
        ...
        let primary = AuthManager::from_auth_for_testing_with_home(
            KodexAuth::Headers(AuthHeaders::new(primary_headers)),
...
async fn gateway_refresh_preserves_primary_auth_and_hides_issuer_errors() {
    for succeeds in [true, false] {
        ...
        let home = tempfile::tempdir().unwrap();
        let primary = AuthManager::from_auth_for_testing_with_home(
            KodexAuth::from_api_key("primary-token"),
            home.path().to_path_buf(),
        );
        let info = ModelProviderInfo {
        ...
        assert_eq!(
            primary.auth_cached(),
            Some(KodexAuth::from_api_key("primary-token"))
        );
        let requests = server.received_requests().await.unwrap();
...
async fn models_cache_is_reused_only_for_matching_gateway_configuration() {
    ...
    let factory = HttpClientFactory::new(OutboundProxyPolicy::ReqwestDefault);
    let mut model = kodex_models_manager::bundled_models_response()
        .unwrap()
        .models
        .remove(0);
...
async fn models_observe_gateway_rotation(instance: ProviderInstance) {
    ...
    let mut first = kodex_models_manager::bundled_models_response()
        .unwrap()
        .models
        .remove(0);
...
async fn gateway_setup_errors_are_reported_when_authentication_is_requested() {
    ...
    let primary = AuthManager::from_auth_for_testing(KodexAuth::from_api_key("primary"));
    ...
    assert!(matches!(
        error.details(),
        kodex_protocol::error::KodexErrorDetails::InvalidRequest(_)
    ));
...
async fn gateway_login_states_stop_request_retries() {
    ...
    let login_control = kodex_login::GatewayLoginControl::for_runtime(&primary.runtime_config());
    ...
    assert_eq!(
        ...
        (
            kodex_login::GatewayAuthError::LoginRequired.to_string(),
            None
        ),
    );
    ...
    let check = async {
        ...
        assert_eq!(
            (error.to_string(), error.retry_delay(/*retry_count*/ 1)),
            (
                kodex_login::GatewayAuthError::LoginInProgress.to_string(),
                None
            ),
        );
        cancel.notify_one();
