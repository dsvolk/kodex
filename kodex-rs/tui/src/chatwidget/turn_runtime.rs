...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        kodex_error_info: &AppServerKodexErrorInfo,
    ) -> bool {
        matches!(
            kodex_error_info,
            AppServerKodexErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Kodex is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.kodex_rate_limit_reached_type.map(|kind| {
        ...
        if self.kodex_rate_limit_reached_type != rate_limit_reached_type {
            self.clear_backend_banner();
        }
        self.kodex_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = kodex_error_info
            .as_ref()
