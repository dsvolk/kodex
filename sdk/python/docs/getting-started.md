...
This guide gets a published OpenAI Kodex Python SDK installation running
...
- Python `>=3.10`
- An existing Kodex account session, or one of the login flows below

The SDK installs its matching `openai-kodex-cli-bin` runtime dependency
automatically. Stable SDK releases track the corresponding stable Kodex CLI release.

## 2. Authenticate When Needed

Existing Kodex authentication is reused automatically. For ChatGPT browser
login:
...
When `sandbox=` is omitted, Kodex uses its configured default. A turn override
...
```python
with Kodex() as kodex:
    thread = kodex.thread_resume("thr_123")
...
from openai_kodex import AsyncKodex, Sandbox


async def main() -> None:
    async with AsyncKodex() as kodex:
        thread = await kodex.thread_start(sandbox=Sandbox.workspace_write)
...
```python
import openai_kodex
from openai_kodex import Kodex, KodexConfig
...
- [Runnable examples](https://github.com/openai/kodex/blob/main/sdk/python/examples/README.md)
