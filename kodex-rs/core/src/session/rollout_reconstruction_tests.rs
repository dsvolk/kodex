...
use kodex_rollout::ModelContextScan;
use kodex_rollout::ModelContextScanProgress;
...
async fn recorded_questions_share_queued_input_order_across_resume() {
    ...
    let question = |call_id: &str| {
        serde_json::from_value::<ResponseItem>(json!({
            ...
            "namespace": "mcp__kodex_apps", "name": "user_messaging_send_message",
...
async fn sender_context_follows_its_delivery_through_checkpoint_and_rollback() {
    ...
    for index in 0..2 {
        ...
        let snapshot = kodex_history::SenderUserMessages {
        ...
        let input = [
            ResponseItemEnvelope {
                item: serde_json::from_value::<ResponseItem>(json!({
                    ...
                    "name": "send_message_to_thread", "namespace": "kodex_app", "output": "Inspect."
                }))
                .unwrap(),
                metadata: Some(kodex_history::KodexHarnessMetadata {
...
async fn reconstruct_history_rollback_clears_history_and_metadata_when_exceeding_user_turns() {
    ...
    let rollout_items = vec![
        ...
        RolloutItem::EventMsg(EventMsg::ThreadRolledBack(
            kodex_protocol::protocol::ThreadRolledBackEvent { num_turns: 99 },
        )),
    ];
...
async fn bounded_replay_matches_full_replay_after_empty_turn_compactions(
    ...
    for window_number in 1..=2 {
        ...
        let mut wake_items = completed_user_turn_rollout(
            ...
            vec![
                RolloutItem::Compacted(CompactedItem {
                    ...
                    compaction_response_id: None,
                    latest_token_usage_record: None,
                    resume_metadata: current.then(|| kodex_history::CompactionResumeMetadata {
                        multi_agent_version: None,
        ...
        wake_items.push(RolloutItem::EventMsg(EventMsg::TurnAborted(
            kodex_protocol::protocol::TurnAbortedEvent {
    ...
    assert_eq!(
        ...
        Some(&kodex_history::GuardianHistoryCheckpoint(vec![
            user_message("original task"),
...
async fn reconstruct_history_legacy_compaction_without_replacement_history_does_not_inject_current_initial_context()
 {
    let (session, turn_context) = make_session_and_context().await;
    let answer = kodex_history::RetainedContextEvent::VerifiedAnswer {
        answer: kodex_history::VerifiedAnswer {
            turn_id: "legacy-turn".to_owned(),
            call_id: "ask-1".to_owned(),
            questions: vec![kodex_history::VerifiedQuestionAnswer {
    ...
    let mut retained = kodex_history::RetainedContext::default();
    retained.record_user_message(
        kodex_history::RetainedUserMessage {
            turn_id: String::new(),
            message_id: None,
            text: "before compact".to_owned(),
            complete: false,
        },
        kodex_history::RetainedInputSource::Local(None),
    );
