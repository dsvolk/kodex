...
async fn standalone_kodex_apps_respects_protocol_setting(
    ...
    let fixture = test_kodex()
        ...
        .with_config(move |config| {
            ...
            if matches!(setting, HostedProtocolSetting::Enabled) {
                config
                    .features
                    .enable(Feature::KodexAppsMcp20260728)
    ...
    wait_for_mcp_server(&fixture.kodex, KODEX_APPS_MCP_SERVER_NAME).await?;
    let methods = server
        .received_requests()
        .await
...
async fn apps_call_survives_catalog_restoration_while_awaiting_approval() -> anyhow::Result<()> {
    ...
    let fixture = apps_enabled_builder(apps.chatgpt_base_url)
        ...
        .with_config(|config| {
            ...
            config.config_layer_stack = config
                .config_layer_stack
                .with_user_config(
                    &config.kodex_home.join("config.toml").abs(),
    ...
    fixture
        ...
        .await?;
    let EventMsg::ElicitationRequest(approval) = wait_for_event(&fixture.kodex, |event| {
        matches!(
            event,
    ...
    assert!(!peer.refresh_kodex_apps_tools().await?.tools.is_empty());
