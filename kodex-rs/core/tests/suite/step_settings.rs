...
use kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
use kodex_protocol::openai_models::ApplyPatchToolType;
...
use kodex_protocol::openai_models::MultiAgentMessages;
use kodex_protocol::openai_models::MultiAgentModeMessages;
use kodex_protocol::openai_models::MultiAgentRoleMessages;
...
use kodex_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
use kodex_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_OPEN_TAG;
...
use kodex_protocol::request_user_input::RequestUserInputEvent;
use kodex_protocol::request_user_input::RequestUserInputResponse;
use kodex_protocol::user_input::UserInput;
use kodex_tools::ConversationHistory;
use kodex_tools::JsonToolOutput;
use kodex_tools::ToolCall;
use kodex_tools::ToolExecutor;
use kodex_tools::ToolExecutorFuture;
use kodex_tools::ToolName;
use kodex_tools::ToolOutput;
...
const TURN_STATE_HEADER: &str = "x-kodex-turn-state";
...
fn step_settings_test() -> TestKodexBuilder {
    test_kodex().with_model(MODEL_A).with_config(move |config| {
        for feature in [
...
fn direct_tool_settings_test() -> TestKodexBuilder {
...
async fn start_paused_turn(thread: &KodexThread) -> Result<RequestUserInputEvent> {
    thread
...
async fn answer_paused_turn(thread: &KodexThread, turn_id: &str) -> Result<()> {
    thread
...
fn request_turn_metadata(request: &ResponsesRequest) -> Value {
    serde_json::from_str(
        request.body_json()["client_metadata"]["x-kodex-turn-metadata"]
            .as_str()
            .expect("request should include turn metadata"),
    )
    .expect("valid turn metadata")
...
async fn tool_result_history_keeps_originating_model_across_switch_and_replay() -> Result<()> {
    ...
    for call_id in ["call-a", "call-b"] {
        let call = wait_for_event_match(&test.kodex, |event| match event {
    ...
    let paused_request = wait_for_event_match(&test.kodex, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    .await?;
    answer_paused_turn(&test.kodex, &paused_request.turn_id).await?;
    ...
    let rollout_path = test.kodex.rollout_path().expect("rollout path");
    async fn saved_outputs(path: &std::path::Path) -> Result<Vec<(Value, Option<usize>)>> {
        let history = kodex_rollout::RolloutRecorder::get_rollout_history(path).await?;
        Ok(history
            .get_rollout_items()
            .iter()
    ...
    for model in [MODEL_A, MODEL_B] {
        ...
        let resumed = test
            ...
            .resume_thread_from_rollout(
                replay_config.clone(),
                rollout_path.clone(),
                kodex_core::test_support::auth_manager_from_auth(KodexAuth::from_api_key("dummy")),
                /*parent_trace*/ None,
...
async fn custom_tool_output_replay_preserves_originating_budget() -> Result<()> {
    ...
    test.kodex.shutdown_and_wait().await?;
    let rollout_path = test.kodex.rollout_path().expect("rollout path");
    let history = kodex_rollout::RolloutRecorder::get_rollout_history(&rollout_path).await?;
    let saved = history
        .get_rollout_items()
        .iter()
...
async fn settings_updates_preserve_turn_identity_and_target(target: SettingsTarget) -> Result<()> {
    ...
    let original_settings = test.kodex.thread_settings_snapshot().await;
    let request = start_paused_turn(&test.kodex).await?;
    ...
    match target {
        ...
        SettingsTarget::Turn => {
            ...
            assert_eq!(
                submit_turn_settings(&test.kodex, "different-turn", update.clone()).await?,
                TurnSettingsUpdateOutcome::TargetUnavailable
            );
            assert_eq!(
                submit_turn_settings(&test.kodex, &request.turn_id, update).await?,
                TurnSettingsUpdateOutcome::Applied
            );
        }
    }
    answer_paused_turn(&test.kodex, &request.turn_id).await?;
    let mut settings_events = Vec::new();
    let mut new_turns = Vec::new();
    let completion = wait_for_event(&test.kodex, |event| match event {
    ...
    assert_eq!(
        test.kodex.thread_settings_snapshot().await,
...
async fn new_context_after_model_switch_uses_captured_extension_window() -> Result<()> {
    ...
    let test = step_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    let paused = start_paused_turn(&test.kodex).await?;
    assert_eq!(
        submit_turn_settings(
            &test.kodex,
            &paused.turn_id,
    ...
    answer_paused_turn(&test.kodex, &paused.turn_id).await?;
    wait_for_event(&test.kodex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn active_model_switch_updates_core_context_from_captured_settings(
    ...
    if reload_user_config {
        std::fs::write(
            test.kodex_home_path().join("config.toml"),
            "[features.token_budget]\nenabled = true\nreminder_message_template = \"Reloaded reminder\"\n",
        )?;
        test.kodex.submit(Op::ReloadUserConfig).await?;
    }
    ...
    .await;
    answer_paused_turn(&test.kodex, &request.turn_id).await?;
    wait_for_event(&test.kodex, |event| match event {
        EventMsg::Error(error) => panic!("settings activation failed: {}", error.message),
        EventMsg::TurnComplete(_) => true,
        _ => false,
    })
...
async fn mcp_confirmation_policy_follows_step_model_changes() -> Result<()> {
    ...
    .await;
    let mcp_url = format!("{}/api/kodex/ps/mcp", server.uri());
    let test = step_settings_test()
        .with_config(move |config| {
            config
                .features
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.kodex, "node_repl").await?;
...
async fn captured_model_replans_tools_and_retains_the_issuing_request_router() -> Result<()> {
    ...
    .await?;
    answer_paused_turn(&test.kodex, &paused.turn_id).await?;
    let paused = wait_for_event_match(&test.kodex, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    let test = direct_tool_settings_test()
        .with_config(|config| {
            config
                ...
                .expect("test permissions");
            for model in &mut config.model_catalog.as_mut().expect("models").models {
                model.input_modalities = vec![kodex_protocol::openai_models::InputModality::Text];
                model.supports_image_detail_original = false;
                if model.slug == MODEL_B {
                    model
                        .input_modalities
                        .push(kodex_protocol::openai_models::InputModality::Image);
...
async fn sparse_updates_preserve_divergent_active_and_future_models() -> Result<()> {
    ...
    .await?;
    answer_paused_turn(&test.kodex, &request.turn_id).await?;
    let second_request = wait_for_event_match(&test.kodex, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    .await?;
    let durable_settings = wait_for_event_match(&test.kodex, |event| match event {
    ...
    answer_paused_turn(&test.kodex, &second_request.turn_id).await?;
    wait_for_event(&test.kodex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_settings_do_not_target_idle_or_finished_turns() -> Result<()> {
    ...
    for (discarded_model, model) in [(MODEL_C, MODEL_B), (MODEL_A, MODEL_C)] {
        ...
        let before = test.kodex.thread_settings_snapshot().await;
        assert_eq!(
            submit_turn_settings(
                &test.kodex,
                &turn_id,
        ...
        assert_eq!(test.kodex.thread_settings_snapshot().await, before);
        core_test_support::submit_thread_settings(
            &test.kodex,
            ThreadSettingsOverrides {
                model: Some(model.to_string()),
...
async fn turn_settings_rejection_preserves_independent_future_settings() -> Result<()> {
    ...
    .await?;
    let future = test.kodex.thread_settings_snapshot().await;
    assert_eq!(
        submit_turn_settings(
            &test.kodex,
            &request.turn_id,
    ...
    assert_eq!(test.kodex.thread_settings_snapshot().await, future);
    answer_paused_turn(&test.kodex, &request.turn_id).await?;
    wait_for_event(&test.kodex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    ...
    let test = direct_tool_settings_test()
        .with_config(|config| {
            ...
            for model in &mut config.model_catalog.as_mut().expect("models").models {
                ...
                model.truncation_policy = kodex_protocol::openai_models::TruncationPolicyConfig {
                    mode: kodex_protocol::openai_models::TruncationMode::Bytes,
...
async fn captured_step_controls_mcp_output_limit(supports_images: bool) -> Result<()> {
                ...
                "jsonrpc": "2.0", "id": body["id"], "result": {
                    "content": [
                        ...
                        {"type": "image", "mimeType": "image/png",
                         ...
                         "_meta": {"kodex/imageDetail": "original"}},
                    ], "isError": false,
                },
            }))
        })
    ...
    let test = direct_tool_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.kodex, "calendar").await?;
    let paused = start_paused_turn(&test.kodex).await?;
    apply_turn_settings(
        &test.kodex,
        &paused.turn_id,
    ...
    .await?;
    answer_paused_turn(&test.kodex, &paused.turn_id).await?;
    let recorded_output = wait_for_event_match(&test.kodex, |event| match event {
...
async fn captured_step_controls_mcp_resource_output() -> Result<()> {
        ...
        vec![
            ...
            sse(vec![
                ...
                ev_function_call(
                    ...
                    "read_mcp_resource",
                    &json!({
                        "server": "resources", "uri": "memo://kodex/example-note",
                    })
                    .to_string(),
                ),
                ev_completed("resp-b"),
    ...
    let test = direct_tool_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.kodex, "resources").await?;
    let paused = start_paused_turn(&test.kodex).await?;
    apply_turn_settings(
        &test.kodex,
        &paused.turn_id,
