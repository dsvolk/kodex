...
async fn test_server(
    ...
    let path = rollout_path(kodex_home.path(), "2026-01-02T00-00-00", &target);
    ...
    for payload in [
        ...
        serde_json::from_value::<kodex_protocol::protocol::EventMsg>(payload.clone())?;
        records.push(json!({
    ...
    server
        ...
        .await?;
    Ok((kodex_home, server, source, target))
}
...
"#
    ...
    let started: kodex_app_server_protocol::ThreadStartResponse =
    ...
    tokio::time::timeout(Duration::from_secs(/*secs*/ 30), async {
        while let Some(event) = model_server.next_event().await {
            match event {
                kodex_app_server_client::AppServerEvent::ServerRequest(request) => {
                    if let kodex_app_server_protocol::ServerRequest::DynamicToolCall {
                        request_id,
                        params,
                    } = *request
                    {
                ...
                kodex_app_server_client::AppServerEvent::ServerNotification(notification) => {
                    if let kodex_app_server_protocol::ServerNotification::TurnCompleted(_) =
                        *notification
                    {
                        return Ok::<_, color_eyre::eyre::Report>(());
                    }
                }
                kodex_app_server_client::AppServerEvent::Lagged { .. }
                | kodex_app_server_client::AppServerEvent::Disconnected { .. } => {
...
fn delegated_prompts_match_desktop_xml_contract() {
    ...
    for namespace in ["kodex_tui", "kodex_app"] {
        assert_eq!(
...
fn activity_metadata_is_retained_without_including_outputs() -> color_eyre::Result<()> {
    ...
    let turn: Turn = serde_json::from_value(json!({
        ...
        "items": [
            ...
            {"type": "functionCallOutput", "id": "delegation-1", "name": "send_message_to_thread",
                "namespace": "kodex_tui", "output": delegated_prompt("source-2", "Follow <up> & report")}
        ]
    }))?;
    ...
    assert_eq!(
        ...
        json!([
            ...
            {"type": "userMessage", "id": "user-1", "content": [
                {"type": "text", "text": delegated_prompt("source-1", "Check <main> & status"),
                    "kodexDelegation": {"sourceThreadId": "source-1", "input": "Check <main> & status"}},
                {"type": "skill", "name": "debug", "path": "/tmp/SKILL.md"},
                {"type": "mention", "name": "docs", "path": "app://docs"}
            ]},
            {"type": "agentMessage", "id": "assistant-1", "text": assistant_text, "phase": "commentary"},
            ...
            {"type": "enteredReviewMode", "id": "review-1", "review": "review changes"},
            {"type": "functionCallOutput", "id": "delegation-1", "name": "send_message_to_thread",
                "namespace": "kodex_tui", "kodexDelegation": {
...
async fn task_management_tools_use_existing_app_server_operations() -> color_eyre::Result<()> {
    let (kodex_home, server, source, target) = test_server(json!({})).await?;
...
async fn wait_threads_preserves_snapshots_and_rejects_self_wait() -> color_eyre::Result<()> {
    let (_kodex_home, server, source, target) = test_server(json!({})).await?;
...
async fn task_creation_and_followup_start_background_turns() -> color_eyre::Result<()> {
    ...
    let ephemeral: kodex_app_server_protocol::ThreadStartResponse =
