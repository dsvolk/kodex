...
use crate::managed_install::managed_kodex_version;
...
pub(super) fn supported(daemon: &Daemon) -> Result<bool> {
    ...
    let bin = daemon.current_managed_kodex_bin()?;
...
pub(super) async fn unsupported(daemon: &Daemon) -> Result<UpdateOutput> {
    ...
    Ok(UpdateOutput {
        ...
        installed_version: managed_kodex_version(&managed_kodex_path).await.ok(),
...
pub(super) async fn run(
    ...
    let previous_identity = executable_identity(&managed_kodex_path).await?;
    let (control, restart) =
    ...
    let current_managed_kodex_path = daemon.current_managed_kodex_bin()?;
    let installed_version = Some(managed_kodex_version(&current_managed_kodex_path).await?);
    let updated = previous_release != selected_release(daemon)?.1
        || previous_identity != executable_identity(&current_managed_kodex_path).await?;
    ...
    Ok(UpdateOutput {
        ...
        managed_kodex_path: current_managed_kodex_path,
        installed_version,
        running_version,
