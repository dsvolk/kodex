...
use kodex_state::Stage1JobClaimOutcome;
...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    for version in [MemoryVersion::V1, MemoryVersion::V2] {
        let root = kodex_home.path().join(version.directory_name());
        ...
        .await?;
        thread_ids.push(seed_stage1_output(&state_db, kodex_home.path(), version).await?);
    }
...
async fn seed_stage1_output(
    ...
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        kodex_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
...
async fn memory_status_requires_successful_v2_consolidation_and_resets() -> Result<()> {
    use kodex_app_server_protocol::MemoryStatusResponse;
