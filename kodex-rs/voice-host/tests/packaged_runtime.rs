...
#[tokio::test]
#[ignore = "requires KODEX_TEST_VOICE_RUNTIME containing real prepared native libraries"]
async fn relocated_runtime_initializes_closes_and_rejects_duplicate_initialization() -> Result<()> {
    let source = std::env::var_os("KODEX_TEST_VOICE_RUNTIME")
        .context("set KODEX_TEST_VOICE_RUNTIME to a matching prepared runtime")?;
    ...
    let runtime = root.join("kodex-resources/voice");
    ...
    let helper_source = cargo_bin("kodex-voice-host")?;
    ...
    let app_name = if cfg!(windows) { "kodex.exe" } else { "kodex" };
