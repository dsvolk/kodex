...
impl TurnItemContributor for RewriteAgentMessageContributor {
    fn contribute<'a>(
        ...
        item: &'a mut TurnItem,
    ) -> kodex_extension_api::ExtensionFuture<'a, Result<(), String>> {
        Box::pin(async move {
...
fn post_sampling_token_estimate_is_disabled_by_always_on_sinks() {
    ...
    let subscriber = tracing_subscriber::registry()
        ...
        .with(tracing_subscriber::fmt::layer().with_filter(kodex_state::log_db::default_filter()));
...
fn realtime_user_verification_notice_excludes_request_payload() {
    let event = EventMsg::ElicitationRequest(kodex_protocol::approvals::ElicitationRequestEvent {
        turn_id: None,
        server_name: "private-server-name".to_string(),
        id: kodex_protocol::mcp::RequestId::String("private-request-id".to_string()),
        request: kodex_protocol::approvals::ElicitationRequest::UserVerification {
            meta: None,
