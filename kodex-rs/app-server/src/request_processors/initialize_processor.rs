...
use kodex_login::default_client::SetOriginatorError;
use kodex_login::default_client::USER_AGENT_SUFFIX;
...
const NON_ORIGINATING_CLIENT_NAMES: &[&str] = &["kodex_app_server_daemon", "kodex-backend"];

#[derive(Clone)]
pub(crate) struct InitializeRequestProcessor {
    gateway_login_control: Arc<kodex_login::GatewayLoginControl>,
...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let mut client_mcp_extensions = kodex_mcp::client_mcp_extensions(
            extensions,
        ...
        let user_verification_enabled = experimental_api_enabled
            && matches!(
                ...
                (ConnectionOrigin::InProcess, "kodex-tui")
                    | (ConnectionOrigin::Stdio, "Kodex Desktop")
            )
        ...
        let kodex_home = self.config.kodex_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for KODEX_INTERNAL_ORIGINATOR_OVERRIDE,
        ...
        #[cfg(windows)]
        if matches!(session.origin, ConnectionOrigin::Stdio) && name == "Kodex Desktop" {
            ...
            let home = kodex_home.clone();
            if !matches!(
                tokio::task::spawn_blocking(move || {
                    kodex_windows_sandbox::register_desktop_installation(&home)
                })
                .await,
                Ok(Ok(()))
            ) {
        ...
        let user_agent = get_kodex_user_agent();
