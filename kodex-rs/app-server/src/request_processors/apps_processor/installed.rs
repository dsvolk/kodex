...
use kodex_connectors::ConnectorRuntimeTool;
use kodex_connectors::connector_runtime_context_key;
use kodex_connectors::connector_tool_is_synthetic;
use kodex_connectors::installed_connector_runtime;
...
use kodex_mcp::host_owned_kodex_apps_enabled;
use kodex_mcp::tool_is_model_visible;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "kodex.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "kodex.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "kodex.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "kodex.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.kodex_home.to_path_buf(), cache_key.clone());
            let mut model_visible_tool_names = None;
            let tools = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    if let Some(thread) = thread {
                        let snapshot = thread.refresh_kodex_apps_tools().await?;
                    ...
                    anyhow::ensure!(
                        ...
                        "host-owned MCP server '{KODEX_APPS_MCP_SERVER_NAME}' is not enabled"
                    );
                    ...
                    let kodex_apps_auth_manager =
                        host_owned_kodex_apps_enabled(&mcp_config, auth.as_ref())
...
fn record_apps_installed_metrics(
    metrics: &kodex_otel::MetricsClient,
    started_at: Instant,
    force_refresh: bool,
