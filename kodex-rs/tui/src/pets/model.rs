...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under KODEX_HOME,
...
impl Pet {
    ...
    /// has already been materialized into KODEX_HOME; if callers skip the
    /// asset-fetch step, they will get a missing-spritesheet error here on
    /// first use.
    pub(super) fn load_with_kodex_home(value: &str, kodex_home: Option<&Path>) -> Result<Self> {
        if path_like(value) {
            return load_pet_path(value);
        }

        if let Some(custom_id) = value.strip_prefix(CUSTOM_PET_PREFIX) {
            return load_custom_pet(custom_id, kodex_home);
        }

        if let Some(builtin) = catalog::builtin_pet(value) {
            return load_builtin_pet(builtin, kodex_home);
        }

        load_custom_pet(value, kodex_home)
    }
...
fn load_builtin_pet(pet: catalog::BuiltinPet, kodex_home: Option<&Path>) -> Result<Pet> {
    let kodex_home = kodex_home.context("KODEX_HOME is not available")?;
    let spritesheet_path = super::builtin_spritesheet_path(kodex_home, pet.spritesheet_file);
...
fn load_custom_pet(value: &str, kodex_home: Option<&Path>) -> Result<Pet> {
    let kodex_home = kodex_home.context("KODEX_HOME is not available")?;
    let pet_dir = kodex_home.join("pets").join(value);
    if pet_dir.join("pet.json").is_file() {
        return load_pet_manifest(&pet_dir, "pet.json", value, &custom_pet_cache_id(value));
    }

    let avatar_dir = kodex_home.join("avatars").join(value);
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_kodex_home(dir.path().to_str().unwrap(), /*kodex_home*/ None).unwrap()
    }

    fn load_pet_error_from_dir(dir: &tempfile::TempDir) -> anyhow::Error {
        Pet::load_with_kodex_home(dir.path().to_str().unwrap(), /*kodex_home*/ None).unwrap_err()
    }

    #[test]
    fn load_builtin_pet_uses_app_catalog_storage() {
        let kodex_home = tempfile::tempdir().unwrap();
        super::super::asset_pack::write_test_pack(kodex_home.path());

        let pet =
            Pet::load_with_kodex_home("dewey", /*kodex_home*/ Some(kodex_home.path())).unwrap();
        ...
        assert_eq!(
            ...
            super::super::builtin_spritesheet_path(kodex_home.path(), "dewey-spritesheet-v4.webp")
        );
    ...
    fn load_pet_json_path_uses_containing_directory() {
        ...
        let pet = Pet::load_with_kodex_home(
    ...
    #[test]
    fn custom_pet_selector_loads_kodex_home_pet_manifest() {
        let dir = write_minimal_pet();
        let kodex_home = tempfile::tempdir().unwrap();
        let pet_dir = kodex_home.path().join("pets").join("chefito");
    ...
    fn custom_pet_selector_falls_back_to_legacy_avatar_manifest() {
        let dir = write_minimal_pet();
        let kodex_home = tempfile::tempdir().unwrap();
        let avatar_dir = kodex_home.path().join("avatars").join("legacy");
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        let kodex_home = tempfile::tempdir().unwrap();
        let pet_dir = kodex_home.path().join("pets").join("escape");
        fs::create_dir_all(&pet_dir).unwrap();
        fs::write(
        ...
        let err = Pet::load_with_kodex_home(
