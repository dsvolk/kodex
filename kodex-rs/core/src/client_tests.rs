...
use super::X_KODEX_INSTALLATION_ID_HEADER;
use super::X_KODEX_PARENT_THREAD_ID_HEADER;
use super::X_KODEX_TURN_METADATA_HEADER;
use super::X_KODEX_WINDOW_ID_HEADER;
...
use crate::test_support::TestKodexResponsesRequestKind;
use crate::test_support::responses_metadata as test_responses_metadata;
use base64::Engine;
use futures::StreamExt;
use kodex_api::AgentIdentityTelemetry;
use kodex_api::ApiError;
use kodex_api::ResponseEvent;
...
use kodex_model_provider::BearerAuthProvider;
use kodex_model_provider::ModelProvider;
use kodex_model_provider::ModelProviderFuture;
use kodex_model_provider::ProviderAccountResult;
use kodex_model_provider::ProviderAuthRecoveryMessages;
...
use kodex_model_provider_info::create_oss_provider_with_base_url;
...
use kodex_rollout_trace::ExecutionStatus;
...
use kodex_rollout_trace::RolloutTrace;
...
async fn workspace_routed_http_rejects_redirects_without_a_routing_header() {
    ...
    use kodex_login::WorkspaceRoutingRequest;
    use kodex_login::WorkspaceRoutingResolver;
    ...
    for routing_override in [Some("NO_CONSTRAINT"), Some("us_cr"), None] {
        ...
        if routing_override.is_some() {
            assert_eq!(
                ...
                "https://gov.chatgpt.com/backend-api/kodex"
            );
        }
...
impl ModelProvider for SetupRefreshProvider {
    ...
    fn auth(&self) -> ModelProviderFuture<'_, Option<KodexAuth>> {
        self.inner.auth()
    }
    ...
    fn api_provider(
        &self,
    ) -> ModelProviderFuture<'_, kodex_protocol::error::Result<kodex_api::Provider>> {
        Box::pin(async move {
            ...
            match &self.refresh {
                ...
                SetupRefresh::ChatGpt {
                    ...
                    kodex_login::auth::login_with_chatgpt_auth_tokens(
    ...
    fn models_manager(
        ...
        self.inner.models_manager(kodex_home, config_model_catalog)
    }
}
...
async fn client_setup_accepts_command_credential_refresh() {
    for routing in [
        ...
        info.auth = Some(kodex_protocol::config_types::ModelProviderAuthInfo {
        ...
        assert_ne!(
            kodex_model_provider::ResponsesConnectionKey::new(
                &setup.api_provider,
                setup.auth_revision
            ),
            kodex_model_provider::ResponsesConnectionKey::new(
...
async fn client_setup_rebuilds_chatgpt_refresh_but_rejects_account_switches() {
    for (user, workspace, expected_calls) in [
        ...
        let initial = KodexAuth::from_external_chatgpt_tokens(
...
-> anyhow::Result<()> {
    ...
    for (base_url, allowed) in [
        ...
        ("https://chatgpt.com/backend-api/kodex", true),
        ...
        for responses_lite in [false, true] {
            ...
            let ws_request = kodex_api::ResponseCreateWsRequest {
                client_metadata: Some(ws_client_metadata),
                ..kodex_api::ResponseCreateWsRequest::from(&request)
            };
            assert_eq!(
...
fn responses_lite_prefix_ids_track_thread_and_payload() -> anyhow::Result<()> {
    ...
    prompt.tools = vec![kodex_tools::ToolSpec::Freeform(kodex_tools::FreeformTool {
        name: "exec".to_string(),
        description: "Execute JavaScript.".to_string(),
        defer_loading: None,
        format: kodex_tools::FreeformToolFormat {
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::KodexTurnStarted {
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(kodex_protocol::ResponseItemId::with_suffix("msg", id)),
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        client_metadata
            .get(X_KODEX_TURN_METADATA_HEADER)
    ...
    for (client_key, metadata_key, expected) in [
        ...
        (
            X_KODEX_WINDOW_ID_HEADER,
            "window_id",
            expected_window_id.as_str(),
        ),
        (
            X_KODEX_PARENT_THREAD_ID_HEADER,
            "parent_thread_id",
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart Kodex, url: {url}"
        )
    );
}
...
impl ModelProvider for TestRecoveryProvider {
    ...
    fn recover_from_unauthorized(
        &self,
    ) -> ModelProviderFuture<'_, kodex_protocol::error::Result<ProviderUnauthorizedRecovery>> {
        self.attempts.fetch_add(1, Ordering::Relaxed);
        Box::pin(async move {
            if self.should_fail {
                Err(KodexErr::Io(std::io::Error::other(
...
async fn provider_owned_auth_recovery_is_bounded_and_preserves_unauthorized_failures() {
    for should_fail in [false, true] {
        ...
        match error.details() {
            KodexErrorDetails::UnexpectedStatus(response) => {
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(CHATGPT_KODEX_BASE_URL.to_string())),
        )
    } else {
        (
            None,
...
fn thread_responses_headers_are_scoped_to_model_and_backend_auth() {
    ...
    model_client.kodex_responses_headers = Some(Arc::new(crate::KodexResponsesHeaders {
        model: "selected-model".to_owned(),
        headers: headers.clone(),
    }));
    let chatgpt_auth = KodexAuth::create_dummy_chatgpt_auth_for_testing();
    let api_key_auth = KodexAuth::from_api_key("test-api-key");
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_kodex_responses(
...
#[tokio::test]
async fn non_chatgpt_kodex_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
