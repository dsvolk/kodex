...
impl AuthProvider for StaticAuthProvider {
    ...
    fn apply_auth(
        &self,
        mut request: kodex_http_client::Request,
    ) -> kodex_api::AuthProviderFuture<'_> {
        Box::pin(async move {
    ...
    fn apply_auth(
        ...
        Box::pin(async move {
            let mut url = url::Url::parse(&request.url)
                .map_err(|error| kodex_api::AuthError::Build(error.to_string()))?;
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                tracing_subscriber::filter::Targets::new()
                    .with_target("kodex_http_client", tracing::Level::TRACE)
                    .with_target("kodex_exec_server", tracing::Level::TRACE),
            ),
    );
    let _guard = tracing::subscriber::set_default(subscriber);
    tracing::debug!(target: "kodex_exec_server", "direct registry log capture sentinel");
