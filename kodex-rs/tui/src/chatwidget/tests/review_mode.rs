...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::KodexOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::KodexOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::KodexOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        kodex_app_server_protocol::ServerNotification::TurnStarted(
    ...
    chat.handle_server_notification(
        kodex_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            kodex_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: kodex_app_server_protocol::ThreadGoal {
                    ...
                    status: kodex_app_server_protocol::ThreadGoalStatus::BudgetLimited,
    ...
    chat.handle_server_notification(
        kodex_app_server_protocol::ServerNotification::TurnCompleted(
            kodex_app_server_protocol::TurnCompletedNotification {
                thread_id: "thread-1".to_string(),
                turn: kodex_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items_view: kodex_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: kodex_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
