...
use kodex_config::load_global_mcp_servers;
use kodex_login::default_client::is_first_party_originator;
...
use kodex_rmcp_client::perform_oauth_login;
...
use kodex_skills::SkillToolDependency;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(kodex_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&kodex_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&kodex_home)
