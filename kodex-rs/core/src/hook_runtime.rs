...
use kodex_analytics::HookRunFact;
...
use kodex_core_plugins::executor_plugin_hook_sources;
use kodex_hooks::InterruptRequest;
use kodex_hooks::PermissionRequestDecision;
use kodex_hooks::PermissionRequestOutcome;
use kodex_hooks::PermissionRequestRequest;
use kodex_hooks::PostToolUseOutcome;
use kodex_hooks::PostToolUseRequest;
use kodex_hooks::PreToolUseOutcome;
use kodex_hooks::PreToolUseRequest;
use kodex_hooks::SessionStartOutcome;
use kodex_hooks::StartHookTarget;
use kodex_hooks::StopHookTarget;
use kodex_hooks::StopOutcome;
use kodex_hooks::SubagentHookContext;
use kodex_hooks::UserPromptSubmitOutcome;
use kodex_hooks::UserPromptSubmitRequest;
use kodex_hooks::hook_execution_mode_label;
use kodex_hooks::hook_handler_type_label;
use kodex_mcp::KODEX_APPS_MCP_SERVER_NAME;
use kodex_otel::HOOK_RUN_DURATION_METRIC;
use kodex_otel::HOOK_RUN_METRIC;
use kodex_plugin::ExecutorPluginHookSource;
use kodex_protocol::items::FunctionCallOutputItem;
...
use kodex_protocol::protocol::HookOutputEntryKind;
use kodex_protocol::protocol::HookRunStatus;
use kodex_protocol::protocol::HookRunSummary;
use kodex_protocol::protocol::HookSource;
use kodex_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    kodex_hooks::SessionStartSource::Startup
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = kodex_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = kodex_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = kodex_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = kodex_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(kodex_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: kodex_hooks::HookEvent::AfterAgent {
                event: kodex_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            kodex_hooks::HookResult::Success => continue,
            kodex_hooks::HookResult::FailedContinue(error) => (error, false),
            kodex_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(kodex_protocol::protocol::ErrorEvent {
        misalignment: None,
        message,
...
fn hook_run_analytics_payload(
    ...
    turn_context: &TurnContext,
    completed: &HookCompletedEvent,
) -> (kodex_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use kodex_protocol::protocol::HookHandlerType;
    use kodex_protocol::protocol::HookRunStatus;
    use kodex_protocol::protocol::HookScope;
    use kodex_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    kodex_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
    ...
    async fn hook_lifecycle_notifications_hide_builtin_and_async_runs_but_preserve_metrics() {
        ...
        assert!(matches!(
            started.msg,
            kodex_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            kodex_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
