...
//! - sensitive subpaths such as `.git`, `.agents`, and `.kodex` remain
...
fn create_filesystem_args(
    ...
    let daemon_directory = kodex_uds::prepare_shared_daemon_socket_directory()?;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.kodex`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".kodex")
                {
                    return None;
                }
    ...
    for daemon_directory in daemon_directories {
        ...
        if let Ok(relative) = daemon_directory.strip_prefix(&source) {
            ...
            let destination = destination
                .to_str()
                .ok_or_else(|| {
                    KodexErr::Fatal("app-server socket mask path must be valid UTF-8".to_string())
                })?
                .to_string();
            args.extend([
                "--perms".to_string(),
...
fn expand_unreadable_globs_with_ripgrep(
    ...
    for pattern in patterns {
        let Some((search_root, glob)) = split_pattern_for_ripgrep(pattern, cwd) else {
            return Err(KodexErr::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_kodex = temp_dir.path().join("real-kodex");
        let logical_kodex = logical_home.join(".kodex");
        let real_memories = real_kodex.join("memories");
        let logical_memories = logical_kodex.join("memories");
        ...
        std::os::unix::fs::symlink(&real_kodex, &logical_kodex)
            .expect("create symlinked kodex home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".kodex"));
    ...
    fn missing_child_git_under_parent_repo_is_mounted_read_only() {
        ...
        assert_eq!(
            synthetic_mount_target_paths(&args),
            vec![workspace.join(".kodex"), dot_git, workspace.join(".agents")],
        );
        assert!(
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_kodex = path_to_string(&temp_dir.path().join(".kodex"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_kodex));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_kodex)));
        assert_eq!(
    ...
    fn non_utf8_daemon_socket_mask_path_is_rejected() {
        let daemon_directory =
            PathBuf::from(OsString::from_vec(b"/tmp/kodex-daemon-\xff".to_vec()));
        assert!(
    ...
    fn read_only_tmp_carveout_keeps_daemon_sockets_masked() {
        ...
        let daemon_directory =
            path_to_string(&kodex_uds::shared_daemon_socket_directory().unwrap());
        assert_eq!(
