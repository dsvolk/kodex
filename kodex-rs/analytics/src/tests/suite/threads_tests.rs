...
async fn thread_initialized_classifies_validated_linked_worktrees() {
    let root = std::env::temp_dir().join(format!(
        "kodex-analytics-worktree-{}",
        kodex_protocol::ThreadId::new()
    ));
    ...
    for (cwd, expected) in [
        ...
        start.thread.cwd = kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(cwd)
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "kodex_thread_initialized");
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        kodex_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        kodex_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        kodex_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_tool_items_inherit_parent_connection_metadata() {
    ...
    assert_eq!(payload[1]["event_type"], "kodex_dynamic_tool_call_event");
