...
use kodex_app_server_protocol::ClientNotification;
...
use kodex_protocol::items::AgentMessageItem;
use kodex_protocol::items::EnteredReviewModeItem;
use kodex_protocol::items::ExitedReviewModeItem;
...
use kodex_protocol::user_input::UserInput as CoreUserInput;
...
async fn same_thread_retry_keeps_subscription_and_restores_draft() -> Result<()> {
    let (mut app, kodex_home) = make_history_test_app().await?;
...
pub(super) async fn start_recording_app_server_with_realtime_speech(
    ...
    let mut embedded = crate::start_embedded_app_server(
        kodex_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        loader_overrides,
        /*strict_config*/ false,
        kodex_config::CloudConfigBundleLoader::default(),
        kodex_feedback::KodexFeedback::new(),
        /*log_db*/ None,
        state_db,
        ...
        loop {
            let frame = tokio::select! {
                ...
                event = embedded.next_event() => {
                    ...
                    if let kodex_app_server_client::InProcessServerEvent::ServerNotification(notification) = event
                        ...
                        .send(Message::Text(
                            serde_json::to_string(&JSONRPCMessage::Response(JSONRPCResponse {
                                ...
                                result: serde_json::json!({
                                    "userAgent": "kodex-tui-test",
                                    "kodexHome": kodex_home,
                                }),
                            }))?
                            .into(),
                        ))
    ...
    Ok((
        AppServerSession::new(app_server, thread_params_mode)
            .with_local_kodex_home(&config.kodex_home),
        requests,
        proxy,
    ))
}
...
async fn delete_current_thread_navigates_only_after_success() -> Result<()> {
    ...
    for target in [
        ...
        let (mut app, _kodex_home) = make_history_test_app().await?;
        let thread_id =
...
async fn external_transport_registers_dynamic_tools_and_finds_task_mentions() -> Result<()> {
    ...
    for params in starts {
        ...
        assert_eq!(params["dynamicTools"][0]["name"], "kodex_tui");
        assert_eq!(
...
async fn local_daemon_registers_approval_gated_mcp_tools_for_both_start_paths() -> Result<()> {
    ...
    app.config
        .web_search_mode
        .set(kodex_protocol::config_types::WebSearchMode::Live)?;
    std::fs::write(
    ...
    let inventory: kodex_app_server_protocol::ListMcpServerStatusResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::McpServerStatusList {
            request_id: AppServerRequestId::String("tui-tool-inventory".to_string()),
            params: kodex_app_server_protocol::ListMcpServerStatusParams {
                cursor: None,
                limit: None,
                detail: Some(kodex_app_server_protocol::McpServerStatusDetail::ToolsAndAuthOnly),
    ...
    let tools = &inventory
        .data
        .iter()
        .find(|server| server.name == "kodex_tui")
    ...
    for params in &starts {
        ...
        let server = &params["config"]["mcp_servers.kodex_tui"];
        assert!(
            server["url"]
                .as_str()
    ...
    let mcp_url = starts[0]["config"]["mcp_servers.kodex_tui"]["url"]
        .as_str()
        .expect("MCP server URL");
    let unauthorized = kodex_http_client::HttpClientBuilder::new()
        .build_direct()?
        .post(mcp_url)
        .send()
        .await?;
    ...
    app.config
        .web_search_mode
        .set(kodex_protocol::config_types::WebSearchMode::Disabled)?;
    ...
    assert_eq!(
        resumed["config"]["mcp_servers.kodex_tui"],
        starts[0]["config"]["mcp_servers.kodex_tui"]
    );
    Box::pin(app_server.resume_thread(
        &app.local_settings,
        app.config.clone(),
    ...
    assert_eq!(
        reattached["config"]["mcp_servers.kodex_tui"],
        starts[0]["config"]["mcp_servers.kodex_tui"]
    );
    ...
    assert_eq!(
        forked["config"]["mcp_servers.kodex_tui"],
        starts[0]["config"]["mcp_servers.kodex_tui"]
    );
    let authorization =
        starts[0]["config"]["mcp_servers.kodex_tui"]["http_headers"]["Authorization"]
            .as_str()
            .expect("MCP bearer token");
    let client = kodex_http_client::HttpClientBuilder::new().build_direct()?;
    ...
    assert_eq!(
        child["config"]["mcp_servers.kodex_tui"],
        starts[0]["config"]["mcp_servers.kodex_tui"]
    );
...
async fn local_mcp_respects_configured_servers_and_managed_requirements() -> Result<()> {
    for scenario in ["conflicting", "blocked", "mismatched", "allowed"] {
        ...
        if scenario == "conflicting" {
            let raw = serde_json::from_value::<kodex_config::RawMcpServerConfig>(
            ...
            servers.insert(
                ...
                kodex_config::McpServerConfig::try_from(raw)
            ...
            if matches!(scenario, "mismatched" | "allowed") {
                let requirement = if scenario == "allowed" {
                    kodex_config::McpServerRequirement::Url(
                        kodex_protocol::mcp_policy::McpServerValueMatcher::Prefix {
                            value: "http://127.0.0.1:".to_string(),
                        },
                    )
                } else {
                    kodex_config::McpServerRequirement::Identity {
                        identity: kodex_config::McpServerIdentity::Url {
            ...
            let requirements = kodex_config::ConfigRequirements {
                mcp_servers: Some(kodex_config::Sourced::new(
                    allowed_servers,
                    kodex_config::RequirementSource::Unknown,
                )),
                ..Default::default()
            };
            app.config.config_layer_stack = kodex_config::ConfigLayerStack::new(
                Vec::new(),
                requirements,
        ...
        if scenario == "allowed" {
            ...
            assert!(start["config"]["mcp_servers.kodex_tui"].is_object());
        } else {
            assert_eq!(
                start["dynamicTools"][0]["tools"].as_array().map(Vec::len),
                Some(6)
            );
            assert!(start["config"]["mcp_servers.kodex_tui"].is_null());
        }
        app_server.shutdown().await?;
        proxy.await??;
    }
...
async fn embedded_server_rejects_unowned_dynamic_tool_calls() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: kodex_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("kodex_app".to_string()),
...
async fn dynamic_tool_requests_ignore_other_namespaces_and_dispatch_tui_namespace() -> Result<()> {
    ...
    for namespace in [Some("kodex_app"), None] {
        app.handle_app_server_event(
            &app_server,
            kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(
                ServerRequest::DynamicToolCall {
        ...
        kodex_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: kodex_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("kodex_tui".to_string()),
                    tool: "list_threads".to_string(),
                    arguments: serde_json::json!({}),
                },
            },
    ...
    app_server
        ...
        .await?;
    let project: kodex_app_server_protocol::ProjectCreateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ProjectCreate {
            request_id: AppServerRequestId::String("create-source-project".to_string()),
            params: kodex_app_server_protocol::ProjectCreateParams {
                name: "Source project".to_string(),
                roots: vec![kodex_app_server_protocol::ProjectRoot {
        ...
        .await?;
    let _: kodex_app_server_protocol::ThreadMetadataUpdateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadMetadataUpdate {
            request_id: AppServerRequestId::String("assign-source-project".to_string()),
            params: kodex_app_server_protocol::ThreadMetadataUpdateParams {
        ...
        .await?;
    let source_settings: kodex_app_server_protocol::ThreadResumeResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadResume {
            request_id: AppServerRequestId::String("read-source-sandbox".to_string()),
            params: kodex_app_server_protocol::ThreadResumeParams {
                thread_id: creation_source.to_string(),
                ..kodex_app_server_protocol::ThreadResumeParams::default()
            },
        })
        .await?;
    ...
    spawn_approved_task_tool_call(
        ...
        kodex_app_server_protocol::DynamicToolCallParams {
            thread_id: creation_source.to_string(),
            turn_id: "turn-1".to_string(),
...
async fn older_pagination_reconciles_review_prompts_across_page_boundaries() -> Result<()> {
    ...
    app.local_settings.tui.alternate_screen = kodex_config::types::AltScreenMode::Never;
...
async fn transcript_alt_beginning_loads_every_older_history_page() -> Result<()> {
    ...
    let export_path = kodex_home.path().join("complete-export.md");
    app.chat_widget
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    ...
    let response = crate::dynamic_tools::execute(
        ...
        kodex_app_server_protocol::ThreadStartParams::default(),
        status_updates,
        /*app_event_tx*/ None,
    )
    .await;
...
async fn underfilled_scrollback_fetches_older_pages_without_opening_the_transcript() -> Result<()> {
    ...
    if let Some(Overlay::Transcript(overlay)) = app.overlay.as_mut() {
        ...
        assert!(!partial.contains("OpenAI Kodex"));
        ...
        assert!(!loading.contains("OpenAI Kodex"));
                ...
                app.handle_app_server_event(
                    ...
                    AppServerEvent::ServerNotification(Box::new(
                        ServerNotification::ThreadStatusChanged(
                            kodex_app_server_protocol::ThreadStatusChangedNotification {
                                thread_id: started.session.thread_id.to_string(),
...
async fn cold_paginated_subagent_transcript_excludes_inherited_parent_history() -> Result<()> {
    let (app, kodex_home) = make_history_test_app().await?;
    let parent_thread_id = create_history_rollout(
        &app.config,
        ThreadHistoryMode::Paginated,
...
async fn managed_worktree_transitions_bind_owner_and_preserve_only_fork_history() -> Result<()> {
    ...
    fs::create_dir_all(source.join(".kodex"))?;
...
"#,
    ...
    fs::write(source.join(".kodex/config.toml"), destination)?;
...
"#,
    ...
    app.config.kodex_home = home.clone().abs();
    ...
    let manager = kodex_worktree::WorktreeManager::new(
        kodex_worktree::WorktreeSettings::for_cli(&home, /*desktop*/ None)
            .map_err(|error| color_eyre::eyre::eyre!(error.to_string()))?,
    );
...
"#,
    ...
    for mode in [ManagedWorktreeMode::New, ManagedWorktreeMode::Fork] {
        ...
        let metadata = kodex_rollout::read_session_meta_line(&rollout).await?;
    ...
    let unowned = manager
        .create(&kodex_worktree::CreateWorktree {
    ...
    kodex_rollout::spawn_rollout_compression_worker(
        home.clone(),
        kodex_rollout::RolloutCompressionTrigger::Startup,
    );
...
async fn changing_directory_preserves_project_trust_permissions_history_and_hooks() -> Result<()> {
    use kodex_protocol::config_types::TrustLevel as T;
    use serde_json::json;
    use std::fs;
    ...
    let [current, trusted, unknown, untrusted, mismatch, failed] =
        names.map(|name| kodex_home.path().join(name));
    fs::create_dir_all(&current)?;
    for directory in [&trusted, &unknown, &untrusted, &mismatch, &failed] {
        fs::create_dir_all(directory.join(".kodex"))?;
        fs::write(directory.join(".kodex/config.toml"), "")?;
    }
    let contents = "developer_instructions = \"destination policy\"\nmodel_reasoning_effort = \"high\"\napproval_policy = \"on-request\"\n[tui]\ntheme = \"dracula\"\n[tui.keymap.global]\nopen_transcript = \"f12\"";
    fs::write(trusted.join(".kodex/config.toml"), contents)?;
    ...
    fs::write(trusted.join(".kodex/hooks.json"), hooks)?;
    let contents = "default_permissions = \"dev\"\n[permissions.dev.filesystem]\n\":root\" = \"write\"\n[tui.keymap.global]\nopen_transcript = \"ctrl-l\"";
    fs::write(mismatch.join(".kodex/config.toml"), contents)?;
    let requirements = kodex_home.path().join("requirements.toml");
    ...
    for dir in [&trusted, &untrusted, &mismatch, &failed] {
        ...
        crate::legacy_core::config::set_project_trust_level(kodex_home.path(), dir, trust)
            .map_err(|error| color_eyre::eyre::eyre!(error.to_string()))?;
    }
    ...
    let meta = kodex_rollout::read_session_meta_line(&forked_rollout).await?;
    ...
    let source = kodex_utils_path_uri::PathUri::from_abs_path(&agents.abs());
...
async fn command_center_read_only_open_requests_and_failure_preservation() -> Result<()> {
    for (history_capabilities, saved_turn_count) in [
        ...
        let (mut app, _kodex_home) = Box::pin(make_history_test_app()).await?;
        std::fs::write(
                ...
                let events = [
                    ...
                    EventMsg::ItemCompleted(ItemCompletedEvent {
                        ...
                        item: TurnItem::AgentMessage(AgentMessageItem {
                            ...
                            phase: Some(kodex_protocol::models::MessagePhase::FinalAnswer),
                            memory_citation: None,
                            delivery: None,
                            questions: None,
                        }),
