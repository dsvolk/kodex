...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    let mut terminal = PtyKodex::start(&repo_root, kodex_home, &["--no-alt-screen"])?;
...
async fn interactive_startup_honors_kodex_home_symlink_opt_out() -> Result<()> {
    ...
    let visualizations = kodex_home.path().join("visualizations");
    std::os::unix::fs::symlink(target.path(), &visualizations)?;
    write_test_config(kodex_home.path(), &workspace_path)?;
    ...
    let mut terminal = PtyKodex::start(
        &workspace_path,
        kodex_home,
        &[
            "--no-alt-screen",
...
fn owned_screen_entry_paints_before_sync_ends_and_exit_clears_inline_draft() -> Result<()> {
    ...
    let first_contents = first_frame.screen().contents();
        ...
        first_contents.contains("OpenAI Kodex")
            && first_contents.contains("Ask Kodex to do anything"),
        "owned-screen synchronization ended before its first complete loading frame:\n{first_contents}"
    );
    let composer_row = first_contents
        .lines()
        .position(|line| line.contains("Ask Kodex to do anything"))
        .context("missing composer in first owned-screen frame")?;
    assert_eq!(
        (
            first_contents.matches("Ask Kodex to do anything").count(),
    ...
    loop {
        ...
        !terminal
            .screen_contents()
            .contains("Ask Kodex to do anything"),
...
fn fullscreen_transcript_defaults_to_terminal_scrollback() -> Result<()> {
    ...
    let kodex_home = tempfile::tempdir()?;
    write_test_config(kodex_home.path(), &repo_root)?;
    let mut terminal = PtyKodex::start(&repo_root, kodex_home, &[])?;
    terminal.wait_for_startup()?;
    terminal.wait_for_screen("GPT-5.6-Terra")?;
    ensure!(
        !terminal
...
impl PtyKodex {
    ...
    fn start_binary(
        ...
        let child = Command::new(kodex)
            ...
            .stderr(slave)
            .spawn()
            .context("start Kodex in focus-test pseudo-terminal")?;
    ...
    pub(super) fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered && self.screen_contains("OpenAI Kodex") {
                return Ok(());
            }

            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "Kodex exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "Kodex did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
    ...
    pub(super) fn wait_for_screen(&mut self, text: &str) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if let Some(status) = self.child.try_wait()? {
                bail!("Kodex exited while waiting for {text:?} ({status})");
            }
        }
    ...
    pub(super) fn ensure_running(&mut self) -> Result<()> {
            ...
            "Kodex exited unexpectedly; screen:\n{}",
            self.screen_contents()
        );
        Ok(())
    }
...
pub(super) fn write_test_config(kodex_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(kodex_home.join("config.toml"), config)
        .context("write focus-test Kodex configuration")?;
    std::fs::write(
...
fn no_daemon_skips_startup_and_discovery() -> Result<()> {
    for running in [false, true] {
        ...
        let socket_path = kodex_app_server_client::app_server_control_socket_path(home.path())?;
        ...
        let mut terminal = PtyKodex::start(workspace.path(), home, &["--no-daemon"])?;
        terminal.wait_for_startup()?;
...
fn auto_daemon_start_failure_exits_with_manual_fallback_hint() -> Result<()> {
    ...
    while Instant::now() < deadline {
        ...
        if let Some(status) = terminal.child.try_wait()? {
            ...
            let failure = kodex_ansi_escape::ansi_escape(failure)
                .to_string()
                .replace(
                    terminal
                        ._kodex_home
                ...
                .replace(
                    terminal._kodex_home.path().to_string_lossy().as_ref(),
                    "[KODEX_HOME]",
                )
                .replace('\r', "");
