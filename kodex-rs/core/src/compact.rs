...
use kodex_analytics::CompactionStatus;
use kodex_analytics::CompactionStrategy;
use kodex_analytics::CompactionTrigger;
use kodex_analytics::KodexCompactionEvent;
use kodex_analytics::now_unix_seconds;
use kodex_context_fragments::AnnotatedContent;
...
pub use kodex_prompts::SUMMARIZATION_PROMPT;
pub use kodex_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    {
        sess.track_turn_kodex_error(turn_context.as_ref(), err);
...
async fn run_compact_task_inner_impl(
    ...
    let compaction_response = loop {
        ...
        match attempt_result {
            ...
            Err(e) if matches!(e.details(), KodexErrorDetails::SessionBudgetExceeded) => {
                return Err(e);
            }
            Err(e) if matches!(e.details(), KodexErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(KodexCompactionEvent {
                ...
                status,
                kodex_error_kind: kodex_error.map(Into::into),
                kodex_error_http_status_code: kodex_error
                    .and_then(KodexErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &KodexResult<T>) -> CompactionStatus {
    match result {
...
pub(crate) struct CompactedUserMessage {
    ...
    harness_metadata: Option<KodexHarnessMetadata>,
}
...
async fn drain_to_completed(
    ...
    prompt: &Prompt,
    phase: CompactionPhase,
) -> KodexResult<CompactionResponse> {
