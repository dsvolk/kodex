...
async fn direct_call_metadata_during_compaction_respects_provider_support(
    ...
    .await;
    let compacted = test.kodex.conversation_history_snapshot().await;
    ...
    match remote_compaction {
        RemoteCompactionSupport::Unsupported => assert!(compacted.iter().any(|item| {
            item["role"] == "user"
                && item["content"][0]["text"]
                    == format!("{}\n{summary}", kodex_core::compact::SUMMARY_PREFIX)
        })),
    ...
    let captured = test.kodex.conversation_history_snapshot().await;
