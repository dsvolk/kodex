# Kodex package builder

This package contains the implementation behind `scripts/build_kodex_package.py`.
...
```bash
just assemble-kodex-package --help
just assemble-kodex-package --variant kodex-app-server
just assemble-kodex-package --target x86_64-unknown-linux-gnu
```

The builder creates a canonical Kodex package directory:
...
host platform. On Linux, that default is a musl target to match Kodex release
...
`kodex` and `kodex-app-server`. The `--package-version` flag sets the version in
`kodex-package.json`; it defaults to `[workspace.package].version` in
`kodex-rs/Cargo.toml`.
...
- all targets: `kodex-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `kodex-command-runner` and `kodex-windows-sandbox-setup`,
...
`--kodex-command-runner-bin` plus `--kodex-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Kodex-built V8 release pair before invoking
...
DotSlash manifest at `scripts/kodex_package/rg`. Downloaded archives are cached
under `$TMPDIR/kodex-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/kodex_package/kodex-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/kodex-package/<target>-zsh` and installed at
`kodex-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed
