...
const READY_MARKER: &str = "__KODEX_CHILD_READY__";
const VALUE_MARKER: &str = "__KODEX_CHILD_VALUE__";
...
async fn assert_normal_exit_preserves_descendant(
    ...
    let marker_base = std::env::temp_dir().join(format!(
        "kodex-job-natural-exit-{backend}-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
...
async fn conpty_delivers_input_to_foreground_children() -> anyhow::Result<()> {
    ...
    let code = format!(
        "print('__KODEX_CHILD_'+'READY__', flush=True); value=input(); print('{VALUE_MARKER}'+value.encode('utf-8').hex(), flush=True)"
    );
