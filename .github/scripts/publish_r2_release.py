#!/usr/bin/env python3
"""Mirror a Kodex GitHub Release to Cloudflare R2.
...
Objects are created under ``kodex/releases/<version>/`` with a validated upload
...
objects are verified, the same metadata advances ``kodex/channels/latest`` when
the release is marked latest and ``kodex/channels/prerelease`` for prereleases.
Stable releases also update the mutable ``kodex/install.sh`` and
``kodex/install.ps1`` bootstrap aliases from their verified versioned assets.
"""
