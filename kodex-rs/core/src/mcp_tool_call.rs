...
use kodex_analytics::AppInvocation;
use kodex_analytics::ElicitationType;
use kodex_analytics::InvocationType;
use kodex_analytics::McpToolCallElicitation;
use kodex_analytics::build_track_events_context;
use kodex_api::HostedFileUploadContext;
use kodex_config::ConfigLayerSource;
use kodex_config::types::AppToolApproval;
use kodex_connectors::AppToolPolicy;
use kodex_connectors::AppToolPolicyEvaluator;
use kodex_connectors::AppToolPolicyInput;
use kodex_extension_api::McpToolContext;
use kodex_features::Feature;
use kodex_mcp::KODEX_APPS_MCP_SERVER_NAME;
use kodex_mcp::MCP_TOOL_KODEX_APPS_META_KEY;
use kodex_mcp::McpPermissionPromptAutoApproveContext;
use kodex_mcp::PreparedMcpCall;
use kodex_mcp::SandboxState;
use kodex_mcp::ToolInfo;
use kodex_mcp::auth_elicitation_completed_result;
use kodex_mcp::build_auth_elicitation_plan;
use kodex_mcp::is_connector_auth_failure_from_tool_result;
use kodex_mcp::mcp_permission_prompt_is_auto_approved;
use kodex_prompts::ResolvedModelMessages;
use kodex_protocol::approvals::ElicitationRequest;
use kodex_protocol::items::McpAppDisplayMode;
use kodex_protocol::items::McpAppUi;
use kodex_protocol::items::McpToolCallError;
use kodex_protocol::items::McpToolCallItem;
use kodex_protocol::items::McpToolCallStatus;
use kodex_protocol::items::TurnItem;
use kodex_protocol::mcp::CONFIRMATION_POLICIES_META_KEY;
use kodex_protocol::mcp::CallToolResult;
use kodex_protocol::mcp::is_node_repl_backed_server;
use kodex_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use kodex_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use kodex_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use kodex_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use kodex_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use kodex_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use kodex_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use kodex_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use kodex_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use kodex_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use kodex_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use kodex_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use kodex_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use kodex_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use kodex_protocol::request_user_input::RequestUserInputQuestion;
use kodex_protocol::request_user_input::RequestUserInputQuestionOption;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "kodex/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "kodex.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "kodex.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == KODEX_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == KODEX_APPS_MCP_SERVER_NAME {
    ...
    if server == KODEX_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
        let result = notify_mcp_tool_call_skip(
            sess.as_ref(),
            turn_context.as_ref(),
            &call_id,
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == KODEX_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.kodex_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    let result = async {
        let result = async {
            ...
            Ok(maybe_request_kodex_apps_auth_elicitation(
                sess,
                turn_context,
    ...
    sess.services.executed_tool_calls.record_mcp_source(
        kodex_protocol::mcp::McpAttributionSource {
    ...
    .await;
    maybe_track_kodex_app_used(sess, step_context, &server, &metadata, elicitation_type).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        kodex.mcp.target.id = Empty,
        kodex.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        kodex.mcp.error.code = Empty,
    );
...
async fn maybe_request_kodex_apps_auth_elicitation(
    sess: &Arc<Session>,
    turn_context: &TurnContext,
    ...
    if server != KODEX_APPS_MCP_SERVER_NAME {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        kodex_connectors::metadata::connector_install_url(
    ...
    refresh_kodex_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_kodex_apps_after_connector_auth(sess: &Arc<Session>, turn_context: &TurnContext) {
    let mcp_tools_result = sess.hard_refresh_latest_kodex_apps_tools().await;
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Kodex Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        kodex_linux_sandbox_exe: prepared_call.config().kodex_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                kodex_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == kodex_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_kodex_app_used(
    sess: &Session,
    step_context: &StepContext,
    server: &str,
    metadata: &McpToolApprovalMetadata,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_ui: Option<McpAppUi>,
    kodex_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
impl Session {
    fn register_mcp_tool_approval_metadata(
        ...
        let metadata = Arc::new((
            (invocation.server == KODEX_APPS_MCP_SERVER_NAME
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, kodex_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_KODEX_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == KODEX_APPS_MCP_SERVER_NAME {
        let mut kodex_apps_meta = metadata
            .and_then(|metadata| metadata.kodex_apps_meta.clone())
            .unwrap_or_default();
        kodex_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_KODEX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(kodex_apps_meta),
        );
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    let action = ApprovalAction::McpToolCall {
        ...
        connected_account_email: (invocation.server == KODEX_APPS_MCP_SERVER_NAME)
    ...
    Some(
        match sess.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Kodex(_)) => ReviewDecision::Abort,
        },
    )
}
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == KODEX_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
fn mcp_tool_metadata(
    ...
    let connector_description = (server == KODEX_APPS_MCP_SERVER_NAME)
        .then(|| tool_info.namespace_description.clone())
        .flatten();

    let kodex_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
        .and_then(|meta| meta.get(MCP_TOOL_KODEX_APPS_META_KEY))
        .and_then(serde_json::Value::as_object)
        .cloned();
    let link_id = if server == KODEX_APPS_MCP_SERVER_NAME {
        account::resolve_account(&tool_info, arguments)?
    } else {
        None
    };
    let connected_account_email = if server == KODEX_APPS_MCP_SERVER_NAME {
        kodex_apps_meta
            .as_ref()
    ...
    if let Some(metadata) = metadata {
                ...
                || metadata.connector_description.is_some())
        {
            ...
            if let Some(link_id) = metadata.link_id.as_ref() {
                ...
                // Match the Kodex Apps service's reserved implicit-link ID prefix.
                meta.insert(
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == KODEX_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_kodex_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_kodex_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, kodex_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
