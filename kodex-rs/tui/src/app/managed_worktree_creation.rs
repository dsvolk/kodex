...
use kodex_app_server_protocol::ThreadBackgroundTerminalsListResponse as ListResponse;
...
pub(super) fn background_terminals_blocker(
    ...
    match result {
        ...
        {
            Some(
                "The local Kodex service cannot check background terminals. Run `kodex app-server daemon update`, then restart Kodex.",
            )
        }
    ...
    pub(super) async fn start_managed_worktree(
        ...
        if !self.config.features.enabled(Feature::Worktrees) {
            self.chat_widget.add_error_message(
                "Enable worktrees in your Kodex configuration to create a worktree.".to_string(),
            );
            ...
            .is_none_or(|thread_id| !self.chat_widget.can_change_working_directory(thread_id))
        {
            ...
            let setup = async {
                ...
                .await?;
                let settings = kodex_worktree::WorktreeSettings::for_cli(
                    &self.config.kodex_home,
    ...
    pub(super) async fn finish_managed_worktree(&mut self, created: ManagedWorktreeCreated) {
        ...
        let config = match self.rebuild_config_for_cwd(checkout.cwd.clone()).await {
            ...
            Ok(_) => {
                return self.retained_worktree_error(
                    &checkout,
                    "The new worktree is not trusted; run Kodex there.",
                );
            }
            Err(error) => {
                return self.retained_worktree_error(
