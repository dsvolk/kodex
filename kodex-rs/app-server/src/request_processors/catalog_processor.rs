...
fn skills_to_info(
    skills: &[kodex_skills::SkillMetadata],
    disabled_paths: &HashSet<AbsolutePathBuf>,
) -> Vec<kodex_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            kodex_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    kodex_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| kodex_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[kodex_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[kodex_skills::SkillError],
) -> Vec<kodex_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| kodex_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
    ...
    pub(crate) async fn model_list(
        ...
        if let Some(gateway) = kodex_model_provider::create_model_provider(
            ...
            if gateway.resolve_access_token().await.is_err() {
                    ...
                    || current.model_provider != self.config.model_provider
                {
                    return Err(invalid_request(
                        "Model provider settings changed. Restart Kodex to apply them, then retry fetching the model list",
                    ));
                }
                return Err(invalid_request(
    ...
    async fn list_models(
        ...
        let presets = self
            .model_catalog
            .list_models(kodex_models_manager::manager::RefreshStrategy::OnlineIfUncached)
            .await
            .map_err(|err| config_load_error(&err))?;
                ...
                async move {
                    let config = match self.load_latest_config(Some(cwd.clone())).await {
                        ...
                        Err(error) => {
                            ...
                            return (
                                index,
                                kodex_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![kodex_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                    ...
                    let skills_input = kodex_skills_extension::HostSkillsLoadInput::new(
                        config.cwd.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(kodex_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![kodex_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let hooks_enabled = config.features.enabled(Feature::KodexHooks);
            let plugin_hooks = if hooks_enabled && config.features.enabled(Feature::Plugins) {
                let plugins_input = config.plugins_config_input();
                let plugin_outcome = plugins_manager.plugins_for_config(&plugins_input).await;
                kodex_core_plugins::PluginHookLoadOutcome {
                ...
                kodex_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = kodex_hooks::list_hooks(kodex_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.kodex_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {
