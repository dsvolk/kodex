...
const REALTIME_WIRE_LOG_TARGET: &str = "kodex_api::realtime_websocket::wire";
    ...
    fn new(
        ...
        let pump_task = tokio::spawn(async move {
            ...
            let report_write_policy_error = |error: &WsError| {
                if let Some(denied) = kodex_websocket_client::network_policy_denial(error) {
                        ...
                        match message {
                            Ok(Message::Ping(payload)) => {
                                ...
                                if let Err(err) = inner.send(Message::Pong(payload)).await {
                                    ...
                                    terminal_policy_tx.send_replace(kodex_websocket_client::network_policy_denial(&err));
...
impl RealtimeWebsocketClient {
    ...
    async fn connect_sideband(
        ...
        for attempt in 0..=self.provider.retry.max_attempts {
            ...
            match result {
                ...
                Err(err @ ApiError::Transport(kodex_http_client::TransportError::Policy(_))) => {
                    return Err(err);
                }
    ...
    async fn connect_realtime_websocket_url(
        ...
        // Realtime websocket TLS should honor the same custom-CA env vars as the rest of Kodex's
        ...
        let tls_config = match tls_config {
            ...
            None => Some(
                kodex_http_client::build_windows_platform_tls_config().map_err(|err| {
...
fn map_realtime_websocket_connect_error(err: WsError) -> ApiError {
    if let Some(denied) = kodex_websocket_client::network_policy_denial(&err) {
        return ApiError::Transport(kodex_http_client::TransportError::Policy(denied));
    }
    match err {
...
mod tests {
    ...
    use kodex_protocol::protocol::RealtimeInputAudioSpeechStarted;
    use kodex_protocol::protocol::RealtimeNoopRequested;
    use kodex_protocol::protocol::RealtimeResponseCancelled;
    use kodex_protocol::protocol::RealtimeResponseCreated;
    use kodex_protocol::protocol::RealtimeResponseDone;
    use kodex_protocol::protocol::RealtimeTranscriptDelta;
    use kodex_protocol::protocol::RealtimeTranscriptDone;
    ...
    async fn writer_policy_failure_reaches_realtime_events() {
        use kodex_http_client::DestinationPolicy::Unrestricted;
        use kodex_http_client::NetworkPolicyController;
        use kodex_http_client::NetworkPolicyDenied::Revoked;
        use kodex_http_client::TransportError::Policy;
        use kodex_websocket_client::network_policy_denial;
    ...
    fn webrtc_frameless_sideband_ignores_provider_base_url() {
        let client = test_client(Provider {
            ...
            base_url: "https://chatgpt.com/backend-api/kodex".to_string(),
            query_params: None,
