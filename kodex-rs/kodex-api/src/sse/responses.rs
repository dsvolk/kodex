...
struct ResponseCompletedUsage {
    ...
    total_tokens: i64,
    #[serde(default)]
    kodex_rollout_budget_units: Option<serde_json::Number>,
}
...
impl From<ResponseCompletedUsage> for TokenUsage {
    fn from(val: ResponseCompletedUsage) -> Self {
        ...
        TokenUsage {
            ...
            kodex_rollout_budget_units: val.kodex_rollout_budget_units,
        }
    }
}
...
fn header_turn_state_value_from_json(value: &Value) -> Option<String> {
    let headers = value.as_object()?;
    headers.iter().find_map(|(name, value)| {
        if name.eq_ignore_ascii_case(X_KODEX_TURN_STATE_HEADER) {
            json_value_as_string(value)
        } else {
            None
        }
            ...
            Ok(Some(Err(e))) => {
                ...
                let error = match e {
                    eventsource_stream::EventStreamError::Transport(
                        error @ kodex_client::TransportError::Policy(_),
...
mod tests {
    ...
    fn parses_cache_write_token_usage() {
        let usage: ResponseCompletedUsage = serde_json::from_value(json!({
            ...
            "total_tokens": 110,
            "kodex_rollout_budget_units": 2.5
        }))
        ...
        assert_eq!(
            ...
            TokenUsage {
                ...
                total_tokens: 110,
                kodex_rollout_budget_units: serde_json::Number::from_f64(2.5),
            }
        );
    }
    ...
    const CYBER_RESTRICTED_MODEL_FOR_TESTS: &str = "gpt-5.3-kodex";
}
