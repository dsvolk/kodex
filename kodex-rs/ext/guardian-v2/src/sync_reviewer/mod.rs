...
use kodex_core::KodexResponsesHeaders;
...
use kodex_core::guardian_review::GuardianReviewSession;
...
use kodex_guardian_reviewer::ReviewDenials;
use kodex_guardian_reviewer::ReviewerPool;
use kodex_guardian_reviewer::ReviewerTasks;
...
impl ThreadLifecycleContributor<Config> for GuardianExtension {
    fn on_thread_start<'a>(
        ...
        Box::pin(async move {
            ...
            input
                .thread_store
                .insert(kodex_guardian_reviewer::ReviewerConfig::<Config>(
                ...
                ReviewerPool::<GuardianReviewSession>::new(
                    ...
                    move |context, key, kind, snapshot, cancel| {
                        ...
                        Box::pin(async move {
                            ...
                            if matches!(
                                kind,
                                kodex_analytics::GuardianReviewSessionKind::EphemeralForked
                            ) {
                            ...
                            let provider = kodex_model_provider::create_model_provider(
                                options.config.model_provider.clone(),
                                /*auth_manager*/ None,
                            );
                            options.thread_extension_init.insert(KodexResponsesHeaders {
                                model: provider.approval_review_preferred_model().to_owned(),
                                headers: http::HeaderMap::from_iter([(
                                    http::HeaderName::from_static("x-kodex-guardian"),
                            ...
                            options
                                .thread_extension_init
                                .insert(kodex_guardian_reviewer::reviewer_tool_policy());
    ...
    fn on_thread_ready<'a>(
        ...
        Box::pin(async move {
            ...
            tokio::spawn(async move {
                ...
                let prepare = async {
                    let context =
                        kodex_core::guardian_review::prepare_review_prewarm(&parent).await?;
