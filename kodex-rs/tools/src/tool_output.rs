...
impl ToolOutput for kodex_protocol::mcp::CallToolResult {
    fn log_output(&self) -> String {
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    kodex_protocol::models::ContentItem::InputText { text }
                    | kodex_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    kodex_protocol::models::ContentItem::InputImage { image, detail } => {
                        FunctionCallOutputContentItem::InputImage {
                            image,
                            detail: detail.or(Some(DEFAULT_IMAGE_DETAIL)),
                        }
                    }
                    kodex_protocol::models::ContentItem::InputAudio { audio_url } => {
