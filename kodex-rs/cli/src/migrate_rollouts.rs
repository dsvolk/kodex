...
use kodex_thread_store::RolloutMigrationMode;
use kodex_thread_store::RolloutMigrationOptions;
use kodex_thread_store::RolloutMigrationProgress;
use kodex_thread_store::RolloutMigrationReport;
use kodex_thread_store::RolloutMigrationStatus;
...
pub(crate) async fn run(
    ...
    kodex_core::otel_init::record_process_start(otel.as_ref(), "kodex_migrate_rollouts");
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            kodex_rollout::state_db::try_init(&config)
                .await
...
fn print_human_report(
    ...
    if mode == RolloutMigrationMode::DryRun && counts.eligible > 0 {
        println!("Run `kodex migrate-rollouts --apply` to migrate eligible sessions.");
    }
...
fn print_outcome(outcome: &kodex_thread_store::RolloutMigrationOutcome) {
...
async fn thread_storage_bytes(kodex_home: &Path, thread_history_db_path: &Path) -> io::Result<u64> {
    let mut bytes = 0_u64;
    let mut directories = vec![
        kodex_home.join(kodex_rollout::SESSIONS_SUBDIR),
        kodex_home.join(kodex_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ];
