...
use kodex_app_server_protocol::SelectedCapabilityRoot;
use kodex_app_server_protocol::ThreadHistoryMode as ApiThreadHistoryMode;
use kodex_app_server_protocol::ThreadRevertParams;
use kodex_app_server_protocol::ThreadRevertResponse;
use kodex_app_server_protocol::ThreadRevertedNotification;
use kodex_app_server_protocol::ThreadSection;
use kodex_app_server_protocol::ThreadSectionAppearance;
use kodex_app_server_protocol::ThreadSectionMoveParams;
use kodex_app_server_protocol::ThreadSectionMoveResponse;
...
use kodex_protocol::SanitizedGitUrl;
...
struct ThreadRevertRuntimeSnapshot {
    config: Config,
    settings: KodexThreadSettingsOverrides,
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: kodex_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                kodex_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = kodex_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
pub(crate) enum ThreadResumeTarget {
    ...
    DaemonRecovery(Option<kodex_app_server_transport::daemon_recovery::InterruptedTurn>),
}
...
impl ThreadRequestProcessor {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<KodexThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            kodex_home: self.config.kodex_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(kodex_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<kodex_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<kodex_app_server_protocol::ThreadStartSource>,
        thread_source: Option<kodex_protocol::protocol::ThreadSource>,
        ...
        let effective_permissions_trust_project = match &effective_permission_profile {
            kodex_protocol::models::PermissionProfile::Disabled
            | kodex_protocol::models::PermissionProfile::External { .. } => true,
            kodex_protocol::models::PermissionProfile::Managed { .. } => {
            ...
            && effective_permissions_trust_project
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                kodex_core::config::set_project_trust_level(
                    &listener_task_context.kodex_home,
        ...
        let mut config_warnings = config
            ...
            .collect::<Vec<_>>();
        if let Ok(Some(err)) =
            kodex_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let new_thread = listener_task_context
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(kodex_app_server_protocol::ThreadStartSource::Startup)
                {
                    kodex_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    kodex_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
        ...
        let NewThread {
            ...
            Err(err) => {
                ...
                return Err(match err.details() {
                    ...
                    KodexErrorDetails::UnsupportedOperation(message) => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<kodex_app_server_protocol::AskForApproval>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(kodex_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(kodex_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            kodex_linux_sandbox_exe: self.arg0_paths.kodex_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = kodex_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.kodex_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.kodex_home.display()
                ))
            })?;
    ...
    async fn thread_revert_response(
        ...
        let revert_result = self
            .thread_store
            .revert_thread(kodex_thread_store::RevertThreadParams {
                thread_id,
                before_turn_id,
    ...
    async fn reload_paginated_thread(
        ...
        let thread_state = self.thread_state_manager.thread_state(thread_id).await;
        self.ensure_listener_task_running(thread_id, Arc::clone(&kodex_thread), thread_state)
            .await?;
        let mut thread = self
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                kodex_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<kodex_app_server_protocol::TurnsPage, JSONRPCErrorError> {
            ...
            .await
        {
            Ok(NewThread {
                ...
                let ThreadResumeTarget::Client(request_id) = target else {
                    ...
                    self.ensure_listener_task_running(thread_id, Arc::clone(&kodex_thread), state)
                        .await?;
                    return Ok(ControlFlow::Break(()));
                };
                ...
                let instruction_sources = kodex_thread.legacy_instruction_sources().await;
                let SessionConfiguredEvent { rollout_path, .. } = session_configured;
                ...
                thread.thread_source = kodex_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = kodex_thread.config_snapshot().await;
                let (turns_backwards_cursor, items_backwards_cursor) =
                    if matches!(config_snapshot.history_mode, ThreadHistoryMode::Paginated) {
    ...
    async fn read_stored_thread_for_resume(
            ...
            && matches!(stored_thread.history_mode, ThreadHistoryMode::Paginated)
        {
            ...
            if let Some(current_path) = current_thread.rollout_path.as_ref()
                && !path_utils::paths_match_after_normalization(
                    kodex_rollout::plain_rollout_path(requested_path).as_path(),
                    kodex_rollout::plain_rollout_path(current_path).as_path(),
                )
            {
                return Err(invalid_request(format!(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `kodex unarchive {thread_id}` to unarchive it first."
            )));
        }
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(kodex_core::util::normalize_thread_name);
        let mut prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    kodex_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    kodex_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => kodex_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(kodex_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            Err(err) => {
                remove_pending_thread_metadata(self.thread_store.as_ref(), reserved_thread_id)
                    .await;
                return Err(match err.details() {
                    KodexErrorDetails::Io(_) | KodexErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: kodex_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        kodex_error_info: error.kodex_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: KodexErr) -> JSONRPCErrorError {
    match err.details() {
        KodexErrorDetails::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        KodexErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
        KodexErrorDetails::UnsupportedOperation(message) => method_not_found(message.clone()),
...
) -> (Thread, Option<kodex_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<kodex_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(kodex_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match kodex_core::parse_turn_item(&item.item) {
                Some(kodex_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<kodex_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
