...
complies, `model/list` returns JSON-RPC error `-32600` asking the client to restart Kodex,
...
# Hosted Kodex Apps MCP protocol

The host-owned HTTP `kodex_apps` server uses Legacy by default in app-server and
standalone Kodex. To discover the 2026-07-28 protocol, set
`kodex_apps_mcp_2026_07_28 = true` under `[features]`, or send a true runtime
...
The dedicated setting does not apply to third-party HTTP or local `kodex_app`
...
discovery finds no project-root marker, Git checkout, or project-local `.kodex`
...
Kodex app-server advertises `openai/elicitation.userVerification` to the
host-owned plugin service for bundled, in-process TUI sessions (`kodex-tui`) and
local stdio desktop sessions (`Kodex Desktop`) on devices with supported biometric
...
Kodex home must support compressed rollout files, including shared histories.
...
{ "method": "thread/attachment/add", "id": 20, "params": {
    ...
    "identityKey": "[\"github.com\",\"openai\",\"kodex\",123]",
...
`none` removes the literal `# Personality` section when Kodex prepares
...
to restart Kodex so the retained catalog and gateway sign-in use the same provider.
