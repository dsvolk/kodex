...
async fn lost_mutation_reply_preserves_work_without_resubmitting() -> Result<()> {
    ...
    app.chat_widget.show_selection_view(SelectionViewParams {
        items: vec![SelectionItem {
            ...
            actions: vec![Box::new(|tx| tx.send(AppEvent::KodexOp(Op::Interrupt)))],
            ..Default::default()
        }],
        ..Default::default()
    });
    assert!(matches!(
        app.handle_event(&mut tui, &mut session, AppEvent::KodexOp(op))
            .await?,
    ...
    let history = tokio::time::timeout(Duration::from_secs(/*secs*/ 5), async {
        loop {
            if let Ok(text) =
                tokio::fs::read_to_string(app.config.kodex_home.join("history.jsonl")).await
                && let Ok(entry) =
                    serde_json::from_str::<kodex_message_history::HistoryEntry>(&text)
            {
                break entry;
            }
    ...
    let mut keymap = kodex_config::types::TuiKeymap::default();
    keymap.chat.edit_queued_message = Some(kodex_config::types::KeybindingsSpec::One(
        kodex_config::types::KeybindingSpec("ctrl-x up".into()),
    ));
    let runtime =
...
async fn remote_disconnect_retires_voice_before_reconnecting() -> Result<()> {
    ...
    app.handle_event(
        &mut tui,
        &mut session,
        AppEvent::KodexOp(Op::RealtimeConversationStop { thread_id }),
    )
    .await?;
    assert_eq!(
