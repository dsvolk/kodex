...
pub(super) struct RemoteControlAuth {
    ...
    pub(super) network_policy: kodex_http_client::NetworkPolicy,
}
    ...
    pub(super) fn capture(manager: Arc<AuthManager>) -> (Self, bool) {
        loop {
            ...
            let authenticated = manager
                .auth_cached()
                .is_some_and(|auth| auth.uses_kodex_backend() && auth.get_account_id().is_some());
            if owner.is_current() {
                return (
                    Self {
...
pub(super) struct RemoteControlConnectionAuth {
    pub(super) http_client_factory: kodex_http_client::HttpClientFactory,
...
async fn load_auth_manager(
    ...
    Ok(RemoteControlConnectionAuth {
        http_client_factory,
        auth_provider: kodex_model_provider::auth_provider_from_auth(&auth),
...
pub(super) async fn request_client(
    ...
    endpoint: &str,
) -> io::Result<kodex_http_client::HttpClient> {
    kodex_login::default_client::create_client_for_route_without_request_logging_async(
        factory.clone(),
        endpoint.to_string(),
...
pub(super) fn is_policy_denial(error: &io::Error) -> bool {
    ...
    cause.is::<kodex_http_client::NetworkPolicyDenied>()
}
...
pub(super) fn request_error(error: kodex_http_client::HttpError) -> io::Error {
    match error {
        kodex_http_client::HttpError::Policy(denied) => {
