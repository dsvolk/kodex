...
use kodex_app_server_protocol::AppInfo;
...
use kodex_rmcp_client::McpOAuthCallbackMode;
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                kodex_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(kodex_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".kodex-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(kodex_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
...
async fn plugin_install_rejects_invalid_remote_plugin_name() -> Result<()> {
    let kodex_home = TempDir::new()?;
    write_remote_plugin_catalog_config(kodex_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = TestAppServer::builder()
        .with_kodex_home(kodex_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
async fn plugin_install_tracks_analytics_when_remote_detail_fetch_fails() -> Result<()> {
    ...
    let payload = wait_for_plugin_analytics_payload(&server, "kodex_plugin_install_failed").await?;
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                kodex_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
            install_attempt_id: None,
...
async fn plugin_install_tracks_analytics_when_marketplace_file_cannot_be_read() -> Result<()> {
    ...
    let marketplace_path = kodex_home.path().join("marketplace-dir");
    ...
    let payload =
        wait_for_plugin_analytics_payload(&analytics_server, "kodex_plugin_install_failed").await?;
    let event_params = &payload["events"][0]["event_params"];
    assert_eq!(
        payload["events"][0]["event_type"],
        "kodex_plugin_install_failed"
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let payload =
        wait_for_plugin_analytics_payload(&analytics_server, "kodex_plugin_installed").await?;
    assert_eq!(
        payload,
        json!({
            "events": [{
                "event_type": "kodex_plugin_installed",
                "event_params": {
...
async fn plugin_install_tracks_remote_plugin_analytics_event() -> Result<()> {
    ...
    let payload = wait_for_plugin_analytics_payload(&server, "kodex_plugin_installed").await?;
    assert_eq!(
        payload,
        json!({
            "events": [{
...
async fn plugin_install_returns_apps_needing_auth() -> Result<()> {
    ...
    let kodex_home = TempDir::new()?;
    write_connectors_config(kodex_home.path(), &server_url)?;
    write_chatgpt_auth(
        kodex_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_install_skips_mcp_oauth_for_chatgpt_dual_surface_plugin() -> Result<()> {
    ...
    let kodex_home = TempDir::new()?;
    write_connectors_config(kodex_home.path(), &apps_server_url)?;
    write_chatgpt_auth(
        kodex_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
"#
    ...
    let persisted_config = std::fs::read_to_string(kodex_home.path().join("config.toml"))?;
...
async fn plugin_install_skips_mcp_oauth_for_unowned_environment() -> Result<()> {
    ...
    let mut executor =
        tokio::process::Command::new(kodex_utils_cargo_bin::cargo_bin("exec-server")?)
    ...
    std::fs::write(
        kodex_home.path().join("environments.toml"),
        format!(
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/kodex/ps/mcp", mcp_service);
...
fn write_connectors_config(kodex_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/kodex/analytics-events/events"))
            ...
            for request in requests.iter().filter(|request| {
                request.method == "POST"
                    && request
                        .url
                        .path()
                        .ends_with("/kodex/analytics-events/events")
            }) {
...
async fn plugin_analytics_wait_ignores_unrelated_requests_and_batched_events() -> Result<()> {
    ...
    for events in [
        json!([{"event_type": "kodex_turn_event"}]),
        json!([{"event_type": "kodex_thread_initialized"}, expected.clone()]),
    ] {
        client
            .post(format!(
                "{}/backend-api/kodex/analytics-events/events",
                server.uri()
            ))
    ...
    assert_eq!(
        wait_for_plugin_analytics_payload(&server, "kodex_plugin_install_failed").await?,
...
fn configure_remote_plugin_test(kodex_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(kodex_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        kodex_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
