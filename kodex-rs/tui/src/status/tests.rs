...
async fn test_config(temp_home: &TempDir) -> Config {
    let mut config = ConfigBuilder::default()
        .kodex_home(temp_home.path().to_path_buf())
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.model = Some("gpt-5.1-kodex-max".to_string());
...
async fn status_snapshot_shows_chatgpt_plan_without_email() {
    ...
    wiremock::Mock::given(wiremock::matchers::path("/api/kodex/config/bundle"))
...
async fn status_uses_server_provider_id_and_auth_requirement() {
    ...
    assert_eq!(
        wide_destinations,
        vec!["https://chatgpt.com/kodex/settings/usage"]
    );
...
async fn status_snapshot_uses_default_reasoning_when_config_empty() {
    ...
    for (is_local_daemon, snapshot) in [
        ...
        let remote_connection = RemoteConnectionStatus {
            address: "unix:///tmp/kodex-home/app-server-control/app-server-control.sock"
                .to_string(),
...
async fn status_snapshot_includes_credits_and_limits() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-kodex".to_string());
    set_workspace_cwd(&mut config, test_path_buf("/workspace/tests").abs());
