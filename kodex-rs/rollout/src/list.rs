...
use crate::state_db;
use kodex_file_search as file_search;
...
pub struct ThreadItem {
    ...
    pub section: Option<kodex_state::ThreadSection>,
...
impl From<kodex_state::Anchor> for Cursor {
    fn from(anchor: kodex_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = kodex_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.kodex/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
        ...
        match rollout_line.item {
            RolloutItem::SessionMeta(session_meta_line) => {
                if !summary.saw_session_meta {
                    ...
                    if kodex_state::is_guardian_review_source(&session_meta_line.meta.source) {
                        // The synthetic prompt is not needed for the thread summary.
                        summary.preview = Some(kodex_state::GUARDIAN_THREAD_PREVIEW.to_string());
                        break;
                    }
                }
            }
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    subdir: &str,
    id_str: &str,
    state_db_ctx: Option<&kodex_state::StateRuntime>,
    ...
    let mut root = kodex_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(kodex_home, SESSIONS_SUBDIR, id_str, state_db_ctx).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(kodex_home, ARCHIVED_SESSIONS_SUBDIR, id_str, state_db_ctx)
        .await
}
...
pub async fn find_rollout_path_by_rollout_id(
    ...
    for subdir in [SESSIONS_SUBDIR, ARCHIVED_SESSIONS_SUBDIR] {
        let path = find_rollout_path_by_rollout_id_from_filenames(
            kodex_home.join(subdir).as_path(),
            rollout_id,
        )
        .await?;
        if path.is_some() {
