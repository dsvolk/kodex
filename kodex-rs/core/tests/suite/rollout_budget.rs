                ...
                json!({
                    ...
                    "response": {
                        ...
                        "usage": {
                            ...
                            "total_tokens": 75,
                            "kodex_rollout_budget_units": rollout_budget_units
                        }
                    }
                }),
            ]),
...
async fn invalid_provider_rollout_budget_units_fail_without_retry() -> Result<()> {
    ...
    completed["response"]["usage"]["kodex_rollout_budget_units"] = json!(-1.0);
    let responses = mount_sse_sequence(
        &server,
    ...
    assert_eq!(
        error.message,
        "Fatal error: response.completed usage.kodex_rollout_budget_units must be finite and non-negative"
    );
    assert_eq!(error.kodex_error_info, Some(KodexErrorInfo::Other));
        ...
        wait_for_event(&test.kodex, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.kodex_error_info == Some(KodexErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.kodex, |event| {
...
async fn compaction_budget_exhaustion_fails_without_retry(
    ...
    if provider_units {
        completed["response"]["usage"]["kodex_rollout_budget_units"] = json!(10.0);
    }
