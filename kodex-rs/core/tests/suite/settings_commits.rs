...
fn assert_checkpoints(test: &TestKodex, expected: &[ThreadSettingsSnapshot]) -> Result<()> {
...
async fn initial_plugin_ids_use_turn_context_without_extra_settings_checkpoints(
    ...
    let (items, _, parse_errors) =
        kodex_rollout::RolloutRecorder::load_rollout_items(&rollout_path).await?;
    ...
    .await?;
    let expected = vec![test.kodex.thread_settings_snapshot().await];
...
async fn settings_notifications_keep_their_commit_across_postcommit_work(
    ...
    let test = test_kodex()
        ...
        .build_with_auto_env(&server)
        .await?;
    let mut initial = test.kodex.restorable_thread_settings().await;
    ...
    let expected = test.kodex.thread_settings_snapshot().await;
    ...
    timeout(TIMEOUT, test.kodex.restore_thread_settings(initial)).await??;
    let restored = test.kodex.thread_settings_snapshot().await;
    ...
    assert_eq!(test.kodex.thread_settings_snapshot().await, restored);
    Ok(())
}
