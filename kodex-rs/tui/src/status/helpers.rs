...
mod tests {
    ...
    async fn test_config(kodex_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let kodex_home = TempDir::new().expect("temp kodex home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = kodex_home.path().join("global.md");
        let config = test_config(&kodex_home, &cwd).await;
    ...
    async fn compose_agents_summary_collapses_home_and_preserves_project_relative_paths() {
        ...
        let kodex_home = TempDir::new().expect("temp kodex home");
        let cwd = TempDir::new().expect("temp cwd");
        let mut config = test_config(&kodex_home, &cwd).await;
        config.cwd = home.join("workspace").join("project").abs();

        let paths = [
            home.join(".kodex").join("AGENTS.md"),
        ...
        insta::assert_snapshot!(
            summary.replace('\\', "/"),
            @"~/.kodex/AGENTS.md, ../AGENTS.md, AGENTS.md, nested/AGENTS.md"
        );
    }
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let kodex_home = TempDir::new().expect("temp kodex home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = kodex_home.path().join("override.md");
        let config = test_config(&kodex_home, &cwd).await;
