...
use kodex_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    kodex_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("kodex-test", "0.0.0-test").with_title("Kodex rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<kodex_http_client::HttpClient> {
    Ok(kodex_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `kodex exec-server --listen`.
