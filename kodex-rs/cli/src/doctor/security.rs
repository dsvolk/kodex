...
pub(super) fn endpoint_check(inspection: EndpointInspection) -> DoctorCheck {
    ...
    let targets = if cfg!(target_os = "windows") {
        "signed Kodex app; kodex.exe; kodex-windows-sandbox-setup.exe; kodex-command-runner.exe; kodex-code-mode-host.exe"
    } else if cfg!(target_os = "macos") {
        "signing team 2DC432GLL2 plus the installed Kodex app identity, signed kodex agent, and required helpers"
    } else {
        "verified Kodex app, kodex agent, and required helpers"
    };
    let mut check = DoctorCheck::new(
        "security.endpoint",
        "security",
        CheckStatus::Warning,
        "endpoint protection detected; Kodex exclusions are unverified",
    )
    .detail(format!("endpoint products: {}", products.join(", ")))
    .detail(format!("exclusion targets: {targets}"))
    .detail("Kodex exclusions: not verified")
    .remediation(
        "ask your security administrator to verify Kodex exclusions and required helper allowances",
    );
    ...
    for product in products {
        let remedy = match product {
            "CrowdStrike Falcon" => {
                "CrowdStrike Falcon: Add a certificate or IOA exclusion for Kodex. If sensor overhead continues, exclude the Kodex agent from sensor visibility. Keep monitoring descendant processes."
            }
            "BeyondTrust Privilege Management" => {
                "BeyondTrust: Remove Kodex from application blocking rules. Add allow rules for Kodex helper executables. Do not grant administrator privileges."
            }
            "Microsoft Defender" => {
                "Microsoft Defender: Add a certificate or executable-path exclusion for Kodex and its helpers. If Attack Surface Reduction blocks Kodex, add a rule exclusion. If Controlled Folder Access blocks Kodex, allow the app."
            }
            "SentinelOne" => {
                "SentinelOne: Add a signer, file-hash, or executable-path exclusion for Kodex and its helpers."
            }
            "Jamf Protect" => {
                "Jamf Protect: Add an Override Threat Prevention exception for Kodex app and helper signing identities. If analytics cause delays, add an Ignore System Events for Analytics exception."
            }
            _ => {
                "Add an exclusion for Kodex and its helpers. Use the endpoint product instructions."
            }
        };
        check = check.issue(
            DoctorIssue::new(
                CheckStatus::Warning,
                format!("{product} can interfere with Kodex. Verify Kodex exclusions."),
            )
            .measured("not verified")
            .expected("Kodex application and helper exclusions")
            .remedy(remedy)
            .field("Kodex exclusions"),
        );
    }
