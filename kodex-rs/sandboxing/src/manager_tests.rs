...
fn transform_linux_seccomp_request(
    kodex_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: &cwd_uri,
            sandbox_exe: Some(kodex_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
        })
        .expect("transform")
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let kodex_linux_sandbox_exe = std::path::PathBuf::from("/tmp/kodex-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&kodex_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(kodex_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let kodex_linux_sandbox_exe = std::path::PathBuf::from("/tmp/kodex");
    let exec_request = transform_linux_seccomp_request(&kodex_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("kodex-linux-sandbox".to_string()));
}
...
async fn linux_unix_socket_grant_uses_effective_managed_policy() -> anyhow::Result<()> {
    ...
    for (label, context, live_proxy, enforce_managed_network, expected) in [
        ...
        let request = manager.transform(SandboxTransformRequest {
            ...
            sandbox_policy_cwd: &cwd_uri,
            sandbox_exe: Some(std::path::Path::new("/tmp/kodex-linux-sandbox")),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
        })?;
...
fn wrapper_runtime_selection_uses_the_parent_not_environment_overrides() {
    for registered_core in [false, true] {
        let mut env = HashMap::from([("kodex_windows_registered_core".into(), "1".into())]);
        super::add_windows_sandbox_wrapper_setup_env_from_vars(
            &mut env,
            [("KODEX_WINDOWS_REGISTERED_CORE".into(), "0".into())],
            registered_core,
        );
        assert_eq!(
            env,
            if registered_core {
                HashMap::from([("KODEX_WINDOWS_REGISTERED_CORE".into(), "1".into())])
            } else {
                HashMap::new()
            }
        );
...
fn transform_for_direct_spawn_windows_materializes_inner_helper() {
    let kodex_home = tempfile::TempDir::new().expect("kodex home");
    let helper_dir = tempfile::TempDir::new().expect("helper dir");
    let configured_helper = helper_dir.path().join("configured-kodex-helper.exe");
    ...
    let manager = SandboxManager::new();
    let exec_request = manager
        .transform_for_direct_spawn_with_kodex_home(
            SandboxDirectSpawnTransformRequest {
                workspace_roots: workspace_roots.as_slice(),
                windows_sandbox_proxy_settings_mode:
                    kodex_windows_sandbox::WindowsSandboxProxySettingsMode::Preserve,
                transform: SandboxTransformRequest {
                    command: SandboxCommand {
                        program: configured_helper.as_os_str().to_owned(),
                        args: vec!["--kodex-run-as-fs-helper".to_string()],
                        cwd: cwd_uri.clone(),
                        env: HashMap::from([(
                            "Path".to_string(),
