...
async fn reconnect_restores_history_permissions_and_keeps_old_input_paused() -> Result<()> {
    for (recovered_queue, edit_offline, resume_error_code, deferred_notice, notice_enabled) in [
        ...
        let mcp = overrides.unwrap().remove("mcp_servers.kodex_tui").unwrap();
        ...
        let notices = history
            .lines()
            .filter(|line| {
                line.contains("Reconnected.") || line.contains("background Kodex service")
            })
            .collect::<Vec<_>>()
            .join("\n");
        if deferred_notice {
...
⚠ A background Kodex service is running v2.0.0, older than your Kodex CLI
"###);
        } else {
...
pub(super) async fn drain_history(
    ...
    while let Ok(event) = events.try_recv() {
        assert!(!matches!(
            event,
            AppEvent::KodexOp(AppCommand::UserTurn { .. })
        ));
        if matches!(
            event,
