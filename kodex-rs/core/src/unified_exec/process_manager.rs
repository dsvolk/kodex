...
use kodex_core_plugins::PluginMetricsSidecar;
use kodex_core_plugins::strip_output_env;
...
use kodex_protocol::protocol::TerminalInteractionEvent;
use kodex_protocol::shell_environment::is_non_inheritable_env_var;
use kodex_sandboxing::SandboxCommand;
use kodex_shell_command::is_dangerous_command::DangerousCommandPlatform;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Kodex sandbox network proxy.";
...
pub(super) fn exec_env_policy_from_shell_policy(
...
) -> kodex_exec_server::ExecEnvPolicy {
    ...
    exclude.extend([
        ...
        kodex_apply_patch::KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
    ...
    r#set.retain(|key, _| {
        ![
            ...
            kodex_apply_patch::KODEX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn exec_server_env_for_request(
    ...
    Option<kodex_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    kodex_exec_server::ExecParams {
        ...
        metadata: tool_ctx.map(|ctx| kodex_exec_server::ExecMetadata {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Kodex(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    async fn write_stdin_inner(
        ...
        if let Some((approval, approval_reason)) = approval {
            ...
            context.step_context.session_telemetry.counter(
                "kodex.unified_exec.stdin_review.size_check",
                /*inc*/ 1,
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<kodex_network_proxy::RemoteNetworkProxyLaunchConfig>,
        environment_id: Option<&str>,
        exec_server_env_config: Option<ExecServerEnvConfig>,
        shell_snapshot: Option<kodex_exec_server::ShellSnapshotRequest>,
        windows_sandbox_proxy_settings_mode: kodex_sandboxing::WindowsSandboxProxySettingsMode,
        tty: bool,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Kodex(KodexErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        if environment.is_remote() || request.exec_server_shell_snapshot.is_some() {
            ...
            tracing::event!(
                name: "kodex.unified_exec.process_start_requested",
                target: "kodex_otel.trace_safe",
                tracing::Level::INFO,
                event.name = "kodex.unified_exec.process_start_requested",
        ...
        let network_proxy_restricting_sid = {
            ...
            {
                if request.sandbox == kodex_sandboxing::SandboxType::WindowsRestrictedToken {
                    request
                        .network
                        .as_ref()
                        .map(|network| {
        ...
        let windows_sandbox =
            if request.sandbox == kodex_sandboxing::SandboxType::WindowsRestrictedToken {
                Some(kodex_sandboxing::WindowsSandboxSpawnRequest {
        ...
        let spawn_result = kodex_sandboxing::spawn_process(kodex_sandboxing::SpawnRequest {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            KODEX_THREAD_ID_ENV_VAR.to_string(),
        ...
        let result = orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Kodex(err) => match err.details() {
