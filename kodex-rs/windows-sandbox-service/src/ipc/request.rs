...
pub(crate) enum ServiceRequest {
    RegisterInstallation { kodex_home: PathBuf },
...
pub(super) fn validate_request(request: &[u8]) -> Result<ServiceRequest> {
    ...
    let request = match frame.message {
        ProvisioningMessage::RegisterInstallationRequest { kodex_home } => {
            validate_home(&kodex_home)?;
            return Ok(ServiceRequest::RegisterInstallation {
                kodex_home: PathBuf::from(kodex_home),
            });
        }
    ...
    validate_home(&request.kodex_home)?;
    ...
    Ok(ServiceRequest::ProvisionSandbox(ProvisioningRequest {
        kodex_home: PathBuf::from(request.kodex_home),
...
fn validate_home(home: &str) -> Result<()> {
    if home.is_empty() || home.contains(['\0', '\r', '\n']) {
        bail!("Kodex home is empty or contains an invalid control character");
    }
    Ok(())
}
