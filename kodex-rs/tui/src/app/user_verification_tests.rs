...
async fn run_verification_rpc_scenario(scenario: RpcScenario) -> color_eyre::Result<()> {
            ...
            match serde_json::from_str::<JSONRPCMessage>(&text)? {
                JSONRPCMessage::Request(request) => {
                    let response = match request.method.as_str() {
                        "initialize" => {
                            serde_json::json!({ "id": request.id, "result": { "userAgent": "kodex-tui-test", "kodexHome": std::env::temp_dir(), "platformFamily": std::env::consts::FAMILY, "platformOs": std::env::consts::OS } })
                        }
    ...
    if scenario == RpcScenario::CancelWhilePending {
        ...
        session
            .request_handle()
            .request_typed::<kodex_app_server_protocol::UserVerificationStatusResponse>(
                ClientRequest::UserVerificationStatus {
                    request_id: RequestId::String("after-cancel".to_string()),
                    params: kodex_app_server_protocol::UserVerificationStatusParams {},
                },
            )
            .await?;
        assert!(
    ...
    if matches!(
        ...
        let message = if scenario == RpcScenario::RemoteWorkspace {
            ...
            "The local Kodex binary could not complete user verification."
        };
