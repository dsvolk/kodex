...
`kodex-otel` is the OpenTelemetry integration crate for Kodex. It provides:

- Provider wiring for log/trace/metric exporters (`kodex_otel::OtelProvider`
  and `kodex_otel::provider`).
- Session-scoped business event emission via `kodex_otel::SessionTelemetry`.
- Low-level metrics APIs via `kodex_otel::metrics`.
- Trace-context helpers via `kodex_otel::trace_context` and crate-root re-exports.
...
```rust
use kodex_otel::config::OtelExporter;
use kodex_otel::config::OtelHttpProtocol;
use kodex_otel::config::OtelSettings;
...
let settings = OtelSettings {
    ...
    kodex_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Kodex
...
Kodex-specific session events. Rich session/business events should go through
...
`kodex-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Kodex-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use kodex_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("kodex.session_started", 1, &[("source", "tui")])?;
metrics.histogram("kodex.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("kodex.turns", 1, &[("model", "gpt-5.1")])?;
metrics.shutdown()?; // flushes in-memory exporter
```

## WebSocket continuation

`kodex.websocket.continuation` counts `response.create` send attempts, including
