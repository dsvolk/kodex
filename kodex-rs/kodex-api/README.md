...
Typed clients for Kodex/OpenAI APIs built on top of the generic transport in `kodex-client`.

- Hosts the request/response models and request builders for Responses and related Kodex APIs.
...
- Serves as the wire-level layer consumed by `kodex-core`; higher layers handle auth refresh and business logic.
...
- **Memory summarize endpoint**
  - Input: `MemorySummarizeInput` (re-exported as `kodex_api::MemorySummarizeInput`):
    - `model: String`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `kodex-api` and `kodex-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or other endpoint-specific response values.
