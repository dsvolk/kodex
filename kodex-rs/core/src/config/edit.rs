...
pub fn apply_blocking(kodex_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(kodex_home: &Path) -> Self {
        Self::for_config_path(&kodex_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(kodex_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.kodex_home.join(CONFIG_TOML_FILE).to_path_buf());
