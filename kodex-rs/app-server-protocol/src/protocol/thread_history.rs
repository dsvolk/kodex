...
use kodex_protocol::items::parse_hook_prompt_message;
use kodex_protocol::protocol::AgentMessageEvent;
...
use kodex_protocol::protocol::ContextCompactedEvent;
...
use kodex_protocol::protocol::ExecCommandEndEvent;
use kodex_protocol::protocol::GuardianAssessmentEvent;
use kodex_protocol::protocol::GuardianAssessmentStatus;
use kodex_protocol::protocol::ImageGenerationBeginEvent;
use kodex_protocol::protocol::ImageGenerationEndEvent;
use kodex_protocol::protocol::ItemCompletedEvent;
use kodex_protocol::protocol::ItemStartedEvent;
use kodex_protocol::protocol::McpToolCallBeginEvent;
use kodex_protocol::protocol::McpToolCallEndEvent;
use kodex_protocol::protocol::PatchApplyBeginEvent;
use kodex_protocol::protocol::PatchApplyEndEvent;
use kodex_protocol::protocol::ThreadRolledBackEvent;
...
use kodex_protocol::protocol::UserMessageImageKind;
use kodex_protocol::protocol::ViewImageToolCallEvent;
use kodex_protocol::protocol::WebSearchBeginEvent;
use kodex_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use kodex_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
...
#[cfg(test)]
use kodex_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use kodex_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `kodex-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &kodex_protocol::models::ResponseItem) {
        let kodex_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &kodex_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            kodex_protocol::items::TurnItem::EnteredReviewMode(_)
                | kodex_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            kodex_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            kodex_protocol::items::TurnItem::HookPrompt(_)
            | kodex_protocol::items::TurnItem::FunctionCallOutput(_)
            | kodex_protocol::items::TurnItem::CommandExecution(_)
            | kodex_protocol::items::TurnItem::DynamicToolCall(_)
            | kodex_protocol::items::TurnItem::CollabAgentToolCall(_)
            | kodex_protocol::items::TurnItem::SubAgentActivity(_)
            | kodex_protocol::items::TurnItem::Extension(_)
            | kodex_protocol::items::TurnItem::EnteredReviewMode(_)
            | kodex_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            kodex_protocol::items::TurnItem::UserMessage(_)
            | kodex_protocol::items::TurnItem::AgentMessage(_)
            | kodex_protocol::items::TurnItem::Reasoning(_)
            | kodex_protocol::items::TurnItem::WebSearch(_)
            | kodex_protocol::items::TurnItem::ImageView(_)
            | kodex_protocol::items::TurnItem::ImageGeneration(_)
            | kodex_protocol::items::TurnItem::FileChange(_)
            | kodex_protocol::items::TurnItem::McpToolCall(_)
            | kodex_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &kodex_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &kodex_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &kodex_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &kodex_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &kodex_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &kodex_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &kodex_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &kodex_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &kodex_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &kodex_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &kodex_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &kodex_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &kodex_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &kodex_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                kodex_error_info: payload.kodex_error_info.clone().map(Into::into),
    ...
    fn handle_turn_complete(&mut self, payload: &TurnCompleteEvent) {
        let terminal_error = payload.error.as_ref().map(|error| V2TurnError {
            ...
            kodex_error_info: error.kodex_error_info.clone().map(Into::into),
            additional_details: None,
        });
...
fn convert_dynamic_tool_content_items(
    items: &[kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use kodex_extension_items::ExtensionItem as CoreExtensionItem;
    use kodex_extension_items::sleep::SleepItem as CoreSleepItem;
    use kodex_protocol::ThreadId;
    use kodex_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use kodex_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use kodex_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use kodex_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use kodex_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use kodex_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use kodex_protocol::items::SubAgentActivityItem as CoreSubAgentActivityItem;
    use kodex_protocol::items::TurnItem as CoreTurnItem;
    use kodex_protocol::items::UserMessageItem as CoreUserMessageItem;
    ...
    use kodex_protocol::models::MessagePhase as CoreMessagePhase;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![kodex_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn replays_image_generation_end_events_into_turn_history() {
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ImageGenerationEnd(ImageGenerationEndEvent {
                ...
                transparent_background: Some(true),
                failure: Some(
                    kodex_extension_items::image_generation::ImageGenerationFailure::UsageLimitExceeded {
        ...
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                action: CoreWebSearchAction::Search {
                    query: Some("kodex".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/kodex",
                })]),
            }),
            EventMsg::ExecCommandEnd(ExecCommandEndEvent {
                call_id: "exec-1".into(),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                kodex_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("kodex_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    kodex_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn assigns_late_legacy_mcp_completion_to_original_turn() {
        use kodex_protocol::items::McpToolCallItem;
        use kodex_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
        ...
        let events = vec![
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                error: Some(ErrorEvent {
                    ...
                    kodex_error_info: Some(KodexErrorInfo::ServerOverloaded),
                }),
                ...
                Turn {
                    ...
                    error: Some(TurnError {
                        ...
                        message: "Selected model is at capacity. Please try a different model."
                            .into(),
                        kodex_error_info: Some(
                            crate::protocol::v2::KodexErrorInfo::ServerOverloaded,
                        ),
                        additional_details: None,
                    }),
                    started_at: Some(10),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(kodex_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(kodex_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: kodex_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(kodex_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                kodex_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                kodex_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                kodex_error_info: Some(KodexErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn out_of_turn_error_does_not_create_or_fail_a_turn() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                kodex_error_info: Some(KodexErrorInfo::BadRequest),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                kodex_error_info: Some(KodexErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                ...
                message: "stream failure".into(),
                kodex_error_info: Some(
                    crate::protocol::v2::KodexErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
        ...
        assert_eq!(
            ...
            vec![Turn {
                ...
                error: Some(TurnError {
                    misalignment: None,
                    message: "Selected model is at capacity. Please try a different model.".into(),
                    kodex_error_info: Some(crate::protocol::v2::KodexErrorInfo::ServerOverloaded),
                    additional_details: None,
                }),
                started_at: Some(10),
                completed_at: Some(20),
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(kodex_protocol::items::HookPromptItem {
    ...
    fn completed_sub_agent_activity_updates_completed_parent_turn() {
        ...
        let child_path = kodex_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path");
        let items = vec![
            RolloutItem::EventMsg(EventMsg::TurnStarted(TurnStartedEvent {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(
                kodex_protocol::models::ResponseItem::Message {
                    id: Some(kodex_protocol::ResponseItemId::with_suffix("msg", "1")),
                    role: "user".into(),
                    content: vec![kodex_protocol::models::ContentItem::InputText {
