...
pub(super) fn reserve_response() -> GetAccountRateLimitsResponse {
    serde_json::from_value(json!({
        ...
        "rateLimits": {"limitId": "kodex", "primary": {"usedPercent": 100}},
...
pub(super) fn configure_reserve_catalog(app: &mut App) {
    ...
    init.has_kodex_backend_auth = true;
...
async fn luna_reserve_entry_dispatches_an_already_queued_turn_with_accepted_settings() -> Result<()>
{
    for mode in [ModeKind::Default, ModeKind::Plan] {
        ...
        let turn: kodex_app_server_protocol::TurnStartParams =
    ...
    for has_credits in [false, true] {
        ...
        let mut pending = std::iter::from_fn(|| events.try_recv().ok())
            .find_map(|event| match event {
                AppEvent::KodexOp(op @ AppCommand::UserTurn { .. }) => Some(op),
                _ => None,
            })
        ...
        recovered.rate_limits.credits = Some(kodex_app_server_protocol::CreditsSnapshot {
            has_credits,
            unlimited: false,
            balance: None,
        });
        ...
        assert!(
            !queued.iter().any(|event| matches!(
                event,
                AppEvent::KodexOp(_) | AppEvent::PersistModelSelection { .. }
            )),
