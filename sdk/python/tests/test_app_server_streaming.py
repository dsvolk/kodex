...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """KodexClient.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            thread = kodex.thread_start()
            chunks = list(kodex._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncKodex(config=harness.app_server_config()) as kodex:
                thread = await kodex.thread_start()
                stream = kodex._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(kodex.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Kodex(config=harness.app_server_config()) as kodex:
            first_thread = kodex.thread_start()
            second_thread = kodex.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncKodex(config=harness.app_server_config()) as kodex:
                first_thread = await kodex.thread_start()
                second_thread = await kodex.thread_start()
