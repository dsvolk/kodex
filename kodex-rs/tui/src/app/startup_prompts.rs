...
pub(super) fn emit_system_bwrap_warning(app_event_tx: &AppEventSender, config: &Config) {
    let Some(message) =
        kodex_sandboxing::system_bwrap_warning(config.permissions.permission_profile())
    else {
        return;
    };
...
pub(super) fn model_upgrade_for_migration(
    ...
    Some(ModelUpgrade {
        ...
        migration_markdown: Some(format!(
            "{current_name} is no longer available\n\nKodex now uses {target_name} in place of {current_name}. Switch to {target_name} to continue.\n"
        )),
        retirement_at: None,
    })
}
...
pub(super) fn migration_prompt_hidden(
    ...
    match migration_config_key {
        HIDE_GPT_5_1_KODEX_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_kodex_max_migration_prompt
            .unwrap_or(false),
...
mod tests {
    ...
    fn skill_load_warning_state_suppresses_repeated_active_errors() {
        ...
        let error = skill_error("/repo/.kodex/skills/abc/SKILL.md", "invalid description");
    ...
    fn skill_load_warning_state_displays_new_message_for_active_path() {
        let mut state = SkillLoadWarningState::default();
        let initial = skill_error("/repo/.kodex/skills/abc/SKILL.md", "invalid description");
        let changed = skill_error("/repo/.kodex/skills/abc/SKILL.md", "invalid frontmatter");
...
⚠ /repo/.kodex/skills/abc/SKILL.md: invalid description
");
    }
}
