...
//! `just test -p kodex-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let kodex_home = home.path().join(".kodex");
    ...
    let mut cmd = Command::new(kodex_utils_cargo_bin::cargo_bin("kodex-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn kodex-rs");
