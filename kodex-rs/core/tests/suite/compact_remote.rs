...
use core_test_support::test_kodex::test_kodex as base_test_kodex;
...
fn test_kodex() -> TestKodexBuilder {
    base_test_kodex().with_config(|config| {
        config.update_plan_enabled = true;
    })
}

fn remote_realtime_test_kodex_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestKodexBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_kodex()
        .with_auth(KodexAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(kodex: &kodex_core::KodexThread) -> Result<()> {
    kodex
    ...
    wait_for_event_match(kodex, |msg| match msg {
...
async fn close_realtime_conversation(kodex: &kodex_core::KodexThread) -> Result<()> {
...
async fn wait_for_turn_complete(kodex: &kodex_core::KodexThread) {
...
fn amazon_bedrock_test_kodex() -> TestKodexBuilder {
    let auth = KodexAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    let builder = || test_kodex().with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing());
    let initial = builder()
    ...
    let resumed = builder()
        ...
        .await?;
    resumed.kodex.submit(Op::Compact).await?;
    wait_for_turn_complete(&resumed.kodex).await;
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let harness = TestKodexHarness::with_auto_env_builder(
        test_kodex()
    ...
    let kodex = &harness.test().kodex;
    let rollout_path = kodex.rollout_path().context("rollout path")?;
    kodex
...
async fn amazon_bedrock_automatic_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    let harness = TestKodexHarness::with_auto_env_builder(amazon_bedrock_test_kodex().with_config(
        |config| {
            config.model_auto_compact_token_limit = Some(200);
        },
    ))
        ...
        .map(|number| {
            if image_field == "file_id" {
                return Ok(UserInput::Image {
                    ...
                    detail: Some(kodex_protocol::models::ImageDetail::Original),
                });
            }
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    let harness = TestKodexHarness::with_builder(
        test_kodex().with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    ...
    let compact_turn_id = kodex.submit(Op::Compact).await?;
    wait_for_turn_complete(&kodex).await;
    ...
    assert_eq!(
        ...
        compact_request.header("x-kodex-window-id").as_deref()
    );
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-kodex-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
...
async fn remote_compact_v2_retries_failures_with_stream_retry_budget() -> Result<()> {
    ...
    .await?;
    let kodex = harness.test().kodex.clone();
...
async fn remote_compact_v2_rewrites_multiple_trailing_function_call_outputs(
    ...
    kodex.inject_response_items(history).await?;
...
async fn active_realtime_refreshes_changed_start_instructions_only_after_compaction() -> Result<()>
{
    ...
    let mut initial_builder = remote_realtime_test_kodex_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.kodex.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.kodex.as_ref()).await?;
    initial.kodex.submit(Op::Shutdown).await?;
    wait_for_event(&initial.kodex, |ev| {
    ...
    let mut resume_builder = remote_realtime_test_kodex_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.kodex.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.kodex.as_ref()).await?;
...
async fn remote_mid_turn_compact_v2_sends_turn_state_over_websocket() -> Result<()> {
    ...
    test.kodex
        ...
        .await?;
    wait_for_turn_complete(&test.kodex).await;
