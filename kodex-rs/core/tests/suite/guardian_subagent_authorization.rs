...
use kodex_core::GuardianRootMessage;
...
use kodex_prompts::render_review_exit_success;
...
fn is_worker_request(request: &wiremock::Request, root_thread_id: ThreadId) -> bool {
    request_body(request).is_some_and(|body| {
        body["client_metadata"]["x-kodex-parent-thread-id"] == json!(root_thread_id)
...
async fn guardian_subagent_review_preserves_late_root_user_authorization(
    ...
    let (messaging_namespace, messaging_tool) = match root_context {
        RootContext::Legacy => ("mcp__kodex_apps__user_messaging", "_send_message"),
        _ => ("mcp__kodex_apps", "user_messaging_send_message"),
    };
    ...
    let mut builder = test_kodex()
        .with_pre_build_hook(move |home| {
            ...
            let mut hooks = json!({"hooks": {"PreToolUse": [{
                "matcher": "mcp__kodex_apps__user_messaging.*send_message",
                "hooks": [{
                    "type": "mcp_tool",
    ...
    if matches!(root_context, RootContext::Migrating) {
        ...
        .await?;
        assert_eq!(
            kodex_core::context::GuardianContextMode::from_history(
                test.kodex.conversation_history_snapshot().await.as_ref()
            ),
            kodex_core::context::GuardianContextMode::Legacy,
        );
    }
    if messaging_case {
        wait_for_mcp_server(&test.kodex, messaging_namespace).await?;
    }
    ...
    if cancel_call {
        ...
        .await;
        assert!(test.kodex.conversation_history_snapshot().await.items().any(|item| {
        ...
        root_history_items.extend(
            [
                format!(
                    ...
                    kodex_core::review_prompts::SUMMARY_PREFIX
                ),
    ...
    test.kodex.inject_response_items(root_history_items).await?;
    ...
    test.kodex
        ...
        .await?;
    let question = wait_for_event_match(&test.kodex, |event| match event {
    ...
    let legacy_answer = kodex_guardian_context::truncate_text(
        &format!(
            "{}{}",
    ...
    let expected_messages = match root_context {
        ...
        RootContext::RetainedAtMessageLimit => {
            ...
            messages.push(GuardianRootMessage::User(
                kodex_guardian_context::truncate_text(
    ...
    let feedback_thread_ids = test
        ...
        .await?;
    let failures = kodex_feedback::guardian_review_failures(&feedback_thread_ids);
