...
use kodex_exec_server_test_support::relay::TEST_TIMEOUT as VERSION_SKEW_TIMEOUT;
use kodex_exec_server_test_support::relay::accept_websocket;
use kodex_exec_server_test_support::relay::assert_relay_data_is_encrypted;
use kodex_exec_server_test_support::relay::proxy_relay_frames;
use kodex_exec_server_test_support::relay::registered_executor_public_key;
...
const RELEASED_KODEX_ENV_VAR: &str = "KODEX_TEST_RELEASED_KODEX";
const CURRENT_KODEX_ENV_VAR: &str = "KODEX_TEST_CURRENT_KODEX";
const EXECUTOR_MARKER_ENV_VAR: &str = "KODEX_EXECUTOR_VERSION_SKEW_MARKER";
...
fn version_skew_binaries() -> Result<Option<(PathBuf, PathBuf)>> {
    let Some(released) = std::env::var_os(RELEASED_KODEX_ENV_VAR) else {
        return Ok(None);
    };
    let current = std::env::var_os(CURRENT_KODEX_ENV_VAR)
        .with_context(|| format!("{CURRENT_KODEX_ENV_VAR} must name the current Kodex binary"))?;
    let current = PathBuf::from(current);
    let released = PathBuf::from(released);
    anyhow::ensure!(
        current.is_file(),
        "current Kodex does not exist: {}",
        current.display()
    );
    anyhow::ensure!(
        released.is_file(),
        "released Kodex does not exist: {}",
        released.display()
    );
...
async fn assert_noise_version_skew(app_binary: &Path, executor_binary: &Path) -> Result<()> {
    ...
    let model = mock_model(kodex_home.path()).await?;
...
"#
    ...
    let mut executor = Command::new(executor_binary)
        ...
        .env("KODEX_API_KEY", REGISTRY_TOKEN)
    ...
    let mut app_server = Command::new(app_binary)
        ...
        .env(KODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR, &registry_url)
        .env(
            KODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR,
            ENVIRONMENT_ID,
        )
        .env(KODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR, REGISTRY_TOKEN)
        .env_remove(KODEX_EXEC_SERVER_URL_ENV_VAR)
    ...
    assert_eq!(
        std::fs::read_to_string(kodex_home.path().join("version-skew-output.txt"))?,
        EXPECTED_OUTPUT
    );
...
async fn mock_model(kodex_home: &Path) -> Result<MockServer> {
