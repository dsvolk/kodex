...
fn interactive_validates_config_before_requiring_terminal() -> Result<()> {
    ...
    for &(args, config_file, contents, expected_error) in cases {
        let kodex_home = TempDir::new()?;
        std::fs::write(kodex_home.path().join(config_file), contents)?;
...
-> Result<()> {
    let kodex_home = TempDir::new()?;
    std::fs::write(kodex_home.path().join("config.toml"), "")?;
    std::fs::write(
...
"#,
    ...
    let remote_only_cwd = kodex_home.path().join("remote-only-working-directory");
...
fn strict_config_is_not_supported_for_cloud_command() -> Result<()> {
    ...
    cmd.args(["--strict-config", "-c", "foo=bar", "cloud", "list"])
        ...
        .failure()
        .stderr(contains(
            "`--strict-config` is not supported for `kodex cloud`",
        ));
...
async fn features_list_honors_cloud_managed_feature_requirements() -> Result<()> {
    ...
    std::fs::write(kodex_home.path().join("config.toml"), &user_config)?;

    let bootstrap_config = load_config_toml_with_layer_stack(
        kodex_home.path(),
        Some(&AbsolutePathBuf::from_absolute_path(kodex_home.path())?),
        Vec::new(),
...
fn remote_start_rejects_add_dir_before_connecting() -> Result<()> {
    ...
    let output = kodex_command(home.path())?
...
fn no_daemon_rejects_agents_and_explicit_remote_targets() -> Result<()> {
    for args in [
        ...
        let expected = if args.contains(&"agents") {
            "--no-daemon cannot be used with kodex agents."
        } else if args.contains(&"queue") && !args.contains(&"--remote") {
            "--no-daemon cannot be used with kodex queue."
        } else {
