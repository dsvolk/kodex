...
fn os_discovery_is_cached_without_freezing_user_agent_overrides() {
    ...
    const PROBE_DIR: &str = "KODEX_TEST_USER_AGENT_PROBE_DIR";
    if std::env::var_os(PROBE_DIR).is_some() {
        ...
        std::thread::scope(|scope| {
            for _ in 0..8 {
                scope.spawn(|| {
                    for _ in 0..4 {
                        assert!(get_kodex_user_agent().contains("Ubuntu 99.7.3"));
                    }
                });
            }
        });
    ...
    let output = std::process::Command::new(std::env::current_exe().expect("test executable"))
        .args([
            ...
            .trim_start_matches("kodex_login::"),
            "--nocapture",
        ])
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("kodex-tui"), true);
    assert_eq!(is_first_party_originator("kodex_vscode"), true);
    assert_eq!(is_first_party_originator("Kodex Something Else"), true);
    assert_eq!(is_first_party_originator("kodex_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("kodex_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("kodex_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("kodex_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Kodex UA for that originator
    let expected_ua = get_kodex_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "kodex_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "kodex_cli_rs/0.0.0 (bad_suffix)"
    );
}
