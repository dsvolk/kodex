...
async fn accepted_model_migration_persists_target_default_reasoning_effort() -> Result<()> {
    ...
    Kodex now uses GPT-6 Sol in place of GPT-5.4. Switch to GPT-6 Sol to continue.

    GPT-5.4 Mini is no longer available

    Kodex now uses GPT-6 Luna in place of GPT-5.4 Mini. Switch to GPT-6 Luna to continue.
