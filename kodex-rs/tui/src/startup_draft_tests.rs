...
fn terminal_app_ssh_fallback_renders_inline_startup() {
    ...
    for terminal_app_over_ssh in [false, true] {
        let owned = crate::determine_alt_screen_mode(
            /*no_alt_screen*/ false,
            kodex_config::types::AltScreenMode::Auto,
...
async fn startup_draft_hydrates_its_header_without_moving_the_composer() {
    let kodex_home = tempfile::tempdir().expect("create temporary Kodex home");
...
async fn startup_draft_applies_editor_keymap_without_enabling_vim() {
    ...
    config.tui_keymap.editor.move_line_start = Some(kodex_config::types::KeybindingsSpec::One(
        kodex_config::types::KeybindingSpec("ctrl-z".to_string()),
    ));
    let mut pump = startup_test_pump(std::iter::empty());
    ...
    config.tui_keymap.composer.submit =
        Some(kodex_config::types::KeybindingsSpec::Many(Vec::new()));
    pump.apply_config(&config);
    assert!(!pump.submission_pending);
    assert_eq!(pump.bottom_pane.composer_text(), "draftx");
}
...
async fn startup_draft_waits_for_onboarding_before_accepting_input() {
    ...
    let kodex_home = tempfile::tempdir().expect("create an existing custom Kodex home");
    std::fs::write(kodex_home.path().join("history.jsonl"), "")
        .expect("create existing startup history");
    let system_config_path = kodex_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        kodex_home.path().join("system.toml"),
    )
    ...
    pump.initial_screen =
        if crate::startup_preflight::has_only_search_config_override(&search_override)
            && crate::startup_preflight::should_delay_startup_composer_for_first_login(
                ...
                || Ok(false),
                |name| (name == "KODEX_HOME").then(|| kodex_home.path().as_os_str().to_os_string()),
            )
        {
