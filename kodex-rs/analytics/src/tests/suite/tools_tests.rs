...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(KodexCommandExecutionEventRequest {
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
                    ...
                    ItemStartedNotification {
                        ...
                        item: {
                            ...
                            if let ThreadItem::CommandExecution { model_context, .. } = &mut item {
                                *model_context =
                                    Some(kodex_protocol::items::ModelInvocationContext {
    ...
    assert_eq!(payload[0]["event_type"], "kodex_command_execution_event");
...
async fn collaborator_tool_events_keep_response_ids_when_completion_races_sampling() {
    for response_first in [false, true] {
        ...
        assert_eq!(
            ...
            json!({
                "type": "kodex_collab_agent_tool_call_event",
                "item": "call-1",
                "origin": "response-1",
    ...
    for (sampled_ids, child_ids, expected) in [
        ...
        assert_eq!(
            payload
                ...
                .collect::<Vec<_>>(),
            vec![
                json!({"event": "kodex_command_execution_event", "type": expected, "status": "completed", "duration": 42}),
                json!({"event": "kodex_control_tool_call_event", "type": expected, "status": "completed", "duration": 42}),
            ],
        );
    }
}
...
async fn mcp_elicitation_classification_survives_turn_completion_and_preserves_call_grain() {
    ...
    let classifications = payload
        ...
        .iter()
        .filter(|event| event["event_type"] == "kodex_mcp_tool_call_event")
        .map(|event| {
            json!({
