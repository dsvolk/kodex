...
fn model_theme_files_preview_select_and_restore() {
    if std::env::var_os("KODEX_MODEL_THEME_TEST_CHILD").is_none() {
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("KODEX_MODEL_THEME_TEST_CHILD", "1")
            .env("FORCE_COLOR", "3")
            .output()
            .unwrap();
        assert!(
    ...
    for name in ["ada", "babbage", "curie", "cushman", "dali", "davinci"] {
        ...
        with_test_default_colors(colors, || {
            let params = build_theme_picker_params(Some(name), /*kodex_home*/ None, Some(120));
