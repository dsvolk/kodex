...
use kodex_app_server_protocol::RemoteControlConnectionStatus;
use kodex_app_server_protocol::RemoteControlPairingStartResponse;
use kodex_app_server_transport::app_server_control_socket_path;
use kodex_utils_home_dir::find_kodex_home;
use managed_install::managed_kodex_bin;
#[cfg(any(unix, windows))]
use managed_install::managed_kodex_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_kodex_path: PathBuf,
    pub managed_kodex_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "kodex app-server daemon lifecycle is only supported on Unix and Windows platforms"
    ))
}
...
struct Daemon {
    ...
    managed_kodex_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let kodex_home = find_kodex_home().context("failed to resolve KODEX_HOME")?;
        let socket_path = app_server_control_socket_path(kodex_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = kodex_home.as_path().join(STATE_DIR_NAME);
        let managed_kodex_bin = managed_kodex_bin(kodex_home.as_path());
        // Old CLIs must not mistake a daemon-owned installation for their backend.
        let (pid_file, update_pid_file) =
            if managed_kodex_bin.starts_with(kodex_home.as_path().join("packages/standalone")) {
    ...
    fn recovery_file(&self) -> Result<PathBuf> {
        Ok(kodex_app_server_transport::daemon_recovery_file_path(
            self.settings_file
                .parent()
                .and_then(Path::parent)
                .context("daemon settings path has no Kodex home")?,
        ))
    }
    ...
    fn current_installation(&self) -> Result<Self> {
        let managed_kodex_bin = self.current_managed_kodex_bin()?;
        let home = self
            .settings_file
            .parent()
            .and_then(Path::parent)
            .context("daemon settings path has no Kodex home")?;
        let (pid, updater) = if managed_kodex_bin.starts_with(home.join("packages/standalone")) {
            (LEGACY_PID_FILE_NAME, LEGACY_UPDATE_PID_FILE_NAME)
        } else {
            (DAEMON_PID_FILE_NAME, DAEMON_UPDATE_PID_FILE_NAME)
        };
    ...
    async fn start(&self, feature_overrides: &BTreeMap<String, bool>) -> Result<LifecycleOutput> {
        ...
        let (status, backend, pid, info) = if let Ok(info) = client::probe(&self.socket_path).await
        {
            ...
            managed.managed_kodex_bin = self.current_managed_kodex_bin()?;
            managed.ensure_managed_kodex_bin()?;
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_kodex_version(managed_kodex_bin).await?)
            } else {
                None
            };
            ...
            if !self.is_stable_standalone_release()?
                || managed_install::resolved_managed_kodex_bin(&self.current_managed_kodex_bin()?)
                    .await
                    .ok()
                    .as_deref()
                    != Some(managed_kodex_bin)
            {
            ...
            let mode = if mode == RestartMode::IfBinaryOrVersionChanged {
                let managed_identity =
                    managed_install::executable_identity(managed_kodex_bin).await?;
            ...
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    #[cfg(windows)]
                    backend::windows::ensure_detached_launch(managed_kodex_bin)?;
                    if let Err(err) = thread_recovery::discard_pending(self) {
                        eprintln!(
                    ...
                    backend
                        ...
                        .await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_kodex_bin)
                        .await?;
                    self.wait_until_ready().await?;
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_kodex_version = self
            .managed_kodex_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_kodex_version}",
            self.managed_kodex_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let managed_kodex_version = managed.managed_kodex_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled,
            remote_control_enabled: settings.remote_control_enabled,
            managed_kodex_path: managed.managed_kodex_bin,
            managed_kodex_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_kodex_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_kodex_bin));
        backend.start().await
    }
    ...
    async fn ensure_managed_updater(&self, settings: &DaemonSettings) -> Result<bool> {
        ...
        let Ok(kodex_bin) =
            managed_install::resolved_managed_kodex_bin(&self.managed_kodex_bin).await
        else {
            if !self.has_latest_selection_marker() {
                updater.stop().await?;
            }
        ...
        if !managed_install::supports_daemon_update_loop(&kodex_bin).await
            || !self.is_stable_standalone_release()?
            || !managed_install::resolved_managed_kodex_bin(&self.managed_kodex_bin)
                .await
                .is_ok_and(|selected| selected == kodex_bin)
        {
            if !self.has_latest_selection_marker() {
                updater.stop().await?;
            }
        ...
        backend::pid_update_loop_backend(self.backend_paths_with_bin(settings, &kodex_bin))
            .start()
            .await?;
        Ok(true)
    }
    ...
    fn is_stable_standalone_release(&self) -> Result<bool> {
        ...
        Ok(managed_install::is_stable_standalone_release(
            kodex_home,
            &self.current_managed_kodex_bin()?,
        ))
    }

    fn current_managed_kodex_bin(&self) -> Result<PathBuf> {
        ...
        let home = self
            ...
            .and_then(Path::parent)
            .context("daemon settings path has no Kodex home")?;
        Ok(managed_install::managed_kodex_bin(home))
    }
    ...
    async fn is_bootstrapped(&self, settings: &DaemonSettings) -> Result<bool> {
        if !settings.auto_update_enabled
            || !self.is_stable_standalone_release()?
            || !managed_install::supports_daemon_update_loop(&self.managed_kodex_bin).await
        {
    ...
    fn ensure_managed_kodex_bin(&self) -> Result<()> {
        if self.managed_kodex_bin.is_file() {
            #[cfg(windows)]
            backend::windows::ensure_detached_launch(&self.managed_kodex_bin)?;
            return Ok(());
        }

        let managed_kodex_path = self.managed_kodex_bin.display();
        Err(anyhow!(
            "daemon executable not found at {managed_kodex_path}; repair the existing installation, or run `kodex app-server daemon start` to install a missing daemon"
        ))
    }

    #[cfg(any(unix, windows))]
    async fn managed_kodex_version_best_effort(&self) -> Option<String> {
        managed_kodex_version(&self.managed_kodex_bin).await.ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_kodex_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            kodex_bin: managed_kodex_bin.to_path_buf(),
    ...
    async fn open_operation_lock_file(&self) -> Result<tokio::fs::File> {
        if let Some(parent) = self.operation_lock_file.parent() {
            ...
            kodex_uds::prepare_private_socket_directory(parent)
                .await
                .with_context(|| {
                    format!(
    ...
    async fn output(
        ...
        let managed_kodex_version = self.managed_kodex_version_best_effort().await;
        LifecycleOutput {
            status,
            backend,
            pid,
            managed_kodex_path: self.managed_kodex_bin.clone(),
            managed_kodex_version,
            socket_path: self.socket_path.clone(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_kodex_path: "kodex".into(),
            managed_kodex_version: Some("1.2.3".to_string()),
            socket_path: "kodex.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedKodexPath": "kodex",
                "managedKodexVersion": "1.2.3",
                "socketPath": "kodex.sock",
                "cliVersion": "1.2.3",
    ...
    async fn waiting_lifecycle_command_uses_migrated_installation() {
        ...
        let daemon = Daemon {
            ...
            managed_kodex_bin: super::managed_kodex_bin(home.path()),
        };
        ...
        assert_eq!(
            (output.status, output.managed_kodex_path),
            (
                LifecycleStatus::NotRunning,
                super::managed_kodex_bin(home.path())
            )
        );
    }
    ...
    async fn stop_creates_missing_home_parent() {
        ...
        let daemon = Daemon {
            ...
            operation_lock_file: state.join("daemon.lock"),
            settings_file: state.join("settings.json"),
            managed_kodex_bin: state.join("missing-kodex"),
        };
        assert_eq!(
            daemon
    ...
    async fn stop_and_fresh_start_discard_pending_thread_restore() {
        let home = TempDir::new().expect("home");
        let state = home.path().join("app-server-daemon");
        kodex_uds::prepare_private_socket_directory(&state)
            .await
        ...
        let daemon = Daemon {
            ...
            operation_lock_file: state.join("daemon.lock"),
            settings_file: state.join("settings.json"),
            managed_kodex_bin: home.path().join("kodex"),
        };
        kodex_app_server_transport::daemon_recovery::write_candidates(
    ...
    async fn managed_local_backend_counts_as_bootstrapped_without_updater() {
        ...
        let local_bin = standalone.join("local-main/bin/kodex");
        ...
        let daemon = Daemon {
            ...
            operation_lock_file: state.join("daemon.lock"),
            settings_file: state.join("settings.json"),
            managed_kodex_bin: standalone.join("current/bin/kodex"),
        };
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_kodex_bin: temp_dir.path().join("missing-kodex"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_kodex_bin.display(),
                stderr_log.display()
            )
        );
    }
