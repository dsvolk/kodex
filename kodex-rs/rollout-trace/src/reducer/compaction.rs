...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.kodex_turns.get(&started.kodex_turn_id) else {
            bail!(
                "compaction request {} referenced unknown kodex turn {}",
                started.compaction_request_id,
                started.kodex_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but kodex turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.kodex_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                kodex_turn_id: started.kodex_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.kodex_turns.get(&kodex_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown kodex turn {kodex_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but kodex turn {kodex_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) kodex_turn_id: String,
