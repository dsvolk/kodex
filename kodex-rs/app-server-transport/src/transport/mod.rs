...
pub fn daemon_recovery_file_path(kodex_home: &Path) -> PathBuf {
    kodex_home
        .join("app-server-daemon")
        .join(DAEMON_RECOVERY_FILE_NAME)
}

pub fn app_server_control_socket_path(kodex_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
pub fn app_server_startup_lock_path(kodex_home: &Path) -> std::io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(
        kodex_home
            .join(APP_SERVER_CONTROL_SOCKET_DIR_NAME)
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let kodex_home = find_kodex_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve KODEX_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&kodex_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),
