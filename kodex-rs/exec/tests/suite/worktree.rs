...
async fn worktree_start_and_fork_use_host_pool_and_preserve_legacy_resume() -> anyhow::Result<()> {
    ...
    fs::write(source.join(".kodex/config.toml"), "[invalid source config")?;
    ...
    for args in [vec!["prompt"], vec!["fork", legacy_id.as_str(), "prompt"]] {
        let request_count = if args.first() == Some(&"fork") {
            ...
            fs::remove_file(source.join(".kodex/config.toml"))?;
            fs::create_dir_all(legacy.cwd.join(".kodex"))?;
            fs::write(
                legacy.cwd.join(".kodex/config.toml"),
                "cli_auth_credentials_store=\"file\"\n",
            )?;
            git(&legacy.cwd, &["add", ".kodex/config.toml"])?;
            git(
                &legacy.cwd,
...
fn worktree_rejects_remote_defaults_before_allocation() -> anyhow::Result<()> {
    for route in ["environment", "file"] {
        ...
        if route == "environment" {
            command.env("KODEX_EXEC_SERVER_URL", "ws://127.0.0.1:9");
        } else {
            fs::write(
