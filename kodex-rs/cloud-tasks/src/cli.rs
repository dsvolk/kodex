...
pub enum Command {
    /// Submit a new Kodex Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Kodex Cloud task.
    Status(StatusCommand),
    /// List Kodex Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Kodex Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Kodex Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Kodex Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `kodex cloud` to browse).
    ...
    /// Git branch to run in Kodex Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Kodex Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Kodex Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Kodex Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
