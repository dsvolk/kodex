...
//! the configured Kodex executable as a subprocess, accepts only valid JSON from
//! `kodex doctor --json --feedback`, derives a small set of Sentry tags, and otherwise
...
use kodex_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use kodex_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `kodex-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `kodex --cd <workspace> doctor --json --feedback` and returns a best-effort
/// feedback attachment.
///
/// Failure to spawn Kodex, finish before the timeout, or parse JSON means the
...
pub(crate) async fn doctor_feedback_report(
    ...
    let mut command = doctor_command(&executable, workspace, config.kodex_home.as_path());
...
fn doctor_command(executable: &Path, cwd: &Path, kodex_home: &Path) -> Command {
...
mod tests {
    ...
    fn doctor_command_keeps_missing_workspace_out_of_process_cwd() {
        let kodex_home = tempdir().expect("kodex home");
        let workspace = kodex_home.path().join("deleted-workspace");

        let command = doctor_command(Path::new("kodex"), &workspace, kodex_home.path());
        let command = command.as_std();

        assert_eq!(command.get_current_dir(), Some(kodex_home.path()));
        assert_eq!(
        ...
        assert_eq!(
            ...
            [("KODEX_HOME".as_ref(), Some(kodex_home.path().as_os_str()))]
        );
    }
