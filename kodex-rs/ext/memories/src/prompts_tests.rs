...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let kodex_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = kodex_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&kodex_home, MemoryVersion::V1)
        .await
        .unwrap();
...
-> Result<(), Box<dyn std::error::Error>> {
    let home = tempdir()?;
    let kodex_home = AbsolutePathBuf::from_absolute_path(home.path())?;
    let v1 = kodex_home.join("memories");
    let v2 = kodex_home.join("memories_v2");
    tokio_fs::create_dir_all(&v1).await?;
    tokio_fs::write(v1.join("memory_summary.md"), "v1\nlegacy content").await?;
    assert_eq!(
        build_memory_tool_developer_instructions(&kodex_home, MemoryVersion::V2).await,
        None
    );
    tokio_fs::create_dir_all(&v2).await?;
    tokio_fs::write(v2.join("memory_summary.md"), "v1\nnew pipeline content").await?;
    let instructions = build_memory_tool_developer_instructions(&kodex_home, MemoryVersion::V2)
        .await
    ...
    assert!(
        build_memory_tool_developer_instructions(&kodex_home, MemoryVersion::V1)
            .await
