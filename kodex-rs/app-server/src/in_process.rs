...
//!
//! # Relationship to `kodex-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `kodex-app-server-client`,
...
use kodex_app_server_protocol::AgentMessageDelivery;
...
pub use kodex_rollout::StateDbHandle;
pub use kodex_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `kodex-app-server-client`, which adds worker-task buffering,
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.kodex_home).await?;
    let auth_manager =
        AuthManager::shared_from_config(args.config.as_ref(), args.enable_kodex_api_key_env)
            .await
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.kodex_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(kodex_home.path()).await);
        let state_db = kodex_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "kodex-in-process-test".to_string(),
                    title: None,
        ...
        client._test_kodex_home = Some(kodex_home);
        client
    }
