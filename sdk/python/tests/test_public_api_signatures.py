...
import openai_kodex
import openai_kodex.types as public_types
...
from openai_kodex._initialize_metadata import validate_initialize_metadata
from openai_kodex.types import InitializeResponse
...
def test_root_exports_kodex_config() -> None:
    """The root package should expose the process configuration object."""
    assert KodexConfig.__name__ == "KodexConfig"
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert openai_kodex.__version__ == pyproject["project"]["version"]
    assert KodexConfig().client_version == openai_kodex.__version__
...
def test_curated_public_api_has_builtin_help_documentation() -> None:
    ...
    documented = {
        ...
        "Sandbox": Sandbox,
        "thread_start": Kodex.thread_start,
        "thread_resume": Kodex.thread_resume,
...
def test_package_includes_py_typed_marker() -> None:
    ...
    marker = resources.files("openai_kodex").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert openai_kodex.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(openai_kodex, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "KodexClient": hasattr(openai_kodex, "KodexClient"),
        "AsyncKodexClient": hasattr(openai_kodex, "AsyncKodexClient"),
        "InitializeResponse": hasattr(openai_kodex, "InitializeResponse"),
        "ThreadStartParams": hasattr(openai_kodex, "ThreadStartParams"),
        "TurnStartParams": hasattr(openai_kodex, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(openai_kodex, "TurnCompletedNotification"),
        "TurnStatus": hasattr(openai_kodex, "TurnStatus"),
    } == {
        "KodexClient": False,
        "AsyncKodexClient": False,
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from openai_kodex import *", namespace)
...
def test_types_module_exports_curated_public_types() -> None:
    """The public type module should expose Kodex protocol models."""
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from openai_kodex.types import *", namespace)
...
def test_lifecycle_methods_are_kodex_scoped() -> None:
    """Lifecycle operations should hang off the client rather than thread objects."""
    assert hasattr(Kodex, "thread_resume")
    assert hasattr(Kodex, "thread_fork")
    assert hasattr(Kodex, "thread_archive")
    assert hasattr(Kodex, "thread_unarchive")
    assert hasattr(AsyncKodex, "thread_resume")
    assert hasattr(AsyncKodex, "thread_fork")
    assert hasattr(AsyncKodex, "thread_archive")
    assert hasattr(AsyncKodex, "thread_unarchive")
    assert not hasattr(Kodex, "thread")
    assert not hasattr(AsyncKodex, "thread")
    ...
    for fn in (
        Kodex.thread_archive,
        Kodex.thread_unarchive,
        AsyncKodex.thread_archive,
        AsyncKodex.thread_unarchive,
    ):
        _assert_no_any_annotations(fn)
...
def test_initialize_metadata_parses_user_agent_shape() -> None:
    ...
    payload = InitializeResponse.model_validate({"userAgent": "kodex-cli/1.2.3"})
    parsed = validate_initialize_metadata(payload)
    assert parsed is payload
    assert parsed.userAgent == "kodex-cli/1.2.3"
    assert parsed.serverInfo is not None
    assert parsed.serverInfo.name == "kodex-cli"
