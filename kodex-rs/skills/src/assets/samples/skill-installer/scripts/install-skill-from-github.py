#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $KODEX_HOME/skills."""
...
def _kodex_home() -> str:
    return os.environ.get("KODEX_HOME", os.path.expanduser("~/.kodex"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "kodex")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "kodex-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_kodex_home(), "skills")
