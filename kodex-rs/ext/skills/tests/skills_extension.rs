...
use kodex_extension_api::ConversationHistory;
...
use kodex_extension_api::FunctionCallError;
use kodex_extension_api::NoopTurnItemEmitter;
...
use kodex_extension_api::SkillInvocationKind;
...
use kodex_extension_api::TurnStartInput;
...
use kodex_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use kodex_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use kodex_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use kodex_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use kodex_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use kodex_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use kodex_skills_extension::HostSkillsSnapshot;
use kodex_skills_extension::InjectedHostSkillPrompts;
use kodex_skills_extension::SkillLoadOutcome;
...
use kodex_skills_extension::install_with_providers_and_metrics;
use kodex_skills_extension::provider::SkillListQuery;
use kodex_skills_extension::provider::SkillProvider;
...
static NEXT_KODEX_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn skill_world_state_fragments(
    registry: &kodex_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            ...
            thread_id: kodex_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn start_registered_turn(
    ...
    let turn_store = ExtensionData::new(turn_id);
    let mode = kodex_protocol::config_types::CollaborationMode {
        mode: kodex_protocol::config_types::ModeKind::Default,
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let kodex_home = test_kodex_home();
    let skill_path = kodex_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(kodex_home)?;
    Ok(())
}
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path = AbsolutePathBuf::try_from(test_kodex_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_kodex_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "kodex.skills.shadow_selection.catalog_entries")
...
async fn shadow_lru_selector_recovers_a_skill_invoked_on_an_earlier_turn() -> TestResult {
    ...
    let metric = snapshot
        .scope_metrics()
        .flat_map(opentelemetry_sdk::metrics::data::ScopeMetrics::metrics)
        .find(|metric| metric.name() == "kodex.skills.shadow_selection.invocation")
    ...
    for cloud_skill_enabled in [false, true] {
        ...
        assert_eq!(
            registry
                ...
                .collect::<Vec<_>>(),
            vec![
                kodex_extension_api::TurnStartPhase::BeforeTaskRegistration,
                if cloud_skill_enabled {
                    kodex_extension_api::TurnStartPhase::RegularTaskStart
                } else {
                    kodex_extension_api::TurnStartPhase::BeforeTaskRegistration
                },
            ]
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    ...
    let kodex_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(kodex_home, Vec::new(), config_layer_stack),
...
fn test_kodex_home() -> PathBuf {
    let id = NEXT_KODEX_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "kodex-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
