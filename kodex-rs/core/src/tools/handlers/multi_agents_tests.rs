...
use kodex_protocol::protocol::FileSystemAccessMode;
use kodex_protocol::protocol::FileSystemPath;
use kodex_protocol::protocol::FileSystemSandboxEntry;
use kodex_protocol::protocol::FileSystemSandboxPolicy;
use kodex_protocol::protocol::InterAgentCommunication;
use kodex_protocol::protocol::ItemCompletedEvent;
use kodex_protocol::protocol::NetworkSandboxPolicy;
...
fn invocation(
    ...
    ToolInvocation {
        ...
        tool_name: kodex_tools::ToolName::plain(tool_name),
...
async fn wait_for_recorded_user_input(thread: &crate::KodexThread, expected: &[UserInput]) {
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.kodex_home)
        .await
        .expect("kodex home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    kodex_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let mut turn = turn;
    turn.config = Arc::new(config);
    turn.multi_agent_version = kodex_protocol::protocol::MultiAgentVersion::V2;
...
fn service_tier_test_catalog() -> kodex_protocol::openai_models::ModelsResponse {
    let mut catalog = kodex_models_manager::bundled_models_response().expect("bundled models");
...
async fn spawn_agent_role_service_tier_cannot_override_root_preference() {
    ...
    tokio::fs::create_dir_all(&turn.config.kodex_home)
        .await
        .expect("kodex home should be created");
    let role_config_path = turn.config.kodex_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.5"
service_tier = "turbo"
...
async fn spawn_agent_reapplies_runtime_sandbox_after_role_config() {
    ...
    let sandbox_runtime = core_test_support::test_kodex::test_kodex()
    ...
    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        ...
        turn.config.kodex_home.to_path_buf(),
...
async fn build_agent_spawn_config_uses_captured_step_settings_and_turn_context_values(
    ...
    config.kodex_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
