//! Aggregates MCP server connections for Kodex.
//!
...
use startup::should_share_kodex_apps_tools_cache;
...
use crate::pagination::MAX_KODEX_APPS_TOOL_CATALOG_ITEMS;
...
use crate::rmcp_client::prepare_kodex_apps_tools_for_model;
...
use kodex_protocol::protocol::McpStartupCompleteEvent;
use kodex_protocol::protocol::McpStartupFailure;
...
impl McpServerView {
    async fn listed_tools(
        ...
        Ok(if self.connection.client.is_kodex_apps_mcp_server {
            prepare_kodex_apps_tools_for_model(tools, tool_plugin_context)
        } else {
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_kodex_backend())
            .map(kodex_model_provider::auth_provider_from_auth);
        let kodex_apps_auth_provider = auth_manager.as_ref().and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_kodex_backend()).map(|auth| {
                kodex_model_provider::auth_provider_from_auth_manager(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let is_host_owned_kodex_apps = registration.is_some_and(|server| {
                server
                    .source()
            ...
            let catalog_item_limit = if is_host_owned_kodex_apps {
                MAX_KODEX_APPS_TOOL_CATALOG_ITEMS
            } else {
            ...
            let protocol_mode = if matches!(
                ...
                registration
                    ...
                    .unwrap_or(if is_host_owned_kodex_apps {
            ...
            // For built-in Kodex Apps, `KODEX_CONNECTORS_TOKEN` is a debug
            ...
            let shares_kodex_apps_tools_cache = is_host_owned_kodex_apps
                && !server.requires_read_only_mcp_tools()
                && should_share_kodex_apps_tools_cache(&server_name, uses_env_bearer_token);
            let kodex_apps_tools_cache_context = shares_kodex_apps_tools_cache.then(|| {
                ...
                {
                    // mcp_server_config_for_url in kodex-rs/kodex-mcp/src/mcp/mod.rs
                ...
                kodex_apps_tools_cache
                    .context(kodex_home.clone(), kodex_apps_tools_cache_key.clone())
                    .with_live_scope(scope.to_string())
            });
            // The reserved Kodex Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == KODEX_APPS_MCP_SERVER_NAME {
                kodex_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If Kodex Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient KodexAuth provider.
            let runtime_auth_provider =
                if server_name == KODEX_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
                    chatgpt_auth_provider_for_server(&server, chatgpt_auth_provider)
                };
            if is_host_owned_kodex_apps {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                auth,
                shares_kodex_apps_tools_cache
                    .then(|| (kodex_home.clone(), kodex_apps_tools_cache_key.clone())),
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("KODEX_MCP_PROTOCOL_VERSION"))
                {
            ...
            let tool_catalog_cache_context = if server_name == KODEX_APPS_MCP_SERVER_NAME
                || server.requires_read_only_mcp_tools()
            {
                None
    ...
    /// Kodex Apps metadata may come from its existing cache; regular MCP server information is
