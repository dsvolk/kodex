...
use kodex_app_server_protocol::ThreadAttachment;
use kodex_app_server_protocol::ThreadAttachmentAddOutcome;
    ...
    let second: ThreadAttachmentAddResponse = server
        .request(|request_id| ClientRequest::ThreadAttachmentAdd {
            ...
            params: ThreadAttachmentAddParams {
                thread_id: first_thread.clone(),
                attachment_type: "pull_request".to_string(),
                identity_key: r#"["github.com","openai","kodex",456]"#.to_string(),
                payload: json!({ "url": "https://github.com/openai/kodex/pull/456" }),
            },
        })
        .await?;
    ...
    let unrelated: ThreadAttachmentAddResponse = server
        .request(|request_id| ClientRequest::ThreadAttachmentAdd {
            ...
            params: ThreadAttachmentAddParams {
                ...
                attachment_type: "pull_request".to_string(),
                identity_key: r#"["github.com","openai","kodex",789]"#.to_string(),
                payload: json!({ "url": "https://github.com/openai/kodex/pull/789" }),
            },
        })
        .await?;
    ...
    for (method, params) in [
        (
            ...
            json!({
                ...
                "identityKey": r#"["github.com","openai","kodex",123]"#,
                "payload": {}
            }),
        ),
        (
    ...
    let result = async {
        ...
        send_request(
            ...
            Some(json!({
                ...
                "attachmentType": "pull_request",
                "identityKey": r#"["github.com","openai","kodex",123]"#,
                "payload": { "url": "https://github.com/openai/kodex/pull/123" }
            })),
        )
        .await?;
