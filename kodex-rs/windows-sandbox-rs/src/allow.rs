...
mod tests {
    ...
    #[test]
    fn denies_kodex_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let kodex_dir = command_cwd.join(".kodex");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&kodex_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&kodex_dir).unwrap(),
