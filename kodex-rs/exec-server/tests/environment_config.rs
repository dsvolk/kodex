...
use kodex_exec_server::EnvironmentConfigLayer;
use kodex_exec_server::EnvironmentConfigLayerStack;
use kodex_exec_server::EnvironmentConfigReadParams;
use kodex_exec_server::EnvironmentConfigReadResponse;
...
async fn remote_environment_reads_projected_executor_config() -> anyhow::Result<()> {
    ...
    let kodex_home =
        AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(server.kodex_home())?)?;
    let config_file = kodex_home.join(CONFIG_TOML_FILE);
    let project = kodex_home.join("project");
    let dot_kodex = project.join(".kodex");
    tokio::fs::create_dir_all(dot_kodex.as_path()).await?;
    ...
    .await?;
    tokio::fs::write(
        dot_kodex.join(CONFIG_TOML_FILE).as_path(),
        r#"
[future_environment]
...
async fn environment_config_read_rejects_empty_selectors() -> anyhow::Result<()> {
    ...
    for (config_paths, expected_message) in [
        ...
        let error = environment
            .read_environment_config(EnvironmentConfigReadParams {
                cwd: PathUri::from_abs_path(&kodex_home),
                config_paths,
                requirements_paths: Vec::new(),
            })
            .await
