...
fn write_installed_marketplace(kodex_home: &std::path::Path, marketplace_name: &str) -> Result<()> {
...
async fn marketplace_remove_preserves_project_marketplace(user_entry: bool) -> Result<()> {
    ...
    // TestAppServer starts in KODEX_HOME, so make it a trusted project as well.
    std::fs::create_dir_all(kodex_home.path().join(".git"))?;
    std::fs::create_dir_all(kodex_home.path().join(".kodex"))?;
    let project_config_path = kodex_home.path().join(".kodex/config.toml");
