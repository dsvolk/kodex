...
use kodex_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "kodex-test-ttl";
const VERSIONED_MODEL: &str = "kodex-test-versioned";
const MISSING_VERSION_MODEL: &str = "kodex-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "kodex-test-different-version";
...
async fn guardian_reused_reviewer_avoids_stale_catalog_lookup() -> Result<()> {
    ...
    let bundled = kodex_models_manager::bundled_models_response()?;
    let catalog = ModelsResponse {
        models: bundled
            .models
            .into_iter()
            .filter(|model| ["gpt-5.4", "kodex-auto-review"].contains(&model.slug.as_str()))
            .collect(),
    };
    ...
    rewrite_cache_timestamp(
        &test.config.kodex_home.join(CACHE_FILE),
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.kodex_home.join(CACHE_FILE);
    ...
    kodex
        .start_or_steer_turn(
            ...
            .with_thread_settings(ThreadSettingsOverrides {
                ...
                approval_policy: Some(kodex_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&kodex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn matching_models_etag_does_not_rewrite_recent_cache() -> Result<()> {
    ...
    let cache_path = test.config.kodex_home.join(CACHE_FILE);
...
async fn uses_cache_when_version_matches() -> Result<()> {
    ...
    let mut builder = test_kodex().with_auth(KodexAuth::create_dummy_chatgpt_auth_for_testing());
    let identity = kodex_model_provider::test_support::models_cache_entry(
        &kodex_model_provider_info::ModelProviderInfo::create_openai_provider(Some(format!(
            "{}/v1",
            server.uri()
        ))),
