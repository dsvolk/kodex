...
pub async fn run_session_queue_command(
    ...
    if options.cli.no_daemon && options.explicit_remote_endpoint.is_none() {
        return Err(eyre!(
            "--no-daemon cannot be used with kodex queue. Queuing must discover the shared server to avoid writing through a separate server."
        ));
    }
    ...
    if !explicit_remote
        ...
        && super::maybe_probe_default_daemon_socket(kodex_home.as_path())
            .await
            .is_some()
    {
        return Err(eyre!(
