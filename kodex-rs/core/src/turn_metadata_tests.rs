...
use kodex_analytics::TurnAnalyticsMetadata;
...
use kodex_sandboxing::get_platform_sandbox;
...
fn test_turn_responses_metadata(
    ...
    state.to_responses_metadata(
        ...
        window_id.to_string(),
        KodexResponsesRequestKind::Turn,
    )
}
...
fn test_compaction_responses_metadata_json(
    ...
    state
        .to_responses_metadata(
            "installation-a".to_string(),
            window_id.to_string(),
            KodexResponsesRequestKind::Compaction(compaction),
        )
        .turn_metadata_json()
        .expect("turn metadata json")
}
...
async fn detached_memory_responses_metadata_starts_an_independent_root_turn() {
    ...
    let thread_manager = crate::ThreadManager::with_models_provider_for_tests(
        kodex_login::KodexAuth::from_api_key("test"),
...
async fn execution_metadata_applies_captured_settings_to_responses_metadata() {
    ...
    config.permissions.approval_policy =
        crate::config::Constrained::allow_any(kodex_protocol::protocol::AskForApproval::OnRequest);
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responses_api_metadata(BTreeMap::from([
        ("kodex_security_surface".to_string(), "sdk".to_string()),
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        (
            "kodex_security_surface".to_string(),
            "client-supplied".to_string(),
        ),
        ...
        ("kodex_version".to_string(), "client-supplied".to_string()),
        (
        ...
        (
            "x-kodex-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-kodex-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert_eq!(json["kodex_security_surface"].as_str(), Some("sdk"));
    ...
    assert!(json.get("x-kodex-installation-id").is_none());
    assert!(json.get("x-kodex-parent-thread-id").is_none());
    ...
    assert_eq!(
        model_request_json["kodex_security_surface"].as_str(),
        Some("sdk")
    );
    assert_eq!(
    ...
    let meta = state
        ...
        .expect("turn metadata should be present");
    assert_eq!(meta["model"].as_str(), Some("gpt-5.4"));
    assert_eq!(meta["kodex_version"], env!("CARGO_PKG_VERSION"));
    assert_eq!(meta["reasoning_effort"].as_str(), Some("high"));
    ...
    for (configured, client, expected) in [
        ...
        assert_eq!(state.kodex_turn_source(), expected);
    }
}
...
fn turn_metadata_state_overlays_compaction_only_on_compaction_requests() {
    ...
    assert_eq!(compact_json["kodex_security_surface"].as_str(), Some("sdk"));
    assert_eq!(
    ...
    assert_eq!(regular_json["kodex_security_surface"].as_str(), Some("sdk"));
