...
use kodex_app_server_protocol::PluginExtensions;
...
pub(crate) const KODEX_PRODUCT_SKU: &str = "kodex";
...
impl RemotePluginServiceConfig {
    ...
    pub fn new(
        ...
        Self {
            chatgpt_base_url,
            product_sku: product_sku.unwrap_or_else(|| KODEX_PRODUCT_SKU.to_string()),
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(
        kodex_home, config, auth, scope, /*collection*/ None, &plugins,
    );
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(kodex_home, config, auth, RemotePluginScope::Global).await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(
            kodex_home, config, auth, *scope, /*collection*/ None,
        );
...
pub async fn fetch_recommended_plugins(
    ...
    let mut url = Url::parse(&format!("{base_url}/ps/plugins/suggested/kodex"))
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(kodex_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(kodex_home.clone())
    ...
    let legacy_remote_plugin_cache_root = kodex_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode != RemotePluginCatalogCacheMode::ForceRefetch
        && let Some(kodex_home) = kodex_home
        && let Some(cached) = catalog_cache::load_cached_directory_plugins(
            kodex_home, config, auth, scope, collection,
        )
    ...
    if let Some(kodex_home) = kodex_home {
        catalog_cache::write_cached_directory_plugins(
            kodex_home, config, auth, scope, collection, &plugins,
        );
    }
...
fn ensure_chatgpt_auth(auth: Option<&KodexAuth>) -> Result<&KodexAuth, RemotePluginCatalogError> {
