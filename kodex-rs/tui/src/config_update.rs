...
use kodex_config::loader::find_project_root;
use kodex_config::loader::normalized_project_trust_keys;
    ...
    let service_tier_edit = service_tier.map_or_else(
        ...
        |service_tier| {
            let config_value = if service_tier == SERVICE_TIER_DEFAULT_REQUEST_VALUE {
                ...
                match kodex_protocol::config_types::ServiceTier::from_request_value(service_tier) {
                    Some(kodex_protocol::config_types::ServiceTier::Fast) => "fast",
                    Some(kodex_protocol::config_types::ServiceTier::Flex) => "flex",
                    None => service_tier,
                }
            };
    ...
    let trust_target = cwd_trust_target
        .or_else(|| {
            disabled_reason
                ...
                .or_else(|| {
                    disabled_project
                        .and_then(|layer| layer["name"]["dotKodexFolder"].as_str())
                        .and_then(|path| {
                            path.strip_suffix("/.kodex")
                                .or_else(|| path.strip_suffix("\\.kodex"))
                        })
                })
        })
        .or_else(|| {
...
pub(crate) async fn read_effective_config_if_supported(
    request_handle: AppServerRequestHandle,
    cwd: &Path,
) -> Result<Option<kodex_app_server_protocol::Config>> {
