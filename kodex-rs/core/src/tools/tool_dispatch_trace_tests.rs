...
use kodex_rollout_trace::ToolCallRequester;
...
impl kodex_code_mode::CodeModeSessionProvider for MissingCellCodeModeSessionProvider {
    fn create_session(&self) -> kodex_code_mode::CodeModeSessionProviderFuture<'_> {
        Box::pin(async {
            Ok(Arc::new(MissingCellCodeModeSession) as Arc<dyn kodex_code_mode::CodeModeSession>)
        })
    }
}
...
impl kodex_code_mode::CodeModeSession for MissingCellCodeModeSession {
    fn execute<'a>(
        &'a self,
    ...
    fn terminate<'a>(
        &'a self,
        cell_id: kodex_code_mode::CellId,
    ) -> kodex_code_mode::CodeModeSessionResultFuture<'a, kodex_code_mode::WaitOutcome> {
        Box::pin(async move {
            Ok(kodex_code_mode::WaitOutcome::MissingCell(
                kodex_code_mode::RuntimeResponse::Result {
    ...
    fn shutdown<'a>(&'a self) -> kodex_code_mode::CodeModeSessionResultFuture<'a, ()> {
        Box::pin(async { Ok(()) })
    }
}
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    let replayed = kodex_rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
    ...
    let result = registry
        .dispatch_any_with_state(
            test_invocation_with_payload(
                ...
                turn,
                "incompatible-call",
                kodex_tools::ToolName::plain("test_tool"),
                ToolCallSource::Direct,
                ToolPayload::Custom {
                    input: "{}".to_string(),
                },
...
fn test_invocation(
    ...
    test_invocation_with_payload(
        ...
        turn,
        call_id,
        kodex_tools::ToolName::plain(tool_name),
        source,
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace =
        kodex_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_kodex_turn_started(turn.sub_id.as_str());
