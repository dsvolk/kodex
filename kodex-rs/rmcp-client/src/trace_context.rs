...
pub(crate) fn with_current_trace(mut meta: Option<RequestMetaObject>) -> Option<RequestMetaObject> {
    if let Some(trace) = kodex_otel::current_span_w3c_trace_context() {
...
pub(crate) fn request_span(message: &Value) -> Span {
    ...
    let Some(context) = kodex_otel::context_from_w3c_trace_context(&trace) else {
        return Span::none();
    };
    let method = message
        .get("method")
    ...
    kodex_otel::set_parent_from_context(&span, context);
    span
}
