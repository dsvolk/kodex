...
use crate::rmcp_client::KODEX_APPS_REFRESH_DURATION_METRIC;
...
impl McpConnectionSet {
    pub(crate) async fn stable_catalog_revisions(
        ...
        for (server_name, view) in &self.servers {
            ...
            let Some(client) = view.connection.client.ready_client() else {
                if !view.connection.client.is_kodex_apps_mcp_server
    ...
    pub(crate) async fn capture_binding_with_metadata(
        ...
        let server_snapshots = join_all(self.servers.iter().map(|(server_name, view)| async move {
                ...
                .load(Ordering::Acquire)
            {
                ...
                let cached_tools = view.connection.client.cached_tools().filter(|tools| {
                    view.connection.client.is_kodex_apps_mcp_server || !tools.is_empty()
                });
                ...
                let must_wait_for_startup = (required
                    ...
                    || (self.is_selected_plugin_mcp_server(server_name)
                        && self
                            .plugin_id_for_mcp_server_name(server_name)
                            .is_some_and(|plugin_id| required_plugins.contains(plugin_id)))
                    || (server_name == KODEX_APPS_MCP_SERVER_NAME && !has_cached_tools);
        ...
        let server_results = join_all(server_snapshots.into_iter().map(|(server_name, view, cached_tools)| async move {
            ...
            let server_tools = if server_name == KODEX_APPS_MCP_SERVER_NAME {
                prepare_kodex_apps_tools_for_model(server_tools, &self.tool_plugin_context)
            } else {
    ...
    pub(crate) async fn refresh_kodex_apps_client_catalog(
        &self,
        config: &crate::McpConfig,
    ) -> Result<KodexAppsToolSnapshot> {
        let refresh_start = Instant::now();
        let view = self
            .servers
            .get(KODEX_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{KODEX_APPS_MCP_SERVER_NAME}'"))?;
        let (tools, _) = self.refresh_kodex_apps_tool_catalog().await?;
        let server_has_permission = config
            .permission_profile_for_server(KODEX_APPS_MCP_SERVER_NAME)
            .is_some();
    ...
    pub async fn refresh_kodex_apps_tools_for_discovery(&self) -> Result<Vec<ToolInfo>> {
        let refresh_start = Instant::now();
        let view = self
            .servers
            .get(KODEX_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{KODEX_APPS_MCP_SERVER_NAME}'"))?;
        let (_, tools) = self.refresh_kodex_apps_tool_catalog().await?;
        let tools = prepare_kodex_apps_tools_for_model(
    ...
    async fn refresh_kodex_apps_tool_catalog(&self) -> Result<(Vec<ToolInfo>, Vec<ToolInfo>)> {
        let view = self
            .servers
            .get(KODEX_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{KODEX_APPS_MCP_SERVER_NAME}'"))?;
        ...
        let (tools, list_start) = managed_client
            ...
            .refresh(
                || async {
                    ...
                    let fetch_ticket = managed_client.kodex_apps_tools_cache_context.as_ref().map(
                        |cache_context| {
                    ...
                    let client_tools = list_tools_for_client_uncached(
                        ...
                        /*kodex_apps_refresh_trigger*/ "explicit",
                    ...
                    .with_context(|| {
                        format!(
                            "failed to refresh tools for MCP server '{KODEX_APPS_MCP_SERVER_NAME}'"
                        )
                    })?;
                ...
                |client_tools, (fetch_ticket, list_start)| {
                    ...
                    let tools = match (
                        managed_client.kodex_apps_tools_cache_context.as_ref(),
                        fetch_ticket,
                    ) {
