...
) -> KodexResult<crate::bwrap::BwrapArgs> {
...
fn exit_with_bwrap_build_error(err: kodex_protocol::error::KodexErr) -> ! {
...
fn apply_inner_command_argv0_for_launcher(
    ...
    if supports_argv0 {
        argv.splice(
            ...
            ["--argv0".to_string(), KODEX_LINUX_SANDBOX_ARG0.to_string()],
        );
        return;
    }
...
fn preflight_proc_mount_support(options: BwrapOptions) -> KodexResult<bool> {
    let preflight_argv = build_preflight_bwrap_argv(options)?;
    let stderr = run_bwrap_in_child_capture_stderr(preflight_argv);
    Ok(!is_proc_mount_failure(stderr.as_str()))
}

fn build_preflight_bwrap_argv(options: BwrapOptions) -> KodexResult<crate::bwrap::BwrapArgs> {
