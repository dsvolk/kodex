...
fn user_layer(kodex_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: kodex_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn project_layer(dot_kodex_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/kodex"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("kodex");
    let project_folder = absolute(temp_dir.path().join("repo/.kodex"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.kodex"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_kodex_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_kodex = workspace.join(".kodex");
    let skill_root = dot_kodex.join("skills");
    fs::create_dir_all(&workspace).expect("create workspace");
    let skill_path = write_skill(&skill_root, "local", "local-skill");
    let config_stack = stack(vec![project_layer(&dot_kodex)]);
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_kodex = repository.join(".kodex");
    let skill_root = dot_kodex.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let kodex_home = home_folder.join("kodex");
    let system_folder = absolute(temp_dir.path().join("etc/kodex"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_kodex = repository.join(".kodex");
    let nested_project_dot_kodex = repository.join("nested/.kodex");
    let user_skills = kodex_home.join("skills");
    let root_project_skill = write_skill(
        &project_dot_kodex.join("skills"),
        "root-duplicate",
        "duplicate-skill",
    );
    let nested_project_skill = write_skill(
        &nested_project_dot_kodex.join("skills"),
        "nested-duplicate",
        "duplicate-skill",
    );
    ...
    let system_skill = write_skill(&kodex_home.join("skills/.system"), "system", "system-skill");
    ...
    let config_stack = stack(vec![
        ...
        user_layer(&kodex_home),
        project_layer(&project_dot_kodex),
        project_layer(&nested_project_dot_kodex),
    ]);
