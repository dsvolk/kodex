...
struct CloudTurnProvider {
    ...
    lists: AtomicUsize,
    reads: AtomicUsize,
    resource_key: Mutex<Option<kodex_mcp::McpResourceServerCacheKey>>,
}
    ...
    fn list(&self, query: SkillListQuery) -> SkillProviderFuture<'_, SkillCatalog> {
        Box::pin(async move {
            ...
            *self.resource_key.lock().expect("resource key lock") = query
                .mcp_resources
                .as_ref()
                .and_then(|client| client.server_cache_key(KODEX_APPS_MCP_SERVER_NAME));
    ...
    for (turn, next_reply, expected_skill, new_account, expected_lists, expected_reads) in [
        ...
        *provider.reply.lock().expect("cloud reply lock") = next_reply;
        let previous_connection_scope = if matches!(
            ...
            let auth = client.auth_cache_key_for_server(KODEX_APPS_MCP_SERVER_NAME);
            let connection = client.server_cache_key(KODEX_APPS_MCP_SERVER_NAME);
        ...
        if let Some(account) = new_account {
            let client = resources
                ...
                .clone()
                .expect("thread MCP resource client");
            let previous_auth = client.auth_cache_key_for_server(KODEX_APPS_MCP_SERVER_NAME);
            ...
            tokio::time::timeout(Duration::from_secs(10), async {
                while client.auth_cache_key_for_server(KODEX_APPS_MCP_SERVER_NAME) == previous_auth
                {
        ...
        let client = resources
            ...
            .expect("thread MCP resource client");
        assert!(
            client.server_cache_key(KODEX_APPS_MCP_SERVER_NAME)
        ...
        if let Some((client, scope, connection)) = previous_connection_scope {
            assert!(client.server_cache_key(KODEX_APPS_MCP_SERVER_NAME) != connection);
            assert!(
                client.auth_cache_key_for_server(KODEX_APPS_MCP_SERVER_NAME) == scope,
