...
use kodex_backend_client::ConfigBundleResponse;
use kodex_backend_client::DeliveredTomlFragment;
use kodex_config::AbsolutePathBuf;
use kodex_config::CloudConfigFragment;
use kodex_config::CloudConfigTomlBundle;
use kodex_config::CloudRequirementsFragment;
use kodex_config::CloudRequirementsTomlBundle;
...
use kodex_login::auth::ExternalAuth;
use kodex_login::auth::ExternalAuthRefreshContext;
...
fn write_auth_json(kodex_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(kodex_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(kodex_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(kodex_home, "/"))
}
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(KodexAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &KodexAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &KodexAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
async fn invalid_cloud_provider_does_not_replace_cached_bundle() {
    let kodex_home = tempdir().expect("tempdir");
    let cache = create_test_cache(kodex_home.path());
...
async fn get_bundle_without_cache_fails_closed_on_request_failure() {
    let kodex_home = tempdir().expect("tempdir");
    create_test_cache(kodex_home.path())
        .save(
            Some("user-12345".to_string()),
            Some("account-12345".to_string()),
            test_bundle(),
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = KodexAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = KodexAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
async fn production_loader_recovers_startup_timeouts_in_background() {
    ...
    assert_eq!(
        ...
        kodex_protocol::protocol::AskForApproval::Never
    );
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(kodex_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            kodex_backend_client::DeliveredRequirementsToml {
