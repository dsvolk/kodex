...
pub enum ExecutedToolCallArguments {
    ...
    Truncated {
        #[serde(rename = "_kodex_executed_tool_call_truncated")]
        truncation: ExecutedToolCallTruncation,
    },
}

/// A model-attempted Kodex tool invocation captured at the shared runtime boundary.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, JsonSchema, TS)]
...
impl ExecutedToolCall {
    ...
    pub fn new(name: String, arguments: serde_json::Value) -> Self {
        let arguments = if arguments
            .as_object()
            .is_some_and(|object| object.contains_key("_kodex_executed_tool_call_truncated"))
        {
            serde_json::json!({ "_kodex_executed_tool_call_raw": arguments })
        } else {
            arguments
        };
        Self {
