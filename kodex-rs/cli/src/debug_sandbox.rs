...
#[cfg(target_os = "macos")]
use kodex_core::spawn::CODEX_SANDBOX_ENV_VAR;
use kodex_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use kodex_core::windows_sandbox::local_binding_policy_for_sandbox;
use kodex_core::windows_sandbox::managed_proxy_routing_for_windows_sandbox;
...
use kodex_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use kodex_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use kodex_sandboxing::seatbelt::create_seatbelt_command_args;
use kodex_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _kodex_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<kodex_mcp::SandboxState>)
        .transpose()
    ...
    let kodex_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .kodex_linux_sandbox_exe
                .clone()
                .or(kodex_linux_sandbox_exe)
        }
        None => kodex_linux_sandbox_exe,
    };
    ...
    if let SandboxType::Windows = sandbox_type {
        ...
        {
            if config.permissions.windows_sandbox_type != kodex_sandboxing::SandboxType::WindowsMxc
            {
                let workspace_roots = config
                    .effective_workspace_roots()
                    .iter()
                    .map(kodex_utils_path_uri::PathUri::to_abs_path)
    ...
    let network = network_proxy
        .as_ref()
        .map(kodex_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            // for the unsandboxed shell that resumes when Kodex exits.
        ...
        SandboxType::Landlock => {
            ...
            let kodex_linux_sandbox_exe = config
                .kodex_linux_sandbox_exe
                .expect("kodex-linux-sandbox executable not found");
        ...
        #[cfg(target_os = "windows")]
        SandboxType::Windows => {
            use kodex_sandboxing::SandboxCommand;
            use kodex_sandboxing::SandboxManager;
            use kodex_sandboxing::SandboxTransformRequest;
            ...
            let kodex_self_exe = std::env::current_exe()?;
            let request = SandboxManager::new().transform(SandboxTransformRequest {
                ...
                sandbox: kodex_sandboxing::SandboxType::WindowsMxc,
                enforce_managed_network,
                environment_id: None,
                network: network.as_ref(),
                sandbox_policy_cwd: &sandbox_policy_cwd,
                sandbox_exe: Some(kodex_self_exe.as_path()),
                use_legacy_landlock: false,
                windows_sandbox_level: kodex_protocol::config_types::WindowsSandboxLevel::Disabled,
            })?;
...
async fn run_command_under_windows_session(
    ...
    use kodex_windows_sandbox::WindowsSandboxProxySettingsMode;
    use kodex_windows_sandbox::WindowsSandboxSessionRequest;
    use kodex_windows_sandbox::resolve_windows_deny_read_paths;
    use kodex_windows_sandbox::spawn_windows_sandbox_session_for_level;
    ...
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        ...
        kodex_home: config.kodex_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = kodex_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_kodex_home(
        cli_overrides,
        kodex_linux_sandbox_exe,
        options,
        /*kodex_home*/ None,
...
async fn load_debug_sandbox_config_with_kodex_home(
    cli_overrides: Vec<(String, TomlValue)>,
    kodex_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `kodex sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            kodex_linux_sandbox_exe: kodex_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        kodex_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(kodex_home) = kodex_home {
        builder = builder
            .kodex_home(kodex_home.clone())
            .fallback_cwd(Some(kodex_home));
    }
    builder.build().await
}
...
"#;
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        write_permissions_profile_config(&kodex_home, &docs, &private)?;
        let kodex_home_path = kodex_home.path().to_path_buf();

        let profile_config = build_debug_sandbox_config(
            Vec::new(),
            ConfigOverrides::default(),
            Some(kodex_home_path.clone()),
        ...
        let config = load_debug_sandbox_config_with_kodex_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = kodex_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_kodex_home(
            ...
            Some(kodex_home.path().to_path_buf()),
            CloudConfigBundleLoader::default(),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = kodex_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
